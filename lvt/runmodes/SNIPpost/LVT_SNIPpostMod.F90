...
! 25 Jul 2025  Eric Kemp  SNIP version
!
! DESCRIPTION:
! Source code for reading SNIP netCDF file, writing back out in GRIB,
! interpolating to predefined Air Force grids, and outputting those grids
! in GRIB1 files.
!--------------------------------------------------------------------------
...
module LVT_SNIPpostMod
  ...
  type, public :: LVT_SNIPpost_t
     private
     character(LVT_CONST_PATH_LEN) :: input_nc_file
     integer :: nc
     integer :: nr
...
contains
  ...
  ! Constructor for LVT_SNIPpost_t object
  subroutine new(this)
    ...
    if (.not. file_exists) then
       write(LVT_logunit,*)'[ERR] Cannot find file ', &
  ...
  ! Destructor of LVT_SNIPpost_t object
  ...
  ! Method for reading SNIP netCDF file.
  subroutine read_snip_ncfile(this)
...
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
    write(LVT_logunit,*)'[INFO] Reading SNIP file ', &
         trim(this%input_nc_file)
    ...
    if (trim(this%map_projection) .ne. 'EQUIDISTANT CYLINDRICAL') then
       write(LVT_logunit,*) &
            '[ERR] Unrecognized map projection found in SNIP file!'
...
#else
    write(LVT_logunit,*) &
         '[ERR] Must compile LVT with netCDF support for SNIPpost mode!'
    write(LVT_logunit,*) &
         '[ERR] LVT will exit gracefully.'
    call LVT_endrun()
#endif

  end subroutine read_snip_ncfile

  ! Output SNIP data in GRIB2 format.
  ...
  ! Interpolate and output SNIP data in GRIB1 format.
  subroutine interp_and_output_grib1(this, gridID)
    ...
    if (write_fullgrib_file) then
       ...
       if (griddesco(1) == 0) then
          ...
          ! If using Air Force polar stereographic, we must flip the
          ! grid so the origin is in the upper-left corner instead of
          ! lower-left
          do r = 1, nr_out
             do c = 1, nc_out
       ...
       if (griddesco(1) == 0) then
          ...
          call neighbor_interp(griddesco, li, gi, lo_neighbor, &
               go_neighbor, &
               (this%nc*this%nr), (nc_out*nr_out), &
               rlat_neighbor, rlon_neighbor, &
               n11_neighbor, &
       ...
       do r = 1, this%nr
          do c = 1, this%nc
             if (this%icecon(c,r) < 0) then
                ...
                li(c + (r-1)*this%nc) = .true.
                gi(c + (r-1)*this%nc) = &
                     this%icecon(c,r)*100 ! GRIB1 is in %
             end if
          end do ! c
       end do ! r
       if (griddesco(1) == 0) then
  ...
  subroutine build_filename_g2(output_dir, yyyymmddhh, filename)
    ...
    character(LVT_CONST_PATH_LEN), intent(out) :: filename
  ...
  subroutine write_grib2(ftn, griddesco, &
    ...
    if (griddesco(1) == 0) then
       ...
       call LVT_grib_set(igrib, 'scaleFactorOfRadiusOfSphericalEarth', &
            10.)
  ...
  subroutine write_netcdf_latlon(griddesco, nc_out, nr_out, go)
    ...
    ! Local variables
    character(LVT_CONST_PATH_LEN) :: outfilename
  ...
  subroutine bilinear_interp_input_usaf(gridDesci, gridDesco, npts, &
    ...
    call compute_grid_coord(gridDesci, mo, fill, xpts, ypts, rlon, &
         rlat, nv)
    do n=1,mo
       xi=xpts(n)
       yi=ypts(n)
