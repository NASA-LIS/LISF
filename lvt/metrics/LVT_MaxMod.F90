                   ...
                   if(metric%computeADC.eq.1) then
                      do l=1,LVT_rc%nadc
                         if(stats%max(m)%count_model_value_adc(t,k,l).gt.&
                            ...
                            stats%max(m)%model_value_adc(t,k,l) = LVT_rc%udef
                         endif
                      enddo
                   endif
                enddo
                      ...
                      if(obs%selectNlevs.ge.1) then
                         do l=1,LVT_rc%strat_nlevels
                            if(stats%max(m)%count_obs_value_total(t,k,l).gt.&
                               ...
                               stats%max(m)%obs_value_total(t,k,l) = LVT_rc%udef
                            endif
                         enddo
                         if(metric%computeSC.eq.1) then
                            do l=1,LVT_rc%nasc
                               if(stats%max(m)%count_obs_value_asc(t,k,l).gt.&
                                  ...
                                  stats%max(m)%obs_value_asc(t,k,l) = LVT_rc%udef
                               endif
                            enddo
                         endif
                         ...
                         if(metric%computeADC.eq.1) then
                            do l=1,LVT_rc%nadc
                               if(stats%max(m)%count_obs_value_adc(t,k,l).gt.&
                                  ...
                                  stats%max(m)%obs_value_adc(t,k,l) = LVT_rc%udef
                               endif
                            enddo
                         endif
                      endif
