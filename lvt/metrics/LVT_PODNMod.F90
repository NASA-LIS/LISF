...
!
!  !DESCRIPTION: 
!   This module handles the Probability of Detection of No Events (PODN)
...
!   and stratified to external dataspodn. 
!
!  !REVISION HISTORY: 
...
!
! !INTERFACE: 
  subroutine LVT_initPODN(selectNlevs,stats,metric)
! 
! !USES:   
!
! !INPUT PARAMETERS: 
...
!EOP
    ...
    allocate(stats%podn(LVT_rc%nensem))

    do m=1,LVT_rc%nensem       
       if(metric%selectOpt.eq.1) then 
          allocate(stats%podn(m)%value_total(LVT_rc%ngrid, selectNlevs(1),&
               LVT_rc%strat_nlevels))
          stats%podn(m)%value_total = 0.0
          allocate(stats%podn(m)%value_total_b(LVT_rc%ngrid, selectNlevs(1),&
               LVT_rc%strat_nlevels))       
          allocate(stats%podn(m)%value_total_d(LVT_rc%ngrid, selectNlevs(1),&
               LVT_rc%strat_nlevels))       
          allocate(stats%podn(m)%count_value_total(LVT_rc%ngrid,selectNlevs(1),&
               LVT_rc%strat_nlevels))

          stats%podn(m)%value_total_b = 0
          stats%podn(m)%value_total_d = 0
          stats%podn(m)%count_value_total = 0 

          allocate(stats%podn(m)%value_ci(selectNlevs(1),LVT_rc%strat_nlevels))
          stats%podn(m)%value_ci = LVT_rc%udef
       endif

       if(metric%timeopt.eq.1) then 
          allocate(stats%podn(m)%value_ts(LVT_rc%ngrid, selectNlevs(1), &
               LVT_rc%strat_nlevels))
          stats%podn(m)%value_ts = 0.0
          allocate(stats%podn(m)%value_ts_b(LVT_rc%ngrid, selectNlevs(1),&
               LVT_rc%strat_nlevels))       
          allocate(stats%podn(m)%value_ts_d(LVT_rc%ngrid, selectNlevs(1),&
               LVT_rc%strat_nlevels))
          allocate(stats%podn(m)%count_value_ts(LVT_rc%ngrid,selectNlevs(1),&
               LVT_rc%strat_nlevels))       
          stats%podn(m)%value_ts_b = 0
          stats%podn(m)%value_ts_d = 0
          stats%podn(m)%count_value_ts = 0 

          allocate(stats%podn(m)%tavg_value_ts(LVT_rc%ngrid, selectNlevs(1), &
               LVT_rc%strat_nlevels))
          stats%podn(m)%tavg_value_ts = 0.0
          allocate(stats%podn(m)%tavg_count_value_ts(LVT_rc%ngrid,selectNlevs(1),&
               LVT_rc%strat_nlevels))       
          stats%podn(m)%tavg_count_value_ts = 0 

          if(metric%computeSC.eq.1) then 
             allocate(stats%podn(m)%value_asc(LVT_rc%ngrid, selectNlevs(1),LVT_rc%nasc))
             stats%podn(m)%value_asc = 0.0
             allocate(stats%podn(m)%count_value_asc(LVT_rc%ngrid, selectNlevs(1),&
                  LVT_rc%nasc))
             stats%podn(m)%count_value_asc = 0
          endif
          if(metric%computeADC.eq.1) then 
             allocate(stats%podn(m)%value_adc(LVT_rc%ngrid, selectNlevs(1),LVT_rc%nadc))
             stats%podn(m)%value_adc = 0.0
             allocate(stats%podn(m)%count_value_adc(LVT_rc%ngrid, selectNlevs(1),&
                  LVT_rc%nadc))
             stats%podn(m)%count_value_adc = 0
          endif
       endif
    enddo
!-------------------------------------------------------------------------
                   ...
                   if(obs%count(t,m,o_k).ne.0.and. &
                      ...
                      if(metric%selectOpt.eq.1) then
                         ...
                         if((model%value(t,m,m_k).gt.metric%minthreshold.and.&
                            ...
                            stats%podn(m)%value_total_b(t,k,1) = &
                                 stats%podn(m)%value_total_b(t,k,1)+1
                         endif
                         ...
                         if(.not.(model%value(t,m,m_k).gt.metric%minthreshold.and.&
                            ...
                            stats%podn(m)%value_total_d(t,k,1) = &
                                 stats%podn(m)%value_total_d(t,k,1)+1
                         endif

                         stats%podn(m)%count_value_total(t,k,1) = &
                              stats%podn(m)%count_value_total(t,k,1) +1
                         ...
                         if(LVT_rc%strat_nlevels.gt.1) then 
                            if(LVT_stats%strat_var(t,m,k).gt.&
                               ...
                               if((model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                    ...
                                    .not.(obs%value(t,m,o_k).gt.metric%minthreshold.and.&
                                    obs%value(t,m,o_k).lt.metric%maxthreshold)) then 
                                  stats%podn(m)%value_total_b(t,k,2) = &
                                       stats%podn(m)%value_total_b(t,k,2)+1
                               endif
                               ...
                               if(.not.(model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                  ...
                                  stats%podn(m)%value_total_d(t,k,2) = &
                                       stats%podn(m)%value_total_d(t,k,2)+1

                               endif
                               stats%podn(m)%count_value_total(t,k,2) = &
                                    stats%podn(m)%count_value_total(t,k,2) +1
                            ...
                            elseif(LVT_stats%strat_var(t,m,k).le.&
                               ...
                               if((model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                    ...
                                    .not.(obs%value(t,m,o_k).gt.metric%minthreshold.and.&
                                    obs%value(t,m,o_k).lt.metric%maxthreshold)) then 
                                  stats%podn(m)%value_total_b(t,k,3) = &
                                       stats%podn(m)%value_total_b(t,k,3)+1
                               endif
                               ...
                               if(.not.(model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                  ...
                                  stats%podn(m)%value_total_d(t,k,3) = &
                                       stats%podn(m)%value_total_d(t,k,3)+1
                               endif

                               stats%podn(m)%count_value_total(t,k,3) = &
                                    stats%podn(m)%count_value_total(t,k,3) +1
                            endif
                         endif
                      endif
                      ...
                      if(metric%timeOpt.eq.1) then 
                         ...
                         if((model%value(t,m,m_k).gt.metric%minthreshold.and.&
                              ...
                              .not.(obs%value(t,m,o_k).gt.metric%minthreshold.and.&
                              obs%value(t,m,o_k).lt.metric%maxthreshold)) then 
                            stats%podn(m)%value_ts_b(t,k,1) = stats%podn(m)%value_ts_b(t,k,1)+1
                         endif
                         ...
                         if(.not.(model%value(t,m,m_k).gt.metric%minthreshold.and.&
                              ...
                              .not.(obs%value(t,m,o_k).gt.metric%minthreshold.and.&
                              obs%value(t,m,o_k).lt.metric%maxthreshold)) then 
                            stats%podn(m)%value_ts_d(t,k,1) = stats%podn(m)%value_ts_d(t,k,1)+1
                         endif

                         stats%podn(m)%count_value_ts(t,k,1) = &
                              stats%podn(m)%count_value_ts(t,k,1) +1
                         ...
                         if(LVT_rc%strat_nlevels.gt.1) then 
                            if(LVT_stats%strat_var(t,m,k).gt.&
                               ...
                               if((model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                  ...
                                  stats%podn(m)%value_ts_b(t,k,2) = &
                                       stats%podn(m)%value_ts_b(t,k,2)+1
                               endif
                               ...
                               if(.not.(model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                    ...
                                    .not.(obs%value(t,m,o_k).gt.metric%minthreshold.and.&
                                    obs%value(t,m,o_k).lt.metric%maxthreshold)) then 
                                  stats%podn(m)%value_ts_d(t,k,2) = &
                                       stats%podn(m)%value_ts_d(t,k,2)+1
                               endif

                               stats%podn(m)%count_value_ts(t,k,2) = &
                                    stats%podn(m)%count_value_ts(t,k,2) +1
                            ...
                            elseif(LVT_stats%strat_var(t,m,k).le.&
                               ...
                               if((model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                    ...
                                    .not.(obs%value(t,m,o_k).gt.metric%minthreshold.and.&
                                    obs%value(t,m,o_k).lt.metric%maxthreshold)) then 
                                  stats%podn(m)%value_ts_b(t,k,3) = &
                                       stats%podn(m)%value_ts_b(t,k,3)+1
                               endif
                               ...
                               if((model%value(t,m,m_k).gt.metric%minthreshold.and.&
                                    ...
                                    obs%value(t,m,o_k).lt.metric%maxthreshold)) then 
                                  stats%podn(m)%value_ts_d(t,k,3) = &
                                       stats%podn(m)%value_ts_d(t,k,3)+1
                               endif

                               stats%podn(m)%count_value_ts(t,k,3) = &
                                    stats%podn(m)%count_value_ts(t,k,3) +1
                            endif
                         endif
                      endif
                   endif
                ...
                if(LVT_rc%lvt_wopt.eq."2d ensemble gridspace") then 
                   do m=1,LVT_rc%nensem
                      do i=1,LVT_rc%ntslocs
                         write(LVT_metrics%podn%ftn_ts_loc(i,m),200,advance='no') &
                   ...
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%podn%ftn_ts_loc(i,1),200,advance='no') &
                           LVT_rc%yr, '',LVT_rc%mo, '', LVT_rc%da, '', &
                           LVT_rc%hr,'',LVT_rc%mn, '' 
                   enddo
                endif
          ...
          if(alarm) then 
             if(LVT_metrics%podn%timeOpt.eq.1.and.&
                ...
                if(LVT_rc%lvt_wopt.eq."2d ensemble gridspace") then 
                   do m=1,LVT_rc%nensem
                      do i=1,LVT_rc%ntslocs
                         write(LVT_metrics%podn%ftn_ts_loc(i,m),fmt='(a1)') ''
                      enddo
                   enddo
                else
                   do i=1,LVT_rc%ntslocs
                      write(LVT_metrics%podn%ftn_ts_loc(i,1),fmt='(a1)') ''
                   enddo
                endif
             ...
             do m=1,LVT_rc%nensem
                do k=1,model%selectNlevs
                   do l=1,LVT_rc%strat_nlevels
                      if((stats%podn(m)%value_ts_b(t,k,l) + & 
                           stats%podn(m)%value_ts_d(t,k,l)).ne.0) then 
                         
                         PODN = stats%podn(m)%value_ts_d(t,k,l) / &
                              (stats%podn(m)%value_ts_b(t,k,l) + &
                              stats%podn(m)%value_ts_d(t,k,l))
                         
                         stats%podn(m)%value_ts(t,k,l) = PODN
                         
                         stats%podn(m)%tavg_value_ts(t,k,l) = & 
                              stats%podn(m)%tavg_value_ts(t,k,l) + & 
                              stats%podn(m)%value_ts(t,k,l)
                         stats%podn(m)%tavg_count_value_ts(t,k,l) = & 
                              stats%podn(m)%tavg_count_value_ts(t,k,l) + 1
                      else
                         stats%podn(m)%value_ts(t,k,l) = LVT_rc%udef
                      endif
                   enddo

                   if(metric%computeSC.eq.1) then 
                      if(stats%podn(m)%value_ts(t,k,1).ne.LVT_rc%udef) then 
                         call LVT_getSeasonalCycleTimeIndex(LVT_rc%scInterval,&
                              tind)
                         stats%podn(m)%value_asc(t,k,tind) = stats%podn(m)%value_asc(t,k,tind) + &
                              stats%podn(m)%value_ts(t,k,1)
                         stats%podn(m)%count_value_asc(t,k,tind) = stats%podn(m)%count_value_asc(t,k,tind) + 1
                      endif
                   endif
                   if(metric%computeADC.eq.1) then 
                      if(stats%podn(m)%value_ts(t,k,1).ne.LVT_rc%udef) then 
                         call LVT_getADCTimeIndex(tind)
                         stats%podn(m)%value_adc(t,k,tind) = stats%podn(m)%value_adc(t,k,tind) + &
                              stats%podn(m)%value_ts(t,k,1)
                         stats%podn(m)%count_value_adc(t,k,tind) = stats%podn(m)%count_value_adc(t,k,tind) + 1
                      endif
                   endif
                enddo
             enddo
          ...
          if(alarm) then 
             do t=1,LVT_rc%ngrid
                do m=1,LVT_rc%nensem
                   do k=1,model%selectNlevs
                      do l=1,LVT_rc%strat_nlevels
                         if(stats%podn(m)%tavg_count_value_ts(t,k,l).gt.0) then 
                            stats%podn(m)%tavg_value_ts(t,k,l) = &      
                                 stats%podn(m)%tavg_value_ts(t,k,l)/&
                                 stats%podn(m)%tavg_count_value_ts(t,k,l)
                         else
                            stats%podn(m)%tavg_value_ts(t,k,l) = LVT_rc%udef
                         endif
                      enddo
                   enddo
                enddo
                ...
                if(LVT_rc%lvt_wopt.eq."2d ensemble gridspace") then 
                   do m=1,LVT_rc%nensem
                      call LVT_writeTSinfo(metric%ftn_ts_loc(:,m),&
                           model,&
                           LVT_rc%ngrid,&
                           stats%podn(m)%tavg_value_ts,&
                           stats%podn(m)%tavg_count_value_ts)
                   enddo
                else
                   ...
                   call LVT_writeTSinfo(metric%ftn_ts_loc(:,1),&
                        ...
                        LVT_rc%ngrid,&
                        tavg_value_ts,&
                        stats%podn(1)%tavg_count_value_ts)
       ...
       if(stats%selectOpt.eq.1.and.obs%selectNlevs.ge.1) then 
          do t=1,LVT_rc%ngrid
             do m=1,LVT_rc%nensem
                do k=1,model%selectNlevs
                   do l=1,LVT_rc%strat_nlevels
                      if((stats%podn(m)%count_value_total(t,k,l).ne.0.and.&
                           stats%podn(m)%count_value_total(t,k,l)&
                           .gt.LVT_rc%obsCountThreshold).and.&
                           (stats%podn(m)%value_total_b(t,k,l)+&
                            stats%podn(m)%value_total_d(t,k,l)).ne.0) then 

                         PODN = stats%podn(m)%value_total_d(t,k,l) / &
                              (stats%podn(m)%value_total_b(t,k,l)+&
                               stats%podn(m)%value_total_d(t,k,l))
                         stats%podn(m)%value_total(t,k,l) = PODN
                      else
                         stats%podn(m)%value_total(t,k,l) = LVT_rc%udef
                      endif
                   enddo
                   if(metric%computeSC.eq.1) then 
                      do l=1,LVT_rc%nasc
                         if(stats%podn(m)%count_value_asc(t,k,l).gt.&
                              LVT_rc%SCCountThreshold) then
                            stats%podn(m)%value_asc(t,k,l) = stats%podn(m)%value_asc(t,k,l)/&
                                 stats%podn(m)%count_value_asc(t,k,l)
                         else
                            stats%podn(m)%value_asc(t,k,l) = LVT_rc%udef
                         endif
                      enddo
                   endif
                   if(metric%computeADC.eq.1) then 
                      do l=1,LVT_rc%nadc
                         if(stats%podn(m)%count_value_adc(t,k,l).gt.&
                              LVT_rc%ADCCountThreshold) then
                            stats%podn(m)%value_adc(t,k,l) = stats%podn(m)%value_adc(t,k,l)/&
                                 stats%podn(m)%count_value_adc(t,k,l)
                         else
                            stats%podn(m)%value_adc(t,k,l) = LVT_rc%udef
                         endif
                      enddo
                   endif
                enddo
          ...
          do m=1,LVT_rc%nensem
             do k=1,model%selectNlevs
                do l=1, LVT_rc%strat_nlevels
                   call LVT_computeCI(stats%podn(m)%value_total(:,k,l),LVT_rc%ngrid,&
                        LVT_rc%pval_CI,stats%podn(m)%value_ci(k,l))
                enddo
             enddo
          enddo
!----------------------------------------------------------------------
                ...
                do t=1,LVT_rc%ngrid                      
                   do k=1,model%selectNlevs                
                      do l=1, LVT_rc%strat_nlevels
                         value_avg(t,k,l) = &
                              value_avg(t,k,l) + & 
                              stats%podn(m)%value_total(t,k,l)
                      enddo
                   enddo
                enddo
             enddo
          ...
          if(metric%computeSC.eq.1) then
             ...
             do t=1,LVT_rc%ngrid
                do k=1,model%selectNlevs                
                   do l=1, LVT_rc%nasc
                      do m=1,LVT_rc%nensem        
                         if(stats%podn(m)%value_asc(t,k,l).ne.LVT_rc%udef) then 
                            value_asc(t,k,l) = &
                                 value_asc(t,k,l) + & 
                                 stats%podn(m)%value_asc(t,k,l)
...
! !DESCRIPTION: 
! 
!   This routine writes the computed PODN values to 
!   an external file.  
!
! !REVISION HISTORY: 
! 
          ...
          if(stats%selectOpt.eq.1) then 
             ...
             do k=1,vlevels
                do l=1,LVT_rc%strat_nlevels
                   do m=1,LVT_rc%nensem
                      value_ts(:,m) = &
                           stats%podn(m)%tavg_value_ts(:,k,l)
                      count_value_ts(:,m) = & 
                           stats%podn(m)%tavg_count_value_ts(:,k,l)
                   enddo
                   if(LVT_metrics%podn%timeOpt.eq.1) then 

                      call LVT_writevar_gridded(LVT_metrics%podn%ftn_ts, &
                           value_ts(:,:),&
                           stats%vid_ts(LVT_PODNid,1),k)

                      call LVT_writevar_gridded(LVT_metrics%podn%ftn_ts, &
                           real(count_value_ts(:,:)),&
                           stats%vid_count_ts(LVT_PODNid,1),k)
                   endif
             ...
             if(stats%selectOpt.eq.1) then
                ...
                do k=1,vlevels
                   do l=1,LVT_rc%strat_nlevels
                      do m=1,LVT_rc%nensem
                         value_total(:,m) = &
                              stats%podn(m)%value_total(:,k,l)
                         count_value_total(:,m) = & 
                              stats%podn(m)%count_value_total(:,k,l)
                         value_ci(m) = stats%podn(m)%value_ci(k,l)

                         if(LVT_metrics%podn%computeSC.eq.1) then 
                            do tind = 1,LVT_rc%nasc
                               value_asc(:,m,tind) = & 
                                    stats%podn(m)%value_asc(:,k,tind)
                            enddo
                         endif
                      ...
                      if(LVT_metrics%podn%selectOpt.eq.1) then 
                         call LVT_writevar_gridded(LVT_metrics%podn%ftn_total, &
                              value_total(:,:),&
                              stats%vid_total(LVT_PODNid,1),k)
                         call LVT_writevar_gridded(LVT_metrics%podn%ftn_total, &
                              real(count_value_total(:,:)),&
                              stats%vid_count_total(LVT_PODNid,1),k)

                         if(LVT_metrics%podn%computeSC.eq.1) then 
                            do tind = 1,LVT_rc%nasc
                               call LVT_writevar_gridded(&
                                    LVT_metrics%podn%ftn_total,&
                                    value_asc(:,:,tind),&
                                    stats%vid_sc_total(tind,LVT_PODNid,1),k)
                            enddo
                         endif
                         if(LVT_metrics%podn%computeADC.eq.1) then 
                            do tind = 1,LVT_rc%nadc
                               call LVT_writevar_gridded(&
                                    LVT_metrics%podn%ftn_total,&
                                    value_adc(:,:,tind),&
                                    stats%vid_adc_total(tind,LVT_PODNid,1),k)
                            enddo
                         endif
...
! !DESCRIPTION: 
! 
!  This routine respodn required variables to support the
!  temporal computation of PODN values. 
!
! !REVISION HISTORY: 
! 
!EOP
       ...
       if(stats%selectOpt.eq.1) then
          do m=1,LVT_rc%nensem         
             do k=1,model%selectNlevs
                if(LVT_metrics%podn%timeOpt.eq.1) then 
                   do l=1,LVT_rc%strat_nlevels
                      stats%podn(m)%value_ts(:,k,l) = 0.0
                      stats%podn(m)%value_ts_b(:,k,l) = 0.0
                      stats%podn(m)%value_ts_d(:,k,l) = 0.0
                      stats%podn(m)%count_value_ts(:,k,l)=0 
                      if(alarm) then 
                         stats%podn(m)%tavg_value_ts(:,k,l) = 0.0
                         stats%podn(m)%tavg_count_value_ts(:,k,l)=0 
                      endif
                   enddo
                endif
             enddo
