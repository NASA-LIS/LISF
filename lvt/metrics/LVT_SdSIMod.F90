...
! 
! !NOTES: 
!   The standard practice is to compute SdSI on monthly scale. Since the 
!   code needs to store the precip values in time to compute SdSI, 
!   currently LVT only supports SdSI computations when the time 
!   averaging interval is set to a multiple of a month (2592000 seconds). 
!
!   Interpretation of SdSI at different timescales: the 6-month SdSI represents
...
!   deficits or excesses. The SdSIs at longer timescales provide estimates
...
!-----------------------------------------------------------------------------
  public :: LVT_initSdSI
  public :: LVT_diagnoseSdSI
  public :: LVT_computeSdSI
  public :: LVT_writeMetric_SdSI
  public :: LVT_resetMetric_SdSI
  public :: LVT_writerestart_SdSI
  public :: LVT_readrestart_SdSI
...
!EOP
  ...
  type(SdSIdec), allocatable :: LVT_SdSI_struc(:)
  private
...
!BOP
!
! !ROUTINE: LVT_initSdSI
! \label{LVT_initSdSI}
! 
! !INTERFACE: 
  subroutine LVT_initSdSI(selectNlevs, stats,metric)
! !ARGUMENTS: 
...
!EOP
    ...
    allocate(LVT_SdSI_struc(LVT_rc%nensem))
    
    if(LVT_metrics%SdSI%selectOpt.eq.1.or.&
         LVT_metrics%SdSI%timeOpt.eq.1) then 
       call ESMF_ConfigGetAttribute(LVT_config,time,&
            label="SdSI timescale of computation:",rc=rc)
       if(rc.ne.0) then 
          write(LVT_logunit,*) "[ERR] 'SdSI timescale of computation:' not defined"
          write(LVT_logunit,*) "[ERR] Options are 1mo, 3mo, 6mo, 9mo, 12mo or 24mo"
          write(LVT_logunit,*) "[ERR] Corresponding to 1-month SdSI, 3-month SdSI and so on"
    ...
    if(LVT_metrics%SdSI%selectOpt.eq.1.or.&
       ...
       do m=1,LVT_rc%nensem
          ...
          call LVT_parseTimeString(time,LVT_SdSI_struc(m)%tscale)
          LVT_SdSI_struc(m)%tscale = LVT_SdSI_struc(m)%tscale/2592000

          if(LVT_rc%tavgInterval.ne.2592000) then 
             write(LVT_logunit,*) '[ERR] '
             write(LVT_logunit,*) '[ERR] SdSI values are computed at a monthly timescale'
             write(LVT_logunit,*) '[ERR] Please set the time averaging interval to a month '
             write(LVT_logunit,*) '[ERR] program stopping...'
             write(LVT_logunit,*) '[ERR] '
             call LVT_endrun()
          endif
          LVT_SdSI_struc(m)%nasc = 12
          
          call computeNmonths(LVT_SdSI_struc(m)%nsize_total, LVT_rc%tavgInterval)
          LVT_SdSI_struc(m)%nsize_season = &
               LVT_SdSI_struc(m)%nsize_total/LVT_SdSI_struc(m)%nasc
          
          if(metric%selectOpt.eq.1) then 
             allocate(LVT_SdSI_struc(m)%model_pcp_final(LVT_rc%ngrid,&
                  LVT_SdSI_struc(m)%nsize_total))
             LVT_SdSI_struc(m)%model_pcp_final = 0.0
             
             allocate(LVT_SdSI_struc(m)%model_beta(LVT_rc%ngrid,&
                  LVT_SdSI_struc(m)%nasc))
             allocate(LVT_SdSI_struc(m)%model_gamma(LVT_rc%ngrid,&
                  LVT_SdSI_struc(m)%nasc))
             allocate(LVT_SdSI_struc(m)%model_pzero(LVT_rc%ngrid,&
                  LVT_SdSI_struc(m)%nasc))
             allocate(LVT_SdSI_struc(m)%model_SdSI(LVT_rc%ngrid))
             LVT_SdSI_struc(m)%model_SdSI = LVT_rc%udef
             
             allocate(LVT_SdSI_struc(m)%model_current(LVT_rc%ngrid,&
                  LVT_SdSI_struc(m)%tscale))
             LVT_SdSI_struc(m)%model_current = LVT_rc%udef

             if(LVT_rc%obssource(2).ne."none") then 
                if(selectNlevs(2).ge.1) then 
                   allocate(LVT_SdSI_struc(m)%obs_pcp_final(LVT_rc%ngrid,&
                        LVT_SdSI_struc(m)%nsize_total))
                   LVT_SdSI_struc(m)%obs_pcp_final = 0.0
                   
                   allocate(LVT_SdSI_struc(m)%obs_beta(LVT_rc%ngrid,&
                        LVT_SdSI_struc(m)%nasc))
                   allocate(LVT_SdSI_struc(m)%obs_gamma(LVT_rc%ngrid,&
                        LVT_SdSI_struc(m)%nasc))
                   allocate(LVT_SdSI_struc(m)%obs_pzero(LVT_rc%ngrid,&
                        LVT_SdSI_struc(m)%nasc))
                   allocate(LVT_SdSI_struc(m)%obs_SdSI(LVT_rc%ngrid))
                   LVT_SdSI_struc(m)%obs_SdSI = LVT_rc%udef
                   
                   allocate(LVT_SdSI_struc(m)%obs_current(LVT_rc%ngrid,1))
                   LVT_SdSI_struc(m)%obs_current = LVT_rc%udef
...
!-------------------------------------------------------------------------
  ...
  end subroutine LVT_initSdSI

!BOP
! 
! !ROUTINE: LVT_diagnoseSdSI
! \label{LVT_diagnoseSdSI}
!
! !INTERFACE: 
  subroutine LVT_diagnoseSdSI(pass)
! 
! !USES:     
...
!   \begin{description}
!    \item[diagnoseSingleModelSdSI](\ref{diagnoseSingleModelSdSI})
!     updates the spi computation for a single variable. This routine
!     stores the precip values to be used later for computing SdSI, 
...
!EOP
    ...
    if(pass.eq.1) then 
       if(LVT_metrics%SdSI%selectOpt.eq.1.or.&
          ...
          do while(associated(model))   
             ...
             call diagnoseSingleSdSI(model, obs, stats,&
                  LVT_metrics%SdSI)
  ...
  end subroutine LVT_diagnoseSdSI


!BOP
! 
! !ROUTINE: diagnoseSingleSdSI
! \label{diagnoseSingleSdSI}
!
! !INTERFACE: 
  subroutine diagnoseSingleSdSI(model, obs, stats,metric)
! 
! !USES:   
    implicit none
!
...
!
! !DESCRIPTION: 
!   This routine updates the SdSI computation of the 
!   specified variable. 
!
!  The arguments are: 
!
...
!EOP
    ...
    if(stats%selectOpt.eq.1.and.&
         ((trim(model%short_name).eq."dS")).and.&
                ...
                if(model%count(t,m,m_k).gt.0) then
                   if(metric%selectOpt.eq.1) then 
                      if(model%value(t,m,m_k).ne.LVT_rc%udef) then 
                         ...
                         LVT_SdSI_struc(m)%model_pcp_final(t,tindex_f) = &
                              model%value(t,m,m_k)
                      endif
                   endif
                endif
                   ...
                   if(obs%selectNlevs.ge.1) then
                      if(obs%count(t,m,o_k).gt.0) then 
                         if(metric%selectOpt.eq.1.and.&
                            ...
                            LVT_SdSI_struc(m)%obs_pcp_final(t,tindex_f) = &
                                 obs%value(t,m,o_k)
                         endif
  ...
  end subroutine diagnoseSingleSdSI

!BOP
! 
! !ROUTINE: LVT_computeSdSI
! \label{LVT_computeSdSI}
!
! !INTERFACE: 
  subroutine LVT_computeSdSI(pass,alarm)
! 
! !USES:   
...
!
! !DESCRIPTION: 
!   This subroutine issues the calls to compute the SdSI values for 
!   desired variables.
!
!   The methods invoked are: 
!   \begin{description}
!    \item[computeSingleModelSdSI](\ref{computeSingleModelSdSI})
!     computes the SdSI values for a single variable
!   \end{description}
! 
! !FILES USED:
!
...
!EOP
    ...
    if(LVT_rc%startmode.eq."coldstart") then 
       if(pass.eq.1) then 
          if(LVT_metrics%SdSI%selectOpt.eq.1.or.LVT_metrics%SdSI%timeOpt.eq.1) then 
             call LVT_getDataStream1Ptr(model)
             call LVT_getDataStream2Ptr(obs)
             call LVT_getstatsEntryPtr(stats)
             
             do while(associated(model))             
                call computeSingleSdSIparams(alarm,&
                     model,obs,stats,LVT_metrics%SdSI)
       ...
       elseif(pass.eq.2) then        
          if(LVT_metrics%SdSI%selectOpt.eq.1.or.LVT_metrics%SdSI%timeOpt.eq.1) then 
             if(alarm) then 
                if(LVT_metrics%SdSI%timeOpt.eq.1.and.&
                     LVT_metrics%SdSI%extractTS.eq.1) then 

                   if(LVT_rc%lvt_wopt.eq."2d ensemble gridspace") then 
                      do m=1,LVT_rc%nensem
                         do i=1,LVT_rc%ntslocs
                            write(LVT_metrics%SdSI%ftn_ts_loc(i,m),200,advance='no') &
                      ...
                      do i=1,LVT_rc%ntslocs
                         write(LVT_metrics%SdSI%ftn_ts_loc(i,1),200,advance='no') &
                              LVT_rc%yr, '',LVT_rc%mo, '', LVT_rc%da, '', &
                              LVT_rc%hr,'',LVT_rc%mn, '' 
                      enddo
                   endif
...
200          format(I4, a1, I2.2, a1, I2.2, a1, I2.2, a1, I2.2,a1)
             ...
             if(alarm) then 
                if(LVT_metrics%SdSI%timeOpt.eq.1.and.&
                   ...
                   if(LVT_rc%lvt_wopt.eq."2d ensemble gridspace") then 
                      do m=1,LVT_rc%nensem
                         do i=1,LVT_rc%ntslocs
                            write(LVT_metrics%SdSI%ftn_ts_loc(i,m),fmt='(a1)') ''
                         enddo
                      enddo
                   else
                      do i=1,LVT_rc%ntslocs
                         write(LVT_metrics%SdSI%ftn_ts_loc(i,1),fmt='(a1)') ''
                      enddo
                   endif
  ...
  end subroutine LVT_computeSdSI
  

!BOP
! 
! !ROUTINE: computeSingleSdSIparams
! \label{computeSingleSdSIparams}
!
! !INTERFACE: 
  subroutine computeSingleSdSIparams(alarm,model,obs,stats,metric)
! 
! !USES:   
...
!
! !DESCRIPTION: 
!  This routine computes the SdSI values for each grid cell. 
!  The arguments are: 
!
!  \begin{description}
!    \item[model] model variable object
...
!EOP
    ...
    if(LVT_rc%endtime.eq.1.and.metric%selectOpt.eq.1) then 
       if(stats%selectOpt.eq.1.and.&
          ...
          do t=1,LVT_rc%ngrid
             do m=1,LVT_rc%nensem
                do k=1,model%selectNlevs
                   do l=1,LVT_SdSI_struc(m)%nasc
                      call SdSI_gamma(t,l,m,LVT_SdSI_struc(m)%nsize_total,&
                           LVT_SdSI_struc(m)%nsize_season,&
                           LVT_SdSI_struc(m)%model_pcp_final(t,:),&
                           LVT_SdSI_struc(m)%model_beta(t,l),&
                           LVT_SdSI_struc(m)%model_gamma(t,l),&
                           LVT_SdSI_struc(m)%model_pzero(t,l))
                   enddo
                enddo
             enddo
          enddo
                   ...
                   if(LVT_rc%obssource(2).ne."none") then 
                      if(obs%selectNlevs.ge.1) then 
                         do l=1,LVT_SdSI_struc(m)%nasc
                            call SdSI_gamma(t,l,m,LVT_SdSI_struc(m)%nsize_total,&
                                 LVT_SdSI_struc(m)%nsize_season,&
                                 LVT_SdSI_struc(m)%obs_pcp_final(t,:),&
                                 LVT_SdSI_struc(m)%obs_beta(t,l),&
                                 LVT_SdSI_struc(m)%obs_gamma(t,l),&
                                 LVT_SdSI_struc(m)%obs_pzero(t,l))
                         enddo
                      endif
                   endif
                enddo
  ...
  end subroutine computeSingleSdSIparams
...
! 
! !INTERFACE: 
  subroutine SdSI_gamma(t,l,kk,nsize,nsize_season,pcp,beta,gamma, pzero)
...
!    real test1(10), rval
    ...
    pcp_season = 0.0
    k = 0 
    do i=l,nsize,LVT_SdSI_struc(kk)%nasc
       k = k+1

       tmppcp = 0.0
       pcp_count = 0 
       do m=1,LVT_SdSI_struc(kk)%tscale
          t1_ind = (i-LVT_SdSI_struc(kk)%tscale+m)
          if(t1_ind.gt.0) then 
...
!    gamma =2.1627 
    ...
    stop
#endif
  end subroutine SdSI_gamma


!BOP
! 
! !ROUTINE: computeSingleSdSI
! \label{computeSingleSdSI}
!
! !INTERFACE: 
  subroutine computeSingleSdSI(alarm,model,obs,stats,metric)
! 
! !USES:   
...
!EOP
    ...
    real     :: model_SdSI(LVT_rc%ngrid,1,1)
    real     :: obs_SdSI(LVT_rc%ngrid,1,1)
    integer  :: count_model_SdSI(LVT_rc%ngrid,1,1)
    integer  :: count_obs_SdSI(LVT_rc%ngrid,1,1)
    ...
    if(stats%selectOpt.eq.1.and.&
       ...
       do m=1,LVT_rc%nensem
          do i=LVT_SdSI_struc(m)%tscale,1,-1
             if(i.ne.1) then 
                LVT_SdSI_struc(m)%model_current(:,i) = &
                     LVT_SdSI_struc(m)%model_current(:,i-1)
             else
                LVT_SdSI_struc(m)%model_current(:,i) = & 
                     model%value(:,m,1)
             endif
       ...
       do t=1,LVT_rc%ngrid
          do m=1,LVT_rc%nensem         
             ...
             do i=1,LVT_SdSI_struc(m)%tscale
                if(LVT_SdSI_struc(m)%model_current(t,i).ne.LVT_rc%udef) then 
                   tmppcp = tmppcp + LVT_SdSI_struc(m)%model_current(t,i)
                   pcp_count = pcp_count + 1
                endif
             enddo
             ...
             call SdSI_zval(t,&
                  tmppcp, &
                  LVT_SdSI_struc(m)%model_beta(t,tind), &
                  LVT_SdSI_struc(m)%model_gamma(t,tind), &
                  LVT_SdSI_struc(m)%model_pzero(t,tind), &
                  LVT_SdSI_struc(m)%model_SdSI(t))

!temp hack 
             LVT_SdSI_struc(m)%model_SdSI(t) = tmppcp
       ...
       if(LVT_rc%obssource(2).ne."none") then 
          if(obs%selectNlevs.ge.1) then 
             do m=1,LVT_rc%nensem         
                do i=LVT_SdSI_struc(m)%tscale,1,-1
                   if(i.ne.1) then 
                      LVT_SdSI_struc(m)%obs_current(:,i) = &
                           LVT_SdSI_struc(m)%obs_current(:,i-1)
                   else
                      LVT_SdSI_struc(m)%obs_current(:,i) = & 
                           obs%value(:,m,1)
                   endif
             ...
             do t=1,LVT_rc%ngrid
                do m=1,LVT_rc%nensem                         
                   ...
                   do i=1,LVT_SdSI_struc(m)%tscale
                      if(LVT_SdSI_struc(m)%obs_current(t,i).ne.LVT_rc%udef) then 
                         tmppcp = tmppcp + LVT_SdSI_struc(m)%obs_current(t,i)
                         pcp_count = pcp_count + 1
                      endif
                   enddo
                   tmppcp = tmppcp/pcp_count
                   ...
                   call SdSI_zval(t,&
                        tmppcp,&
                        LVT_SdSI_struc(m)%obs_beta(t,tind), &
                        LVT_SdSI_struc(m)%obs_gamma(t,tind), &
                        LVT_SdSI_struc(m)%obs_pzero(t,tind), &
                        LVT_SdSI_struc(m)%obs_SdSI(t))
       ...
       if(metric%extractTS.eq.1) then 
          if(LVT_rc%lvt_wopt.eq."2d ensemble gridspace") then 
             do m=1,LVT_rc%nensem
                model_SdSI(:,1,1) = LVT_SdSI_struc(m)%model_SdSI(:)
                if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
                   obs_SdSI(:,1,1) = LVT_SdSI_struc(m)%obs_SdSI(:)
             ...
             if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
                ...
                do m=1,LVT_rc%nensem
                   do t=1,LVT_rc%ngrid
                      if(LVT_SdSI_struc(m)%model_SdSI(t).ne.LVT_rc%udef) then 
       ...
       do m=1,LVT_rc%nensem
          call LVT_computeCI(LVT_SdSI_struc(m)%model_SdSI(:),LVT_rc%ngrid,&
               LVT_rc%pval_CI, LVT_SdSI_struc(m)%model_SdSI_ci(1,1))
          
          if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
             call LVT_computeCI(LVT_SdSI_struc(m)%obs_SdSI(:),LVT_rc%ngrid,&
                  LVT_rc%pval_CI,LVT_SdSI_struc(m)%obs_SdSI_ci(1,1))
          endif
       enddo

#if 0 
       call LVT_writeDataBasedStrat(model,obs,stats,metric,&
            LVT_rc%ngrid, model_SdSI)

       if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
          call LVT_writeDataBasedStrat(model,obs,stats,metric,&
               LVT_rc%ngrid, model_SdSI, &
               LVT_rc%ngrid, obs_SdSI)
          
       endif
#endif
    endif

  end subroutine computeSingleSdSI

!BOP
! 
! !ROUTINE: LVT_writeMetric_SdSI
! \label(LVT_writeMetric_SdSI)
!
! !INTERFACE:
  subroutine LVT_writeMetric_SdSI(pass,final,vlevels,stats,obs)
! 
! !USES:   
...
!    real                    :: obs_SdSI(LVT_rc%ngrid,1,1)
    ...
    integer                 :: count_model_SdSI(LVT_rc%ngrid)
    integer                 :: count_obs_SdSI(LVT_rc%ngrid)

    count_model_SdSI = LVT_rc%udef
    count_obs_SdSI = LVT_rc%udef

    if(pass.eq.LVT_metrics%SdSI%npass) then 
       if(final.ne.1) then
          if(stats%selectOpt.eq.1) then 
             ...
             if(LVT_metrics%SdSI%timeOpt.eq.1) then 
                do k=1,vlevels
                   do m=1,LVT_rc%nensem
                      value_ts(:,m) = & 
                           LVT_SdSI_struc(m)%model_SdSI(:)
                      count_value_ts(:,m) = & 
                           count_model_SdSI(:)
                   enddo
                   call LVT_writevar_gridded(LVT_metrics%SdSI%ftn_ts, &
                        value_ts(:,:),&
                        stats%vid_ts(LVT_SdSIid,1),k)
                   call LVT_writevar_gridded(LVT_metrics%SdSI%ftn_ts, &
                        real(count_value_ts(:,:)),&
                        stats%vid_count_ts(LVT_SdSIid,1),k)
                enddo
                ...
                if(LVT_rc%obssource(2).ne."none") then 
                   ...
                   do k=1,vlevels
                      do m=1,LVT_rc%nensem
                         value_ts(:,m) = & 
                              LVT_SdSI_struc(m)%obs_SdSI(:)
                         count_value_ts(:,m) = & 
                              count_obs_SdSI(:)
                      enddo
                      call LVT_writevar_gridded(LVT_metrics%SdSI%ftn_ts, &
                           value_ts(:,:),&
                           stats%vid_ts(LVT_SdSIid,2),k)
                      call LVT_writevar_gridded(LVT_metrics%SdSI%ftn_ts, &
                           real(count_value_ts(:,:)),&
                           stats%vid_count_ts(LVT_SdSIid,2),k)
                   enddo
                endif
             ...
             call LVT_writeSummaryStats(&
                  LVT_metrics%SdSI%ftn_summ,&
                  LVT_metrics%SdSI%short_name,&
                  LVT_rc%ngrid,&
                  model_SdSI, &
                  count_model_SdSI,&
                  stats%short_name,&
                  LVT_SdSI_struc(m)%model_SdSI_ci)
             if(LVT_rc%obssource(2).ne."none".and.obs%selectNlevs.ge.1) then 
                obs_SdSI(:,1,1) = LVT_SdSI_struc(m)%obs_SdSI(:)
                call LVT_writeSummaryStats(&
                     LVT_metrics%SdSI%ftn_summ,&
                     ...
                     LVT_SdSI_struc(m)%obs_SdSI_ci)
             endif
#endif
  end subroutine LVT_writeMetric_SdSI

!BOP
! 
! !ROUTINE: LVT_resetMetric_SdSI
! \label(LVT_resetMetric_SdSI)
!
! !INTERFACE:
  subroutine LVT_resetMetric_SdSI
! 
! !USES:   
!
! !INPUT PARAMETERS: 
...
!EOP
  ...
  end subroutine LVT_resetMetric_SdSI

!BOP
! 
! !ROUTINE: LVT_writerestart_SdSI
! 
! !INTERFACE:
  subroutine LVT_writerestart_SdSI(ftn,pass)
! !USES: 
...
! !DESCRIPTION: 
!  This routine writes the restart file for SdSI metric computations
! 
!EOP
...
!    if(LVT_rc%endtime.eq.1) then 
       ...
       do while(associated(model))
          if(trim(model%short_name).eq."dS") then 
             do m=1,LVT_rc%nensem
                if(LVT_metrics%SdSI%selectOpt.eq.1) then 
                   do l=1,LVT_SdSI_struc(m)%nasc
                      call LVT_writevar_restart(ftn,&
                           LVT_SdSI_struc(m)%model_beta(:,l))
                      call LVT_writevar_restart(ftn,&
                           LVT_SdSI_struc(m)%model_gamma(:,l))
                      call LVT_writevar_restart(ftn,&
                           LVT_SdSI_struc(m)%model_pzero(:,l))
                   enddo
                   if(LVT_rc%obssource(2).ne."none") then 
                      do l=1,LVT_SdSI_struc(m)%nasc
                         call LVT_writevar_restart(ftn,&
                              LVT_SdSI_struc(m)%obs_beta(:,l))
                         call LVT_writevar_restart(ftn,&
                              LVT_SdSI_struc(m)%obs_gamma(:,l))
                         call LVT_writevar_restart(ftn,&
                              LVT_SdSI_struc(m)%obs_pzero(:,l))
                      enddo
                   endif                
                end if
             enddo
...
!    endif
  ...
  end subroutine LVT_writerestart_SdSI

!BOP
! 
! !ROUTINE: LVT_readrestart_SdSI
! 
! !INTERFACE:
  subroutine LVT_readrestart_SdSI(ftn)
! !USES: 
...
! !DESCRIPTION: 
!  This routine reads the restart file for SdSI metric computations
! 
!EOP
...
!EOP
    ...
    do while(associated(model))
       if(trim(model%short_name).eq."dS") then 
          if(LVT_metrics%SdSI%selectOpt.eq.1) then 
!             do l=1,LVT_SdSI_struc(m)%nsize_total
!                call LVT_readvar_restart(ftn,&
!                     LVT_SdSI_struc(m)%model_pcp_final(:,l))
!             enddo
             do m=1,LVT_rc%nensem
                do l=1,LVT_SdSI_struc(m)%nasc
                   call LVT_readvar_restart(ftn,&
...
!                do l=1,LVT_SdSI_struc(m)%nsize_total
!                   call LVT_readvar_restart(ftn,&
!                        LVT_SdSI_struc(m)%obs_pcp_final(:,l))
!                enddo
                   do l=1,LVT_SdSI_struc(m)%nasc
                      call LVT_readvar_restart(ftn,&
                           LVT_SdSI_struc(m)%obs_beta(:,l))
  ...
  end subroutine LVT_readrestart_SdSI
...
! 
! !INTERFACE: 
  subroutine SdSI_zval(t,pcp, beta, gamma, pzero, zval)
! !ARGUMENTS:
    integer, intent(in) :: t
    real,    intent(in) :: pcp
...
! 
! !DESCRPTION:
!   This subroutine computes the SdSI values. The cumulative 
!   probability of a given event is computed from the given 
!   gamma distribution. The cumulative probability is then 
!   transformed to the standard normal random variable (SdSI) 
