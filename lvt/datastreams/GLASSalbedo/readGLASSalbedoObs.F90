...
!EOP
  ...
  integer*2, allocatable  :: albedo_raw_bs_vis(:),albedo_raw_ws_vis(:),albedo_raw_bs_nir(:),albedo_raw_ws_nir(:),albedo_raw_bs_sw(:),albedo_raw_ws_sw(:)
  ...
  allocate(albedo_raw_bs_vis(nc*nr))
  allocate(albedo_raw_ws_vis(nc*nr))
  allocate(albedo_raw_bs_nir(nc*nr))
  allocate(albedo_raw_ws_nir(nc*nr))
  allocate(albedo_raw_bs_sw(nc*nr))
  allocate(albedo_raw_ws_sw(nc*nr))

  iret1 = gdrdfld(grid_id,albedo_name_bs_vis,start,stride,edge,albedo_raw_bs_vis)
  iret2 = gdrdfld(grid_id,albedo_name_ws_vis,start,stride,edge,albedo_raw_ws_vis)
  iret3 = gdrdfld(grid_id,albedo_name_bs_nir,start,stride,edge,albedo_raw_bs_nir)
  iret4 = gdrdfld(grid_id,albedo_name_ws_nir,start,stride,edge,albedo_raw_ws_nir)
  iret5 = gdrdfld(grid_id,albedo_name_bs_sw,start,stride,edge,albedo_raw_bs_sw)
  iret6 = gdrdfld(grid_id,albedo_name_ws_sw,start,stride,edge,albedo_raw_ws_sw)         
     ...
     do r=1,GLASSalbedoobs(source)%nr
        do c=1,GLASSalbedoobs(source)%nc
           ...
           r1 = nr - (r + lat_off) + 1
             ...
             if(albedo_raw_bs_vis(c1+(r1-1)*nc).gt.0.and.&
                albedo_raw_bs_vis(c1+(r1-1)*nc).ne.2550) then 
              albedo_in1(1,c+(r-1)*GLASSalbedoobs(source)%nc) =&
                   albedo_raw_bs_vis(c1+(r1-1)*nc)*0.01
...
!
             if(albedo_raw_ws_vis(c1+(r1-1)*nc).gt.0.and.&
                albedo_raw_ws_vis(c1+(r1-1)*nc).ne.2550) then 
              albedo_in1(2,c+(r-1)*GLASSalbedoobs(source)%nc) =&
                   albedo_raw_ws_vis(c1+(r1-1)*nc)*0.01
...
!
             if(albedo_raw_bs_nir(c1+(r1-1)*nc).gt.0.and.&
                albedo_raw_bs_nir(c1+(r1-1)*nc).ne.2550) then 
              albedo_in1(3,c+(r-1)*GLASSalbedoobs(source)%nc) =&
                   albedo_raw_bs_nir(c1+(r1-1)*nc)*0.01
...
!
             if(albedo_raw_ws_nir(c1+(r1-1)*nc).gt.0.and.&
                albedo_raw_ws_nir(c1+(r1-1)*nc).ne.2550) then 
              albedo_in1(4,c+(r-1)*GLASSalbedoobs(source)%nc) =&
                   albedo_raw_ws_nir(c1+(r1-1)*nc)*0.01
...
!
             if(albedo_raw_bs_sw(c1+(r1-1)*nc).gt.0.and.&
                albedo_raw_bs_sw(c1+(r1-1)*nc).ne.2550) then 
              albedo_in1(5,c+(r-1)*GLASSalbedoobs(source)%nc) =&
                   albedo_raw_bs_sw(c1+(r1-1)*nc)*0.01
...
!
             if(albedo_raw_ws_sw(c1+(r1-1)*nc).gt.0.and.&
                albedo_raw_ws_sw(c1+(r1-1)*nc).ne.2550) then 
              albedo_in1(6,c+(r-1)*GLASSalbedoobs(source)%nc) =&
                   albedo_raw_ws_sw(c1+(r1-1)*nc)*0.01
...
!
        enddo
     enddo
     deallocate(albedo_raw_bs_vis)
     deallocate(albedo_raw_ws_vis)
     deallocate(albedo_raw_bs_nir)
     deallocate(albedo_raw_ws_nir)
     deallocate(albedo_raw_bs_sw)
     deallocate(albedo_raw_ws_sw)
...
! 
! !INTERFACE: 
subroutine create_GLASSalbedo_filename(ndir,  yr, doy, gridname, filename)
! !USES:   
