...
!EOP
  ...
  integer                        :: c,r,t
  logical*1                      :: sm_data_b(SMAPNRT_smobs(source)%nc*SMAPNRT_smobs(source)%nr)
  logical*1                      :: smobs_b_ip(LVT_rc%lnc*LVT_rc%lnr)
  real                           :: sm_data(SMAPNRT_smobs(source)%nc*SMAPNRT_smobs(source)%nr)
  ...
  do r=1,LVT_rc%lnr
     do c=1,LVT_rc%lnc
        if(smobs_ip(c+(r-1)*LVT_rc%lnc).ne.-9999.0) then 
           ...
           SMAPNRT_smobs(source)%smtime(c,r) = & 
                time
        endif
     enddo
  enddo
