...
module FLUXNET2015NC_obsMod
  ...
  type, public :: FLUXNET2015NCobsdec
     ...
     real,          allocatable  :: Qtau(:,:)       ! Momentum Flux is actually observed as the friction velocity
     real,          allocatable  :: GPP(:,:)        ! Gross primary productivity of biomass
  end type FLUXNET2015NCobsdec

  type(FLUXNET2015NCobsdec), allocatable :: FLUXNET2015NCobs(:)

contains
  
!BOP
! 
! !ROUTINE: FLUXNET2015NC_obsInit
! \label{FLUXNET2015NC_obsInit}
!
! !INTERFACE: 
...
!  structures required for reading FLUXNET2015NC data.
! 
! !FILES USED:
!
! !REVISION HISTORY: 
...
!EOP
    ...
    if(.not.allocated(FLUXNET2015NCobs)) then 
       allocate(FLUXNET2015NCobs(LVT_rc%nDataStreams))
    endif
...
!------------------------------------------------------------------------------
    ...
    call ESMF_ConfigGetAttribute(LVT_config, FLUXNET2015NCobs(i)%odir, &
    ...
    read(ftn,*)
    read(ftn,*) FLUXNET2015NCobs(i)%n_stns
    read(ftn,*) 

    allocate(FLUXNET2015NCobs(i)%stn_name(FLUXNET2015NCobs(i)%n_stns))
    allocate(FLUXNET2015NCobs(i)%stnlat(FLUXNET2015NCobs(i)%n_stns))
    allocate(FLUXNET2015NCobs(i)%stnlon(FLUXNET2015NCobs(i)%n_stns))
...
!svk : to be safe:
    ...
    allocate(FLUXNET2015NCobs(i)%Qle(FLUXNET2015NCobs(i)%n_stns, arrayLen))
    allocate(FLUXNET2015NCobs(i)%Qh(FLUXNET2015NCobs(i)%n_stns, arrayLen))
    allocate(FLUXNET2015NCobs(i)%Qtau(FLUXNET2015NCobs(i)%n_stns, arrayLen))
    allocate(FLUXNET2015NCobs(i)%GPP(FLUXNET2015NCobs(i)%n_stns, arrayLen))

    FLUXNET2015NCobs(i)%Qle = LVT_rc%udef
    FLUXNET2015NCobs(i)%Qh = LVT_rc%udef
    FLUXNET2015NCobs(i)%Qtau = LVT_rc%udef
    FLUXNET2015NCobs(i)%GPP = LVT_rc%udef

    write(LVT_logunit,*) '[INFO] Processing FLUXNET2015NC stations'
!------------------------------------------------------------------------------
! For each station, this reads the site name, station name, and station
! position in lat, lon coordinates.
!------------------------------------------------------------------------------
    do k=1,FLUXNET2015NCobs(i)%n_stns
       read(ftn,'(a)') currentLine

       iloc = Index(currentLine, ";")
       READ(currentLine(1: iloc - 1), *) FLUXNET2015NCobs(i)%stn_name(k)
       currentLine = currentLine(iloc + 1: Len(currentLine))

       iloc = Index(currentLine, ";")
       READ(currentLine(1: iloc - 1), *) FLUXNET2015NCobs(i)%stnlat(k)
       currentLine = currentLine(iloc + 1: Len(currentLine))

       iloc = Index(currentLine, ";")
       READ(currentLine(1: iloc - 1), *) FLUXNET2015NCobs(i)%stnlon(k)
       currentLine = currentLine(iloc + 1: Len(currentLine))

       write(LVT_logunit,*) '[INFO] ',k, &
            FLUXNET2015NCobs(i)%stn_name(k), &
            FLUXNET2015NCobs(i)%stnlat(k), FLUXNET2015NCobs(i)%stnlon(k)
    end do
...
!------------------------------------------------------------------------------
    ...
    FLUXNET2015NCobs(i)%startflag = .true. 
    FLUXNET2015NCobs(i)%yr = -1

    call ESMF_TimeIntervalSet(FLUXNET2015NCobs(i)%timestep, s=1800, rc=status)
    call LVT_verify(status, 'error in setting timestep (FLUXNET2015NCobs)')

    call LVT_update_timestep(LVT_rc, 1800)

  end subroutine FLUXNET2015NC_obsinit

end module FLUXNET2015NC_obsMod
