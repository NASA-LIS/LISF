...
_STARTDT = datetime.datetime(2024, 9, 26,  9)
...
def _main():
    ...
    varlist.append("PS41Snow_inst") # PS41 snow vars handled specially below
    varlist.sort()
    for var in varlist:
        newlines = []
        for line in lines:
            if "LVT output format:" in line:
                ...
                elif var == "PS41Snow_inst":
                    keys = sorted(list(_VAR_ATTRIBUTES_PS41_SNOW.keys()))
                    for key in keys:
                        line += f"{_VAR_ATTRIBUTES_PS41_SNOW[key]}\n"
                else:
