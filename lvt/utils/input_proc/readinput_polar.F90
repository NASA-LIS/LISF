...
!  real, allocatable :: param_dd(:,:)
  ...
  call ESMF_ConfigFindLabel(LVT_config,"run domain true lat:",rc=rc)
  do n=1,LVT_rc%nnest
     call ESMF_ConfigGetAttribute(LVT_config,run_dd(n,3),rc=rc)
     call LVT_verify(rc,'run domain true lat: not defined')
  enddo
  ...
  call ESMF_ConfigFindLabel(LVT_config,"run domain orientation:",rc=rc)
  do n=1,LVT_rc%nnest
     call ESMF_ConfigGetAttribute(LVT_config,run_dd(n,5),rc=rc)
     call LVT_verify(rc,'run domain orientation: not defined')
  enddo
...
!  call LVT_readDomainConfigSpecs("param domain",param_dd)
  ...
  write(*,*)'DOMAIN details: Running in polar stereographic projection'
  ...
  do n=1,LVT_rc%nnest
     ...
     LVT_rc%gridDesc(n,7) = orient
     LVT_rc%gridDesc(n,8) = xmesh
     LVT_rc%gridDesc(n,9) = xmesh
     LVT_rc%gridDesc(n,10) = truelat
...
!     LVT_rc%pnr(n) = LVT_rc%gridDesc(n,33)
     ...
     call map_set(PROJ_PS,LVT_rc%gridDesc(n,4),LVT_rc%gridDesc(n,5),&
          LVT_rc%gridDesc(n,8)*1000.0,LVT_rc%gridDesc(n,11),LVT_rc%gridDesc(n,10),&
          0.0,LVT_rc%lnc(n),LVT_rc%lnr(n),LVT_domain(n)%lvtproj)
