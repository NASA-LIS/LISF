...
! NASA GSFC Land surface Verification Toolkit (LVT) V1.0 (LVT)
!
!
...
subroutine compute_stnwts(nstns, gridDesc,stnlat, stnlon,&
     npts, stnwt)
  use LVT_logMod, only : LVT_endrun
  implicit none
! !ARGUMENTS:   
  integer     :: nstns
  real        :: gridDesc(50)
...
!  if(gridDesc(1) .eq. 0) then 
     call compute_earth_coord(gridDesc,npts,fill,xpts,ypts,rlon,rlat,nv)
