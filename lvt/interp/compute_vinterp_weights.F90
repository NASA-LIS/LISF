...
!computation of surface weights
  ...
  if(LVT_rc%vinterp_option.eq.1) then 
     nlayers = 1
     ...
     enddo
  elseif(LVT_rc%vinterp_option.eq.2) then 
!Implemented by Jon Case at NASA MSFC
     nlayers = 1
     ...
     do k = 1,obs_nlayers
        ! top-level weight is simply mid-point between lev1 and lev2
        if ( (k.eq.1) .and. (obs_nlayers.gt.1) ) then
           sf_wt(k) = 0.5*(obs_d(1)+obs_d(2))
           ! intermediate-level weights are half the distance between obs levels above and below current level
        elseif ( (k.gt.1) .and. (k.lt.obs_nlayers) .and. (0.5*(obs_d(k)+obs_d(k+1)).le.lis_sf_d) ) then
           sf_wt(k) = 0.5*(obs_d(k+1)-obs_d(k-1))
           ! bottom-level weight is simply distance from mid-point to bottom of LIS layer
        elseif ( (k.gt.1) .and. (obs_d(k)+0.0001.ge.lis_sf_d) .and. (obs_d(k-1)-0.0001.lt.lis_sf_d) ) then
           if ( 0.5*(obs_d(k)+obs_d(k-1)) .le. lis_sf_d ) then
              sf_wt(k) = lis_sf_d - 0.5*(obs_d(k)+obs_d(k-1))
           else
              sf_wt(k) = 0
           endif
        else
     ...
     ! J.Case – modified weight code below.
     do k = 1,obs_nlayers
        ! top-level weight is simply mid-point between lev1 and lev2
        if ( (k.eq.1) .and. (obs_nlayers.gt.1) ) then
           rz_wt(k) = 0.5*(obs_d(1)+obs_d(2))
           ! intermediate-level weights are half the distance between obs levels above and below current level
        elseif ( (k.gt.1) .and. (k.lt.obs_nlayers) .and. (0.5*(obs_d(k)+obs_d(k+1)).le.lis_rz_d) ) then
           rz_wt(k) = 0.5*(obs_d(k+1)-obs_d(k-1))
           ! bottom-level weight is simply distance from mid-point to bottom of LIS layer
        elseif ( (k.gt.1) .and. (obs_d(k)+0.0001.ge.lis_rz_d) .and. (obs_d(k-1)-0.0001.lt.lis_rz_d) ) then
           if ( 0.5*(obs_d(k)+obs_d(k-1)) .le. lis_rz_d ) then
              rz_wt(k) = lis_rz_d - 0.5*(obs_d(k)+obs_d(k-1))
           else
              rz_wt(k) = 0
           endif
        else
