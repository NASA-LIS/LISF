...
Now a working branch is ready for Bob to work on.
...
Repeat until done.  Alice should perform the last "fetch-bob/merge-bob/push-to-origin" step.  Now Alice can submit a pull request.  And once accepted and merged, both Alice and Bob can clean up their local repositories.
