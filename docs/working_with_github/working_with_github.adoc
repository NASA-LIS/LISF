...
There are many instances of `git clone` commands in this document.  Consider these to be *examples*.  Please see Section <<sec_step3>> for instructions regarding the actual `git clone` command that you should run.
...
anchor:sec_step3[Clone the repository in your GitHub work account onto your local machine]

The correct path to use may be obtained by clicking on the green "<> Code" button.

On your local machine run:

[subs="attributes+,-callouts"]
....
% git clone --recurse-submodules github:<your-user-name>/{lisf_git} <dir>
....

[NOTE]
====
The above clone command is required to get the NoahMP-5 physics. If you do not want NoahMP-5, run:
...
Then create a file named _lis/make/user.cfg_ and add the following line:
....
NoahMP.5.0: Off
....
====
...
//[subs="attributes+,-callouts"]
//....
//% git clone --recurse-submodules github:jvgeiger/{lisf_git}
//....
//====
