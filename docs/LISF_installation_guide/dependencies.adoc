...
==== Earth System Modeling Framework (ESMF) version 8.5.0 (or higher)
...
==== ecCodes version 2.32.0 (or higher)
...
==== NetCDF either version 3.6.3 or version 4.9.2 (or higher)

*URL*: http://www.unidata.ucar.edu/software/netcdf

Please read the on-line documentation for details on installing NetCDF.

NOTE: The use of NetCDF for building LISF is now deprecated.
...
We recommend that you compile NetCDF 4 with compression enabled.

An example of configuring NetCDF 4 without compression:

....
% ./configure --prefix=$HOME/local/netcdf-4.9.2 --disable-netcdf-4
....

An example of configuring NetCDF 4 with compression:

....
% CPPFLAGS=-I$HOME/local/hdf5/1.14.2/include \
> LDFLAGS=-L$HOME/local/hdf5/1.14.2/lib \
> ./configure --prefix=$HOME/local/netcdf/4.9.2 --enable-netcdf-4
....

You must also download the _netcdf-fortran-4.6.1.tar.gz_ file.  First install the NetCDF C library, then install the NetCDF Fortran library.  Again, please read the on-line documentation for more details.

An example of configuring the NetCDF 4 Fortran library:

....
% LD_LIBRARY_PATH=$HOME/local/netcdf/4.9.2/lib:$LD_LIBRARY_PATH \
> CPPFLAGS=-I$HOME/local/netcdf/4.9.2/include \
> LDFLAGS=-L$HOME/local/netcdf/4.9.2/lib \
> ./configure --prefix=$HOME/local/netcdf/4.9.2
....
...
===== HDF4 4.2.16-2 (or higher)

If you choose to have HDF version 4 support, please download the HDF source from https://www.hdfgroup.org/solutions/hdf4/ and compile the source to generate the HDF library.  Make sure that you configure the build process to include the Fortran interfaces by adding the `--enable-fortran` option to the `configure` command.

WARNING: HDF4 contains its own embedded version of NetCDF.  You must disable this support by adding the `--disable-netcdf` option to the `configure` command.


An example of configuring the HDF4 library:

....
% ./configure --prefix=$HOME/local/hdf4/4.2.16-2 --enable-fortran --disable-netcdf
....
...
==== HDF-EOS2 version 3.0 (or higher)

*URL*: http://hdfeos.org/software/library.php

If you choose to have HDF-EOS2 support, make sure that you configure the build process to include the Fortran interfaces by adding the `--enable-fortran` option to the `configure` command.

An example of configuring the HDF-EOS2 library:

....
% ./configure CC="$HOME/local/hdf4/4.2.16-2/bin/h4cc -Df2cFortran" F77=$HOME/local/hdf4/4.2.16-2/bin/h4fc --prefix=$HOME/local/hdfeos2/3.0 --with-hdf4=$HOME/local/hdf4/4.2.16-2 --enable-fortran 
....

NOTE: This library depends on HDF4.

==== GDAL version 3.7.2 (or higher)

NOTE: LDT and LVT dependency

*URL*: https://gdal.org

IMPORTANT: When installing the GDAL library, you must also install FortranGIS because this library provides Fortran interfaces to the GDAL library.

==== GeoTIFF version 1.7.1 (or higher)
...
==== FortranGIS 3.0-1 (or higher)

*URL*: http://fortrangis.sourceforge.net/

NOTE: Required by GDAL.

==== SQLite3 version 3.43.2 (or higher)
...
. PROJ (optional) +
  Required to use both GeoTIFF and GDAL.
. GeoTIFF (optional)
. GDAL (optional)
. FortranGIS (optional) +
