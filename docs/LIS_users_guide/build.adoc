...
| `LIS_MAPL`      | path to MAPL library             | optional (required to use PFIO)
| `LIS_FLAP`      | path to FLAP library             | optional (required to use PFIO)
...
* for `Use PFIO to produce nc4 HISTORY? (1-yes, 0-no, default=0):`, PFIO requires MPI.
