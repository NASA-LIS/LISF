...
Set the appropriate LIS environment variables.  These are used to control the compilation of LIS and to specify the locations of the required and optional libraries.

|====
| Variable        | Description                      | Usage

| `LIS_ARCH`      | Architecture                     | required
...
| `LIS_HDF4`      | path to HDF4 library             | optional (required to use HDFEOS2)
...
//{cpp} is C++
Note that the `LIS_CC` variable must be set to a C compiler, not a {cpp} compiler.  A {cpp} compiler may mangle internal names in a manner that is not consistent with the Fortran compiler.  This will cause errors during linking.
...
It is suggested that you set these environment variables in a modulefile footnote:modulefile[See the "`Creating a Custom Modulefile`" document found at {nasalisfpages}] to load or in an environment script to source before compiling and/or running LIS.
...
This script will prompt the user with a series of questions regarding support to compile into LIS.
...
NETCDF use deflate level? (1 to 9-yes, 0-no, default = 1):
