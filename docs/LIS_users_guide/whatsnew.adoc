...
NOTE: There were no public releases for versions 7.6 or 7.7.
...
. Includes NoahMP 3.6
. Includes CABLE 1.4b
. Includes Flake 1.0 {emdash} #internal use only!# (available only in development branch)
. Includes flood irrigation
. Includes drip irrigation
. Supports SMOS L2 soil moisture observations (available only in development branch)
. Supports simulated GRACE products (available only in development branch)
. Supports GCOMW AMSR2 L3 soil moisture observations (available only in development branch)
...
. Supports LDT-generated met forcing data (available only in development branch)
