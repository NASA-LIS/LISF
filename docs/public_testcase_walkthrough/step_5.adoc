...
A new directory named `RS_DATA` contains sample soil moisture observations from the link:https://nsidc.org/data/SPL3SMP/versions/5[_SMAP L3 Radiometer Global Daily 36 km EASE-Grid Soil Moisture, Version 5, window="_blank"]_ data product.
...
<7> Specifies the radius in which LDT search to detect open water Then removes all pixels within the radius in the CDF calculations.
