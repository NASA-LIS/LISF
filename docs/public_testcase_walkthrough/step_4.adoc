...
:step-label: Step 4
:download-filename: walkthrough_step4.tar.gz
:input-filesize-compressed: 366KB
:input-filesize-unpacked: 805KB
:ldt-run-dir: DA_proc_LSM
:ldt-config-filename: ldt.config.step4.noah36_cdf
...
|`target_step4/`                  |A directory containing the files below
...
|`target_cdf_noah36.xdf`          |GrADS description file for viewing `*cdf_noah36.nc` output and target files
...
Review the contents of `{ldt-config-filename}` to view the
...
LIS soil moisture output directory:         ./run_step2
...
During this run LDT produced a domain file, `cdf_noah36_domain.nc`, and a LSM CDF file,  `cdf_noah36.nc` in the `run_step4` directory. If GrADS is installed, use the provided GrADS descriptor files to plot the output shown below.
