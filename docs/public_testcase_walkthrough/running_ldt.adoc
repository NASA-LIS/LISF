...
NOTE: If the terminal reports the error "./LDT: symbol lookup error:", this may be due extraneous modules loaded into your environment that do not allow LIS to run properly. View your modules using the command `module list`.  If extraneous modules are loaded, run `module purge` to clear your environment and then `module load [lisf module file]` to load an environment suitable to running LISF.  See the "discover_quick_start" document in the {url-lisf-docs}[LISF Users`' Guides,window=_blank] for more information.
