...
Set the appropriate LVT environment variables.  These are used to control the compilation of LVT and to specify the locations of the required and optional libraries.

[cols="<,<,<",]
|===
| Variable         | Description                | Usage

| `LVT_ARCH`       | Architecture               | required
...
//{cpp} is C++
Note that the `LVT_CC` variable must be set to a C compiler, not a {cpp} compiler.  A {cpp} compiler may mangle internal names in a manner that is not consistent with the Fortran compiler.  This will cause errors during linking.
...
It is suggested that you set these environment variables in a modulefile footnote:modulefile[See the "`Creating a Custom Modulefile`" document found at {nasalisfpages}] to load or in an environment script to source before compiling and/or running LVT.
...
This script will prompt the user with a series of questions regarding support to compile into LVT.
