...
!BOP
! !ROUTINE: HYMAP2_updateSWOT
!  \label{HYMAP2_updateSWOT}
!
! !REVISION HISTORY:
! 15 Apr 24: Yeosang Yoon; Initial specification;
!                          copied from HYMAP2_updateWL
!
! !INTERFACE:
subroutine HYMAP2_updateSWOT(n, Routing_State, Routing_Incr_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  real, allocatable      :: checkWeight(:,:)
  integer                :: max_index
  real                   :: thresh

  call ESMF_StateGet(Routing_State,"Surface elevation",sfcelevField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateGet: Surface elevation failed in HYMAP2_updateSWOT")

  call ESMF_FieldGet(sfcelevField,localDE=0,farrayPtr=sfcelev,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Surface elevation failed in HYMAP2_updateSWOT")
  ...
  allocate(checkWeight(HYMAP2_routing_struc(n)%nseqall*LIS_rc%nensem(n), &
           HYMAP2_daSWOT_struc(n)%localupdDX*HYMAP2_daSWOT_struc(n)%localupdDX))
  sfcelevIncr_tmp = 0.0
  nsfcelevIncr_tmp = 0
  checkWeight = 0.0
  ...
  do i=1,HYMAP2_routing_struc(n)%nseqall
     do m=1,LIS_rc%nensem(n)
        ...
        if(abs(sfcelevIncr(t))>=10.0) then
           sfcelevIncr(t) = 0.0
           cycle
        elseif (abs(sfcelevIncr(t))>thresh) then
           sfcelevIncr(t) = MAX(-thresh, MIN(sfcelevIncr(t), thresh))
        endif
        ...
        if (HYMAP2_daSWOT_struc(n)%useLocalUpd.eq.1) then
           if(abs(sfcelevIncr(t)).gt.0) then
              ...
              siteid = HYMAP2_daSWOT_struc(n)%sites(ix,iy)
              if(siteid.ne.-9999.0) then
                 localweight(:,:) = &
                 ...
                 c1=max(1,ix-HYMAP2_daSWOT_struc(n)%localupdDX)
                 c2=min(LIS_rc%lnc(n),ix+HYMAP2_daSWOT_struc(n)%localupdDX)
                 r1=max(1,iy-HYMAP2_daSWOT_struc(n)%localupdDX)
                 r2=min(LIS_rc%lnr(n),iy+HYMAP2_daSWOT_struc(n)%localupdDX)
  ...
  do i=1,HYMAP2_routing_struc(n)%nseqall
     do m=1,LIS_rc%nensem(n)
        t = (i-1)*LIS_rc%nensem(n)+m
        if(abs(sfcelevIncr_tmp(t)).gt.0) then
...
end subroutine HYMAP2_updateSWOT
