...
!  subroutine HYMAP3_resop_main(resoptype,mis,nz,time,dt,rivsto,fldsto,runoff,&
!                        outflow,inflow_down,rivelv_down,rivdph_down,rivvel,fldvel,&
!                        elevtn,fldhgt,fldstomax,grarea,rivstomax,rivelv,rivlen,rivwth,inflow,&
!                        ntresop,tresopalt,resopalt,resopoutmin)
    use LIS_logMod
    implicit none
    ...
    if(resoptype==1)then
      ...
      call HYMAP3_resop_inter(time,ntresop,tresopalt,dble(resopalt),mis,elvobs) 
      
      !calculate water volume as a function of observed elevation
      call HYMAP3_get_volume_profile(nz,dble(elevtn),dble(fldhgt),dble(fldstomax),dble(grarea),dble(rivstomax),&
      ...
      call HYMAP3_get_elevation_profile(nz,dble(elevtn),dble(fldhgt),dble(fldstomax),dble(grarea),dble(rivstomax),&
    ...
    elseif(resoptype==2)then
      ...
      write(LIS_logunit,*) 'Please check reservoir operation type'
      write(LIS_logunit,*) 'Ending run at HYMAP3_resop_main'
      call LIS_endrun()
    endif
