...
!EOP
  ...
  do n=1, LIS_rc%nnest
     ...
     if(read_restart) then 
        ...
        call LIS_verify(status, "Error in nf90_close in HYMAP3_routing_readrst")
#endif
     endif
  enddo
!stop
...
!EOP
    ...
    call LIS_verify(status,'Error in nf90_inq_varid in HYMAP3_readvar_restart')
    status = nf90_get_var(ftn,varid,gtmp)
    call LIS_verify(status,'Error in nf90_get_var in HYMAP3_readvar_restart')
#endif       
...
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
    status = nf90_inq_varid(ftn,trim(varname),varid)
    call LIS_verify(status,'Error in nf90_inq_varid in HYMAP3_readvar_restart_ens')
    status = nf90_get_var(ftn,varid,gtmp)
    call LIS_verify(status,'Error in nf90_get_var in HYMAP3_readvar_restart_ens')
#endif       
    do i=1,LIS_rc%nroutinggrid(n)
...
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
    status = nf90_inq_varid(ftn,trim(varname),varid)
    call LIS_verify(status,'Error in nf90_inq_varid in HYMAP3_readbif_restart')
    status = nf90_get_var(ftn,varid,var)
    call LIS_verify(status,'Error in nf90_get_var in HYMAP3_readbif_restart')
#endif       
