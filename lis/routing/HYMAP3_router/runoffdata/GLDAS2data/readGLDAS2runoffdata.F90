...
! !USES:
  ...
  if(trim(GLDAS2runoffdata_struc(n)%previous_filename)/=trim(filename))then
     ...
     if(file_exists) then
        ...
        call LIS_verify(ios, &
             'failed to inquire Qs_acc field in readGLDAS2runoffdata')

        ios = nf90_inq_varid(nid,'Qsb_acc',qsbid)
        call LIS_verify(ios, &
             'failed to inquire Qsb_acc field in readGLDAS2runoffdata')

        ios = nf90_get_var(nid,qsid,GLDAS2runoffdata_struc(n)%qs)
        call LIS_verify(ios, &
             'failed to read Qs_acc field in readGLDAS2runoffdata')

        ios = nf90_get_var(nid,qsbid,GLDAS2runoffdata_struc(n)%qsb)
        call LIS_verify(ios, &
             'failed to read Qsb_acc field in readGLDAS2runoffdata')

        ios = nf90_get_att(nid,qsid,'missing_value',undef)
        call LIS_verify(ios, &
             'failed to read missing_value in readGLDAS2runoffdata')
           ...
           do r=1, nr
              do c=1, nc
                 if ( GLDAS2runoffdata_struc(n)%qs(c,r) .ne. undef ) then
                    GLDAS2runoffdata_struc(n)%qs(c,r) = &
                         GLDAS2runoffdata_struc(n)%qs(c,r) / &
                         GLDAS2runoffdata_struc(n)%outInterval
                    GLDAS2runoffdata_struc(n)%qsb(c,r) = &
                         GLDAS2runoffdata_struc(n)%qsb(c,r) / &
                         GLDAS2runoffdata_struc(n)%outInterval
                 endif
              end do
     ...
     call interp_GLDAS2runoffdata(n,nc,nr,GLDAS2runoffdata_struc(n)%qs, &
          surface_runoff)
     call interp_GLDAS2runoffdata(n,nc,nr,GLDAS2runoffdata_struc(n)%qsb,&
          baseflow)
...
subroutine readGLDAS2evapdata(n, evap, potevap)
  ...
  if(file_exists) then
     ...
     call LIS_verify(ios, &
          'failed to inquire Evap_tavg field in readGLDAS2evapdata')

     ios = nf90_inq_varid(nid,'PotEvap_tavg',potevpid)
     call LIS_verify(ios, &
          'failed to inquire PotEvap_tavg field in readGLDAS2evapdata')

     ios = nf90_get_var(nid,evpid,evp)
     call LIS_verify(ios, &
          'failed to read Evap_tavg field in readGLDAS2evapdata')

     ios = nf90_get_var(nid,potevpid,potevp)
     call LIS_verify(ios, &
          'failed to read PotEvap_tavg field in readGLDAS2evapdata')

     ios = nf90_get_att(nid,evpid,'missing_value',undef)
     call LIS_verify(ios,&
          'failed to read missing_value in readGLDAS2evapdata')
