...
!EOP
  ...
  if(file_exists) then
     ...
     ios = nf90_open(path=trim(filename),mode=NF90_NOWRITE, &
          ncid = ftn)
     if(ios.ne.0) then
        write(LIS_logunit,*) '[ERR] Opening ERAILand file ', &
             trim(filename),&
             'failed'
        call LIS_endrun()
     endif
     ...
     if(hr_elapsed.ne.time(1)) then
        write(LIS_logunit,*) &
             '[ERR] The starting time in the ERA interim fcst file'
        write(LIS_logunit,*) &
             '[ERR] do not match the computed starting time'
        call LIS_endrun()
     endif
  ...
  call interp_ERAILandrunoffdata(n,nc,nr,qs,scale_qs,offset_qs, &
       surface_runoff)
  call interp_ERAILandrunoffdata(n,nc,nr,qsb,scale_qsb, offset_qsb, &
       baseflow)
