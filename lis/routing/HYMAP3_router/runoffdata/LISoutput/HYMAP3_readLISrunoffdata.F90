...
!  real                         :: total_evapotranspiration(LIS_rc%lnc(n),LIS_rc%lnr(n))
  ...
  !ag - 17Mar2016
  real                  :: qs2d(HYMAP3_LISrunoffdata_struc(n)%nc,HYMAP3_LISrunoffdata_struc(n)%nr)
  real                  :: qsb2d(HYMAP3_LISrunoffdata_struc(n)%nc,HYMAP3_LISrunoffdata_struc(n)%nr)
  logical*1             :: lb(HYMAP3_LISrunoffdata_struc(n)%nc*HYMAP3_LISrunoffdata_struc(n)%nr)
  real                  :: var_input(HYMAP3_LISrunoffdata_struc(n)%nc*HYMAP3_LISrunoffdata_struc(n)%nr)
  ...
  if(trim(HYMAP3_LISrunoffdata_struc(n)%previous_filename)/=trim(filename))then
    ...
    if(file_exists) then 
      ...
      call LIS_verify(ios,'Error opening file in HYMAP3_readLISrunoffdata')
     
      ios = nf90_inq_varid(nid,'Qs_tavg',qsid)
      call LIS_verify(ios,'failed to read Qs_tavg field in HYMAP3_readLISrunoffdata')
     
      ios = nf90_inq_varid(nid,'Qsb_tavg',qsbid)
      call LIS_verify(ios,'failed to read Qsb_tavg field in HYMAP3_readLISrunoffdata')
      ...
      if(HYMAP3_LISrunoffdata_struc(n)%domainCheck) then 
         if(LIS_rc%wopt.eq."2d gridspace") then 
            ios = nf90_get_var(nid,qsid,HYMAP3_LISrunoffdata_struc(n)%qs, &
                 start=(/LIS_ews_halo_ind(n,LIS_localPet+1),LIS_nss_halo_ind(n,LIS_localPet+1)/),&
                 count=(/LIS_rc%lnc(n),LIS_rc%lnr(n)/))
            call LIS_verify(ios, 'failed to read Qs_tavg field in HYMAP3_readLISrunoffdata')
            
            ios = nf90_get_var(nid,qsbid,HYMAP3_LISrunoffdata_struc(n)%qsb,&
                 start=(/LIS_ews_halo_ind(n,LIS_localPet+1),LIS_nss_halo_ind(n,LIS_localPet+1)/),&
                 count=(/LIS_rc%lnc(n),LIS_rc%lnr(n)/))
            call LIS_verify(ios, 'failed to read Qsb_tavg field in HYMAP3_readLISrunoffdata')
            
            !ios = nf90_get_var(nid,evapid,HYMAP3_LISrunoffdata_struc(n)%evap,&
            !     start=(/LIS_ews_halo_ind(n,LIS_localPet+1),LIS_nss_halo_ind(n,LIS_localPet+1)/),&
            !     count=(/LIS_rc%lnc(n),LIS_rc%lnr(n)/))
            !call LIS_verify(ios, 'failed to read Evap_tavg field in readLISrunoffdata')
         else
         ...
         if(LIS_rc%wopt.eq."2d gridspace") then
            ...
            if(LIS_isAtAfinerResolution(n,HYMAP3_LISrunoffdata_struc(n)%datares)) then

               lb = .true. 
               do r=1,HYMAP3_LISrunoffdata_struc(n)%nr
                  do c=1,HYMAP3_LISrunoffdata_struc(n)%nc
                     var_input(c+(r-1)*HYMAP3_LISrunoffdata_struc(n)%nc) = qs2d(c,r)
                     if(qs2d(c,r).lt.0) then
                        lb(c+(r-1)*HYMAP3_LISrunoffdata_struc(n)%nc) = .false.
                     endif
                  enddo
               enddo
               ...
               do r=1,LIS_rc%lnr(n)
                  do c=1,LIS_rc%lnc(n)
                     HYMAP3_LISrunoffdata_struc(n)%qs(c,r) = var_out(c+(r-1)*LIS_rc%lnc(n))
                  enddo
               enddo
               ...
               do r=1,HYMAP3_LISrunoffdata_struc(n)%nr
                  do c=1,HYMAP3_LISrunoffdata_struc(n)%nc
                     var_input(c+(r-1)*HYMAP3_LISrunoffdata_struc(n)%nc) = qsb2d(c,r)
               ...
               do r=1,LIS_rc%lnr(n)
                  do c=1,LIS_rc%lnc(n)
                     HYMAP3_LISrunoffdata_struc(n)%qsb(c,r) = var_out(c+(r-1)*LIS_rc%lnc(n))
                  enddo
               enddo
  ...
  where(HYMAP3_LISrunoffdata_struc(n)%qs/=LIS_rc%udef)
    ...
    !total_evapotranspiration = HYMAP3_LISrunoffdata_struc(n)%evap
  else where
    surface_runoff = 0.0
    baseflow = 0.0
