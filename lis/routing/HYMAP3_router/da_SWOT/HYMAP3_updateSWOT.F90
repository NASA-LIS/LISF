...
!EOP
  ...
  allocate(checkWeight(HYMAP3_routing_struc(n)%nseqall*LIS_rc%nensem(n), &
           HYMAP3_daSWOT_struc(n)%localupdDX*HYMAP3_daSWOT_struc(n)%localupdDX))
  sfcelevIncr_tmp = 0.0
  nsfcelevIncr_tmp = 0
  checkWeight = 0.0
     ...
     do m=1,LIS_rc%nensem(n)
        ...
        if (HYMAP3_daSWOT_struc(n)%useLocalUpd.eq.1) then
           if(abs(sfcelevIncr(t)).gt.0) then
              ...
              siteid = HYMAP3_daSWOT_struc(n)%sites(ix,iy)
              if(siteid.ne.-9999.0) then
                 ...
                 c2=min(LIS_rc%lnc(n),ix+HYMAP3_daSWOT_struc(n)%localupdDX)
                 r1=max(1,iy-HYMAP3_daSWOT_struc(n)%localupdDX)
                 r2=min(LIS_rc%lnr(n),iy+HYMAP3_daSWOT_struc(n)%localupdDX)
