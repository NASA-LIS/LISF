...
!BOP
! !ROUTINE: HYMAP3_getSWOT
! \label{HYMAP3_getSWOT}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine HYMAP3_getSWOT(n, Routing_State)
...
!EOP
  ...
  call LIS_verify(status,'ESMF_StateGet failed for sfcelev in HYMAP3_getSWOT')

  call ESMF_FieldGet(sfcelevField,localDE=0,farrayPtr=sfcelev,rc=status)
  call LIS_verify(status,'ESMF_FieldGet failed for sfcelev in HYMAP3_getSWOT')
