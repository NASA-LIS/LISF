...
contains
  ! ================================================               
  subroutine HYMAP3_manag_rules(time,managtype,nmanagcoef,managqmax,managact,managcoef,rivout,rivinf,rivinf1)
    use LIS_logMod
    implicit none
...
!      !single linear equation (ax + b)
    ...
    !two polynomial equations (ax2 + bx + c) used as a function of simulated peak discharge
    elseif(managtype==2)then
      if(rivout>=managqmax)then
          zdistmp=min(max(managcoef(1)*(rivout**2)+managcoef(2)*rivout+managcoef(3),managcoef(7)),managcoef(8))
      else
          zdistmp=min(max(managcoef(4)*(rivout**2)+managcoef(5)*rivout+managcoef(6),managcoef(7)),managcoef(8))
      endif
      managqmax=rivout
      zdistmp=max(0.,min(zdistmp,rivinf))
      rivinf=max(0.,rivinf-zdistmp)
    ...
    !this option considers that the water use doesn't go to another grid point
  ...
  !=============================================
  subroutine HYMAP3_get_management_rules(managheader,nx,ny,sindex,nmanag,nmanagcoef,managloc,managqmax,managact,managtype,managcoef)
    use LIS_logMod
    implicit none
