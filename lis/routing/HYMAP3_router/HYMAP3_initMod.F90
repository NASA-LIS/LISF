...
!#if 0
  ...
  subroutine HYMAP3_set_fldstg(nz, nseqall, fldhgt, areamat, rivlen, &
    ...
    do iseq=1,nseqall
       ...
       do i=1,nz
          stonow=rivlen(iseq)*(rivwth(iseq)+wthinc*(real(i)-0.5)) * &
               (fldhgt(iseq,i)-hgtpre)
       ...
       do i=1,nz
          ...
          if(levhgt(iseq)>0.and.fldhgt(iseq,i)>levhgt(iseq).and. &
               flag==0)then
             flag=1
             hfrac=(levhgt(iseq)-hgtpre)/(fldhgt(iseq,i)-hgtpre)
             sfrac=rivlen(iseq)*(wthinc*(real(i)-1.+hfrac))* &
                  (levhgt(iseq)-hgtpre)
             fldstoatlev(iseq)=stopre+sfrac
          endif
          !get maximum floodplain-only storage
          stonow=rivlen(iseq)*(wthinc*(real(i)-0.5))* &
               (fldhgt(iseq,i)-hgtpre)
