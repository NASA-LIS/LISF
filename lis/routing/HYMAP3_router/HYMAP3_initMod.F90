...
contains
  ! ================================================          
  ! ================================================              
  subroutine HYMAP3_grid2vector(nx,ny,nz,nseqall,imis,seqx,seqy,grid,vector)
    implicit none
  ...
  ! ================================================          
  ! ================================================              
  subroutine HYMAP3_vector2grid(nx,ny,nz,nseqall,imis,seqx,seqy,grid,vector)
    implicit none
    integer, intent(in)  :: nx             !number of grids in horizontal
    integer, intent(in)  :: ny             !number of grids in vertical
    integer, intent(in)  :: nz             !number of array layers
  ...
  ! ================================================          
  ! ================================================              
  subroutine HYMAP3_get_vector_size(nx,ny,nxg,nyg,offx,offy,imis,nextx,mask,nseqall)
    implicit none
    integer, intent(in)  ::  nx                !number of grids in horizontal
    integer, intent(in)  ::  ny                !number of grids in vertical
...
!#if 0        
  ...
  subroutine HYMAP3_set_fldstg(nz,nseqall,fldhgt,areamat,rivlen,&
    ...
    real,    intent(out) :: fldonlystomax(nseqall,nz) !maximum floodplain-only storage - excludes river storage [m3]
    real,    intent(out) :: fldstoatlev(nseqall)  !isolated floodplain storage at levee elevation - excludes river storage [m3]
    ...
    do iseq=1,nseqall
      ...
      flag=0
      do i=1,nz
        !get floodplain-only storage at levee height
        if(levhgt(iseq)>0.and.fldhgt(iseq,i)>levhgt(iseq).and.flag==0)then
          flag=1
          hfrac=(levhgt(iseq)-hgtpre)/(fldhgt(iseq,i)-hgtpre)
          sfrac=rivlen(iseq)*(wthinc*(real(i)-1.+hfrac))*(levhgt(iseq)-hgtpre)
          fldstoatlev(iseq)=stopre+sfrac
        endif
        !get maximum floodplain-only storage
        stonow=rivlen(iseq)*(wthinc*(real(i)-0.5))*(fldhgt(iseq,i)-hgtpre)
        fldonlystomax(iseq,i)=stopre+stonow
        hgtpre=fldhgt(iseq,i)
        stopre=fldonlystomax(iseq,i)
      enddo
   enddo

 end subroutine HYMAP3_set_fldstg

end module HYMAP3_initMod
