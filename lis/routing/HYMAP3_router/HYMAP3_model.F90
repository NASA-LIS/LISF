  ...
  elseif(steptype==2)then
    do ic=1,nseqall
      ...
      if((flowtype==0.or.flowtype==4).and.(flowmap(ic)==1.or.flowmap(ic)==5))then
        ...
        call HYMAP3_dynstp(dt,nxtdst(ic),rivdph(ic),grv,cadp,dta(ic),nt(ic))
        if(dta(ic)>dt)then
          dta(ic)=dt
            nt(ic)=1
        endif
...
!find the maximum of i and minimum dt across all processors
  ...
  do it=1,maxi
    !call HYMAP3_date2frac(yr,mo,da,hr,mn,ss,real(dta(i)*(it-1)-dt),time)
  ...
  HYMAP3_routing_struc(n)%dt_proc=HYMAP3_routing_struc(n)%dt_proc+real(countf-counti)/real(count_rate)
  tmp=minval(dtaout)
#endif
