...
! 20 April 2017
! NASA GSFC
! This routine computes ET based on the Penman-Monteith equation, as
! presented in Shuttleworth (1993)
! Shuttleworth, W.J., 1993. Evaporation. In: Maidment, D.R., Handbook of
...
module HYMAP3_evapMod

  public :: HYMAP3_evap_main
...
contains
  ...
  subroutine HYMAP3_evap_main(evaptype, n, nseqall, mis, outlet, pres, &
    ...
    do ic=1,nseqall
       ...
       if(evaptype==1)then
          call HYMAP3_penman_monteith(pres(ic),tair(ic),wind(ic), &
               qair(ic),qnet(ic),ewat(ic))
       else
          write(LIS_logunit,*) &
               "[ERR] HYMAP routing model evaporation flag: unknown value"
          call LIS_endrun
       endif
  ...
  end subroutine HYMAP3_penman_monteith
  !=======================================================================
end module HYMAP3_evapMod
