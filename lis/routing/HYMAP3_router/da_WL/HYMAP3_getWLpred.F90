...
!BOP
! !ROUTINE: HYMAP3_getWLpred
! \label{HYMAP3_getWLpred}
!
! !REVISION HISTORY:
...
!EOP
  ...
  do i=1,HYMAP3_routing_struc(n)%nseqall
     do m=1,LIS_rc%nensem(n)
        ...
        wl(t) = HYMAP3_routing_struc(n)%sfcelv(i,m)
     enddo
  enddo
...
end subroutine HYMAP3_getWLpred

!BOP
! !ROUTINE: HYMAP3_convertPatchSpaceToObsEnsSpace
...
!  ensemble grid space. If the observation space is at a coarser
!  resolution than the patch space, then the variable is upscaled. On the
!  other hand, the variable is spatially interpolated to the observation
!  space. These transformations are done separately for each ensemble
!  member.
!
! The arguments are:
!  \begin{description}
...
!EOP
  ...
  real                         :: &
       obs_gvar(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k))
  integer                      :: iret
  ...
  do m=1,LIS_rc%nensem(n)
     ...
     do i=1,HYMAP3_routing_struc(n)%nseqall
        t = (i-1)*LIS_rc%nensem(n)+m
        c = HYMAP3_routing_struc(n)%seqx(i)
        r = HYMAP3_routing_struc(n)%seqy(i)
...
end subroutine HYMAP3_convertPatchSpaceToObsEnsSpace
