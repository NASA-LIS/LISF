...
! ================================================
  ...
  if(dwiflag==1)then
    do ic=1,nseqall 
      call HYMAP3_dwi(runoff0(ic),basflw0(ic),grarea(ic),rnfdwi_ratio(ic),&
 ...
 ! ================================================
  do ic=1,nseqall 
    ...
    call HYMAP3_calc_fldstg_lev(nz,grarea(ic),rivlen(ic),rivwth(ic),rivstomax(ic),&
              fldstomax(ic,:),fldhgt(ic,:),rivare(ic),levhgt(ic),levstomax(ic),&
              fldonlystomax(ic,:),fldstoatlev(ic),elevtn(ic),&
              rivsto(ic),fldsto(ic),rivdph(ic),flddph(ic),flddph1(ic),fldelv1(ic),&
  ...
  do ic=1,nseqall
    ...
    elseif(outlet(ic)>0)then
      ...
      fldelv1_down=rivelv_down !fldelv1(ic)-slpmin*nxtdst(ic)
      rivdph_pre_down=max(0.,-rivelv_down) !rivdph_pre(ic)+slpmin*nxtdst(ic)
      flddph_pre_down=rivdph_pre_down !flddph_pre(ic)+slpmin*nxtdst(ic)
      rivdph_down=max(0.,-rivelv_down) !rivdph(ic)+slpmin*nxtdst(ic)
      flddph1_down=rivdph_down !flddph1(ic)+slpmin*nxtdst(ic)
      rivsto_down=1e20
      fldsto_down=1e20
      ! ================================================
      ...
      if(HYMAP3_routing_struc(n)%sealevelflag==1.and.&
         HYMAP3_routing_struc(n)%outletid(ic)/=HYMAP3_routing_struc(n)%imis)then
    ...
    if(flowmap(ic)==1)then
      ...
      call HYMAP3_calc_rivout_kine(outlet(ic),dt,rivelv(ic),rivelv_down,nxtdst(ic),&
      ...
      if(floodflag==1)then
        ...
        call HYMAP3_calc_rivout_kine(outlet(ic),dt,fldelv1(ic),fldelv1_down,&
      ...
      elseif(floodflag==0)then
        ...
        call HYMAP3_no_floodplain_flow(fldelv1(ic),flddph1(ic),fldout(ic),&
             fldvel(ic),sfcelv0(ic),fldout_pre(ic),flddph_pre(ic))
      else
        write(LIS_logunit,*)"[ERR] HYMAP3 floodplain dynamics: unknown value"
        call LIS_endrun()
      endif
    !ag (27Apr2020)
    elseif(flowmap(ic)==2.or.flowmap(ic)==3)then
      !ag(03Aug2022) Add option to merge the computation of river and floodplain dynamics
      if(floodflag==2)then
        call HYMAP3_merge_river_floodplain(rivsto(ic),rivman(ic),rivdph(ic),rivwth(ic),&
             fldsto(ic),fldman(ic),flddph1(ic),rivlen(ic),rivdph_down,rivsto_down,fldsto_down,&
             tmpsto,tmpman,tmpdph,tmpwth,tmpsto_down,tmpdph_down)
        !Calculate river flow based on the local inertia wave equation
        call HYMAP3_calc_rivout_iner(time,ic,2,rslpmax,bckslpmax,mis,outlet(ic),dt,rivelv(ic),rivelv_down,&
        ...
        !Calculate river flow based on the local inertia wave equation
        call HYMAP3_calc_rivout_iner(time,ic,1,rslpmax,bckslpmax,mis,outlet(ic),dt,rivelv(ic),rivelv_down,&
        ...
        if(floodflag==1)then
          ...
          call HYMAP3_calc_rivout_iner(time,ic,2,fslpmax,-fslpmax,rivout(ic),outlet(ic),dt,fldelv1(ic),fldelv1_down,&
    ...
    elseif(flowmap(ic)==5)then
      ...
      write(LIS_logunit,*)"HYMAP3 routing method: unknown value",ic,flowmap(ic)
      call LIS_endrun()
    endif
  enddo
! ================================================
  ...
  if(HYMAP3_routing_struc(n)%bifflag==1)then
    ...
    do ibif=1,HYMAP3_routing_struc(n)%nbif
      ...
      do ielv=1,HYMAP3_routing_struc(n)%nbifelv
        ...
        if(HYMAP3_routing_struc(n)%bifwth(ibif,ielv)>0.and.sfcelv1>bifelv1)then
          bifsto1=max(0.,rivsto_glb(icg)+fldsto_glb(icg)-HYMAP3_routing_struc(n)%bifsto(ibif,ielv)-&
             (rivout_glb(icg)+fldout_glb(icg)+HYMAP3_routing_struc(n)%bifout(ibif))*dt)
          bifsto1_down=max(0.,rivsto_glb(icg_down)+fldsto_glb(icg_down)-(max(0.,rivout_glb(icg_down))+&
             max(0.,fldout_glb(icg_down)))*dt)
          call HYMAP3_calc_bifout_iner(dt,rivelv_glb(icg),rivelv_glb(icg_down),& 
          ...
          HYMAP3_routing_struc(n)%bifout(ibif)=HYMAP3_routing_struc(n)%bifout(ibif)+bifout1
        endif
      enddo
      rivout_glb(icg)=rivout_glb(icg)+HYMAP3_routing_struc(n)%bifout(ibif)
      rivinf_glb(icg_down)=rivinf_glb(icg_down)+HYMAP3_routing_struc(n)%bifout(ibif)
    enddo
#if (defined SPMD)
  ...
  if(flowtype==4)then
    ...
    do ic=1,nseqall
      if(flowmap(ic)==4)then
        call HYMAP3_calc_urban_drain_stonxt(dt,drvel,drtotwth(ic),drstomax(ic),drout(ic),drinf(ic),rivdph(ic),drsto(ic),rivsto(ic))
      endif
    enddo
  ...
  if(HYMAP3_routing_struc(n)%resopflag>0)then
    ...
    do iresop=1,HYMAP3_routing_struc(n)%nresop
      if(HYMAP3_routing_struc(n)%resoploc(iresop)==HYMAP3_routing_struc(n)%imis.and.&
         HYMAP3_routing_struc(n)%resoploc_dwn(iresop)==HYMAP3_routing_struc(n)%imis)cycle
      ...
      if(HYMAP3_routing_struc(n)%resoploc(iresop)/=HYMAP3_routing_struc(n)%imis)then
        ic=HYMAP3_routing_struc(n)%resoploc(iresop)
        rivout(ic)=rivout_glb(icg)
        fldout(ic)=fldout_glb(icg)
      endif
      if(HYMAP3_routing_struc(n)%resoploc_dwn(iresop)/=HYMAP3_routing_struc(n)%imis)then
  ...
  if(HYMAP3_routing_struc(n)%managflag==1)then
    ...
    do ic=1,HYMAP3_routing_struc(n)%nmanag          
      ...
      icg_down=HYMAP3_routing_struc(n)%next_glb(icg)
      rivout_glb(icg)=rivout_glb(icg)+fldout_glb(icg)
      rivinf_glb(icg_down1)=rivinf_glb(icg_down1)+fldinf_glb(icg_down1)
      rivinf_glb(icg_down)=rivinf_glb(icg_down)+fldinf_glb(icg_down)
      fldout_glb(icg)=0.
      fldinf_glb(icg_down1)=0.
      fldinf_glb(icg_down)=0.
      call HYMAP3_manag_rules(time,HYMAP3_routing_struc(n)%managtype(ic),&
                              ...
                              rivout_glb(icg),rivinf_glb(icg_down),rivinf_glb(icg_down1))
    enddo
    endif
  ...
  if(HYMAP3_routing_struc(n)%insertflag==1)then
    if(LIS_masterproc) then
      do icg=1,LIS_rc%glbnroutinggrid(n) !nseqall 
        !call HYMAP3_map_l2g_index(n, ic,icg)
        if(minval(abs(HYMAP3_routing_struc(n)%insertloc-icg))==0)then
          ...
          call HYMAP3_resop_inter(time,HYMAP3_routing_struc(n)%ntinsert,&
  ...
  do ic=1,nseqall 
    call HYMAP3_calc_stonxt(dt,rivout(ic),fldout(ic),rivinf(ic),fldinf(ic),rivsto(ic),fldsto(ic))
  enddo
  ...
  do ic=1,nseqall 
    if(outlet(ic)==mis)cycle
    call HYMAP3_calc_runoff(dt,fldfrc(ic),runoff(ic),rivsto(ic),fldsto(ic))
  enddo
