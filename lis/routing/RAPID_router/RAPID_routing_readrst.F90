...
subroutine RAPID_routing_readrst
  ...
  if(LIS_masterproc) then   
     do n=1, LIS_rc%nnest
        ...
        if(RAPID_routing_struc(n)%startmode.eq."restart") then !cold start
           read_restart = .true. 
        endif
        ...
        if(LIS_rc%runmode.eq."ensemble smoother") then 
           if(LIS_rc%iterationId(n).gt.1) then 
              ...
              if(RAPID_routing_struc(n)%rstInterval.eq.2592000) then 
        ...
        if(read_restart) then 
           ...
           open(ftn,file=trim(RAPID_routing_struc(n)%rstfile),&
                status='old',form='unformatted',&
                iostat = ios)
           if(ios.ne.0) then 
              write(LIS_logunit,*) '[ERR] File '//trim(RAPID_routing_struc(n)%rstfile)//& 
                   'does not exist '
              call LIS_endrun()
           endif
