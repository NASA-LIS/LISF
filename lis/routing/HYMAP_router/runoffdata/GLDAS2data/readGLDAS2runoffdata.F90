...
! convert units from kg m-2 3hr-1 to kg m-2 sec-1.
   if ( GLDAS2runoffdata_struc(n)%outInterval .eq. 10800 ) then
    do r=1, nr
     do c=1, nc
        if ( qs(c,r) .ne. undef ) then
          qs(c,r) = qs(c,r) / GLDAS2runoffdata_struc(n)%outInterval
          qsb(c,r) = qsb(c,r) / GLDAS2runoffdata_struc(n)%outInterval
        endif
     end do
    end do
   else
  ...
  call interp_GLDAS2runoffdata(n,nc,nr,qs,surface_runoff)
  call interp_GLDAS2runoffdata(n,nc,nr,qsb,baseflow)
