...
## Create directory structure:
   ...
   wget --no-check-certificate -r --accept "Public-Noah-3.3-HYMAP-OpenLoop_output_v72-dev.tar.gz" -l1 -nv ${file} -nH --cut-dirs=5 -a download_output.log 

   wget --no-check-certificate -r --accept "Public-Noah-3.3-HYMAP-DA_output_v72-dev.tar.gz" -l1 -nv ${file} -nH --cut-dirs=5 -a download_output.log 

  echo " -----  COMPLETED DOWNLOADING FILES FOR:: " $sourcedir " -----  "
  echo " -----  UNZIPPING FILES FOR:: " $sourcedir " -----  "

  gzip -dc Public-Noah-3.3-HYMAP-OpenLoop_output_v72-dev.tar.gz | tar xf -
  gzip -dc Public-Noah-3.3-HYMAP-DA_output_v72-dev.tar.gz | tar xf -
