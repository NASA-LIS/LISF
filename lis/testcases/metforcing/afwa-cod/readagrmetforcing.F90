...
!  stop
  if(LIS_rc%run_model) then
     ...
     do r=1,LIS_rc%lnr(n)
        do c=1,LIS_rc%lnc(n)
           if(LIS_domain(n)%gindex(c,r).ne.-1) then
              LIS_forc(n,findex)%metdata2(1,LIS_domain(n)%gindex(c,r)) = &
                   agrmet_struc(n)%sfctmp(dindex,c,r)
              LIS_forc(n,findex)%metdata2(7,LIS_domain(n)%gindex(c,r)) = &
                   agrmet_struc(n)%sfcprs(dindex,c,r)
              LIS_forc(n,findex)%metdata2(5,LIS_domain(n)%gindex(c,r)) = &
                   agrmet_struc(n)%sfcspd(dindex,c,r)
              LIS_forc(n,findex)%metdata2(6,LIS_domain(n)%gindex(c,r)) = 0.0
     ...
     do r=1,LIS_rc%lnr(n)
        do c=1,LIS_rc%lnc(n)
           if(LIS_domain(n)%gindex(c,r).ne.-1) then
              LIS_forc(n,findex)%metdata2(2,LIS_domain(n)%gindex(c,r)) = varfield(c,r)
           endif
        enddo
     enddo
     ...
     do r=1,LIS_rc%lnr(n)
        do c=1,LIS_rc%lnc(n)
           if(LIS_domain(n)%gindex(c,r).ne.-1) then
              LIS_forc(n,findex)%metdata2(4,LIS_domain(n)%gindex(c,r)) = longwv(c,r)
           endif
        enddo
     enddo
...
open(unit=666,file='trans.bin',access='direct',recl=LIS_rc%gnc(n)*LIS_rc%gnr(n)*4)
endif
call LIS_writevar_bin(666, n, t1, 1)
call LIS_writevar_bin(666, n, t2, 2)
call LIS_writevar_bin(666, n, t3, 3)
call LIS_writevar_bin(666, n, r1, 4)
call LIS_writevar_bin(666, n, r2, 5)
call LIS_writevar_bin(666, n, r3, 6)
...
#if 1
if ( LIS_localPet == 0 ) then
write(UNIT=filename,FMT='(a,i4,i2.2,i2.2,i2.2,a)') 'cod.',yr1,mo1,da1,hr1,'.bin'
open(unit=666,file=trim(filename),access='direct',recl=LIS_rc%gnc(n)*LIS_rc%gnr(n)*4)
endif
...
#if 0
if ( LIS_localPet == 0 ) then
open(unit=666,file='trans_cod_b4.bin',access='direct',recl=LIS_rc%gnc(n)*LIS_rc%gnr(n)*4)
endif
call LIS_writevar_bin(666, n, t1, 1)
call LIS_writevar_bin(666, n, t2, 2)
call LIS_writevar_bin(666, n, t3, 3)
...
#if 1
if ( LIS_localPet == 0 ) then
write(UNIT=filename,FMT='(a,i4,i2.2,i2.2,i2.2,a)') 'tr_b4.',yr1,mo1,da1,hr1,'.bin'
open(unit=666,file=trim(filename),access='direct',recl=LIS_rc%gnc(n)*LIS_rc%gnr(n)*4)
endif
call LIS_writevar_bin(666, n, t1, 1)
call LIS_writevar_bin(666, n, t2, 2)
...
#if 1
if ( LIS_localPet == 0 ) then
write(UNIT=filename,FMT='(a,i4,i2.2,i2.2,i2.2,a)') 'tr_af.',yr1,mo1,da1,hr1,'.bin'
open(unit=666,file=trim(filename),access='direct',recl=LIS_rc%gnc(n)*LIS_rc%gnr(n)*4)
endif
call LIS_writevar_bin(666, n, t1, 1)
call LIS_writevar_bin(666, n, t2, 2)
...
#if 1
if ( LIS_localPet == 0 ) then
write(UNIT=filename,FMT='(a,i4,i2.2,i2.2,i2.2,a)') 'tr.',yr1,mo1,da1,hr1,'.bin'
open(unit=666,file=trim(filename),access='direct',recl=LIS_rc%gnc(n)*LIS_rc%gnr(n)*4)
endif
call LIS_writevar_bin(666, n, t1, 1)
call LIS_writevar_bin(666, n, t2, 2)
...
#if 1
if ( LIS_localPet == 0 ) then
write(UNIT=filename,FMT='(a,i4,i2.2,i2.2,i2.2,a)') 'cldamt.',yr1,mo1,da1,hr1,'.bin'
open(unit=666,file=trim(filename),access='direct',recl=LIS_rc%gnc(n)*LIS_rc%gnr(n)*4)
endif
call LIS_writevar_bin(666, n, cldamt(1,:,:), 1)
call LIS_writevar_bin(666, n, cldamt(2,:,:), 2)
call LIS_writevar_bin(666, n, cldamt(3,:,:), 3)
call MPI_Barrier(LIS_mpi_comm, istat)
if ( LIS_localPet == 0 ) then
close(666)
endif
...
call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_TRANSMITTANCE,value=&
t1(c,r),vlevel=1,unit="-",direction="-",&
valid_min = 0.0, valid_max=1.0)

call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_TRANSMITTANCE,value=&
t2(c,r),vlevel=2,unit="-",direction="-",&
valid_min = 0.0, valid_max=1.0)

call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_TRANSMITTANCE,value=&
t3(c,r),vlevel=3,unit="-",direction="-",&
valid_min = 0.0, valid_max=1.0)

call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_REFLECTANCE,value=&
r1(c,r),vlevel=1,unit="-",direction="-",&
valid_min = 0.0, valid_max=1.0)

call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_REFLECTANCE,value=&
r2(c,r),vlevel=2,unit="-",direction="-",&
valid_min = 0.0, valid_max=1.0)

call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_REFLECTANCE,value=&
r3(c,r),vlevel=3,unit="-",direction="-",&
valid_min = 0.0, valid_max=1.0)
...
!</debug -- jim testing>
     ...
     do r=1,LIS_rc%lnr(n)
        do c=1,LIS_rc%lnc(n)
           if(LIS_domain(n)%gindex(c,r).ne.-1) then
              LIS_forc(n,findex)%metdata2(3,LIS_domain(n)%gindex(c,r)) = swdown(c,r)
           endif
        enddo
     enddo
