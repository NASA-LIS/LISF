...
!BOP
!
! !ROUTINE: read_SPORTgfrac
! \label{read_SPORTgfrac}
!
! !REVISION HISTORY:
...
!  30 Jul 2015: Ryan Ruhge: Modified to work with LIS 7.0, updated neighbor searching.
! !INTERFACE:
subroutine read_SPORTgfrac(n, wt1, wt2, array1, array2)
! !USES:
  use ESMF
...
!EOP      
  ...
  if (gfracAlarmCheck) then
     ...
     call get_SPORTgfrac(n,filename1,array1)
     call get_SPORTgfrac(n,filename2,array2)
...
end subroutine read_SPORTgfrac
...
! R. Ruhge (7/30/2015) -- variables for finding nearest neighbor
! Using Great-Circle distance equation, recommended to use 64-bit floating
! point variables to prevent roundoff error.
! https://en.wikipedia.org/wiki/Great-circle_distance
  real*8, parameter :: piover180=acos(-1.0)/180.0
  real, parameter :: rearth=6371.004
  real*8 :: dist, mindist, lat, lon, lat1, lon1, lati, loni, dlat, dlon

  external readgvfmodis
...
! J.Case (10/29/2014) Implemented reading of gzipped GVF files, as in LIS6.
  ...
  if (file_exists) then
    ...
    write (LIS_logunit,*) "Successful readgvfmodis; gvf_nc / gvf_nr / npts = ",gvf_nc,gvf_nr,npts
...
! J.Case (10/29/2014) -- end of mods to read gzipped GVF file.
  ...
  lati = LIS_domain(n)%lisproj%lat1
  loni = LIS_domain(n)%lisproj%lon1
  dlat = LIS_domain(n)%lisproj%dlat
  dlon = LIS_domain(n)%lisproj%dlon

  do t=1,LIS_rc%ntiles(n)
     c = LIS_domain(n)%tile(t)%col
     r = LIS_domain(n)%tile(t)%row
     
     !RR - Get lat/lon of the tile(in radians) in case we need to do a nearest neighbor interpolation
     lat = (lati + dlat*(r-1))*piover180
     lon = (loni + dlat*(c-1))*piover180

     !RR - Determine if gvf value for this tile is valid, if not, do a neighbor search.
     ...
     if(array(t).lt.0) then 
        ...
        !RR - Set default min distance high
        mindist=9999
        do j=str,enr
           do i=stc,enc
              if(gvfinterp(i+(j-1)*LIS_rc%lnc(n)).ne.-9999) then 
              
                 !RR -Neighbor is valid, find it's lat/lon
                 lat1=(lati + dlat*(j-1))*piover180
                 lon1=(loni + dlat*(i-1))*piover180

                 !RR - Find distance and use value from here if it's the minimum we've found so far.
                 dist=rearth*acos(sin(lat)*sin(lat1) + cos(lat)*cos(lat1)*cos(lon-lon1))
