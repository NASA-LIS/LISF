...
!BOP
!
! !ROUTINE: setup_SPORTgfrac
! \label{setup_SPORTgfrac}
!
! !REVISION HISTORY:
!  16 Jul 2008: Sujay Kumar; Initial Specification
!  10 Aug 2010: Jonathan Case: Modified for SPORT/MODIS GVF.
!
! !INTERFACE:
subroutine setup_SPORTgfrac(n)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!  fraction daily composite data from SPORT
!  
!  The arguments are:
!  \begin{description}
!  \item[n]
...
! lis.config file.
  ...
  call ESMF_ConfigGetattribute(LIS_config,LIS_gfrac(n)%realtimemode,&
       label="SPORT GVF use realtime mode:",rc=status)
  call LIS_verify(status,'SPORT GVF use realtime mode: not defined')

  call ESMF_ConfigGetattribute(LIS_config,gridDesci(4),&
       label="SPORT GVF lower left lat:",rc=status)
  call LIS_verify(status,'SPORT GVF lower left lat: not defined')

  call ESMF_ConfigGetattribute(LIS_config,gridDesci(5),&
       label="SPORT GVF lower left lon:",rc=status)
  call LIS_verify(status,'SPORT GVF lower left lon: not defined')


  call ESMF_ConfigGetattribute(LIS_config,gridDesci(7),&
       label="SPORT GVF upper right lat:",rc=status)
  call LIS_verify(status,'SPORT GVF upper right lat: not defined')


  call ESMF_ConfigGetattribute(LIS_config,gridDesci(8),&
       label="SPORT GVF upper right lon:",rc=status)
  call LIS_verify(status,'SPORT GVF upper right lon: not defined')

  call ESMF_ConfigGetattribute(LIS_config,gridDesci(9),&
       label="SPORT GVF resolution (dx):",rc=status)
  call LIS_verify(status,'SPORT GVF resolution (dx): not defined')


  call ESMF_ConfigGetattribute(LIS_config,gridDesci(10),&
       label="SPORT GVF resolution (dy):",rc=status)
  call LIS_verify(status,'SPORT GVF resolution (dy): not defined')
  ...
  call ESMF_ConfigFindLabel(LIS_config,"SPORT greenness data directory:",rc=rc)
  call ESMF_ConfigGetAttribute(LIS_config,LIS_gfrac(n)%gfracfile,rc=rc)
  call LIS_verify(rc,'SPORT greenness data directory: not defined')
...
end subroutine setup_SPORTgfrac
