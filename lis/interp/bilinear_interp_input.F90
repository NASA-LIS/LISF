...
! !ARGUMENTS:
  ...
  integer             :: n11(LIS_rc%lnc(nest)*LIS_rc%lnr(nest)),&
       n12(LIS_rc%lnc(nest)*LIS_rc%lnr(nest)),&
       n21(LIS_rc%lnc(nest)*LIS_rc%lnr(nest)),&
       n22(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
  real                :: w11(LIS_rc%lnc(nest)*LIS_rc%lnr(nest)),&
       w12(LIS_rc%lnc(nest)*LIS_rc%lnr(nest)),&
       w21(LIS_rc%lnc(nest)*LIS_rc%lnr(nest)),&
       w22(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
!
! !DESCRIPTION: 
...
!EOP
  integer             :: nc,nr
  real                :: rlat(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
  real                :: rlon(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))

  integer             :: n
  integer             :: mo, nv 
  real, parameter     :: fill = -9999.0
  real                :: xpts(LIS_rc%lnc(nest)*LIS_rc%lnr(nest)), &
       ypts(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
  ...
  nc = LIS_rc%lnc(nest)
  nr = LIS_rc%lnr(nest)
  rlat = LIS_domain(nest)%lat
  rlon = LIS_domain(nest)%lon
  mo = nc*nr
  !------------------------------------------------------------------------
