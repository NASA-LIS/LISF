...
! !ARGUMENTS: 
    ...
    integer             :: n112(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25),&
         n122(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25),&
         n212(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25),&
         n222(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25)
    real                :: w112(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25),&
         w122(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25),&
         w212(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25),&
         w222(LIS_rc%lnc(nest)*LIS_rc%lnr(nest),25)
...
!EOP
    integer             :: nc, nr
    real                :: rlat2(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
    real                :: rlon2(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))

    integer             :: npts
    real                :: xpts(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
    real                :: ypts(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
    real                :: xptb(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
    real                :: yptb(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
    real                :: rlob(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
    real                :: rlab(LIS_rc%lnc(nest)*LIS_rc%lnr(nest))
    ...
    nc = LIS_rc%lnc(nest)
    nr = LIS_rc%lnr(nest)
    rlat2 = LIS_domain(nest)%lat
    rlon2 = LIS_domain(nest)%lon
    iret=0
    ipopt = 0
    ipopt(1) = -1
    ipopt(2) = -1
...
!             else
                ...
                tindex = xindex + (yindex -1) *LIS_rc%lnc_b(nest)
                rlab(n) = LIS_domain(nest)%lat_b(tindex)
                rlob(n) = LIS_domain(nest)%lon_b(tindex)
!             endif
          enddo
