...
!  18 Jun 2014: Ben Zaitchik; Modified for drip
!
! !USES: 
  use ESMF
  use LIS_coreMod
  ...
  PUBLIC  :: drip_irrigation_init
  PUBLIC  :: drip_irrigation_updates

contains
  
  subroutine drip_irrigation_init(irrigState)
    ...
    do n=1,LIS_rc%nnest
       ...
       write(LIS_logunit,*) " Running the 'Drip' irrigation method ... "

       call ESMF_ConfigGetAttribute(LIS_config,maxrootdepthfile,&
            label="Drip irrigation max root depth file:",&
            rc=rc)
       call LIS_verify(rc,&
            'Drip irrigation max root depth file: option not specified in the config file')
       ...
       call LIS_verify(status, &
            "ESMF_ArraySpecSet failed in drip_irrigation_init")
       ...
       call LIS_verify(status, &
            "ESMF_FieldCreate failed in drip_irrigation_init")
       
       call ESMF_StateAdd(irrigState(n),(/irrigRateField/),rc=status)
       call LIS_verify(status,&
            "ESMF_StateAdd for irrigRate failed in drip_irrigation_init")
       ...
       call LIS_verify(status,&
            "ESMF_StateAdd for irrigFrac failed in drip_irrigation_init")
       ...
       call LIS_verify(status,&
            "ESMF_StateAdd for max root depth failed in drip_irrigation_init")
       ...
       call LIS_verify(status,&
            'ESMF_StateAdd for irrigation scale failed in drip_irrigation_init')
       deallocate(irrigScale)
    enddo


  end subroutine drip_irrigation_init
  

  subroutine drip_irrigation_updates(n, irrigState)
    ...
    call ESMF_StateGet(LIS_FORC_State(n),&
         ...
         rc=status)
    call LIS_verify(status,&
         'ESMF_StateGet failed for rainf in drip_irrigation')

    
    call ESMF_FieldGet(prcpField,localDE=0, farrayPtr=prcp,rc=status)
    call LIS_verify(status,&
         'ESMF_FieldGet failed for rainf in drip_irrigation')
  ...
  end subroutine drip_irrigation_updates
  ...
  subroutine read_irrigFrac(n,frac)
    ...
    if(file_exists) then 
       ...
       call LIS_verify(ios,'nf90_get_var failed for in drip_irrigationMod')

       ios = nf90_close(nid)
       call LIS_verify(ios,'nf90_close failed in drip_irrigationMod')
