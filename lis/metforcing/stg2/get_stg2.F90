...
!    Interpolates STAGE2 data to LIS grid
!  \end{description}
!EOP
...
!-- Determine LIS's current time and the time of the STAGE 2 file:
    yr1 = LIS_rc%yr
    mo1 = LIS_rc%mo
    da1 = LIS_rc%da
    hr1 = LIS_rc%hr    ! Advance STAGEII by one hour increment
    mn1 = 0
    ss1 = 0
    ts1 = 0
    call LIS_tick( stg2_file_time1, doy1, gmt1, yr1, mo1, da1, hr1, mn1, ss1, ts1 )

!-- STAGE2 product time; end accumulation time data
    yr2 = LIS_rc%yr     
    mo2 = LIS_rc%mo
    da2 = LIS_rc%da
    hr2 = LIS_rc%hr+1  ! Advance STAGEII by one hour increment
    mn2 = 0
    ss2 = 0
    ts2 = 0
    call LIS_tick( stg2_file_time2, doy2, gmt2, yr2, mo2, da2, hr2, mn2, ss2, ts2 )

!-- Determine if STAGE II Grid LIS_domain has changed (before opening file)   
    if( LIS_rc%time > stg2_struc(n)%griduptime1 .and. &
       stg2_struc(n)%gridchange1 ) then 

       write(LIS_logunit,*) "** Changing STAGE2 Grid to 2002 -" 
...
!-- Check for and get STAGE2 Precipitation data
  ...
  ! LIS timestep < STAGEII time interval (1hr)
    if( LIS_rc%ts < stg2_struc(n)%ts ) then
     ...
     ! Assign latest STAGE II file time to stored STAGE II time variable
       stg2_struc(n)%stg2time = stg2_file_time2
      endif

  ! LIS timestep == STAGEII time interval (1hr)
    elseif( LIS_rc%ts == stg2_struc(n)%ts ) then
     ...
     ! Open, read, and reinterpolate STAGE II field to LIS-defined grid
       call read_stg2 ( n, file_name, findex, order, ferror_stg2 )
     ! Assign latest STAGE II file time to stored STAGE II time variable
       stg2_struc(n)%stg2time = stg2_file_time1

    else
      write(LIS_logunit,*) "ERR MSG: STAGE II READER CANNOT HANDLE LIS "
