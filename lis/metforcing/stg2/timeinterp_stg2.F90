...
!    if( LIS_FORC_CRainf%selectOpt .ne. 1 ) then
   ...
   !- Assign STAGE II rainfall product to LIS variable for use in LSM (rate of mm/s)
      do c = 1,LIS_rc%ntiles(n)
         index1 = LIS_domain(n)%tile(c)%index
         if( stg2_struc(n)%metdata2(1,index1) .ne. LIS_rc%udef .and. &
             stg2_struc(n)%metdata2(1,index1) >= 0.0 ) then
           pcp(c) = stg2_struc(n)%metdata2(1,index1) / 3600.0  ! mm/s
         else
           pcp(c) = LIS_rc%udef 
         endif
      enddo
