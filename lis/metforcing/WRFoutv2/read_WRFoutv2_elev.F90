...
!
! !INTERFACE:
subroutine read_WRFoutv2_elev(n,findex)
! !USES:
...
! !DESCRIPTION:
!
!  Opens and reads WRF-4km model elevation to the LIS grid.
...
!EOP
  ...
  if ( trim(LIS_rc%met_ecor(findex)) .ne. "none" ) then 
     ...
     write(LIS_logunit,*) '[INFO] Reading the WRFoutv2 elevation map ...'
     
     call LIS_read_param(n,"ELEV_WRFoutv2",go)
