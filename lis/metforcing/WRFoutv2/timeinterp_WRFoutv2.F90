  ...
  call LIS_verify(status)
   ...
   do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
       ...
       if(WRFoutv2_struc(n)%metdata1(kk,3,index1).ne.LIS_rc%udef.and.&
         WRFoutv2_struc(n)%metdata2(kk,3,index1).ne.LIS_rc%udef) then

         swd(t) = WRFoutv2_struc(n)%metdata1(kk,3,index1)*zw1+&
                  WRFoutv2_struc(n)%metdata2(kk,3,index1)*zw2

         ! In cases of small cos(zenith) angles, use linear weighting
         !  to avoid overly large weights
         if( swd(t) > WRFoutv2_struc(n)%metdata1(kk,3,index1).and. &
             swd(t) > WRFoutv2_struc(n)%metdata2(kk,3,index1).and. &
            (czb.lt.0.1.or.cze.lt.0.1)) then

           swd(t) = WRFoutv2_struc(n)%metdata1(kk,3,index1)*swt1+&
                    WRFoutv2_struc(n)%metdata2(kk,3,index1)*swt2
         endif
       end if
       ...
       if (swd(t) < 0) then
          ...
          write(LIS_logunit,*) 'WRFout1 =', WRFoutv2_struc(n)%metdata1(kk,3,index1)
          write(LIS_logunit,*) 'WRFout2 =', WRFoutv2_struc(n)%metdata2(kk,3,index1)
          call LIS_endrun
       end if
       if (swd(t).gt.LIS_CONST_SOLAR) then
          write(LIS_logunit,fmt=*)'[WARN] SW radiation too high!!'
          write(LIS_logunit,fmt=*)'[WARN] it is', swd(t)
          write(LIS_logunit,fmt=*)'[WARN] data1=',WRFoutv2_struc(n)%metdata1(kk,3,index1)
          write(LIS_logunit,fmt=*)'[WARN] data2=',WRFoutv2_struc(n)%metdata2(kk,3,index1)
  ...
  ! Time Averaged Longwave, Block Interpolation
   do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
       ...
       lwd(t) = wt1 * WRFoutv2_struc(n)%metdata1(kk,4,index1) &
              + wt2 * WRFoutv2_struc(n)%metdata2(kk,4,index1)
