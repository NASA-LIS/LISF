...
!BOP
!
! !ROUTINE: timeinterp_imerg
! \label{timeinterp_imerg}
! 
! !REVISION HISTORY: 
...
!
! !INTERFACE:
subroutine timeinterp_imerg(n,findex)
! !USES:
  use ESMF
...
!EOP
  ...
  mfactor = LIS_rc%nensem(n)/imerg_struc(n)%nIter
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
       t = m + (k-1)*mfactor
       index1 = LIS_domain(n)%tile(t)%index
       kk = LIS_get_iteration_index(n, k, index1, mfactor)
       if( imerg_struc(n)%metdata2(kk,1,index1) .ne. LIS_rc%udef .and. &
            imerg_struc(n)%metdata2(kk,1,index1) .ge. 0.0 ) then
          pcp(t) = imerg_struc(n)%metdata2(kk,1,index1) / 3600.0  ! mm/s
       else
          pcp(t) = LIS_rc%udef
       endif
     end do
...
end subroutine timeinterp_imerg
