...
!BOP
! !ROUTINE: timeinterp_hmalpm
! \label{timeinterp_hmalpm}
!
! !REVISION HISTORY:
...
subroutine timeinterp_hmalpm(n,findex)
...
!EOP
  ...
  btime=hmalpm_struc(n)%hmalpmtime1
  ...
  btime=hmalpm_struc(n)%hmalpmtime2
...
!=== Interpolate Data in time      
  wt1=(hmalpm_struc(n)%hmalpmtime2-LIS_rc%time)/ & 
       (hmalpm_struc(n)%hmalpmtime2-hmalpm_struc(n)%hmalpmtime1)
  wt2=1.0-wt1
  ...
  mfactor = LIS_rc%nensem(n)/hmalpm_struc(n)%nIter
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        if (hmalpm_struc(n)%metdata1(kk,3,index1).ne.LIS_rc%udef.and.&
             hmalpm_struc(n)%metdata2(kk,3,index1).ne.LIS_rc%udef) then 
           swd(t) = hmalpm_struc(n)%metdata1(kk,3,index1)*zw1 + &
                hmalpm_struc(n)%metdata2(kk,3,index1)*zw2

           ! In cases of small cos(zenith) angles, use linear weighting
           !  to avoid overly large weights

           if((swd(t).gt.hmalpm_struc(n)%metdata1(kk,3,index1).and. & 
                swd(t).gt.hmalpm_struc(n)%metdata2(kk,3,index1)).and. & 
                (czb.lt.0.1.or.cze.lt.0.1))then
              swd(t) = hmalpm_struc(n)%metdata1(kk,3,index1)*swt1+ & 
                   hmalpm_struc(n)%metdata2(kk,3,index1)*swt2
           endif
           ...
           if (swd(t).gt.LIS_CONST_SOLAR) then
              ...
              write(unit=LIS_logunit,fmt=*)'[WARN] hmalpmdata1=',&
                   hmalpm_struc(n)%metdata1(kk,3,index1)
              write(unit=LIS_logunit,fmt=*)'[WARN] hmalpmdata2=',&
                   hmalpm_struc(n)%metdata2(kk,3,index1)
        ...
        if ((swd(t).ne.LIS_rc%udef).and.(swd(t).lt.0)) then
           if (swd(t).gt.-0.00001) then 
              ...
              write(LIS_logunit,*) &
                   '[ERR] timeinterp_hmalpm -- Stopping because ', & 
                   'forcing not udef but lt0,'
              write(LIS_logunit,*)'[ERR] timeinterp_hmalpm -- ', & 
                   t,swd(t),hmalpm_struc(n)%metdata2(kk,3,index1), & 
                   ' (',LIS_localPet,')'
              call LIS_endrun
           endif
        endif
        ...
        if (swd(t).gt.LIS_CONST_SOLAR) then
           swd(t)=hmalpm_struc(n)%metdata2(kk,3,index1)
        endif
     enddo
  enddo
...
!-----------------------------------------------------------------------  
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        pcp(t) = hmalpm_struc(n)%metdata1(kk,7,index1)
...
!-----------------------------------------------------------------------
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        tmp(t) = hmalpm_struc(n)%metdata1(kk,1,index1)*wt1 + &
             hmalpm_struc(n)%metdata2(kk,1,index1)*wt2
        q2(t)  = hmalpm_struc(n)%metdata1(kk,2,index1)*wt1 + &
             hmalpm_struc(n)%metdata2(kk,2,index1)*wt2
        lwd(t)  = hmalpm_struc(n)%metdata1(kk,4,index1)*wt1 + &
             hmalpm_struc(n)%metdata2(kk,4,index1)*wt2

        uwind(t) = hmalpm_struc(n)%metdata1(kk,5,index1)*wt1+&
             hmalpm_struc(n)%metdata2(kk,5,index1)*wt2
        vwind(t) = 0.0
        psurf(t) = hmalpm_struc(n)%metdata1(kk,6,index1)*wt1 + &
             hmalpm_struc(n)%metdata2(kk,6,index1)*wt2

     enddo
  enddo

end subroutine timeinterp_hmalpm
