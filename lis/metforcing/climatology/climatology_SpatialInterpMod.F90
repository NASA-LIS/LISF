...
#include "LIS_misc.h"
!BOP
! !MODULE: climatology_SpatialInterpMod
! \label{climatology_SpatialInterpMod}
! 
! !DESCRIPTION: 
...
!------------------------------------------------------------------------------
  public :: climatology_interp_init      ! initialize interp routine inputs/outputs
  public :: climatology_interp_data      ! transform input data/field to LIS target grid
  public :: climatology_interp_finalize  ! deallocate/reinit any interp arrays/inputs
!EOP
...
!BOP
! !ROUTINE: climatology_interp_init 
!  \label{climatology_interp_init}
! 
! !INTERFACE: 
!
 subroutine climatology_interp_init( n, findex, input_gridDesc )
!
! !USES: 
...
!- Determine if reprojection option:
   ...
   ! Spatial averaging ("upscaling"):
     ...
     ! resolution for lat/lon projection: 
       if( input_gridDesc(10) .gt. LIS_rc%gridDesc(n,10)  ) then
          ...
          write(LIS_logunit,*) "[ERR]  > the LDT-generated climatological forcing resolution."
          write(LIS_logunit,*) "[ERR]  Program stopping ..."
          call LIS_endrun()
       endif
 ...
 end subroutine climatology_interp_init


!BOP
! !ROUTINE: climatology_interp_data
!  \label{climatology_interp_data}
! 
! !INTERFACE: 
!
 subroutine climatology_interp_data( n, findex,  &
               input_nc, input_nr, ntimes, input_2dvar, &
...
! !ARGUMENTS: 
   ...
   real,    intent(in) :: input_2dvar(input_nc,input_nr,ntimes)            ! Input Var
   real, intent(inout) :: output_2dvar(LIS_rc%lnc(n),LIS_rc%lnr(n),ntimes) ! Output Var
...
!--------------------------------------------------------------------    
 ...
 end subroutine climatology_interp_data


!BOP
! !ROUTINE: climatology_interp_finalize
!  \label{climatology_interp_finalize}
! 
! !INTERFACE: 
!
 subroutine climatology_interp_finalize( findex )
!
! !USES: 
   use LIS_coreMod, only : LIS_rc, LIS_domain
   use LIS_logMod,  only : LIS_logunit, LIS_endrun
 ...
 end subroutine climatology_interp_finalize

end module climatology_SpatialInterpMod
