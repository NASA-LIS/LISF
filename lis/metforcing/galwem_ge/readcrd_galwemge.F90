...
!EOP
  ...
  call ESMF_ConfigFindLabel(LIS_config, "GALWEM-GE model CDF directory:", rc=rc)
  call LIS_verify(rc, 'GALWEM-GE model CDF directory: not defined')
  do n = 1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, galwemge_struc(n)%cdf_dir, rc=rc)
  enddo
  ...
  do n=1,LIS_rc%nnest
     ...
     write(LIS_logunit,*) '[INFO] GALWEM-GE forecast number of ensemble members:',galwemge_struc(n)%max_ens_members
     write(LIS_logunit,*) '[INFO] Using GALWEM-GE precipitation bias correction:',galwemge_struc(n)%bc
     if (galwemge_struc(n)%bc == 1) then
        write(LIS_logunit,*) '[INFO] GALWEM-GE model CDF directory: ',trim(galwemge_struc(n)%cdf_dir)
     endif
  enddo
