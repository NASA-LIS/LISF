           ...
           if (galwemge_struc(n)%metdata1(3,m,index1).ne.LIS_rc%udef.and.&
              ...
              if (swd(tid).lt.0.0) then
                 write(unit=LIS_logunit,fmt=*) &
                      '[ERR] SW radiation is unphysical'
                 write(unit=LIS_logunit,fmt=*)&
                      '[ERR] it is', LIS_localPet, tid, swd(tid)
                 write(unit=LIS_logunit,fmt=*)&
                      '[ERR] data1=', &
                      galwemge_struc(n)%metdata1(3,m,index1)
                 write(unit=LIS_logunit,fmt=*) &
                      '[ERR] data2=', &
                      galwemge_struc(n)%metdata2(3,m,index1)
                 write(unit=LIS_logunit,fmt=*) &
                      '[ERR] wts=', wt1, wt2
                 call LIS_endrun
              endif
        ...
        do k=1,mfactor
           ...
           if (galwemge_struc(n)%bc == 1) then  !1 - use; or 0
              if (galwemge_struc(n)%pcp_bc(m,index1).ne.LIS_rc%udef) then
                 ...
                 pcp(tid) = &
                      galwemge_struc(n)%pcp_bc(m,index1)/real(3600*fcsthr_intv)
  ...
  do t=1,LIS_rc%ntiles(n)/LIS_rc%nensem(n)
     do m=1,galwemge_struc(n)%max_ens_members
        do k=1,mfactor
           ...
           ! 2-meter air temp
           if ((galwemge_struc(n)%metdata1(1,m,index1).ne.LIS_rc%udef) &
                .and.&
           ...
           ! Specific humidity
           if ((galwemge_struc(n)%metdata1(2,m,index1).ne.LIS_rc%udef) &
                .and.&
           ...
           if ((galwemge_struc(n)%metdata1(4,m,index1).ne.LIS_rc%udef) &
                .and.&
           ...
           ! U-wind component
           if ((galwemge_struc(n)%metdata1(5,m,index1).ne.LIS_rc%udef) &
                .and. &
           ...
           ! V-wind component
           if ((galwemge_struc(n)%metdata1(6,m,index1).ne.LIS_rc%udef) &
                .and.&
           ...
           if ((galwemge_struc(n)%metdata1(7,m,index1).ne.LIS_rc%udef) &
                .and.&
