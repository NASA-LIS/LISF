...
! -- Apply elevation correction to humidity:
  ...
  rh_corr = (force_hum / fqsat) * 100.0
  orig_hum = (rh_corr * qsat) / 100.0
...
!----------------------------------------------------------------------
  ...
  emiss =  1.08 * (1.0-exp(-mee**(orig_tmp/bb)))
  femiss = 1.08 * (1.0-exp(-mfe**(force_tmp/bb)))
...
end subroutine nldas20_ec_removal
