...
!=== End Variable Definition =======================
  if(order.eq.1) then 
     stg4_struc(n)%metdata1 = LIS_rc%udef
  elseif(order.eq.2) then 
     stg4_struc(n)%metdata2 = LIS_rc%udef
  endif
...
!-- Check initially if file exists:
  ...
  call LIS_verify(iret,'error grib_open_file in read_stg4')
  
  call grib_new_from_file(ftn,igrib,iret)
  call LIS_verify(iret,'error in grib_new_from_file in read_stg4')
  
  call grib_get(igrib,'indicatorOfParameter',pds5_val,iret)
  call LIS_verify(iret, 'error in grib_get: indicatorOfParameter in read_stg4')
  
  call grib_get(igrib,'level',pds7_val,iret)
  call LIS_verify(iret, 'error in grib_get: level in read_stg4')

  if(pds5_val.eq.61.and.pds7_val.eq.0) then 
     call grib_get(igrib,'values',stg4in,iret)
     call LIS_verify(iret, 'error in grib_get:values in read_stg4')
     
     call grib_get(igrib,'missingValue',missingValue,iret)
     call LIS_verify(iret, 'error in grib_get:missingValue in read_stg4')
     ...
     call interp_stg4( n, findex, ndata, stg4in, lb, LIS_rc%gridDesc(n,:), &
     ...
     do j = 1, LIS_rc%lnr(n)
        do i = 1, LIS_rc%lnc(n)
           ...
           if ( precip_regrid(i,j) .ne. LIS_rc%udef ) then
              ...
              if(index1 .ne. -1) then
                 if(order.eq.1) then 
                    stg4_struc(n)%metdata1(1,index1) = precip_regrid(i,j) 
                 elseif(order.eq.2) then 
                    stg4_struc(n)%metdata2(1,index1) = precip_regrid(i,j) 
                 endif
              endif
           endif
     ...
     call LIS_verify(iret,'error in grib_release in read_stg4')
