...
subroutine interp_TRMM3B42V7(n, nTRMM3B42V7, f, lb, lis_gds, nc, nr, varfield, &
                             findex) 
! !USES:
...
!EOP
  ...
  if(LIS_rc%met_interp(findex).eq."budget-bilinear") then ! SY
   call conserv_interp(lis_gds,lb,f,lo,lis1d,TRMM3B42V7_struc(n)%mi,mo,&
  ...
  ! SY: begin for nearest neighbor
   call neighbor_interp(lis_gds,lb,f,lo,lis1d,TRMM3B42V7_struc(n)%mi,mo,&
        LIS_domain(n)%lat, LIS_domain(n)%lon, &
        TRMM3B42V7_struc(n)%n112, LIS_rc%udef,iret)
