...
!      Interpolates TRMM 3B42V7 data to LIS grid
!  \end{description}
!EOP
...
!=== End Variable Definition =======================
 ...
 !------------------------------------------------------------------------
   ...
   ss1 = LIS_rc%ss
   ts1 = 0
   call LIS_tick( TRMM3B42V7_struc(n)%LIS_timeAtTStepEnd, doy1, gmt1, &
        yr1, mo1, da1, hr1, mn1, ss1, real(ts1))
   ts1 = (-1)*LIS_rc%nts(n) ! SY: for start of model time step
   call LIS_tick( TRMM3B42V7_struc(n)%LIS_timeAtTStepStart, doy1, gmt1, &
        yr1, mo1, da1, hr1, mn1, ss1, real(ts1))
 ...
 !------------------------------------------------------------------------
   ...
   ss2 = 0
   ts2 = 0
   call LIS_tick( TRMM3B42V7_struc(n)%TRMM3B42V7time_TStepStart, doy2, gmt2, &
   ...
   ss3 = 0
   ts3 = 0
   call LIS_tick( TRMM3B42V7_struc(n)%TRMM3B42V7time_TStepEnd, doy3, gmt3, &
 ...
 !------------------------------------------------------------------------
   ...
   if  (LIS_get_nstep(LIS_rc, n).eq. 1 .or. LIS_rc%rstflag(n) .eq. 1) then
     ...
     TRMM3B42V7_struc(n)%metdata1 = LIS_rc%udef
     TRMM3B42V7_struc(n)%metdata2 = LIS_rc%udef
     ferror_TRMM3B42V7 = 0
     write(LIS_logunit, *) '[INFO] TRMM yr,mo,da,hr for time step start:', &
                           TRMM3B42V7_struc(n)%TRMM3B42V7yr_TStepStart, &
                           TRMM3B42V7_struc(n)%TRMM3B42V7mo_TStepStart, &
                           TRMM3B42V7_struc(n)%TRMM3B42V7da_TStepStart, &
     ...
     order = 1
     do kk= TRMM3B42V7_struc(n)%st_iterid, TRMM3B42V7_struc(n)%en_iterid
       call TRMM3B42V7file( filename, n, kk, findex, &
                      TRMM3B42V7_struc(n)%TRMM3B42V7yr_TStepStart, &
                      TRMM3B42V7_struc(n)%TRMM3B42V7mo_TStepStart, &
                      TRMM3B42V7_struc(n)%TRMM3B42V7da_TStepStart, &
                      TRMM3B42V7_struc(n)%TRMM3B42V7hr_TStepStart )
       write(LIS_logunit, *)'[INFO] Getting new TRMM 3B42V7 satellite precip data:',trim(filename)
       call read_TRMM3B42V7(n, kk, filename, findex, order, ferror_TRMM3B42V7)
     end do
   ...
   elseif (.NOT. ((TRMM3B42V7_struc(n)%TRMM3B42V7yr_TStepStart .EQ. &
     ...
     write(LIS_logunit, *) '[INFO] TRMM yr,mo,da,hr for time step start:', &
                           ...
                           TRMM3B42V7_struc(n)%TRMM3B42V7da_TStepStart, &
                           TRMM3B42V7_struc(n)%TRMM3B42V7hr_TStepStart
     write(LIS_logunit, *)'[INFO] Values from time step end assigned'
     TRMM3B42V7_struc(n)%metdata1 = TRMM3B42V7_struc(n)%metdata2 
     ! SY: equal to TRMM3B42V7_struc(n)%metdata2 before possible change in TRMM3B42V7_struc(n)%metdata2 below
   endif
   ...
   if (.NOT. ((TRMM3B42V7_struc(n)%TRMM3B42V7yr_TStepEnd .EQ. &
     ...
     ferror_TRMM3B42V7 = 0
     write(LIS_logunit, *)'[INFO] TRMM yr,mo,da,hr for time step end:', &
                           TRMM3B42V7_struc(n)%TRMM3B42V7yr_TStepEnd, &
                           TRMM3B42V7_struc(n)%TRMM3B42V7mo_TStepEnd, &
                           TRMM3B42V7_struc(n)%TRMM3B42V7da_TStepEnd, &
...
!
! !INTERFACE:
subroutine TRMM3B42V7file( filename, n, kk, findex, yr, mo, da, hr)

! !DESCRIPTION: This subroutine puts together 3B42V7 file name for
...
!EOP
  ...
  integer, intent(in) :: findex
  character(len=LIS_PATH_LEN)       :: filename
  integer             :: yr, mo, da, hr
