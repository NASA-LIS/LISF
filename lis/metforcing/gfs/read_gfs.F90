...
!  GFS forecast datasets, transforms into 9 LIS forcing 
...
!=== End Variable Definition =======================
  ...
  ngfs = (gfs_struc(n)%ncold*gfs_struc(n)%nrold)
!--------------------------------------------------------------------------
     ...
     do t=1,LIS_rc%ngrid(n)
        if(F06flag) then 
           if(iv.eq.3.or.iv.eq.4.or.iv.eq.8.or.iv.eq.9) then! these are time avgd fields
              if(order.eq.1) then 
                 gfs_struc(n)%metdata1(iv,t) = 2*glbdata_a_F06(iv,t)-glbdata_a(iv,t)
              else
                 gfs_struc(n)%metdata2(iv,t) = 2*glbdata_a_F06(iv,t)-glbdata_a(iv,t)
              endif
           else ! these are instantaneous
              if(order.eq.1) then 
                 gfs_struc(n)%metdata1(iv,t) = glbdata_i(iv,t)
              else
                 gfs_struc(n)%metdata2(iv,t) = glbdata_i(iv,t)
              endif
           endif
        else
           if(iv.eq.3.or.iv.eq.4.or.iv.eq.8.or.iv.eq.9) then! these are time avgd fields
              if(order.eq.1) then 
                 gfs_struc(n)%metdata1(iv,t) = glbdata_a(iv,t)                         
              else
                 gfs_struc(n)%metdata2(iv,t) = glbdata_a(iv,t)                         
              endif
           else
              if(order.eq.1) then 
                 gfs_struc(n)%metdata1(iv,t) = glbdata_i(iv,t)                         
...
! index 10 indicates instantaneous, 003 indicates time average
  ...
  if (file_exists) then      
     ...
     call LIS_verify(iret, 'error in grib_count_in_file in read_gfs')

     allocate(lb(gfs_struc(n)%ncold*gfs_struc(n)%nrold))
     allocate(f(gfs_struc(n)%ncold*gfs_struc(n)%nrold))
     
     do kk=1,nvars
        call grib_new_from_file(ftn, igrib, iret)
        call LIS_warning(iret, 'error in grib_new_from_file in read_gfs')
        if(iret.ne.0) then 
           write(LIS_logunit,*) &
        ...
        call LIS_verify(rc, 'error in grib_get: indicatorOfParameter in read_gfs')

        call grib_get(igrib,'level',pds7_val,rc)
        call LIS_verify(rc, 'error in grib_get: level in read_gfs')

        call grib_get(igrib,'timeRangeIndicator',pds16_val,rc)
        call LIS_verify(rc, 'error in grib_get: timeRangeIndicator in read_gfs')
        ...
        call LIS_warning(rc, 'error in grib_get:values in read_gfs')
        ...
        call LIS_verify(rc, 'error in grib_get:missingValue in read_gfs')

        call grib_release(igrib,rc)
        call LIS_verify(rc, 'error in grib_release in read_gfs')
...
!-----------------------------------------------------------------------  
  if(trim(LIS_rc%met_interp(findex)).eq."bilinear") then 
     call bilinear_interp(lis_gds,lb,f,lo,lis1d,gfs_struc(n)%mi,mo,&
          ...
          gfs_struc(n)%n211,gfs_struc(n)%n221,LIS_rc%udef, iret)
  elseif(trim(LIS_rc%met_interp(findex)).eq."budget-bilinear") then 
     if (pcp_flag)then     
        call conserv_interp(lis_gds,lb,f,lo,lis1d,gfs_struc(n)%mi,mo, & 
             ...
             gfs_struc(n)%n212,gfs_struc(n)%n222,LIS_rc%udef,iret)
     else 
        call bilinear_interp(lis_gds,lb,f,lo,lis1d,gfs_struc(n)%mi,mo,&
             LIS_domain(n)%lat, LIS_domain(n)%lon,&
             gfs_struc(n)%w111,gfs_struc(n)%w121,&
