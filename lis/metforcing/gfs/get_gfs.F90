...
!  \item[read\_gfs](\ref{read_gfs}) \newline
!      Interpolates GFS data to LIS grid
...
!-----------------------------------------------------------------
  ...
  if ( time1 >= gfs_struc(n)%griduptime1 .and. &
     ...
     write(LIS_logunit,*) 'MSG: get_gfs -- changing grid to 1991 -- 20000124'

     gfs_struc(n)%ncold = 384
     gfs_struc(n)%nrold = 190
...
!-------------------------------------------------------------------
  ...
  elseif ( time1 >= gfs_struc(n)%griduptime2 .and. &
     ...
     write(LIS_logunit,*) 'MSG: get_gfs -- changing grid to 20000124 -- 20021029'

     gfs_struc(n)%ncold = 512
     gfs_struc(n)%nrold = 256
     gfs_struc(n)%mi = gfs_struc(n)%ncold*gfs_struc(n)%nrold
!-------------------------------------------------------------------
! Reinitialize the weights and neighbors
!-------------------------------------------------------------------
  ...
  elseif ( time1 >= gfs_struc(n)%griduptime3 .and. & 
     ...
     write(LIS_logunit,*) 'MSG: get_gfs -- changing grid to 20021029 -- 20050531'

     gfs_struc(n)%ncold = 768
     gfs_struc(n)%nrold = 384
     gfs_struc(n)%mi = gfs_struc(n)%ncold*gfs_struc(n)%nrold
!-------------------------------------------------------------------
! Reinitialize the weights and neighbors
!-------------------------------------------------------------------
  ...
  elseif ( time1 >= gfs_struc(n)%griduptime4 .and. &
     ...
     write(LIS_logunit,*) 'MSG: get_gfs -- changing grid to 20050531 -- 20100728'

     gfs_struc(n)%ncold = 1152
     gfs_struc(n)%nrold = 576
     gfs_struc(n)%mi = gfs_struc(n)%ncold*gfs_struc(n)%nrold
!-------------------------------------------------------------------
! Reinitialize the weights and neighbors
!-------------------------------------------------------------------
  ...
  elseif ( time1 >= gfs_struc(n)%griduptime5 .and. &
           time1 < gfs_struc(n)%griduptime6 .and. &
           gfs_struc(n)%gridchange5) then

     write(LIS_logunit,*) 'MSG: get_gfs -- changing grid to 20100728 -- 20150114'

     gfs_struc(n)%ncold = 1760
     gfs_struc(n)%nrold = 880
     gfs_struc(n)%mi = gfs_struc(n)%ncold*gfs_struc(n)%nrold
!-------------------------------------------------------------------
! Reinitialize the weights and neighbors
!-------------------------------------------------------------------
  ...
  elseif ( time1 >= gfs_struc(n)%griduptime6 .and. &
           gfs_struc(n)%gridchange6 ) then

     write(LIS_logunit,*) 'MSG: get_gfs -- changing grid to 20150114 --'

     gfs_struc(n)%ncold = 3072
     gfs_struc(n)%nrold = 1536
     gfs_struc(n)%mi = gfs_struc(n)%ncold*gfs_struc(n)%nrold
!-------------------------------------------------------------------
! Reinitialize the weights and neighbors
!-------------------------------------------------------------------
...
!-----------------------------------------------------------------
           write(LIS_logunit,*) 'ERROR: GFS data gap exceeds 10 days on file 1'
           call LIS_endrun
        endif
     end do
  end if
  ...
  if ( movetime == 1 ) then  
     ...
     do f = 1, LIS_rc%met_nf(findex)
        do t = 1, LIS_rc%ngrid(n)
           gfs_struc(n)%metdata1(f,t) = gfs_struc(n)%metdata2(f,t)
        end do
     end do
  end if
...
! ferror still=0, so roll back one day & start again
!-----------------------------------------------------------------
           call LIS_tick( dumbtime2, doy2, gmt2, yr1, mo1, da1, hr1, mn1, ss1, ts2 )
        end if
        if ( try > ndays ) then  
!-----------------------------------------------------------------
! data gap exceeds 10 days so stop
!-----------------------------------------------------------------
           write(LIS_logunit,*) 'ERROR: GFS data gap exceeds 10 days on file 2'
           call LIS_endrun
        end if
     end do
  end if
...
!-----------------------------------------------------------------
  do f = 1,LIS_rc%met_nf(findex)  
     if ( (f == 3) .or. (f == 4) ) then
        do t=1,LIS_rc%ngrid(n)
           if ( (gfs_struc(n)%metdata2(f,t) /= -9999.9) .and.  &
                (gfs_struc(n)%metdata2(f,t) < 0)) then
              gfs_struc(n)%metdata2(f,t) = (-1) * &
                   gfs_struc(n)%metdata2(f,t)
           endif
           if ( (gfs_struc(n)%metdata1(f,t) /= -9999.9) .and.  &
                (gfs_struc(n)%metdata1(f,t) < 0)) then
              gfs_struc(n)%metdata1(f,t) = (-1) * &
                   gfs_struc(n)%metdata1(f,t)
           endif
        enddo
     endif
  enddo
