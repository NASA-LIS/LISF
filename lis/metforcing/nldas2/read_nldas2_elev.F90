...
!
! !INTERFACE:
subroutine read_nldas2_elev(n,findex)
! !USES:
  use LIS_coreMod
...
!EOP
   ...
   if(file_exists) then 
      ...
      write(LIS_logunit,*) " Reading NLDAS2 elevation data ... "

      call LIS_verify(nf90_open(path=LIS_rc%paramfile(n),&
           mode=NF90_NOWRITE,ncid=nid),&
           'nf90_open failed in read_nldas2_elev')
      call LIS_verify(nf90_inq_varid(nid,"ELEV_NLDAS2",elevId),&
           'nf90_inq_varid failed in read_nldas2_elev')
      call LIS_verify(nf90_get_var(nid,elevId,elev),&
           'nf90_get_var failed in read_nldas2_elev')      
   ...
   write(LIS_logunit,*) & 
        "Finish reading original NLDAS2 elevation data"
#endif
