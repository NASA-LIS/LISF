...
#include "LIS_misc.h"
!BOP
! !MODULE: metForcGen_SpatialInterpMod
! \label{metForcGen_SpatialInterpMod}
! 
! !DESCRIPTION: 
...
!------------------------------------------------------------------------------
  public :: metForcGen_interp_init      ! initialize interp routine inputs/outputs
  public :: metForcGen_interp_data      ! transform input data/field to LIS target grid
  public :: metForcGen_interp_finalize  ! deallocate/reinit any interp arrays/inputs
!EOP
...
!BOP
! !ROUTINE: metForcGen_interp_init 
!  \label{metForcGen_interp_init}
! 
! !INTERFACE: 
!
 subroutine metForcGen_interp_init( n, findex, input_gridDesc )
!
! !USES: 
 ...
 ! Determine LIS output grid projection type:
   select case( nint(LIS_rc%gridDesc(n,1)) )
    ...
    case default
      ...
      write(LIS_logunit,*) "[ERR] SUPPORTED BY THE LDT-GENERATED MET FORCING READER."
...
!- Determine if reprojection option:
   ...
   ! Spatial averaging ("upscaling"):
     ...
     ! resolution for lat/lon projection 
       if( input_gridDesc(10) .gt. LIS_rc%gridDesc(n,10)  ) then
          ...
          write(LIS_logunit,*) "[ERR] greater than the LDT-generated forcing resolution."
          write(LIS_logunit,*) "[ERR] Program stopping ..."
          call LIS_endrun()
       endif
   ...
   ! Budget-bilinear (conservative) interpolation ("downscaling"):
     ...
     case ( "none" )
       ...
       write(LIS_logunit,*)"[INFO] == NO spatial transform of input forcing (only subset may occur). "
 ...
 end subroutine metForcGen_interp_init


!BOP
! !ROUTINE: metForcGen_interp_data
!  \label{metForcGen_interp_data}
! 
! !INTERFACE: 
!
 subroutine metForcGen_interp_data( n, findex,  &
               input_nc, input_nr, input_2dvar, &
               output_2dvar )
!
! !USES: 
   use LIS_coreMod, only : LIS_rc, LIS_domain
...
! !ARGUMENTS: 
   ...
   real,    intent(in) :: input_2dvar(input_nc,input_nr) ! Input Var
   real, intent(inout) :: output_2dvar(LIS_rc%lnc(n),LIS_rc%lnr(n)) ! Output Var
...
!- Create input 1D array:
   ...
   do r=1, input_nr
      do c=1, input_nc
         count1 = count1 + 1
         input_1d(count1) = input_2dvar(c, r)
      end do
   end do
     ...
     ! Otherwise, subset for same domain resolutions:
         ...
         do r = 1, sinterp_struc(n)%subset_nr
           do c = 1, sinterp_struc(n)%subset_nc
             output_2dvar(c,r) = &
               input_2dvar(sinterp_struc(n)%lon_line(c,r),sinterp_struc(n)%lat_line(c,r))
           enddo ! columns
         enddo ! rows
         return
       endif
...
!   if( iret /= 0 ) print *, " ERR: IN INTERP OPTION ... ", iret
...
!--------------------------------------------------------------------    
   ...
   do r=1, LIS_rc%lnr(n)
      do c=1, LIS_rc%lnc(n)
         count1 = count1 + 1
         output_2dvar(c,r) = output_1d(count1)
      end do
   end do


 end subroutine metForcGen_interp_data


!BOP
! !ROUTINE: metForcGen_interp_finalize
!  \label{metForcGen_interp_finalize}
! 
! !INTERFACE: 
!
 subroutine metForcGen_interp_finalize( findex )
!
! !USES: 
   use LIS_coreMod, only : LIS_rc, LIS_domain
   use LIS_logMod,  only : LIS_logunit, LIS_endrun
 ...
 end subroutine metForcGen_interp_finalize

end module metForcGen_SpatialInterpMod
