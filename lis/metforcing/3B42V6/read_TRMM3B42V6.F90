...
!               data
!  25 Aug 2006: Yudong Tian; Modified for LIS 4.2 release
...
!  TRMM 3B42V6 data and interpolates to the LIS domain.
!
!  The arguments are:
!  \begin{description}
!  \item[n]
...
!EOP
  ...
  if(order.eq.1) then
     TRMM3B42V6_struc(n)%metdata1 = LIS_rc%udef ! J.Case
  elseif(order.eq.2) then 
     TRMM3B42V6_struc(n)%metdata2 = LIS_rc%udef ! J.Case
  endif
  allocate (precip_regrid(LIS_rc%lnc(n),LIS_rc%lnr(n)))
!  precip = -1.0
!  if(order.eq.1) then
!     TRMM3B42V6_struc(n)%metdata1 = -1.0
!  elseif(order.eq.2) then 
!     TRMM3B42V6_struc(n)%metdata2 = -1.0
!  endif
...
! write (99,*) precip
     ...
     nTRMM3B42V6 = TRMM3B42V6_struc(n)%ncold*TRMM3B42V6_struc(n)%nrold
     allocate(lb(nTRMM3B42V6))
     lb = .true.
     call interp_TRMM3B42V6(n, nTRMM3B42V6, precip, lb, LIS_rc%gridDesc, &
     ...
     do j = 1,LIS_rc%lnr(n)
        do i = 1,LIS_rc%lnc(n)
           if (precip_regrid(i,j) .ne. -1.0) then
              ...
              if(index1 .ne. -1) then
                 if(order.eq.1) then 
                    TRMM3B42V6_struc(n)%metdata1(1,index1) = precip_regrid(i,j)   !here is mm/h
                 elseif(order.eq.2) then 
                    TRMM3B42V6_struc(n)%metdata2(1,index1) = precip_regrid(i,j)   !here is mm/h
                 endif
              endif
           endif
        enddo
