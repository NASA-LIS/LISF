...
!  ECMWF data, transforms into 9 LIS forcing 
!  parameters and interpolates to the LIS domain.

!  ECMWF model output variables used to force LIS fall into 2 categories:
...
!  instantaneous and available every 6 hours.  At this time, all LIS LSMs 
...
!EOP
  ...
  real                            :: glbdata_i(9,LIS_rc%ngrid(n))
  real                            :: glbdata_a(9,LIS_rc%ngrid(n))
   ...
   do iv=1,9
     do t=1,LIS_rc%ngrid(n)
        ...
        if ( iv.eq.3 .or. iv.eq.4 .or. iv.eq.8 .or. iv.eq.9 ) then
           if(order.eq.1) then 
              ecmwf_struc(n)%metdata1(iv,t) = glbdata_a(iv,t)
           else
              ecmwf_struc(n)%metdata2(iv,t) = glbdata_a(iv,t)
           endif
        ! these are instantaneous
        else
           if(order.eq.1) then 
              ecmwf_struc(n)%metdata1(iv,t) = glbdata_i(iv,t)
           else
              ecmwf_struc(n)%metdata2(iv,t) = glbdata_i(iv,t)
           endif
        endif
     enddo
   enddo
...
!EOP
  ...
  if ( file_exists ) then 
     ...
     call LIS_verify(iret, 'failed to open file '//trim(instfile))

     call grib_count_in_file(ftn,nvars,iret)
     call LIS_verify(iret, 'error in grib_count_in_file in retrieve_inst_ecmwfvars')
     ...
     do kk=1,nvars
        ...
        call LIS_verify(iret, 'failed to get values in retrieve_inst_ecmwfvars')

        if ( iret .ne. 0 ) then   ! read not successful
         write(LIS_logunit,*) 'problem reading inst for ',shortName,iret
        ...
        call LIS_verify(iret,'failed to get missingValue in retrieve_inst_ecmwfvars')
        call grib_release(igrib)
        call LIS_verify(iret,'error in grib_release in read_ecmwf')
     ...
     write(LIS_logunit,*) 'ERR: Could not find file (I): ',instfile   
     fret = -1
  endif
#endif
...
! !ARGUMENTS:
  ...
  real                         :: glbdata1(9,LIS_rc%ngrid(n))
  integer                      :: hr
  integer, intent(inout)       :: fret
!
! !DESCRIPTION: 
...
!EOP
  ...
  logical            :: pcp_flag
  real               :: temp_val
  real               :: glbdata2(9,LIS_rc%ngrid(n))
  ...
  if ( file_exists ) then 
     ...
     call LIS_verify(iret, 'failed to open file '//trim(avgfile2))

     call grib_count_in_file(ftn,nvars,iret)
     call LIS_verify(iret, 'error in grib_count_in_file in retrieve_avg_ecmwfvars')
     ...
     do kk=1,nvars
        ...
        f = -9999.0
        call grib_get(igrib,'values',f,iret)
        call LIS_verify(iret, 'failed to get values in retrieve_accum_ecmwfvars')
        if ( iret .ne. 0 ) then  ! read not successful
         write(LIS_logunit,*) 'problem reading accum2 for ',shortName,iret
         call grib_release(igrib)
         deallocate(lb)
         deallocate(f)     
         fret = -1
        ...
        call grib_get(igrib,'missingValue',missingValue,iret)
        call LIS_verify(iret,'failed to get missingValue in retrieve_accum_ecmwfvars')
        ...
        if(var_found) then
           ...
           do r=1,LIS_rc%lnr(n)
              do c=1,LIS_rc%lnc(n)
                 if ( LIS_domain(n)%gindex(c,r) /= -1 ) then 
                    glbdata2(var_index,LIS_domain(n)%gindex(c,r)) = &
                       varfield(c,r)
                 endif
              enddo
           enddo
     ...
     write(LIS_logunit,*) 'ERR: Could not find file (A2) : ',avgfile2   
     fret = -1
  endif
  gbret = gbret + fret
  ...
  if ( .not. ((hr == 03) .or. (hr == 15)) ) then
     ...
     if ( file_exists ) then 
        ...
        call LIS_verify(iret, 'failed to open file '//trim(avgfile1))
        
        call grib_count_in_file(ftn,nvars,iret)
        call LIS_verify(iret, 'error in grib_count_in_file in retrieve_accum_ecmwfvars')
        ...
        do kk=1,nvars
           ...
           call LIS_verify(iret, 'failed to get values in retrieve_accum_ecmwfvars')
           if ( iret .ne. 0 ) then   ! read not successful
            write(LIS_logunit,*) 'problem reading accum1 for ',shortName,iret
            call grib_release(igrib)
            deallocate(lb)
            deallocate(f)     
            fret = -1
           ...
           if(var_found) then
              ...
              do r=1,LIS_rc%lnr(n)
                 do c=1,LIS_rc%lnc(n)
                    if ( LIS_domain(n)%gindex(c,r) /= -1 ) then 
                       glbdata1(var_index,LIS_domain(n)%gindex(c,r)) = &
                          varfield(c,r)
                    endif
                 enddo
              enddo
        ...
        write(LIS_logunit,*) 'ERR: Could not find file (A1): ',avgfile1   
        fret = -1
     endif
...
!
! !INTERFACE:
subroutine interp_ecmwf(n, findex, pcp_flag, necmwf,f,lb,lis_gds,nc,nr, &
     varfield)
! !USES:
  use LIS_coreMod,        only : LIS_rc, LIS_domain
  use ecmwf_forcingMod,   only : ecmwf_struc
