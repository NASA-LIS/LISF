...
! __________________________________________________________________________
  ...
  if(read_flag) then
     ...
     if (cmonth == month .and. cyear == year) then
        ...
        rec_size = dim_len
        write(LIS_logunit,*)'[INFO] in current month, reset rec_size =', &
             rec_size
        era5cds_struc(n)%tdimsize = rec_size
     endif
     ...
     if(era5cds_struc(n)%uselml.eq.1) then
        ...
        if(file_exists) then
           ...
           call LIS_verify(nf90_get_att(ftn,psId, "_FillValue", &
                missingValue), &
           ...
           do l=1,rec_size
              ...
              call interp_era5cds_var(n,findex,month,datalis,missingValue, &
                   .false.,&
           ...
           call LIS_verify(nf90_get_att(ftn,tmpId, "_FillValue", &
                missingValue), &
           ...
           call LIS_verify(nf90_get_att(ftn,uwindId, "_FillValue", &
                missingValue), &
           ...
           call LIS_verify(nf90_get_att(ftn,vwindId, "_FillValue", &
                missingValue), &
        ...
        if(file_exists) then
           ...
           write(LIS_logunit,*)'[INFO] Reading ERA5 2m10m (bookend,', &
                order,' -',trim(instfile), ')'
           ...
           allocate(qair(era5cds_struc(n)%ncold,era5cds_struc(n)%nrold, &
                rec_size))
              ...
              do r=1,LIS_rc%lnr(n)
                 do c=1,LIS_rc%lnc(n)
                    ...
                    if(era5cds_struc(n)%ps(k,l).ne.LIS_rc%udef.and.&
                       ...
                       era5cds_struc(n)%qair(k,l) = &
                            epsln*(A * EXP(-B/era5cds_struc(n)%qair(k,l))) / p_kPa
                    endif
                 enddo
              enddo
           enddo
           ...
           allocate(uwind(era5cds_struc(n)%ncold,era5cds_struc(n)%nrold, &
                rec_size))
           ...
           allocate(vwind(era5cds_struc(n)%ncold,era5cds_struc(n)%nrold, &
                rec_size))
     ...
     if(file_exists) then
        ...
        call LIS_verify(nf90_get_att(ftn,rainfId, "_FillValue", &
             missingValue), &
        ...
        allocate(crainf(era5cds_struc(n)%ncold,era5cds_struc(n)%nrold, &
             rec_size))
        ...
        call LIS_verify(nf90_get_att(ftn,crainfId, "_FillValue", &
             missingValue), &
     ...
     if(file_exists) then
        ...
        allocate(var1(era5cds_struc(n)%ncold,era5cds_struc(n)%nrold, &
             n_last_steps))
        call LIS_verify(nf90_inq_varid(ftn,'lsp',rainfId), &
             'nf90_inq_varid failed for lsp in read_era5cds')
        call LIS_verify(nf90_get_var(ftn,rainfId, var1, &
             start=(/1,1,start_time_index/), &
             count=(/era5cds_struc(n)%ncold,era5cds_struc(n)%nrold, &
             n_last_steps/)),&
             'nf90_get_var failed for lsp in read_era5cds')
  ...
  if (era5cds_struc(n)%tdimsize < tindex) then
     write(LIS_logunit,*) &
          '[WARN] Current month passed available data, filling with last data'
  ...
  if ( atindex .le. 0 ) then
     ...
     call assign_processed_era5cdsf(n,kk,order,3, &
          era5cds_struc(n)%prev_swd(:,atindex))
     call assign_processed_era5cdsf(n,kk,order,4, &
          era5cds_struc(n)%prev_lwd(:,atindex))
     call assign_processed_era5cdsf(n,kk,order,8, &
          era5cds_struc(n)%prev_rainf(:,atindex))
     call assign_processed_era5cdsf(n,kk,order,9, &
          era5cds_struc(n)%prev_crainf(:,atindex))
  else
     call assign_processed_era5cdsf(n,kk,order,3, &
          era5cds_struc(n)%swd(:,atindex))
     call assign_processed_era5cdsf(n,kk,order,4, &
          era5cds_struc(n)%lwd(:,atindex))
     call assign_processed_era5cdsf(n,kk,order,8, &
          era5cds_struc(n)%rainf(:,atindex))
     call assign_processed_era5cdsf(n,kk,order,9, &
          era5cds_struc(n)%crainf(:,atindex))
   endif
...
! !ARGUMENTS:
  ...
  integer, intent(in)    :: findex
  integer, intent(in)    :: month
  real,    intent(in)    :: input_var(era5cds_struc(n)%ncold, &
     ...
     if( era5cds_struc(n)%subset_nc == input_nc .and. &
        ...
        do r = 1, era5cds_struc(n)%subset_nr
           do c = 1, era5cds_struc(n)%subset_nc
              count1 = count1 + 1
              nrec = (era5cds_struc(n)%lat_line(c,r)-1)*input_nc + &
                   era5cds_struc(n)%lon_line(c,r)
...
subroutine era5grid_2_lisgrid( nx, ny, grid_data, out_data )
  ...
  if ((nx /= 1440) .or. (ny /= 720)) then
     write (LIS_logunit,*) 'era5grid_2_gldasgrid(): This routine has only been'
     write (LIS_logunit,*) 'checked for nx=1440 and ny=720. STOPPING'
     call LIS_endrun()
  end if
  if ((mod(nx,2) /= 0)) then
     write (LIS_logunit,*) 'era5grid_2_gldasgrid(): This routine can only work'
     write (LIS_logunit,*) 'for even nx. STOPPING.'
     call LIS_endrun()
  end if
