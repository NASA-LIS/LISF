...
! 
! !INTERFACE:
subroutine get_WRF_AKdom(n,findex)
...
!  
! !DESCRIPTION:
!  Opens, reads, and interpolates hourly, 4-km polar stereographic
!  WRF-AK forcing. At the beginning of a simulation, the code 
...
!  \item[read\_WRFAK](\ref{read_WRF_AKdom}) \newline
!    call to read the WRF AK data and perform spatial interpolation 
!   \item[read\_WRFAK\_elev](\ref{read_WRFAK_elev}) \newline
!    reads the native elevation of the WRF-AK data to be used
...
!EOP
 ...
 if(mod(nint(LIS_rc%ts),3600).eq.0) then
  if( LIS_rc%time.ge.WRFAK_struc(n)%WRFouttime2 ) then
  ...
  if(movetime.eq.1) then
     WRFAK_struc(n)%WRFouttime1 = WRFAK_struc(n)%WRFouttime2
     do f=1,LIS_rc%met_nf(findex)
        do c=1,LIS_rc%ngrid(n)
           WRFAK_struc(n)%metdata1(:,f,c)=WRFAK_struc(n)%metdata2(:,f,c)
        enddo
     enddo
  endif  ! if movetime=1
  ...
  if( WRFAK_struc(n)%findtime1==1 ) then  ! Need to get new time1 from the past
     ...
     do
        ...
        call read_WRF_AKdom(order,n,findex,yr1,mo1,da1,hr1,ferror)

        if ( ferror == 1 ) then !successfully retrieved forcing data
           WRFAK_struc(n)%WRFouttime1=time1
  ...
  if( WRFAK_struc(n)%findtime2.eq.1 ) then ! need new time2 data
     ...
     do
        ...
        try = try+1
        call read_WRF_AKdom(order,n,findex,yr2,mo2,da2,hr2,ferror)

        if ( ferror == 1 ) then !successfully retrieved forcing data
           write(LIS_logunit,*) '[INFO] reset WRFouttime2 to time2'
           WRFAK_struc(n)%WRFouttime2=time2
        else  ! ferror still=0, so roll back one day
