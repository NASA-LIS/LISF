...
!  NAM forecast datasets, transforms into 9 LIS forcing 
...
!==== Local Variables=======================
  ...
  real    :: namdata_i(10,LIS_rc%ngrid(n))
  real    :: namdata_a(10,LIS_rc%ngrid(n))
  real    :: namdata_a_f06(10,LIS_rc%ngrid(n))
  real    :: value
  !integer :: nvars
...
!=== End Variable Definition =======================
  ...
  namdata_i = LIS_rc%udef
  namdata_a = LIS_rc%udef
  namdata_a_f06 = LIS_rc%udef
  varfield = 0.0
  nnam = (nam242_struc(n)%ncold*nam242_struc(n)%nrold)
...
!--------------------------------------------------------------------------
  ...
  allocate(lb(nam242_struc(n)%ncold*nam242_struc(n)%nrold))
  allocate(f(nam242_struc(n)%ncold*nam242_struc(n)%nrold))
  ...
  if (file_exists) then
     ...
     if ( iret == 0 ) then
        ...
        elseif (fcstcode == '12') then
           ...
           write(LIS_logunit,*) 'ERR: read_nam242: Incorrect forecast time', &
                                fcstcode
        endif
     else
        write(LIS_logunit,*) 'ERR: read_nam242: Could not open ',trim(fname)
        ferror = 0
        deallocate(f)
        deallocate(lb)
        return
     ...
     !call LIS_verify(iret, 'ERR: read_nam242: Could not get number of vars')
     ...
     do
        ...
        call LIS_verify(iret,'ERR: read_nam242: could not get discipline value')
        ...
        call LIS_verify(iret, 'ERR: read_nam242: could not get level value')
        ...
        call LIS_verify(iret, 'ERR: read_nam242: could not get forecastTime value')
        ...
        if ( var_found ) then
           ...
           if ( iret /= 0 ) then
           !write(LIS_logunit,*) 'ERR: read_nam242: Could not get missingValue'
...
!--------------------------------------------------------------------------
           ...
           do r = 1, LIS_rc%lnr(n)
              do c = 1, LIS_rc%lnc(n)
                 if ( LIS_domain(n)%gindex(c,r) /= -1 ) then 
                    namdata_i(iv,LIS_domain(n)%gindex(c,r)) = varfield(c,r)
                 endif
              enddo
           enddo
        endif
        ...
        call grib_release(igrib,iret)
        call LIS_verify(iret, 'ERR: read_nam242: Could not release igrib')
     enddo
     ...
     write(LIS_logunit,*) 'ERR: read_nam242: ',trim(fname),' does not exist.'
     ferror = 0
     deallocate(f)
     deallocate(lb)
     return
...
!--------------------------------------------------------------------------
           ...
           do r = 1, LIS_rc%lnr(n)
              do c = 1, LIS_rc%lnc(n)
                 if ( LIS_domain(n)%gindex(c,r) /= -1 ) then 
                    namdata_a(iv,LIS_domain(n)%gindex(c,r)) = varfield(c,r)
                 endif
              enddo
           enddo
        endif
...
!--------------------------------------------------------------------------
 ...
 !--------------------------------------------------------------------------
            ...
            do r = 1, LIS_rc%lnr(n)
               do c = 1, LIS_rc%lnc(n)
                  if ( LIS_domain(n)%gindex(c,r) /= -1 ) then 
                     namdata_a_f06(iv,LIS_domain(n)%gindex(c,r)) = varfield(c,r)
                  endif
               enddo
            enddo
         endif
     ...
     do t=1,LIS_rc%ngrid(n)
        if ( F06flag ) then 
           if(iv.eq.8.or.iv.eq.9) then ! 3-6 hour time avgd: APCP, ACPCP
              if ( namdata_a_F06(iv,t) /= LIS_rc%udef ) then
                 value = namdata_a_F06(iv,t)/10800.0
              else
                 value = LIS_rc%udef
              endif
              if ( order == 1 ) then
                 nam242_struc(n)%metdata1(iv,t) = value
              else
                 nam242_struc(n)%metdata2(iv,t) = value
              endif
           else ! instantaneous
              if ( order == 1 ) then
                 nam242_struc(n)%metdata1(iv,t) = namdata_i(iv,t)
              else
                 nam242_struc(n)%metdata2(iv,t) = namdata_i(iv,t)
              endif
           endif
        else
           if(iv.eq.8.or.iv.eq.9) then ! 0-3 hour time avgd: APCP, ACPCP
              if ( namdata_a(iv,t) /= LIS_rc%udef ) then
...
#else
  write(LIS_logunit,*) 'ERR: read_nam242 requires GRIB-API'
...
!
! !INTERFACE:
subroutine interp_nam242(n,findex,iparam,nnam,f,lb,lis_gds,nc,nr,varfield)
! !USES:
  use LIS_coreMod,       only : LIS_rc, LIS_domain
  use nam242_forcingMod, only : nam242_struc
...
!-----------------------------------------------------------------------  
  if(LIS_rc%met_interp(findex).eq."bilinear") then 
     call bilinear_interp(lis_gds,lb,f,lo,lis1d,nam242_struc(n)%mi,mo,&
