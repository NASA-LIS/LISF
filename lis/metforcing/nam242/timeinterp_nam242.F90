...
!EOP
  ...
  wt1 = (nam242_struc(n)%namtime2-LIS_rc%time) / & 
...
!-----------------------------------------------------------------
  do f = 1,LIS_rc%nf  
     if ( (f == 3) .or. (f == 4) ) then
        do t=1,LIS_rc%ngrid(n)
           if ( (LIS_forc(n)%glbdata2(f,t) /= -9999.9) .and.  &
                (LIS_forc(n)%glbdata2(f,t) < 0)) then
              LIS_forc(n)%glbdata2(f,t) = (-1) * LIS_forc(n)%glbdata2(f,t)
           endif
           if ( (LIS_forc(n)%glbdata1(f,t) /= -9999.9) .and.  &
                (LIS_forc(n)%glbdata1(f,t) < 0)) then
              LIS_forc(n)%glbdata1(f,t) = (-1) * LIS_forc(n)%glbdata1(f,t)
           endif
        enddo
     endif
  enddo
  ...
  do t = 1, LIS_rc%ntiles(n)
     ...
     if ( nam242_struc(n)%metdata1(3,index1) /= LIS_rc%udef .and. &
          nam242_struc(n)%metdata2(3,index1) /= LIS_rc%udef ) then
        swd(t) = zw1 * nam242_struc(n)%metdata1(3,index1) + &
                 zw2 * nam242_struc(n)%metdata2(3,index1)
     endif

     if ( swd(t) < 0 .and. swd(t) /= LIS_rc%udef ) then
        write(LIS_logunit,*) '2 warning!!!  SW radiation is negative!!'
        write(LIS_logunit,*) 'sw=', swd(t), '... negative'
        write(LIS_logunit,*) 'nam2=', nam242_struc(n)%metdata2(3,index1)
        call LIS_endrun
     end if
           
     if ( nam242_struc(n)%metdata2(3,index1) /= LIS_rc%udef ) then
        if (swd(t).gt.LIS_CONST_SOLAR) then
           swd(t)=nam242_struc(n)%metdata2(3,index1)
        endif
     endif
  end do
  ...
  do t = 1, LIS_rc%ntiles(n)     
     index1 = LIS_domain(n)%tile(t)%index
     if ( nam242_struc(n)%metdata1(4,index1) /= LIS_rc%udef .and. &
          nam242_struc(n)%metdata2(4,index1) /= LIS_rc%udef ) then
        lwd(t) = wt1 * nam242_struc(n)%metdata1(4,index1) + &
                 wt2 * nam242_struc(n)%metdata2(4,index1)
     endif

     if ( lwd(t) < 0 .and. lwd(t) /= LIS_rc%udef ) then
        write(LIS_logunit,*) '2 warning!!!  LW radiation is negative!!'
        write(LIS_logunit,*) 'lw=', lwd(t), '... negative'
        write(LIS_logunit,*) 'nam2=', nam242_struc(n)%metdata2(4,index1)
        call LIS_endrun
     end if
  end do
  ...
  do t = 1, LIS_rc%ntiles(n)
     index1 = LIS_domain(n)%tile(t)%index
     if ( nam242_struc(n)%metdata2(8,index1) /= LIS_rc%udef ) then
        pcp(t) = nam242_struc(n)%metdata2(8,index1)   
     endif
  end do
  ...
  do t = 1, LIS_rc%ntiles(n)
     index1 = LIS_domain(n)%tile(t)%index     
     if ( nam242_struc(n)%metdata2(9,index1) /= LIS_rc%udef ) then
        cpcp(t) = nam242_struc(n)%metdata2(9,index1)     
     endif
  end do

  do t = 1, LIS_rc%ntiles(n)
     index1 = LIS_domain(n)%tile(t)%index
     if ( nam242_struc(n)%metdata1(1,index1) /= LIS_rc%udef .and. &
          nam242_struc(n)%metdata2(1,index1) /= LIS_rc%udef ) then
        tmp(t) = wt1 * nam242_struc(n)%metdata1(1,index1) + &
                 wt2 * nam242_struc(n)%metdata2(1,index1)
     endif
  end do

  do t = 1, LIS_rc%ntiles(n)
     index1 = LIS_domain(n)%tile(t)%index
     if ( nam242_struc(n)%metdata1(2,index1) /= LIS_rc%udef .and. &
          nam242_struc(n)%metdata2(2,index1) /= LIS_rc%udef ) then
        q2(t) = wt1 * nam242_struc(n)%metdata1(2,index1) + &
                wt2 * nam242_struc(n)%metdata2(2,index1)
     endif
  end do

  do t = 1, LIS_rc%ntiles(n)
     index1 = LIS_domain(n)%tile(t)%index
     if ( nam242_struc(n)%metdata1(5,index1) /= LIS_rc%udef .and. &
          nam242_struc(n)%metdata2(5,index1) /= LIS_rc%udef ) then
        uwind(t) = wt1 * nam242_struc(n)%metdata1(5,index1) + &
                   wt2 * nam242_struc(n)%metdata2(5,index1)
     endif
  end do

  do t = 1, LIS_rc%ntiles(n)
     index1 = LIS_domain(n)%tile(t)%index
     if ( nam242_struc(n)%metdata1(6,index1) /= LIS_rc%udef .and. &
          nam242_struc(n)%metdata2(6,index1) /= LIS_rc%udef ) then
        vwind(t) = wt1 * nam242_struc(n)%metdata1(6,index1) + &
                   wt2 * nam242_struc(n)%metdata2(6,index1)
     endif
  end do

  do t = 1, LIS_rc%ntiles(n)
     index1 = LIS_domain(n)%tile(t)%index
     if ( nam242_struc(n)%metdata1(7,index1) /= LIS_rc%udef .and. &
          nam242_struc(n)%metdata2(7,index1) /= LIS_rc%udef ) then
        psurf(t) = wt1 * nam242_struc(n)%metdata1(7,index1) + &
                   wt2 * nam242_struc(n)%metdata2(7,index1)
     endif
  end do
