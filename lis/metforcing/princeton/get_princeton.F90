...
!EOP
  ...
  if(LIS_rc%time.gt.princeton_struc(n)%princetontime2) then
     movetime=1
  ...
  if (princeton_struc(n)%findtime1==1) then  !need to get new time1 from the past
     ...
     do
        ...
        if ( try > ndays ) then 
           write(LIS_logunit,*) '[ERR] Princeton data gap exceeds 10 days'
  ...
  if(movetime.eq.1) then
     ...
     do f=1,nforce
        do c=1,LIS_rc%ngrid(n)
           princeton_struc(n)%metdata1(:,f,c)=princeton_struc(n)%metdata2(:,f,c)
        enddo
     enddo
  endif  ! if movetime=1
  ...
  if(princeton_struc(n)%findtime2.eq.1) then ! need new time2 data
     ...
     do
        ...
        if ( ferror == 1 ) then !successfully retrieved forcing data
           write(LIS_logunit,*) '[INFO] reset princetontime2 to time2'
