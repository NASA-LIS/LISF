...
#include "LIS_misc.h"
!BOP
! !ROUTINE: read_edas_elev
!  \label{read_edas_elev}
!
! !REVISION HISTORY:
!
!  17Dec2004; Sujay Kumar; Initial Specificaton
!  24 Aug 2007: Chuck Alonge; Modified for use with EDAS data
!
! !INTERFACE:
subroutine read_edas_elev(n,findex)
! !USES:
  use LIS_coreMod
...
! !DESCRIPTION:
!
!  Opens, reads, and interpolates EDAS model elevation to the LIS
...
!EOP
   ...
   if(file_exists) then 
      ...
      write(LIS_logunit,*) " Reading EDAS elevation data ... "

      call LIS_verify(nf90_open(path=LIS_rc%paramfile(n),&
           mode=NF90_NOWRITE,ncid=nid),&
           'nf90_open failed in read_edas_elev')
      call LIS_verify(nf90_inq_varid(nid,"ELEV_NLDAS1",elevId),&
           'nf90_inq_varid failed in read_edas_elev')
      call LIS_verify(nf90_get_var(nid,elevId,elev),&
           'nf90_get_var failed in read_edas_elev')      
   ...
   write(LIS_logunit,*) & 
        "Finish reading original EDAS elevation data"
#endif

end subroutine read_edas_elev
