...
! !USES:
  ...
  use LIS_logMod, only         : LIS_logunit, LIS_verify, LIS_warning, LIS_endrun
...
!  NLDAS-1 data, transforms into 9 LIS forcing 
!  parameters and interpolates to the LIS spatial domain.
...
!EOP
  ...
  ferror = 1
  iv_total = 15
  nldas1 = (nldas1_struc(n)%ncold*nldas1_struc(n)%nrold)
...
!  pds2 = (/ 084,084,084,084,084,084,084,084,084,084,154,154,154,155,156/)
  ...
  if (file_exists) then   
     ...
     call LIS_verify(iret, 'error in grib_count_in_file in read_nldas1')
     
     do iv=1,nvars

        call grib_new_from_file(ftn, igrib, iret)
        call LIS_warning(iret,&
        ...
        call LIS_verify(rc, 'error in grib_get: level in read_nldas1')
        ...
        call LIS_warning(rc,'error in grib_get:values in read_nldas1')
        ...
        call LIS_verify(rc, 'error in grib_release in read_nldas1')       
     enddo

     call grib_close_file(ftn)

  else
     write(LIS_logunit,*) 'Could not find file: ',trim(name)
     ferror = 0
     deallocate(f)
     deallocate(f2)
     deallocate(lb)
...
!         if ( f(t,6) .ge. -9998.0 ) then
         ...
         write(LIS_logunit,*) 'NLDAS1 shortwave radiation field:'
         write(LIS_logunit,*) ' must be equal to either "NLDAS1"'
         write(LIS_logunit,*) ' or to "EDAS".  Please check your'
      ...
      elseif (nldas1_struc(n)%prec_field.eq."STAGEII") then
         ...
         write(LIS_logunit,*) 'NLDAS1 precipitation field:'
         write(LIS_logunit,*) ' must be equal to "NLDAS1", "EDAS",'
         write(LIS_logunit,*) ' or to "STAGEII".  Please check your'
         write(LIS_logunit,*) ' config file.  Stopping....'
         call LIS_endrun()
      endif
...
!- Reinterpolate original NLDAS-1 forcing to LIS grid:
  do iv=1,9
     lb = .false. 
     do t=1,nldas1
     ...
     if(nldas1_struc(n)%applymask.eq.1) then 
        call apply_conusmask(n, LIS_rc%lnc(n), LIS_rc%lnr(n), &
             varfield)
     endif
      ...
      do r=1,LIS_rc%lnr(n)
         do c=1,LIS_rc%lnc(n)
            if(LIS_domain(n)%gindex(c,r).ne.-1) then 
               if(order.eq.1) then 
                  nldas1_struc(n)%metdata1(iv,LIS_domain(n)%gindex(c,r)) =&
                       varfield(c,r)
               elseif(order.eq.2) then 
                  nldas1_struc(n)%metdata2(iv,LIS_domain(n)%gindex(c,r)) =&
                       varfield(c,r)
               endif
            endif
         end do
