...
!BOP
! !ROUTINE: timeinterp_hmaens
! \label{timeinterp_hmaens}
!
! !REVISION HISTORY:
...
subroutine timeinterp_hmaens(n,findex)
...
!EOP
  ...
  btime=hmaens_struc(n)%hmaenstime1
  ...
  btime=hmaens_struc(n)%hmaenstime2
...
!=== Interpolate Data in time      
  wt1=(hmaens_struc(n)%hmaenstime2-LIS_rc%time)/ & 
       (hmaens_struc(n)%hmaenstime2-hmaens_struc(n)%hmaenstime1)
  wt2=1.0-wt1
  ...
  call ESMF_StateGet(LIS_FORC_Base_State(n,findex),LIS_FORC_Prec%varname(1),pcpField,&
       rc=status)
  call LIS_verify(status, 'Error: Enable Prec in the forcing variables list')
  ...
  mfactor = LIS_rc%nensem(n)/hmaens_struc(n)%nIter
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        if (hmaens_struc(n)%metdata1(kk,3,index1).ne.LIS_rc%udef.and.&
             hmaens_struc(n)%metdata2(kk,3,index1).ne.LIS_rc%udef) then 
           swd(t) = hmaens_struc(n)%metdata1(kk,3,index1)*zw1 + &
                hmaens_struc(n)%metdata2(kk,3,index1)*zw2

           ! In cases of small cos(zenith) angles, use linear weighting
           !  to avoid overly large weights

           if((swd(t).gt.hmaens_struc(n)%metdata1(kk,3,index1).and. & 
                swd(t).gt.hmaens_struc(n)%metdata2(kk,3,index1)).and. & 
                (czb.lt.0.1.or.cze.lt.0.1))then
              swd(t) = hmaens_struc(n)%metdata1(kk,3,index1)*swt1+ & 
                   hmaens_struc(n)%metdata2(kk,3,index1)*swt2
           endif
           ...
           if (swd(t).gt.LIS_CONST_SOLAR) then
              ...
              write(unit=LIS_logunit,fmt=*)'[WARN] hmaensdata1=',&
                   hmaens_struc(n)%metdata1(kk,3,index1)
              write(unit=LIS_logunit,fmt=*)'[WARN] hmaensdata2=',&
                   hmaens_struc(n)%metdata2(kk,3,index1)
        ...
        if ((swd(t).ne.LIS_rc%udef).and.(swd(t).lt.0)) then
           if (swd(t).gt.-0.00001) then 
              ...
              write(LIS_logunit,*) &
                   '[ERR] timeinterp_hmaens -- Stopping because ', & 
                   'forcing not udef but lt0,'
              write(LIS_logunit,*)'[ERR] timeinterp_hmaens -- ', & 
                   t,swd(t),hmaens_struc(n)%metdata2(kk,3,index1), & 
                   ' (',LIS_localPet,')'
              call LIS_endrun
           endif
        endif
        ...
        if (swd(t).gt.LIS_CONST_SOLAR) then
           swd(t)=hmaens_struc(n)%metdata2(kk,3,index1)
        endif
     enddo
  enddo
...
!-----------------------------------------------------------------------  
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
        pcp(t) = hmaens_struc(n)%metdata1(kk,7,index1)
...
!-----------------------------------------------------------------------
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        tmp(t) = hmaens_struc(n)%metdata1(kk,1,index1)*wt1 + &
             hmaens_struc(n)%metdata2(kk,1,index1)*wt2
        q2(t)  = hmaens_struc(n)%metdata1(kk,2,index1)*wt1 + &
             hmaens_struc(n)%metdata2(kk,2,index1)*wt2
        lwd(t)  = hmaens_struc(n)%metdata1(kk,4,index1)*wt1 + &
             hmaens_struc(n)%metdata2(kk,4,index1)*wt2

        uwind(t) = hmaens_struc(n)%metdata1(kk,5,index1)*wt1+&
             hmaens_struc(n)%metdata2(kk,5,index1)*wt2
        vwind(t) = 0.0
        psurf(t) = hmaens_struc(n)%metdata1(kk,6,index1)*wt1 + &
             hmaens_struc(n)%metdata2(kk,6,index1)*wt2

     enddo
  enddo

end subroutine timeinterp_hmaens
