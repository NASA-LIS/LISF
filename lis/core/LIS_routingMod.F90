...
!EOP
    ...
    if(LIS_rc%routingmodel.eq."RAPID router") then
       ...
       do n=1, LIS_rc%nnest
          ...
          LIS_runoff_state(n) = ESMF_StateCreate( &
               name="LIS Runoff State"//&
       ...
       do n=1,LIS_rc%nnest
          ...
          allocate(LIS_routing(n)%ntiles_pergrid( &
       ...
       do n=1,LIS_rc%nnest
          ...
          do k=1,LIS_rc%nroutinggrid(n)*LIS_rc%nensem(n)
             ...
             !no subgrid routing tiling for now
             LIS_routing(n)%tile(k)%fgrd = 1.0
          enddo
...
!EOP
    ...
    external :: lsmroutinggetrunoff
    external :: glacierroutinggetrunoff
    external :: routingrun
    external :: lsmroutinggetsws
    ...
    if(LIS_rc%routingmodel.ne."none") then
       ...
       if(LIS_rc%lsm.ne."none" .and. &
            LIS_rc%routingmodel.ne."RAPID router") then
...
!EOP
    ...
    if(LIS_rc%routingmodel.ne."none" .and. &
         LIS_rc%routingmodel.ne."RAPID router") then
       do k=1, LIS_rc%nperts
          if(LIS_rc%Routing_DAinst_valid(k)) then
             if(LIS_rc%perturb_state(k).ne."none") then
                ...
                if(mod(curr_time, &
                     real(LIS_rc%pertstateInterval(k))).eq.0) then

!------------------------------------------------------------------------
!   Returns the perturbed state based on the chosen algorithm
!------------------------------------------------------------------------
                   call perturbmethod( &
                        trim(LIS_rc%perturb_state(k))//char(0),&
                        4, n,k,&
                        LIS_Routing_State(n,k), &
                        LIS_Routing_Pert_State(n,k))
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
                   call routingdaqcstate( &
                        trim(LIS_rc%routingModel)//"+"//&
                        trim(LIS_rc%daset(k))//char(0), n, &
                        LIS_Routing_State(n,k))
                   call routingdasetstatevar( &
                        trim(LIS_rc%routingModel)//"+"//&
                        trim(LIS_rc%daset(k))//char(0), n, &
                        LIS_Routing_State(n,k))
                endif
...
!
! !INTERFACE:
  subroutine applyRoutingPert(n, k, LIS_Routing_State, &
       LIS_Routing_Pert_State)
! !USES:
...
!EOP
    ...
    call ESMF_StateGet(LIS_Routing_State,itemCount=lsm_state_count, &
         rc=status)
    call LIS_verify(status, &
    ...
    call ESMF_StateGet(LIS_Routing_State,itemNameList=lsm_state_objs, &
         rc=status)
    call LIS_verify(status,&
         "ESMF_StateGet failed in applyRoutingPert")

    do i=1,lsm_state_count
       call ESMF_StateGet(LIS_Routing_State,lsm_state_objs(i), &
            lsm_field(i),&
            rc=status)
       call LIS_verify(status,&
            "ESMF_StateGet failed in applyRoutingPert")
       call ESMF_FieldGet(lsm_field(i), localDE=0,farrayPtr=lsm_temp, &
            rc=status)
       call LIS_verify(status,&
...
!EOP
    ...
    if(LIS_rc%routingmodel.ne."none" .and. &
       ...
       if(LIS_rc%routing_DAinst_valid(k)) then
          ...
          call ESMF_StateGet(LIS_Routing_State(n,k), &
               itemNameList=routing_state_objs,&
               rc=status)
          call LIS_verify(status, &
          ...
          do v=1,LIS_rc%nstvars(k)
             call ESMF_StateGet(LIS_Routing_State(n,k), &
                  trim(routing_state_objs(v)),&
                  routing_field(v),rc=status)
             call LIS_verify(status, &
                  "ESMF_StateGet failed in enkf_increments")

             call ESMF_FieldGet(routing_field(v),localDE=0, &
                  farrayPtr=stdata,rc=status)
             call LIS_verify(status,&
...
!EOP
    ...
    type(ESMF_Field)                  :: routing_field(LIS_rc%nstvars(k))
    type(ESMF_Field)                  :: &
         routing_incr_field(LIS_rc%nstvars(k))
    real,         pointer             :: stdata(:)
    ...
    if(LIS_rc%routingmodel.ne."none" .and. &
       ...
       if(LIS_rc%routing_DAinst_valid(k)) then
          ...
          do v=1,LIS_rc%nstvars(k)
             ...
             call ESMF_StateGet(LIS_Routing_Incr_State(n,k), &
                  trim(routing_state_objs(v)),&
             ...
             call ESMF_FieldGet(routing_incr_field(v),localDE=0, &
                  farrayPtr=stincrdata,&
                  rc=status)
             call LIS_verify(status, &
                  'ESMF_FieldGet failed in enkf_increments')
...
! !DESCRIPTION:
!
!  This interface extracts the variable names from the state vector
!  object
!
!  The arguments are:
!  \begin{description}
...
!EOP
    if(LIS_rc%routingmodel.ne."none" .and. &
         LIS_rc%routingmodel.ne."RAPID router") then
       if(LIS_rc%Routing_DAinst_valid(k)) then
          call routingdagetstatespacesize( &
               trim(LIS_rc%routingmodel)//"+"//&
