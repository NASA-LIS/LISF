...
!
!EOP
  end interface LIS_create_dapert_filename
...
!
!EOP
  end interface LIS_create_restart_filename
...
!EOP
  ...
  end interface LIS_create_output_filename
contains
...
!EOP
    ...
    elseif(LIS_rc%wstyle.eq."557WW NRT forecast convention" .or. &
    ...
    if (ios .ne. 0) then
       write(LIS_logunit,*)'[WARN] creating directory ',trim(out_dname)
       flush(LIS_logunit)
    end if
...
!EOP
    ...
    elseif(LIS_rc%wstyle.eq."557WW NRT forecast convention") then
       write(unit=fint,fmt='(i2.2)') nint(writeint)/3600
       write(unit=cdate1, fmt='(i4.4, i2.2, i2.2)') &
            LIS_rc%yr, LIS_rc%mo, LIS_rc%da
...
!EOP
    ...
    elseif(LIS_rc%wstyle.eq."2 level hierarchy".or.&
         ...
         LIS_rc%wstyle.eq."557WW NRT forecast convention" .or. &
         LIS_rc%wstyle.eq."557WW medium range forecast convention") then
       write(unit=cdate1, fmt='(i4.4, i2.2, i2.2, i2.2, i2.2)') &
            LIS_rc%yr, LIS_rc%mo, &
            LIS_rc%da, LIS_rc%hr,LIS_rc%mn
...
!EOP
    ...
    elseif(LIS_rc%param_proj.eq."hrap") then !HRAP
       ...
       write(LIS_logunit,*) &
            '[ERR] Reading configuration settings for this projection ',&
            trim(LIS_rc%param_proj)
       write(LIS_logunit,*) 'is not supported'
