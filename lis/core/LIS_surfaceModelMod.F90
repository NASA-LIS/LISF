...
!EOP
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "Surface model output interval:", rc=rc)
    do n=1,LIS_rc%nnest
       ...
       if ( time == "dekad" ) then
          ...
          call LIS_registerAlarm("LIS surface model output alarm "// &
               trim(fnest), &
               LIS_sfmodel_struc(n)%ts,&
               LIS_sfmodel_struc(n)%outInterval,&
               intervalType="dekad", dek_offset=0, &
               when="end")
       else
...
!               trim(LIS_sfmodel_struc(n)%models_used)//&
!               "+"//trim(LIS_rc%lakemodel)
       elseif(LIS_rc%sf_model_type_select(m).eq. &
            LIS_rc%openwater_index) then
...
!EOP
    ...
    do m=1,LIS_rc%nsf_model_types
       ...
       elseif(LIS_rc%sf_model_type_select(m).eq.LIS_rc%openwater_index) &
            then
          ...
          if(LIS_rc%output_at_specifictime.eq.LIS_rc%lsm_index) then
             ...
             if ( LIS_sfmodel_struc(n)%outIntervalType == "dekad" ) then
                alarmCheck = LIS_isAlarmRinging(LIS_rc, &
                     "LIS surface model output alarm "// &
                     trim(fnest), &
                     LIS_sfmodel_struc(n)%outIntervalType)
             else
                alarmCheck = LIS_isAlarmRinging(LIS_rc, &
          ...
          if(alarmCheck) then
             ...
             if(LIS_masterproc) then
                if (LIS_sfmodel_struc(n)%stats_file_open) then
                   call LIS_create_stats_filename(n, statsfile, &
                        'SURFACEMODEL')
...
!EOP
    ...
    if(LIS_rc%wopt_rst.ne.0) then
       do m=1,LIS_rc%nsf_model_types
          ...
          elseif(LIS_rc%sf_model_type_select(m).eq.LIS_rc%lake_index) &
               then
...
!EOP
    ...
    do m=1,LIS_rc%nsf_model_types
       ...
       elseif(LIS_rc%sf_model_type_select(m).eq.LIS_rc%glacier_index) &
            then
...
!
! !INTERFACE:
  subroutine LIS_surfaceModel_DAextractStateVector(n,k,Nstate, &
       state_size,stvar)
...
!building grid space from surface model tiles
    ...
    call LIS_tile2grid(n,LISWRF_export(n)%avgsurft, &
         LISWRF_export(n)%avgsurft_t)
    call LIS_tile2grid(n,LISWRF_export(n)%qh,LISWRF_export(n)%qh_t)
    call LIS_tile2grid(n,LISWRF_export(n)%eta_kinematic, &
         LISWRF_export(n)%eta_kinematic_t)
    ...
    call LIS_tile2grid(n,LISWRF_export(n)%rootmoist, &
         LISWRF_export(n)%rootmoist_t)
