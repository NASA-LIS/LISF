...
!EOP
   ...
   if ( gaussian_find_row == -9999 ) then
      write(*,fmt=*) 'ERR: gaussian_find_row -- Could not find lat',lat
      stop
   endif
...
!EOP
   ...
   if ( gaussian_find_col == -9999 ) then
      write(*,fmt=*) 'ERR: gaussian_find_col -- Could not find lon',lon
      stop
   endif
...
!EOP
   ...
   write(unit=*,fmt=*) 'gaussian lats:', gaussian_lat_array
...
!EOP
   ...
   write(unit=*,fmt=*) 'gaussian lons:', gaussian_lon_array
...
!EOP
   ...
   !write(unit=LIS_logunit,fmt=*) 'gaussian lats:', gaussian_latitudes(1,:)
   !write(unit=LIS_logunit,fmt=*) 'gaussian lons:', gaussian_longitudes(:,1)
   write(unit=*,fmt=*) 'gaussian lats:', gaussian_lat_array
   write(unit=*,fmt=*) 'gaussian lons:', gaussian_lon_array
