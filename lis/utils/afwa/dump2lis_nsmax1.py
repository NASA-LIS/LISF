...
class dump2lis(object):
  ...
  def prepare(self):
    ...
    if os.path.isfile(self.dump2nc)==True:
      ...
      print("File %s has been deleted. A new file will be created." %self.dump2nc)
  ...
  def update_lis_variable(self, var_name, lev=-1):
    ...
    if lev==-1:
      ...
      print("data range: %12.6f %12.6f" %(self.nc_rst[var_name][:].min(), self.nc_rst[var_name][:].max()))
    else:
      self.nc_rst[var_name][lev,:] = self.lis_var.transpose()
      print("data range: %12.6f %12.6f" %(self.nc_rst[var_name][lev,:].min(), self.nc_rst[var_name][lev,:].max()))
