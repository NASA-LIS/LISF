...
def generate_forecast_hours():
    ...
    return [f"{i:03d}" for i in range(0, 195, 3)] + [f"{i:03d}" for i in range(198, 385, 6)]

def generate_members():
    """Generate a list of member directory names."""
    return ["member000"] + [f"member{str(i).zfill(3)}" for i in range(28, 37)]

def build_filename(base_prefix, grib_suffix, memb_id, fh):
    """Construct the expected GRIB2 filename for a given member and forecast hour."""
...
def write_log_file(log_path, date_str, cycle_str, missing_members, \
    ...
    with open(full_path, "w", encoding="ascii") as log:
        ...
        if missing_members:
            log.write(f"Missing member folders ({len(missing_members)}): {', '.join(missing_members)}\n")
        else:
