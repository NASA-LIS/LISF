...
def _handle_dates(config, year, month, input_numfcstmons):
...
def _driver(config):
    ...
    for nmme_model in _NMME_MODELS:
        ...
        if nseg == 1:
            # The entire forecast is run by a single job
            dates = _handle_dates(config, FORECAST_YEAR, FORECAST_MONTH, input_numfcstmons)
            lisconfig_target = \
            ...
            for i in range (nseg):
                jno = '_{:02d}'.format(i)
                if i == 0:
                    dates.append(_handle_dates(config, FORECAST_YEAR, FORECAST_MONTH, slen[i]))
                    lisconfig_target = \
                        f"{final_outputdir}/{lisconfig_nameconv}" + \
                        f"{dates[0]['start'].year:04d}{dates[0]['start'].month:02d}" +  '_{:02d}'.format(i)
                    shutil.copy(lisconfig_template, lisconfig_target)
                    _customize_lisconfig(lisconfig_target, config, dates[0], \
                                         nmme_model, fcstdir)
                else:
                    dates.append(_handle_dates(config, dates[i-1].get('end').year, dates[i-1].get('end').month, slen[i]))
                    lisconfig_target = \
                        f"{final_outputdir}/{lisconfig_nameconv}" + \
                        f"{dates[0]['start'].year:04d}{dates[0]['start'].month:02d}" +  '_{:02d}'.format(i)
                    shutil.copy(lisconfig_template, lisconfig_target)
