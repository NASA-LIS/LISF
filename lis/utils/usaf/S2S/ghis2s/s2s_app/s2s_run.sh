...
submit_job(){
	...
	python $LISHDIR/ghis2s/s2s_app/s2s_api.py -s $JOB_SCHEDULE -m $submit_ID -f $2 -c $BWD/$CFILE
    else
	submit_ID="`sbatch --dependency=afterok:$1 $2 |  cut -d' ' -f4`"
	c2c=`echo $1 | sed "s|:|,|g"`
	python $LISHDIR/ghis2s/s2s_app/s2s_api.py -s $JOB_SCHEDULE -m $submit_ID -f $2 -a `echo $c2c` -c $BWD/$CFILE
    fi
    echo $submit_ID
}
...
lis_darun(){
    ...
    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c ${BWD}/${CFILE} -f lisda_run.j -H 5 -j lisda_ -w ${CWD} -L Y
    if [[ $NODE_NAME =~ discover* ]] || [[ $NODE_NAME =~ borg* ]]; then
...
ldt_ics(){
    ...
    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ldtics_run.j -t 1 -H 2 -j ldtics_ -w ${CWD}
...
bcsd_fcst(){
	...
	do
		...
		job_comm=`grep python ${jfile} | cut -d'|' -f1`
	...
	if [ $GROUP_JOBS == "Y" ]; then
	    bcsd01_ID=
	    /bin/rm bcsd01_*.j
	    split -l 2  $cmdfile part_
	    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_01_run.j -t 1 -H 3 -j ${jobname}_01_ -w ${CWD} -C "part_aa"
	    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_02_run.j -t 1 -H 3 -j ${jobname}_02_ -w ${CWD} -C "part_ab"
	    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_03_run.j -t 1 -H 3 -j ${jobname}_03_ -w ${CWD} -C "part_ac"
	    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_04_run.j -t 1 -H 3 -j ${jobname}_04_ -w ${CWD} -C "part_ad"
	    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_05_run.j -t 1 -H 3 -j ${jobname}_05_ -w ${CWD} -C "part_ae"
	    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_06_run.j -t 1 -H 3 -j ${jobname}_06_ -w ${CWD} -C "part_af"
	    /bin/rm ${cmdfile} "part_aa" "part_ab" "part_ac" "part_ad" "part_ae" "part_af"
	    ...
	    bcsd01_ID=`echo $bcsd01_ID`' '$thisID
	    thisID=$(submit_job "" "${jobname}_05_run.j")
	    bcsd01_ID=`echo $bcsd01_ID`' '$thisID
	    thisID=$(submit_job "" "${jobname}_06_run.j")
	    bcsd01_ID=`echo $bcsd01_ID`' '$thisID
	    bcsd01_ID=`echo $bcsd01_ID | sed "s| |:|g"`
	fi
	...
	if [ $GROUP_JOBS == "Y" ]; then
	    bcsd03_ID=
	    /bin/rm bcsd03*.j
	    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_run.j -t 1 -H 3 -j ${jobname}_ -w ${CWD} -C ${cmdfile}
	    /bin/rm ${cmdfile}
	...
	python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_01_run.j -t 1 -H 4 -j ${jobname}_01_ -w ${CWD} -C "part_aa"
	python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_02_run.j -t 1 -H 4 -j ${jobname}_02_ -w ${CWD} -C "part_ab"
	...
	bcsd06_ID=
	/bin/rm bcsd06*.j
	python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_run.j -t 1 -H 5 -j ${jobname}_ -w ${CWD} -C ${cmdfile}
	/bin/rm ${cmdfile}
	...
	/bin/rm bcsd09*.j
	/bin/rm bcsd10*.j
	python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_run.j -t 1 -H 6 -j ${jobname}_ -w ${CWD} -C ${cmdfile}
	/bin/rm ${cmdfile}
	bcsd09_ID=$(submit_job "$bcsd08_ID" "${jobname}_run.j")
	bcsd10_ID=
    fi
...
lis_fcst(){
	...
	do
		...
		if [ $FileNo  -eq 1 ]; then
		    if [ $GROUP_JOBS == "Y" ]; then
			thisID=$(submit_job "$bcsd11_ID" "$jfile")
		    else
...
s2smetrics(){
    ...
    if [ $GROUP_JOBS == "Y" ]; then
	/bin/rm s2smetric_*.j
	python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_run.j -t 1 -H 4 -j ${jobname}_ -w ${CWD} -C $cmdfile
	/bin/rm ${cmdfile}
	s2smetric_ID=$(submit_job "$s2spost_ID" "${jobname}_run.j")
    fi
    
    # write tiff file
    python $LISHDIR/ghis2s/s2s_app/s2s_api.py -c $BWD/$CFILE -f ${jobname}_tiff_run.j -t 1 -H 3 -j ${jobname}_tiff_ -w ${CWD}
...
if [ "$REPORT" = 'Y' ] || [ "$REPORT" = 'y' ]; then
    # Print status report
    if [[ $NODE_NAME =~ discover* ]] || [[ $NODE_NAME =~ borg* ]]; then
	#python $LISHDIR/ghis2s/s2s_app/s2s_api.py -r Y  -c $BWD/$CFILE -w ${E2ESDIR} -d ${YYYY}${MM}
	print_walltimes
    else
	print_walltimes
    fi
