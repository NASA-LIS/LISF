'''
GHI-S2S Main Script that sets upp the experiment
'''
...
class S2Srun(DownloadForecasts):
    ...
    def clim_files_checker(self):
        ...
        obs_path = self.e2esdir + f'hindcast/bcsd_fcst/{self.obs_model}_{resol}/raw/Climatology/'

        for var in obs_var_list:
            check_file(obs_clim_file_template.format(obs_path, var), f'{self.obs_model}_{resol} Clim')
    ...
    def write_log_monitoring_script(self):
        ...
        with open(log_script_path, 'w', encoding="utf-8") as f:
            ...
            if not is_nccs:
                f.write('#SBATCH --cluster-constraint=' + self.config['SETUP']['CONSTRAINT'] + '\n')
    ...
    def bcsd(self, clim=None):
        """ BCSD 7 steps """
        lats, _ = utils.get_domain_info(self.config_file, coord=True)
        self._resol = f'{round((lats[1] - lats[0])*100)}km'
        fcast_clim_dir=\
            f'{self.e2esroot}/hindcast/bcsd_fcst/{self.fcst_model}_{self._resol}/raw/Climatology/'
        nmme_clim_dir=f'{self.e2esroot}/hindcast/bcsd_fcst/NMME/raw/Climatology/'
        usaf_25km=f'{self.e2esroot}/hindcast/bcsd_fcst/{self.obs_model}_{self._resol}/raw/Climatology/'

        os.makedirs(self.e2esdir + '/bcsd_fcst', exist_ok=True)
        os.chdir(self.e2esdir + '/bcsd_fcst')
        os.makedirs(f'{self.obs_model}_{self._resol}/raw', exist_ok=True)
        os.makedirs(f'{self.fcst_model}_{self._resol}/raw/', exist_ok=True)
        ...
        self._cwd=self.scrdir + 'bcsd_fcst'

        date_obj = datetime.strptime(f"{self.yyyy}-{self.mm}-01", "%Y-%m-%d")
        self._mmm = date_obj.strftime("%b").lower()

        if self.hindcast:
            if clim:
                os.makedirs(f'{self.e2esdir}/bcsd_fcst/{self.fcst_model}_{self._resol}/raw/Climatology/', exist_ok=True)
                clim_syr = self.config['BCSD']['clim_start_year']
                clim_eyr = self.config['BCSD']['clim_end_year']
                self._mf_regrid(clim_syr, clim_eyr=clim_eyr)
                if self.config['BCSD']['source']['precip'] is not None:
                    os.makedirs(f'{self.e2esdir}/bcsd_fcst/NMME/raw/Climatology/', exist_ok=True)
                    self._pr_regrid()
                return
        else:
    ...
    def _mf_regrid(self, clim_syr, clim_eyr=None):
        ...
        for i, sub_val in enumerate(slurm_sub):
            ...
            # self._cwd, command_list=sub_val)
    ...
    def _pr_regrid(self):
        ...
        if self.hindcast:
            slurm_commands = bcsd.precip_regridding.main(self.e2esroot +'/' + self.config_file,
                                                         self.month, jobname, 1, str(2), self._cwd,
                                                         py_call=True)
            l_sub = 1
            slurm_sub = self.split_list(slurm_commands, l_sub)
            info = {}
        ...
        if self.hindcast:
            ...
            tfile = self.sublist_to_file(slurm_commands, self._cwd)
            try:
                s2s_api.python_job_file(self.e2esroot +'/' + self.config_file, jobname + 'run.j',
                                        jobname, 1, str(3), self._cwd, tfile.name)
    ...
    def _mf_biascorr(self):
        ...
        slurm_commands = bcsd.metforce_biascorrection.main(
            self.e2esroot +'/' + self.config_file, self.year, self._mmm,
            self.mm, jobname, 1, 3, self._cwd, py_call=True)
        ...
        for i, sub_val in enumerate(slurm_sub):
            ...
            try:
                s2s_api.python_job_file(self.e2esroot +'/' + self.config_file,
                                        jobname + f'{i+1:02d}_run.j', jobname + f'{i+1:02d}_',
                                        1, str(6), self._cwd, tfile.name, parallel_run=par_info)
            ...
            shutil.copy(jobname + f'{i+1:02d}_run.j', jobname + f'{i+1:02d}_run.sh')
            utils.remove_sbatch_lines(jobname + f'{i+1:02d}_run.sh')
            #utils.cylc_job_scripts(jobname + '{:02d}_run.sh'.format(i+1), 4, self._cwd,
            # command_list=sub_val)

    def _pr_biascorr(self):
        ...
        for nmme_model in self.models:
            var1 = bcsd.precip_biascorrection.main(self.e2esroot +'/' + self.config_file,
                                                   self.year, self._mmm, self.mm, jobname,
                                                   1, 3, self._cwd, nmme_model, py_call=True)
        ...
        for i, sub_val in enumerate(slurm_sub):
            ...
            try:
                s2s_api.python_job_file(self.e2esroot +'/' + self.config_file,
                                        ...
                                        str(6), self._cwd, tfile.name, parallel_run=par_info)
                self.create_dict(jobname + f'{i+1:02d}_run.j', 'bcsd_fcst', prev=prev)
            finally:
                tfile.close()
                os.unlink(tfile.name)
    ...
    def _mf_tempdis(self):
        ...
        slurm_commands = bcsd.metforce_temporal_disaggregation.main(self.e2esroot +'/' + \
                                                                    ...
                                                                    self.year, self._mmm, self.mm,
                                                                    jobname, 1, 3, self._cwd,
        ...
        for i, sub_val in enumerate(slurm_sub):
            ...
            shutil.copy(jobname + f'{i+1:02d}_run.j', jobname + f'{i+1:02d}_run.sh')
            utils.remove_sbatch_lines(jobname + f'{i+1:02d}_run.sh')
            #utils.cylc_job_scripts(jobname + '{:02d}_run.sh'.format(i+1), 5, self._cwd,
            # command_list=sub_val)

    def _pr_tempdis(self):
        ...
        for nmme_model in self.models:
            var1 = bcsd.precip_temporal_disaggregation.main(self.e2esroot +'/' + self.config_file,
                                                            self.year, self.year, self._mmm, self.mm,
                                                            jobname, 1, 3, self._cwd, self.e2esdir,
    ...
    def _combine_files(self):
        ...
        slurm_9_10 = bcsd.combine_forcings.main(self.e2esroot +'/' + self.config_file, self.year,
                                                self.year, self._mmm, self.mm, jobname,1, 4, self._cwd,
        ...
        if self._resol == '25km':
            tfile = self.sublist_to_file([slurm_9_10[0]], self._cwd)
            try:
                s2s_api.python_job_file(self.e2esroot +'/' + self.config_file, jobname + 'run.j',
                                        jobname, 1, str(6), self._cwd, tfile.name, parallel_run=par_info)
            ...
            for i, sub_val in enumerate(slurm_sub):
                ...
                shutil.copy(jobname + f'{i+1:02d}_run.j', jobname + f'{i+1:02d}_run.sh')
                utils.remove_sbatch_lines(jobname + f'{i+1:02d}_run.sh')
        #utils.cylc_job_scripts(jobname + 'run.sh', 6, self._cwd, command_list=slurm_9_10[0])
        os.chdir(self.e2esdir)
