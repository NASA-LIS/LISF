...
#-------------------------------------------------------------------------
#
# SCRIPT: servir_preprocess_task_02.py
#
# PURPOSE: Creates observational climatology data
#
# REVISION HISTORY:
...
def _driver():
    ...
    jobfile = f"{job_name}_run.j"
    jobname = f"{job_name}_"

    cmd_list = []
    for var_name in get_all_hydrosfs_names():
        cmd = 'python'
        cmd += f" {dir_src}/create_observational_climatology.py"
        cmd += f" {config_file}"
        cmd += f" {var_name}"
        cmd_list.append(cmd)
    
    # Write job script
    utils.job_script(config_file, jobfile, jobname, ntasks, hours, cwd, command_list = cmd_list, mem = 180)
