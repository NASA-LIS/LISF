...
from bcsd_helper_functions import (create_lat_lon_xr_dataset,
                                   create_regridder_weights_file,
...
def generate_geosv3_3hr_surface_filename(
    ...
    file_directory = (f"{dir_geos}/sfc_tavg_3hr_glo_L720x361_sfc/"
                      f"{fcst_init_year:04d}{fcst_init_month:02d}/"
                      f"ens{ensemble_number:02d}")
...
def create_geosv3_5km_regridder_weights_file(method : str, dir_out : str, dir_supplementary : str):
    ...
    # Example File to use
    file_path_geosv3 = f"{dir_supplementary}/sample/geosv3_samplefile.nc"
...
def create_geosv3_elevation_difference_file(dir_supplementary : str) -> xr.DataArray:
    ...
    # MERIT Elevation
    file_path_merit = (f"{dir_supplementary}/lis_darun/"
                       f"lis_input.global_5km_hydroscs_mask_topo_merit1km_wclimppt.nc")
