...
def read_nc_files(infile, varname):
    ...
    nc_ = nc4_dataset(infile, 'r')
    data = nc_.variables[varname][:]
    nc_.close()
    return data

def write_4d_netcdf(infile, var, varname, description, source, var_units, lons, lats, \
    ...
    varname = rootgrp.createVariable(varname,'f4',('Lead', 'Ens', \
                                                   'latitude','longitude'), \
...
def create_lat_lon_xr_dataset(lon_start, lon_end, lat_start, lat_end, cell_step):
    ''' creates a new xarray dataset '''
    ds_out = xr.Dataset(
        {
            "lat": (["lat"], np.arange(lat_start, lat_end + cell_step,
                                       cell_step)),
            "lon": (["lon"], np.arange(lon_start, lon_end + cell_step,
                                       cell_step))
        }
    )
    return ds_out
