...
if __name__ == "__main__":
    ...
    if DATATYPE == 'forecast':
        ...
        nmme_da = load_ncdata(INFILE.strip(), [logger, SUBTASK], var_name="prec", decode_times=False)
        ...
        if NMME_MODEL == 'CFSv2':
            if MON[MM] == 'Jan' or MON[MM] == 'Feb':
                ...
                INFILE = INFILE_TEMP.format(NMME_DOWNLOAD_DIR, nmme_path[0], nmme_path[1],
                                            MON[MM], SYR2, EYR2)
                logger.info(f"Reading: {INFILE}", subtask=SUBTASK)
                nmme_da2 = load_ncdata(INFILE, [logger, SUBTASK], var_name="prec", decode_times=False)
                ...
                INFILE = INFILE_TEMP.format(NMME_DOWNLOAD_DIR, nmme_path[0], nmme_path[1],
                                            MON[MM], SYR3, EYR3)
                logger.info(f"Reading: {INFILE}", subtask=SUBTASK)
                nmme_da3 = load_ncdata(INFILE, [logger, SUBTASK], var_name="prec", decode_times=False)
