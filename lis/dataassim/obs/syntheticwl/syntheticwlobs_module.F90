...
!BOP
!
! !MODULE: syntheticwlobs_module
! 
! !DESCRIPTION: 
!   This module contains interfaces and subroutines to
!   handle synthetic water level observations (such as the 
!   one generated from a previous LIS simulation)
!   
! !REVISION HISTORY: 
...
!-----------------------------------------------------------------------------
  PUBLIC :: syntheticwlobs_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  PUBLIC :: synthetic_wl_struc

  type, public :: synthetic_wl_dec
     integer                :: nbins
     integer                :: ntimes
     integer                :: ngrid

  end type synthetic_wl_dec
  type(synthetic_wl_dec), allocatable :: synthetic_wl_struc(:)
contains
!BOP
! 
! !ROUTINE: syntheticwlobs_setup
! \label{syntheticwlobs_setup}
! 
! !INTERFACE: 
  subroutine syntheticwlobs_setup(k, OBS_State, OBS_Pert_State)
! !USES: 
    use LIS_coreMod
    use LIS_logMod
...
!EOP
    ...
    allocate(synthetic_wl_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,"Synthetic water level data directory:",&
         rc=status)
    do n=1,LIS_rc%nnest
    ...
    write(LIS_logunit,*)'[INFO] read Synthetic water level data specifications'
...
!to obsarr
    ...
    do n=1,LIS_rc%nnest
       ...
       synthetic_wl_struc(n)%ngrid = LIS_rc%obs_ngrid(k)

       if(synthetic_wl_struc(n)%ngrid.gt.0) then 
          call ESMF_AttributeSet(pertField(n),"Standard Deviation",&
                  ssdev,itemCount=synthetic_wl_struc(n)%ngrid,rc=status)
  ...
  end subroutine syntheticwlobs_setup
  
end module syntheticwlobs_module
