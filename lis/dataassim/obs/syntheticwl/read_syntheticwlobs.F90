...
!BOP
! !ROUTINE: read_syntheticwlobs
! \label{read_syntheticwlobs}
!
! !REVISION HISTORY:
!  07 Nov 2019: Sujay Kumar; Initial Specification
!
! !INTERFACE: 
subroutine read_syntheticwlobs(n, k, OBS_State, OBS_Pert_state) 
! !USES: 
  use ESMF
  use LIS_mpiMod
  use LIS_historyMod
...
!
!EOP
  real,  parameter    :: MAX_WL_VALUE=0.45, MIN_WL_VALUE=0.0001
  ...
  call synwl_filename(name,wlobsdir,&
  ...
  if (readflag) then 
     ...
     call LIS_verify(nf90_inq_varid(ftn,'SurfElev_tavg',wlid),&
          'Error nf90_inq_varid: SurfElev_tavg')
     
     call LIS_verify(nf90_get_var(ftn,wlid,wlobs),&
...
end subroutine read_syntheticwlobs

!BOP
!
! !ROUTINE: synwl_filename
! \label{synwl_filename}
!
! !INTERFACE: 
subroutine synwl_filename(name, ndir, yr, mo,da,hr,mn)
