...
!BOP
!
! !MODULE: MCD64A1ba_Mod
! 
! !DESCRIPTION: 
!   This module contains interfaces and subroutines to
!   handle the MODIS MCD64A1 BA data.
! 
! !REVISION HISTORY: 
...
!-----------------------------------------------------------------------------
  public :: MCD64A1ba_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: MCD64A1ba_struc
!EOP
  type, public:: MCD64A1ba_dec
     ...
     integer                :: nr
     integer                :: mi
     real,     allocatable  :: baobs1(:)
     real,     allocatable  :: baobs2(:)
     real                   :: gridDesci(50)    
     real*8                 :: time1, time2
     integer                :: fnd
  ...
  end type MCD64A1ba_dec
  
  type(MCD64A1ba_dec),allocatable :: MCD64A1ba_struc(:)
  
contains

!BOP
! 
! !ROUTINE: MCD64A1ba_setup
! \label{MCD64A1ba_setup}
! 
! !INTERFACE: 
  subroutine MCD64A1ba_setup(k, OBS_State, OBS_Pert_State)
! !USES: 
    use LIS_coreMod
    use LIS_timeMgrMod
    use LIS_historyMod
...
!   creation of data structures required for handling MCD64A1 BA data.
!  
!   The arguments are: 
!   \begin{description}
...
!EOP
    ...
    allocate(MCD64A1ba_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,"MCD64A1 burned area data directory:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,baobsdir,&
            rc=status)
       call LIS_verify(status, 'MCD64A1 burned area data directory: is missing')

       call ESMF_AttributeSet(OBS_State(n),"Data Directory",&
            baobsdir, rc=status)
       call LIS_verify(status)

       MCD64A1ba_struc(n)%version = "006"       
    enddo
   ...
   do n=1,LIS_rc%nnest
    ...
    write(LIS_logunit,*)&
         '[INFO] read MCD64A1 burned area data specifications'       

    do n=1,LIS_rc%nnest

       allocate(MCD64A1ba_struc(n)%baobs1(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
       allocate(MCD64A1ba_struc(n)%baobs2(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
       ...
       call ESMF_FieldGet(obsField(n),localDE=0,farrayPtr=obsl,rc=status)
...
!Perturbations State
    ...
    do n=1,LIS_rc%nnest
       ...
       MCD64A1ba_struc(n)%nc = nint((cornerlon2-cornerlon1)/0.00416667)+1
       MCD64A1ba_struc(n)%nr = nint((cornerlat2-cornerlat1)/0.00416667)+1

       MCD64A1ba_struc(n)%gridDesci(1) = 0 
       MCD64A1ba_struc(n)%gridDesci(2) = MCD64A1ba_struc(n)%nc
       MCD64A1ba_struc(n)%gridDesci(3) = MCD64A1ba_struc(n)%nr 
       MCD64A1ba_struc(n)%gridDesci(4) = cornerlat1
       MCD64A1ba_struc(n)%gridDesci(5) = cornerlon1
       MCD64A1ba_struc(n)%gridDesci(6) = 128
       MCD64A1ba_struc(n)%gridDesci(7) = cornerlat2
       MCD64A1ba_struc(n)%gridDesci(8) = cornerlon2
       MCD64A1ba_struc(n)%gridDesci(9) = 0.00416667
       MCD64A1ba_struc(n)%gridDesci(10) = 0.00416667
       MCD64A1ba_struc(n)%gridDesci(20) = 64

       MCD64A1ba_struc(n)%mi = MCD64A1ba_struc(n)%nc*MCD64A1ba_struc(n)%nr
...
!-----------------------------------------------------------------------------
       if(LIS_rc%obs_gridDesc(k,10).le.0.00416667) then 
          ...
          allocate(MCD64A1ba_struc(n)%rlat(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%rlon(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%n11(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%n12(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%n21(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%n22(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%w11(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%w12(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%w21(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          allocate(MCD64A1ba_struc(n)%w22(LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
          
          call bilinear_interp_input_withgrid(MCD64A1ba_struc(n)%gridDesci(:), &
               LIS_rc%obs_gridDesc(k,:),&
               LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k),&
               MCD64A1ba_struc(n)%rlat, MCD64A1ba_struc(n)%rlon,&
               MCD64A1ba_struc(n)%n11, MCD64A1ba_struc(n)%n12, &
               MCD64A1ba_struc(n)%n21, MCD64A1ba_struc(n)%n22, &
               MCD64A1ba_struc(n)%w11, MCD64A1ba_struc(n)%w12, &
               MCD64A1ba_struc(n)%w21, MCD64A1ba_struc(n)%w22)
       else
          
          allocate(MCD64A1ba_struc(n)%n11(&
               MCD64A1ba_struc(n)%nc*MCD64A1ba_struc(n)%nr))

          call upscaleByAveraging_input(MCD64A1ba_struc(n)%gridDesci(:),&
               LIS_rc%obs_gridDesc(k,:),&
               MCD64A1ba_struc(n)%nc*MCD64A1ba_struc(n)%nr, &
               LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k), MCD64A1ba_struc(n)%n11)
       endif
       
       call LIS_registerAlarm("MCD64A1 burned area read alarm",&
            86400.0, 86400.0)

       MCD64A1ba_struc(n)%startMode = .true. 

       call ESMF_StateAdd(OBS_State(n),(/obsField(n)/),rc=status)
       call LIS_verify(status)
     
    enddo
  end subroutine MCD64A1ba_setup
end module MCD64A1ba_Mod
