...
!-----------------------------------------------------------------------------
  public :: SMOSNRTNNL2sm_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: SMOSNRTNNL2sm_struc
!EOP
...
!BOP
! 
! !ROUTINE: SMOSNRTNNL2sm_setup
! \label{SMOSNRTNNL2sm_setup}
! 
! !INTERFACE: 
  subroutine SMOSNRTNNL2sm_setup(k, OBS_State, OBS_Pert_State)
! !USES: 
    use LIS_coreMod
    use LIS_timeMgrMod
    use LIS_historyMod
...
!   creation of data strctures required for handling SMOS NRT L2 
!   soil moisture data. 
!  
!   The arguments are: 
!   \begin{description}
...
!EOP
    ...
    allocate(SMOSNRTNNL2sm_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,"SMOS NRT NN soil moisture data directory:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,rtsmopssmobsdir,&
            rc=status)
       call LIS_verify(status, 'SMOS NRT NN soil moisture data directory: is missing')
    ...
    call ESMF_ConfigFindLabel(LIS_config,"SMOS NRT NN soil moisture use scaled standard deviation model:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOSNRTNNL2sm_struc(n)%useSsdevScal,&
            rc=status)
       call LIS_verify(status, 'SMOS NRT NN soil moisture use scaled standard deviation model: is missing')
    enddo

    call ESMF_ConfigFindLabel(LIS_config,"SMOS NRT NN model CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOSNRTNNL2sm_struc(n)%modelcdffile,rc=status)
       call LIS_verify(status, 'SMOS NRT NN model CDF file: not defined')
    enddo

    call ESMF_ConfigFindLabel(LIS_config,"SMOS NRT NN observation CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOSNRTNNL2sm_struc(n)%obscdffile,rc=status)
       call LIS_verify(status, 'SMOS NRT NN observation CDF file: not defined')
    enddo

    call ESMF_ConfigFindLabel(LIS_config, "SMOS NRT NN soil moisture number of bins in the CDF:", rc=status)
    do n=1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOSNRTNNL2sm_struc(n)%nbins, rc=status)
       call LIS_verify(status, "SMOS NRT NN soil moisture number of bins in the CDF: not defined")
    enddo

    do n=1, LIS_rc%nnest
      SMOSNRTNNL2sm_struc(n)%cdf_read_mon = .false.

      call ESMF_ConfigFindLabel(LIS_config, "SMOS NRT NN CDF read option:", rc=status)    ! 0: read CDF for all months/year 
                                                                                         ! 1: read CDF for current month
      call ESMF_ConfigGetAttribute(LIS_config,SMOSNRTNNL2sm_struc(n)%cdf_read_opt, rc=status)
      call LIS_verify(status, "SMOS NRT NN CDF read option: not defined")
   enddo
   ...
   do n=1,LIS_rc%nnest
    ...
    write(LIS_logunit,*)&
         '[INFO] read SMOS NRT NN soil moisture data specifications'

!----------------------------------------------------------------------------
!   Create the array containers that will contain the observations and
!   the perturbations. SMOS NRT NN L2
...
! Set obs err to be uniform (will be rescaled later for each grid point). 
          ...
          SMOSNRTNNL2sm_struc(n)%ssdev_inp = obs_pert%ssdev(1)
...
! initializing the perturbations to be zero 
    ...
    enddo
    write(LIS_logunit,*) &
         '[INFO] Created the States to hold the SMOS NRT NN observations data'
    do n=1,LIS_rc%nnest
       SMOSNRTNNL2sm_struc(n)%nc = LIS_rc%gnc(n)
       SMOSNRTNNL2sm_struc(n)%nr = LIS_rc%gnr(n)

       allocate(SMOSNRTNNL2sm_struc(n)%smobs(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       allocate(SMOSNRTNNL2sm_struc(n)%smtime(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       allocate(SMOSNRTNNL2sm_struc(n)%SMOS_lookup(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
    ...
    do n=1,LIS_rc%nnest
       ...
       if(LIS_rc%dascaloption(k).eq."CDF matching") then
          ...
          call LIS_getCDFattributes(k,SMOSNRTNNL2sm_struc(n)%modelcdffile,&
               SMOSNRTNNL2sm_struc(n)%ntimes,ngrid)

          if (SMOSNRTNNL2sm_struc(n)%cdf_read_opt.eq.0) then
             allocate(SMOSNRTNNL2sm_struc(n)%model_xrange(&
                  LIS_rc%obs_ngrid(k), SMOSNRTNNL2sm_struc(n)%ntimes, &
                  SMOSNRTNNL2sm_struc(n)%nbins))
             allocate(SMOSNRTNNL2sm_struc(n)%obs_xrange(&
                  LIS_rc%obs_ngrid(k), SMOSNRTNNL2sm_struc(n)%ntimes, &
                  SMOSNRTNNL2sm_struc(n)%nbins))
             allocate(SMOSNRTNNL2sm_struc(n)%model_cdf(&
                  LIS_rc%obs_ngrid(k), SMOSNRTNNL2sm_struc(n)%ntimes, &
                  SMOSNRTNNL2sm_struc(n)%nbins))
             allocate(SMOSNRTNNL2sm_struc(n)%obs_cdf(&
                  LIS_rc%obs_ngrid(k), SMOSNRTNNL2sm_struc(n)%ntimes, &
                  SMOSNRTNNL2sm_struc(n)%nbins))
             allocate(SMOSNRTNNL2sm_struc(n)%model_mu(LIS_rc%obs_ngrid(k),&
                  SMOSNRTNNL2sm_struc(n)%ntimes))
             allocate(SMOSNRTNNL2sm_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),&
                  SMOSNRTNNL2sm_struc(n)%ntimes))
             allocate(SMOSNRTNNL2sm_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),&
                  SMOSNRTNNL2sm_struc(n)%ntimes))
             allocate(SMOSNRTNNL2sm_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),&
                  SMOSNRTNNL2sm_struc(n)%ntimes))
          else
             allocate(SMOSNRTNNL2sm_struc(n)%model_xrange(&
             ...
             allocate(SMOSNRTNNL2sm_struc(n)%obs_cdf(&
                  LIS_rc%obs_ngrid(k), 1, &
                  SMOSNRTNNL2sm_struc(n)%nbins))
             allocate(SMOSNRTNNL2sm_struc(n)%model_mu(LIS_rc%obs_ngrid(k),1))
             allocate(SMOSNRTNNL2sm_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),1))
             allocate(SMOSNRTNNL2sm_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),1))
             allocate(SMOSNRTNNL2sm_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),1))
          endif
!----------------------------------------------------------------------------
...
!----------------------------------------------------------------------------
          ...
          if(SMOSNRTNNL2sm_struc(n)%useSsdevScal.eq.1) then
             ...
             do t=1,LIS_rc%obs_ngrid(k)
                if(SMOSNRTNNL2sm_struc(n)%obs_sigma(t,jj).gt.0) then
                   ...
                   print*, ssdev(t), SMOSNRTNNL2sm_struc(n)%model_sigma(t,jj),&
    ...
    do n=1,LIS_rc%nnest
       ...
       SMOSNRTNNL2sm_struc(n)%startMode = .true.
  ...
  end subroutine SMOSNRTNNL2sm_setup
end module SMOSNRTNNL2sm_Mod
