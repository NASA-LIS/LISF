...
!
! REVISION HISTORY:
! 17 Jul 2025  Eric Kemp  Initial specification, based on USAFSIobs_Mod.
!
! DESCRIPTION:
...
module SNIPobs_Mod
  ...
  ! Public routines
  public :: SNIPobs_finalize
  public :: SNIPobs_setup

  ! Public variables
  public :: SNIP_obs, SNIP_obs_t
  ...
  type(SNIP_obs_t), allocatable :: SNIP_obs(:)

contains

  ! Complete runtime initializations for SNIP product
  subroutine SNIPobs_setup(k, OBS_State, OBS_Pert_State)
    ...
    character(LIS_CONST_PATH_LEN)         ::  SNIPobsdir
    character(LIS_CONST_PATH_LEN)         ::  SNIP_infile_name
    integer                ::  n
    ...
    allocate(SNIP_obs(LIS_rc%nnest))
    ...
    call ESMF_ArraySpecSet(pertArrSpec, rank=2, &
         typekind=ESMF_TYPEKIND_R4, &
         rc=status)
    call LIS_verify(status)

    call ESMF_ConfigFindLabel(LIS_config, "SNIP data directory:",&
         rc=status)
    do n = 1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config, SNIPobsdir,&
            rc=status)
       if (status .ne. ESMF_SUCCESS) then
          write(LIS_logunit,*) "[ERR] SNIP data directory is missing"
       end if
       call LIS_verify(status)
       call ESMF_AttributeSet(OBS_State(n), "Data Directory",&
            SNIPobsdir, rc=status)
       call LIS_verify(status)
    end do ! n

    call ESMF_ConfigFindLabel(LIS_config, &
         "SNIP netcdf filename prefix:", &
         rc=status)
    do n = 1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config, SNIP_infile_name,&
            rc=status)
       if (status .ne. ESMF_SUCCESS)then
          write(LIS_logunit,*) &
               "[ERR] SNIP netcdf filename prefix is missing"
          call LIS_endrun()
       end if
       call LIS_verify(status)
       call ESMF_AttributeSet(OBS_State(n), "Netcdf filename prefix",&
            SNIP_infile_name, rc=status)
       call LIS_verify(status)
    end do ! n
    ...
    write(LIS_logunit,*)'[INFO] Read SNIP data specifications'
    ...
    do n = 1, LIS_rc%nnest
       ...
       ! Perturbations state
       write(LIS_logunit,*) &
            '[INFO] Opening attributes for observations ', &
       ...
       if (trim(LIS_rc%perturb_obs(k)) .ne. "none") then
          ...
          call ESMF_StateAdd(OBS_Pert_State(n), (/pertField(n)/), &
               rc=status)
          call LIS_verify(status)
       ...
       ! The SNIP grid information will be read in from the LDT
       ! netCDF file.  For now, put in dummy data.
       SNIP_obs(n)%gridDesco(:) = 0
       SNIP_obs%mo1 = 0
       SNIP_obs%nc_ldt = 0
       SNIP_obs%nr_ldt = 0
       SNIP_obs%nc_lis = 0
       ...
       call LIS_registerAlarm("SNIP read alarm", 21600.0, 21600.0)
  ...
  end subroutine SNIPobs_setup

  ! Dummy subroutine to finalize work with SNIP data
  subroutine SNIPobs_finalize()
    implicit none
  end subroutine SNIPobs_finalize

end module SNIPobs_Mod
