...
subroutine read_SNIPobs(n, k, OBS_State, OBS_Pert_State)
  ...
  alarmCheck = LIS_isAlarmRinging(LIS_rc, "SNIP read alarm")
  ...
  if (alarmCheck) then
     ...
     call SNIP_filename(filename, obsdir, prefix, yyyy, mm, dd, hh)
  ...
  call SNIP_reader(filename, n, k, snoanl, ierr)
  if (ierr .ne. 0) then
     write(LIS_logunit,*) &
          "[WARN] Could not read from SNIP file ", trim(filename)
     call ESMF_AttributeSet(OBS_State, "Data Update Status",&
          .false., rc=status)
     call LIS_verify(status)
     return
  ...
  call interp_SNIPfield(n, k, snoanl, LIS_rc%udef, varfield)
  ...
  deallocate(snoanl)
  deallocate(varfield)
  deallocate(SNIP_obs(n)%rlat1)
  deallocate(SNIP_obs(n)%rlon1)
  deallocate(SNIP_obs(n)%n111)

contains

  ! Constructs SNIP filename
  subroutine SNIP_filename(filename, dir, prefix, yyyy, mm, dd, hh)

    ! Defaults
    implicit none

    ! Arguments
    character(LIS_CONST_PATH_LEN), intent(inout) :: filename
    character(LIS_CONST_PATH_LEN), intent(in) :: dir
    ...
    filename = trim(dir) // "/" &
         ...
         // ".nc"
  end subroutine SNIP_filename

#if(defined USE_NETCDF3 || defined USE_NETCDF4)

  ! Reads SNIP netcdf file
  subroutine SNIP_reader(filename, n, k, snoanl, ierr)
    ...
    ! Arguments
    character(LIS_CONST_PATH_LEN), intent(in) :: filename
    integer, intent(in) :: n
    integer, intent(in) :: k
    ...
    write(LIS_logunit,*) &
         "[INFO] Reading SNIP file ", trim(filename)
    ...
    if (trim(map_projection_name) .ne. "EQUIDISTANT CYLINDRICAL") then
       write(LIS_logunit,*) &
            "[ERR] Unsupported map projection for SNIP product!"
       write(LIS_logunit,*) &
    ...
    ! Fetch the SNIP data resolution
    call LIS_verify(nf90_get_att(ncid=ncid, &
         varid=NF90_GLOBAL, &
         name="DX", &
         values=dx), &
    ...
    SNIP_obs(n)%nc_ldt = nlon
    SNIP_obs(n)%nr_ldt = nlat
    SNIP_obs(n)%mi = nlon*nlat ! On LDT grid
    ...
    SNIP_obs(n)%mo1 = SNIP_obs(n)%nc_lis * SNIP_obs(n)%nr_lis

    SNIP_obs(n)%gridDesco(1) = 0
    SNIP_obs(n)%gridDesco(2) = SNIP_obs(n)%nc_lis
    SNIP_obs(n)%gridDesco(3) = SNIP_obs(n)%nr_lis
    SNIP_obs(n)%gridDesco(4) = LIS_rc%obs_gridDesc(k,4)
    SNIP_obs(n)%gridDesco(5) = LIS_rc%obs_gridDesc(k,5)
    SNIP_obs(n)%gridDesco(6) = LIS_rc%obs_gridDesc(k,6)
    SNIP_obs(n)%gridDesco(7) = LIS_rc%obs_gridDesc(k,7)
    SNIP_obs(n)%gridDesco(8) = LIS_rc%obs_gridDesc(k,8)
    SNIP_obs(n)%gridDesco(9) = LIS_rc%obs_gridDesc(k,9)
    SNIP_obs(n)%gridDesco(10) = LIS_rc%obs_gridDesc(k,10)
    SNIP_obs(n)%gridDesco(20) = 255

    allocate(SNIP_obs(n)%rlat1(SNIP_obs(n)%mo1))
    allocate(SNIP_obs(n)%rlon1(SNIP_obs(n)%mo1))
    allocate(SNIP_obs(n)%n111(SNIP_obs(n)%mo1))

    call neighbor_interp_input_withgrid(tmp_gridDesci, &
         SNIP_obs(n)%gridDesco, &
         SNIP_obs(n)%mo1, SNIP_obs(n)%rlat1, &
         SNIP_obs(n)%rlon1, SNIP_obs(n)%n111)

    ! Normal exit
    ierr = 0
  end subroutine SNIP_reader
  ...
  ! Interpolates the SNIP data to the LIS grid
  subroutine interp_SNIPfield(n, k, snoanl, udef, varfield)
    ...
    integer, intent(in) :: n
    integer, intent(in) :: k
    real, intent(in) :: snoanl(SNIP_obs(n)%nc_ldt, SNIP_obs(n)%nr_ldt)
    real, intent(in) :: udef
    ...
    allocate(gi(SNIP_obs(n)%nc_ldt*SNIP_obs(n)%nr_ldt))
    gi = 0
    allocate(li(SNIP_obs(n)%nc_ldt*SNIP_obs(n)%nr_ldt))
    li = .false.

    do r = 1, SNIP_obs(n)%nr_ldt
       do c = 1, SNIP_obs(n)%nc_ldt
          if (snoanl(c,r) < 0) cycle
          li(c + (r-1)*SNIP_obs(n)%nc_ldt) = .true.
          gi(c + (r-1)*SNIP_obs(n)%nc_ldt) = snoanl(c,r)
       end do ! c
    end do ! r
    ...
    call neighbor_interp(SNIP_obs(n)%gridDesco, &
         li, gi, lo, go, &
         SNIP_obs(n)%mi, SNIP_obs(n)%mo1, &
         SNIP_obs(n)%rlat1, &
         SNIP_obs(n)%rlon1, &
         SNIP_obs(n)%n111, &
         udef, rc)
    ...
    deallocate(go)
    deallocate(gi)
  end subroutine interp_SNIPfield

end subroutine read_SNIPobs
