...
!BOP
! 
! !ROUTINE: write_syntheticsmobs
! \label{write_syntheticsmobs}
! 
! !REVISION HISTORY: 
! 25Jan2008: Sujay Kumar; Initial Specification
! 
! !INTERFACE: 
subroutine write_syntheticsmobs(n, k, OBS_State)
! !USES: 
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_AttributeGet: Data Update Status failed in write_syntheticsmobs")
  ...
  if(data_update) then 
     ...
     call ESMF_StateGet(OBS_State, "Observation01",smField, &
          rc=status)
     call LIS_verify(status, &
          "ESMF_StateGet failed in write_syntheticsmobs")
     
     call ESMF_FieldGet(smField, localDE=0, farrayPtr=smobs, rc=status)
     call LIS_verify(status,&
          "ESMF_FieldGet failed in write_syntheticsmobs")
     ...
     if(LIS_masterproc) then 
        ...
        call synthetic_smobsname(n,k,obsname)        
...
end subroutine write_syntheticsmobs

!BOP
! !ROUTINE: synthetic_smobsname
! \label{synthetic_smobsname}
! 
! !INTERFACE: 
subroutine synthetic_smobsname(n,k,obsname)
! !USES: 
...
end subroutine synthetic_smobsname
