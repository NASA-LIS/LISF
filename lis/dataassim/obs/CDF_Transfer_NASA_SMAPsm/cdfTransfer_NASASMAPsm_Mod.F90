...
#IF 0
    call ESMF_ConfigFindLabel(LIS_config,"SMAP(NASA) model CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,cdfT_SMAPsm_struc(n)%modelcdffile,rc=status)
...
!-------------------------------------------------------------------------------
    do n=1,LIS_rc%nnest
       !allocate(cdfT_SMAPsm_struc(n)%ref_p_climo(LIS_rc%lnc(n), LIS_rc%lnr(n), cdfT_SMAPsm_struc(n)%ntimes))
       ...
       !call read_Precip_climo (n,k,LIS_rc%lnc(n), LIS_rc%lnr(n),&
       !                        cdfT_SMAPsm_struc(n)%target_p_climo_file,&
