...
!-----------------------------------------------------------------------------
  public :: SMOSNESDISsm_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: SMOSNESDISsm_struc
!EOP
  type, public:: SMOSNESDISsm_dec
  ...
  end type SMOSNESDISsm_dec
  
  type(SMOSNESDISsm_dec),allocatable :: SMOSNESDISsm_struc(:)
  
contains

!BOP
! 
! !ROUTINE: SMOSNESDISsm_setup
! \label{SMOSNESDISsm_setup}
! 
! !INTERFACE: 
  subroutine SMOSNESDISsm_setup(k, OBS_State, OBS_Pert_State)
! !USES: 
    use LIS_coreMod
    use LIS_timeMgrMod
    use LIS_historyMod
...
!   creation of data strctures required for handling SMOSNESDIS 
!   soil moisture data. 
!  
!   The arguments are: 
!   \begin{description}
...
!EOP
    ...
    allocate(SMOSNESDISsm_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,"SMOS(NESDIS) soil moisture data directory:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,rtsmopssmobsdir,&
            rc=status)
       call LIS_verify(status, 'SMOS(NESDIS) soil moisture data directory: is missing')
    ...
    call ESMF_ConfigFindLabel(LIS_config,"SMOS(NESDIS) soil moisture use scaled standard deviation model:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOSNESDISsm_struc(n)%useSsdevScal,&
            rc=status)
       call LIS_verify(status, 'SMOS(NESDIS) soil moisture use scaled standard deviation model: is missing')
       
    enddo
    call ESMF_ConfigFindLabel(LIS_config,"SMOS(NESDIS) model CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,modelcdffile(n),rc=status)
       call LIS_verify(status, 'SMOS(NESDIS) model CDF file: not defined')
    enddo

    call ESMF_ConfigFindLabel(LIS_config,"SMOS(NESDIS) observation CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,obscdffile(n),rc=status)
       call LIS_verify(status, 'SMOS(NESDIS) observation CDF file: not defined')
    enddo
    
    call ESMF_ConfigFindLabel(LIS_config, "SMOS(NESDIS) soil moisture number of bins in the CDF:", rc=status)
    do n=1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOSNESDISsm_struc(n)%nbins, rc=status)
       call LIS_verify(status, "SMOS(NESDIS) soil moisture number of bins in the CDF: not defined")
    enddo
   ...
   do n=1,LIS_rc%nnest
    ...
    write(LIS_logunit,*)'[INFO] read SMOS(NESDIS) soil moisture data specifications'       

!----------------------------------------------------------------------------
!   Create the array containers that will contain the observations and
!   the perturbations. SMOSNESDIS
...
! Set obs err to be uniform (will be rescaled later for each grid point). 
          ...
          SMOSNESDISsm_struc(n)%ssdev_inp = obs_pert%ssdev(1)
...
! initializing the perturbations to be zero 
    ...
    enddo
    write(LIS_logunit,*) &
         '[INFO] Created the States to hold the SMOS NESDIS observations data'
    do n=1,LIS_rc%nnest
       SMOSNESDISsm_struc(n)%nc = 1440
       SMOSNESDISsm_struc(n)%nr = 720
       allocate(SMOSNESDISsm_struc(n)%smobs(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       allocate(SMOSNESDISsm_struc(n)%smtime(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
    ...
    do n=1,LIS_rc%nnest
       ...
       call LIS_getCDFattributes(k,modelcdffile(n),&
            SMOSNESDISsm_struc(n)%ntimes,ngrid)
       
       allocate(SMOSNESDISsm_struc(n)%model_xrange(&
            LIS_rc%obs_ngrid(k), SMOSNESDISsm_struc(n)%ntimes, &
            SMOSNESDISsm_struc(n)%nbins))
       allocate(SMOSNESDISsm_struc(n)%obs_xrange(&
            LIS_rc%obs_ngrid(k), SMOSNESDISsm_struc(n)%ntimes, &
            SMOSNESDISsm_struc(n)%nbins))
       allocate(SMOSNESDISsm_struc(n)%model_cdf(&
            LIS_rc%obs_ngrid(k), SMOSNESDISsm_struc(n)%ntimes, &
            SMOSNESDISsm_struc(n)%nbins))
       allocate(SMOSNESDISsm_struc(n)%obs_cdf(&
            LIS_rc%obs_ngrid(k), SMOSNESDISsm_struc(n)%ntimes, &
            SMOSNESDISsm_struc(n)%nbins))
       allocate(SMOSNESDISsm_struc(n)%model_mu(LIS_rc%obs_ngrid(k),&
            SMOSNESDISsm_struc(n)%ntimes))
       allocate(SMOSNESDISsm_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),&
            SMOSNESDISsm_struc(n)%ntimes))
       allocate(SMOSNESDISsm_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),&
            SMOSNESDISsm_struc(n)%ntimes))
       allocate(SMOSNESDISsm_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),&
            SMOSNESDISsm_struc(n)%ntimes))
...
!----------------------------------------------------------------------------
       call LIS_readMeanSigmaData(n,k,&
            ...
            modelcdffile(n), &
            "SoilMoist",&
            SMOSNESDISsm_struc(n)%model_mu,&
            SMOSNESDISsm_struc(n)%model_sigma)
       ...
       call LIS_readMeanSigmaData(n,k,&
            ...
            obscdffile(n), &
            "SoilMoist",&
            SMOSNESDISsm_struc(n)%obs_mu,&
            SMOSNESDISsm_struc(n)%obs_sigma)
       ...
       if(SMOSNESDISsm_struc(n)%useSsdevScal.eq.1) then 
          ...
          do r=1,LIS_rc%obs_lnr(k)
             ...
             enddo
          enddo
          print*, LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)
...
!          stop
    ...
    do n=1,LIS_rc%nnest
       ...
       allocate(SMOSNESDISsm_struc(n)%n11(SMOSNESDISsm_struc(n)%nc*SMOSNESDISsm_struc(n)%nr))
       allocate(SMOSNESDISsm_struc(n)%rlat(&
            SMOSNESDISsm_struc(n)%nc*SMOSNESDISsm_struc(n)%nr))
       allocate(SMOSNESDISsm_struc(n)%rlon(&
            SMOSNESDISsm_struc(n)%nc*SMOSNESDISsm_struc(n)%nr))
       ...
       call neighbor_interp_input_withgrid(gridDesci,&
            ...
            SMOSNESDISsm_struc(n)%rlat, &
            SMOSNESDISsm_struc(n)%rlon, &
            SMOSNESDISsm_struc(n)%n11)
       
       call LIS_registerAlarm("SMOSNESDIS read alarm",&
            86400.0, 86400.0)

       SMOSNESDISsm_struc(n)%startMode = .true. 
    ...
    enddo
  end subroutine SMOSNESDISsm_setup
end module SMOSNESDISsm_Mod
