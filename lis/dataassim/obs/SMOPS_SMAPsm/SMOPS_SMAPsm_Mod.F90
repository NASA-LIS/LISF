...
!BOP
!
! !MODULE: SMOPS_SMAPsm_Mod
! 
! !DESCRIPTION: 
!   This module contains interfaces and subroutines to
!   handle NOAA/NESDIS-based SMAP Soil Moisture (SM) Files (aka, SMOPS)
!
...
!  28 Sep 2017: Mahdi Navari; Updated to read SMAP from SMOPS V3
! 
module SMOPS_SMAPsm_Mod
! !USES: 
  use ESMF
...
!-----------------------------------------------------------------------------
  public :: SMOPS_SMAPsm_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: SMOPS_SMAPsm_struc
!EOP
  type, public:: SMOPS_SMAPsm_dec
  ...
  end type SMOPS_SMAPsm_dec
  
  type(SMOPS_SMAPsm_dec),allocatable :: SMOPS_SMAPsm_struc(:)
  
contains

!BOP
! 
! !ROUTINE: SMOPS_SMAPsm_setup
! \label{SMOPS_SMAPsm_setup}
! 
! !INTERFACE: 
  subroutine SMOPS_SMAPsm_setup(k, OBS_State, OBS_Pert_State)
! !USES: 
    use LIS_coreMod
    use LIS_timeMgrMod
    use LIS_historyMod
...
!EOP
    ...
    allocate(SMOPS_SMAPsm_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,"SMOPS use realtime data:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOPS_SMAPsm_struc(n)%useRealtime,&
            rc=status)
       call LIS_verify(status, 'SMOPS use realtime data: is missing')

    enddo

    call ESMF_ConfigFindLabel(LIS_config,"SMOPS soil moisture use SMAP data:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOPS_SMAPsm_struc(n)%useSMAP,&
            rc=status)
       call LIS_verify(status, 'SMOPS soil moisture use SMAP data: is missing')
    ...
    call ESMF_ConfigFindLabel(LIS_config,"SMOPS soil moisture use scaled standard deviation model:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOPS_SMAPsm_struc(n)%useSsdevScal,&
            rc=status)
    ...
    do n=1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOPS_SMAPsm_struc(n)%nbins, rc=status)
    ...
    do n=1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,SMOPS_SMAPsm_struc(n)%conv, rc=status)
...
! Set obs err to be uniform (will be rescaled later for each grid point). 
          ...
          SMOPS_SMAPsm_struc(n)%ssdev_inp = obs_pert%ssdev(1)
...
! initializing the perturbations to be zero 
    ...
    do n=1,LIS_rc%nnest
       SMOPS_SMAPsm_struc(n)%nc = 1440
       SMOPS_SMAPsm_struc(n)%nr = 720
       allocate(SMOPS_SMAPsm_struc(n)%smobs(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       allocate(SMOPS_SMAPsm_struc(n)%smtime(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
    ...
    do n=1,LIS_rc%nnest
       ...
       call LIS_getCDFattributes(k,modelcdffile(n),&
            SMOPS_SMAPsm_struc(n)%ntimes,ngrid)
       
       allocate(SMOPS_SMAPsm_struc(n)%model_xrange(&
            LIS_rc%obs_ngrid(k), SMOPS_SMAPsm_struc(n)%ntimes, &
            SMOPS_SMAPsm_struc(n)%nbins))
       allocate(SMOPS_SMAPsm_struc(n)%obs_xrange(&
            LIS_rc%obs_ngrid(k), SMOPS_SMAPsm_struc(n)%ntimes, &
            SMOPS_SMAPsm_struc(n)%nbins))
       allocate(SMOPS_SMAPsm_struc(n)%model_cdf(&
            LIS_rc%obs_ngrid(k), SMOPS_SMAPsm_struc(n)%ntimes, &
            SMOPS_SMAPsm_struc(n)%nbins))
       allocate(SMOPS_SMAPsm_struc(n)%obs_cdf(&
            LIS_rc%obs_ngrid(k), SMOPS_SMAPsm_struc(n)%ntimes, &
            SMOPS_SMAPsm_struc(n)%nbins))
       allocate(SMOPS_SMAPsm_struc(n)%model_mu(LIS_rc%obs_ngrid(k),&
            SMOPS_SMAPsm_struc(n)%ntimes))
       allocate(SMOPS_SMAPsm_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),&
            SMOPS_SMAPsm_struc(n)%ntimes))
       allocate(SMOPS_SMAPsm_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),&
            SMOPS_SMAPsm_struc(n)%ntimes))
       allocate(SMOPS_SMAPsm_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),&
            SMOPS_SMAPsm_struc(n)%ntimes))
...
!----------------------------------------------------------------------------
       call LIS_readMeanSigmaData(n,k,&
            ...
            modelcdffile(n), &
            "SoilMoist",&
            SMOPS_SMAPsm_struc(n)%model_mu,&
            SMOPS_SMAPsm_struc(n)%model_sigma)
       ...
       call LIS_readMeanSigmaData(n,k,&
            ...
            obscdffile(n), &
            "SoilMoist",&
            SMOPS_SMAPsm_struc(n)%obs_mu,&
            SMOPS_SMAPsm_struc(n)%obs_sigma)
...
!          stop
    ...
    do n=1,LIS_rc%nnest
       ...
       call LIS_date2time(SMOPS_SMAPsm_struc(n)%version2_time,updoy,upgmt, &
                          yr1,mo1,da1,hr1,mn1,ss1)

       yr1 = 2017; mo1 = 8; da1 = 24; hr1 = 12; mn1 = 0; ss1 = 0
       call LIS_date2time(SMOPS_SMAPsm_struc(n)%version3_time,updoy,upgmt, &
                          yr1,mo1,da1,hr1,mn1,ss1)
       ...
       allocate(SMOPS_SMAPsm_struc(n)%n11(SMOPS_SMAPsm_struc(n)%nc*SMOPS_SMAPsm_struc(n)%nr))
       allocate(SMOPS_SMAPsm_struc(n)%rlat(&
            SMOPS_SMAPsm_struc(n)%nc*SMOPS_SMAPsm_struc(n)%nr))
       allocate(SMOPS_SMAPsm_struc(n)%rlon(&
            SMOPS_SMAPsm_struc(n)%nc*SMOPS_SMAPsm_struc(n)%nr))
       
       call neighbor_interp_input_withgrid(gridDesci,&
            LIS_rc%obs_gridDesc(k,:),&
            SMOPS_SMAPsm_struc(n)%nc*&
            SMOPS_SMAPsm_struc(n)%nr,&
            SMOPS_SMAPsm_struc(n)%rlat, &
            SMOPS_SMAPsm_struc(n)%rlon, &
            SMOPS_SMAPsm_struc(n)%n11)
       ...
       SMOPS_SMAPsm_struc(n)%startMode = .true. 
    ...
    enddo
  end subroutine SMOPS_SMAPsm_setup
end module SMOPS_SMAPsm_Mod
