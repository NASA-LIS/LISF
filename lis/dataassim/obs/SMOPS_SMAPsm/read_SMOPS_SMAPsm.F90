...
smDataNotAvailable = .false. 
  ...
  if ( timenow < SMOPS_SMAPsm_struc(n)%version2_time ) then
!     param_SMAP = 213; param_SMAP_qa = 234
!     param_SMAP_hr = 223; param_SMAP_mn = 224
     write(LIS_logunit,*) '[Warning] SMAP is not availabe in SMOPS version: 1.3'
     smDataNotAvailable = .true.   
     smobs_ip = LIS_rc%udef
     smtime_ip = LIS_rc%udef   	
  elseif ( timenow >= SMOPS_SMAPsm_struc(n)%version2_time .and. &
           timenow <  SMOPS_SMAPsm_struc(n)%version3_time ) then
!     param_SMAP = 213; param_SMAP_qa = 234
!     param_SMAP_hr = 223; param_SMAP_mn = 224
    write(LIS_logunit,*) '[Warning] SMAP is not availabe in SMOPS version: 2.0'
     smDataNotAvailable = .true. 
     smobs_ip = LIS_rc%udef
     smtime_ip = LIS_rc%udef
  elseif ( timenow >= SMOPS_SMAPsm_struc(n)%version3_time ) then
	if ( SMOPS_SMAPsm_struc(n)%useRealtime == 1 ) then
     ...
     write(LIS_logunit,*) '      ', SMOPS_SMAPsm_struc(n)%version2_time
     write(LIS_logunit,*) '      ', SMOPS_SMAPsm_struc(n)%version3_time
     call LIS_endrun()
  endif
