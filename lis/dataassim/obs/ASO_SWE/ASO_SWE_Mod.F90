...
!-----------------------------------------------------------------------------
  public :: ASO_SWE_setup
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: ASO_SWE_struc
!EOP
  type, public:: ASO_SWE_dec


  end type ASO_SWE_dec
  
  type(ASO_SWE_dec),allocatable :: ASO_SWE_struc(:)
  
contains

!BOP
! 
! !ROUTINE: ASO_SWE_setup
! \label{ASO_SWE_setup}
! 
! !INTERFACE: 
...
!EOP
    ...
    allocate(ASO_SWE_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,"ASO SWE data directory:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,ASOsweobsdir,&
            rc=status)
       call LIS_verify(status, 'ASO SWE data directory: is missing')

       call ESMF_AttributeSet(OBS_State(n),"Data Directory",&
            ASOsweobsdir, rc=status)
       call LIS_verify(status)
    enddo
   ...
   do n=1,LIS_rc%nnest
    ...
    write(LIS_logunit,*)'[INFO] read ASO SWE data specifications'       
...
!Perturbations State
    ...
    write(LIS_logunit,*) '[INFO] Created the States to hold the ASO observations data'
    
!    call LIS_registerAlarm("ASO SWE read alarm", 86400.0, 86400.0)

  end subroutine ASO_SWE_setup


end module ASO_SWE_Mod
