...
!   handle the processing of THySM
!   retrievals
! 
! !REVISION HISTORY: 
...
!EOP
  ...
  type(THySM_dec),allocatable :: THySM_struc(:)
...
!EOP
    ...
    character(len=LIS_CONST_PATH_LEN) ::  smapsmdir
    character*100          ::  temp
    ...
    allocate(THySM_struc(LIS_rc%nnest))
    ...
    call ESMF_ConfigFindLabel(LIS_config,&
         ...
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,smapsmdir,&
            rc=status)
       call LIS_verify(status, &
            'THySM data directory: is missing')
    ...
    call ESMF_ConfigFindLabel(LIS_config,&
         ...
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,THySM_struc(n)%useSsdevScal,&
            rc=status)
       call LIS_verify(status, 'THySM use scaled standard deviation model: is missing')
    enddo
    
    call ESMF_ConfigFindLabel(LIS_config,"THySM model CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,modelcdffile(n),rc=status)
       call LIS_verify(status, 'THySM model CDF file: not defined')
    enddo

    call ESMF_ConfigFindLabel(LIS_config,"THySM observation CDF file:",&
         rc=status)
    do n=1,LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,obscdffile(n),rc=status)   
       call LIS_verify(status, 'THySM observation CDF file: not defined')
    enddo
    
    call ESMF_ConfigFindLabel(LIS_config, "THySM soil moisture number of bins in the CDF:", rc=status)
    do n=1, LIS_rc%nnest
       call ESMF_ConfigGetAttribute(LIS_config,THySM_struc(n)%nbins, rc=status)
       call LIS_verify(status, "THySM soil moisture number of bins in the CDF: not defined")
    enddo
   ...
   do n=1,LIS_rc%nnest
    ...
    write(LIS_logunit,*)&
         '[INFO] read THySM  data specifications'       
...
! Set obs err to be uniform (will be rescaled later for each grid point). 
          ...
          THySM_struc(n)%ssdev_inp = obs_pert%ssdev(1)
...
! initializing the perturbations to be zero 
    ...
    enddo
    write(LIS_logunit,*) &
         '[INFO] Created the States to hold the THySM observations data'
    do n=1,LIS_rc%nnest
       if(LIS_rc%lis_obs_map_proj(k).eq."latlon") then
          cornerlat1 = max(25.005, nint((LIS_rc%obs_gridDesc(k,4)-25.005)/0.01)*0.01+25.005-50*0.01)
          cornerlon1 = max(-124.995, nint((LIS_rc%obs_gridDesc(k,5)+124.995)/0.01)*0.01-124.995-50*0.01)
          cornerlat2 = min(52.995, nint((LIS_rc%obs_gridDesc(k,7)-25.005)/0.01)*0.01+25.005+50*0.01)
          cornerlon2 = min(-67.005, nint((LIS_rc%obs_gridDesc(k,8)+124.995)/0.01)*0.01-124.995+50*0.01)
       elseif(LIS_rc%lis_obs_map_proj(k).eq."lambert") then
!          cornerlat1 = max(25.005, nint((LIS_rc%minLat(n)-25.005)/0.01)*0.01-50*0.01)
!          cornerlon1 = max(-124.995, nint((LIS_rc%minLon(n)+124.995)/0.01)*0.01-124.995-50*0.01)
!          cornerlat2 = min(52.995, nint((LIS_rc%maxLat(n)-25.005)/0.01)*0.01+50*0.01)
!          cornerlon2 = min(-67.005, nint((LIS_rc%maxLon(n)+124.995)/0.01)*0.01-124.995+50*0.01)
          write(LIS_logunit,*) '[ERR] lambert domain support is not implemented for THySM'
          call LIS_endrun()
       endif

       THySM_struc(n)%nc = nint((cornerlon2-cornerlon1)/0.01)+1
       THySM_struc(n)%nr = nint((cornerlat2-cornerlat1)/0.01)+1
       
       gridDesci(1) = 0 
       gridDesci(2) = THySM_struc(n)%nc
       gridDesci(3) = THySM_struc(n)%nr 
       ...
       THySM_struc(n)%r1 = nint((cornerlat1 - 25.005)/0.01)+1
       THySM_struc(n)%r2 = nint((cornerlat2 - 25.005)/0.01)+1
       THySM_struc(n)%c1 = nint((cornerlon1 + 124.995)/0.01)+1
       THySM_struc(n)%c2 = nint((cornerlon2 + 124.995)/0.01)+1

       allocate(THySM_struc(n)%smobs(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       allocate(THySM_struc(n)%smtime(LIS_rc%obs_lnc(k),LIS_rc%obs_lnr(k)))
       
       THySM_struc(n)%smtime = -1

       allocate(THySM_struc(n)%n11(LIS_rc%obs_lnc(k)*&
            LIS_rc%obs_lnr(k)))
       allocate(THySM_struc(n)%rlat(&
            LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
       allocate(THySM_struc(n)%rlon(&
            LIS_rc%obs_lnc(k)*LIS_rc%obs_lnr(k)))
       ...
       call LIS_registerAlarm("THySM read alarm",&
            86400.0, 86400.0)
    ...
    do n=1,LIS_rc%nnest
       ...
       if(LIS_rc%dascaloption(k).ne."none") then 
          ...
          call LIS_getCDFattributes(k,modelcdffile(n),&
               THySM_struc(n)%ntimes,ngrid)
          
          allocate(THySM_struc(n)%model_xrange(&
               LIS_rc%obs_ngrid(k), THySM_struc(n)%ntimes, &
               THySM_struc(n)%nbins))
          allocate(THySM_struc(n)%obs_xrange(&
               LIS_rc%obs_ngrid(k), THySM_struc(n)%ntimes, &
               THySM_struc(n)%nbins))
          allocate(THySM_struc(n)%model_cdf(&
               LIS_rc%obs_ngrid(k), THySM_struc(n)%ntimes, &
               THySM_struc(n)%nbins))
          allocate(THySM_struc(n)%obs_cdf(&
               LIS_rc%obs_ngrid(k), THySM_struc(n)%ntimes, &
               THySM_struc(n)%nbins))
          allocate(THySM_struc(n)%model_mu(LIS_rc%obs_ngrid(k),&
               THySM_struc(n)%ntimes))
          allocate(THySM_struc(n)%model_sigma(LIS_rc%obs_ngrid(k),&
               THySM_struc(n)%ntimes))
          allocate(THySM_struc(n)%obs_mu(LIS_rc%obs_ngrid(k),&
               THySM_struc(n)%ntimes))
          allocate(THySM_struc(n)%obs_sigma(LIS_rc%obs_ngrid(k),&
               THySM_struc(n)%ntimes))
...
!          stop
    ...
    enddo
  end subroutine THySM_setup
end module THySM_Mod
