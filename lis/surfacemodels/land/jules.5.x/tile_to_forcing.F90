...
! Specific humidity (Kg/Kg)
  qw_1_ij(1,1) = jules5x_struc(n)%jules5x(t)%qair/jules5x_struc(n)%forc_count
! Air temperature (k)
  tl_1_ij(1,1) = jules5x_struc(n)%jules5x(t)%tair/jules5x_struc(n)%forc_count
! W'ly wind component (m s-1)
  u_1_ij(1,1)  = jules5x_struc(n)%jules5x(t)%wind_e/jules5x_struc(n)%forc_count
! S'ly wind component (m s-1)
  v_1_ij(1,1)  = jules5x_struc(n)%jules5x(t)%wind_n/jules5x_struc(n)%forc_count
! Surface pressure (Pascals)
  pstar_ij(1,1)   = jules5x_struc(n)%jules5x(t)%psurf/jules5x_struc(n)%forc_count
...
! All rain goes to Large-scale rainfall (kg m-2/s)
  if(jules5x_struc(n)%jules5x(t)%rainf .ne. LIS_rc%udef) then
    ls_rain_ij(1,1) = jules5x_struc(n)%jules5x(t)%rainf/ jules5x_struc(n)%forc_count
  else
    ls_rain_ij(1,1) = 0.0
  endif

  if (LIS_FORC_Snowf%selectOpt .eq. 1) then
    ls_snow_ij(1,1) = jules5x_struc(n)%jules5x(t)%snowf / jules5x_struc(n)%forc_count
  else 
    if (jules5x_struc(n)%jules5x(t)%tair .lt. LIS_CONST_TKFRZ) then
...
! Surface downward SW radiation (W m-2)
  sw_down_ij(1,1) = jules5x_struc(n)%jules5x(t)%swdown/jules5x_struc(n)%forc_count
! Surface downward LW radiation (W m-2)
  lw_down_ij(1,1) = jules5x_struc(n)%jules5x(t)%lwdown/jules5x_struc(n)%forc_count
