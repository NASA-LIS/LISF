...
!BOP
!
! !ROUTINE: jules5x_initialize
! \label{jules5x_initialize}
!
! !REVISION HISTORY:
!
! !INTERFACE:
subroutine jules5x_initialize(mtype)
   ! !USES:
...
! !DESCRIPTION:
!
!  This routine initializes the jules5x state variables with some
...
!EOP
   ...
   do n=1, LIS_rc%nnest
      write(LIS_logunit,*) "MSG: jules5x_initialize -- initialzie for jules5x"
      k = 1
      do
         ...
         do t=start_k, end_k
           ...
           bexp_soilt(1,1,:)   = jules5x_struc(n)%jules5x(t)%b
           sathh_soilt(1,1,:)  = jules5x_struc(n)%jules5x(t)%sathh
           satcon_soilt(1,1,:) = jules5x_struc(n)%jules5x(t)%satcon
           smvcst_soilt(1,1,:) = jules5x_struc(n)%jules5x(t)%sm_sat
           smvccl_soilt(1,1,:) = jules5x_struc(n)%jules5x(t)%sm_crit
           smvcwt_soilt(1,1,:) = jules5x_struc(n)%jules5x(t)%sm_wilt
           hcap_soilt(1,1,:)   = jules5x_struc(n)%jules5x(t)%hcap
           hcon_soilt(1,1,:)   = jules5x_struc(n)%jules5x(t)%hcon
           albsoil_soilt(1,1)  = jules5x_struc(n)%jules5x(t)%albsoil 
         ...
         call init_ic(jules5x_struc(n)%namelist_dir) 
         call init_params(jules5x_struc(n)%namelist_dir)
         call init_parms()
