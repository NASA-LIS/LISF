...
! EMK BEGIN New output variables
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_SNOW_SOOT,                          &
                     value=jules5x_struc(n)%jules5x(t)%soot_ij,   &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_GRND_SNOW,                                       &
                      value =  jules5x_struc(n)%jules5x(t)%snow_grnd(pft), &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_SURFT_SNOW,                                       &
                      value =  jules5x_struc(n)%jules5x(t)%snow_tile(pft), &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  ...
                  !!! nsmax > 0, multi-layer snow physics
                    ...
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SOWN_NLAYER,                         &
                       value=jules5x_struc(n)%jules5x(t)%nsnow(pft)*1.0, & ! nsnow is integer, -> real by *1.0
                       vlevel=1, unit="-", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
