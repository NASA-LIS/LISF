...
! !DESCRIPTION:
!
!  Forcing-only option (jules5x) for calling the forcing transfer routines.
! 
!  The arguments are: 
!  \begin{description}
!  \item[n]
...
!EOP
  ...
  jules5x_struc(n)%forc_count = jules5x_struc(n)%forc_count + 1 
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     jules5x_struc(n)%jules5x(t)%tair   = jules5x_struc(n)%jules5x(t)%tair   + tmp(tid)
     jules5x_struc(n)%jules5x(t)%qair   = jules5x_struc(n)%jules5x(t)%qair   + q2(tid)
     jules5x_struc(n)%jules5x(t)%swdown = jules5x_struc(n)%jules5x(t)%swdown + swd(tid)
     jules5x_struc(n)%jules5x(t)%lwdown = jules5x_struc(n)%jules5x(t)%lwdown + lwd(tid)
     jules5x_struc(n)%jules5x(t)%wind_e = jules5x_struc(n)%jules5x(t)%wind_e + uwind(tid)
     jules5x_struc(n)%jules5x(t)%wind_n = jules5x_struc(n)%jules5x(t)%wind_n + vwind(tid)
     jules5x_struc(n)%jules5x(t)%psurf  = jules5x_struc(n)%jules5x(t)%psurf  + psurf(tid)
     jules5x_struc(n)%jules5x(t)%rainf  = jules5x_struc(n)%jules5x(t)%rainf  + rain(tid)
     
     if(LIS_FORC_CRainf%selectOpt .eq. 1) then
       if(crain(tid) .ne. LIS_rc%udef) then 
          jules5x_struc(n)%jules5x(t)%rainf_c = jules5x_struc(n)%jules5x(t)%rainf_c + crain(tid)
       else
          jules5x_struc(n)%jules5x(t)%rainf_c = jules5x_struc(n)%jules5x(t)%rainf_c + 0.0
       endif
     else
     ...
     if(LIS_FORC_Snowf%selectOpt .eq. 1) then
        if(snowf(tid) .ne. LIS_rc%udef) then 
          jules5x_struc(n)%jules5x(t)%snowf = jules5x_struc(n)%jules5x(t)%snowf + snowf(tid)
        else
          jules5x_struc(n)%jules5x(t)%snowf = jules5x_struc(n)%jules5x(t)%snowf + 0.0
        endif
     else
