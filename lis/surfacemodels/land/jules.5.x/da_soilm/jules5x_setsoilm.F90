...
!BOP
! !ROUTINE: jules5x_setsoilm
!  \label{jules5x_setsoilm}
!
! !REVISION HISTORY:
...
!    adjust the states  
! !INTERFACE:
subroutine jules5x_setsoilm(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!  real                   :: MaxEns_p_s_sthu1, MaxEns_p_s_sthu2, MaxEns_p_s_sthu3, MaxEns_p_s_sthu4
  ...
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 1 failed in jules5x_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 2 failed in jules5x_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 3 failed in jules5x_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 4 failed in jules5x_setsoilm")

  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in jules5x_setsoilm")
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in jules5x_setsoilm")
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in jules5x_setsoilm")
  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 4 failed in jules5x_setsoilm")
...
! differs from that of the prognostic variable (p_s_sthu, p_s_smvcst) [m3/m3].
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     MAX_THRESHOLD = jules5x_struc(n)%jules5x(t)%p_s_smvcst(1) ! Volumetric saturation point (m^3 m-3 of soil)
     ...
     !MN: delta = X(+) - X(-)
     !NOTE: "jules5x_updatesoilm.F90" updates the soilmx(t)   
     delta1 = (soilm1(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(1))* 1/dzsoil(1)*1/1000  ! [kg/m2]*1/m*1/(kg/m3) --> [m3/m3]
     delta2 = (soilm2(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(2))* 1/dzsoil(2)*1/1000 
     delta3 = (soilm3(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(3))* 1/dzsoil(3)*1/1000
     delta4 = (soilm4(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(4))* 1/dzsoil(4)*1/1000
	...
	+delta1.lt.sm_threshold) then 
     ...
     if(jules5x_struc(n)%jules5x(t)%p_s_sthu(2)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(2)&
	+delta2.gt.MIN_THRESHOLD .and.&
        jules5x_struc(n)%jules5x(t)%p_s_sthu(2)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(2)&
	+delta2.lt.sm_threshold) then 
     ...
     if(jules5x_struc(n)%jules5x(t)%p_s_sthu(3)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(3)&
	+delta3.gt.MIN_THRESHOLD .and.&
        jules5x_struc(n)%jules5x(t)%p_s_sthu(3)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(3)&
	+delta3.lt.sm_threshold) then 
     ...
     if(jules5x_struc(n)%jules5x(t)%p_s_sthu(4)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(4)&
	+delta4.gt.MIN_THRESHOLD .and.&
        jules5x_struc(n)%jules5x(t)%p_s_sthu(4)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(4)&
...
! compute the ensemble min and max for case 1-2        
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           MAX_THRESHOLD = jules5x_struc(n)%jules5x(t)%p_s_smvcst(1) ! [m3/m3] assumed value for all layers are the same
           sm_threshold  = MAX_THRESHOLD - MIN_THRESHOLD       
	      ...
	      delta4 = (soilm4(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(4))* 1/dzsoil(4)*1/1000   
              ...
              jules5x_struc(n)%jules5x(t)%p_s_sthu(1) = &
		(jules5x_struc(n)%jules5x(t)%p_s_sthu(1)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(1)&
                 +delta1)/jules5x_struc(n)%jules5x(t)%p_s_smvcst(1) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
              jules5x_struc(n)%jules5x(t)%smcl_soilt(1) = soilm1(t) ![kg/m2]
	      ...
	      !Because both “update_flag_new” and “update_flag_tile” are TRUE 
              ...
              !if(jules5x_struc(n)%jules5x(t)%p_s_sthu(2)+delta2.gt.MIN_THRESHOLD .and.&   
              !     jules5x_struc(n)%jules5x(t)%p_s_sthu(2)+delta2.lt.sm_threshold) then 
              !   jules5x_struc(n)%jules5x(t)%p_s_sthu(2) = jules5x_struc(n)%jules5x(t)%p_s_sthu(2)+&
              !        soilm2(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(2)
              jules5x_struc(n)%jules5x(t)%p_s_sthu(2) = &
		(jules5x_struc(n)%jules5x(t)%p_s_sthu(2)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(2)&
                 +delta2)/jules5x_struc(n)%jules5x(t)%p_s_smvcst(2) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
              jules5x_struc(n)%jules5x(t)%smcl_soilt(2) = soilm2(t)
              if(soilm2(t).lt.0) then 
                 print*, 'setsoilm2 ',t,soilm2(t)
                   stop
              endif
              !endif
              !if(jules5x_struc(n)%jules5x(t)%p_s_sthu(3)+delta3.gt.MIN_THRESHOLD .and.&
              !     jules5x_struc(n)%jules5x(t)%p_s_sthu(3)+delta3.lt.sm_threshold) then 
              !   jules5x_struc(n)%jules5x(t)%p_s_sthu(3) = jules5x_struc(n)%jules5x(t)%p_s_sthu(3)+&
              !        soilm3(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(3)
              jules5x_struc(n)%jules5x(t)%p_s_sthu(3) = &
		(jules5x_struc(n)%jules5x(t)%p_s_sthu(3)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(3)&
                 +delta3)/jules5x_struc(n)%jules5x(t)%p_s_smvcst(3) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
              jules5x_struc(n)%jules5x(t)%smcl_soilt(3) = soilm3(t)
              ...
              !endif
              ! surface layer
              !if(jules5x_struc(n)%jules5x(t)%p_s_sthu(4)+delta4.gt.MIN_THRESHOLD .and.&
              !     jules5x_struc(n)%jules5x(t)%p_s_sthu(4)+delta4.lt.sm_threshold) then 
              !   jules5x_struc(n)%jules5x(t)%p_s_sthu(4) = jules5x_struc(n)%jules5x(t)%p_s_sthu(4)+&
              !        soilm4(t)-jules5x_struc(n)%jules5x(t)%smcl_soilt(4)
              jules5x_struc(n)%jules5x(t)%p_s_sthu(4) = &
		(jules5x_struc(n)%jules5x(t)%p_s_sthu(4)*jules5x_struc(n)%jules5x(t)%p_s_smvcst(4)&
                 +delta4)/jules5x_struc(n)%jules5x(t)%p_s_smvcst(4) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
              jules5x_struc(n)%jules5x(t)%smcl_soilt(4) = soilm4(t)
...
!-----------------------------------------------------------------------------------------
			...
			jules5x_struc(n)%jules5x(t)%p_s_smvcst(1) ! ([kg/m2]*(1/m*1/(kg/m3)))/[m3/m3]--> fraction
              jules5x_struc(n)%jules5x(t)%smcl_soilt(1) = smc_tmp
			...
			jules5x_struc(n)%jules5x(t)%p_s_smvcst(2)
              jules5x_struc(n)%jules5x(t)%smcl_soilt(2) = smc_tmp
			...
			jules5x_struc(n)%jules5x(t)%p_s_smvcst(3)
              jules5x_struc(n)%jules5x(t)%smcl_soilt(3) = smc_tmp
			...
			jules5x_struc(n)%jules5x(t)%p_s_smvcst(4)
              jules5x_struc(n)%jules5x(t)%smcl_soilt(4) = smc_tmp
...
!--------------------------------------------------------------------------
		...
		! NOTE: be careful with the unit. do not change the unit of delta in the loop
              ! [m3w/m3s]  + ([-] - [-])*[m3/m3] 
                       delta(j) = delta(j)+ & !  * 1/dzsoil(j)*1/1000 
                            (jules5x_struc(n)%jules5x(t)%p_s_sthu(j) - &
                             jules5x_struc(n)%jules5x(t_unpert)%p_s_sthu(j)) * &
		      ...
		      MAX_THRESHOLD = jules5x_struc(n)%jules5x(t)%p_s_smvcst(1) ! assume it is the same for all layers [m3/m3]
                    sm_threshold  = MAX_THRESHOLD-MIN_THRESHOLD
              ...
              do j=1,4
		         ...
		         MAX_THRESHOLD = jules5x_struc(n)%jules5x(t)%p_s_smvcst(1)
                       if(.not.(tmpval.le.0.0 .or.&
                          ...
                          jules5x_struc(n)%jules5x(t)%smcl_soilt(j) = &
                               jules5x_struc(n)%jules5x(t)%smcl_soilt(j) - delta(j)/(1/dzsoil(j)*1/1000) ! [kg/m2] - [m3/m3] / ([1/m]*[1/kg/m3]) --> [kg/m2]

                          jules5x_struc(n)%jules5x(t)%p_s_sthu(j) = &
                          (jules5x_struc(n)%jules5x(t)%p_s_sthu(j)*&
			      jules5x_struc(n)%jules5x(t)%p_s_smvcst(j)-&
			      delta(j)) /& ! * 1/dzsoil(j)*1/1000)/&
			      jules5x_struc(n)%jules5x(t)%p_s_smvcst(j) !([-][m3/m3]-[m3/m3]) / [m3/m3] --> [-]
                    ...
                    tmpval = jules5x_struc(n)%jules5x(t)%p_s_sthu(j) * &
			        jules5x_struc(n)%jules5x(t)%p_s_smvcst(j) ! [-][m3/m3]
...
!                 write(LIS_logunit,*) '[ERR] Please adjust the perturbation settings ..'
			  ...
			  MAX_THRESHOLD = jules5x_struc(n)%jules5x(t)%p_s_smvcst(1)                       
                       if(jules5x_struc(n)%jules5x(t)%p_s_sthu(j)* &
			     jules5x_struc(n)%jules5x(t)%p_s_smvcst(j).gt.MAX_THRESHOLD.or.&
                          jules5x_struc(n)%jules5x(t)%smcl_soilt(j) * &
			     ...
			     (1/dzsoil(j)*1/1000)   ! [m3w/m3s] / ([1/m1s][m3w/kg]) --> kg/m2s
                       ...
                       if(jules5x_struc(n)%jules5x(t)%p_s_sthu(j)* &
			     jules5x_struc(n)%jules5x(t)%p_s_smvcst(j).lt.MIN_THRESHOLD.or.&
                          jules5x_struc(n)%jules5x(t)%smcl_soilt(j)* & 
...
end subroutine jules5x_setsoilm
