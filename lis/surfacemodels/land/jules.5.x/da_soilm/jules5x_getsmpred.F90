...
!BOP
! !ROUTINE: jules5x_getsmpred
! \label{jules5x_getsmpred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules5x_getsmpred(n, k,obs_pred)
! !USES:
  use ESMF
...
!use dzsoil instead.
 ...
 smc1(t) = jules5x_struc(n)%jules5x(t)%smcl_soilt(1) * 1/dzsoil(1)*1/1000  ! [kg/m2]*1/m*1/(kg/m3) --> [m3/m3] 
    ...
    if (i.eq.1376)then
        do m=1,LIS_rc%nensem(n)
           t = (i-1)*LIS_rc%nensem(n)+m
                      tmp1(m) = jules5x_struc(n)%jules5x(t)%smcl_soilt(1)
	              tmp2(m) = smc1(t)
                      tmp3(m) = obs_pred(i,m)
...
end subroutine jules5x_getsmpred
