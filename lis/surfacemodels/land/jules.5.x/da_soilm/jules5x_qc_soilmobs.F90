...
!BOP
! !ROUTINE: jules5x_qc_soilmobs
! \label{jules5x_qc_soilmobs}
!
! !REVISION HISTORY:
...
!  real                     :: sthu4_obs(LIS_rc%obs_ngrid(k))
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in jules5x_qc_soilmobs")
  call ESMF_FieldGet(obs_sm_field,localDE=0,farrayPtr=smobs,rc=status) ! [m3/m3] ?
  call LIS_verify(status,& 
       "ESMF_FieldGet failed in jules5x_qc_soilmobs")
...
#if 0 
     smc1(t) = jules5x_struc(n)%jules5x(t)%smcl_soilt(1) ! [kg/m2]
     smc2(t) = jules5x_struc(n)%jules5x(t)%smcl_soilt(2)
     smc3(t) = jules5x_struc(n)%jules5x(t)%smcl_soilt(3)
     smc4(t) = jules5x_struc(n)%jules5x(t)%smcl_soilt(4)
     ! Unfrozen soil moisture
     sthu1(t) = jules5x_struc(n)%jules5x(t)%p_s_sthu(1) ![-]    Noah -->sh2o
     sthu2(t) = jules5x_struc(n)%jules5x(t)%p_s_sthu(2)
     sthu3(t) = jules5x_struc(n)%jules5x(t)%p_s_sthu(3)
     sthu4(t) = jules5x_struc(n)%jules5x(t)%p_s_sthu(4)
#endif

     ! Frozen soil moisture
     sthf1(t) = jules5x_struc(n)%jules5x(t)%p_s_sthf(1) ![-]    
     sthf2(t) = jules5x_struc(n)%jules5x(t)%p_s_sthf(2)
     sthf3(t) = jules5x_struc(n)%jules5x(t)%p_s_sthf(3)
     sthf4(t) = jules5x_struc(n)%jules5x(t)%p_s_sthf(4)

     stc1(t) = jules5x_struc(n)%jules5x(t)%t_soil(1) ! stc
     stc2(t) = jules5x_struc(n)%jules5x(t)%t_soil(2)
     stc3(t) = jules5x_struc(n)%jules5x(t)%t_soil(3)
     stc4(t) = jules5x_struc(n)%jules5x(t)%t_soil(4)
...
sneqv = 0
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
      ...
      vegt(t) = pft  
      sneqv(t) = jules5x_struc(n)%jules5x(t)%snowdepth(pft) * &
                     jules5x_struc(n)%jules5x(t)%rho_snow_grnd(pft) ![kg m-2]
      ...
      !   fsno(t) = TANH( jules5x_struc(n)%jules5x(t)%snowdepth(pft) * 100 /(2.5* 0.01 * FMELT(t))) ! Z0 = 0.01 , convert the snow depth from m to mm 
      !endif
      ...
      !snow_frac = 0.0
       ...
       if(l_aggregate) then
         ...
         if(jules5x_struc(n)%jules5x(t)%snow_tile(pft)+jules5x_struc(n)%jules5x(t)%snow_grnd(pft)>1.0) then
            fsno(t) = jules5x_struc(n)%jules5x(t)%frac(pft)
         endif
       endif
      t_skin(t) = jules5x_struc(n)%jules5x(t)%tstar_tile(pft)
...
!       enddo         
      ...
      !!frac_tmp(t)= jules5x_struc(n)%jules5x(t)%frac(pft)
      !frac_tmp(t)= 1-EXP(-0.5*jules5x_struc(n)%jules5x(t)%lai(pft))
      lai_tmp(t)=jules5x_struc(n)%jules5x(t)%lai(pft)
      !sliq_tmp(t)= sum(jules5x_struc(n)%jules5x(t)%sliq(pft,:))
      !nsnow_tmp(t)= jules5x_struc(n)%jules5x(t)%nsnow(pft)
      p_s_smvcwt_tmp(t)= jules5x_struc(n)%jules5x(t)%p_s_smvcwt(1) ! m3/m3
      p_s_smvcst_tmp(t)= jules5x_struc(n)%jules5x(t)%p_s_smvcst(1) ! m3/m3
 enddo


  call LIS_convertPatchSpaceToObsSpace(n,k,&       
       LIS_rc%lsm_index, &
       jules5x_struc(n)%jules5x(:)%rainf,&
       rainf_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       sneqv,&
...
!           smobs(t) = LIS_rc%udef
        ...
        elseif(sthf4_obs(t).gt.0.0001) then
           smobs(t) = LIS_rc%udef
! layer temperature --> jules5x_struc(n)%jules5x(t)%t_soil(i) 
        ...
        elseif(stc4_obs(t).le.LIS_CONST_TKFRZ) then
           smobs(t) = LIS_rc%udef
! MN: skin temperature 
! jules5x_struc(n)%jules5x(t)%tstar_tile(pft)
...
end subroutine jules5x_qc_soilmobs
