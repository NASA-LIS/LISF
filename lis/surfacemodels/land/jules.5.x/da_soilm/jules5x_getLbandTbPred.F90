...
!BOP
! !ROUTINE: jules5x_getLbandTbPred
! \label{jules5x_getLbandTbPred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules5x_getLbandTbPred(n, obs_pred)
! !USES:
  use ESMF
...
!call the forward model
  ...
  call LIS_verify(status, &
       "Error in StateGet in jules5x_getLbandTbPred for Lband_Hpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbH, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in jules5x_getLbandTbPred for Lband_Hpol')

  call ESMF_StateGet(LIS_forwardState(n), "Lband_Vpol", varField, rc=status)
  call LIS_verify(status, &
       "Error in StateGet in jules5x_getLbandTbPred for Lband_Vpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbV, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in jules5x_getLbandTbPred for Lband_Vpol')
...
end subroutine jules5x_getLbandTbPred
