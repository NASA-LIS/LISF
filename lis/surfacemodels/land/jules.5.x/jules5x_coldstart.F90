...
!BOP
!
! !ROUTINE: jules5x_coldstart
! \label{jules5x_coldstart}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules5x_coldstart(mtype)
   ! !USES:
...
!EOP
   ...
   do n=1, LIS_rc%nnest
      if (trim(LIS_rc%startcode) .eq. "coldstart") then
         write(LIS_logunit,*) "MSG: jules5x_coldstart -- cold-starting jules5x"
         k = 1
         do
