...
!BOP
! !ROUTINE: jules52_getsmpred
! \label{jules52_getsmpred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules52_getsmpred(n, k,obs_pred)
! !USES:
  use ESMF
...
!     smc1(t) = jules52_struc(n)%jules52(t)%smcl_soilt(1) * 1/LIS_sfmodel_struc(n)%lyrthk(1)*1/1000  ! [kg/m2]*1/m*1/(kg/m3) --> [m3/m3] 

!MN: Eric changed the unit of LIS_sfmodel_struc%lyrthk so LIS_sfmodel_struc%lyrthk is in cm, we
!use dzsoil instead.

 smc1(t) = jules52_struc(n)%jules52(t)%smcl_soilt(1) * 1/dzsoil(1)*1/1000  ! [kg/m2]*1/m*1/(kg/m3) --> [m3/m3] 
    ...
    if (i.eq.1376)then
        do m=1,LIS_rc%nensem(n)
           t = (i-1)*LIS_rc%nensem(n)+m
                      tmp1(m) = jules52_struc(n)%jules52(t)%smcl_soilt(1)
	              tmp2(m) = smc1(t)
                      tmp3(m) = obs_pred(i,m)
...
end subroutine jules52_getsmpred
