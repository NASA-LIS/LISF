...
!BOP
!
! !ROUTINE: jules52_read_multilevel_param
!  \label{jules52_read_multilevel_param}
!
! !REVISION HISTORY:
...
! !INTERFACE:
subroutine jules52_read_multilevel_param(n, ncvar_name, level, placeholder)
! !USES:
    use esmf
    use netcdf
...
!EOP      
    ...
    if(file_exists) then
        ...
        call LIS_verify(ios, 'Error in nf90_open in jules52_read_multilevel_param')

        ios = nf90_inq_dimid(nid, 'east_west', nc_ID)
        call LIS_verify(ios, 'Error in nf90_inq_dimid in jules52_read_multilevel_param')

        ios = nf90_inq_dimid(nid, 'north_south', nr_ID)
        call LIS_verify(ios, 'Error in nf90_inq_dimid in jules52_read_multilevel_param')

        ios = nf90_inquire_dimension(nid, nc_ID, len=nc)
        call LIS_verify(ios, 'Error in nf90_inquire_dimension in jules52_read_multilevel_param')
        ...
        call LIS_verify(ios, 'Error in nf90_get_var in jules52_read_multilevel_param')

        ios = nf90_close(nid)
        call LIS_verify(ios, 'Error in nf90_close in jules52_read_multilevel_param')
 ...
 end subroutine jules52_read_multilevel_param
