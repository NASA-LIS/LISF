...
!BOP
!
! !ROUTINE: jules52_coldstart
! \label{jules52_coldstart}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules52_coldstart(mtype)
   ! !USES:
...
!EOP
   ...
   do n=1, LIS_rc%nnest
      if (trim(LIS_rc%startcode) .eq. "coldstart") then
         write(LIS_logunit,*) "MSG: jules52_coldstart -- cold-starting jules52"
         k = 1
         do
