...
!BOP
!
! !ROUTINE: jules52_main
! \label{jules52_main}
!
! !ROUTINE: jules52_main.F90
!
! !REVISION HISTORY:
...
! !DESCRIPTION:
!
!  Calls the run routines for the forcing-only option (jules52)
!
!  The arguments are:
!  \begin{description}
!  \item[n]
...
!EOP
   ...
   alarmCheck = LIS_isAlarmRinging(LIS_rc, "JULES.5.2 model alarm")
   if (alarmCheck) Then
         ...
         do
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                 ...
                 tstar_box=tstar_box+jules52_struc(n)%jules52(t)%frac(pft)*jules52_struc(n)%jules52(t)%tstar_tile(pft)
               endif
            enddo
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                  ...
                  jules52_struc(n)%jules52(t)%tstar=tstar_box
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,         &
                        LIS_MOC_SOILTEMP,                            &
                        value=jules52_struc(n)%jules52(t)%t_soil(i), &
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,       &
                        LIS_MOC_SOILMOIST,                         &
                        value=jules52_struc(n)%jules52(t)%smcl_soilt(i), &
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     ...
                     smc_liq = jules52_struc(n)%jules52(t)%p_s_sthu(i) * jules52_struc(n)%jules52(t)%p_s_smvcst(i) 
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_SMLIQFRAC,                             &
                        value=smc_liq,                                 &
                        vlevel=i, unit="m^3 m-3", direction="-",       &
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     ...
                     smc_sat_frac = jules52_struc(n)%jules52(t)%p_s_sthu(i)+jules52_struc(n)%jules52(t)%p_s_sthf(i)
                     if (smc_sat_frac > tiny(smc_sat_frac)) then 
                        smc_liq = jules52_struc(n)%jules52(t)%p_s_sthu(i) / smc_sat_frac
                     else
                        smc_liq = 0.0
                     endif 
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     smc_ice = jules52_struc(n)%jules52(t)%p_s_sthf(i) * jules52_struc(n)%jules52(t)%p_s_smvcst(i) 
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_SMFROZFRAC,                            &
                        value=smc_ice,                                 &
                        vlevel=i, unit="m^3 m-3", direction="-",       &
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     ...
                     smc_sat_frac = jules52_struc(n)%jules52(t)%p_s_sthu(i)+jules52_struc(n)%jules52(t)%p_s_sthf(i)
                     if (smc_sat_frac > tiny(smc_sat_frac)) then
                        smc_ice = jules52_struc(n)%jules52(t)%p_s_sthf(i) / smc_sat_frac
                     else
                        smc_ice = 0.0
                     endif
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_JULES_STHU,                            &
                        value=jules52_struc(n)%jules52(t)%p_s_sthu(i), &
                        vlevel=i, unit="-", direction="-",             &
                        valid_min=0.0, valid_max=1.0,                  &
                        surface_type=LIS_rc%lsm_index)
                  enddo
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_JULES_STHF,                            &
                        value=jules52_struc(n)%jules52(t)%p_s_sthf(i), &
                        vlevel=i, unit="-", direction="-",             &
                        valid_min=0.0, valid_max=1.0,                  &
                        surface_type=LIS_rc%lsm_index)
                  enddo
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,               &
                     ...
                     value=jules52_struc(n)%jules52(t)%p_s_sthu_min(1), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules52_struc(n)%jules52(t)%p_s_smvccl(1), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules52_struc(n)%jules52(t)%p_s_smvcst(1), &
                     vlevel=1, unit="m^3 m-3", direction="-",         &
                     valid_min=0.0, valid_max=1.0,                    &
                        surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules52_struc(n)%jules52(t)%p_s_smvcwt(1), &
                     vlevel=1, unit="m^3 m-3", direction="-",         &
                     valid_min=0.0, valid_max=1.0,                    &
                        surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_EVAP,                              &
                     value=jules52_struc(n)%jules52(t)%fqw_1,   &
                     vlevel=1,unit="kg m-2 s-1",direction="UP", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,     &
                     LIS_MOC_QH,                              &
                     value=jules52_struc(n)%jules52(t)%ftl_1, &
                     vlevel=1,unit="W m-2",direction="UP",    &
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,         &
                     LIS_MOC_ECANOP,                             &
                     value=jules52_struc(n)%jules52(t)%ecan,     &
                     vlevel=1,unit="kg m-2 s-1", direction="UP", &
                     surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n,t,         &
                     LIS_MOC_JULES_ESOIL,                             &
                     value=jules52_struc(n)%jules52(t)%esoil,     &
                     vlevel=1,unit="kg m-2 s-1", direction="UP", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,       &
                     LIS_MOC_SUBSNOW,                          &
                     value=jules52_struc(n)%jules52(t)%ei,     &
                  ...
                  do i=1, jules52_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n,t,             &
                        LIS_MOC_SOILET,                                 &
                        vlevel=i,                                       &
                        value=jules52_struc(n)%jules52(t)%ext(i),       &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,               &
                     LIS_MOC_SNOWDEPTH,                                &
                     vlevel=1,                                         &
                     value=jules52_struc(n)%jules52(t)%snowdepth(pft), &
                     unit="m", direction="-",&
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_SWE,                                       &
                      value =  jules52_struc(n)%jules52(t)%snow_mass_ij, &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,              &
                      LIS_MOC_WATERTABLED,                             &
                      value = JULES52_struc(n)%jules52(t)%zw,          &
                      vlevel=1, unit="m", direction="-",               &
                      surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,                &
                     LIS_MOC_AVGSURFT,                                  &
                     value=jules52_struc(n)%jules52(t)%tstar_tile(pft), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,               &
                     LIS_MOC_EMISSFORC,                                 &
                     value = jules52_struc(n)%jules52(t)%emis_tile(pft),&
                     vlevel=1, unit="-", direction="-",                 &
                     surface_type = LIS_rc%lsm_index)

                  !***  surface radiative temperature 
                  trad_gb_mean = 0.0
                  do p=1, jules52_struc(n)%jules52(t)%nsurft
                    trad_gb_mean = trad_gb_mean + frac(1,p) * jules52_struc(n)%jules52(t)%tstar_tile(p)**4.0
                  enddo
                  ...
                  lw_upward = sbcon * jules52_struc(n)%jules52(t)%emis_tile(pft) * jules52_struc(n)%jules52(t)%tstar_tile(pft)**4 
                  lw_net    = jules52_struc(n)%jules52(t)%emis_tile(pft) * jules52_struc(n)%jules52(t)%lwdown - lw_upward 
                  call LIS_diagnoseSurfaceOutputVar(n, t,                          &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,                 &
                     LIS_MOC_ROUGHNESS,                                  &
                     value=jules52_struc(n)%jules52(t)%p_s_z0_tile(pft), &
                     vlevel=1,unit="m",direction="-",                    &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,                       &
                     LIS_MOC_THERMAL_ROUGHNESS,                                &
                     value=jules52_struc(n)%jules52(t)%p_s_z0_tile(pft) * z0h_z0m(pft), &
                     vlevel=1,unit="m",direction="-",                          &
                     surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QSB,                                             &
                     value = jules52_struc(n)%jules52(t)%sub_surf_roff,       &
                     vlevel=1, unit="kg m-2 s-1", direction="OUT", surface_type = LIS_rc%lsm_index)
                  
                  ! EMK
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_QS,                              &
                     value=jules52_struc(n)%jules52(t)%surf_roff*dt,   &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_QSB,                              &
                     value=jules52_struc(n)%jules52(t)%sub_surf_roff*dt,   &
                     vlevel=1,unit="kg m-2",direction="OUT", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QS,                                              &
                     value = jules52_struc(n)%jules52(t)%surf_roff,           &
                     vlevel=1, unit="kg m-2 s-1", direction="OUT", surface_type = LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QSM,                                             &
                     value = jules52_struc(n)%jules52(t)%snow_melt_gb,        &
                     vlevel=1, unit="kg m-2 s-1", direction="S2L", surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_DRIP,                                            &
                     value = jules52_struc(n)%jules52(t)%tot_tfall,           &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_CANOPINT,                                        &
                     value = jules52_struc(n)%jules52(t)%canopy(pft),      &
                     vlevel=1, unit="kg m-2", direction="-", &
                     surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_SWNET,                                           &
                     value= jules52_struc(n)%jules52(t)%sw_tile(pft),         &  
                     vlevel=1,  unit="W m-2", direction="DN", surface_type=LIS_rc%lsm_index)

                  if(pft <= jules52_struc(n)%npft) then  
                    ! vegetation surface types 
                    call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                       LIS_MOC_LAI,                                             &
                       value= jules52_struc(n)%jules52(t)%lai(pft),             &  
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_JULES_FSAT,                                             &
                     value= jules52_struc(n)%jules52(t)%fsat,             &  
                     vlevel=1,  unit="-", direction="-", surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_JULES_FWETL,                                             &
                     value= jules52_struc(n)%jules52(t)%fwetl,             &  
                     vlevel=1,  unit="-", direction="-", surface_type=LIS_rc%lsm_index)
...
! EMK BEGIN New output variables
                  ...
                  sfcprs = jules52_struc(n)%jules52(t)%psurf
                  sfctmp = jules52_struc(n)%jules52(t)%tair
                  q2 = jules52_struc(n)%jules52(t)%qair
                  es = 611.0*exp(2.501E6/461.0*(1./273.15 - 1./sfctmp))
                  q2sat = 0.622*es/(sfcprs-(1.-0.622)*es)
                  if (sfctmp .lt. &
                       jules52_struc(n)%jules52(t)%tair_agl_min) then
                     jules52_struc(n)%jules52(t)%tair_agl_min = sfctmp
                     jules52_struc(n)%jules52(t)%rhmin = q2 / q2sat
                  end if

                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_RHMIN,                              &
                     value=jules52_struc(n)%jules52(t)%rhmin,   &
                     vlevel=1,unit="-",direction="-", &
                     surface_type=LIS_rc%lsm_index)
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_RHMIN,                              &
                     value=(jules52_struc(n)%jules52(t)%rhmin*100),   &
                     vlevel=1,unit="%",direction="-", &
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_SNOW_SOOT,                          &
                     value=jules52_struc(n)%jules52(t)%soot_ij,   &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_GRND_SNOW,                                       &
                      value =  jules52_struc(n)%jules52(t)%snow_grnd(pft), &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_SURFT_SNOW,                                       &
                      value =  jules52_struc(n)%jules52(t)%snow_tile(pft), &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  
                  ! bulk snow density 
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDENSITY, &
                      value =  jules52_struc(n)%jules52(t)%rho_snow_grnd(pft),&
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWGRAIN, &
                      value =  jules52_struc(n)%jules52(t)%rgrain(pft),&
                  ...
                  if(nsmax .eq.0) then
                    ...
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWICEBULK,                         &
                       value=jules52_struc(n)%jules52(t)%sice(1,1), &
                       vlevel=1, unit="kg m-2", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                    ! snow liquid water 
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWLIQBULK,                         &
                       value=jules52_struc(n)%jules52(t)%sliq(1,1), &
                       vlevel=1, unit="kg m-2", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                    ! snow temperature
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWT,                         &
                       value=jules52_struc(n)%jules52(t)%tsnow(1,1), &
                       vlevel=1, unit="K", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                  !!! nsmax > 0, multi-layer snow physics
                    ...
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SOWN_NLAYER,                         &
                       value=jules52_struc(n)%jules52(t)%nsnow(pft)*1.0, & ! nsnow is integer, -> real by *1.0
                       vlevel=1, unit="-", direction="-",    &
                       surface_type=LIS_rc%lsm_index)

                    do i=1, jules52_struc(n)%nsmax
                      ! layer snow ice 
                       call LIS_diagnoseSurfaceOutputVar(n, t,       &
                          LIS_MOC_SNOWICEPROF,                         &
                          value=jules52_struc(n)%jules52(t)%sice(pft,i), &
                          vlevel=i, unit="kg m-2", direction="-",    &
                          surface_type=LIS_rc%lsm_index)
                      ! layer snow liquid water 
                       call LIS_diagnoseSurfaceOutputVar(n, t,       &
                          LIS_MOC_SNOWLIQPROF,                         &
                          value=jules52_struc(n)%jules52(t)%sliq(pft,i), &
                          vlevel=i, unit="kg m-2", direction="-",    &
                          surface_type=LIS_rc%lsm_index)
                      ! layer snow temperature K 
                       call LIS_diagnoseSurfaceOutputVar(n, t,            &
                          LIS_MOC_SNOWTPROF,                              &
                          value=jules52_struc(n)%jules52(t)%tsnow(pft,i), &
                          vlevel=i, unit="K", direction="-",              &
                          surface_type=LIS_rc%lsm_index)
                      ! layer snow density 
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDENPROF, &
                        value = jules52_struc(n)%jules52(t)%rho_snow(pft,i),&
                      ...
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWGRNPROF, &
                        value =  jules52_struc(n)%jules52(t)%rgrainl(pft,i),&
                      ...
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDEPPROF, &
                        value = jules52_struc(n)%jules52(t)%ds(pft,i),& 
                  ...
                  if(l_aggregate) then
                    ...
                    if(jules52_struc(n)%jules52(t)%snow_tile(pft)+jules52_struc(n)%jules52(t)%snow_grnd(pft)>1.0) then
                      snow_frac = jules52_struc(n)%jules52(t)%frac(pft)
                    endif
                  endif
                  ...
                  prcp = jules52_struc(n)%jules52(t)%rainf/jules52_struc(n)%forc_count
                  
                  if (jules52_struc(n)%jules52(t)%tair  .lt. 273.16) then 
                  ...
                  cpcp   = jules52_struc(n)%jules52(t)%rainf_c/jules52_struc(n)%forc_count
                  ...
                  albedo_land = wght_alb(1) * jules52_struc(n)%jules52(t)%land_albedo(1) &
                              + wght_alb(2) * jules52_struc(n)%jules52(t)%land_albedo(2) &
                              + wght_alb(3) * jules52_struc(n)%jules52(t)%land_albedo(3) &
                              + wght_alb(4) * jules52_struc(n)%jules52(t)%land_albedo(4) 
...
!                 ! JULES GPP and NPP
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_GPP, value = 1000.0 * jules52_struc(n)%jules52(t)%gpp, &
                  ...
                  ! (g m-2 s-1) = 1000.0 * (kg m-2 s-1)
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_NPP, value = 1000.0 * jules52_struc(n)%jules52(t)%npp, &
                                              vlevel=1, unit="g m-2 s-1", direction="IN", surface_type = LIS_rc%lsm_index)
!                ! End JULES GPP and NPP 

!                   !EMK Below code is borrowed from Noah 3.6
                  do i=1, jules52_struc(n)%sm_levels
                     smc = jules52_struc(n)%jules52(t)%smcl_soilt(i)/ &
                          (1000.0*dzsoil(i))
                     smcwilt = jules52_struc(n)%jules52(t)%sm_wilt
                     smcmax = jules52_struc(n)%jules52(t)%sm_sat
...
! EMK END New output variables
   ...
   ! EMK...See if jules52_struc(n)%jules52(t)%tair_agl_min needs to be reset
   ! for calculating RHMin.  
   write(fnest,'(i3.3)') n
   alarmCheck = LIS_isAlarmRinging(LIS_rc, &
        "JULES.5.2 RHMin alarm "//trim(fnest))
   if (alarmCheck) then
      write(LIS_logunit,*)'[INFO] Resetting tair_agl_min for RHMin calculation'
     do t = 1,LIS_rc%npatch(n,LIS_rc%lsm_index)
        jules52_struc(n)%jules52(t)%tair_agl_min = 999.
     end do
  end if

end subroutine jules52_main
