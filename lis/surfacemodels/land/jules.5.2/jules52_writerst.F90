...
!BOP
!
! !ROUTINE: jules52_writerst
! \label{jules52_writerst}
! !REVISION HISTORY:
...
!
! !DESCRIPTION:
!  This program writes restart files for jules52.
...
! \item[jules52\_dump\_restart](\ref{jules52_dump_restart})\\
!   writes the jules52 variables into the restart file
! \end{description}
!EOP
    ...
    ! set restart alarm
    alarmCheck = LIS_isAlarmRinging(LIS_rc, "JULES.5.2 restart alarm")
    ...
    if(alarmCheck .or. (LIS_rc%endtime ==1)) then
        If (LIS_masterproc) Then
            ...
            elseif(wformat .eq. "netcdf") then
                ...
                call LIS_verify(status,"Error in nf90_open in jules52_writerst")
#endif
        ...
        call jules52_dump_restart(n, ftn, wformat)
        ...
        if (LIS_masterproc) then
            ...
            elseif(wformat .eq. "netcdf") then
                ...
                call LIS_verify(status, "Error in nf90_close in jules52_writerst")
#endif
            endif
            write(LIS_logunit, *) "jules52 archive restart written: ", filen
        endif
    endif
end subroutine jules52_writerst

!BOP
!
! !ROUTINE: jules52_dump_restart
! \label{jules52_dump_restart}
!
! !REVISION HISTORY:
!  This subroutine is generated with the Model Implementation Toolkit developed
!  by Shugong Wang for the NASA Land Information System Version 7. The initial 
!  specification of the subroutine is defined by Sujay Kumar. 
!  5/9/16: Shugong Wang, initial implementation for LIS 7 and jules52
! !INTERFACE:
subroutine jules52_dump_restart(n, ftn, wformat)
...
!
!
!  The following is the list of variables written in the jules52
!  restart file:
!  \begin{verbatim}
...
!EOP 
    ...
    call LIS_writeGlobalHeader_restart(ftn, n, LIS_rc%lsm_index,                       &
                                   "JULES50",                                          & 
                                   dim1 =jules52_struc(n)%nsurft,                        &   
                                   dim2 =jules52_struc(n)%npft,                         &
                                   dim3 =jules52_struc(n)%sm_levels,                        &
                                   dim4 =jules52_struc(n)%nsmax,                        &
                                   dim5 =jules52_struc(n)%ns_deep,                      &
                                   dim6 =jules52_struc(n)%dim_cs1,                      &
                                   dim7 =jules52_struc(n)%nsurft*jules52_struc(n)%nsmax, &
                                   dim8 =jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers, &
                                   dim9 =jules52_struc(n)%dim_cslayer,                  &
                                   dim10=jules52_struc(n)%npft*jules52_struc(n)%dim_cslayer,  &
                                   dimID=dimID,                                        &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, nsnow_ID, "NSNOW",    &
                                 ...
                                 "-", vlevels=JULES52_struc(n)%nsurft ,           &
                                 valid_min=-99999.0, valid_max=99999.0,           &
                                 var_flag = "dim1")
    ! Number of snow layers on ground on tiles
    do l=1, JULES52_struc(n)%nsurft   
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = 1.0 * JULES52_struc(n)%jules52(t)%nsnow(l)
        enddo
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tsurf_elev_surft_ID, "TSURF_ELEV_SURFT",    &
                                 ...
                                 "K", vlevels=JULES52_struc(n)%nsurft ,           &
                                 valid_min=-99999.0, valid_max=99999.0,           &
                                 var_flag = "dim1")
    ! Number of snow layers on ground on tiles
    do l=1, JULES52_struc(n)%nsurft   
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%tsurf_elev_surft(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    !
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%z0msea, &
    ...
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%frac_past_prev, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, n_inorg_soilt_lyrs_ID, "N_INORG_SOILT_LYRS",    &
                                 ...
                                 "kg N/m2", vlevels=JULES52_struc(n)%dim_cslayer , &
                                 valid_min=-99999.0, valid_max=99999.0,            &
                                 var_flag = "dim9")
    !  Gridbox Inorganic N pool on soil levels (kg N/m2)
    do l=1, JULES52_struc(n)%dim_cslayer   
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = 1.0 * JULES52_struc(n)%jules52(t)%n_inorg_soilt_lyrs(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen,                 &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, n_inorg_avail_pft_ID, "N_INORG_AVAIL_PFT",       &
                                 ...
                                 vlevels=JULES52_struc(n)%npft*JULES52_struc(n)%dim_cslayer ,    &
                                 valid_min=-99999.0, valid_max=99999.0,                          &
                                 var_flag = "dim10")
    ...
    do l=1, JULES52_struc(n)%npft*JULES52_struc(n)%dim_cslayer
        ...
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%n_inorg_avail_pft(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%triffid_co2_gb, &
    ...
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%canopy_gb, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, snow_mass_ij_ID, "SNOW_MASS_IJ", &
                                ...
                                "(kg/m2)", vlevels=1, valid_min=-99999.0, valid_max=99999.0)
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%snow_mass_ij, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tsoil_deep_ID, "TSOIL_DEEP", &
                                 ...
                                 "K", vlevels=JULES52_struc(n)%ns_deep , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim5") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, canht_ft_ID, "CANHT_FT", &
                                 "Canopy height", &
                                 "m", vlevels=JULES52_struc(n)%npft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim2")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, canopy_ID, "CANOPY", &
                                 ...
                                 "kg m-2", vlevels=JULES52_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, ns_ID, "NS", &
                                 ...
                                 "kg m-2", vlevels=JULES52_struc(n)%dim_cs1 , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim6") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, gc_ID, "GC", &
                                 ...
                                 "m s-1", vlevels=JULES52_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, lai_ID, "LAI", &
                                 ...
                                 "-", vlevels=JULES52_struc(n)%npft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim2") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rgrain_ID, "RGRAIN", &
                                 ...
                                 "micron", vlevels=JULES52_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, smcl_ID, "SMCL", &
                                 ...
                                 "kg m-2", vlevels=JULES52_struc(n)%sm_levels , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim3") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, t_soil_ID, "T_SOIL", &
                                 ...
                                 "K", vlevels=JULES52_struc(n)%sm_levels , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim3")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tstar_tile_ID, "TSTAR_TILE", &
                                 ...
                                 "K", vlevels=JULES52_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, c_veg_ID, "C_VEG", &
                                 ...
                                 "kg m-2", vlevels=JULES52_struc(n)%npft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim2") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, sthu_ID, "STHU", &
                                 ...
                                 "-", vlevels=JULES52_struc(n)%sm_levels , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim3")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, sice_ID, "SICE", &
                                 ...
                                 !"kg m-2", vlevels=JULES52_struc(n)%nsurft*JULES52_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 "kg m-2", vlevels=jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim8")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, snowdepth_ID, "SNOWDEPTH", &
                                 ...
                                 "m", vlevels=JULES52_struc(n)%nsurft, valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tsnow_ID, "TSNOW", &
                                 ...
                                 !"K", vlevels=JULES52_struc(n)%nsurft*JULES52_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 "K", vlevels=jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim8") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rgrainl_ID, "RGRAINL", &
                                 ...
                                 !"microns", vlevels=JULES52_struc(n)%nsurft*JULES52_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 "microns", vlevels=jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim8")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rho_snow_grnd_ID, "RHO_SNOW_GRND", &
                                 ...
                                 "kg/m3", vlevels=JULES52_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim7")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rho_snow_ID, "RHO_SNOW", &
                                 ...
                                 "m", vlevels=JULES52_struc(n)%nsurft*JULES52_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim7")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, ds_ID, "DS",                                            &
                                 ...
                                 vlevels=jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers,      & 
                                 valid_min=-99999.0, valid_max=99999.0,                                 &
                                 var_flag = "dim8")
 
    ! close header of restart file
    call LIS_closeHeader_restart(ftn, n, LIS_rc%lsm_index, dimID, JULES52_struc(n)%rstInterval)
    ...
    do l=1, JULES52_struc(n)%ns_deep   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%tsoil_deep(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
                                  varid=tsoil_deep_ID, dim=l, wformat=wformat)
    enddo
    ! Fast-turnover wood product C pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%wood_prod_fast, &
                              varid=wood_prod_fast_ID, dim=1, wformat=wformat)

    ! Medium-turnover wood product C pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%wood_prod_med, &
                              varid=wood_prod_med_ID, dim=1, wformat=wformat)

    ! Slow-turnover wood product C pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%wood_prod_slow, &
                              varid=wood_prod_slow_ID, dim=1, wformat=wformat)

    ! Agricultural fraction from previous TRIFFID call
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%frac_agr_prev, &
                              varid=frac_agr_prev_ID, dim=1, wformat=wformat)

    ! Agricultural fraction
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%frac_agr, &
                              varid=frac_agr_ID, dim=1, wformat=wformat)

    ! Gridbox Inorganic N pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%n_inorg, &
    ...
    do l=1, JULES52_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%canht_ft(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%canopy(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Soil Organic Nitrogen
    do l=1, JULES52_struc(n)%dim_cs1*JULES52_struc(n)%dim_cslayer   ! TODO: check loop
        call l_to_rc(l, jules52_struc(n)%dim_cs1, r_idx, c_idx)
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%ns(r_idx,c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%dim_cs1*JULES52_struc(n)%dim_cslayer   ! TODO: check loop
        ...
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%cs_pool_soilt(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%gc_surft(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Stomatal conductance to evaporation
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%gs_gb, &
    ...
    do l=1, JULES52_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%lai(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%rgrain(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Soil moisture in a layer at the surface
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%smc_soilt, &
    ...
    do l=1, JULES52_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%smcl_soilt(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%snow_tile(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%snow_grnd(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Snow soot content (kg kg-1)
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%soot_ij, &
    ...
    do l=1, JULES52_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%t_soil(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%tstar_tile(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Number of atmospheric timesteps since last call to TRIFFID
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%asteps_since_triffid, &
    ...
    do l=1, JULES52_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%lai_phen(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%c_veg(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Gridbox mean vegetation carbon
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%cv, &
                              varid=cv_ID, dim=1, wformat=wformat)

    ! Decay factor in Sat. Conductivity in water table layer
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%fexp, &
                              varid=fexp_ID, dim=1, wformat=wformat)

    ! Mean topographic index
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%ti_mean, &
                              varid=ti_mean_ID, dim=1, wformat=wformat)

    ! Standard deviation of topographic index
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%ti_sig, &
                              varid=ti_sig_ID, dim=1, wformat=wformat)

    ! Water table depth
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%zw, &
                              varid=zw_ID, dim=1, wformat=wformat)
    
    ! Surface saturation fraction
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%fsat, &
                              varid=fsat_ID, dim=1, wformat=wformat)

    ! Wetland fraction 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%fwetl, &
                              varid=fwetl_ID, dim=1, wformat=wformat)

    ! soil moist fraction in deep (water table) layer
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES52_struc(n)%jules52%sthzw, &
    ...
    do l=1, JULES52_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%p_s_sthu(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%p_s_sthf(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers   ! TODO: check loop
        ...
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%sice(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers   ! TODO: check loop
        ...
        call l_to_rc(l, jules52_struc(n)%temp_tiles, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%sliq(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%snowdepth(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers   ! TODO: check loop
        ...
        call l_to_rc(l, jules52_struc(n)%temp_tiles, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%tsnow(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers   ! TODO: check loop
        ...
        call l_to_rc(l, jules52_struc(n)%temp_tiles, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%rgrainl(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%rho_snow_grnd(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES52_struc(n)%nsurft*jules52_struc(n)%nsmax   ! TODO: check loop
        ...
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%rho_snow(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules52_struc(n)%temp_tiles*jules52_struc(n)%temp_layers   ! TODO: check loop
        ...
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES52_struc(n)%jules52(t)%ds(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
                                  varid=ds_ID, dim=l, wformat=wformat)
    enddo
end subroutine jules52_dump_restart
