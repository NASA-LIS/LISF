...
MODULE clm2_shr_orb_mod

   use clm2_shr_kind_mod, only: CLM2_SHR_KIND_R8, CLM2_SHR_KIND_IN, CLM2_SHR_KIND_R4
   ...
   !----------------------------------------------------------------------------
   public clm2_shr_orb_cosz, clm2_shr_orb_params, clm2_shr_orb_decl, clm2_shr_orb_print
   real   (CLM2_SHR_KIND_R8),public, parameter :: SHR_ORB_UNDEF_REAL = 1.e36      ! undefined real
   integer(CLM2_SHR_KIND_IN),public, parameter :: SHR_ORB_UNDEF_INT  = 2000000000 ! undefined int
   !----------------------------------------------------------------------------
   ! PRIVATE: by default everything else is private to this module
   !----------------------------------------------------------------------------
   private
   real   (CLM2_SHR_KIND_R8),parameter :: pi = SHR_CONST_PI
   real   (CLM2_SHR_KIND_R8),parameter :: SHR_ORB_ECCEN_MIN  =   0.0 ! min value for eccen
   real   (CLM2_SHR_KIND_R8),parameter :: SHR_ORB_ECCEN_MAX  =   0.1 ! max value for eccen
   real   (CLM2_SHR_KIND_R8),parameter :: SHR_ORB_OBLIQ_MIN  = -90.0 ! min value for obliq
   real   (CLM2_SHR_KIND_R8),parameter :: SHR_ORB_OBLIQ_MAX  = +90.0 ! max value for obliq
   real   (CLM2_SHR_KIND_R8),parameter :: SHR_ORB_MVELP_MIN  =   0.0 ! min value for mvelp
   real   (CLM2_SHR_KIND_R8),parameter :: SHR_ORB_MVELP_MAX  = 360.0 ! max value for mvelp

CONTAINS

!===============================================================================

 real(CLM2_SHR_KIND_R8) FUNCTION clm2_shr_orb_cosz(jday,lat,lon,declin)
   ...
   real   (CLM2_SHR_KIND_R8),intent(in) :: jday   ! Julian cal day (1.xx to 365.xx)
   real   (CLM2_SHR_KIND_R8),intent(in) :: lat    ! Centered latitude (radians)
   real   (CLM2_SHR_KIND_R8),intent(in) :: lon    ! Centered longitude (radians)
   real   (CLM2_SHR_KIND_R8),intent(in) :: declin ! Solar declination (radians)

   !----------------------------------------------------------------------------
   clm2_shr_orb_cosz = sin(lat)*sin(declin) - &
   &              cos(lat)*cos(declin)*cos(jday*2.0*pi + lon)

END FUNCTION clm2_shr_orb_cosz

!===============================================================================

SUBROUTINE clm2_shr_orb_params( )
...
!-------------------------------------------------------------------------------
   ...
   !----------------------------- Arguments ------------------------------------
!   real   (CLM2_SHR_KIND_R8),intent(inout) :: eccen     ! orbital eccentricity
!   real   (CLM2_SHR_KIND_R8),intent(inout) :: obliq     ! obliquity in degrees
   real   (CLM2_SHR_KIND_R8) :: mvelp     ! moving vernal equinox long
!   integer(CLM2_SHR_KIND_IN),intent(in)    :: iyear_AD  ! Year to calculate orbit for
   logical                :: log_print ! Flags print of status/error
!   real   (CLM2_SHR_KIND_R8),intent(out)   :: obliqr    ! Earths obliquity in rad
!   real   (CLM2_SHR_KIND_R8),intent(out)   :: lambm0    ! Mean long of perihelion at
                                                   ! vernal equinox (radians)
!   real   (CLM2_SHR_KIND_R8),intent(out)   :: mvelpp    ! moving vernal equinox long
                                                   ! of perihelion plus pi (rad)

   !------------------------------ Parameters ----------------------------------
   integer(CLM2_SHR_KIND_IN),parameter :: poblen =47 ! # of elements in series wrt obliquity
   integer(CLM2_SHR_KIND_IN),parameter :: pecclen=19 ! # of elements in series wrt eccentricity
   integer(CLM2_SHR_KIND_IN),parameter :: pmvelen=78 ! # of elements in series wrt vernal equinox
   real   (CLM2_SHR_KIND_R8),parameter :: psecdeg = 1.0/3600.0 ! arc sec to deg conversion

   real   (CLM2_SHR_KIND_R8) :: degrad = pi/180.   ! degree to radian conversion factor
   real   (CLM2_SHR_KIND_R8) :: yb4_1950AD         ! number of years before 1950 AD
 
   ! Cosine series data for computation of obliquity: amplitude (arc seconds),
   ! rate (arc seconds/year), phase (degrees).
 
   real   (CLM2_SHR_KIND_R8), parameter :: obamp(poblen) =  & ! amplitudes for obliquity cos series
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: obrate(poblen) = & ! rates for obliquity cosine series
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: obphas(poblen) = & ! phases for obliquity cosine series
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: ecamp (pecclen) = & ! ampl for eccen/fvelp cos/sin series
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: ecrate(pecclen) = & ! rates for eccen/fvelp cos/sin series
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: ecphas(pecclen) = & ! phases for eccen/fvelp cos/sin series
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: mvamp (pmvelen) = & ! amplitudes for mvelp sine series 
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: mvrate(pmvelen) = & ! rates for mvelp sine series 
   &      (/    31.609974, 32.620504, 24.172203,   &
   ...
   real   (CLM2_SHR_KIND_R8), parameter :: mvphas(pmvelen) = & ! phases for mvelp sine series
   &      (/    251.9025, 280.8325, 128.3057,   &
   ...
   !---------------------------Local variables----------------------------------
   integer(CLM2_SHR_KIND_IN) :: i       ! Index for series summations
   real   (CLM2_SHR_KIND_R8) :: obsum   ! Obliquity series summation
   real   (CLM2_SHR_KIND_R8) :: cossum  ! Cos series summation for eccentricity/fvelp
   real   (CLM2_SHR_KIND_R8) :: sinsum  ! Sin series summation for eccentricity/fvelp
   real   (CLM2_SHR_KIND_R8) :: fvelp   ! Fixed vernal equinox long of perihelion
   real   (CLM2_SHR_KIND_R8) :: mvsum   ! mvelp series summation
   real   (CLM2_SHR_KIND_R8) :: beta    ! Intermediate argument for lambm0
   real   (CLM2_SHR_KIND_R8) :: years   ! Years to time of interest ( pos <=> future)
   real   (CLM2_SHR_KIND_R8) :: eccen2  ! eccentricity squared
   real   (CLM2_SHR_KIND_R8) :: eccen3  ! eccentricity cubed

   !-------------------------- Formats -----------------------------------------
   character(len=*),parameter :: F00 = "('(clm2_shr_orb_params) ',4a)"
   character(len=*),parameter :: F01 = "('(clm2_shr_orb_params) ',a,i9)"
   character(len=*),parameter :: F02 = "('(clm2_shr_orb_params) ',a,f6.3)"
   character(len=*),parameter :: F03 = "('(clm2_shr_orb_params) ',a,es14.6)"
   ...
   ! radinp and algorithms below will need a degree to radian conversion factor
    if(LIS_masterproc) then
         ...
         doalb = .true.
   if ( log_print ) then
     write(LIS_logunit,F00) 'Calculate characteristics of the orbit:'
     write(LIS_logunit,F00) 'CVS revision: $Revision: 1.7 $'
     write(LIS_logunit,F00) 'CVS Tag     : $Name:  $'
   end if
   ...
   IF ( iyear_AD == SHR_ORB_UNDEF_INT ) THEN
      ...
      if( obliq == SHR_ORB_UNDEF_REAL )then
        if ( log_print ) then
          write(LIS_logunit,F00) 'Have to specify orbital parameters:'
          write(LIS_logunit,F00) 'Either set: iyear_AD, OR [obliq, eccen, and mvelp]:'
          write(LIS_logunit,F00) 'iyear_AD is the year to simulate orbit for (ie. 1950): '
          write(LIS_logunit,F00) 'obliq, eccen, mvelp specify the orbit directly:'
          write(LIS_logunit,F00) 'The AMIP II settings (for a 1995 orbit) are: '
          write(LIS_logunit,F00) ' obliq =  23.4441'
          write(LIS_logunit,F00) ' eccen =   0.016715'
          write(LIS_logunit,F00) ' mvelp = 102.7'
        end if
        call clm2_shr_sys_abort()
      else if ( log_print ) then
         write(LIS_logunit,F00) 'Use input orbital parameters: '
      end if
      if( (obliq < SHR_ORB_OBLIQ_MIN).or.(obliq > SHR_ORB_OBLIQ_MAX) ) then
         if ( log_print ) then
            write(LIS_logunit,F03) 'Input obliquity unreasonable: ', obliq
         end if
         call clm2_shr_sys_abort()
      end if
      if( (eccen < SHR_ORB_ECCEN_MIN).or.(eccen > SHR_ORB_ECCEN_MAX) ) then
         if ( log_print ) then
            write(LIS_logunit,F03) 'Input eccentricity unreasonable: ', eccen
         end if
         call clm2_shr_sys_abort()
      end if
      if( (mvelp < SHR_ORB_MVELP_MIN).or.(mvelp > SHR_ORB_MVELP_MAX) ) then
         if ( log_print ) then
            write(LIS_logunit,F03) 'Input mvelp unreasonable: ' , mvelp
         end if
         call clm2_shr_sys_abort()
      end if
      eccen2 = eccen*eccen
   ...
   ELSE  ! Otherwise calculate based on years before present
      ...
      if ( log_print ) then
         write(LIS_logunit,F01) 'Calculate orbit for year: ' , iyear_AD
      end if
      yb4_1950AD = 1950.0 - float(iyear_AD)
      if ( abs(yb4_1950AD) .gt. 1000000.0 )then
         if ( log_print ) then
            write(LIS_logunit,F00) 'orbit only valid for years+-1000000'
            write(LIS_logunit,F00) 'Relative to 1950 AD'
            write(LIS_logunit,F03) '# of years before 1950: ',yb4_1950AD
            write(LIS_logunit,F01) 'Year to simulate was  : ',iyear_AD
         end if
         call clm2_shr_sys_abort()
      end if
   ...
   if ( log_print ) then
     write(LIS_logunit,F03) '------ Computed Orbital Parameters ------'
     write(LIS_logunit,F03) 'Eccentricity      = ',eccen
     write(LIS_logunit,F03) 'Obliquity (deg)   = ',obliq
     write(LIS_logunit,F03) 'Obliquity (rad)   = ',obliqr
     write(LIS_logunit,F03) 'Long of perh(deg) = ',mvelp
     write(LIS_logunit,F03) 'Long of perh(rad) = ',mvelpp
     write(LIS_logunit,F03) 'Long at v.e.(rad) = ',lambm0
     write(LIS_logunit,F03) '-----------------------------------------'
   end if
 
END SUBROUTINE clm2_shr_orb_params

!===============================================================================

SUBROUTINE clm2_shr_orb_decl(calday ,eccen ,mvelpp ,lambm0 ,obliqr ,delta ,eccf)
...
!-------------------------------------------------------------------------------
   ...
   !------------------------------Arguments--------------------------------
   real   (CLM2_SHR_KIND_R4),intent(in)  :: calday ! Calendar day, including fraction
   real   (CLM2_SHR_KIND_R4),intent(in)  :: eccen  ! Eccentricity
   real   (CLM2_SHR_KIND_R4),intent(in)  :: obliqr ! Earths obliquity in radians
   real   (CLM2_SHR_KIND_R4),intent(in)  :: lambm0 ! Mean long of perihelion at the 
                                              ! vernal equinox (radians)
   real   (CLM2_SHR_KIND_R4),intent(in)  :: mvelpp ! moving vernal equinox longitude
                                              ! of perihelion plus pi (radians)
   real   (CLM2_SHR_KIND_R4),intent(out) :: delta  ! Solar declination angle in rad
   real   (CLM2_SHR_KIND_R4),intent(out) :: eccf   ! Earth-sun distance factor (ie. (1/r)**2)
 
   !---------------------------Local variables-----------------------------
   real   (CLM2_SHR_KIND_R4),parameter :: dayspy = 365.0  ! days per year
   real   (CLM2_SHR_KIND_R4),parameter :: ve     = 80.5   ! Calday of vernal equinox
                                                     ! assumes Jan 1 = calday 1
 
   real   (CLM2_SHR_KIND_R4) ::   lambm  ! Lambda m, mean long of perihelion (rad)
   real   (CLM2_SHR_KIND_R4) ::   lmm    ! Intermediate argument involving lambm
   real   (CLM2_SHR_KIND_R4) ::   lamb   ! Lambda, the earths long of perihelion
   real   (CLM2_SHR_KIND_R4) ::   invrho ! Inverse normalized sun/earth distance
   real   (CLM2_SHR_KIND_R4) ::   sinl   ! Sine of lmm
...
END SUBROUTINE clm2_shr_orb_decl

!===============================================================================

SUBROUTINE clm2_shr_orb_print( iyear_AD, eccen, obliq, mvelp )
  use LIS_logMod, only : LIS_logunit
!-------------------------------------------------------------------------------
!
...
!-------------------------------------------------------------------------------
   ...
   !---------------------------Arguments----------------------------------------
   integer(CLM2_SHR_KIND_IN),intent(in) :: iyear_AD ! requested Year (AD)
   real   (CLM2_SHR_KIND_R8),intent(in) :: eccen    ! eccentricity (unitless) 
                                               ! (typically 0 to 0.1)
   real   (CLM2_SHR_KIND_R8),intent(in) :: obliq    ! obliquity (-90 to +90 degrees) 
                                               ! typically 22-26
   real   (CLM2_SHR_KIND_R8),intent(in) :: mvelp    ! moving vernal equinox at perhel
                                               ! (0 to 360 degrees)
   !-------------------------- Formats -----------------------------------------
   character(len=*),parameter :: F00 = "('(clm2_shr_orb_print) ',4a)"
   character(len=*),parameter :: F01 = "('(clm2_shr_orb_print) ',a,i9.4)"
   character(len=*),parameter :: F02 = "('(clm2_shr_orb_print) ',a,f6.3)"
   character(len=*),parameter :: F03 = "('(clm2_shr_orb_print) ',a,es14.6)"
   !----------------------------------------------------------------------------
 
   if ( iyear_AD .ne. SHR_ORB_UNDEF_INT ) then
     if ( iyear_AD > 0 ) then
       write(LIS_logunit,F01) 'Orbital parameters calculated for year: AD ',iyear_AD
     else
       write(LIS_logunit,F01) 'Orbital parameters calculated for year: BC ',iyear_AD
     end if
   else if ( obliq /= SHR_ORB_UNDEF_REAL ) then
     write(LIS_logunit,F03) 'Orbital parameters: '
     write(LIS_logunit,F03) 'Obliquity (degree):              ', obliq
     write(LIS_logunit,F03) 'Eccentricity (unitless):         ', eccen
     write(LIS_logunit,F03) 'Long. of moving Perhelion (deg): ', mvelp
   else
     write(LIS_logunit,F03) 'Orbit parameters not set!'
   end if
 
END SUBROUTINE clm2_shr_orb_print
!===============================================================================

END MODULE clm2_shr_orb_mod
