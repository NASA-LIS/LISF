...
!
! !INTERFACE:
subroutine clm2_getlst(n, LSM_State)
...
! !DESCRIPTION:
!
!  Returns the land surface temperature related state prognostic variables for
!  data assimilation
! 
!  The arguments are: 
!  \begin{description}
...
!EOP
  ...
  do t=1,LIS_rc%ntiles(n)
     lst1(t) = clm2_struc(n)%clm(t)%t_veg
     tground(t) = clm2_struc(n)%clm(t)%t_grnd
     tsoil1(t) = clm2_struc(n)%clm(t)%t_soisno(1)
     trad(t) = clm2_struc(n)%clm(t)%t_rad
  enddo

end subroutine clm2_getlst
