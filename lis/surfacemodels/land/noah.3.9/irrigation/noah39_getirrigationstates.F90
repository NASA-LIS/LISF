...
!EOP
  ...
  TILE_LOOP: do TileNo = 1,LIS_rc%npatch(nest,LIS_rc%lsm_index)
     ...
     if((noah39_struc(nest)%noah(TileNo)%smc(1) - &
          noah39_struc(nest)%noah(TileNo)%sh2o(1)).gt.0.0001) then 
        irrig_check_frozen_soil = .true. 
     elseif((noah39_struc(nest)%noah(TileNo)%smc(2) - &
          noah39_struc(nest)%noah(TileNo)%sh2o(2)).gt.0.0001) then 
        irrig_check_frozen_soil = .true. 
     elseif((noah39_struc(nest)%noah(TileNo)%smc(3) - &
          noah39_struc(nest)%noah(TileNo)%sh2o(3)).gt.0.0001) then 
        irrig_check_frozen_soil = .true. 
     elseif((noah39_struc(nest)%noah(TileNo)%smc(4) - &
          noah39_struc(nest)%noah(TileNo)%sh2o(4)).gt.0.0001) then 
        irrig_check_frozen_soil = .true. 
     elseif(noah39_struc(nest)%noah(TileNo)%stc(2).le.LIS_CONST_TKFRZ) then
        irrig_check_frozen_soil = .true. 
     elseif(noah39_struc(nest)%noah(TileNo)%stc(3).le.LIS_CONST_TKFRZ) then
        irrig_check_frozen_soil = .true. 
     elseif(noah39_struc(nest)%noah(TileNo)%stc(4).le.LIS_CONST_TKFRZ) then
        irrig_check_frozen_soil = .true. 
     endif
                ...
                FLOOD: if ( IM%irrigType(TileNo) == 3 ) then
                   ...
                   ! PADDY-- surface layer only
                     noah39_struc(nest)%noah(TileNo)%smc(1) =  &
                           ...
                           (1-IM%IrrigScale(TileNo))*noah39_struc(nest)%noah(TileNo)%smc(1)
                     IM%irrigAppRate(TileNo) = (noah39_struc(nest)%noah(TileNo)%smcmax-noah39_struc(nest)%noah(TileNo)%smc(1))*sldpth(1)*1000./LIS_rc%ts
                   else
