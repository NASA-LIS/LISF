...
! Code modified by S. Wang and Z. Wang for adding Noah 3.9.1 into LIS
! 
#include "LIS_misc.h"

MODULE module_sf_noah39lsm
  USE module_model_constants_39, only : CP, R_D, XLF, XLV, RHOWATER, STBOLT, KARMAN
...
!
      ...
      SUBROUTINE SFLX (IILOC,JJLOC,FFROZP,ISURBAN,DT,ZLVL,NSOIL,SLDPTH, &    !C
                       ...
                       TH2,Q2SAT,DQSDT2,                                &    !I
! SLOPE is passed to SFLX instead of SLOPETYP - D. Mocko,SW, ZW 
...
! 1. CONFIGURATION INFORMATION (C):
! ----------------------------------------------------------------------
      INTEGER, INTENT(IN) :: NSOIL,SOILTYP,VEGTYP !SW, ZW 
...
! ----------------------------------------------------------------------
      ...
      ! Added by Zhuo Wang on 04/14/2019
...
#endif
!svk - moved from redprm, SW, ZW 08/24/2018 
         DO I = 1,NROOT
...
! ----------------------------------------------------------------------
         ...
         ELSE
            RC = 0.0
            PC = 1.0 ! added by D. Mocko on 04/16/2019
         END IF
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
         ...
         IF (SNEQV  == 0.0) THEN
            CALL NOPAC (ETP,ETA,PRCP,SMC,SMCMAX,SMCWLT,                  &
                            ...
                            SSOIL,TSOIL,                                 & ! Modified by Z Wang
...
END MODULE module_sf_noah39lsm
