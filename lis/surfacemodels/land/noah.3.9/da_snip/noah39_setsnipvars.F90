...
!BOP
! !ROUTINE: noah39_setsnipvars
! \label{noah39_setsnipvars}
!
! !REVISION HISTORY:
!  17 Jul 2025: Eric Kemp; Initial specification (copied from USAFSI version)
!
! !INTERFACE:
subroutine noah39_setsnipvars(n, LSM_State)
  ...
  do t = 1, LIS_rc%npatch(n, LIS_rc%lsm_index)
     if (noah39_struc(n)%noah(t)%t1 >= 273.15+2.0) then
  ...
  if (LIS_rc%snowsrc(n) .gt. 0) then
     ...
     do t = 1, LIS_rc%npatch(n, LIS_rc%lsm_index)
        ...
        LIS_snow_struc(n)%sneqv(tid) = &
             LIS_snow_struc(n)%sneqv(tid) + swe(t)
     end do
     npts = 0
     do t = 1, LIS_rc%npatch(n, LIS_rc%lsm_index)
        !transform t to the patch
...
end subroutine noah39_setsnipvars
