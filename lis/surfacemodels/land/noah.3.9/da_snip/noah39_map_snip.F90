...
!BOP
! !ROUTINE: noah39_map_snip
! \label{noah39_map_snip}
!
! !REVISION HISTORY:
!  17 Jul 2025: Eric Kemp; Initial specification (copied from USAFSI version)
!
! !INTERFACE:
subroutine noah39_map_snip(n,k,OBS_State,LSM_Incr_State)
  ! !USES:
  use ESMF
  ...
  call ESMF_FieldGet(snodincrField, localDE=0, farrayPtr=snodincr, &
       rc=status)
  call LIS_verify(status)
  ...
  call ESMF_StateGet(OBS_State, obs_state_objs(1), obs_snip_field, &
       rc=status)
  call LIS_verify(status)
  call ESMF_FieldGet(obs_snip_field, localDE=0, farrayPtr=snipobs, &
       rc=status)
  call LIS_verify(status)
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if (snipobs(st_id).ge.0) then
        ...
        snod(t) = snipobs(st_id)

        ! Based on SNIP, we manually update SWE
...
end subroutine noah39_map_snip
