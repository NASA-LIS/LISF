...
!BOP
!
! !ROUTINE: noah39_setsoils
!  \label{noah39_setsoils}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah39_setsoils(mtype)
! !USES:
...
!
! !DESCRIPTION:
!  This subroutine sets the soil parameters in Noah-3.9.  Noah-3.9 uses a
...
!  real, allocatable     :: sbeta(:)
  ...
  do n=1,LIS_rc%nnest
     ...
     write(unit=LIS_logunit,fmt=*)                                     &
                            '[INFO] noah39_setsoils -- reading soil files'

     if(LIS_rc%usetexturemap(n).eq."none") then
        do i=1,LIS_rc%npatch(n,mtype)
           call LIS_mapSoilType(noah39_struc(n)%soilscheme,&
     ...
     if (noah39_struc(n)%fixedsoiltype.ne.0) then
        write(LIS_logunit,*) '[INFO] Fixing Noah-3.9 soil index to type: ',   &
                              noah39_struc(n)%fixedsoiltype
     endif
     do i=1,LIS_rc%npatch(n,mtype)
        if (noah39_struc(n)%fixedsoiltype.eq.0) then
           noah39_struc(n)%noah(i)%soiltype = &
                LIS_surface(n,mtype)%tile(i)%soilt
        else
           noah39_struc(n)%noah(i)%soiltype = noah39_struc(n)%fixedsoiltype
        endif
        !     if(noah39_struc(n)%noah(i)%soiltype.eq.-9999) noah39_struc(n)%noah(i)%soiltype = 14     
        if (noah39_struc(n)%noah(i)%soiltype .eq. -9999.or.&
             noah39_struc(n)%noah(i)%soiltype.eq.0) then
           if ( noah39_struc(n)%soilscheme == 2 ) then ! statsgo
              noah39_struc(n)%noah(i)%soiltype = 12 ! clay
           else ! zobler
              noah39_struc(n)%noah(i)%soiltype = 6 ! clay loam
           endif
        endif
        if ( noah39_struc(n)%noah(i)%soiltype .eq. 14 .and. &
             noah39_struc(n)%noah(i)%vegt .ne. LIS_rc%waterclass ) then
           noah39_struc(n)%noah(i)%soiltype = 7
        endif
     enddo
!Hardcoded for STATSGO classes only for soil types found to be water
! at a land point

     if (noah39_struc(n)%fixedslopetype.ne.0) then
        write(LIS_logunit,*) '[INFO] Fixing Noah-3.9 slope index to type: ',  &
                              noah39_struc(n)%fixedslopetype
     else
        write(LIS_logunit,*) '[INFO] Reading Noah-3.9 slope index from maps'
     ...
     if(noah39_struc(n)%usedsoilmap.ne.0) then 
        ...
        do t=1,LIS_rc%npatch(n,mtype)
           allocate(noah39_struc(n)%noah(t)%lyrthk(noah39_struc(n)%nslay))
           zsoil = 0.0
           c = LIS_surface(n,mtype)%tile(t)%col
           r = LIS_surface(n,mtype)%tile(t)%row

           do k=1,noah39_struc(n)%nslay
              if(k.ne.noah39_struc(n)%nslay) then 
                 noah39_struc(n)%noah(t)%lyrthk(k) = noah39_struc(n)%lyrthk(k)
                 zsoil = zsoil +  noah39_struc(n)%lyrthk(k)
              else
                 if((dsoil(c,r)-zsoil).gt.0) then
                    noah39_struc(n)%noah(t)%lyrthk(k) = &
                         dsoil(c,r) - zsoil
                 else
     ...
     if(noah39_struc(n)%usedrootmap.ne.0) then 
        ...
        do t=1,LIS_rc%npatch(n,mtype)
           ...
           zsoil = 0.0
           noah39_struc(n)%noah(t)%nroot = 1
           do k=1,noah39_struc(n)%nslay
              zsoil = zsoil + noah39_struc(n)%noah(t)%lyrthk(k)
...
!-----------------------------------------------------------------------
! Read in the Noah-3.9 Soil Parameter File
!-----------------------------------------------------------------------
     ftn = LIS_getNextUnitNumber()       
     write(LIS_logunit,*) '[INFO] Reading Noah-3.9 soil parameter file: ',    &
                                             trim(noah39_struc(n)%sfile)
     open(unit=ftn,file=noah39_struc(n)%sfile,status='old',            &
                                                    access='sequential')
     read(ftn,*) 
!     read(ftn,*) (noah39_struc(n)%lyrthk(i), i=1,noah39_struc(n)%nslay)
!     read(ftn,*)
...
!-----------------------------------------------------------------------
     do i = 1,LIS_rc%npatch(n,mtype)
        noah39_struc(n)%noah(i)%smcwlt = &
             wltsmc(noah39_struc(n)%noah(i)%soiltype)
        noah39_struc(n)%noah(i)%smcdry = &
             drysmc(noah39_struc(n)%noah(i)%soiltype)
        noah39_struc(n)%noah(i)%smcref = &
             refsmc(noah39_struc(n)%noah(i)%soiltype)
        noah39_struc(n)%noah(i)%dwsat = &
             satdw(noah39_struc(n)%noah(i)%soiltype)
     end do     
! Set tile soil porosity
     if (LIS_rc%useporositymap(n).eq."none") then ! default, from look-up table
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%smcmax = &
                maxsmc(noah39_struc(n)%noah(i)%soiltype)
        end do
     else
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%smcmax = &
...
! Set tile soil Psi-sat
     if (LIS_rc%usepsisatmap(n).eq."none") then ! default, from look-up table
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%psisat = &
                satpsi(noah39_struc(n)%noah(i)%soiltype)
        end do
     else
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%psisat = &
...
! Set tile soil K-sat
     if (LIS_rc%useksatmap(n).eq."none") then ! default, from look-up table
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%dksat = &
                satdk(noah39_struc(n)%noah(i)%soiltype)
        end do
     else
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%dksat = &
...
! Set tile soil b-parameter
     if (LIS_rc%usebexpmap(n).eq."none") then ! default, from look-up table
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%bexp = &
                bb(noah39_struc(n)%noah(i)%soiltype)
        end do
     else
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%bexp = &
...
! Set tile soil Qz content
     if (LIS_rc%usequartzmap(n).eq."none") then ! default
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%quartz = &
                qtz(noah39_struc(n)%noah(i)%soiltype)
        end do
     else 
        do i = 1,LIS_rc%npatch(n,mtype)
           noah39_struc(n)%noah(i)%quartz = &
     ...
     do i = 1,LIS_rc%npatch(n,mtype)
        noah39_struc(n)%noah(i)%f1 = &
             f11(noah39_struc(n)%noah(i)%soiltype)
!        noah39_struc(n)%noah(i)%fxexp = &
!             fxexp(noah39_struc(n)%noah(i)%soiltype)
!        noah39_struc(n)%noah(i)%sbeta = &
!             sbeta(noah39_struc(n)%noah(i)%soiltype)
     enddo
     deallocate(bb)
     deallocate(drysmc)
     deallocate(f11)
...
! These are parameters from the GENPARM.TBL
     write(LIS_logunit,*) '[INFO] Reading Noah-3.9 general parameter file: ',  &
                           trim(noah39_struc(n)%gfile)
     open(unit=ftn,file=noah39_struc(n)%gfile,status='old',             &
                       access='sequential')
     read(ftn,*)
     read(ftn,*)
     read(ftn,*) NUM_SLOPE
     ...
     do i = 1,LIS_rc%npatch(n,mtype)
        if (LIS_rc%useslopemap(n).eq."none") then
           noah39_struc(n)%noah(i)%slope =                             &
                slope_data(noah39_struc(n)%noah(i)%slopetype)
        else
           gid = LIS_surface(n,mtype)%tile(i)%tile_id
           noah39_struc(n)%noah(i)%slope = LIS_domain(n)%tile(gid)%slope
        endif
        noah39_struc(n)%noah(i)%sbeta = SBETA_DATA
        noah39_struc(n)%noah(i)%fxexp = FXEXP_DATA
        noah39_struc(n)%noah(i)%csoil = CSOIL_DATA
        noah39_struc(n)%noah(i)%salp = SALP_DATA
        noah39_struc(n)%noah(i)%refdk = REFDK_DATA
        noah39_struc(n)%noah(i)%refkdt = REFKDT_DATA
        noah39_struc(n)%noah(i)%frzk = FRZK_DATA
        noah39_struc(n)%noah(i)%zbot = ZBOT_DATA
        noah39_struc(n)%noah(i)%czil = CZIL_DATA
        noah39_struc(n)%noah(i)%lvcoef = LVCOEF_DATA
     enddo

! overwrite default values with PTF 
     if(noah39_struc(n)%useptf.eq.1) then !use cosby et al.(1984) PTFs
        do i = 1,LIS_rc%npatch(n,mtype)
           
           noah39_struc(n)%noah(i)%smcmax = 0.489 - &
                0.00126*LIS_surface(n,mtype)%tile(i)%sand*100.0

           noah39_struc(n)%noah(i)%psisat = (10.0*(10.0**(1.88-0.0131*&
                LIS_surface(n,mtype)%tile(i)%sand*100.0)))/1000.0

           noah39_struc(n)%noah(i)%dksat =  (0.0070556*(10.0**(-0.884+0.0153*&
                LIS_surface(n,mtype)%tile(i)%sand*100.0)))/1000.0

           noah39_struc(n)%noah(i)%bexp = 2.91 + (0.159*&
...
end subroutine noah39_setsoils
