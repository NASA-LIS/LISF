...
!BOP
! !ROUTINE: noah39_qc_soilmobs
! \label{noah39_qc_soilmobs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah39_qc_soilmobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     smc1(t) = noah39_struc(n)%noah(t)%smc(1)
     smc2(t) = noah39_struc(n)%noah(t)%smc(2)
     smc3(t) = noah39_struc(n)%noah(t)%smc(3)
     smc4(t) = noah39_struc(n)%noah(t)%smc(4)

     sh2o1(t) = noah39_struc(n)%noah(t)%sh2o(1)
     sh2o2(t) = noah39_struc(n)%noah(t)%sh2o(2)
     sh2o3(t) = noah39_struc(n)%noah(t)%sh2o(3)
     sh2o4(t) = noah39_struc(n)%noah(t)%sh2o(4)

     stc1(t) = noah39_struc(n)%noah(t)%stc(1)
     stc2(t) = noah39_struc(n)%noah(t)%stc(2)
     stc3(t) = noah39_struc(n)%noah(t)%stc(3)
     stc4(t) = noah39_struc(n)%noah(t)%stc(4)

     vegt(t) = noah39_struc(n)%noah(t)%vegt
  enddo


  call LIS_convertPatchSpaceToObsSpace(n,k,&       
       LIS_rc%lsm_index, &
       noah39_struc(n)%noah(:)%rainf,&
       rainf_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       noah39_struc(n)%noah(:)%sneqv,&
       sneqv_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       noah39_struc(n)%noah(:)%sca,&
       sca_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       noah39_struc(n)%noah(:)%shdfac,&
       shdfac_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       noah39_struc(n)%noah(:)%t1,&
       t1_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       noah39_struc(n)%noah(:)%smcmax,&
       smcmax_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       noah39_struc(n)%noah(:)%smcwlt,&
       smcwlt_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       smc1,&
...
end subroutine noah39_qc_soilmobs
