...
!BOP
! !ROUTINE: noah39_getLbandTbPred
! \label{noah39_getLbandTbPred}
!
! !REVISION HISTORY:
! 21Oct2018: Mahdi Navari; Sujay Kumar ; Initial Specification
!
! !INTERFACE:
subroutine noah39_getLbandTbPred(n, obs_pred)
! !USES:
  use ESMF
...
!call the forward model
  ...
  call LIS_verify(status, &
       "Error in StateGet in noah39_getLbandTbPred for Lband_Hpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbH, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in noah39_getLbandTbPred for Lband_Hpol')

  call ESMF_StateGet(LIS_forwardState(n), "Lband_Vpol", varField, rc=status)
  call LIS_verify(status, &
       "Error in StateGet in noah39_getLbandTbPred for Lband_Vpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbV, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in noah39_getLbandTbPred for Lband_Vpol')
...
end subroutine noah39_getLbandTbPred
