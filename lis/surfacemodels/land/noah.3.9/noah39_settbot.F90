...
!BOP
!
! !ROUTINE: noah39_settbot
! \label{noah39_settbot}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah39_settbot(mtype)
! !USES:
...
!
! !DESCRIPTION:
!  This subroutine retrieves bottom boundary temperature for Noah-3.9 LSM.
...
!-----------------------------------------------------------------------
     ...
     if (noah39_struc(n)%fixedtbot.ne.0.0) then
        write(LIS_logunit,*) '[INFO] Fixing Noah-3.9 deep soil temperature: ',&
                              noah39_struc(n)%fixedtbot
     else
     ...
     do i = 1, LIS_rc%npatch(n,mtype)
        if (noah39_struc(n)%fixedtbot.ne.0.0) then
           noah39_struc(n)%noah(i)%tempbot = noah39_struc(n)%fixedtbot
        else
           if (placetbot1(LIS_surface(n,mtype)%tile(i)%col,                   &
                          LIS_surface(n,mtype)%tile(i)%row).ne.-9999.00) then
              noah39_struc(n)%noah(i)%tempbot =                        &
                            placetbot1(LIS_surface(n,mtype)%tile(i)%col,      &
                                       LIS_surface(n,mtype)%tile(i)%row)
           else
              noah39_struc(n)%noah(i)%tempbot = 285.0
              write(LIS_logunit,*) '[INFO] Noah-3.9 bottom temperature -- ',  &
        ...
        call LIS_initTmnUpdateTile(n,i,noah39_struc(n)%noah(i)%tempbot)

     enddo
     deallocate(placetbot1)
  enddo

end subroutine noah39_settbot
