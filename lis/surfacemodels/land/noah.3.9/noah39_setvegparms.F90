...
!BOP
!
! !ROUTINE: noah39_setvegparms
! \label{noah39_setvegparms}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah39_setvegparms(mtype)
! !USES:
...
! !DESCRIPTION:
!  This subroutine retrieves Noah-3.9 vegetation parameters.
...
! Set the section of the vegetation parameter table to read for Noah-3.9
...
!-----------------------------------------------------------------------
! Set Noah-3.9 vegetation type at tile from the LIS domain
!-----------------------------------------------------------------------
     if (noah39_struc(n)%fixedvegtype.ne.0) then
        write(LIS_logunit,*) '[INFO] Fixing Noah-3.9 vegetation to type: ',   &
                             noah39_struc(n)%fixedvegtype
     endif
     do t=1,LIS_rc%npatch(n,mtype)
        if (noah39_struc(n)%fixedvegtype.eq.0) then
           noah39_struc(n)%noah(t)%vegt = LIS_surface(n,mtype)%tile(t)%vegt
        else
           noah39_struc(n)%noah(t)%vegt = noah39_struc(n)%fixedvegtype
        endif
     enddo
     
!-----------------------------------------------------------------------
! Get Vegetation Parameters for Noah-3.9 Model in Tile Space
! Read in the Noah-3.9 Static Vegetation Parameter Files
!-----------------------------------------------------------------------
     write(LIS_logunit,*) '[INFO] Reading Noah-3.9 vegetation parameter file: ',&
          trim(noah39_struc(n)%vfile)
     ftn = LIS_getNextUnitNumber()
     open(unit=ftn,file=noah39_struc(n)%vfile,status='old')
     lumatch = 0
     do while (lumatch.eq.0)
        read(ftn,*,end=2002) lutype
        if (lutype.eq.mminlu) then
           read(ftn,*) noah39_struc(n)%lucats,iindex
           write(LIS_logunit,*) '[INFO] Noah-3.9 Landuse type ',mminlu,       &
                                ' - found ',noah39_struc(n)%lucats,    &
                                ' categories'
           lumatch = 1
        endif
     enddo
 ...
 2002   continue
     ...
     allocate(noah39_struc(n)%emissmin(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%emissmax(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%laimin(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%laimax(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%albmin(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%albmax(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%z0min(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%z0max(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%shdtbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%nrotbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%rstbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%rgltbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%hstbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%snuptbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%maxalb(noah39_struc(n)%lucats))
!     allocate(noah39_struc(n)%cziltbl(noah39_struc(n)%lucats))
!     allocate(noah39_struc(n)%cmcmaxtbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%ztopvtbl(noah39_struc(n)%lucats))
     allocate(noah39_struc(n)%zbotvtbl(noah39_struc(n)%lucats))

     do j=1,NOAH39_STRUC(N)%LUCATS
        read(ftn,*) IINDEX, noah39_struc(n)%shdtbl(j), &
             noah39_struc(n)%nrotbl(j), noah39_struc(n)%rstbl(j), &
             noah39_struc(n)%rgltbl(j), &
             noah39_struc(n)%hstbl(j), noah39_struc(n)%snuptbl(j),    &
             noah39_struc(n)%maxalb(j), &
             noah39_struc(n)%laimin(j),  noah39_struc(n)%laimax(j),   &
             noah39_struc(n)%emissmin(j),noah39_struc(n)%emissmax(j), &
             noah39_struc(n)%albmin(j),  noah39_struc(n)%albmax(j),   &
             noah39_struc(n)%z0min(j),   noah39_struc(n)%z0max(j),    &
!             noah39_struc(n)%cziltbl(j), noah39_struc(n)%cmcmaxtbl(j)
!             noah39_struc(n)%cziltbl(j), noah39_struc(n)%cmcmaxtbl(j),&
             noah39_struc(n)%ztopvtbl(j),noah39_struc(n)%zbotvtbl(j)
...
! This is for UCM 
     ...
     if (LIS_rc%usegreennessmap(n).eq."none") then
        write(LIS_logunit,*) '[INFO] Reading Noah-3.9 greenness fraction ',   &
        ...
        do t=1,LIS_rc%npatch(n,mtype)
           if (placeshdmax(LIS_surface(n,mtype)%tile(t)%col,                  &
              ...
              noah39_struc(n)%noah(t)%shdmax = 1.0
              write(LIS_logunit,*) '[WARN] Noah-3.9 maximum greenness -- ',   &
                                   'not defined for point ',t
              write(LIS_logunit,*) '[INFO] Noah-3.9 maximum greenness -- ',   &
                                   'set to default value of 1.0'
           endif
           if (placeshdmin(LIS_surface(n,mtype)%tile(t)%col,                  &
                           LIS_surface(n,mtype)%tile(t)%row).ne.-9999.00) then
              noah39_struc(n)%noah(t)%shdmin =                         &
                            placeshdmin(LIS_surface(n,mtype)%tile(t)%col,     &
                                        LIS_surface(n,mtype)%tile(t)%row)
           else
              noah39_struc(n)%noah(t)%shdmin = 0.0
              write(LIS_logunit,*) '[WARN] Noah-3.9 minimum greenness -- ',   &
                                   'not defined for point ',t
              write(LIS_logunit,*) '[WARN] Noah-3.9 minimum greenness -- ',   &
     ...
     if (LIS_rc%usealbedomap(n).eq."none") then
        write(LIS_logunit,*) '[INFO] Calculating Noah-3.9 albedo from ',      &
                             'greenness and albedo min/max in table'
     else
        write(LIS_logunit,*) '[INFO] Reading Noah-3.9 albedo from ',          &
                             'albedo maps'
     endif
     if (LIS_rc%uselaimap(n).eq."none") then
        write(LIS_logunit,*) '[INFO] Calculating Noah-3.9 LAI from ',         &
                             'greenness and LAI min/max in table'
     else
        write(LIS_logunit,*) '[INFO] Reading Noah-3.9 LAI from ',             &
                             'LAI maps'
     endif

     do t=1,LIS_rc%npatch(n,mtype)
        if (LIS_rc%startcode.eq."coldstart") then
           noah39_struc(n)%noah(t)%albedo = month_d(noah39_struc(n)%albedo_monthly, &
                                                   LIS_rc%yr,LIS_rc%mo,LIS_rc%da)
           if(LIS_rc%useroughnessmap(n).ne."none") then 
              tid = LIS_surface(n,mtype)%tile(t)%tile_id
              noah39_struc(n)%noah(t)%z0 = LIS_roughness(n)%roughness(tid)
           else
              noah39_struc(n)%noah(t)%z0 = &
                   month_d(noah39_struc(n)%z0brd_monthly,      &
                   LIS_rc%yr,LIS_rc%mo,LIS_rc%da)
           endif
...
! local time instead of on UTC time. - dmm
           if (Bondvillecheck) then
              change = 0
              if (t.eq.1) &
              write(LIS_logunit,*) '[INFO] Performing benchmark of Noah-3.9 ',&
           ...
           noah39_struc(n)%noah(t)%shdfac =                            &
                               ...
                               locyr,locmo,locda)
           noah39_struc(n)%noah(t)%shdmax = 0.0
           noah39_struc(n)%noah(t)%shdmin = 1.0
           do m = 1,12
              noah39_struc(n)%noah(t)%shdmax =                         &
                                  max(noah39_struc(n)%noah(t)%shdmax,  &
                                      noah39_struc(n)%shdfac_monthly(m))
              noah39_struc(n)%noah(t)%shdmin =                         &
                                  min(noah39_struc(n)%noah(t)%shdmin,  &
                                      noah39_struc(n)%shdfac_monthly(m))
           enddo
        else
           tid = LIS_surface(n,mtype)%tile(t)%tile_id
        ...
        if ((LIS_rc%usealbedomap(n).ne."none").and.(LIS_rc%startcode.eq."coldstart")) then
           ...
           noah39_struc(n)%noah(t)%albedo = noah39_struc(n)%noah(t)%alb
        endif
        ...
        noah39_struc(n)%noah(t)%topt = topt_data
        noah39_struc(n)%noah(t)%cmcmax = cmcmax_data
        noah39_struc(n)%noah(t)%rsmax = rsmax_data
        noah39_struc(n)%noah(t)%cfactr = cfactr_data
        
     enddo

     if (LIS_rc%usemxsnalbmap(n).eq."fixed") then
        write(LIS_logunit,*) '[INFO] Fixing Noah-3.9 max snow albedo: ',      &
                              noah39_struc(n)%fixedmxsnalb
        if (noah39_struc(n)%fixedmxsnalb.eq.0.0) then
           write(LIS_logunit,*) '[WARN] Noah-3.9 max snow albedo ',  &
                                'set to zero!'
        endif
     else
        if (LIS_rc%usemxsnalbmap(n).eq."LDT") then
           write(LIS_logunit,*) '[INFO] Reading Noah-3.9 max snow albedo ',   &
     ...
     do t=1,LIS_rc%npatch(n,mtype)
        noah39_struc(n)%noah(t)%nroot = &
             noah39_struc(n)%nrotbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%rsmin = &
             noah39_struc(n)%rstbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%rgl = &
             noah39_struc(n)%rgltbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%hs = &
             noah39_struc(n)%hstbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%snup = &
             noah39_struc(n)%snuptbl(noah39_struc(n)%noah(t)%vegt)
!        noah39_struc(n)%noah(t)%czil = &
!             noah39_struc(n)%cziltbl(noah39_struc(n)%noah(t)%vegt)
!        noah39_struc(n)%noah(t)%cmcmax = &
!             noah39_struc(n)%cmcmaxtbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%ztopv = &
             noah39_struc(n)%ztopvtbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%zbotv = &
             noah39_struc(n)%zbotvtbl(noah39_struc(n)%noah(t)%vegt)

        if (LIS_rc%usemxsnalbmap(n).eq."fixed") then
           noah39_struc(n)%noah(t)%mxsnalb = noah39_struc(n)%fixedmxsnalb
        else
           if (LIS_rc%usemxsnalbmap(n).eq."LDT") then
              tid = LIS_surface(n,mtype)%tile(t)%tile_id
              noah39_struc(n)%noah(t)%mxsnalb = LIS_alb(n)%mxsnalb(tid)
           else
              noah39_struc(n)%noah(t)%mxsnalb = noah39_struc(n)%maxalb(noah39_struc(n)%noah(t)%vegt)/100.0
           endif
        endif
        if(LIS_rc%useroughnessmap(n).ne."none") then 
           tid = LIS_surface(n,mtype)%tile(t)%tile_id
           ...
           if (LIS_rc%startcode.eq."coldstart") then
              noah39_struc(n)%noah(t)%z0brd = noah39_struc(n)%noah(t)%z0
           endif
        endif
     enddo

! If not coupled to WRF, set any open water points to (ice = 1),
! to prevent the Noah LSM from running in noah39_main.F90 - dmm
#if (!defined COUPLED)
     do t=1,LIS_rc%npatch(n,mtype)
        if (noah39_struc(n)%noah(t)%vegt.eq.LIS_rc%glacierclass) then
           noah39_struc(n)%noah(t)%xice = -1
           noah39_struc(n)%noah(t)%alb =                               &
                    noah39_struc(n)%albmax(noah39_struc(n)%noah(t)%vegt)
           noah39_struc(n)%noah(t)%z0brd =                             &
                     noah39_struc(n)%z0min(noah39_struc(n)%noah(t)%vegt)
        elseif (noah39_struc(n)%noah(t)%vegt.eq.LIS_rc%waterclass) then
           noah39_struc(n)%noah(t)%xice = 1
        else
           noah39_struc(n)%noah(t)%xice = 0
        endif
     enddo
#endif
  enddo

end subroutine noah39_setvegparms

!BOP
!
! !ROUTINE: noah39_resetvegparms
! \label{noah39_resetvegparms}
!
! !REVISION HISTORY:
!  07 May 2009: Sujay Kumar; Initial Implementation
!  27 Oct 2010: David Mocko, changes for Noah3.1 in LIS6.1
...
!
! !INTERFACE:
subroutine noah39_resetvegparms(mtype)
! !USES:
  use LIS_coreMod, only : LIS_rc, LIS_domain, LIS_surface
  use LIS_vegDataMod,    only : LIS_gfrac, LIS_read_shdmax, &
       LIS_read_shdmin, LIS_lai, LIS_roughness
...
! !DESCRIPTION:
!  This subroutine resets Noah-3.9 vegetation parameters.
!  The current implementation uses a table-based lookup based
!  on vegetation classes to initialize the following parameters
!  \begin{verbatim}
!   nroot  - number of root zones
...
!!!!!
!!!!!       write(LIS_logunit,*)                        &
!!!!!            'Noah-3.9 resetting vegparms'
!!!!!
!!!!!
!!!!!! Reset the section of the vegetation parameter table to read for Noah-3.9
!!!!!     if (LIS_rc%lcscheme.eq."AVHRR") mminlu = 'UMD '
!!!!!     if (LIS_rc%lcscheme.eq."USGS") mminlu = 'USGS'
!!!!!     if (LIS_rc%lcscheme.eq."MODIS") mminlu = 'MODI'
!!!!!     if (LIS_rc%lcscheme.eq."ECOCLIMAP2") mminlu = 'ECM2'
!!!!!!-----------------------------------------------------------------------
!!!!!! Reset Noah-3.9 vegetation type at tile from the LIS domain
!!!!!!-----------------------------------------------------------------------
!!!!!     if (noah39_struc(n)%fixedvegtype.ne.0) then
!!!!!        write(LIS_logunit,*) 'Fixing Noah-3.9 vegetation to type: ',    &
!!!!!                             noah39_struc(n)%fixedvegtype
!!!!!     endif
!!!!!     do t=1,LIS_rc%npatch(n,mtype)
!!!!!        if (noah39_struc(n)%fixedvegtype.eq.0) then
...
!!!!!! Read in the Noah-3.9 Static Vegetation Parameter Files
!!!!!!-----------------------------------------------------------------------
!!!!!     write(LIS_logunit,*) 'Reading Noah-3.9 vegetation parameter file: ',&
!!!!!          noah39_struc(n)%vfile
!!!!!     open(unit=11,file=noah39_struc(n)%vfile,status='old')
!!!!!     lumatch = 0
!!!!!     do while (lumatch.eq.0)
!!!!!        read(11,*,end=2002) lutype
!!!!!        if (lutype.eq.mminlu) then
!!!!!           read(11,*) noah39_struc(n)%lucats,iindex
!!!!!           write(LIS_logunit,*) 'Noah-3.9 Landuse type ',mminlu,       &
!!!!!                                ' - found ',noah39_struc(n)%lucats,    &
!!!!!                                ' categories'
!!!!!           lumatch = 1
!!!!!        endif
...
!!!!!!!!!
!!!!!     do j=1,NOAH39_STRUC(N)%LUCATS
!!!!!        read(11,*) IINDEX, noah39_struc(n)%shdtbl(j), &
!!!!!             noah39_struc(n)%nrotbl(j), noah39_struc(n)%rstbl(j),&
!!!!!             noah39_struc(n)%rgltbl(j), &
!!!!!             noah39_struc(n)%hstbl(j), noah39_struc(n)%snuptbl(j), &
!!!!!             noah39_struc(n)%maxalb(j), &
...
!!!!!  
!!!!!     if (LIS_rc%usegreennessmap(n).eq."none") then
!!!!!        write(LIS_logunit,*) 'Reading Noah-3.9 greenness fraction ',   &
!!!!!                             'from lis.config file'
!!!!!     else
!!!!!        write(LIS_logunit,*) 'Reading Noah-3.9 greenness fraction ',   &
!!!!!                             'from greenness maps'
...
!!!!!           else
!!!!!              noah39_struc(n)%noah(t)%shdmax = 1.0
!!!!!              write(LIS_logunit,*) 'Noah-3.9 maximum greenness -- ',   &
!!!!!                                   'not defined for point ',t
!!!!!              write(LIS_logunit,*) 'Noah-3.9 maximum greenness -- ',   &
...
!!!!!           else
!!!!!              noah39_struc(n)%noah(t)%shdmin = 0.0
!!!!!              write(LIS_logunit,*) 'Noah-3.9 minimum greenness -- ',   &
!!!!!                                   'not defined for point ',t
!!!!!              write(LIS_logunit,*) 'Noah-3.9 minimum greenness -- ',   &
...
!!!!!     endif
  if (LIS_rc%usealbedomap(n).eq."none") then
        write(LIS_logunit,*) 'Calculating Noah-3.9 albedo from ',      &
                             'greenness and albedo min/max in table'
     else
        write(LIS_logunit,*) 'Reading Noah-3.9 albedo from ',          &
                             'albedo maps'
     endif
     if (LIS_rc%uselaimap(n).eq."none") then
        write(LIS_logunit,*) 'Calculating Noah-3.9 LAI from ',         &
                             'greenness and LAI min/max in table'
     else
        write(LIS_logunit,*) 'Reading Noah-3.9 LAI from ',             &
                             'LAI maps'
     endif
...
!!!!!        noah39_struc(n)%noah(t)%cfactr = cfactr_data
     ...
     if (LIS_rc%usemxsnalbmap(n).eq."fixed") then
        write(LIS_logunit,*) 'Fixing Noah-3.9 max snow albedo: ',      &
                              noah39_struc(n)%fixedmxsnalb
        if (noah39_struc(n)%fixedmxsnalb.eq.0.0) then
           write(LIS_logunit,*) 'WARNING: Noah-3.9 max snow albedo ',  &
                                'set to zero!'
        endif
     else
        if (LIS_rc%usemxsnalbmap(n).eq."LDT") then
           write(LIS_logunit,*) 'Reading Noah-3.9 max snow albedo ',   &
                                'from max snow albedo maps'
        else
           write(LIS_logunit,*) 'Reading Noah-3.9 max snow albedo ',   &
                                'from vegetation parameter file'
     ...
     do t=1,LIS_rc%npatch(n,mtype)
        noah39_struc(n)%noah(t)%nroot = noah39_struc(n)%nrotbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%rsmin = noah39_struc(n)%rstbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%rgl = noah39_struc(n)%rgltbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%hs = noah39_struc(n)%hstbl(noah39_struc(n)%noah(t)%vegt)
        noah39_struc(n)%noah(t)%snup = noah39_struc(n)%snuptbl(noah39_struc(n)%noah(t)%vegt)
        if (LIS_rc%usemxsnalbmap(n).eq."fixed") then
           noah39_struc(n)%noah(t)%mxsnalb = noah39_struc(n)%fixedmxsnalb
        else
           if (LIS_rc%usemxsnalbmap(n).eq."LDT") then
...
end subroutine noah39_resetvegparms
