...
!BOP
! !ROUTINE: noah39_getsnodeppred
! \label{noah39_getsnodeppred}
!
! !REVISION HISTORY:
...
!              space updates. 
! 16 Apr 2019: Eric Kemp; updated for Noah 3.9
!
! !INTERFACE:
subroutine noah39_getsnodeppred(n, k, obs_pred)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if (SNODEP_obs_obj(n)%mesh .eq. 8) then
        snwd(t) = noah39_struc(n)%noah(t)%snowh*39.37 !convert from meter to inch
     elseif (SNODEP_obs_obj(n)%mesh .eq. 16 .or. SNODEP_obs_obj(n)%mesh .eq. 25) then
        snwd(t) = noah39_struc(n)%noah(t)%snowh
     endif
  enddo
...
end subroutine noah39_getsnodeppred
