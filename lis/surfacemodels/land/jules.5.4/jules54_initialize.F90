...
!BOP
!
! !ROUTINE: jules54_initialize
! \label{jules54_initialize}
!
! !REVISION HISTORY:
!
! !INTERFACE:
subroutine jules54_initialize(mtype)
   ! !USES:
...
! !DESCRIPTION:
!
!  This routine initializes the jules54 state variables with some
...
!EOP
   ...
   do n=1, LIS_rc%nnest
      write(LIS_logunit,*) "MSG: jules54_initialize -- initialzie for jules54"
      k = 1
      do
         ...
         do t=start_k, end_k
           ...
           bexp_soilt(1,1,:)   = jules54_struc(n)%jules54(t)%b
           sathh_soilt(1,1,:)  = jules54_struc(n)%jules54(t)%sathh
           satcon_soilt(1,1,:) = jules54_struc(n)%jules54(t)%satcon
           smvcst_soilt(1,1,:) = jules54_struc(n)%jules54(t)%sm_sat
           smvccl_soilt(1,1,:) = jules54_struc(n)%jules54(t)%sm_crit
           smvcwt_soilt(1,1,:) = jules54_struc(n)%jules54(t)%sm_wilt
           hcap_soilt(1,1,:)   = jules54_struc(n)%jules54(t)%hcap
           hcon_soilt(1,1,:)   = jules54_struc(n)%jules54(t)%hcon
           albsoil_soilt(1,1)  = jules54_struc(n)%jules54(t)%albsoil 
         ...
         call init_ic(jules54_struc(n)%namelist_dir) 
         call init_params(jules54_struc(n)%namelist_dir)
         call init_parms()
