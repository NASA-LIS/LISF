...
! !DESCRIPTION:
!
!  Forcing-only option (jules54) for calling the forcing transfer routines.
! 
!  The arguments are: 
!  \begin{description}
!  \item[n]
...
!EOP
  ...
  jules54_struc(n)%forc_count = jules54_struc(n)%forc_count + 1 
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     jules54_struc(n)%jules54(t)%tair   = jules54_struc(n)%jules54(t)%tair   + tmp(tid)
     jules54_struc(n)%jules54(t)%qair   = jules54_struc(n)%jules54(t)%qair   + q2(tid)
     jules54_struc(n)%jules54(t)%swdown = jules54_struc(n)%jules54(t)%swdown + swd(tid)
     jules54_struc(n)%jules54(t)%lwdown = jules54_struc(n)%jules54(t)%lwdown + lwd(tid)
     jules54_struc(n)%jules54(t)%wind_e = jules54_struc(n)%jules54(t)%wind_e + uwind(tid)
     jules54_struc(n)%jules54(t)%wind_n = jules54_struc(n)%jules54(t)%wind_n + vwind(tid)
     jules54_struc(n)%jules54(t)%psurf  = jules54_struc(n)%jules54(t)%psurf  + psurf(tid)
     jules54_struc(n)%jules54(t)%rainf  = jules54_struc(n)%jules54(t)%rainf  + rain(tid)
     
     if(LIS_FORC_CRainf%selectOpt .eq. 1) then
       if(crain(tid) .ne. LIS_rc%udef) then 
          jules54_struc(n)%jules54(t)%rainf_c = jules54_struc(n)%jules54(t)%rainf_c + crain(tid)
       else
          jules54_struc(n)%jules54(t)%rainf_c = jules54_struc(n)%jules54(t)%rainf_c + 0.0
       endif
     else
     ...
     if(LIS_FORC_Snowf%selectOpt .eq. 1) then
        if(snowf(tid) .ne. LIS_rc%udef) then 
          jules54_struc(n)%jules54(t)%snowf = jules54_struc(n)%jules54(t)%snowf + snowf(tid)
        else
          jules54_struc(n)%jules54(t)%snowf = jules54_struc(n)%jules54(t)%snowf + 0.0
        endif
     else
