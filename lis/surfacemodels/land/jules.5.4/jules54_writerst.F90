...
!BOP
!
! !ROUTINE: jules54_writerst
! \label{jules54_writerst}
! !REVISION HISTORY:
...
!
! !DESCRIPTION:
!  This program writes restart files for jules54.
...
! \item[jules54\_dump\_restart](\ref{jules54_dump_restart})\\
!   writes the jules54 variables into the restart file
! \end{description}
!EOP
    ...
    ! set restart alarm
    alarmCheck = LIS_isAlarmRinging(LIS_rc, "JULES.5.4 restart alarm")
    
    ! set restart file format (read from LIS configration file_
    wformat = trim(JULES54_struc(n)%rformat)
    ...
    if(alarmCheck .or. (LIS_rc%endtime ==1)) then
        If (LIS_masterproc) Then
            ...
            elseif(wformat .eq. "netcdf") then
                ...
                call LIS_verify(status,"Error in nf90_open in jules54_writerst")
#endif
        ...
        call jules54_dump_restart(n, ftn, wformat)
        ...
        if (LIS_masterproc) then
            ...
            elseif(wformat .eq. "netcdf") then
                ...
                call LIS_verify(status, "Error in nf90_close in jules54_writerst")
#endif
            endif
            write(LIS_logunit, *) "jules54 archive restart written: ", filen
        endif
    endif
end subroutine jules54_writerst

!BOP
!
! !ROUTINE: jules54_dump_restart
! \label{jules54_dump_restart}
!
! !REVISION HISTORY:
!  This subroutine is generated with the Model Implementation Toolkit developed
!  by Shugong Wang for the NASA Land Information System Version 7. The initial 
!  specification of the subroutine is defined by Sujay Kumar. 
!  5/9/16: Shugong Wang, initial implementation for LIS 7 and jules54
! !INTERFACE:
subroutine jules54_dump_restart(n, ftn, wformat)
...
!
!
!  The following is the list of variables written in the jules54
!  restart file:
!  \begin{verbatim}
!    nc, nr, nsurfts             - grid and tile space dimensions
!    tsoil_deep                 - jules54 deep soil temperatures [K]
!    wood_prod_fast             - jules54 Fast-turnover wood product C pool [-]
!    wood_prod_med              - jules54 Medium-turnover wood product C pool [-]
!    wood_prod_slow             - jules54 Slow-turnover wood product C pool [-]
!    frac_agr_prev              - jules54 Agricultural fraction from previous TRIFFID call [-]
!    frac_agr                   - jules54 Agricultural fraction [-]
!    n_inorg                    - jules54 Gridbox Inorganic N pool [kg m-2]
!    canht_ft                   - jules54 Canopy height [m]
!    canopy                     - jules54 Surface/canopy water for snow-free land tiles [kg m-2]
!    ns                         - jules54 Soil Organic Nitrogen [kg m-2]
!    cs                         - jules54 Soil carbo [kg m-2]
!    gc                         - jules54 Stomatal conductance to evaporation for land tiles [m s-1]
!    gs                         - jules54 Stomatal conductance to evaporation [m s-1]
!    lai                        - jules54 LAI of plant functional types [-]
!    rgrain                     - jules54 Snow surface grain size on tiles [micron]
!    smc                        - jules54 Soil moisture in a layer at the surface [kg m-2]
!    smcl                       - jules54 Soil moisture content of layers [kg m-2]
!    snow_tile                  - jules54 Lying snow on tiles [kg m-2]
!    snow_grnd                  - jules54 Snow on the ground [kg m-2]
!    soot                       - jules54 Snow soot content (kg kg-1) [kg/kg]
!    t_soil                     - jules54 Sub-surface temperatures [K]
!    tstar_tile                 - jules54 Tile surface temperatures [K]
!    asteps_since_triffid       - jules54 Number of atmospheric timesteps since last call to TRIFFID [-]
!    lai_phen                   - jules54 LAI of PFTs after phenology. Required as separate variable for top-level argument list matching with VEG_IC2A [-]
!    c_veg                      - jules54 Total carbon content of the vegetation [kg m-2]
!    cv                         - jules54 Gridbox mean vegetation carbon [kg m-2]
!    fexp                       - jules54 Decay factor in Sat. Conductivity in water table layer [-]
!    ti_mean                    - jules54 Mean topographic index [-]
!    ti_sig                     - jules54 Standard deviation of topographic index [-]
!    zw                         - jules54 Water table depth [m]
!    sthzw                      - jules54 soil moist fraction in deep (water table) layer []
!    sthu                       - jules54 Unfrozen soil moisture content of the layers as a fraction of saturation [-]
!    sthf                       - jules54 Frozen soil moisture content of the layers as a fraction of saturation [-]
!    sice                       - jules54 Snow layer ice mass on tiles [kg m-2]
!    sliq                       - jules54 Snow layer liquid mass on tiles [kg m-2]
!    snowdepth                  - jules54 Snow depth on ground on tiles [m]
!    tsnow                      - jules54 Snow layer temperature [K]
!    rgrainl                    - jules54 Snow layer grain size on tiles [microns]
!    rho_snow_grnd              - jules54 Snowpack bulk density [kg/m3]
!    rho_snow                   - jules54 Snow layer densities [m]
!    ds                         - jules54 Snow layer thickness [m]
!  \end{verbatim}
!
! The routines invoked are:
! \begin{description}
...
!EOP 
    ...
    call LIS_writeGlobalHeader_restart(ftn, n, LIS_rc%lsm_index,                       &
                                   "JULES50",                                          & 
                                   dim1 =jules54_struc(n)%nsurft,                        &   
                                   dim2 =jules54_struc(n)%npft,                         &
                                   dim3 =jules54_struc(n)%sm_levels,                        &
                                   dim4 =jules54_struc(n)%nsmax,                        &
                                   dim5 =jules54_struc(n)%ns_deep,                      &
                                   dim6 =jules54_struc(n)%dim_cs1,                      &
                                   dim7 =jules54_struc(n)%nsurft*jules54_struc(n)%nsmax, &
                                   dim8 =jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers, &
                                   dim9 =jules54_struc(n)%dim_cslayer,                  &
                                   dim10=jules54_struc(n)%npft*jules54_struc(n)%dim_cslayer,  &
                                   dimID=dimID,                                        &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, nsnow_ID, "NSNOW",    &
                                 ...
                                 "-", vlevels=JULES54_struc(n)%nsurft ,           &
                                 valid_min=-99999.0, valid_max=99999.0,           &
                                 var_flag = "dim1")
    ! Number of snow layers on ground on tiles
    do l=1, JULES54_struc(n)%nsurft   
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = 1.0 * JULES54_struc(n)%jules54(t)%nsnow(l)
        enddo
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tsurf_elev_surft_ID, "TSURF_ELEV_SURFT",    &
                                 ...
                                 "K", vlevels=JULES54_struc(n)%nsurft ,           &
                                 valid_min=-99999.0, valid_max=99999.0,           &
                                 var_flag = "dim1")
    ! Number of snow layers on ground on tiles
    do l=1, JULES54_struc(n)%nsurft   
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%tsurf_elev_surft(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    !
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%z0msea, &
    ...
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%frac_past_prev, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, n_inorg_soilt_lyrs_ID, "N_INORG_SOILT_LYRS",    &
                                 ...
                                 "kg N/m2", vlevels=JULES54_struc(n)%dim_cslayer , &
                                 valid_min=-99999.0, valid_max=99999.0,            &
                                 var_flag = "dim9")
    !  Gridbox Inorganic N pool on soil levels (kg N/m2)
    do l=1, JULES54_struc(n)%dim_cslayer   
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = 1.0 * JULES54_struc(n)%jules54(t)%n_inorg_soilt_lyrs(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen,                 &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, n_inorg_avail_pft_ID, "N_INORG_AVAIL_PFT",       &
                                 ...
                                 vlevels=JULES54_struc(n)%npft*JULES54_struc(n)%dim_cslayer ,    &
                                 valid_min=-99999.0, valid_max=99999.0,                          &
                                 var_flag = "dim10")
    ! Availabile inorganic N for PFTs (depends on roots) 
    do l=1, JULES54_struc(n)%npft*JULES54_struc(n)%dim_cslayer
        call l_to_rc(l, jules54_struc(n)%npft, r_idx, c_idx)
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%n_inorg_avail_pft(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%triffid_co2_gb, &
    ...
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%canopy_gb, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, snow_mass_ij_ID, "SNOW_MASS_IJ", &
                                ...
                                "(kg/m2)", vlevels=1, valid_min=-99999.0, valid_max=99999.0)
    !Pasture fraction from previous TRIFFID call 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%snow_mass_ij, &
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tsoil_deep_ID, "TSOIL_DEEP", &
                                 ...
                                 "K", vlevels=JULES54_struc(n)%ns_deep , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim5") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, canht_ft_ID, "CANHT_FT", &
                                 "Canopy height", &
                                 "m", vlevels=JULES54_struc(n)%npft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim2")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, canopy_ID, "CANOPY", &
                                 ...
                                 "kg m-2", vlevels=JULES54_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, ns_ID, "NS", &
                                 ...
                                 "kg m-2", vlevels=JULES54_struc(n)%dim_cs1 , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim6") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, gc_ID, "GC", &
                                 ...
                                 "m s-1", vlevels=JULES54_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, lai_ID, "LAI", &
                                 ...
                                 "-", vlevels=JULES54_struc(n)%npft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim2") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rgrain_ID, "RGRAIN", &
                                 ...
                                 "micron", vlevels=JULES54_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, smcl_ID, "SMCL", &
                                 ...
                                 "kg m-2", vlevels=JULES54_struc(n)%sm_levels , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim3") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, t_soil_ID, "T_SOIL", &
                                 ...
                                 "K", vlevels=JULES54_struc(n)%sm_levels , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim3")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tstar_tile_ID, "TSTAR_TILE", &
                                 ...
                                 "K", vlevels=JULES54_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, c_veg_ID, "C_VEG", &
                                 ...
                                 "kg m-2", vlevels=JULES54_struc(n)%npft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim2") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, sthu_ID, "STHU", &
                                 ...
                                 "-", vlevels=JULES54_struc(n)%sm_levels , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim3")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, sice_ID, "SICE", &
                                 ...
                                 !"kg m-2", vlevels=JULES54_struc(n)%nsurft*JULES54_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 "kg m-2", vlevels=jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim8")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, snowdepth_ID, "SNOWDEPTH", &
                                 ...
                                 "m", vlevels=JULES54_struc(n)%nsurft, valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim1")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, tsnow_ID, "TSNOW", &
                                 ...
                                 !"K", vlevels=JULES54_struc(n)%nsurft*JULES54_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 "K", vlevels=jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim8") 
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rgrainl_ID, "RGRAINL", &
                                 ...
                                 !"microns", vlevels=JULES54_struc(n)%nsurft*JULES54_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 "microns", vlevels=jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim8")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rho_snow_grnd_ID, "RHO_SNOW_GRND", &
                                 ...
                                 "kg/m3", vlevels=JULES54_struc(n)%nsurft , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim7")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, rho_snow_ID, "RHO_SNOW", &
                                 ...
                                 "m", vlevels=JULES54_struc(n)%nsurft*JULES54_struc(n)%nsmax , valid_min=-99999.0, valid_max=99999.0, &
                                 var_flag = "dim7")
    ...
    call LIS_writeHeader_restart(ftn, n, dimID, ds_ID, "DS",                                            &
                                 ...
                                 vlevels=jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers,      & 
                                 valid_min=-99999.0, valid_max=99999.0,                                 &
                                 var_flag = "dim8")
 
    ! close header of restart file
    call LIS_closeHeader_restart(ftn, n, LIS_rc%lsm_index, dimID, JULES54_struc(n)%rstInterval)

    ! write state variables into restart file
    ! deep soil temperatures
    do l=1, JULES54_struc(n)%ns_deep   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%tsoil_deep(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
                                  varid=tsoil_deep_ID, dim=l, wformat=wformat)
    enddo
    ! Fast-turnover wood product C pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%wood_prod_fast, &
                              varid=wood_prod_fast_ID, dim=1, wformat=wformat)

    ! Medium-turnover wood product C pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%wood_prod_med, &
                              varid=wood_prod_med_ID, dim=1, wformat=wformat)

    ! Slow-turnover wood product C pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%wood_prod_slow, &
                              varid=wood_prod_slow_ID, dim=1, wformat=wformat)

    ! Agricultural fraction from previous TRIFFID call
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%frac_agr_prev, &
                              varid=frac_agr_prev_ID, dim=1, wformat=wformat)

    ! Agricultural fraction
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%frac_agr, &
                              varid=frac_agr_ID, dim=1, wformat=wformat)

    ! Gridbox Inorganic N pool
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%n_inorg, &
                              varid=n_inorg_ID, dim=1, wformat=wformat)

    ! Canopy height
    do l=1, JULES54_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%canht_ft(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Surface/canopy water for snow-free land tiles
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%canopy(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Soil Organic Nitrogen
    do l=1, JULES54_struc(n)%dim_cs1*JULES54_struc(n)%dim_cslayer   ! TODO: check loop
        call l_to_rc(l, jules54_struc(n)%dim_cs1, r_idx, c_idx)
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%ns(r_idx,c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%dim_cs1*JULES54_struc(n)%dim_cslayer   ! TODO: check loop
        ...
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%cs_pool_soilt(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%gc_surft(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Stomatal conductance to evaporation
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%gs_gb, &
    ...
    do l=1, JULES54_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%lai(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%rgrain(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Soil moisture in a layer at the surface
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%smc_soilt, &
                              varid=smc_ID, dim=1, wformat=wformat)

    ! Soil moisture content of layers
    do l=1, JULES54_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%smcl_soilt(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%snow_tile(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%snow_grnd(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Snow soot content (kg kg-1)
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%soot_ij, &
    ...
    do l=1, JULES54_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%t_soil(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%tstar_tile(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Number of atmospheric timesteps since last call to TRIFFID
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%asteps_since_triffid, &
    ...
    do l=1, JULES54_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%lai_phen(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%npft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%c_veg(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Gridbox mean vegetation carbon
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%cv, &
                              varid=cv_ID, dim=1, wformat=wformat)

    ! Decay factor in Sat. Conductivity in water table layer
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%fexp, &
                              varid=fexp_ID, dim=1, wformat=wformat)

    ! Mean topographic index
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%ti_mean, &
                              varid=ti_mean_ID, dim=1, wformat=wformat)

    ! Standard deviation of topographic index
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%ti_sig, &
                              varid=ti_sig_ID, dim=1, wformat=wformat)

    ! Water table depth
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%zw, &
                              varid=zw_ID, dim=1, wformat=wformat)
    
    ! Surface saturation fraction
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%fsat, &
                              varid=fsat_ID, dim=1, wformat=wformat)

    ! Wetland fraction 
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%fwetl, &
                              varid=fwetl_ID, dim=1, wformat=wformat)

    ! soil moist fraction in deep (water table) layer
    call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, JULES54_struc(n)%jules54%sthzw, &
    ...
    do l=1, JULES54_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%p_s_sthu(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%sm_levels   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%p_s_sthf(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Snow layer ice mass on tiles
    do l=1, jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers   ! TODO: check loop
        tmptilen = 0
        call l_to_rc(l, jules54_struc(n)%temp_tiles, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%sice(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers   ! TODO: check loop
        ...
        call l_to_rc(l, jules54_struc(n)%temp_tiles, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%sliq(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%snowdepth(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers   ! TODO: check loop
        ...
        call l_to_rc(l, jules54_struc(n)%temp_tiles, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%tsnow(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers   ! TODO: check loop
        ...
        call l_to_rc(l, jules54_struc(n)%temp_tiles, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%rgrainl(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, JULES54_struc(n)%nsurft   ! TODO: check loop
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%rho_snow_grnd(l)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    ! Snow layer densities
    do l=1, JULES54_struc(n)%nsurft*jules54_struc(n)%nsmax   ! TODO: check loop
        tmptilen = 0
        call l_to_rc(l, jules54_struc(n)%nsurft, r_idx, c_idx)
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%rho_snow(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
    ...
    do l=1, jules54_struc(n)%temp_tiles*jules54_struc(n)%temp_layers   ! TODO: check loop
        ...
        tmptilen = 0
        do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
            tmptilen(t) = JULES54_struc(n)%jules54(t)%ds(r_idx, c_idx)
        enddo
        call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
                                  varid=ds_ID, dim=l, wformat=wformat)
    enddo
end subroutine jules54_dump_restart
