...
! Specific humidity (Kg/Kg)
  qw_1_ij(1,1) = jules54_struc(n)%jules54(t)%qair/jules54_struc(n)%forc_count
! Air temperature (k)
  tl_1_ij(1,1) = jules54_struc(n)%jules54(t)%tair/jules54_struc(n)%forc_count
! W'ly wind component (m s-1)
  u_1_ij(1,1)  = jules54_struc(n)%jules54(t)%wind_e/jules54_struc(n)%forc_count
! S'ly wind component (m s-1)
  v_1_ij(1,1)  = jules54_struc(n)%jules54(t)%wind_n/jules54_struc(n)%forc_count
! Surface pressure (Pascals)
  pstar_ij(1,1)   = jules54_struc(n)%jules54(t)%psurf/jules54_struc(n)%forc_count
...
! All rain goes to Large-scale rainfall (kg m-2/s)
  if(jules54_struc(n)%jules54(t)%rainf .ne. LIS_rc%udef) then
    ls_rain_ij(1,1) = jules54_struc(n)%jules54(t)%rainf/ jules54_struc(n)%forc_count
  else
    ls_rain_ij(1,1) = 0.0
  endif

  if (LIS_FORC_Snowf%selectOpt .eq. 1) then
    ls_snow_ij(1,1) = jules54_struc(n)%jules54(t)%snowf / jules54_struc(n)%forc_count
  else 
    if (jules54_struc(n)%jules54(t)%tair .lt. LIS_CONST_TKFRZ) then
...
! Surface downward SW radiation (W m-2)
  sw_down_ij(1,1) = jules54_struc(n)%jules54(t)%swdown/jules54_struc(n)%forc_count
! Surface downward LW radiation (W m-2)
  lw_down_ij(1,1) = jules54_struc(n)%jules54(t)%lwdown/jules54_struc(n)%forc_count
