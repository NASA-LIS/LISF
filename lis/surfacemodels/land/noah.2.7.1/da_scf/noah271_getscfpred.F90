...
!BOP
! !ROUTINE: noah271_getscfpred
! \label{noah271_getscfpred}
!
! !REVISION HISTORY:
! 13Nov2007: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine noah271_getscfpred(n, obs_pred)
! !USES:
  use ESMF
...
! !DESCRIPTION:
!
!  Returns the snow cover obs pred (model's estimate of 
...
!EOP
  ...
  do i=1,LIS_rc%ntiles(n),LIS_rc%nensem(n)
     do m=1,LIS_rc%nensem(n)
        ...
        obs_pred(gid,m)= noah271_struc(n)%noah(t)%sca*100.0
     enddo
  enddo
  
end subroutine noah271_getscfpred
