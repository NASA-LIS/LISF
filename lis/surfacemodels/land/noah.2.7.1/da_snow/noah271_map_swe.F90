...
!BOP
! !ROUTINE: noah271_map_swe
! \label{noah271_map_swe}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah271_map_swe(n,OBS_State,LSM_State)
! !USES:
  use ESMF
...
! Based on SWE, we manually update snowdepth    
  ...
  do t=1,LIS_rc%ntiles(n)
     if(sweobs(LIS_domain(n)%tile(t)%index).gt.0) then 
        ...
        newsn = swe(t)
        snowhc = noah271_struc(n)%noah(t)%snowh*100.0
        newsnc = newsn*100.0
...
end subroutine noah271_map_swe
