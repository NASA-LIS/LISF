...
!  31 May 2004: Kristi Arsenault; Updated Noah LSM to 2.7 version
!  24 Nov 2004: Kristi Arsenault; Updated Noah LSM to 2.7.1
!  06 Apr 2005: Jesse Meng; Fix FDOWN, RTDIS, ETA, EVP
!  16 Oct 2006: Jesse Meng; Complete Noah 2.7.1 
!
! ----------------------------------------------------------------------
module module_sf_noah271lsm
! -- 1. MAIN PHYSICS SUBROUTINE ==>  SUBROUTINE SFLX  ------------------ 
...
! !INTERFACE:
      SUBROUTINE SFLX (ntl, &
       ...
       SOILW,SOILT,SOILM,ETANRG, &
! Save soil surface temperature and emissivity for output - D. Mocko
       TSOIL,EMISSI_OUT)
!EOP

      IMPLICIT NONE

  integer, parameter :: c1 = 588!15123
! ----------------------------------------------------------------------
...
!                (VOLUMETRIC)
!   NROOT      NUMBER OF ROOT LAYERS, A FUNCTION OF VEG TYPE
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
! CALCULATE RTDIS. Jesse 20050406
! ----------------------------------------------------------------------
        DO KZ = 1, NROOT
           RTDIS(KZ) = -SLDPTH(KZ)/ZSOIL(NROOT)
        END DO
...
! ----------------------------------------------------------------------
      ...
      ELSE
        SNDENS = SNEQV/SNOWH
        if(snowh < sneqv) print*,'snow ',ntl,sneqv,snowh, sndens
!        SNCOND = CSNOW(SNDENS) 
...
! ----------------------------------------------------------------------
          ...
          EMISSI_OUT = (0.95*SNCOVR) + (EMISSI*(1.0-SNCOVR))
        ENDIF
...
!      COMMENTED OUT 5/31/2008 C. Alonge (moved call to noah_main.F90
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
      ...
      IF (SNEQV .EQ. 0.0) THEN
        CALL NOPAC (ETP,ETA,PRCP,SMC,SMCMAX,SMCWLT, &
                   ...
                   QUARTZ,FXEXP,CSOIL, &
                   BETA,DRIP,DEW,FLX1,FLX2,FLX3, EMISSI)
        TSOIL = T1
        eta_kinematic = eta
      ELSE
...
! ----------------------------------------------------------------------
!JESSE, SAVE TOTAL ET IN KG M-2 S-1
!       EVP = ETA
      IF (ETP .GT. 0.) THEN
...
!Check to screen out low SWE values (added by SVK)
      if(t1>=TFREEZ+2.0) then 
         if(snowh<=1E-6.or.sneqv<=1E-3) then 
            snowh = 0.0
            sneqv = 0.0
         endif
      endif
...
! -- 2. PHYSICS SUBROUTINE ==>  SUBROUTINE ALCALC ---------------------- 
...
! -- 3. PHYSICS SUBROUTINE ==>  SUBROUTINE CANRES  --------------------
...
! ----------------------------------------------------------------------
      if(SMCREF.eq.SMCWLT) then 
         print*, 'problem: ',smcref, smcwlt
      endif
...
! -- 4. PHYSICS SUBROUTINE ==>  SUBROUTINE DEVAP  --------------------- 
...
! -- 5. PHYSICS SUBROUTINE ==>  SUBROUTINE EVAPO  --------------------- 
...
! -- 6. PHYSICS SUBROUTINE ==>  SUBROUTINE HRT ------------------------
...
! -- 7. PHYSICS SUBROUTINE ==>  SUBROUTINE HRTICE ----------------------
...
! -- 8. PHYSICS SUBROUTINE ==>  SUBROUTINE HSTEP ----------------------
...
! -- 9. PHYSICS SUBROUTINE ==>  SUBROUTINE NOPAC  ----------------------
...
! -- 10. PHYSICS SUBROUTINE ==>  SUBROUTINE PENMAN  --------------------

      SUBROUTINE PENMAN (SFCTMP,SFCPRS,CH,T2V,TH2,PRCP,FDOWN,T24, &
...
! -- 12. PHYSICS SUBROUTINE ==>  SUBROUTINE SFCDIF ---------------------

      SUBROUTINE SFCDIF (ZLM,Z0,THZ0,THLM,SFCSPD,CZIL,AKMS,AKHS,USTAR)
...
! -- 13. PHYSICS SUBROUTINE ==>  SUBROUTINE SHFLX  ---------------------
...
! -- 14. PHYSICS SUBROUTINE ==>  SUBROUTINE SMFLX ----------------------
...
! -- 15. PHYSICS SUBROUTINE ==>  SUBROUTINE SNFRAC  --------------------
...
! ----------------------------------------------------------------------
! END FUNCTION SNKSRC_271
! ----------------------------------------------------------------------
77    RETURN
    END SUBROUTINE SNKSRC

! -- 16. PHYSICS SUBROUTINE ==>  SUBROUTINE SNOPAC ---------------------
...
!JESSE 20041228
!USE MIKE EK'S SNOW EMISSIVITY 0.95
      T12A = ( (FDOWN-FLX1-FLX2- &
             (EMISSI_S*SNCOVR + (1.0-SNCOVR))*SIGMA*T24)/RCH &
            + TH2 - SFCTMP - ETANRG/RCH ) / RR
...
!USE MIKE EK'S SNOW EMISSIVITY 0.95
          FLX3 = FDOWN - FLX1 - FLX2 - &
                 (EMISSI_S*SNCOVR + (1.0-SNCOVR))*SIGMA*T14 &
                 - SSOIL - SEH - ETANRG
...
! ----------------------------------------------------------------------
      ...
      REAL BFAC,C1,C2,SNDENS,DSX,DTHR,DTSEC,DW,SNOWHC,SNOWH,PEXP, &
          TAVGC,TSNOW,TSNOWC,TSOIL,TSOILC,ESD,ESDC,ESDCX,G,KN
...
! THIS LAYER
! ----------------------------------------------------------------------
          CALL WDFCND (WDF2,WCND2,SH2OA(NSOIL),SMCMAX,BEXP,DKSAT, &
                      DWSAT,SICEMAX)
...
end module module_sf_noah271lsm
