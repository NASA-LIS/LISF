...
!BOP
! !ROUTINE: noah271_wrf_f2t
! \label{noah271_wrf_f2t}
!
! !REVISION HISTORY:
!  15 Oct 2002: Sujay Kumar; Initial Code
!  11 Dec 2015; Eric Kemp; Updated to LIS 7.1
!
! !INTERFACE:
subroutine noah271_wrf_f2t(n)
! !USES:      
  use ESMF
...
!  call LIS_verify(status, 'Error in ESMF_StateGet: Snowf_f')
  ...
  noah271_struc(n)%forcing_ch = 1 ! EMK NUWRF
...
!  call LIS_verify(status)
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     noah271_struc(n)%noah(t)%tair=tmp(tid)
     noah271_struc(n)%noah(t)%qair=q2(tid)
     noah271_struc(n)%noah(t)%swdown=swd(tid)
     noah271_struc(n)%noah(t)%lwdown=lwd(tid)
     noah271_struc(n)%noah(t)%uwind=uwind(tid)
     noah271_struc(n)%noah(t)%vwind=vwind(tid)
     noah271_struc(n)%noah(t)%psurf=psurf(tid)
     noah271_struc(n)%noah(t)%xice=xice(tid)
     if(pcp(t).ne.LIS_rc%udef) then 
        noah271_struc(n)%noah(t)%rainf=pcp(tid)
     else
     ...
     if(ch(tid).ne.LIS_rc%udef) then 
        noah271_struc(n)%noah(t)%ch=ch(tid)
     endif

     if(chs2(tid).ne.LIS_rc%udef) then 
        noah271_struc(n)%noah(t)%chs2=chs2(tid)
     endif

     if(cqs2(tid).ne.LIS_rc%udef) then 
        noah271_struc(n)%noah(t)%cqs2=cqs2(tid)
     endif

     if(q2sat(tid).ne.LIS_rc%udef) then 
        noah271_struc(n)%noah(t)%q2sat=q2sat(tid)
     endif

     if(emiss(tid).ne.LIS_rc%udef) then 
        noah271_struc(n)%noah(t)%emiss=emiss(tid)
     endif

     if(zval(tid).ne.LIS_rc%udef) then 
        noah271_struc(n)%noah(t)%z=zval(tid)
     endif
     noah271_struc(n)%forc_count = 1
  enddo

end subroutine noah271_wrf_f2t
