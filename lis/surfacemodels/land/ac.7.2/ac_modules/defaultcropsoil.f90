...
subroutine ResetDefaultCrop(use_default_crop_file)
    ...
    call SetCrop_Tbase(5.5_sp)  ! Basal temperature (degC)
    call SetCrop_Tupper(30.0_sp) ! Cut-off temperature (degC)
    call SetCrop_pLeafDefUL(0.25_sp) ! Soil water depletion factor for leaf
                                     ! expansion (p-leaf) - Upper Limit
    call SetCrop_pLeafDefLL(0.60_sp) ! Soil water depletion factor for leaf
                                     ! expansion (p-leaf) - Lower Limit
    call SetCrop_KsShapeFactorLeaf(3._sp) ! Shape factor for Water stress
                                          ! coefficient Leaf expansion
                                          ! (0 = straight line)
    call SetCrop_pdef(0.50_sp) ! Soil water depletion fraction for stomatal
                               ! control (p - stomatal)
    call SetCrop_KsShapeFactorStomata(3._sp) ! Shape factor for Water stress
                                        ! coefficient Stomatal Control
                                        ! (0 = straight line)
    call SetCrop_pSenescence(0.85_sp) ! Soil water depletion factor for
                                      ! canopy senescence (p-senescence)
    call SetCrop_KsShapeFactorSenescence(3._sp) ! Shape factor for Water
    ...
    call SetCrop_pPollination(0.90_sp)
    ...
    call SetCrop_StressResponse_Stress(50_int8)  ! Soil fertility stress at
                                                 ! calibration (%)
    call SetCrop_StressResponse_ShapeCGC(2.16_sp)  ! Shape factor for response
                                                   ! of Canopy Growth Coefficient
                                                   ! to soil fertility stress
    call SetCrop_StressResponse_ShapeCCX(0.79_sp) ! Shape factor for response of
                              ! Maximum Canopy Coefficient to soil fertility stress
    call SetCrop_StressResponse_ShapeWP(1.67_sp)  ! Shape factor for response of
                              ! Crop water productivity to soil fertility stress
    call SetCrop_StressResponse_ShapeCDecline(1.67_sp) ! Shape factor for response
    ...
    call SetCrop_GDtranspLow(11.1_sp) ! Minimum growing degrees required for full
                                      ! crop transpiration (degC - day)
    call SetCrop_KcTop(1.10_sp) ! Crop coefficient when complete cover and prior to
                                ! senescence (Kc,top)
    call SetCrop_KcDecline(0.150_sp) ! Decline crop coefficient (%/day) as a result
    ...
    call SetCrop_SmaxTopQuarter(0.048_sp) ! Maximum root water extraction (m3water/m3soil.day)
                                          ! in top quarter of root zone
    call SetCrop_SmaxBotQuarter(0.012_sp) ! Maximum root water extraction (m3water/m3soil.day)
    ...
    call SetCrop_SizeSeedling(6.50_sp) ! Canopy cover per seedling (cm2)
    ...
    call SetCrop_CCo((GetCrop_SizeSeedling()/10000) &
                        * (GetCrop_PlantingDens()/10000._sp)) ! Starting canopy size
                                                              ! (CCo) in fraction
    call SetCrop_CCini(GetCrop_CCo())
    call SetCrop_CGC(0.15_sp) ! Canopy growth coefficient (CGC): Increase in canopy
    ...
    call SetCrop_CCxRoot(real(undef_int, kind=sp)) ! shape factor of the decline of CCx over the
                                    ! years due to self-thinning - Perennials
    call SetCrop_CCx(0.80_sp) ! Maximum canopy cover (CCx) in fraction
    call SetCrop_CDC(0.1275_sp) ! Canopy decline coefficient (CDC): Decrease in
    ...
    call SetCrop_WP(17.0_sp) ! (normalized) Water productivity (gram/m2)
    ...
    call SetCrop_aCoeff(10._sp) ! Coefficient describing positive impact of
                                ! restricted vegetative growth at flowering on HI
    call SetCrop_bCoeff(8._sp) ! Coefficient describing reduction of impact of
                               ! stomatal closure at flowering on HI
    call SetCrop_DHImax(15_int8) ! Allowable maximum increase (%) of specified HI
    call SetCrop_dHIdt(-9._sp) ! Caluclated as Crop_HI/Crop_DaysToHIo
    ...
    call SetCrop_GDDCGC(-9.000000_sp) ! CGC for GGDays: Increase in canopy cover
                                      ! (in fraction) per growing-degree day
    call SetCrop_GDDCDC(-9.000000_sp) ! CDC for GGDays: Decrease in canopy cover
...
subroutine ResetDefaultSoil(use_default_soil_file)
    ...
    call SetSoilLayer_Thickness(1, 4._sp)
    call SetSoilLayer_SAT(1, 50.0_sp)
    call SetSoilLayer_FC(1, 30.0_sp)
    call SetSoilLayer_WP(1, 10.0_sp)
    call SetSoilLayer_InfRate(1, 500.0_sp)
    call SetSoilLayer_Penetrability(1, 100_int8)
    call SetSoilLayer_GravelMass(1, 0_int8)
    call SetSoilLayer_GravelVol(1, 0._sp)
