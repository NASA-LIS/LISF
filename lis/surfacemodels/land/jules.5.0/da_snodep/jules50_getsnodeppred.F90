...
!BOP
! !ROUTINE: jules50_getsnodeppred
! \label{jules50_getsnodeppred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules50_getsnodeppred(n, k, obs_pred)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if (SNODEP_obs_obj(n)%mesh .eq. 8) then
        snwd(t) = jules50_struc(n)%jules50(t)%snowdepth(pft)*39.37 !convert from meter to inch
     elseif (SNODEP_obs_obj(n)%mesh .eq. 16 .or. SNODEP_obs_obj(n)%mesh .eq. 25) then
        snwd(t) = jules50_struc(n)%jules50(t)%snowdepth(pft)
     endif
  enddo
...
end subroutine jules50_getsnodeppred
