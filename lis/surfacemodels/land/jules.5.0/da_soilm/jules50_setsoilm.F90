...
! differs from that of the prognostic variable (p_s_sthu, p_s_smvcst) [m3/m3].
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     MAX_THRESHOLD = jules50_struc(n)%jules50(t)%p_s_smvcst(1) ! Volumetric saturation point (m^3 m-3 of soil)
    ...
    if (jules50_struc(n)%jules50(t)%p_s_smvcst(1) == 0) then     !Yonghwan Kwon: set update_flag to false for glacier land 
     ...
     if(jules50_struc(n)%jules50(t)%p_s_sthu(1)*jules50_struc(n)%jules50(t)%p_s_smvcst(1)&
	+ delta1.gt.MIN_THRESHOLD .and.&
        jules50_struc(n)%jules50(t)%p_s_sthu(1)*jules50_struc(n)%jules50(t)%p_s_smvcst(1)&
	+delta1.lt.sm_threshold) then 
     ...
     if(jules50_struc(n)%jules50(t)%p_s_sthu(2)*jules50_struc(n)%jules50(t)%p_s_smvcst(2)&
	+delta2.gt.MIN_THRESHOLD .and.&
        jules50_struc(n)%jules50(t)%p_s_sthu(2)*jules50_struc(n)%jules50(t)%p_s_smvcst(2)&
	+delta2.lt.sm_threshold) then 
     ...
     if(jules50_struc(n)%jules50(t)%p_s_sthu(3)*jules50_struc(n)%jules50(t)%p_s_smvcst(3)&
	+delta3.gt.MIN_THRESHOLD .and.&
        jules50_struc(n)%jules50(t)%p_s_sthu(3)*jules50_struc(n)%jules50(t)%p_s_smvcst(3)&
	+delta3.lt.sm_threshold) then 
     ...
     if(jules50_struc(n)%jules50(t)%p_s_sthu(4)*jules50_struc(n)%jules50(t)%p_s_smvcst(4)&
	+delta4.gt.MIN_THRESHOLD .and.&
        jules50_struc(n)%jules50(t)%p_s_sthu(4)*jules50_struc(n)%jules50(t)%p_s_smvcst(4)&
...
! compute the ensemble min and max for case 1-2        
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           MAX_THRESHOLD = jules50_struc(n)%jules50(t)%p_s_smvcst(1) ! [m3/m3] assumed value for all layers are the same
           sm_threshold  = MAX_THRESHOLD - MIN_THRESHOLD       
	      ...
	      delta4 = (soilm4(t)-jules50_struc(n)%jules50(t)%smcl_soilt(4))* 1/dzsoil(4)*1/1000   
              ...
              jules50_struc(n)%jules50(t)%p_s_sthu(1) = &
		(jules50_struc(n)%jules50(t)%p_s_sthu(1)*jules50_struc(n)%jules50(t)%p_s_smvcst(1)&
                 +delta1)/jules50_struc(n)%jules50(t)%p_s_smvcst(1) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
	      ...
	      !Because both “update_flag_new” and “update_flag_tile” are TRUE 
              ...
              jules50_struc(n)%jules50(t)%p_s_sthu(2) = &
		(jules50_struc(n)%jules50(t)%p_s_sthu(2)*jules50_struc(n)%jules50(t)%p_s_smvcst(2)&
                 +delta2)/jules50_struc(n)%jules50(t)%p_s_smvcst(2) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
              ...
              jules50_struc(n)%jules50(t)%p_s_sthu(3) = &
		(jules50_struc(n)%jules50(t)%p_s_sthu(3)*jules50_struc(n)%jules50(t)%p_s_smvcst(3)&
                 +delta3)/jules50_struc(n)%jules50(t)%p_s_smvcst(3) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
              ...
              jules50_struc(n)%jules50(t)%p_s_sthu(4) = &
		(jules50_struc(n)%jules50(t)%p_s_sthu(4)*jules50_struc(n)%jules50(t)%p_s_smvcst(4)&
                 +delta4)/jules50_struc(n)%jules50(t)%p_s_smvcst(4) ! ([-]*[m3/m3] + [m3/m3])/[m3/m3] --> fraction  
...
!-----------------------------------------------------------------------------------------
			...
			jules50_struc(n)%jules50(t)%p_s_smvcst(1) ! ([kg/m2]*(1/m*1/(kg/m3)))/[m3/m3]--> fraction
			...
			jules50_struc(n)%jules50(t)%p_s_smvcst(2)
			...
			jules50_struc(n)%jules50(t)%p_s_smvcst(3)
			...
			jules50_struc(n)%jules50(t)%p_s_smvcst(4)
...
!--------------------------------------------------------------------------
		...
		! NOTE: be careful with the unit. do not change the unit of delta in the loop
              ! [m3w/m3s]  + ([-] - [-])*[m3/m3] 
                       delta(j) = delta(j)+ & !  * 1/dzsoil(j)*1/1000 
                            (jules50_struc(n)%jules50(t)%p_s_sthu(j) - &
                             jules50_struc(n)%jules50(t_unpert)%p_s_sthu(j)) * &
		      ...
		      MAX_THRESHOLD = jules50_struc(n)%jules50(t)%p_s_smvcst(1) ! assume it is the same for all layers [m3/m3]
                    sm_threshold  = MAX_THRESHOLD-MIN_THRESHOLD
                    
                    tmpval = jules50_struc(n)%jules50(t)%p_s_sthu(j) * &
		    	        jules50_struc(n)%jules50(t)%p_s_smvcst(j) - &
                             delta(j) !* 1/dzsoil(j)*1/1000 ! [-][m3/m3]-[m3/m3] --> [m3/m3] 

                    if(tmpval.le.MIN_THRESHOLD) then 

                       if (jules50_struc(n)%jules50(t)%p_s_smvcst(j) == 0) then
                          jules50_struc(n)%jules50(t)%p_s_sthu(j) = 0
                       else
                          jules50_struc(n)%jules50(t)%p_s_sthu(j) = &
                               (max(jules50_struc(n)%jules50(t_unpert)%p_s_sthu(j)&
                                   *jules50_struc(n)%jules50(t)%p_s_smvcst(j),&
                                    MIN_THRESHOLD)) / jules50_struc(n)%jules50(t)%p_s_smvcst(j)! max( [-][m3/m3] , [m3/m3] ) / [m3/m3] --> fraction
                       endif
			        ...
			        *1/dzsoil(j)*1/1000,&
                    ...
                    elseif(tmpval.ge.sm_threshold) then
                       ...
                       if (jules50_struc(n)%jules50(t)%p_s_smvcst(j) == 0) then
                          ...
                          jules50_struc(n)%jules50(t)%p_s_sthu(j) = &
                               (min(jules50_struc(n)%jules50(t_unpert)%p_s_sthu(j)&
                                   *jules50_struc(n)%jules50(t)%p_s_smvcst(j),&
                                   sm_threshold)) / jules50_struc(n)%jules50(t)%p_s_smvcst(j) ! min( [-][m3/m3] , [m3/m3] ) / [m3/m3] --> fraction
                       endif
...
!--------------------------------------------------------------------------
		...
		! NOTE: be careful with the unit. do not change the unit of delta in the loop
              ...
              do j=1,4
		         ...
		         MAX_THRESHOLD = jules50_struc(n)%jules50(t)%p_s_smvcst(1)
			      ...
			      jules50_struc(n)%jules50(t)%p_s_smvcst(j) !([-][m3/m3]-[m3/m3]) / [m3/m3] --> [-]
                    ...
                    tmpval = jules50_struc(n)%jules50(t)%p_s_sthu(j) * &
			        jules50_struc(n)%jules50(t)%p_s_smvcst(j) ! [-][m3/m3]
...
!                 write(LIS_logunit,*) '[ERR] Please adjust the perturbation settings ..'
			  ...
			  MAX_THRESHOLD = jules50_struc(n)%jules50(t)%p_s_smvcst(1)                       
                       if(jules50_struc(n)%jules50(t)%p_s_sthu(j)* &
			     jules50_struc(n)%jules50(t)%p_s_smvcst(j).gt.MAX_THRESHOLD.or.&
                          jules50_struc(n)%jules50(t)%smcl_soilt(j) * &
			     1/dzsoil(j)*1/1000.gt.MAX_THRESHOLD) then 
                       
                          jules50_struc(n)%jules50(t)%p_s_sthu(j) = MAX_THRESHOLD / &
			     jules50_struc(n)%jules50(t)%p_s_smvcst(j) ! [m3/m3]/[m3/m3] --> [-]  

                          jules50_struc(n)%jules50(t)%smcl_soilt(j) = MAX_THRESHOLD / & 
			     (1/dzsoil(j)*1/1000)   ! [m3w/m3s] / ([1/m1s][m3w/kg]) --> kg/m2s
                       endif
                       
                       if(jules50_struc(n)%jules50(t)%p_s_sthu(j)* &
			     jules50_struc(n)%jules50(t)%p_s_smvcst(j).lt.MIN_THRESHOLD.or.&
                          jules50_struc(n)%jules50(t)%smcl_soilt(j)* & 
			     1/dzsoil(j)*1/1000.lt.MIN_THRESHOLD) then   
                     
                          jules50_struc(n)%jules50(t)%p_s_sthu(j) = MIN_THRESHOLD / &
			     jules50_struc(n)%jules50(t)%p_s_smvcst(j) ! [m3/m3]/[m3/m3] --> [-] 

                          jules50_struc(n)%jules50(t)%smcl_soilt(j) = MIN_THRESHOLD / & 
			     (1/dzsoil(j)*1/1000)   ! [m3w/m3s] / ([1/m1s][m3w/kg]) --> kg/m2s
                       endif
                    !print*, i, m
