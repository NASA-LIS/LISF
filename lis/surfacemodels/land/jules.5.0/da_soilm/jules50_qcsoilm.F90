...
!BOP
! !ROUTINE: jules50_qcsoilm
! \label{jules50_qcsoilm}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules50_qcsoilm(n, LSM_State)
...
!  real, pointer          :: soilm4(:)
  ...
  call LIS_verify(status,&
       "ESMF_StateGet for Soil Moisture Layer 1 failed in jules50_qcsoilm")
 
  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet for Soil Moisture Layer 1 failed in jules50_qcsoilm")

  call ESMF_AttributeGet(sm1Field,"Max Value",smmax1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Max Value failed in jules50_qcsoilm")
  call ESMF_AttributeGet(sm1Field,"Min Value",smmin1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Min Value failed in jules50_qcsoilm")

  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)

! MN: the unit of the smmax1, smmin1 It comes from jules_sm_attribs.txt 
! MN: the unit of the smmax1, smmin1 are [m3/m3] they are comming from the  It comes from jules_sm_attribs.txt 
...
end subroutine jules50_qcsoilm
