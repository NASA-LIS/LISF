...
!BOP
! !ROUTINE: jules50_getLbandTbPred
! \label{jules50_getLbandTbPred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine jules50_getLbandTbPred(n, obs_pred)
! !USES:
  use ESMF
...
!call the forward model
  ...
  call LIS_verify(status, &
       "Error in StateGet in jules50_getLbandTbPred for Lband_Hpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbH, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in jules50_getLbandTbPred for Lband_Hpol')

  call ESMF_StateGet(LIS_forwardState(n), "Lband_Vpol", varField, rc=status)
  call LIS_verify(status, &
       "Error in StateGet in jules50_getLbandTbPred for Lband_Vpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbV, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in jules50_getLbandTbPred for Lband_Vpol')
...
end subroutine jules50_getLbandTbPred
