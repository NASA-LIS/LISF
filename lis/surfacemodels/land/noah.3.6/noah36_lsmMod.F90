...
! control the operation of Noah-3.6 LSM. It also provides the entry
! method for the initialization of Noah-3.6-specific variables.
...
! \begin{description}
!  \item[rfile]
!    name of the Noah-3.6 restart file
!  \item[vfile]
...
!  \item[noah]
!   Noah-3.6 LSM specific variables
! \end{description} 
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------------
  public :: noah36_lsm_ini
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: noah36_struc
!EOP
  type, public ::  noah36_type_dec 
...
!     real, allocatable    :: cmcmaxtbl(:)
     ...
     integer                  :: forc_count
     real                     :: ts
     type(noah36dec), allocatable :: noah(:)
  end type noah36_type_dec

  type(noah36_type_dec), allocatable :: noah36_struc(:)
  SAVE
contains
!BOP
!
! !ROUTINE: noah36_lsm_ini
! \label{noah36_lsm_ini}
!
! !INTERFACE:
  subroutine noah36_lsm_ini()
! !USES:
...
!  \begin{description}
!   \item[noah36\_readcrd](\ref{noah36_readcrd}) \newline
...
!EOP
    ...
    allocate(noah36_struc(LIS_rc%nnest))
    call noah36_readcrd()
    do n=1,LIS_rc%nnest
       allocate(noah36_struc(n)%noah(LIS_rc%npatch(n,LIS_rc%lsm_index)))
       do i=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
          allocate(noah36_struc(n)%noah(i)%stc(noah36_struc(n)%nslay))
          allocate(noah36_struc(n)%noah(i)%smc(noah36_struc(n)%nslay))
          allocate(noah36_struc(n)%noah(i)%sh2o(noah36_struc(n)%nslay))
          allocate(noah36_struc(n)%noah(i)%relsmc(noah36_struc(n)%nslay))
       enddo
...
!------------------------------------------------------------------------
       ...
       write(fnest,'(i3.3)') n
       call LIS_registerAlarm("Noah36 model alarm "//trim(fnest),&
            noah36_struc(n)%ts,&
            noah36_struc(n)%ts)

       call LIS_registerAlarm("Noah36 restart alarm "//trim(fnest),&
            noah36_struc(n)%ts,&
            noah36_struc(n)%rstInterval)
...
! Will be set to q2 for the first timestep in noah36_main
    do n=1,LIS_rc%nnest
       do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
          ...
          noah36_struc(n)%noah(t)%wchange_prev = 0 
       enddo
    enddo

  end subroutine noah36_lsm_ini

end module noah36_lsmMod
