...
! 
! !DESCRIPTION: 
!  This routine transfers the LIS provided forcing onto the Noah-3.6
!  model tiles. 
! 
!  The arguments are: 
!  \begin{description}
...
! if not, use the lowest model layer as the height.
     ...
     do v=1,LIS_FORC_Forc_Hgt%vlevels
        call ESMF_StateGet(LIS_FORC_State(n),(LIS_FORC_Forc_Hgt%varname(v)),&
             fhgtField, rc=status)
        call LIS_verify(status,'noah36_f2t: error getting Forc_Hgt')
        call ESMF_FieldGet(fhgtField,localDE=0,farrayPtr=fheight,rc=status)
        fheight1(v,:) = fheight(:)
        call LIS_verify(status,'noah36_f2t: error retrieving fheight')
     enddo

     call noah36_find_forcing_heights(LIS_FORC_Forc_Hgt%vlevels, LIS_rc%ntiles(n),&
          fheight1, noah36_struc(n)%zh, noah36_struc(n)%zm, layer_h, layer_m)

     if (LIS_rc%ntiles(n).ne.0) then
        noah36_struc(n)%zh = fheight1(layer_h(1),1)
        noah36_struc(n)%zm = fheight1(layer_m(1),1)
     endif
...
! corresponds to the reference heights specified in the config file (zh, zm)
  ...
  if(LIS_FORC_Snowf%selectOpt.eq.1) then
     call ESMF_StateGet(LIS_FORC_State(n),(LIS_FORC_Snowf%varname(1)),snowfField,&
          rc=status)
     call LIS_verify(status,'noah36_f2t: error getting Snowf')
  endif

  if ( LIS_FORC_Ch%selectOpt.eq.1 ) then
     noah36_struc(n)%forcing_ch = 1
     call ESMF_StateGet(LIS_FORC_State(n),(LIS_FORC_Ch%varname(1)),&
          acondField,rc=status)
     call LIS_verify(status,'noah36_f2t: error getting ch')
  else
     noah36_struc(n)%forcing_ch = 0
  endif

  if ( LIS_FORC_Cm%selectOpt.eq.1 ) then
     noah36_struc(n)%forcing_cm = 1
  else
     noah36_struc(n)%forcing_cm = 0
  endif

  if ( LIS_FORC_GVF%selectOpt.eq.1 ) then
     call ESMF_StateGet(LIS_FORC_State(n),(LIS_FORC_GVF%varname(1)),&
          gvfField,rc=status)
     call LIS_verify(status,'noah36_f2t: error getting GVF')
  endif
  if ( LIS_FORC_Alb%selectOpt.eq.1 ) then
     call ESMF_StateGet(LIS_FORC_State(n),(LIS_FORC_Alb%varname(1)),&
          albField,rc=status)
     call LIS_verify(status,'noah36_f2t: error getting Alb')
  endif
  if ( LIS_FORC_Z0%selectOpt.eq.1 ) then
     call ESMF_StateGet(LIS_FORC_State(n),(LIS_FORC_Z0%varname(1)),&
          z0Field,rc=status)
     call LIS_verify(status,'noah36_f2t: error getting Z0')
  endif

  call ESMF_FieldGet(tmpField, localDE=0, farrayPtr= tmp,rc=status)
  call LIS_verify(status, 'noah36_f2t: error retrieving tmp')

  call ESMF_FieldGet(q2Field,localDE=0, farrayPtr=q2,rc=status)
  call LIS_verify(status,'noah36_f2t: error retrieving q2')

  call ESMF_FieldGet(swdField,localDE=0, farrayPtr=swd,rc=status)
  call LIS_verify(status,'noah36_f2t: error retrieving swd')

  call ESMF_FieldGet(lwdField,localDE=0, farrayPtr=lwd,rc=status)
  call LIS_verify(status,'noah36_f2t: error retrieving lwd')

  call ESMF_FieldGet(uField,localDE=0, farrayPtr=uwind,rc=status)
  call LIS_verify(status,'noah36_f2t: error retrieving u')

  call ESMF_FieldGet(vField,localDE=0, farrayPtr=vwind,rc=status)
  call LIS_verify(status,'noah36_f2t: error retrieving v')

  call ESMF_FieldGet(psurfField,localDE=0, farrayPtr=psurf,rc=status)
  call LIS_verify(status,'noah36_f2t: error retrieving psurf')

  call ESMF_FieldGet(pcpField,localDE=0, farrayPtr=pcp,rc=status)
  call LIS_verify(status,'noah36_f2t: error retrieving pcp')

  if(LIS_FORC_CRainf%selectOpt.eq.1) then
     call ESMF_FieldGet(cpcpField,localDE=0, farrayPtr=cpcp,rc=status)
     call LIS_verify(status, 'noah36_f2t: error retrieving cpcp')
  endif

  if(LIS_FORC_Forc_Hgt%selectOpt.eq.1) then
     call ESMF_FieldGet(fhgtField,localDE=0, farrayPtr=fheight,rc=status)
     call LIS_verify(status,'noah36_f2t: error retrieving forc_hgt')
  endif

  if(LIS_FORC_Snowf%selectOpt.eq.1) then
     call ESMF_FieldGet(snowfField,localDE=0, farrayPtr=snowf,rc=status)
     call LIS_verify(status,'noah36_f2t: error retrieving snowf')
  endif

  if ( LIS_FORC_Ch%selectOpt.eq.1 ) then
     call ESMF_FieldGet(acondField,localDE=0, farrayPtr=acond,rc=status)
     call LIS_verify(status,'noah36_f2t: error retrieving acond')
  endif

 if ( LIS_FORC_GVF%selectOpt.eq.1 ) then
     call ESMF_FieldGet(gvfField,localDE=0, farrayPtr=gvf,rc=status)
     call LIS_verify(status,'noah36_f2t: error retrieving gvf')
  endif
  if ( LIS_FORC_Alb%selectOpt.eq.1 ) then
     call ESMF_FieldGet(albField,localDE=0, farrayPtr=alb,rc=status)
     call LIS_verify(status,'noah36_f2t: error retrieving alb')
  endif
  if ( LIS_FORC_Z0%selectOpt.eq.1 ) then
     call ESMF_FieldGet(z0Field,localDE=0, farrayPtr=z0,rc=status)
     call LIS_verify(status,'noah36_f2t: error retrieving z0')
  endif

  noah36_struc(n)%forc_count  =   noah36_struc(n)%forc_count  + 1

  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     !transform t to the patch
     tid = LIS_surface(n,LIS_rc%lsm_index)%tile(t)%tile_id 
     noah36_struc(n)%noah(t)%tair=noah36_struc(n)%noah(t)%tair+ tmp(tid)
     noah36_struc(n)%noah(t)%qair=noah36_struc(n)%noah(t)%qair + q2(tid)
     noah36_struc(n)%noah(t)%swdown=noah36_struc(n)%noah(t)%swdown + swd(tid)
     noah36_struc(n)%noah(t)%lwdown=noah36_struc(n)%noah(t)%lwdown + lwd(tid)
     noah36_struc(n)%noah(t)%uwind=noah36_struc(n)%noah(t)%uwind + uwind(tid)
     noah36_struc(n)%noah(t)%vwind=noah36_struc(n)%noah(t)%vwind + vwind(tid)
     noah36_struc(n)%noah(t)%psurf=noah36_struc(n)%noah(t)%psurf + psurf(tid)
     if ( LIS_FORC_Forc_Hgt%selectOpt.eq.1) then
        noah36_struc(n)%noah(t)%fheight=fheight(tid)
     endif
     ...
     if ( LIS_FORC_Ch%selectOpt.eq.1) then
        if(acond(tid).le.0) then
           noah36_struc(n)%noah(t)%ch=noah36_struc(n)%noah(t)%ch + 0.00001
        else
           noah36_struc(n)%noah(t)%ch=noah36_struc(n)%noah(t)%ch + acond(tid)
        endif
     endif

     if(pcp(tid).ne.LIS_rc%udef) then
        noah36_struc(n)%noah(t)%rainf=noah36_struc(n)%noah(t)%rainf + pcp(tid)
     else
        noah36_struc(n)%noah(t)%rainf=noah36_struc(n)%noah(t)%rainf + 0.0
     endif
     ...
     if ( LIS_FORC_CRainf%selectOpt.eq.1) then
        if(cpcp(tid).ne.LIS_rc%udef) then
           noah36_struc(n)%noah(t)%rainf_c=noah36_struc(n)%noah(t)%rainf_c+ cpcp(tid)
        else
           noah36_struc(n)%noah(t)%rainf_c=noah36_struc(n)%noah(t)%rainf_c + 0.0
        endif
     endif
...
! separate rainf and snowf.  It determines what to do with precipitation.
     if ( LIS_FORC_Snowf%selectOpt.eq.1) then
        if(snowf(tid).ne.LIS_rc%udef) then
           noah36_struc(n)%noah(t)%rainf=noah36_struc(n)%noah(t)%rainf + & 
                snowf(tid)
        endif
     endif
     noah36_struc(n)%noah(t)%snowf = noah36_struc(n)%noah(t)%snowf+ 0.0
     if(LIS_FORC_GVF%selectOpt.eq.1) then 
        noah36_struc(n)%noah(t)%shdfac=noah36_struc(n)%noah(t)%shdfac + & 
             gvf(tid)
     endif

     if(LIS_FORC_ALB%selectOpt.eq.1) then 
        noah36_struc(n)%noah(t)%alb=noah36_struc(n)%noah(t)%alb + & 
             alb(tid)
     endif
     
     if(LIS_FORC_Z0%selectOpt.eq.1) then 
        noah36_struc(n)%noah(t)%z0=noah36_struc(n)%noah(t)%z0 + & 
             z0(tid)
     endif
  enddo
end subroutine noah36_f2t

subroutine noah36_find_forcing_heights(vlevels, ntiles, fheight, zh, &
     zm, layer_h, layer_m)
...
end subroutine noah36_find_forcing_heights
