...
!BOP
! !ROUTINE: noah36_map_swe
! \label{noah36_map_swe}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah36_map_swe(n,OBS_State,LSM_State)
! !USES:
  use ESMF
...
! Based on SWE, we manually update snowdepth    
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if(sweobs(LIS_surface(n,1)%tile(t)%index).gt.0) then 
        ...
        newsn = swe(t)
        snowhc = noah36_struc(n)%noah(t)%snowh*100.0
        newsnc = newsn*100.0
