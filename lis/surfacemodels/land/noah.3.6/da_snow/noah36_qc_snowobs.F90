...
!BOP
! !ROUTINE: noah36_qc_snowobs
! \label{noah36_qc_snowobs}
!
! !REVISION HISTORY:
! 25Feb2008: Sujay Kumar: Initial Specification
!
! !INTERFACE:
subroutine noah36_qc_snowobs(n,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(snowobs(gid).ne.LIS_rc%udef) then 
        ...
        elseif(noah36_struc(n)%noah(t)%vegt.le.4) then !forest types
           snowobs(gid) = LIS_rc%udef
!assume that snow will not form at 5 deg. celcius or higher ground temp. 
       elseif(noah36_struc(n)%noah(t)%t1.ge.278.15) then 
           snowobs(gid) = LIS_rc%udef
       elseif(noah36_struc(n)%noah(t)%stc(1).ge.278.15) then 
           snowobs(gid) = LIS_rc%udef
        endif
     endif
  enddo

end subroutine noah36_qc_snowobs
