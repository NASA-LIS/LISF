...
!BOP
! !ROUTINE: noah36_transform_snow
! \label{noah36_transform_snow}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah36_transform_snow(n,OBS_State)
...
!EOP
  ...
  call LIS_verify(status, 'attributeget error in noah36_transform_snow')
  call ESMF_StateGet(OBS_State,"Observation01",obs_SCA_field,&
       rc=status)
  call LIS_verify(status,'stateget error in noah36_transform_snow')
  call ESMF_FieldGet(obs_SCA_field,localDE=0,farrayPtr=SCAobs,rc=status)
  call LIS_verify(status,'fieldget error in noah36_transform_snow')
...
end subroutine noah36_transform_snow
