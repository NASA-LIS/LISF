...
!BOP
! !ROUTINE: noah36_map_snow_DI
! \label{noah36_map_snow_DI}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah36_map_snow_DI(n,OBS_State,LSM_Incr_State)
! !USES:
  use ESMF
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(scaobs(LIS_domain(n)%tile(t)%index).ne.-9999.0) then 
        ...
        elseif((scaobs(LIS_domain(n)%tile(t)%index).lt.ANSASCFsnow_struc(n)%di_scaobs3).and.&
           swe(t).gt.noah36_struc(n)%noah(t)%snup) then
           melt_rate = min(ANSASCFsnow_struc(n)%di_maxmelt/1000, swe(t)/ANSASCFsnow_struc(n)%di_ndaymelt) 
        ...
        if (swe(t).le.0.0) then 
            ...
            ! get snow density computed by Noah-3.6
            sndens = noah36_struc(n)%noah(t)%sndens
...
end subroutine noah36_map_snow_DI
