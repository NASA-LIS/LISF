...
!BOP
! !ROUTINE: noah36_coldstart
! \label{noah36_coldstart}
!
! !REVISION HISTORY:
...
! 
! !INTERFACE:
subroutine noah36_coldstart(mtype)
! !USES:
...
! !DESCRIPTION:
!  
!  This routine initializes the Noah-3.6 state variables with some 
...
!EOP
   ...
   do n=1,LIS_rc%nnest
      if (trim(LIS_rc%startcode).eq."coldstart") then
         write(LIS_logunit,*)                                          &
               '[INFO] noah36_coldstart -- cold-starting Noah-3.6'
         do t=1,LIS_rc%npatch(n,mtype)
            noah36_struc(n)%noah(t)%sigma_sm=0.04 ! needed for MCMC uncertainty analysis
            noah36_struc(n)%noah(t)%t1=noah36_struc(n)%initskintemp
            noah36_struc(n)%noah(t)%cmc=noah36_struc(n)%initcanopywater
            noah36_struc(n)%noah(t)%snowh=noah36_struc(n)%initsnowdepth
            noah36_struc(n)%noah(t)%sneqv=noah36_struc(n)%initsnowequiv
            noah36_struc(n)%noah(t)%ch=1.E-4
            noah36_struc(n)%noah(t)%cm=1.E-4
            do l=1,noah36_struc(n)%nslay
               noah36_struc(n)%noah(t)%stc(l)=noah36_struc(n)%inittemp(l)
               noah36_struc(n)%noah(t)%smc(l)=noah36_struc(n)%initsm(l)
               noah36_struc(n)%noah(t)%sh2o(l)=noah36_struc(n)%initsmliq(l)
            enddo
            noah36_struc(n)%noah(t)%emiss = 0.96
            noah36_struc(n)%noah(t)%snotime1 = 0.0
         enddo
      endif
      ...
      write(LIS_logunit,*) '[INFO] noah36_coldstart -- ',              &
                         'Using the specified start time ',LIS_rc%time
   enddo
end subroutine noah36_coldstart
