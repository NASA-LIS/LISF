...
!BOP
! !ROUTINE: noah36_wrf_f2t
! \label{noah36_wrf_f2t}
!
! !REVISION HISTORY:
...
!  call LIS_verify(status, 'Error in ESMF_StateGet: Snowf_f')
  ...
  noah36_struc(n)%forcing_ch = 1 ! EMK NUWRF
...
!  call LIS_verify(status)
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     noah36_struc(n)%noah(t)%tair=tmp(tid)
     noah36_struc(n)%noah(t)%qair=q2(tid)
     noah36_struc(n)%noah(t)%swdown=swd(tid)
     noah36_struc(n)%noah(t)%lwdown=lwd(tid)
     noah36_struc(n)%noah(t)%uwind=uwind(tid)
     noah36_struc(n)%noah(t)%vwind=vwind(tid)
     noah36_struc(n)%noah(t)%psurf=psurf(tid)
     noah36_struc(n)%noah(t)%xice=xice(tid)
     if(pcp(t).ne.LIS_rc%udef) then 
        noah36_struc(n)%noah(t)%rainf=pcp(tid)
     else
        noah36_struc(n)%noah(t)%rainf=0.0
     endif
     noah36_struc(n)%noah(t)%snowf=0.0

     if(ch(tid).ne.LIS_rc%udef) then 
        noah36_struc(n)%noah(t)%ch=ch(tid)
     endif

     if(chs2(tid).ne.LIS_rc%udef) then 
        noah36_struc(n)%noah(t)%chs2=chs2(tid)
     endif

     if(cqs2(tid).ne.LIS_rc%udef) then 
        noah36_struc(n)%noah(t)%cqs2=cqs2(tid)
     endif

     if(q2sat(tid).ne.LIS_rc%udef) then 
        noah36_struc(n)%noah(t)%q2sat=q2sat(tid)
     endif

     if(emiss(tid).ne.LIS_rc%udef) then 
        noah36_struc(n)%noah(t)%emiss=emiss(tid)
     endif

     if(zval(tid).ne.LIS_rc%udef) then 
        noah36_struc(n)%noah(t)%z=zval(tid)
     endif
     noah36_struc(n)%forc_count = 1
  enddo

end subroutine noah36_wrf_f2t
