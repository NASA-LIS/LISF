...
! !DESCRIPTION:
!
!  Forcing-only option (jules53) for calling the forcing transfer routines.
! 
!  The arguments are: 
!  \begin{description}
!  \item[n]
...
!EOP
  ...
  jules53_struc(n)%forc_count = jules53_struc(n)%forc_count + 1 
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     jules53_struc(n)%jules53(t)%tair   = jules53_struc(n)%jules53(t)%tair   + tmp(tid)
     jules53_struc(n)%jules53(t)%qair   = jules53_struc(n)%jules53(t)%qair   + q2(tid)
     jules53_struc(n)%jules53(t)%swdown = jules53_struc(n)%jules53(t)%swdown + swd(tid)
     jules53_struc(n)%jules53(t)%lwdown = jules53_struc(n)%jules53(t)%lwdown + lwd(tid)
     jules53_struc(n)%jules53(t)%wind_e = jules53_struc(n)%jules53(t)%wind_e + uwind(tid)
     jules53_struc(n)%jules53(t)%wind_n = jules53_struc(n)%jules53(t)%wind_n + vwind(tid)
     jules53_struc(n)%jules53(t)%psurf  = jules53_struc(n)%jules53(t)%psurf  + psurf(tid)
     jules53_struc(n)%jules53(t)%rainf  = jules53_struc(n)%jules53(t)%rainf  + rain(tid)
     
     if(LIS_FORC_CRainf%selectOpt .eq. 1) then
       if(crain(tid) .ne. LIS_rc%udef) then 
          jules53_struc(n)%jules53(t)%rainf_c = jules53_struc(n)%jules53(t)%rainf_c + crain(tid)
       else
          jules53_struc(n)%jules53(t)%rainf_c = jules53_struc(n)%jules53(t)%rainf_c + 0.0
       endif
     else
     ...
     if(LIS_FORC_Snowf%selectOpt .eq. 1) then
        if(snowf(tid) .ne. LIS_rc%udef) then 
          jules53_struc(n)%jules53(t)%snowf = jules53_struc(n)%jules53(t)%snowf + snowf(tid)
        else
          jules53_struc(n)%jules53(t)%snowf = jules53_struc(n)%jules53(t)%snowf + 0.0
        endif
     else
