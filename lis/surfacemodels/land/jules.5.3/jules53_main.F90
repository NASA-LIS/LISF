...
!BOP
!
! !ROUTINE: jules53_main
! \label{jules53_main}
!
! !ROUTINE: jules53_main.F90
!
! !REVISION HISTORY:
...
! !DESCRIPTION:
!
!  Calls the run routines for the forcing-only option (jules53)
!
!  The arguments are:
!  \begin{description}
!  \item[n]
...
!EOP
   ...
   alarmCheck = LIS_isAlarmRinging(LIS_rc, "JULES.5.3 model alarm")
   if (alarmCheck) Then
         ...
         do
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                 ...
                 tstar_box=tstar_box+jules53_struc(n)%jules53(t)%frac(pft)*jules53_struc(n)%jules53(t)%tstar_tile(pft)
               endif
            enddo
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                  ...
                  jules53_struc(n)%jules53(t)%tstar=tstar_box
            ...
            do t=start_k, end_k
               if ( LIS_surface(n,LIS_rc%lsm_index)%tile(t)%ensem == m ) then
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,         &
                        LIS_MOC_SOILTEMP,                            &
                        value=jules53_struc(n)%jules53(t)%t_soil(i), &
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,       &
                        LIS_MOC_SOILMOIST,                         &
                        value=jules53_struc(n)%jules53(t)%smcl_soilt(i), &
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     ...
                     smc_liq = jules53_struc(n)%jules53(t)%p_s_sthu(i) * jules53_struc(n)%jules53(t)%p_s_smvcst(i) 
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_SMLIQFRAC,                             &
                        value=smc_liq,                                 &
                        vlevel=i, unit="m^3 m-3", direction="-",       &
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     ...
                     smc_sat_frac = jules53_struc(n)%jules53(t)%p_s_sthu(i)+jules53_struc(n)%jules53(t)%p_s_sthf(i)
                     if (smc_sat_frac > tiny(smc_sat_frac)) then 
                        smc_liq = jules53_struc(n)%jules53(t)%p_s_sthu(i) / smc_sat_frac
                     else
                        smc_liq = 0.0
                     endif 
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     smc_ice = jules53_struc(n)%jules53(t)%p_s_sthf(i) * jules53_struc(n)%jules53(t)%p_s_smvcst(i) 
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_SMFROZFRAC,                            &
                        value=smc_ice,                                 &
                        vlevel=i, unit="m^3 m-3", direction="-",       &
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     ...
                     smc_sat_frac = jules53_struc(n)%jules53(t)%p_s_sthu(i)+jules53_struc(n)%jules53(t)%p_s_sthf(i)
                     if (smc_sat_frac > tiny(smc_sat_frac)) then
                        smc_ice = jules53_struc(n)%jules53(t)%p_s_sthf(i) / smc_sat_frac
                     else
                        smc_ice = 0.0
                     endif
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_JULES_STHU,                            &
                        value=jules53_struc(n)%jules53(t)%p_s_sthu(i), &
                        vlevel=i, unit="-", direction="-",             &
                        valid_min=0.0, valid_max=1.0,                  &
                        surface_type=LIS_rc%lsm_index)
                  enddo
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n, t,           &
                        LIS_MOC_JULES_STHF,                            &
                        value=jules53_struc(n)%jules53(t)%p_s_sthf(i), &
                        vlevel=i, unit="-", direction="-",             &
                        valid_min=0.0, valid_max=1.0,                  &
                        surface_type=LIS_rc%lsm_index)
                  enddo
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,               &
                     ...
                     value=jules53_struc(n)%jules53(t)%p_s_sthu_min(1), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules53_struc(n)%jules53(t)%p_s_smvccl(1), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules53_struc(n)%jules53(t)%p_s_smvcst(1), &
                     vlevel=1, unit="m^3 m-3", direction="-",         &
                     valid_min=0.0, valid_max=1.0,                    &
                        surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,             &
                     ...
                     value=jules53_struc(n)%jules53(t)%p_s_smvcwt(1), &
                     vlevel=1, unit="m^3 m-3", direction="-",         &
                     valid_min=0.0, valid_max=1.0,                    &
                        surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_EVAP,                              &
                     value=jules53_struc(n)%jules53(t)%fqw_1,   &
                     vlevel=1,unit="kg m-2 s-1",direction="UP", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,     &
                     LIS_MOC_QH,                              &
                     value=jules53_struc(n)%jules53(t)%ftl_1, &
                     vlevel=1,unit="W m-2",direction="UP",    &
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,         &
                     LIS_MOC_ECANOP,                             &
                     value=jules53_struc(n)%jules53(t)%ecan,     &
                     vlevel=1,unit="kg m-2 s-1", direction="UP", &
                     surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n,t,         &
                     LIS_MOC_JULES_ESOIL,                             &
                     value=jules53_struc(n)%jules53(t)%esoil,     &
                     vlevel=1,unit="kg m-2 s-1", direction="UP", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,       &
                     LIS_MOC_SUBSNOW,                          &
                     value=jules53_struc(n)%jules53(t)%ei,     &
                  ...
                  do i=1, jules53_struc(n)%sm_levels
                     call LIS_diagnoseSurfaceOutputVar(n,t,             &
                        LIS_MOC_SOILET,                                 &
                        vlevel=i,                                       &
                        value=jules53_struc(n)%jules53(t)%ext(i),       &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,               &
                     LIS_MOC_SNOWDEPTH,                                &
                     vlevel=1,                                         &
                     value=jules53_struc(n)%jules53(t)%snowdepth(pft), &
                     unit="m", direction="-",&
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_SWE,                                       &
                      value =  jules53_struc(n)%jules53(t)%snow_mass_ij, &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,              &
                      LIS_MOC_WATERTABLED,                             &
                      value = JULES53_struc(n)%jules53(t)%zw,          &
                      vlevel=1, unit="m", direction="-",               &
                      surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,                &
                     LIS_MOC_AVGSURFT,                                  &
                     value=jules53_struc(n)%jules53(t)%tstar_tile(pft), &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,               &
                     LIS_MOC_EMISSFORC,                                 &
                     value = jules53_struc(n)%jules53(t)%emis_tile(pft),&
                     vlevel=1, unit="-", direction="-",                 &
                     surface_type = LIS_rc%lsm_index)

                  !***  surface radiative temperature 
                  trad_gb_mean = 0.0
                  do p=1, jules53_struc(n)%jules53(t)%nsurft
                    trad_gb_mean = trad_gb_mean + frac(1,p) * jules53_struc(n)%jules53(t)%tstar_tile(p)**4.0
                  enddo
                  ...
                  lw_upward = sbcon * jules53_struc(n)%jules53(t)%emis_tile(pft) * jules53_struc(n)%jules53(t)%tstar_tile(pft)**4 
                  lw_net    = jules53_struc(n)%jules53(t)%emis_tile(pft) * jules53_struc(n)%jules53(t)%lwdown - lw_upward 
                  call LIS_diagnoseSurfaceOutputVar(n, t,                          &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,                 &
                     LIS_MOC_ROUGHNESS,                                  &
                     value=jules53_struc(n)%jules53(t)%p_s_z0_tile(pft), &
                     vlevel=1,unit="m",direction="-",                    &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n,t,                       &
                     LIS_MOC_THERMAL_ROUGHNESS,                                &
                     value=jules53_struc(n)%jules53(t)%p_s_z0_tile(pft) * z0h_z0m(pft), &
                     vlevel=1,unit="m",direction="-",                          &
                     surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QSB,                                             &
                     value = jules53_struc(n)%jules53(t)%sub_surf_roff,       &
                     vlevel=1, unit="kg m-2 s-1", direction="OUT", surface_type = LIS_rc%lsm_index)
                  
                  ! EMK
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_QS,                              &
                     value=jules53_struc(n)%jules53(t)%surf_roff*dt,   &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_QSB,                              &
                     value=jules53_struc(n)%jules53(t)%sub_surf_roff*dt,   &
                     vlevel=1,unit="kg m-2",direction="OUT", &
                     surface_type=LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QS,                                              &
                     value = jules53_struc(n)%jules53(t)%surf_roff,           &
                     vlevel=1, unit="kg m-2 s-1", direction="OUT", surface_type = LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_QSM,                                             &
                     value = jules53_struc(n)%jules53(t)%snow_melt_gb,        &
                     vlevel=1, unit="kg m-2 s-1", direction="S2L", surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_DRIP,                                            &
                     value = jules53_struc(n)%jules53(t)%tot_tfall,           &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_CANOPINT,                                        &
                     value = jules53_struc(n)%jules53(t)%canopy(pft),      &
                     vlevel=1, unit="kg m-2", direction="-", &
                     surface_type = LIS_rc%lsm_index)

                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_SWNET,                                           &
                     value= jules53_struc(n)%jules53(t)%sw_tile(pft),         &  
                     vlevel=1,  unit="W m-2", direction="DN", surface_type=LIS_rc%lsm_index)

                  if(pft <= jules53_struc(n)%npft) then  
                    ! vegetation surface types 
                    call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                       LIS_MOC_LAI,                                             &
                       value= jules53_struc(n)%jules53(t)%lai(pft),             &  
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_JULES_FSAT,                                             &
                     value= jules53_struc(n)%jules53(t)%fsat,             &  
                     vlevel=1,  unit="-", direction="-", surface_type=LIS_rc%lsm_index)
                  
                  call LIS_diagnoseSurfaceOutputVar(n, t,                     &
                     LIS_MOC_JULES_FWETL,                                             &
                     value= jules53_struc(n)%jules53(t)%fwetl,             &  
                     vlevel=1,  unit="-", direction="-", surface_type=LIS_rc%lsm_index)
...
! EMK BEGIN New output variables
                  ...
                  sfcprs = jules53_struc(n)%jules53(t)%psurf
                  sfctmp = jules53_struc(n)%jules53(t)%tair
                  q2 = jules53_struc(n)%jules53(t)%qair
                  es = 611.0*exp(2.501E6/461.0*(1./273.15 - 1./sfctmp))
                  q2sat = 0.622*es/(sfcprs-(1.-0.622)*es)
                  if (sfctmp .lt. &
                       jules53_struc(n)%jules53(t)%tair_agl_min) then
                     jules53_struc(n)%jules53(t)%tair_agl_min = sfctmp
                     jules53_struc(n)%jules53(t)%rhmin = q2 / q2sat
                  end if

                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_RHMIN,                              &
                     value=jules53_struc(n)%jules53(t)%rhmin,   &
                     vlevel=1,unit="-",direction="-", &
                     surface_type=LIS_rc%lsm_index)
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_RHMIN,                              &
                     value=(jules53_struc(n)%jules53(t)%rhmin*100),   &
                     vlevel=1,unit="%",direction="-", &
                     surface_type=LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n,t,        &
                     LIS_MOC_SNOW_SOOT,                          &
                     value=jules53_struc(n)%jules53(t)%soot_ij,   &
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_GRND_SNOW,                                       &
                      value =  jules53_struc(n)%jules53(t)%snow_grnd(pft), &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t,                &
                      LIS_MOC_SURFT_SNOW,                                       &
                      value =  jules53_struc(n)%jules53(t)%snow_tile(pft), &
                      vlevel=1, unit="kg m-2", direction="-",            &
                      surface_type = LIS_rc%lsm_index)
                  
                  ! bulk snow density 
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDENSITY, &
                      value =  jules53_struc(n)%jules53(t)%rho_snow_grnd(pft),&
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWGRAIN, &
                      value =  jules53_struc(n)%jules53(t)%rgrain(pft),&
                  ...
                  if(nsmax .eq.0) then
                    ...
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWICEBULK,                         &
                       value=jules53_struc(n)%jules53(t)%sice(1,1), &
                       vlevel=1, unit="kg m-2", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                    ! snow liquid water 
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWLIQBULK,                         &
                       value=jules53_struc(n)%jules53(t)%sliq(1,1), &
                       vlevel=1, unit="kg m-2", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                    ! snow temperature
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SNOWT,                         &
                       value=jules53_struc(n)%jules53(t)%tsnow(1,1), &
                       vlevel=1, unit="K", direction="-",    &
                       surface_type=LIS_rc%lsm_index)
                  !!! nsmax > 0, multi-layer snow physics
                    ...
                    call LIS_diagnoseSurfaceOutputVar(n, t,       &
                       LIS_MOC_SOWN_NLAYER,                         &
                       value=jules53_struc(n)%jules53(t)%nsnow(pft)*1.0, & ! nsnow is integer, -> real by *1.0
                       vlevel=1, unit="-", direction="-",    &
                       surface_type=LIS_rc%lsm_index)

                    do i=1, jules53_struc(n)%nsmax
                      ! layer snow ice 
                       call LIS_diagnoseSurfaceOutputVar(n, t,       &
                          LIS_MOC_SNOWICEPROF,                         &
                          value=jules53_struc(n)%jules53(t)%sice(pft,i), &
                          vlevel=i, unit="kg m-2", direction="-",    &
                          surface_type=LIS_rc%lsm_index)
                      ! layer snow liquid water 
                       call LIS_diagnoseSurfaceOutputVar(n, t,       &
                          LIS_MOC_SNOWLIQPROF,                         &
                          value=jules53_struc(n)%jules53(t)%sliq(pft,i), &
                          vlevel=i, unit="kg m-2", direction="-",    &
                          surface_type=LIS_rc%lsm_index)
                      ! layer snow temperature K 
                       call LIS_diagnoseSurfaceOutputVar(n, t,            &
                          LIS_MOC_SNOWTPROF,                              &
                          value=jules53_struc(n)%jules53(t)%tsnow(pft,i), &
                          vlevel=i, unit="K", direction="-",              &
                          surface_type=LIS_rc%lsm_index)
                      ! layer snow density 
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDENPROF, &
                        value = jules53_struc(n)%jules53(t)%rho_snow(pft,i),&
                        vlevel=i, unit="kg m-3", direction="-", surface_type = LIS_rc%lsm_index)
                      
                      ! layer snow grain size 
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWGRNPROF, &
                        value =  jules53_struc(n)%jules53(t)%rgrainl(pft,i),&
                        vlevel=i, unit="micron", direction="-", surface_type = LIS_rc%lsm_index)

                      ! layer thickness of snow layers  
                      call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_SNOWDEPPROF, &
                        value = jules53_struc(n)%jules53(t)%ds(pft,i),& 
                  ...
                  snow_frac = 0.0
                  if(l_aggregate) then
                    if(jules53_struc(n)%jules53(t)%snow_tile(1)+jules53_struc(n)%jules53(t)%snow_grnd(1)>1.0) then
                      snow_frac = 1.0
                    endif
                  else
                    if(jules53_struc(n)%jules53(t)%snow_tile(pft)+jules53_struc(n)%jules53(t)%snow_grnd(pft)>1.0) then
                      snow_frac = jules53_struc(n)%jules53(t)%frac(pft)
                    endif
                  endif
                  ...
                  prcp = jules53_struc(n)%jules53(t)%rainf/jules53_struc(n)%forc_count
                  
                  if (jules53_struc(n)%jules53(t)%tair  .lt. 273.16) then 
                  ...
                  cpcp   = jules53_struc(n)%jules53(t)%rainf_c/jules53_struc(n)%forc_count
                  ...
                  albedo_land = wght_alb(1) * jules53_struc(n)%jules53(t)%land_albedo(1) &
                              + wght_alb(2) * jules53_struc(n)%jules53(t)%land_albedo(2) &
                              + wght_alb(3) * jules53_struc(n)%jules53(t)%land_albedo(3) &
                              + wght_alb(4) * jules53_struc(n)%jules53(t)%land_albedo(4) 
...
!                 ! JULES GPP and NPP
                  ...
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_GPP, value = 1000.0 * jules53_struc(n)%jules53(t)%gpp, &
                  ...
                  ! (g m-2 s-1) = 1000.0 * (kg m-2 s-1)
                  call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_NPP, value = 1000.0 * jules53_struc(n)%jules53(t)%npp, &
                                              vlevel=1, unit="g m-2 s-1", direction="IN", surface_type = LIS_rc%lsm_index)
!                ! End JULES GPP and NPP 

!                   !EMK Below code is borrowed from Noah 3.6
                  do i=1, jules53_struc(n)%sm_levels
                     smc = jules53_struc(n)%jules53(t)%smcl_soilt(i)/ &
                          (1000.0*dzsoil(i))
                     smcwilt = jules53_struc(n)%jules53(t)%sm_wilt
                     smcmax = jules53_struc(n)%jules53(t)%sm_sat
...
! EMK END New output variables
                  ...
                  jules53_struc(n)%jules53(t)%rainf_c = 0.0
   ...
   ! EMK...See if jules53_struc(n)%jules53(t)%tair_agl_min needs to be reset
   ! for calculating RHMin.  
   write(fnest,'(i3.3)') n
   alarmCheck = LIS_isAlarmRinging(LIS_rc, &
        "JULES.5.3 RHMin alarm "//trim(fnest))
   if (alarmCheck) then
      write(LIS_logunit,*)'[INFO] Resetting tair_agl_min for RHMin calculation'
     do t = 1,LIS_rc%npatch(n,LIS_rc%lsm_index)
        jules53_struc(n)%jules53(t)%tair_agl_min = 999.
     end do
  end if

end subroutine jules53_main
