...
!BOP
! !ROUTINE: noah33_setsoilm
!  \label{noah33_setsoilm}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah33_setsoilm(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!           print*, 'set1 ',t,noah33_struc(n)%noah(t)%smc(1),&
...
!     endif
 ...
 !       can get module variables MAXSMC and WLTSMC from the module_sf_noahlsm_36
     MAX_THRESHOLD = noah33_struc(n)%noah(t)%smcmax
     ...
     delta1 = soilm1(t)-noah33_struc(n)%noah(t)%smc(1)
     delta2 = soilm2(t)-noah33_struc(n)%noah(t)%smc(2)
     delta3 = soilm3(t)-noah33_struc(n)%noah(t)%smc(3)
     delta4 = soilm4(t)-noah33_struc(n)%noah(t)%smc(4)
     
     ! MN: check    MIN_THRESHOLD < volumetric liquid soil moisture < threshold 
     if(noah33_struc(n)%noah(t)%sh2o(1)+delta1.gt.MIN_THRESHOLD .and.&
          noah33_struc(n)%noah(t)%sh2o(1)+delta1.lt.&
          sm_threshold) then 
     ...
     if(noah33_struc(n)%noah(t)%sh2o(2)+delta2.gt.MIN_THRESHOLD .and.&
          noah33_struc(n)%noah(t)%sh2o(2)+delta2.lt.sm_threshold) then 
        update_flag(gid) = update_flag(gid).and.(.true.)
        update_flag_tile(t) = update_flag_tile(t).and.(.true.)
     else
        update_flag(gid) = update_flag(gid).and.(.false.)
     ...
     if(noah33_struc(n)%noah(t)%sh2o(3)+delta3.gt.MIN_THRESHOLD .and.&
          noah33_struc(n)%noah(t)%sh2o(3)+delta3.lt.sm_threshold) then 
        update_flag(gid) = update_flag(gid).and.(.true.)
        update_flag_tile(t) = update_flag_tile(t).and.(.true.)
     else
        update_flag(gid) = update_flag(gid).and.(.false.)
     ...
     if(noah33_struc(n)%noah(t)%sh2o(4)+delta4.gt.MIN_THRESHOLD .and.&
          noah33_struc(n)%noah(t)%sh2o(4)+delta4.lt.sm_threshold) then 
        update_flag(gid) = update_flag(gid).and.(.true.)
        update_flag_tile(t) = update_flag_tile(t).and.(.true.)
     else
        update_flag(gid) = update_flag(gid).and.(.false.)
...
! flag_tile = TRUE --> means met the both min and max threshold 
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           if(update_flag_tile(t)) then
              ...
              tmp1(m) = soilm1(t) !noah33_struc(n)%noah(t)%smc(1)
              tmp2(m) = soilm2(t) !noah33_struc(n)%noah(t)%smc(2)
              tmp3(m) = soilm3(t) !noah33_struc(n)%noah(t)%smc(3)
              tmp4(m) = soilm4(t) !noah33_struc(n)%noah(t)%smc(4)
           endif
        enddo
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           if(update_flag_tile(t)) then
              ...
              ! bottom layer 
              noah33_struc(n)%noah(t)%sh2o(1) = noah33_struc(n)%noah(t)%sh2o(1)+&
                   delta1
              noah33_struc(n)%noah(t)%smc(1) = soilm1(t)
              ...
              if(noah33_struc(n)%noah(t)%sh2o(2)+delta2.gt.MIN_THRESHOLD .and.&
                   noah33_struc(n)%noah(t)%sh2o(2)+delta2.lt.sm_threshold) then 
                 noah33_struc(n)%noah(t)%sh2o(2) = noah33_struc(n)%noah(t)%sh2o(2)+&
                      soilm2(t)-noah33_struc(n)%noah(t)%smc(2)
                 noah33_struc(n)%noah(t)%smc(2) = soilm2(t)
              ...
              if(noah33_struc(n)%noah(t)%sh2o(3)+delta3.gt.MIN_THRESHOLD .and.&
                   noah33_struc(n)%noah(t)%sh2o(3)+delta3.lt.sm_threshold) then 
                 noah33_struc(n)%noah(t)%sh2o(3) = noah33_struc(n)%noah(t)%sh2o(3)+&
                      soilm3(t)-noah33_struc(n)%noah(t)%smc(3)
                 noah33_struc(n)%noah(t)%smc(3) = soilm3(t)
              ...
              if(noah33_struc(n)%noah(t)%sh2o(4)+delta4.gt.MIN_THRESHOLD .and.&
                   noah33_struc(n)%noah(t)%sh2o(4)+delta4.lt.sm_threshold) then 
                 noah33_struc(n)%noah(t)%sh2o(4) = noah33_struc(n)%noah(t)%sh2o(4)+&
                      soilm4(t)-noah33_struc(n)%noah(t)%smc(4)
                 noah33_struc(n)%noah(t)%smc(4) = soilm4(t)
...
!              if(LIS_localPet.eq.8) then 
!                 if(t.ge.61.and.t.le.80) then 
!                    print*, 'set3 ',t,noah33_struc(n)%noah(t)%smc(1),&
!                         noah33_struc(n)%noah(t)%smc(2),&
!                         noah33_struc(n)%noah(t)%smc(3),&
!                         noah33_struc(n)%noah(t)%smc(4)
!                 endif
...
!-----------------------------------------------------------------------------------------
              ...
              noah33_struc(n)%noah(t)%sh2o(1) = smc_tmp 
              noah33_struc(n)%noah(t)%smc(1) = smc_tmp
              
              
              ! scale the random number to soil moisture MIN_THRESHOLD and max value from DA
              !smc_tmp = smc_rnd *(MaxEnsSM2 - MIN_THRESHOLD) + MIN_THRESHOLD 
              !sh2o_tmp = sh2o_rnd *(MaxEns_sh2o2 - MIN_THRESHOLD) + MIN_THRESHOLD

              smc_tmp = (MaxEnsSM2 - MinEnsSM2)/2 + MinEnsSM2            
              noah33_struc(n)%noah(t)%sh2o(2) = smc_tmp
              noah33_struc(n)%noah(t)%smc(2) = smc_tmp
              
              
              ! scale the random number to soil moisture MIN_THRESHOLD and max value from DA
              !smc_tmp = smc_rnd *(MaxEnsSM3 - MIN_THRESHOLD) + MIN_THRESHOLD 
              !sh2o_tmp = sh2o_rnd *(MaxEns_sh2o3 - MIN_THRESHOLD) + MIN_THRESHOLD

              smc_tmp = (MaxEnsSM3 - MinEnsSM3)/2 + MinEnsSM3
              noah33_struc(n)%noah(t)%sh2o(3) = smc_tmp
              noah33_struc(n)%noah(t)%smc(3) = smc_tmp
              
              
              ! scale the random number to soil moisture MIN_THRESHOLD and max value from DA
              !smc_tmp = smc_rnd *(MaxEnsSM4 - MIN_THRESHOLD) + MIN_THRESHOLD 
              !sh2o_tmp = sh2o_rnd *(MaxEns_sh2o4 - MIN_THRESHOLD) + MIN_THRESHOLD

              smc_tmp = (MaxEnsSM4 - MinEnsSM4)/2 + MinEnsSM4
              noah33_struc(n)%noah(t)%sh2o(4) = smc_tmp
              noah33_struc(n)%noah(t)%smc(4) = smc_tmp
           ...
           do while(bounds_violation) 
              ...
              do j=1,4
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(m.ne.LIS_rc%nensem(n)) then 
                       delta(j) = delta(j) + &
                            (noah33_struc(n)%noah(t)%sh2o(j) - &
                            noah33_struc(n)%noah(t_unpert)%sh2o(j))
                    endif
              ...
              do j=1,4
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    tmpval = noah33_struc(n)%noah(t)%sh2o(j) - &
                         delta(j)
                    if(tmpval.le.MIN_THRESHOLD) then 
                       noah33_struc(n)%noah(t)%sh2o(j) = &
                            max(noah33_struc(n)%noah(t_unpert)%sh2o(j),&
                            MIN_THRESHOLD)
                       noah33_struc(n)%noah(t)%smc(j) = &
                            max(noah33_struc(n)%noah(t_unpert)%smc(j),&
                            MIN_THRESHOLD)
                       ens_flag(m) = .false. 
                    elseif(tmpval.ge.sm_threshold) then
                       noah33_struc(n)%noah(t)%sh2o(j) = &
                            min(noah33_struc(n)%noah(t_unpert)%sh2o(j),&
                            sm_threshold)
                       noah33_struc(n)%noah(t)%smc(j) = &
                            min(noah33_struc(n)%noah(t_unpert)%smc(j),&
                            sm_threshold)
                       ens_flag(m) = .false. 
                    endif
                 enddo
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(ens_flag(m)) then 
                       ...
                       if(.not.(tmpval.le.0.0 .or.&
                          ...
                          noah33_struc(n)%noah(t)%smc(j) = &
                               noah33_struc(n)%noah(t)%smc(j) - delta(j)
                          noah33_struc(n)%noah(t)%sh2o(j) = &
                               noah33_struc(n)%noah(t)%sh2o(j) - delta(j)
...
!                 write(LIS_logunit,*) '[ERR] Please adjust the perturbation settings ..'
                 do j=1,4
                    do m=1,LIS_rc%nensem(n)
                       ...
                       if(noah33_struc(n)%noah(t)%sh2o(j).gt.MAX_THRESHOLD.or.&
                            noah33_struc(n)%noah(t)%smc(j).gt.MAX_THRESHOLD) then                        
                          noah33_struc(n)%noah(t)%sh2o(j) = MAX_THRESHOLD
                          noah33_struc(n)%noah(t)%smc(j) = MAX_THRESHOLD
                       endif
                       
                       if(noah33_struc(n)%noah(t)%sh2o(j).lt.MIN_THRESHOLD.or.&
                            noah33_struc(n)%noah(t)%smc(j).lt.MIN_THRESHOLD) then                        
                          noah33_struc(n)%noah(t)%sh2o(j) = MIN_THRESHOLD
                          noah33_struc(n)%noah(t)%smc(j) = MIN_THRESHOLD
                       endif
!                    print*, i, m
!                    print*, 'smc',t, noah33_struc(n)%noah(t)%smc(:)
!                    print*, 'sh2o ',t,noah33_struc(n)%noah(t)%sh2o(:)
!                    print*, 'max ',t,MAX_THRESHOLD !noah33_struc(n)%noah(t)%smcmax
                    enddo
!                 call LIS_endrun()
                 enddo
              endif
...
!     if(LIS_localPet.eq.8) then 
!        if(t.ge.61.and.t.le.80) then 
!           print*, 'set2 ',t,noah33_struc(n)%noah(t)%smc(1),&
!                noah33_struc(n)%noah(t)%smc(2),&
!                noah33_struc(n)%noah(t)%smc(3),&
!                noah33_struc(n)%noah(t)%smc(4)
!        endif
...
end subroutine noah33_setsoilm
