...
!BOP
! !ROUTINE: noah33_getLbandTbPred
! \label{noah33_getLbandTbPred}
!
! !REVISION HISTORY:
! 13 Sept 2012: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine noah33_getLbandTbPred(n, obs_pred)
! !USES:
  use ESMF
...
!call the forward model
  ...
  call LIS_verify(status, &
       "Error in StateGet in noah33_getLbandTbPred for Lband_Hpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbH, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in noah33_getLbandTbPred for Lband_Hpol')

  call ESMF_StateGet(LIS_forwardState(n), "Lband_Vpol", varField, rc=status)
  call LIS_verify(status, &
       "Error in StateGet in noah33_getLbandTbPred for Lband_Vpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbV, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in noah33_getLbandTbPred for Lband_Vpol')
...
end subroutine noah33_getLbandTbPred
