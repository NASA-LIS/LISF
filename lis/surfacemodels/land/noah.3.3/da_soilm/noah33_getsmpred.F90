...
!BOP
! !ROUTINE: noah33_getsmpred
! \label{noah33_getsmpred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah33_getsmpred(n, k,obs_pred)
! !USES:
  use ESMF
...
!rescaling to relative wetness space. 
     if(LIS_rc%dascaloption(k).eq."Linear scaling") then 
        ...
        if(noah33_dasm_struc(n)%ntimes.gt.1) then 
           kk = LIS_rc%mo
        else
           kk = 1
        endif
        ...
        do i=1,LIS_rc%obs_ngrid(k)
           do m=1,LIS_rc%nensem(n)
              obs_tmp = (obs_pred(i,m) - noah33_dasm_struc(n)%model_xrange(i,kk,1))/&
                   (noah33_dasm_struc(n)%model_xrange(i,kk,noah33_dasm_struc(n)%nbins) - & 
                   noah33_dasm_struc(n)%model_xrange(i,kk,1))
     ...
     do i=1,LIS_rc%npatch(n,LIS_rc%lsm_index),LIS_rc%nensem(n)
        do m=1,LIS_rc%nensem(n)
           ...
           inputs_tp(1) = noah33_struc(n)%noah(t)%smc(1)
           inputs_tp(2) = noah33_struc(n)%noah(t)%rainf
           inputs_tp(3) = noah33_struc(n)%noah(t)%snowf
           inputs_tp(4) = noah33_struc(n)%noah(t)%shdfac
           inputs_tp(5) = noah33_struc(n)%noah(t)%stc(1)
           inputs_tp(6) = noah33_struc(n)%noah(t)%sneqv*LIS_CONST_RHOFW
...
#endif
end subroutine noah33_getsmpred
