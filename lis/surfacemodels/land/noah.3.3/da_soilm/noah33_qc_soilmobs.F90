...
!BOP
! !ROUTINE: noah33_qc_soilmobs
! \label{noah33_qc_soilmobs}
!
! !REVISION HISTORY:
! 25Feb2008: Sujay Kumar: Initial Specification
!
! !INTERFACE:
subroutine noah33_qc_soilmobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in noah33_qc_soilmobs")
  call ESMF_FieldGet(obs_sm_field,localDE=0,farrayPtr=smobs,rc=status)
  call LIS_verify(status,& 
       "ESMF_FieldGet failed in noah33_qc_soilmobs")

  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     smc1(t) = noah33_struc(n)%noah(t)%smc(1)
     smc2(t) = noah33_struc(n)%noah(t)%smc(2)
     smc3(t) = noah33_struc(n)%noah(t)%smc(3)
     smc4(t) = noah33_struc(n)%noah(t)%smc(4)

     sh2o1(t) = noah33_struc(n)%noah(t)%sh2o(1)
     sh2o2(t) = noah33_struc(n)%noah(t)%sh2o(2)
     sh2o3(t) = noah33_struc(n)%noah(t)%sh2o(3)
     sh2o4(t) = noah33_struc(n)%noah(t)%sh2o(4)

     stc1(t) = noah33_struc(n)%noah(t)%stc(1)
     stc2(t) = noah33_struc(n)%noah(t)%stc(2)
     stc3(t) = noah33_struc(n)%noah(t)%stc(3)
     stc4(t) = noah33_struc(n)%noah(t)%stc(4)
  ...
  call LIS_convertPatchSpaceToObsSpace(n,k,&       
       LIS_rc%lsm_index, &
       noah33_struc(n)%noah(:)%rainf,&
       rainf_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       noah33_struc(n)%noah(:)%sneqv,&
       sneqv_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
...
end subroutine noah33_qc_soilmobs
