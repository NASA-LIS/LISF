...
!BOP
! !ROUTINE: noah33_wrf_f2t
! \label{noah33_wrf_f2t}
!
! !REVISION HISTORY:
!  15 Oct 2002: Sujay Kumar; Initial Code
!
! !INTERFACE:
subroutine noah33_wrf_f2t(n)
! !USES:      
  use ESMF
...
!  call LIS_verify(status, 'Error in ESMF_StateGet: Snowf_f')
  ...
  noah33_struc(n)%forcing_ch = 1 ! EMK NUWRF
  ...
  call ESMF_StateGet(LIS_FORC_State(n),trim(LIS_FORC_Xice%varname(1)),&
       xiceField,rc=status)
  call LIS_verify(status,'noah33_f2t: error getting xice')
...
!  call LIS_verify(status)
  ...
  call LIS_verify(status,'noah33_f2t: error retrieving xice')


  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tid = LIS_surface(n,1)%tile(t)%tile_id 
     noah33_struc(n)%noah(t)%tair=tmp(tid)
     noah33_struc(n)%noah(t)%qair=q2(tid)
     noah33_struc(n)%noah(t)%swdown=swd(tid)
     noah33_struc(n)%noah(t)%lwdown=lwd(tid)
     noah33_struc(n)%noah(t)%uwind=uwind(tid)
     noah33_struc(n)%noah(t)%vwind=vwind(tid)
     noah33_struc(n)%noah(t)%psurf=psurf(tid)
     noah33_struc(n)%noah(t)%xice=xice(tid)
     if(pcp(t).ne.LIS_rc%udef) then 
        noah33_struc(n)%noah(t)%rainf=pcp(tid)
     else
        noah33_struc(n)%noah(t)%rainf=0.0
     endif
     noah33_struc(n)%noah(t)%snowf=0.0

     if(ch(tid).ne.LIS_rc%udef) then 
        noah33_struc(n)%noah(t)%ch=ch(tid)
     endif

     if(chs2(tid).ne.LIS_rc%udef) then 
        noah33_struc(n)%noah(t)%chs2=chs2(tid)
     endif

     if(cqs2(tid).ne.LIS_rc%udef) then 
        noah33_struc(n)%noah(t)%cqs2=cqs2(tid)
     endif

     if(q2sat(tid).ne.LIS_rc%udef) then 
        noah33_struc(n)%noah(t)%q2sat=q2sat(tid)
     endif

     if(emiss(tid).ne.LIS_rc%udef) then 
        noah33_struc(n)%noah(t)%emiss=emiss(tid)
     endif

     if(zval(tid).ne.LIS_rc%udef) then 
        noah33_struc(n)%noah(t)%z=zval(tid)
     endif
     noah33_struc(n)%forc_count = 1
  enddo

end subroutine noah33_wrf_f2t
