...
!BOP
!
! !ROUTINE: noah33_settbot
! \label{noah33_settbot}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah33_settbot(mtype)
! !USES:
...
!
! !DESCRIPTION:
!  This subroutine retrieves bottom boundary temperature for Noah3.3 LSM.
...
!-----------------------------------------------------------------------
     ...
     if (noah33_struc(n)%fixedtbot.ne.0.0) then
        write(LIS_logunit,*) '[INFO] Fixing Noah3.3 deep soil temperature: ', &
                              noah33_struc(n)%fixedtbot
     else
     ...
     do i = 1, LIS_rc%npatch(n,mtype)
        if (noah33_struc(n)%fixedtbot.ne.0.0) then
           noah33_struc(n)%noah(i)%tempbot = noah33_struc(n)%fixedtbot
        else
           if (placetbot1(LIS_surface(n,mtype)%tile(i)%col,                   &
                          LIS_surface(n,mtype)%tile(i)%row).ne.-9999.00) then
              noah33_struc(n)%noah(i)%tempbot =                        &
                            placetbot1(LIS_surface(n,mtype)%tile(i)%col,      &
                                       LIS_surface(n,mtype)%tile(i)%row)
           else
              noah33_struc(n)%noah(i)%tempbot = 285.0
              write(LIS_logunit,*) '[INFO] Noah3.3 bottom temperature -- ',   &
        ...
        call LIS_initTmnUpdateTile(n,i,noah33_struc(n)%noah(i)%tempbot)

     enddo
     deallocate(placetbot1)
  enddo

end subroutine noah33_settbot


!BOP
!
! !ROUTINE: noah33_resettbot
! \label{noah33_resettbot}
!
! !REVISION HISTORY:
!  28 Apr 2002: Kristi Arsenault;  Added Noah2.5 LSM, Initial Code
!  13 Oct 2003: Sujay Kumar; Domain independent modifications
...
!
! !INTERFACE:
subroutine noah33_resettbot(mtype)
! !USES:
  use LIS_coreMod,    only : LIS_rc, LIS_surface
  use LIS_logMod,     only : LIS_logunit
  use LIS_fileIOMod,  only : LIS_read_param
...
!  This subroutine retrieves bottom boundary temperature for Noah3.3 LSM.
!  Currently a static data is being used. If a climatology is available, 
!  it can be used by resetting the appropriate frequency of the climatology
!
!  The routines invoked are: 
!  \begin{description}
!  \item[LIS\_read\_param](\ref{LIS_read_param}) \newline
...
!-----------------------------------------------------------------------
! Read in bottom temperature fields and adjust for elevation difference
! with either Eta (NLDAS) or GDAS (GLDAS) correction dataresets. 
!-----------------------------------------------------------------------
!!!     allocate(placetbot1(LIS_rc%lnc(n),LIS_rc%lnr(n)))
!!!     call LIS_read_param(n,"TBOT",placetbot1)
!!!
!!!     if (noah33_struc(n)%fixedtbot.ne.0.0) then
!!!        write(LIS_logunit,*) 'Fixing Noah3.3 deep soil temperature: ', &
!!!                              noah33_struc(n)%fixedtbot
!!!     endif
!!!     do i = 1, LIS_rc%npatch(n,mtype)
!!!        if (noah33_struc(n)%fixedtbot.ne.0.0) then
...
!!!           else
!!!              noah33_struc(n)%noah(i)%tempbot = 285.0
!!!              write(LIS_logunit,*) 'Noah3.3 bottom temperature -- ',   &
!!!                                   'not defined for point ',i
!!!              write(LIS_logunit,*) 'Noah3.3 bottom temperature -- ',   &
!!!                                   'reset to default value of 285.0'
!!!           endif
!!!        endif
!!!     enddo
!!!     deallocate(placetbot1)
...
end subroutine noah33_resettbot
