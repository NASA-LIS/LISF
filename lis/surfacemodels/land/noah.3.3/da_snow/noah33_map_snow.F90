...
!BOP
! !ROUTINE: noah33_map_snow
! \label{noah33_map_snow}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah33_map_snow(n,OBS_State,LSM_Incr_State)
! !USES:
  use ESMF
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     swe(t)  = noah33_struc(n)%noah(t)%sneqv
     snod(t) = noah33_struc(n)%noah(t)%snowh
  enddo
...
!                swe(t)
           ...
           snowhc = snowh*100.0
           newsnc = newsn*100.0
           if(noah33_struc(n)%noah(t)%tair.lt.LIS_CONST_TKFRZ) then 
              tempc = noah33_struc(n)%noah(t)%tair-LIS_CONST_TKFRZ
           else
              tempc = 0.0
           endif
           if(tempc .le. -15) then 
...
!             swe(t)
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     sweincr(t)  = swe(t)  - noah33_struc(n)%noah(t)%sneqv
     snodincr(t) = snod(t) - noah33_struc(n)%noah(t)%snowh
  enddo 
  deallocate(obs_state_objs)
  deallocate(swe)
  deallocate(snod)

end subroutine noah33_map_snow
