...
!BOP
! !ROUTINE: noah33_qc_scfobs
! \label{noah33_qc_scfobs}
!
! !REVISION HISTORY:
! 25Feb2008: Sujay Kumar: Initial Specification
!
! !INTERFACE:
subroutine noah33_qc_scfobs(n,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(snowobs(gid).ne.LIS_rc%udef) then 
        !if(noah33_struc(n)%noah(t)%shdfac.gt.0.7)  snowobs(gid) = LIS_rc%udef        
        if(noah33_struc(n)%noah(t)%vegt.le.4) snowobs(gid) = LIS_rc%udef  !forest types          

     endif
  enddo

end subroutine noah33_qc_scfobs
