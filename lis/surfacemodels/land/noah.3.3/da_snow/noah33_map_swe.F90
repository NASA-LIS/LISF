...
!BOP
! !ROUTINE: noah33_map_swe
! \label{noah33_map_swe}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah33_map_swe(n,OBS_State,LSM_State)
! !USES:
  use ESMF
...
! Based on SWE, we manually update snowdepth    
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if(sweobs(LIS_surface(n,1)%tile(t)%index).gt.0) then 
        swe(t) = sweobs(LIS_surface(n,1)%tile(t)%index)
        newsn = swe(t)
        snowhc = noah33_struc(n)%noah(t)%snowh*100.0
        newsnc = newsn*100.0
