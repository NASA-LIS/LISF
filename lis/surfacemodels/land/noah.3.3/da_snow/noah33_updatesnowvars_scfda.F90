...
!BOP
! !ROUTINE: noah33_updatesnowvars_scfda
! \label{noah33_updatesnowvars_scfda}
!
! !REVISION HISTORY:
...
! !INTERFACE:
subroutine noah33_updatesnowvars_scfda(n, LSM_State, LSM_Incr_State)
! !USES:
  use ESMF
...
!EOP
  ...
  if (ANSASCFsnow_struc(n)%useEnKFwithDI .eq. 1) then
     ...
     idx1 = 0
     call noah33_getscfpred(n,obs_pred)
     ...
     call noah33_transform_snow(n, LIS_OBS_State(n,k1))
     call noah33_map_snow_DI(n,LIS_OBS_State(n,k1),LSM_Incr_State)

     call ESMF_StateGet(LSM_Incr_State,"SWE",sweIncrField,rc=status)
     call LIS_verify(status, 'failed to retrieve SWE state in noah33_updatesnowvars')
     call ESMF_StateGet(LSM_Incr_State,"Snowdepth",snodIncrField,rc=status)
     call LIS_verify(status, 'failed to retrieve Snowdepth state in noah33_updatesnowvars')  
     call ESMF_FieldGet(sweIncrField,localDE=0,farrayPtr=sweincr,rc=status)
     call LIS_verify(status, 'failed to retrieve SWE field in noah33_updatesnowvars')
     call ESMF_FieldGet(snodIncrField,localDE=0,farrayPtr=snodincr,rc=status)
     call LIS_verify(status,'failed to retrieve Snowdepth field in noah33_updatesnowvars')
  ...
  enddo
end subroutine noah33_updatesnowvars_scfda
