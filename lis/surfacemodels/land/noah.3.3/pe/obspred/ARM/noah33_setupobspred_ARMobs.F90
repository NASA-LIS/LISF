...
!BOP
! !ROUTINE: noah33_setupobspred_ARMobs
!  \label{noah33_setupobspred_ARMobs}
!
! !REVISION HISTORY:
! 16 Jul 2009: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine noah33_setupobspred_ARMobs(OBSPred)
! !USES:
  use ESMF
...
!EOP
  ...
  qgField = ESMF_FieldCreate(arrayspec=realarrspec, grid=LIS_vecPatch(n,LIS_rc%lsm_index), &
  ...
  sfstField = ESMF_FieldCreate(arrayspec=realarrspec, grid=LIS_vecPatch(n,LIS_rc%lsm_index), &
  ...
  call LIS_verify(status)
#endif
end subroutine noah33_setupobspred_ARMobs
