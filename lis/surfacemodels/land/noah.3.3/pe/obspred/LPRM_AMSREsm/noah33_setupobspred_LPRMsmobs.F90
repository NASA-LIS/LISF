...
!BOP
! !ROUTINE: noah33_setupobspred_LPRM_AMSREsmObs
!  \label{noah33_setupobspred_LPRM_AMSREsmObs}
!
! !REVISION HISTORY:
! 16 Jul 2009: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine noah33_setupobspred_LPRM_AMSREsmObs(OBSPred)
! !USES:
  use ESMF
...
!EOP
  ...
  sfsmField = ESMF_FieldCreate(arrayspec=realarrspec, grid=LIS_vecPatch(n,LIS_rc%lsm_index), &
       name="LPRM AMSRE Surface Soil Moisture", rc=status)
  call LIS_verify(status)
  
  call ESMF_StateAdd(OBSPred,(/sfsmField/),rc=status)
  call LIS_verify(status)

end subroutine noah33_setupobspred_LPRM_AMSREsmObs
