...
!
! !INTERFACE:
subroutine noah33_sfc2cmem3(n, sfcState)
! !USES:      
  use ESMF
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     lsm_var(t) = sqrt(noah33_struc(n)%noah(t)%uwind*&
          noah33_struc(n)%noah(t)%uwind+&
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if(noah33_struc(n)%noah(t)%sneqv.gt.0)  then 
        ...
        lsm_var(t) = noah33_struc(n)%noah(t)%sneqv &
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if (noah33_struc(n)%noah(t)%snowh  .gt. 0.0001) then  
        snow_cov(t)=1.0
        land_cov(t)=0.0
     else
        snow_cov(t)=0.0
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     lsm_var(t)     = noah33_struc(n)%noah(t)%vegt 
  enddo
  call LIS_patch2tile(n,LIS_rc%lsm_index,land_type, lsm_var)
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     lsm_var(t) = noah33_struc(n)%noah(t)%t1
  enddo
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     ! --------------------------------------------------------
     lsm_var(t) = noah33_struc(n)%noah(t)%cmc*LIS_CONST_RHOFW 
  enddo
  call LIS_patch2tile(n,LIS_rc%lsm_index,canopy_water_content, lsm_var)
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     lsm_var(t) = noah33_struc(n)%noah(t)%lai
  enddo
  call LIS_patch2tile(n,LIS_rc%lsm_index,leaf_area_index, lsm_var)
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
           lsm_var(t) = &   !could expose noah's relsmc variable instead; same calc
...
end subroutine noah33_sfc2cmem3
