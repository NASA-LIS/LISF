...
!BOP
!
! !ROUTINE: noah33_readrst
! \label{noah33_readrst}
!
! !REVISION HISTORY:
...
! !USES:
  ...
  use LIS_logMod,     only : LIS_logunit, LIS_endrun, &
       ...
       LIS_verify
  use module_sfcdif_wrf_33, only: MYJSFCINIT
  use noah33_lsmMod
...
!
! !DESCRIPTION:
!  This program reads restart files for Noah3.3.  This
!  includes all relevant water/energy storages and tile information. 
!  The following is the list of variables specified in the Noah3.3 
!  restart file: 
!  \begin{verbatim}
...
! \item[noah33\_coldstart](\ref{noah33_coldstart}) \newline
!   initializes the Noah3.3 state variables
! \end{description}
!EOP
  implicit none      
...
!-------------------------------------------------------------------------
        inquire(file=noah33_struc(n)%rfile,exist=file_exists) 

        if(.not.file_exists) then 
           write(LIS_logunit,*) '[ERR] Noah3.3 restart file ',               &
                noah33_struc(n)%rfile,' does not exist '
           write(LIS_logunit,*) '[ERR] Program stopping ...'
           call LIS_endrun()
        endif
        write(LIS_logunit,*)                        &
             '[INFO] Noah3.3 restart file used: ',noah33_struc(n)%rfile
        
        if(wformat.eq."binary") then 
           ftn = LIS_getNextUnitNumber()
           open(ftn,file=noah33_struc(n)%rfile,form='unformatted')        
...
!------------------------------------------------------------------------
           if(nc.ne.LIS_rc%gnc(n) .or. nr.ne.LIS_rc%gnr(n))then
              write(LIS_logunit,*) noah33_struc(n)%rfile,                 &
...
!------------------------------------------------------------------------
...
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
           status=nf90_open(path=noah33_struc(n)%rfile,&
                mode=NF90_NOWRITE,ncid=ftn)
           call LIS_verify(status,&
                'Error opening file '//noah33_struc(n)%rfile)
#endif
        endif

        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah33_struc(n)%noah%t1, &
             varname="T1",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah33_struc(n)%noah%cmc,&
             varname="CMC", wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah33_struc(n)%noah%snowh,&
             varname="SNOWH",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah33_struc(n)%noah%sneqv,&
        ...
        do l=1,noah33_struc(n)%nslay
           call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,tmptilen, varname="STC",&
                dim=l,vlevels = noah33_struc(n)%nslay, &
                wformat=wformat)
           do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
              noah33_struc(n)%noah(t)%stc(l) = tmptilen(t)
           enddo
        enddo
        ...
        do l=1,noah33_struc(n)%nslay
           call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,tmptilen, varname="SMC",&
                ...
                wformat=wformat)
           do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
              noah33_struc(n)%noah(t)%smc(l) = tmptilen(t)
           enddo
        enddo
        ...
        do l=1,noah33_struc(n)%nslay
           ...
           do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
              ...
              noah33_struc(n)%noah(t)%sh2o(l) = tmptilen(t)
           enddo
        enddo
        
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah33_struc(n)%noah%ch,&
             varname="CH",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah33_struc(n)%noah%cm,&
             varname="CM",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah33_struc(n)%noah%z0,&
             varname="Z0",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah33_struc(n)%noah%emiss,&
             varname="EMISS",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah33_struc(n)%noah%albedo,&
             varname="ALBEDO",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah33_struc(n)%noah%snotime1,&
             varname="SNOTIME1",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah33_struc(n)%noah%q1,&
  ...
  do n=1,LIS_rc%nnest
     if (noah33_struc(n)%sfcdifoption.eq.1) then
        call MYJSFCINIT(noah33_struc(n)%ztmax2, &
             noah33_struc(n)%dzeta2, &
             noah33_struc(n)%psih2,  &
             noah33_struc(n)%psim2)
     endif
  enddo
end subroutine noah33_readrst
