...
!BOP
! !ROUTINE: noah33_dynsetup
! \label{noah33_dynsetup}
!
! !REVISION HISTORY:
...
! 
! !DESCRIPTION: 
!  This routine sets up the time-dependent variables in Noah3.3
! 
!EOP     
  implicit none
  integer, intent(in) :: n 
...
! local time instead of on UTC time. - dmm
     ...
     noah33_struc(n)%noah(t)%z0brd_old = noah33_struc(n)%noah(t)%z0brd
     ...
     if(noah33_struc(n)%noah(t)%shdfac.ge.noah33_struc(n)%noah(t)%shdmax) then
        if(LIS_rc%useemissmap(n).ne."none") then
           ...
           if(noah33_struc(n)%embrd_upd.eq.0) then
        ...
        if(LIS_rc%useroughnessmap(n).ne."none") then
           ...
           if(noah33_struc(n)%z0brd_upd.eq.0) then
        ...
        if(LIS_rc%usealbedomap(n).eq."none") then ! no map specified
           if(noah33_struc(n)%alb_upd.eq.0) then
        ...
        if(LIS_rc%uselaimap(n).eq."none") then ! no map specified
           if(noah33_struc(n)%lai_upd.eq.0) then
  ...
  return
end subroutine noah33_dynsetup
