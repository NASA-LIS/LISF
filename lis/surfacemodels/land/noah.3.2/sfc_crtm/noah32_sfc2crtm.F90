...
#include "LIS_misc.h"
!BOP
! !ROUTINE: noah32_sfc2crtm
! \label{noah32_sfc2crtm}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah32_sfc2crtm(n, sfcState)
! !USES:      
  use ESMF
...
! 
! !DESCRIPTION: 
! This subroutine assigns the noah32 specific surface variables
! to CRTM. 
!
!EOP
  ...
  do t=1,LIS_rc%ntiles(n)
     ...
     wind_speed(t) = sqrt(noah32_struc(n)%noah(t)%uwind*noah32_struc(n)%noah(t)%uwind+&
          noah32_struc(n)%noah(t)%vwind*noah32_struc(n)%noah(t)%vwind)
     ...
     Snow_Depth(t) = noah32_struc(n)%noah(t)%snowh *1000.0
     if(noah32_struc(n)%noah(t)%sneqv.gt.0)  then 
        ...
        Snow_Density(t) =1000000.0*  noah32_struc(n)%noah(t)%sneqv &
             /  &
     ...
     Land_Type(t)     = noah32_struc(n)%noah(t)%vegt 
     Land_Temperature(t) = noah32_struc(n)%noah(t)%t1
     Snow_Temperature(t) = noah32_struc(n)%noah(t)%t1
     soil_moisture_content(t) = noah32_struc(n)%noah(t)%smc(1)
     soil_temperature(t) = noah32_struc(n)%noah(t)%stc(1)
     Canopy_Water_Content(t) = noah32_struc(n)%noah(t)%cmc*LIS_CONST_RHOFW
     Vegetation_Fraction(t) = noah32_struc(n)%noah(t)%shdfac
     Leaf_Area_Index(t) = noah32_struc(n)%noah(t)%lai

  enddo
end subroutine noah32_sfc2crtm
