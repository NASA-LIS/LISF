...
!BOP
!
! !ROUTINE: noah32_readrst
! \label{noah32_readrst}
!
! !REVISION HISTORY:
...
!
! !DESCRIPTION:
!  This program reads restart files for Noah3.2.  This
!  includes all relevant water/energy storages and tile information. 
!  The following is the list of variables specified in the Noah3.2 
!  restart file: 
!  \begin{verbatim}
...
! \item[noah32\_coldstart](\ref{noah32_coldstart}) \newline
!   initializes the Noah3.2 state variables
! \end{description}
!EOP
  implicit none      
...
!-------------------------------------------------------------------------
        inquire(file=noah32_struc(n)%rfile,exist=file_exists) 

        if(.not.file_exists) then 
           write(LIS_logunit,*) 'Noah3.2 restart file ',               &
                          trim(noah32_struc(n)%rfile),' does not exist '
           write(LIS_logunit,*) 'Program stopping ...'
           call LIS_endrun()
           
        endif
        write(LIS_logunit,*) 'Noah3.2 restart file used: ',&
             noah32_struc(n)%rfile
        
        if(wformat.eq."binary") then 
           ftn = LIS_getNextUnitNumber()
           open(ftn,file=noah32_struc(n)%rfile,form='unformatted')        
...
!------------------------------------------------------------------------
           if(nc.ne.LIS_rc%gnc(n) .or. nr.ne.LIS_rc%gnr(n))then
              write(LIS_logunit,*) noah32_struc(n)%rfile,                 &
                   'grid space mismatch - Noah3.2 halted'
              call LIS_endrun
           endif
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
...
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
           status=nf90_open(path=noah32_struc(n)%rfile,&
                mode=NF90_NOWRITE,ncid=ftn)
           call LIS_verify(status,&
                'Error opening file '//trim(noah32_struc(n)%rfile))
#endif
        endif

        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah32_struc(n)%noah%t1, &
             varname="T1",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah32_struc(n)%noah%cmc,&
             varname="CMC", wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah32_struc(n)%noah%snowh,&
             varname="SNOWH",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,noah32_struc(n)%noah%sneqv,&
        ...
        do l=1,noah32_struc(n)%nslay
           call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,tmptilen, varname="STC",&
                dim=l,vlevels = noah32_struc(n)%nslay, &
                wformat=wformat)
           do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
              noah32_struc(n)%noah(t)%stc(l) = tmptilen(t)
           enddo
        enddo
        ...
        do l=1,noah32_struc(n)%nslay
           call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index,tmptilen, varname="SMC",&
                ...
                wformat=wformat)
           do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
              noah32_struc(n)%noah(t)%smc(l) = tmptilen(t)
           enddo
        enddo
        ...
        do l=1,noah32_struc(n)%nslay
           ...
           do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
              ...
              noah32_struc(n)%noah(t)%sh2o(l) = tmptilen(t)
           enddo
        enddo
        
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah32_struc(n)%noah%ch,&
             varname="CH",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah32_struc(n)%noah%cm,&
             varname="CM",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah32_struc(n)%noah%z0,&
             varname="Z0",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah32_struc(n)%noah%emiss,&
             varname="EMISS",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah32_struc(n)%noah%albedo,&
             varname="ALBEDO",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah32_struc(n)%noah%snotime1,&
             varname="SNOTIME1",wformat=wformat)
        call LIS_readvar_restart(ftn,n,LIS_rc%lsm_index, noah32_struc(n)%noah%q1,&
        ...
        elseif(wformat.eq."netcdf") then 
           ...
           call LIS_verify(status,'Error in nf90_close in noah32_readrst')
#endif     
        endif
        deallocate(tmptilen)
        do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
  ...
  do n=1,LIS_rc%nnest
     if (noah32_struc(n)%sfcdifoption.eq.1) then
        call MYJSFCINIT(noah32_struc(n)%ztmax2, &
                        noah32_struc(n)%dzeta2, &
                        noah32_struc(n)%psih2,  &
                        noah32_struc(n)%psim2)
     endif
  enddo
end subroutine noah32_readrst
