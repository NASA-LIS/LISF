...
!BOP
! !ROUTINE: noah32_map_snodep
! \label{noah32_map_snodep}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah32_map_snodep(n,OBS_State,LSM_Incr_State)
! !USES:
  use ESMF
...
!EOP
  ...
  real                     :: noah32_swe(LIS_rc%ntiles(n))
  real                     :: noah32_snod(LIS_rc%ntiles(n))
  real                     :: snod(LIS_rc%ntiles(n))
  real                     :: tmpsneqv
  real, allocatable            :: snodepobs(:)
  integer                  :: t
  integer                  :: status
  ...
  do t=1,LIS_rc%ntiles(n)
     noah32_swe(t)  = noah32_struc(n)%noah(t)%sneqv
     noah32_snod(t) = noah32_struc(n)%noah(t)%snowh
  enddo


  do t=1,LIS_rc%ntiles(n)
     if(snodepobs(LIS_domain(n)%tile(t)%index).ge.0) then 
        if(noah32_snod(t).gt.1e-6) then 
           tmpsneqv = noah32_swe(t)/noah32_snod(t)
        else
           tmpsneqv = 0.0
        endif
...
! Based on SNODEP, we manually update SWE
        ...
        sweincr(t) = tmpsneqv*snod(t) - noah32_swe(t)
        snodincr(t) = snod(t) - noah32_snod(t)
!        if(sweincr(t).gt.20) then 
!           print*, 'sweincr ',t, sweincr(t), snod(t), tmpsneqv, noah32_swe(t)
!        endif
     else
        sweincr(t) = 0 
        snodincr(t) = 0 
...
end subroutine noah32_map_snodep
