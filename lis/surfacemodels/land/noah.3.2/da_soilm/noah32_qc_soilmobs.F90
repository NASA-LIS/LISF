...
!BOP
! !ROUTINE: noah32_qc_soilmobs
! \label{noah32_qc_soilmobs}
!
! !REVISION HISTORY:
! 25Feb2008: Sujay Kumar: Initial Specification
!
! !INTERFACE:
subroutine noah32_qc_soilmobs(n,OBS_State)
! !USES:
  use ESMF
...
!EOP
  ...
  do t=1,N_obs_size
     if(noah32_struc(n)%noah(t)%rainf.gt.3E-6.and.smobs(t).ne.LIS_rc%udef) then 
!        print*, 'flagging for rain ',t, smobs(t), noah32_struc(n)%noah(t)%rainf
        smobs(t) = LIS_rc%udef
     elseif(noah32_struc(n)%noah(t)%stc(1).le.LIS_CONST_TKFRZ.and.&
          smobs(t).ne.LIS_rc%udef) then 
!        print*, 'flagging for stc ',t, smobs(t), noah32_struc(n)%noah(t)%stc(1)
        smobs(t) = LIS_rc%udef
     elseif(noah32_struc(n)%noah(t)%sca.gt.0.001.and.&
          smobs(t).ne.LIS_rc%udef) then 
!        print*, 'flagging for sca ',t, smobs(t), noah32_struc(n)%noah(t)%sca
        smobs(t) = LIS_rc%udef
     elseif(noah32_struc(n)%noah(t)%shdfac.gt.0.7) then 
        smobs(t) = LIS_rc%udef        
     endif
  enddo
  
end subroutine noah32_qc_soilmobs
