...
!BOP
!
! !ROUTINE: noah32_settbot
! \label{noah32_settbot}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noah32_settbot(mtype)
! !USES:
...
!
! !DESCRIPTION:
!  This subroutine retrieves bottom boundary temperature for Noah3.2 LSM.
...
!-----------------------------------------------------------------------
     ...
     if (noah32_struc(n)%fixedtbot.ne.0.0) then
        write(LIS_logunit,*) 'Fixing Noah3.2 deep soil temperature: ', &
                              noah32_struc(n)%fixedtbot
     else
     ...
     do i = 1, LIS_rc%npatch(n,mtype)
        if (noah32_struc(n)%fixedtbot.ne.0.0) then
           noah32_struc(n)%noah(i)%tempbot = noah32_struc(n)%fixedtbot
        else
           if (placetbot1(LIS_surface(n,mtype)%tile(i)%col,                   &
                          LIS_surface(n,mtype)%tile(i)%row).ne.-9999.00) then
              noah32_struc(n)%noah(i)%tempbot =                        &
                            placetbot1(LIS_surface(n,mtype)%tile(i)%col,      &
                                       LIS_surface(n,mtype)%tile(i)%row)
           else
              noah32_struc(n)%noah(i)%tempbot = 285.0
              write(LIS_logunit,*) 'Noah3.2 bottom temperature -- ',   &
        ...
        call LIS_initTmnUpdateTile(n,i,noah32_struc(n)%noah(i)%tempbot)

     enddo
     deallocate(placetbot1)
  enddo

end subroutine noah32_settbot
