    ...
    do n=1, LIS_rc%nnest
        ...
        !WN ---create restart filename based on timewindow for EnKS
            ...
            do l=1, AC71_struc(n)%nsoil ! TODO: check loop
                ...
                do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
                    AC71_struc(n)%ac71(t)%sh2o(l) = tmptilen(t)
                enddo
            enddo
            ...
            do l=1, AC71_struc(n)%nsoil + AC71_struc(n)%nsnow
                call LIS_readvar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
                     ...
                     vlevels = AC71_struc(n)%nsoil + &
                               ...
                               wformat=wformat)
                do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
                    AC71_struc(n)%ac71(t)%zss(l) = tmptilen(t)
                enddo
            enddo

            ! read: snow height
            call LIS_readvar_restart(ftn, n, LIS_rc%lsm_index, &
                 AC71_struc(n)%ac71%snowh, &
                 varname="SNOWH", wformat=wformat)

            ! read: snow water equivalent
            call LIS_readvar_restart(ftn, n, LIS_rc%lsm_index, &
                 AC71_struc(n)%ac71%sneqv, &
            ...
            do l=1, AC71_struc(n)%nsnow ! TODO: check loop
                call LIS_readvar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
                     varname="SNOWICE", &
                     dim=l, vlevels = AC71_struc(n)%nsnow, wformat=wformat)
                do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
                    AC71_struc(n)%ac71(t)%snowice(l) = tmptilen(t)
                enddo
            enddo
