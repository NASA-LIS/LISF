...
! 4. irrigation
  ...
  ! SY: Begin moving to here from AC_VEG_PARAMETERS_71 to facilitate OPTUE
  ...
  ! SY: End moving to here from AC_VEG_PARAMETERS_71 to facilitate OPTUE

!------------------------------------------------------------------------------------------!
END MODULE AC_GLOBALS_71
!------------------------------------------------------------------------------------------!
!------------------------------------------------------------------------------------------!
MODULE AC_VEG_PARAMETERS_71

    USE AC_GLOBALS_71, ONLY: MVT, MBAND,  &
    ...
    ! SY: Begin commenting out and moving to AC_GLOBALS_71 and using from there to facilitate OPTUE
!    INTEGER, PARAMETER :: MVT   = 27
!    INTEGER, PARAMETER :: MBAND = 2
    ! SY: End commenting out and moving to AC_GLOBALS_71 and using from there to facilitate OPTUE
    ...
    ! SY: Begin commenting out some variables and moving to AC_GLOBALS_71 and using from there to facilitate OPTUE
!    REAL :: CH2OP(MVT)       !maximum intercepted h2o per unit lai+sai (mm)
!    REAL :: DLEAF(MVT)       !characteristic leaf dimension (m)
!    REAL :: Z0MVT(MVT)       !momentum roughness length (m)
!    REAL :: HVT(MVT)         !top of canopy (m)
...
!    REAL :: WDPOOL(MVT)      !wood pool (switch 1 or 0) depending on woody or not [-]
!    REAL :: TDLEF(MVT)       !characteristic T for leaf freezing [K]
    ! SY: End commenting out some variables and moving to AC_GLOBALS_71 and using from there to facilitate OPTUE
...
END MODULE AC_VEG_PARAMETERS_71
! ==================================================================================================
! ==================================================================================================
MODULE AC_RAD_PARAMETERS_71
...
END MODULE AC_RAD_PARAMETERS_71
! ==================================================================================================

MODULE AC_ROUTINES_71
  USE AC_GLOBALS_71
  IMPLICIT NONE
...
! --------------------------------------------------------------------------------------------------
  USE AC_VEG_PARAMETERS_71
  USE AC_RAD_PARAMETERS_71
! --------------------------------------------------------------------------------------------------
  implicit none
! --------------------------------------------------------------------------------------------------
! input
...
! DRY THERMAL CONDUCTIVITY IN W.M-1.K-1
    ...
    THKDRY = (0.135* GAMMD+ 64.7)/ (2710. - 0.947* GAMMD)
! FROZEN
...
! evaluate stability-dependent variables using moz from prior iteration
    IF (MOZ .LT. 0.) THEN
       ...
       TMP3 = LOG((1.+TMP1)/2.)
       FMNEW = 2.*TMP3 + TMP2 - 2.*ATAN(TMP1) + 1.5717963
       FHNEW = 2*TMP2
...
! 2-meter
       ...
       TMP32 = LOG((1.+TMP12)/2.)
       FM2NEW = 2.*TMP32 + TMP22 - 2.*ATAN(TMP12) + 1.5717963
       FH2NEW = 2*TMP22
    ELSE
       FMNEW = -5.*MOZ
       FHNEW = FMNEW
...
! AKB(1976), EQ(16).                                                                                                      
     ...
     ! Raleigh, NC, 1976                                                                                                  
       ...
       IF ( ZOL .GT. 0.5 ) THEN ! linear form ok                                                                          
        ! Holtslag and de Bruin, J. App. Meteor 27, 689-714, 1988;                                                        
...
! local
  ...
  PARAMETER (B0=6.109177956    , B1=5.034698971E-01,  &
             B2=1.886013408E-02, B3=4.176223716E-04,  &
             B4=5.824720280E-06, B5=4.838803174E-08,  &
             B6=1.838826904E-10)

  PARAMETER (C0= 4.438099984E-01, C1=2.857102636E-02,  &
...
!    QINSUR = PONDING/DT * 0.001
    ...
    if(fldfrc==1)then
      QINSUR = QINSUR + (rivsto + fldsto) !surface water storage units are in m/s (See HYMAP2_routing_run.F90 and ac71_getsws_hymap2.F90)
    endif
...
! lake/soil water balances
    ...
    if(fldfrc==0)then
      RUNSRF = RUNSRF + (rivsto + fldsto)*1000. !surface water storage units are in m/s (See HYMAP2_routing_run.F90 and ac71_getsws_hymap2.F90)
    endif
...
! ------------------------------------------------------------------------------------------
  SUBROUTINE BVOCFLUX(VOCFLX,  VEGTYP,  VEGFRAC,  APAR,   TV )
    use AC_VEG_PARAMETERS_71 , ONLY : SLAREA, EPS
! ------------------------------------------------------------------------------------------
