...
! !INTERFACE
subroutine ac_driver_71(iloc, jloc, &
  ...
  use ac_globals_71, only: dveg, opt_crs , opt_btr , opt_run ,      &
  ...
  use ac_routines_71, only: ac_sflx_71, redprm 
  implicit none
...
!  jloc = 1 
  ...
  call calc_declin_71(nowdate(1:4)//"-"//nowdate(5:6)//"-"//nowdate(7:8)//"_"//nowdate(9:10)//":"//nowdate(11:12)//":00", &
      latitude, longitude, cosz, yearlen, julian)

  ! SY: Begin for enabling OPTUE
  ! SY: Begin lines following those in REDPRM and Ac71_setup
  ...
  ! SY: End lines following those in REDPRM and Ac71_setup
  ! SY: Begin lines following those in read_mp_veg_parameters and Ac71_setup
  ...
  MRP(vegetype) = p_MRP
  ! SY: End lines following those in read_mp_veg_parameters and Ac71_setup
  ...
  if ( dveg == 1 ) then
    ...
    shdfac = month_d_71(shdfac_monthly, nowdate)
  else
  ...
  deallocate(snliq)
  deallocate(stc)
end subroutine ac_driver_71

real function month_d_71(a12, nowdate) result (nowval)
  !
...
end function month_d_71 

SUBROUTINE calc_declin_71 ( nowdate, latitude, longitude, cosz, yearlen, julian)
  use kwm_date_utilities_71 
!---------------------------------------------------------------------
  IMPLICIT NONE
!---------------------------------------------------------------------
...
END SUBROUTINE calc_declin_71 

! Subroutine SNOW_INIT grabbed from NOAH-MP-WRF
SUBROUTINE SNOW_INIT_71 ( jts, jtf, its, itf, ims, ime, jms, jme, NSNOW, NSOIL, ZSOIL,  &
