...
int vic412_read_vegparam(FILE *vegparam,
                         int   gridcel,
                         int   Nveg_type,
                         int   lis_veg_class,
                         veg_con_struct *lis_temp)
/**********************************************************************
...
**********************************************************************/
{
  ...
  /* vic_veg_clas is defined to save the vegetation class in vegetation parameter file (called real vegetation class after). 
     temp is a veg_con_struct. Firstly, the real vegetation class will be read in and save in the veg_class of temp. Then, 
     vic412_read_vegparam will find the index number of the real vegetation class in vegetation parameter library (veg_lib). 
     In the end, the veg_class of temp will be replced with the index number. In other word, the vegetation class in veg_con_struct
     will not be the real vegetatoin class. In stead, it will be the corresponding index number in vegetatoin parameter library.
     
     In this function (vic412_read_vegparam), lis_veg_class is real vegetation class. In order to update the LAI and related values in
     veg_lib correctly, lis_veg_class should be compared with the real vegetation class (vic_veg_class) */
  int vic_veg_class;
  ...
  for (i = 0; i < vegetat_type_num; i++) {
    ...
    /* added by Shugong Wang to read root zone information for LIS-tiling. 
       ...
       information to lis_temp, which points to lis_veg_con.                            */
    if(vic_veg_class == lis_veg_class)
    {
         // added by Shugong Wang. lis_temp is a pointer of lis_veg_con. In the following lines, lis_temp will retrieve
    ...
    if ( options.VEGPARAM_LAI) {
      ...
      { 
        // set flag = 1 indicating find the parameters for the vegetion class
        flag = 1; 
        for ( j = 0; j < 12; j++ ) 
        {
