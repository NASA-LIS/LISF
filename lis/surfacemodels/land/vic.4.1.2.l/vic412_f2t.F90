...
!EOP
   ...
   do t = 1, NPATCH     
      ...
      if ( vic412_struc(n)%debugging_convert_units == 1 ) then
      ! convert from mm s-1 to mm
      rainf_temp(tid) = rainf_temp(tid) * LIS_rc%nts(n)
      call vic412_add_atmosdata(t, k, VAR, rainf_temp(tid))
      else
   ...
   do t = 1, NPATCH     
      tid = LIS_surface(n,LIS_rc%lsm_index)%tile(t)%tile_id 
      call vic412_add_atmosdata(t, k, VAR, air_temp_temp(tid))
   enddo
   ...
   if ( LIS_FORC_Snowflag%selectOpt == 1 ) then
      ...
      do t = 1, NPATCH     
          tid = LIS_surface(n,LIS_rc%lsm_index)%tile(t)%tile_id 
         call vic412_add_atmosdata(t, k, VAR, tempPtr(tid))
      enddo
   else
      do t = 1, NPATCH     
         tid = LIS_surface(n,LIS_rc%lsm_index)%tile(t)%tile_id 
   ...
   if ( LIS_FORC_Wind%selectOpt == 1 ) then
      ...
      do t = 1, NPATCH     
         ...
         call vic412_add_atmosdata(t, k, VAR, wind)
      enddo
   endif
   ...
   do t = 1, NPATCH     
      tid = LIS_surface(n,LIS_rc%lsm_index)%tile(t)%tile_id 
      call vic412_add_atmosdata(t, k, VAR, pressure_tempPtr(tid))
   enddo
   ...
   if ( LIS_FORC_VAPORPRESS%selectOpt == 1 ) then
      ...
      do t = 1, NPATCH     
         ...
         call vic412_add_atmosdata(t, k, VAR, tempvar)
         vp_temp(tid) = tempvar
      enddo
   endif
   ...
   if ( LIS_FORC_VAPORPRESSDEFICIT%selectOpt == 1 ) then
      ...
      do t = 1, NPATCH     
         ...
         tempvar = fsvp(tempC) - vp_temp(tid)
         call vic412_add_atmosdata(t, k, VAR, tempvar)
      enddo
   endif
   ...
   if ( alarmCheck ) then 
      ! Do not turn off ringer.  It is also needed in vic412_main.
      call vic412_nr_atmosdata(NPATCH)
      call vic412_write_atmos(NPATCH)
      vic412_struc(n)%snowstep = 1
   else
      vic412_struc(n)%snowstep = vic412_struc(n)%snowstep + 1
   endif


end subroutine vic412_f2t
