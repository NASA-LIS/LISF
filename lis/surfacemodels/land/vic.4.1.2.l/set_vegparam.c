...
int real_2_vic(int real_type, int Nveg_type)
{
   ...
   for(k=0; k<Nveg_type; k++)
    { 
        if(real_type == veg_lib[k].veg_class)
        {
            vic_type = k;
            flag = 1;
            break;
    ...
    if(flag==0)
    {
        //printf("LIS-VIC error (set_vegparam.c - real_2_vic): undefined land cover type in veg_lib!\n");
...
int is_understory_type(int real_type, int Nveg_type)
{
    extern veg_lib_struct *veg_lib;
    int vic_type;
    ...
    vic_type=real_2_vic(real_type, Nveg_type);
    if(vic_type==-1)
    {
        return(0);
    }
    if( veg_lib[vic_type].overstory==1)
    {
        return(0);
    }
    else
...
int is_overstory_type(int real_type, int Nveg_type)
{
    extern veg_lib_struct *veg_lib;
    int vic_type;
    if(real_type == BARETYPE)
...
int number_of_understory_types(int Nveg_type, int Nlc_type, float *veg_fracs)
{
    ...
    for(n=0; n<Nlc_type; n++)
    {
        ...
        {
            real_type = n + 1; 
            if(is_understory_type(real_type, Nveg_type)==1)
            {
               num_understory++;
            }
        }
...
int number_of_overstory_types(int Nveg_type, int Nlc_type, float *veg_fracs)
{
    int num_overstory = 0;
    int n, real_type;
    ...
    for(n=0; n<Nlc_type; n++)
    {
        ...
        {
            real_type = n + 1; 
            if(is_overstory_type(real_type, Nveg_type)==1)
            {
               num_overstory++;
            }
        }
...
double normalize_cv(int real_type000, int Nveg_type, int Nlc_type, float *fracs)
{
    ...
    {
        real_type = n + 1;
        if(is_understory_type(real_type, Nveg_type))
        {   
...
int has_above_treeline_band(int tile_idx)
{
    ...
    {
        if(lis_soil_con[tile_idx].AboveTreeLine[band] == TRUE)
        {
            flag = 1;
            break;        
        }
...
// !INTERFACE:
veg_con_struct *set_vegparam(int tile_idx, 
                             int vegclass, 
                             int Nveg_type,
                             int gridcel, 
                             FILE *fp_vegparam, 
...
//EOP
{
   ...
   if(options.SNOW_BAND>1                       &&  // there are multiple snow bands
      options.COMPUTE_TREELINE > 0              &&  // COMPUTE_TREELINE optin is turned on
      vegclass != BARETYPE                      &&  // not bare soil - BARETYPE may be replaced with LIS option later
      is_overstory_type(vegclass, Nveg_type)==1 &&  // current tile has overstory vegetation
      has_above_treeline_band(tile_idx))          // current grid has above-treeline band(s)
   {
        // (2) Determine whether there are understory vegetation class in the grid or not
        if(number_of_understory_types(Nveg_type, Nlc_type, veg_fracs)>=1)
        {
            AllOverstory = 0;
        }
        else
        ...
        {
            n_under = number_of_understory_types(Nveg_type, Nlc_type, veg_fracs);         
   ...
   temp[0].veg_class = vegclass;    // At this moment, veg_class is still the real vegetation class, VIC will
                                    // convert it into the index of corresponding vegetation class in veg_lib
                                    // in vic412_read_vegparam.c
   /* compute_treeline option, continue */
      ...
      has_above_treeline_band(tile_idx))            // current grid has above-treeline band(s)
   {
      ...
      if(AllOverstory==0)
      {
        ...
        for(n=1; n<vegetat_type_num; n++)
        {
            ...
            for(k=n0; k<Nveg_type; k++)
            {
                ...
                if(veg_fracs[k]>0 && is_understory_type(real_type, Nveg_type)==1)
                {
                    //printf("vegetat_type_num = %d veg_class = %d\n",vegetat_type_num, real_type);
                    temp[n].veg_class = real_type;
                    temp[n].Cv        = 0.001 * normalize_cv(real_type, Nveg_type, Nlc_type, veg_fracs);
                    tmp1 += temp[n].Cv;
                    n0 = k + 1;
                    break;
                }
      ...
      else
      {
        ...
        if(options.AboveTreelineVeg >=0) 
        {
            temp[1].veg_class = options.AboveTreelineVeg;
            temp[1].Cv        = 0.001;
        }
   ...
   if ( vegclass != BARETYPE )
   {
      for ( i = 0; i < vegetat_type_num; i++ )
      {
         ...
         flag = vic412_read_vegparam(fp_vegparam, gridcel , Nveg_type, vegclass, &(temp[i]));
         if(flag==0) // cannot find veg parameters
         {
            // Since root zones are not defined they are copied from the last
            // vegetation type.
            if(options.ROOT_ZONES>3)
            {
                n_root_zone = 3;
            }
            else
            {
                n_root_zone = options.ROOT_ZONES;
            }
