/*
 * SUMMARY:      vic411_IceEnergyBalance.c - Calculate lake ice energy balance
 ...
 * DESCRIP-END.
 * FUNCTIONS:    vic411_IceEnergyBalance()
 * COMMENTS:     
 */
...
static char vcid[] = "$Id: vic411_IceEnergyBalance.c,v 5.9 2006/11/10 01:28:55 vicadmin Exp $";

/*****************************************************************************
  Function name: vic411_IceEnergyBalance()
  ...
  16-Jul-04 Renamed VaporMassFlux to vapor_flux to denote fact that its
	    units are m/timestep rather than kg m-2 s-1.  Created new variable
	    VaporMassFlux with units of kg m-2 s-1.  After VaporMassFlux is
...
*****************************************************************************/
double vic411_IceEnergyBalance(double TSurf, va_list ap)
{

  extern vic411_option_struct vic411_options;

  const char *Routine = "vic411_IceEnergyBalance";
  ...
  double *qf;		/* Ground Heat Flux (W m-2) */
  ...
  double NetRad;			/* Net radiation exchange at surface (W m-2) */
...
/*   TMean = 0.5 * (OldTSurf + TSurf); */
  ...
  /* Correct aerodynamic conductance for stable conditions
     ...
     velocity (vel_2m) is expected to be in m s-1ec */                        
  ...
  if (Wind > 0.0)
    *Ra_used = Ra / vic411_StabilityCorrection(Z, 0.f, TMean, Tair, Wind, Z0);
    /* *Ra_used = Ra / vic411_StabilityCorrection(2.f, 0.f, TMean, Tair, Wind, Z0);*/
  else
    *Ra_used = HUGE_RESIST;
...
/*     EsSnow *= 1.0 + .00972 * TMean + .000042 * pow((double)TMean,(double)2.0); */
  ...
  /* blowing_flux was calculated outside of the vic411_root_brent iteration */
  BlowingMassFlux = *blowing_flux * Density / (Dt * SECPHOUR);

  vic411_latent_heat_from_snow(AirDens, EactAir, Lv, Press, Ra, TMean, Vpd,
