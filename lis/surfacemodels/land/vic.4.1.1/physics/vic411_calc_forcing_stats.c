...
**********************************************************************/
  ...
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].air_temp[vic411_NR];
  ...
  /** Density **/
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].density[vic411_NR];
  ...
  /** Longwave **/
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].longwave[vic411_NR];
  ...
  /** Precipitation **/
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].prec[vic411_NR];
  ...
  /** Pressure **/
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].pressure[vic411_NR];
  ...
  /** Shortwave **/
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].shortwave[vic411_NR];
  ...
  /** Vapor Pressure **/
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].vp[vic411_NR];
  ...
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].vpd[vic411_NR];
  ...
  /** Wind Speed **/
  for ( rec = 0; rec < Nrecs; rec ++ )
    values[rec] = atmos[rec].wind[vic411_NR];
  printf("wind speed (m s-1):\t%f\t%f\t%f\t%f\t%f\n", 
	 get_mean(values, Nrecs, -999), 
	 get_stdev(values, Nrecs, get_mean(values, Nrecs, -999), -999),
	 get_sum(values, Nrecs, -999), get_max(values, Nrecs, -999), 
	 get_min(values, Nrecs, -999) );
