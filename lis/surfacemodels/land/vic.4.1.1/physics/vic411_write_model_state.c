...
static char vcid[] = "$Id: vic411_write_model_state.c,v 5.16.2.8 2009/09/28 21:46:58 vicadmin Exp $";


void vic411_write_model_state(vic411_dist_prcp_struct    *prcp,
...
/*********************************************************************
  vic411_write_model_state      Keith Cherkauer           April 14, 2000
  ...
  06-06-03 It is not necessary to store the current ice content as
           it is recomputed in vic411_initialize_model_state.         KAC
...
*********************************************************************/
{
  ...
  if ( vic411_options.BINARY_STATE_FILE ) {
    Nbytes = ( vic411_options.Nnode * sizeof(double) // dz_node
	       + vic411_options.Nnode * sizeof(double) // Zsum_node
#if EXCESS_ICE
	       + vic411_options.Nlayer * sizeof(double) //soil depth
	       + vic411_options.Nlayer * sizeof(double) //effective porosity
	       ...
	       + (Nveg+1) * Nbands * Ndist * vic411_options.Nlayer * sizeof(double) // soil moisture
#if SPATIAL_FROST
	       + (Nveg+1) * Nbands * Ndist * vic411_options.Nlayer * FROST_SUBAREAS * sizeof(double) // soil ice
#else
	       + (Nveg+1) * Nbands * Ndist * vic411_options.Nlayer * sizeof(double) // soil ice
#endif // SPATIAL_FROST
	       ...
	       + (Nveg+1) * Nbands * vic411_options.Nnode * sizeof(double) ); // soil temperatures
	...
	+ Ndist * vic411_options.Nlayer * sizeof(double) // soil moisture
#if SPATIAL_FROST
	+ Ndist * vic411_options.Nlayer * FROST_SUBAREAS * sizeof(double) // soil ice
#else
	+ Ndist * vic411_options.Nlayer * sizeof(double) // soil ice
#endif // SPATIAL_FROST
	+ sizeof(int) // last_snow
	+ sizeof(char) // MELTING
	+ sizeof(double) * 9 // other snow parameters
	+ vic411_options.Nnode * sizeof(double) // soil temperatures
	;
    }
  ...
  for ( veg = 0; veg <= Nveg; veg++ ) {
	...
	if ( vic411_options.BINARY_STATE_FILE )
      ...
      if ( !vic411_options.BINARY_STATE_FILE ) fprintf( filep->statefile, "\n" );
