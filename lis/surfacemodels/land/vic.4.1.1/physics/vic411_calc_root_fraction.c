...
void vic411_calc_root_fractions(vic411_veg_con_struct  *veg_con,
...
**********************************************************************/
{
  ...
  for(veg=0;veg<Nveg;veg++) {
    ...
    while(zone<vic411_options.ROOT_ZONES) {
	...
	if(Zsum < Lsum - Lstep) {
	  ...
	  Zmin_fract = vic411_linear_interp(Zmin_depth,Zsum,Zsum+Zstep,0,
	...
	sum_fract += vic411_linear_interp(Zmax,Zsum,Zsum+Zstep,0,
	...
	Zsum += Zstep;
	zone ++;
	if(layer<vic411_options.Nlayer) {
	...
	else if(layer==vic411_options.Nlayer) {
	  Lstep  = Zsum + Zstep - Lsum;
	  if(zone<vic411_options.ROOT_ZONES-1) {
	    for(i=zone+1;i<vic411_options.ROOT_ZONES;i++) {
    ...
    if(sum_fract > 0 && layer >= vic411_options.Nlayer) {
      veg_con[veg].root[vic411_options.Nlayer-1] += sum_fract;
    }
    else if(sum_fract > 0) {
      veg_con[veg].root[layer] += sum_fract;
    }
