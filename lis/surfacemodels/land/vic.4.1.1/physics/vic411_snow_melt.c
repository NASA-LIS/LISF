...
static char vcid[] = "$Id: vic411_snow_melt.c,v 5.7.2.5 2009/10/08 21:30:59 vicadmin Exp $";
...
/*****************************************************************************
  ...
  04-Jun-04 For the case in which snowpack is too thin to solve
	    separately, added message explaining that vic411_root_brent's
  ...
  16-Jul-04 Added "month" to parameter list to allow this function to
	    pass month to vic411_latent_heat_from_snow().  Changed calculations
  ...
  16-Jul-04 Changed the type of the last few variables (lag_one, iveg,
	    etc) to be double in the parameter lists of vic411_root_brent
	    and vic411_CalcSnowPackEnergyBalance.  For some reason, passing
  ...
  05-Aug-04 Removed overstory, lag_one, sigma_slope, fetch, iveg, Nveg,
	    ...
	    call to vic411_SnowPackEnergyBalance (and vic411_ErrorSnowPackEnergyBalance),
  ...
  21-Sep-04 Added ErrorString to store error messages from
	    ...
	    Removed message explaining non-fatal vic411_root_brent warning.
  ...
  2007-Apr-11 Modified to handle grid cell errors by returning to the
	      vic411_main subroutine, rather than ending the simulation.	KAC via TJB
...
*****************************************************************************/
int  vic411_snow_melt(double            Le, 
	       ...
	       vic411_soil_con_struct  *soil_con)
{
  ...
  double RefreezeEnergy;         /* refreeze/melt energy in surface layer (W m-2) */
  ...
  double vic411_advected_sensible_heat;		
  ...
  Qnet = vic411_CalcSnowPackEnergyBalance((double)0.0, delta_t, aero_resist, aero_resist_used,
				   displacement, z2, Z0, 
				   ...
				   Tcanopy, Tgrnd, 
				   &advection, &vic411_advected_sensible_heat, 
				   &deltaCC, 
...
//        snow->surface_flux *= -( snow->surf_water / snow->vapor_flux );
    ...
    /* Else, vic411_SnowPackEnergyBalance(T=0.0) <= 0.0 */
    else  {
	...
	snow->surf_temp = vic411_root_brent((double)(snow->surf_temp-SNOW_DT), 
				     ...
				     ErrorString, vic411_SnowPackEnergyBalance, 
	    ...
	    error = vic411_ErrorSnowPackEnergyBalance(snow->surf_temp, rec, iveg, band,
					 delta_t, aero_resist, aero_resist_used,
					 displacement, z2, Z0, 
					 density, vp, LongSnowIn, Le, pressure,
					 RainFall, NetShortSnow, vpd, 
...
//        fprintf(stderr,"Snowpack is too thin to solve separately; it will be solved in conjunction with ground surface energy balance\n");
	...
	Qnet = vic411_CalcSnowPackEnergyBalance(snow->surf_temp, 
					 delta_t, aero_resist, aero_resist_used,
					 displacement, z2, Z0, 
					 density, vp, LongSnowIn, Le, pressure,
					 RainFall, NetShortSnow, vpd, 
...
/*  printf("%d %d %g\n", y, x, MassBalanceError);*/
  ...
  *save_advected_sensible  = vic411_advected_sensible_heat;
...
/*****************************************************************************
  Function name: vic411_CalcSnowPackEnergyBalance()
  ...
  Required     : 
    ...
    other arguments required by vic411_SnowPackEnergyBalance()
...
double vic411_CalcSnowPackEnergyBalance(double Tsurf, ...)
{
  ...
  va_start(ap, Tsurf);
  Qnet = vic411_SnowPackEnergyBalance(Tsurf, ap);
  va_end(ap);
...
double vic411_ErrorSnowPackEnergyBalance(double Tsurf, ...)
{
  ...
  va_start(ap, Tsurf);
  Qnet = vic411_ErrorPrintSnowPackEnergyBalance(Tsurf, ap);
  va_end(ap);
  
  return Qnet;
}

double vic411_ErrorPrintSnowPackEnergyBalance(double TSurf, va_list ap)
{
  ...
  double LongSnowIn;              /* Incoming longwave radiation (W m-2) */
  ...
  double *DeltaPackColdContent;   /* Change in sold content of pack 
				     layer (W/m^2) */
  double *GroundFlux;		  /* Ground Heat Flux (W m-2) */
  ...
  fprintf(stderr, "ERROR: vic411_snow_melt failed to converge to a solution in vic411_root_brent.  Variable values will be dumped to the screen, check for invalid values.\n");
