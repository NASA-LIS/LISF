...
static char vcid[] = "$Id: vic411_snow_intercept.c,v 5.7.2.7 2009/09/20 02:32:07 vicadmin Exp $";
...
/*****************************************************************************
  ...
  2-20-03 Added check of intercepted before mass balance error 
        ...
        in vic411_redistribute_during_storm as it will be unable to conserve
  ...
  04-Jun-04 Added descriptive error message to beginning of screen dump
	    in vic411_error_print_canopy_energy_bal.			TJB
  ...
  2007-Apr-11 Modified to handle grid cell errors by returning to the
	      vic411_main subroutine, rather than ending the simulation.	KAC
  ...
  2007-Aug-31 Checked vic411_root_brent return value against -998 rather than
	      -9998.							JCA
...
*****************************************************************************/
int vic411_snow_intercept(double  AirDens,
		    double  Dt, 
		    double  EactAir, 		    
		    double  F,  
		    double  LAI, 
		    ...
		    vic411_veg_var_struct    *veg_var_wet)
{
  ...
  /* double AdvectedEnergy; */         /* Energy advected by the rain (W m-2) */
  ...
  double ReleasedMass;           /* Amount of mass release of intercepted snow
				    (m) */ 
  /* double SensibleHeat; */           /* Sensible heat flux (W m-2) */
  ...
  *IntSnow += DeltaSnowInt;
  ...
  if ( *IntSnow > 0 || *SnowFall > 0 ) {
    ...
    *NetShortOver = (1. - *AlbedoOver) * ShortOverIn; // net SW in canopy
    ...
    Qnet = vic411_solve_canopy_energy_bal(0., band, month, rec, Dt, 
				   soil_con->elevation, 
  ...
  if ( Tupper != MISSING && Tlower != MISSING ) {
    ...
    *Tfoliage = vic411_root_brent(Tlower, Tupper, ErrorString, vic411_func_canopy_energy_bal,  band, 
    ...
    if ( *Tfoliage <= -998 ) {
      if (vic411_options.TFALLBACK) {
        ...
        *Tfoliage_fbflag = 1;
        ...
        Qnet = vic411_error_calc_canopy_energy_bal(*Tfoliage, band, month, rec, Dt, 
					    soil_con->elevation, 
					    soil_con->Wcr, soil_con->Wpwp, 
					    soil_con->depth, 
#if SPATIAL_FROST
    ...
    Qnet = vic411_solve_canopy_energy_bal(*Tfoliage, band, month, rec, Dt, 
  ...
  if ( *Tfoliage == 0 ) {
    ...
    if ((*IntRain + PotSnowMelt) <= MaxWaterInt) {
      ...
      *IntRain = MaxWaterInt;
      ...
      vic411_MassRelease(IntSnow, TempIntStorage, &ReleasedMass, &Drip);
    }
...
double vic411_solve_canopy_energy_bal(double Tfoliage, ...)
{
  ...
  Qnet = vic411_func_canopy_energy_bal(Tfoliage, ap);
  va_end(ap);

  return Qnet;
}

double vic411_error_calc_canopy_energy_bal(double Tfoliage, ...)
{
  va_list  ap;
  double   Qnet;

  va_start(ap, Tfoliage);
  Qnet = vic411_error_print_canopy_energy_bal(Tfoliage, ap);
  va_end(ap);

  return Qnet;
}

double vic411_error_print_canopy_energy_bal(double Tfoliage, va_list ap)
{  
  ...
  fprintf(stderr, "ERROR: vic411_snow_intercept failed to converge to a solution in vic411_root_brent.  Variable values will be dumped to the screen, check for invalid values.\n");
