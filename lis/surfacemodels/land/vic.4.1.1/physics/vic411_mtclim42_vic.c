/*
  Note:  The following code is largely taken from mtclim4.2, a weather
	 ...
	 typedefs and function definitions have been moved to vic411_mtclim42_vic.h
	 functions are called from vic411_mtclim42_wrapper.c
...
changes from vic411_version 3.1 to vic411_version 4.0:
...
Changes from vic411_version 2.0 to vic411_version 3.1
...
Some other differences between vic411_version 3.1 and previous versions of MTCLIM:
...
Radiation            W m-2, average over daylight period
...
Updated  1/ 4/1993 by Raymond Hunt (vic411_version 2.1)
Updated  3/26/1997 by Peter Thornton (vic411_version 3.0)
Updated  5/16/1997 P.T. corrected error in calculation of tmean and tday,
spotted by Mike White
Updated  7/14/1997 P.T. (vic411_version 3.1) replaced an older vic411_version of the vapor
pressure correction with a newer algorithm. The algorithm implemented in this
vic411_version corresponds to that found in:
...
Updated 7/28/1997 P.T. Finalized MTCLIM vic411_version 3.1
...
1                          (int) Humidity output vic411_flag        (0=VPD, 1=VP)
...
******************************
There is a vic411_flag in the initialization file that controls the input of dewpoint
temperature information. If your input file does not contain dewpoint data,
set this vic411_flag to 0. Otherwise, if you have dewpoint temperature information 
in your input file, set this vic411_flag to 1, and be sure that the dewpoint
temperature field is between the tmin and prcp fields, as specified under
the heading "INPUT FILE INFO", above.

There is another vic411_flag in the initialization file that controls the output of
...
to the ambient vapor pressure, in Pa. When the vic411_flag is set to 1, the output is
simply the  ambient vapor pressure in Pa.  By using the ambient vapor pressure
(vic411_flag  set to 1), you can avoid possible errors related to a difference between
the temperature chosen for saturation calculations (in this case tday) and the
actual temperature at any given time of day.

Another vic411_flag controls the treatment of a year-field in the input and output
files. When the vic411_flag is set (1), it is assumed that the first field in the
...
MTCLIM vic411_version 3.1 testing
...
#include <vic411_mtclim42_vic.h>   /* physical constants */
...
/* vic411_data_alloc() allocates space for input and output data arrays */
int vic411_data_alloc(const control_struct *ctrl, data_struct *data)
{
  ...
  if (ok && ctrl->inyear && !(data->year = (int*) malloc(ndays * sizeof(int)))) {
    printf("vic411_Error allocating for year array\n");
    ok=0;
  } 
  if (ok && !(data->yday = (int*) malloc(ndays * sizeof(int)))) {
    printf("vic411_Error allocating for yearday array\n");
    ok=0;
  } 
  if (ok && !(data->tmax = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for tmax array\n");
    ok=0;
  }
  if (ok && !(data->tmin = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for tmin array\n");
    ok=0;
  }
  if (ok && !(data->prcp = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for prcp array\n");
    ok=0;
  }
  if (ok && ctrl->indewpt && 
      !(data->tdew = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for input humidity array\n");
    ok=0;
  }
  if (ok && !(data->s_tmax = (double*) malloc(ndays * sizeof(double))))	{
    printf("vic411_Error allocating for site Tmax array\n");
    ok=0;
  }
  if (ok && !(data->s_tmin = (double*) malloc(ndays * sizeof(double))))	{
    printf("vic411_Error allocating for site Tmin array\n");
    ok=0;
  }
  if (ok && !(data->s_tday = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for site Tday array\n");
    ok=0;
  }
  if (ok && !(data->s_prcp = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for site prcp array\n");
    ok=0;
  }
  if (ok && !(data->s_hum = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for site VPD array\n");
    ok=0;
  }
  if (ok && !(data->s_srad = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for site radiation array\n");
    ok=0;
  }
  if (ok && !(data->s_dayl = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for site daylength array\n");
    ok=0;
  }
  /* start vic_change */
  if (ok && !(data->s_tskc = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for site cloudiness array\n");
    ok=0;
  }
  /* end vic_change */
  return (!ok);
} /* end of vic411_data_alloc() */

/* vic411_calc_tair() calculates daily air temperatures */
...
/* end of vic411_calc_tair() */

/* vic411_calc_prcp() calculates daily total precipitation */
	      ...
	      data_struct *data)
{
  ...
  if ( p->site_isoh < 1e-10 && p->base_isoh < 1e-10 ) {
    ...
    vic411_vicerror("vic411_Error in vic411_calc_prcp(): base_isoh == 0 and site_isoh/base_isoh == NaN.");
  }
  else {
...
/* end vic_change */
{
  ...
  if (!(dtr = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for DTR array\n");
    ok=0;
  }
  if (!(sm_dtr = (double*) malloc(ndays * sizeof(double)))) {
    printf("vic411_Error allocating for smoothed DTR array\n");
    ok=0;
  }
  ...
  if (ndays >= 30) {
    if (vic411_pulled_boxcar(dtr, sm_dtr, ndays, 30, 0)) {
      printf("vic411_Error in boxcar smoothing, vic411_calc_srad_humidity()\n");
      ok=0;
    }
  }
  else /* smoothing window width = ndays */ {
    if (vic411_pulled_boxcar(dtr, sm_dtr, ndays, ndays, 0)) {
      printf("vic411_Error in boxcar smoothing, vic411_calc_srad_humidity()\n");
      ok=0;
    }
  }
  ...
  /*****************************************
   *                                       *
   * start of the vic411_main radiation algorithm *
   *                                       *
   *****************************************/
  ...
  return (!ok);
} /* end of vic411_calc_srad_humidity() */
  ...
  /* end vic_change */
{
  ...
  pa = vic411_atm_pres(p->site_elev);
  for (i=0 ; i<ndays ; i++) {
    tmink = data->s_tmin[i] + 273.15;
    pet = vic411_calc_pet(data->s_srad[i],data->s_tday[i],pa,data->s_dayl[i]);
  ...
  return (!ok);
} /* end of vic411_calc_srad_humidity_iterative() */
  
/* vic411_data_free frees the memory previously allocated by vic411_data_alloc() */
...
/* vic411_calc_pet() calculates the potential evapotranspiration for aridity 
...
double vic411_calc_pet(double rad, double ta, double pa, double dayl)
{
  /* input parameters and units :
     double rad      (W m-2)  daylight average incident shortwave radiation
  ...
  */
  ...
  /* calculate the psychrometer parameter: gamma = (cp pa)/(lhvap epsilon)
     where:
     cp       (J kg-1 K)   specific heat of air
...
/* vic411_atm_pres() calculates the atmospheric pressure as a function of elevation */
...
/* vic411_pulled_boxcar() calculates a moving average of antecedent values in an
