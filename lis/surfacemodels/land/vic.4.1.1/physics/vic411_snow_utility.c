...
double vic411_snow_density(vic411_snow_data_struct *snow, double new_snow, double sswq, double Tgrnd, double Tair, double dt)
{
/**********************************************************************
  vic411_snow_density		Keith Cherkauer		May 28, 1997

  This subroutine computes the snow density based on swe and snow metamorphism.

  One of two algorithms may be used, depending on the value of vic411_options.SNOW_DENSITY:
...
**********************************************************************/
  ...
  if (new_snow > 0.)
    density_new = vic411_new_snow_density(Tair);
  else
    density_new = 0.0;
...
//  Tavg = (snow->surf_temp+Tgrnd)/2.0+KELVIN;
  ...
  if (vic411_options.SNOW_DENSITY == DENS_SNTHRM) {
...
//    f = sswq/80. * exp(-sswq/100.);
  ...
  else if (vic411_options.SNOW_DENSITY == DENS_BRAS) {
...
double vic411_new_snow_density(double air_temp) {
/**********************************************************************
  vic411_new_snow_density		Keith Cherkauer		May 28, 1997
...
double vic411_snow_albedo(double new_snow,
                   double swq,
                   double depth,
                   double albedo,
                   double cold_content,
		   ...
		   char   MELTING) {
/**********************************************************************
  vic411_snow_albedo		Keith Cherkauer		June 10, 1997

  This subroutine computes the snow pack surface albedo.  Original vic411_version
  ...
  Modified:
  06-15-02 Added MELTING vic411_flag which tells the algorithm whether or not
  ...
  2008-Apr-21 Added vic411_flag to include new albedo calculation based on
...
**********************************************************************/
  ...
  /** Aged Snow **/
    ...
    if ( vic411_options.SNOW_ALBEDO == SUN1999 ) {
