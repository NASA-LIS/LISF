/*
 * SUMMARY:      vic411_CalcBlowingSnow.c - Calculate energy of sublimation from blowing snow
 * USAGE:        Part of VIC
 *
 ...
 * DESCRIP-END.
 * FUNCTIONS:    vic411_CalcBlowingSnow()
 * COMMENTS:     
 * Modifications:
 *   05-Aug-04 Merged with Laura Bowling's updated code to fix the following problems:
 *	       - vic411_Error in array declaration line 373 and 375
 ...
 *   2007-Apr-03 Module returns an ERROR value that can be trapped in vic411_main      GCT
 */
...
#include <vic411_mtclim42_vic.h>

static char vcid[] = "$Id: vic411_CalcBlowingSnow.c,v 1.4.2.1 2007/04/18 20:18:58 vicadmin Exp $";
...
#define CSALT 0.68              /* saltation constant m s-1 */
#define UTHRESH 0.25            /* threshold shear velocity m s-1 */
...
#define SETTLING 0.3            /* Particle settling velocity m s-1 */
#define UPARTICLE 2.8*UTHRESH   /* Horizontal particle velocity m s-1 */
...
double vic411_qromb(double (*vic411_sub_with_height)(), double es, double Wind, double AirDens, double ZO, 
	     double EactAir, double F, double hsalt, double phi_r, double ushear, double Zrh, 
	     double a, double b);
double (*vic411_funcd)(double z,double es,  double Wind, double AirDens, double ZO,          
			  double EactAir,double F, double hsalt, double phi_r,         
			  double ushear, double Zrh);
double vic411_sub_with_height(double z,double es,  double Wind, double AirDens, double ZO,          
			  double EactAir,double F, double hsalt, double phi_r,         
			  double ushear, double Zrh);
double vic411_transport_with_height(double z,double es,  double Wind, double AirDens, double ZO,
				double EactAir,double F, double hsalt, double phi_r,         
				double ushear, double Zrh);
double vic411_rtnewt(double x1, double x2, double xacc, double Ur, double Zr);
void vic411_get_shear(double x, double *f, double *df, double Ur, double Zr);
double vic411_get_prob(double Tair, double Age, double SurfaceLiquidWater, double U10);
double vic411_get_thresh(double Tair, double SurfaceLiquidWater, double Zo_salt, int vic411_flag);
void vic411_shear_stress(double U10, double ZO,double *ushear, double *Zo_salt, double utshear);
double vic411_CalcSubFlux(double EactAir, double es, double Zrh, double AirDens, double utshear, 
		   double ushear, double fe, double Tsnow, double Tair, double U10, 
		   double Zo_salt, double F, double *Transport);

/*****************************************************************************
  Function name: vic411_CalcBlowingSnow()
  ...
  double Wind;                    Wind speed (m s-1), 2 m above snow 
  double Ls;                      Latent heat of sublimation (J kg-1) 
  double AirDens;                 Density of air (kg/m3) 
  double Lv;                      Latent heat of vaporization (J kg-13) 
  ...
  Comments     : Called from vic411_SnowPackEnergyBalance
    Reference:  

*****************************************************************************/
double vic411_CalcBlowingSnow( double Dt, 
			double Tair,
			int LastSnow,
...

{
  ...
  *TotalTransport = 0.0;
  ...
  if(snowdepth > 0.0) {
	...
	lower = upper;
	fprintf(stderr,"Warning: vic411_Error with probability boundaries in vic411_CalcBlowingSnow()\n");
      }
	...
	fprintf(stderr,"ERROR in vic411_CalcBlowingSnow.c: Problem with probability ranges\n");
	...
	Uveg = U10;
      ...
      prob_occurence = vic411_get_prob(Tair, Age, SurfaceLiquidWater, Uveg);
      ...
      utshear = vic411_get_thresh(Tair, SurfaceLiquidWater, ZO, VAR_THRESHOLD);
   
      /* Iterate to find actual shear stress during saltation. */

      vic411_shear_stress(U10, ZO, &ushear, &Zo_salt, utshear);
      
      if(ushear > utshear) {
	
	SubFlux = vic411_CalcSubFlux(EactAir, es, Zrh, AirDens, utshear,ushear, fe, Tsnow, 
	...
	Uveg = U10;
      ...
      vic411_shear_stress(Uo, ZO, &ushear, &Zo_salt, utshear);
...
double vic411_qromb(double (*vic411_funcd)(), double es, double Wind, double AirDens, double ZO, 
	     double EactAir, double F, double hsalt, double phi_r, double ushear, double Zrh, 
	     double a, double b)
...
{
  void vic411_polint(double xa[], double ya[], int n, double x, double *y, double *dy);
  double vic411_trapzd(double (*vic411_funcd)(), double es, double Wind, double AirDens, 
  ...
  for(j=1; j<=MAX_ITER; j++) {
    s[j]=vic411_trapzd(vic411_funcd,es, Wind, AirDens, ZO, EactAir, F, hsalt, phi_r, 
		ushear, Zrh, a,b,j);
    if(j >= K) {
      vic411_polint(&h[j-K],&s[j-K],K,0.0,&ss,&dss);
      if (fabs(dss) <= MACHEPS*fabs(ss)) return ss;
    }
    h[j+1]=0.25*h[j];
  }
  vic411_nrerror("Too many steps in routine vic411_qromb");
  return 0.0;
}
...
void vic411_polint(double xa[], double ya[], int n, double x, double *y, double *dy)
{
  ...
  if(!c) vic411_nrerror("allocation failure in vector()");
  d=(double *)malloc((size_t) ((n+1)*sizeof(double)));
  if(!d) vic411_nrerror("allocation failure in vector()");
  ...
  *y=ya[ns--];
  for(m=1;m<n;m++) {
    for(i=1; i<=n-m; i++) {
      ...
      hp=xa[i+m]-x;
      w=c[i+1]-d[i];
      if ( (den=ho-hp) == 0.0) vic411_nrerror("vic411_Error in routine vic411_polint");
      den = w/den;
      d[i]=hp*den;
      c[i]=ho*den;
    }
...
double vic411_trapzd(double (*vic411_funcd)(), double es, double Wind, double AirDens, double ZO, 
	      ...
	      double Zrh, double a, double b, int n)
{
  ...
  if (n==1) {
    return (s=0.5*(b-a)*((*vic411_funcd)(a, es, Wind, AirDens, ZO, EactAir, F, hsalt, 
					 phi_r, ushear, Zrh) + 
			 (*vic411_funcd)(b,es, Wind, AirDens, ZO, EactAir, F, hsalt, 
					 phi_r, ushear, Zrh)));
  }
  else {
    ...
    del=(b-a)/tnm;
    x=a+0.5*del;
    for(sum=0.0, j=1; j<=it; j++, x+=del ) sum += (*vic411_funcd)(x,es, Wind, AirDens, ZO, EactAir, 
...
double vic411_rtnewt(double x1, double x2, double acc, double Ur, double Zr)
{
  int j;
  double df, dx, dxold, f, fh, fl;
  double temp, xh, xl, rts;

    vic411_get_shear(x1,&fl,&df, Ur, Zr);
    vic411_get_shear(x2,&fh,&df, Ur, Zr);

    if ((fl > 0.0 && fh > 0.0) || (fl < 0.0 && fh < 0.0)) {
      fprintf(stderr, "Root must be bracketed in vic411_rtnewt.\n");
      exit(0);
    }
    ...
    dxold=fabs(x2-x1);
    dx=dxold;
    vic411_get_shear(rts,&f,&df, Ur, Zr);
    for(j=1; j<=MAX_ITER; j++) {
    ...
    fprintf(stderr, "Maximum number of iterations exceeded in vic411_rtnewt.\n");
    return 0.0;
}
...
/*****************************************************************************
  Function name: vic411_sub_with_height()
  ...
  Required     :
    ...
    double ushear;         - shear velocity (m s-1) 
...
*****************************************************************************/
double vic411_sub_with_height(double z,
		       double es,          
		       double Wind,          
		       double AirDens,     
		       double ZO,          
...
void vic411_shear_stress(double U10, double ZO,double *ushear, double *Zo_salt, double utshear)
{
  ...
  /* Check to see if value is bracketed. */
   vic411_get_shear(umin,&fl,&df, U10, 10.);
   vic411_get_shear(umax,&fh,&df, U10, 10.);

    if(fl < 0.0 && fh < 0.0) { 
      fprintf(stderr, "Solution in vic411_rtnewt surpasses upper boundary.\n");
    ...
    if(fl > 0.0 && fh > 0.0) {
      ...
      *ushear = vic411_rtnewt (umin, umax, xacc, U10, 10.);
...

{
  ...
  if(SIMPLE) {   
    ...
    //  Sublimation flux (kg m-2*s) = mass-concentration * sublimation rate * height
	...
	psi_s =  vic411_sub_with_height(hsalt/2.,es, U10, AirDens, Zo_salt, EactAir, F, hsalt, 
				 phi_s, ushear, Zrh);
    
	// Sublimation from the saltation layer in kg m-2*s
	SubFlux = phi_s*psi_s*hsalt;
    
	//  Suspension layer must be integrated
	SubFlux += vic411_qromb(vic411_sub_with_height, es, U10, AirDens, Zo_salt, EactAir, F, hsalt,
    ...
    suspension_transport = vic411_qromb(vic411_transport_with_height, es, U10, AirDens, Zo_salt, 
...
/*****************************************************************************
  Function name: vic411_transport_with_height()
...
*****************************************************************************/
double vic411_transport_with_height(double z,
		       double es,          
		       double Wind,          
		       double AirDens,     
		       double ZO,          
