...
static char vcid[] = "$Id: vic411_canopy_evap.c,v 5.3.2.1 2009/06/09 09:54:10 vicadmin Exp $";

/**********************************************************************
	vic411_canopy_evap.c	Dag Lohmann		September 1995
  ...
  vic411_atmos_data_struct    atmos         N/A   atmospheric forcing data structure
  vic411_layer_data_struct   *layer         N/A   soil layer variable structure
  vic411_veg_var_struct      *veg_var       N/A   vegetation variable structure
  ...
  vic411_global_param_struct  global        N/A   global parameter structure
  ...
  4-12-98  Code cleaned and final vic411_version prepared, KAC
...
double vic411_canopy_evap(vic411_layer_data_struct *layer_wet,
...
/********************************************************************** 
  ...
  Modified 
     ...
     6-8-2000 Modified to use spatially distributed soil frost if 
              present.                                           KAC
     5-8-2001 Modified to close the canopy energy balance.       KAC
  2009-Jun-09 Moved computation of canopy resistance rc out of vic411_penman()
	      and into separate function vic411_calc_rc().				TJB
...

{
  ...
  vic411_veg_var_struct    *tmp_veg_var;
  ...
  for ( dist = 0; dist < Ndist; dist++ ) {
    ...
    for ( i = 0; i < vic411_options.Nlayer; i++ ) layerevap[i] = 0;
    canopyevap = 0;
    throughfall = 0;
    ...
    if(mu > 0) {
      ...
      ****************************************************/
      ...
      if (tmp_Wdew > vic411_veg_lib[veg_class].Wdmax[month-1]) {
	throughfall = tmp_Wdew - vic411_veg_lib[veg_class].Wdmax[month-1];
	tmp_Wdew    = vic411_veg_lib[veg_class].Wdmax[month-1];
      }
      
      rc = vic411_calc_rc((double)0.0, net_short, vic411_veg_lib[veg_class].RGL,
		   air_temp, vpd, vic411_veg_lib[veg_class].LAI[month-1],
		   (double)1.0, FALSE);
      canopyevap = pow((tmp_Wdew / vic411_veg_lib[veg_class].Wdmax[month-1]),(2.0/3.0))
		   * vic411_penman(air_temp, elevation, rad,
			    vpd, ra, rc, vic411_veg_lib[veg_class].rarc)
		   * delta_t / SEC_PER_DAY;
	...
	tmp_Wdew = vic411_veg_lib[veg_class].Wdmax[month-1];
      ...
      *******************************************/
      if(CALC_EVAP)
	vic411_transpiration(tmp_layer, veg_class, month, rad, vpd, net_short, 
...
void vic411_transpiration(vic411_layer_data_struct *layer,
		   int veg_class, 
		   int month, 
		   double rad,
		   double vpd,
...
**********************************************************************/
{
  ...
  **********************************************************************/
  ...
  moist1 = 0.0;
  Wcr1 = 0.0;  
  for(i=0;i<vic411_options.Nlayer-1;i++){
    if(root[i] > 0.) {
#if SPATIAL_FROST
      avail_moist[i] = 0;
      for ( frost_area = 0; frost_area < FROST_SUBAREAS; frost_area++ ) {
  ...
  ******************************************************************/
  ...
  if( (moist1>=Wcr1 && moist2>=Wcr[vic411_options.Nlayer-1] && Wcr1>0.) ||
      (moist1>=Wcr1 && (1-root[vic411_options.Nlayer-1])>= 0.5) ||
      (moist2>=Wcr[vic411_options.Nlayer-1] &&
      root[vic411_options.Nlayer-1]>=0.5) ){
    gsm_inv=1.0;
    rc = vic411_calc_rc(vic411_veg_lib[veg_class].rmin, net_short, vic411_veg_lib[veg_class].RGL,
		 air_temp, vpd, vic411_veg_lib[veg_class].LAI[month-1], gsm_inv, FALSE);
    evap = vic411_penman(air_temp, elevation, rad, vpd, ra, rc,
		  vic411_veg_lib[veg_class].rarc)
	   * delta_t / SEC_PER_DAY
	   * (1.0-f*pow((Wdew/vic411_veg_lib[veg_class].Wdmax[month-1]), (2.0/3.0)));
  ...
  *********************************************************************/
	...
	/** Compute potential evapotranspiration **/
        rc = vic411_calc_rc(vic411_veg_lib[veg_class].rmin, net_short, vic411_veg_lib[veg_class].RGL,
		     air_temp, vpd, vic411_veg_lib[veg_class].LAI[month-1], gsm_inv, FALSE);
        layerevap[i] = vic411_penman(air_temp, elevation, rad, vpd, ra, rc,
			      vic411_veg_lib[veg_class].rarc)
		       * delta_t / SEC_PER_DAY * (double)root[i]
		       * (1.0-f*pow((Wdew/vic411_veg_lib[veg_class].Wdmax[month-1]),
					 (2.0/3.0)));
      }
