...
static char vcid[] = "$Id: vic411_func_surf_energy_bal.c,v 5.8.2.14 2009/09/20 02:32:07 vicadmin Exp $";

double vic411_func_surf_energy_bal(double Ts, va_list ap)
/**********************************************************************
	vic411_func_surf_energy_bal	Keith Cherkauer		January 3, 1996
  ...
  16-Jul-04 Renamed VaporMassFlux, BlowingMassFlux, and SurfaceMassFlux
	    ...
	    kg m-2 s-1.  Created new variables VaporMassFlux, BlowingMassFlux,
	    and SurfaceMassFlux with units of kg m-2 s-1.  The addresses of
	    the *MassFlux variables are passed to vic411_latent_heat_from_snow()
  ...
  05-Aug-04 Removed iveg, LastSnow, dt, SnowDepth, lag_one, sigma_slope,
	    ...
	    vic411_latent_heat_from_snow() which no longer needs them.	TJB
  ...
  2007-Apr-11 Modified to handle grid cell errors by returning to the
              vic411_main subroutine, rather than ending the simulation.	GCT
  ...
  2007-Aug-07 Moved Implicit error counting above call for
              vic411_solve_T_profile.						JCA
  2008-Aug-08 Added option for EXCESS_ICE.				JCA
              including: passing entire soil_con structure to
              vic411_calc_surf_energy_bal
  2007-Aug-24 Modified to use vic411_arno_evap rather than vic411_canopy_evap if LAI
  ...
  2009-May-20 Corrected the deltaH and fusion terms to account for
	      ...
	      Added vic411_options.GRND_FLUX_TYPE to allow backwards-compatibility
...
/*   double snow_depth; */
  ...
  double Evap;		/** Total evap in m s-1 **/
...
/*   double             kappa_snow; */
  ...
  ************************************/
  ...
  /* model structures */
  soil_con                = (vic411_soil_con_struct *) va_arg(ap, vic411_soil_con_struct *);
  ...
  ***************/
  ...
  if(vic411_options.GRND_FLUX) {
    ...
    ***************************************************************/
	...
	Flux
      **************************************************************/
      *T1 = vic411_estimate_T1(TMean, T1_old, T2, D1, D2, kappa1, kappa2, Cs1, 
			Cs2, dp, delta_t);
    ...
    *************************************************************/
	...
	vic411_Error = vic411_solve_T_profile_implicit(Tnew_node, T_node, Zsum_node, kappa_node, Cs_node, 
	...
	if(FIRST_SOLN[1]){
      ...
      if(!vic411_options.IMPLICIT || vic411_Error == 1) {
	if(vic411_options.IMPLICIT)
	  FIRST_SOLN[0] = TRUE;
#if QUICK_FS
	vic411_Error = vic411_solve_T_profile(Tnew_node, T_node, Tnew_fbflag, Tnew_fbcount, Zsum_node, kappa_node, Cs_node, 
	...
	vic411_Error = vic411_solve_T_profile(Tnew_node, T_node, Tnew_fbflag, Tnew_fbcount, Zsum_node, kappa_node, Cs_node, 
      ...
      if ( (int)vic411_Error == ERROR ) {
	fprintf(stderr, "ERROR: vic411_func_surf_energy_bal calling vic411_solve_T_profile\n");
	return( ERROR ); 
      }
      *T1 = Tnew_node[1];
    ...
    ******************************************************/
    if((FS_ACTIVE && vic411_options.FROZEN_SOIL) && (TMean+ *T1)/2.<0.) {
      ice = moist - vic411_maximum_unfrozen_water((TMean+ *T1)/2.,
#if EXCESS_ICE
    ...
    if (vic411_options.GRND_FLUX_TYPE == GF_FULL) {
	...
	*deltaCC = CH_ICE * (snow_swq - snow_water) * (TMean - OldTSurf) 
	  / delta_t;
      *refreeze_energy = (snow_water * Lf * vic411_snow_density) / delta_t;
  ...
  if ( wind[UnderStory] > 0.0 && overstory && SNOWING )
    ra_under = ra[UnderStory] 
      / vic411_StabilityCorrection(ref_height[UnderStory], 0.f, TMean, Tair, 
			    wind[UnderStory], roughness[UnderStory]);
  else if ( wind[UnderStory] > 0.0 )
    ra_under = ra[UnderStory] 
      / vic411_StabilityCorrection(ref_height[UnderStory], displacement[UnderStory], 
  ...
  /*************************************************
    ...
    winter crop planted).
  *************************************************/
  if ( VEG && !SNOWING && vic411_veg_lib[veg_class].LAI[month-1] > 0 ) {
    Evap = vic411_canopy_evap(layer_wet, layer_dry, veg_var_wet, veg_var_dry, TRUE, 
    ...
    Evap = vic411_arno_evap(layer_wet, layer_dry, NetBareRad, Tair, vpd, 
  ...
  *latent_heat_sub = 0.;
  ...
  if (INCLUDE_SNOW) {
    ...
    /* Convert sublimation terms from m/timestep to kg m-2 s-1 */
    ...
    vic411_latent_heat_from_snow(atmos_density, vp, Le, atmos_pressure, 
    ...
    *latent_heat_sub = temp_latent_heat_sub * snow_coverage;
    ...
    /* Convert sublimation terms from kg m-2 s-1 to m/timestep */
  ...
  ************************************************/
  ...
  if(vic411_options.GRND_FLUX) {
    ...
    /*************************************
      Compute Surface Energy Balance vic411_Error
    *************************************/
