/*
 * SUMMARY:      vic411_water_energy_balance.c - Calculate snow accumulation and melt for the lake model
 * USAGE:        
 *
...
//static char vcid[] = "$Id: vic411_water_energy_balance.c,v 5.5.2.3 2008/03/04 06:43:27 vicadmin Exp $";

/*****************************************************************************
  Function name: vic411_water_energy_balance()
  ...
  2007-Apr-03 Modified to handle grid cell errors by returning to the
	      vic411_main subroutine, rather than ending the simulation.		KAC via GCT
...
*****************************************************************************/
int vic411_water_energy_balance(int     numnod,
			 double *surface,
			 double *evapw,
			 int     dt,
			 int     freezeflag,
	      ...
	      
{
  ...
  vic411_energycalc(T, &jouleold, numnod,dz, surfdz, surface, cp, water_density);
  ...
  while((fabs(Tmean - Ts) > epsilon) && iterations < MAX_ITER) {
    ...
    /* ....................................................................
     * Send an ice height of 0 meters to vic411_latsens for the calculation
     * of latent and sensible heat over liquid water.
     * ....................................................................*/
 
    vic411_latsens (Tskin, Tcutk, 0.0, Tair, wind, pressure, vp, air_density, 
	     evapw, Qh, wind_h);
    ...
    **********************************************************************/
    ...
    Le = (2.501 - 0.002361 * Tair) * 1.0e6;   /*J kg-1  */
    ...
    *************************************************************/
    ...
    vic411_eddy(1, wind, T, water_density, de, lat, numnod, dz, surfdz);
    ...
    vic411_temp_area(shortwave*a1, shortwave*a2, *Qle+*Qh+*LWnet, T, Tnew,
    ...
    if(Tnew[0] <  Tcutoff) {
      ...
      vic411_iceform (energy_ice_formation, Tnew, Tcutoff, fracprv, new_ice_area, 
	       numnod, dt, dz, surfdz, cp, surface, new_ice_height, water_density,
	       new_ice_water_eq, lvolume);

      vic411_energycalc(Tnew, &sumjouli, numnod, dz, surfdz, surface, cp, water_density);    
  ...
  if(fabs(Tmean - Ts) <= epsilon) {
    ...
    *LWnet = longwave -EMH2O*STEFAN_B*Tskin*Tskin*Tskin*Tskin;
    ...
    if(T[0] <  Tcutoff) {
      vic411_iceform (energy_ice_formation, T, Tcutoff, fracprv, new_ice_area,
               numnod, dt, dz, surfdz, cp, surface, new_ice_height, water_density,
               new_ice_water_eq, lvolume);
    }
    else {
