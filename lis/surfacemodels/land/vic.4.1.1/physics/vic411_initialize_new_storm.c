...
static char vcid[] = "$Id: vic411_initialize_new_storm.c,v 5.1.2.1 2007/04/18 20:18:59 vicadmin Exp $";

int  vic411_initialize_new_storm(vic411_cell_data_struct ***cell,
			  ...
			  double              new_mu) {
/**********************************************************************
  vic411_initialize_new_storm.c	Keith Cherkauer		June 24, 1998
  ...
  2007-Apr-04 Modified to return to vic411_main subroutine on cell error GCT/KAC
**********************************************************************/
  ...
  for(layer = 0; layer < vic411_options.Nlayer; layer++) {
    ...
    for(band = 0; band < vic411_options.SNOW_BAND; band++) {
      ...
      error = vic411_average_moisture_for_storm(&temp_wet, &temp_dry, old_mu, new_mu);
      if(error) {
...
#endif
	error = vic411_average_moisture_for_storm(&temp_wet, &temp_dry, old_mu, 
					   new_mu);
	if(error) {
#if SPATIAL_FROST
...
unsigned char vic411_average_moisture_for_storm(double *wet_value,
					 double *dry_value,
					 double  old_mu,
					 double  new_mu) {
/**********************************************************************
