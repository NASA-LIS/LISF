...
static char vcid[] = "$Id: vic411_read_atmos_data.c,v 5.5.2.1 2007/02/06 01:33:14 vicadmin Exp $";

void vic411_read_atmos_data(FILE                 *infile,
		     vic411_global_param_struct   vic411_global_param,
		     int                   file_num,
		     int                   forceskip,
		     double              **forcing_data)
/**********************************************************************
  vic411_read_atmos_data
  ...
  Supported Input Field Combinations, vic411_options in parenthesis optional:
  ...
  * get_global flags are implemented
  ...
  01/10/00 Modified to read a generic Binary, or ASCII column 
           ...
           data arrays.                                    KAC
  ??-???-?? Replaced vic411_NF with vic411_global_param.dt in condition checking
  ...
  **********************************************************************/
{
  ...
  Nfields     = vic411_param_set.N_TYPES[file_num];
  field_index = vic411_param_set.FORCE_INDEX[file_num];
  ...
  skip_recs = (int)((float)(vic411_global_param.dt * forceskip)) 
    / (float)vic411_param_set.FORCE_DT[file_num];
  if((((vic411_global_param.dt < 24 && (vic411_param_set.FORCE_DT[file_num] * forceskip) 
	% vic411_global_param.dt) > 0)) 
     || (vic411_global_param.dt == 24 && (vic411_global_param.dt 
				   % vic411_param_set.FORCE_DT[file_num] > 0)))
    vic411_nrerror("Currently unable to handle a model starting date that does not correspond to a line in the forcing file.");

  /** vic411_Error checking - Model can be run at any time step using daily forcing
  ...
  if(vic411_param_set.FORCE_DT[file_num] < 24 
     && vic411_global_param.dt != vic411_param_set.FORCE_DT[file_num]) {
    sprintf(ErrStr,"When forcing the model with sub-daily data, the model must be run at the same time step as the forcing data.  Currently the model time step is %i hours, while forcing file %i has a time step of %i hours.",vic411_global_param.dt,file_num,vic411_param_set.FORCE_DT[file_num]);
  ...
  ***************************/
  ...
  if(vic411_param_set.FORCE_FORMAT[file_num] == BINARY){
    ...
    if (feof(infile))
      vic411_nrerror("No data in the forcing file.  Model stopping...");
    for (i=0; i<4; i++) {
      fread(&ustmp,sizeof(unsigned short),1,infile);
      if (endian != vic411_param_set.FORCE_ENDIAN[file_num]) {
    ...
    if (feof(infile))
      vic411_nrerror("No data for the specified time period in the forcing file.  Model stopping...");
	  
    /** Read BINARY forcing data **/
    rec = 0;
	  
    while ( !feof(infile) && (rec * vic411_param_set.FORCE_DT[file_num] 
			      < vic411_global_param.nrecs * vic411_global_param.dt) ) {

      for(i=0;i<Nfields;i++) {
	if(vic411_param_set.TYPE[field_index[i]].SIGNED) {
	  fread(&stmp,sizeof(short int),1,infile);
	  if (endian != vic411_param_set.FORCE_ENDIAN[file_num]) {
	    stmp = ((stmp & 0xFF) << 8) | ((stmp >> 8) & 0xFF);
	  }
	  forcing_data[field_index[i]][rec] 
	    = (double)stmp / vic411_param_set.TYPE[field_index[i]].multiplier;
	}
	else {
	  ...
	  forcing_data[field_index[i]][rec] 
	    = (double)ustmp / vic411_param_set.TYPE[field_index[i]].multiplier;
	}
      }
  ...
  **************************/
    ...
    // in vic411_open_file().  However, if we wanted to read information from the header,
    // we'd want to do it here, after rewinding to the beginning of the file (or
    // moving the code that deals with headers from vic411_open_file() to this function
  ...
  if(rec * vic411_param_set.FORCE_DT[file_num] 
     < vic411_global_param.nrecs * vic411_global_param.dt ) {
    sprintf(ErrStr,"Not enough records in the forcing file (%i * %i = %i) to run the number of records defined in the global file (%i * %i = %i).  Check forcing file time step, and global file", rec, vic411_param_set.FORCE_DT[file_num], 
	    rec*vic411_param_set.FORCE_DT[file_num], vic411_global_param.nrecs, 
	    vic411_global_param.dt, vic411_global_param.nrecs*vic411_global_param.dt);
    vic411_nrerror(ErrStr);
  }
