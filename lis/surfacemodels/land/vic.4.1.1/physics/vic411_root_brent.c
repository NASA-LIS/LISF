...
static char vcid[] = "$Id: vic411_root_brent.c,v 5.3.2.3 2009/05/24 07:55:25 vicadmin Exp $";
...
/*****************************************************************************
  ...
  Comments     :
    04-Jun-04 Removed message announcing the dumping of variables,
	      since that doesn't always happen when vic411_root_brent fails.
  ...
  2007-Aug-31 Corrected handling of Function return value if Function returns
	      ERROR.  This can happen if Function is vic411_func_surf_energy_bal.	JCA
...
*****************************************************************************/
double vic411_root_brent(double LowerBound, double UpperBound, char *ErrorString,
                double (*Function)(double Estimate, va_list ap), ...)
{
  ...
  for (i = 0; i < MAXITER; i++) {
	...
	if ((2 * p) < ( 3 * m * q - fabs(tol * q)) && p < fabs(0.5 * s * q))
      ...
      if(fb == ERROR){
	sprintf(ErrorString,"ERROR returned to vic411_root_brent on iteration %d: temperature = %.4f\n",i+1,b);
	va_end(ap);
	return( ERROR );
      }      
