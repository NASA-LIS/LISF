...
#if LINK_DEBUG
void write_debug(vic411_atmos_data_struct    *atmos,
...
/**********************************************************************
  ...
  from vic411_full_energy.c are removed.  Due to the number of static variables
...
**********************************************************************/
  ...
  if(debug.PRT_FLUX && vic411_options.FULL_ENERGY) {
	...
	+= ((1.-energy[band].AlbedoOver)*energy[band].shortwave
      ...
      INSHORT[veg]        += (1.-energy[band].AlbedoOver)*atmos->shortwave[vic411_NR] 
	* mu / soil_con->AreaFract[band];
      INLONG[veg]         += atmos->longwave[vic411_NR] * mu 
    ...
    if((vic411_options.DIST_PRCP && dist==1) || !vic411_options.DIST_PRCP) {
  ...
  if(debug.PRT_SNOW) {
	...
	fprintf(debug.fg_snow,"DEPTH\tKAPPA\tCANOPY\tCNPYFLUX\n");
      ...
      fprintf(debug.fg_snow,"\t%7.3f\t%7.4f",
	      atmos->air_temp[vic411_NR], grnd_flux);
  ...
  if(debug.PRT_BALANCE) {
    ...
    /***** Compute Water Balance vic411_Error *****/
    ...
    if(NEWCELL && dist==0) {
      ...
      for(i=0;i<=Nveg;i++)
        MOIST_ERROR[i] = (double *)calloc(vic411_options.Nlayer+3,sizeof(double));
    }
    if(rec==0 && dist==0) {
      for(band = 0; band < Nbands; band++) {
	INIT_MOIST[veg] = debug.store_moist[WET][band][vic411_options.Nlayer+2] * mu;
	INIT_MOIST[veg] += debug.store_moist[DRY][band][vic411_options.Nlayer+2] 
	  * (1. - mu);
      }
      for(i=0;i<vic411_options.Nlayer+3;i++) MOIST_ERROR[veg][i] = 0.;
    }
    if(dist==0) {
      INFLOW[veg] = 0.;
      RUNOFF[veg] = 0.;
    ...
    Evap       = (double *)calloc(vic411_options.Nlayer+3,sizeof(double));
    curr_moist = (double *)calloc(vic411_options.Nlayer+3,sizeof(double));
    for(band = 0; band < Nbands; band++) {
      if(soil_con->AreaFract[band]>0) {
	Evap[vic411_options.Nlayer+2] = 0.;
	curr_moist[vic411_options.Nlayer+2] = 0.;
	if(veg < Nveg) {
	  /** Vegetation Present **/
	  ...
	  Evap[vic411_options.Nlayer+2]       += Evap[0];
	  curr_moist[vic411_options.Nlayer+2] += curr_moist[0];
	}
	else {
	  /** No vegetation **/
	  Evap[0]       = 0.;
	...
	Evap[vic411_options.Nlayer+2]       += Evap[1];
	curr_moist[1]                 = snow[band].swq * 1000.;
	curr_moist[vic411_options.Nlayer+2] += curr_moist[1];
	
	for(i = 0; i < vic411_options.Nlayer; i++) {
	  /** All Soil Layers **/
	  Evap[i+2]                     = cell[band].layer[i].evap;
	  Evap[vic411_options.Nlayer+2]       += Evap[i+2];
	  curr_moist[i+2]               = cell[band].layer[i].moist;
	  curr_moist[vic411_options.Nlayer+2] += curr_moist[i+2];
	}
	
	/** Compute Moisture Balance vic411_Error **/
	for(i = 0; i < vic411_options.Nlayer+3; i++) {
	  ...
	  if(fabs(MOIST_ERROR[veg][i]) > 1.e-4) {
	    fprintf(stderr,"WARNING: Debug Layer %i has a Moisture Balance vic411_Error of %f in rec %i, veg %i, band %i, precip dist %i\n",i,MOIST_ERROR[veg][i],rec,veg,band,dist);
	  }
	}

	/** Store Variables **/
	INFLOW[veg]   += atmos->prec[vic411_NR] * soil_con->Pfactor[band] 
	  / soil_con->AreaFract[band];
	BASEFLOW[veg] += cell[band].baseflow * mu / soil_con->AreaFract[band];
	RUNOFF[veg]   += cell[band].vic411_runoff * mu / soil_con->AreaFract[band];
	EVAP[veg]     += Evap[vic411_options.Nlayer+2] * mu / soil_con->AreaFract[band];
      }
    }
    ...
    if((vic411_options.DIST_PRCP && dist==1) || !vic411_options.DIST_PRCP) {
      ...
      fprintf(debug.fg_balance,"\t%f\t%f\t%f\n",
          EVAP[veg],curr_moist[vic411_options.Nlayer+2]-INIT_MOIST[veg],
          MOIST_ERROR[veg][vic411_options.Nlayer+2]);
    }
    free((char*)Evap);
  ...
  if(debug.PRT_TEMP) {
	...
	fprintf(debug.fg_temp,"\t%6.2f\t%6.2f\t%6.2f",
		atmos->air_temp[vic411_NR], energy->fdepth[0] * 100., 
		energy->tdepth[0] * 100.);
  ...
  if(debug.PRT_MOIST) {
	...
	fprintf(debug.fg_moist,"\t%6.2f\t%6.4f\t%6.4f",
		atmos->air_temp[vic411_NR], cell[band].inflow, cell[band].vic411_runoff);
