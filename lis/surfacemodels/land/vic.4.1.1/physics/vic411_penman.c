/********************************************************************************
  ...
  2009-Jun-09 Moved calculation of canopy resistance from vic411_penman() into
	      separate function vic411_calc_rc().  Also allowed for reference
	      crop case in vic411_calc_rc().					TJB
...
static char vcid[] = "$Id: vic411_penman.c,v 5.3.2.1 2009/06/09 09:54:11 vicadmin Exp $";

#define CLOSURE 4000		/** Pa **/
#define RSMAX 5000
#define VPDMINFACTOR 0.1

double vic411_calc_rc(double rs, 
	       double net_short, 
	       float  RGL, 
	       double tair, 
	       double vpd, 
...
double vic411_penman(double tair,
	      double elevation,
	      double rad,
	      double vpd,
	      double ra,
	      ...
	      double rarc)
{
  ...
  double lv;                    /* latent heat of vaporization (J kg-1) */
