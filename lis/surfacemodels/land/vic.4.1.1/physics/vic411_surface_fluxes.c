...
static char vcid[] = "$Id: vic411_surface_fluxes.c,v 5.12.2.17 2009/09/28 21:39:21 vicadmin Exp $";
...
int vic411_surface_fluxes(char                 overstory,
		   double               BareAlbedo,
		   double               height,
		   double               ice0,
		   double               moist0,
		   ...
		   vic411_cell_data_struct    *cell_dry,
		   ...
		   float              fetch)
/**********************************************************************
	vic411_surface_fluxes	Keith Cherkauer		February 29, 2000

  Formerly a part of vic411_full_energy.c this routine computes all surface
  ...
  are defined in vic411_full_energy before the routine is called).
  ...
  04-23-03 Indexing fix sent SNOW_STEP to vic411_calc_surf_energy_bal rather
  ...
  16-Jul-04 Moved calculation of blowing_flux from this function
	    into vic411_latent_heat_from_snow().				TJB
  05-Aug-04 Moved calculation of blowing_flux back into this function
	    from vic411_latent_heat_from_snow().  Updated arg lists to
	    vic411_calc_surf_energy_bal() and vic411_solve_snow() accordingly.	TJB
  ...
  2006-Sep-26 Moved tracking of out_rain and out_snow to vic411_solve_snow.c.	TJB
  ...
  24-Apr-07 Features included for IMPLICIT frozen soils option.		JCA
	    (passing nrecs to  vic411_calc_surf_energy_bal)
  ...
  2008-Oct-23 In call to CalcBlowing(), replaced
	        vic411_veg_lib[iveg].displacement[dmy[rec].month-1] and
	        vic411_veg_lib[iveg].roughness[dmy[rec].month-1] with
  ...
  2009-Jun-09 Added call to vic411_compute_pot_evap() to compute potential
...
**********************************************************************/
{
  ...
  vic411_energy_bal_struct      snow_energy; // energy fluxes at snowpack surface
  vic411_energy_bal_struct      soil_energy; // energy fluxes at soil surface
  vic411_veg_var_struct         snow_veg_var[2]; // veg fluxes/storages in presence of snow
  vic411_veg_var_struct         soil_veg_var[2]; // veg fluxes/storages in soil energy balance
  vic411_snow_data_struct       step_snow;
  vic411_layer_data_struct      step_layer[2][MAX_LAYERS];

  // Structures holding values for current iteration
  vic411_energy_bal_struct      iter_snow_energy; // energy fluxes at snowpack surface
  vic411_energy_bal_struct      iter_soil_energy; // energy fluxes at soil surface
  vic411_veg_var_struct         iter_snow_veg_var[2]; // veg fluxes/storages in presence of snow
  vic411_veg_var_struct         iter_soil_veg_var[2]; // veg fluxes/storages in soil energy balance
  vic411_snow_data_struct       iter_snow;
  vic411_layer_data_struct      iter_layer[2][MAX_LAYERS];
  ...
  ***********************************************************************/
  ...
  runoff_dry = &(cell_dry->vic411_runoff);
  runoff_wet = &(cell_wet->vic411_runoff);
  ...
  ********************************/
  ...
  if(snow->swq > 0 || snow->snow_canopy > 0 || atmos->snowflag[vic411_NR]) {
    hidx      = 0;
    step_inc  = 1;
    endhidx   = hidx + vic411_NF;
    step_dt   = vic411_options.SNOW_STEP;
  }
  else {
    hidx      = vic411_NR;
    step_inc  = 1;
  ...
  *************************/
  ...
  do {
    ...
    if ( vic411_options.GRND_FLUX ) Tgrnd = energy->T[0];
    else Tgrnd = Tair;
    ...
    if( !overstory && vic411_options.BLOWING && step_snow.swq > 0.) {
      Ls = (677. - 0.07 * step_snow.surf_temp) * JOULESPCAL * GRAMSPKG;
      step_snow.blowing_flux = vic411_CalcBlowingSnow((double) step_dt, Tair,
	...
	step_melt = vic411_solve_snow(overstory, BareAlbedo, LongUnderOut, 
			       ...
			       wind, root, UNSTABLE_SNOW, vic411_options.Nnode, 
...
// iter_snow_energy.sensible + iter_snow_energy.latent + iter_snow_energy.latent_sub + NetShortSnow + NetLongSnow + ( snow_grnd_flux + iter_snow_energy.advection - iter_snow_energy.deltaCC + iter_snow_energy.refreeze_energy + iter_snow_energy.advected_sensible ) * step_snow.coverage
	...
	Tsurf = vic411_calc_surf_energy_bal((*Le), LongUnderIn, NetLongSnow, 
				     ...
				     UnderStory, vic411_options.Nnode, Nveg, band, 
				     step_dt, hidx, iveg, vic411_options.Nlayer, 
        ...
        if ( (int)Tsurf == ERROR ) {
          // Return error vic411_flag to skip rest of grid cell
          return ( ERROR );
        }
	...
	if ( iter_snow.snow && overstory && MAX_ITER > 0 ) {
	  ...
	  Tcanopy = vic411_calc_atmos_energy_bal(iter_snow_energy.canopy_sensible,
    ...
    **************************************/
    ...
    vic411_compute_pot_evap(veg_class, dmy, rec, gp->dt, atmos->shortwave[hidx], iter_soil_energy.NetLongAtmos, Tair, VPDcanopy, soil_con->elevation, step_aero_resist, iter_pot_evap);
...
// energy->AtmosSensible + energy->AtmosLatent + energy->AtmosLatentSub + energy->NetShortAtmos + energy->NetLongAtmos + energy->grnd_flux + energy->deltaH + energy->fusion + energy->advection - energy->deltaCC + energy->refreeze_energy + energy->advected_sensible
  ...
  ********************************************************/
  ...
  if(SubsidenceUpdate != 2){
    ...
    ErrorFlag = vic411_runoff(cell_wet, cell_dry, energy, soil_con, ppt, 
#if EXCESS_ICE
		       SubsidenceUpdate,
#endif
#if SPATIAL_FROST
...
#endif // SPATIAL_FROST
		       mu, gp->dt, vic411_options.Nnode, band, rec, iveg);
