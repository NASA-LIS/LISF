...
static char vcid[] = "$Id: vic411_redistribute_during_storm.c,v 5.1.2.1 2007/04/18 20:18:59 vicadmin Exp $";

int  vic411_redistribute_during_storm(vic411_cell_data_struct ***cell,
			       ...
			       double             *max_moist) {
/**********************************************************************
  vic411_redistribute_during_storm.c     Keith Cherkauer     January 13, 1998
...
**********************************************************************/
  ...
  for(layer = 0; layer < vic411_options.Nlayer; layer++) {
    ...
    for(band = 0; band < vic411_options.SNOW_BAND; band++) {
      ...
      error = vic411_redistribute_moisture_for_storm(&temp_wet, &temp_dry, 
					      max_moist[layer], old_mu, 
					      new_mu);
      if(error) {
	fprintf(stderr,"%s: vic411_Error in moist accounting %f -> %f record %i\n",
	...
	if(error) {
#if SPATIAL_FROST
	  fprintf(stderr,"%s: vic411_Error in ice accounting %f -> %f record %i\n",
  ...
  ****************************************/
  if(veg < Nveg) {
    for(band = 0; band < vic411_options.SNOW_BAND; band++) {
      ...
      error = vic411_redistribute_moisture_for_storm(&temp_wet, &temp_dry, Wdmax, 
					      old_mu, new_mu);
      if(error) {
	fprintf(stderr,"%s: vic411_Error in Wdew accounting %f -> %f record %i\n",
...
unsigned char vic411_redistribute_moisture_for_storm(double *wet_value,
					      double *dry_value,
					      double  max_moist,
					      double  old_mu,
					      double  new_mu) {
