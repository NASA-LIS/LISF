...
static char vcid[] = "$Id: vic411_dist_prec.c,v 5.10.2.5 2009/09/28 21:46:57 vicadmin Exp $";

int  vic411_dist_prec(vic411_atmos_data_struct   *atmos,
	       ...
	       int                 *init_DRY_TIME) {
/**********************************************************************
  vic411_dist_prec		Keith Cherkauer		October 9, 1997
  ...
  2005-Mar-24 Modified parameter list of vic411_put_data() to accomodate support
  ...
  2008-Oct-23 Modified call to vic411_put_data() to store ErrorFlag.		TJB
  2009-Mar-03 Modified routine to store vic411_put_data() error in ErrorFlag2 and 
  ...
  2009-Sep-28 Added logic for initial (pre-simulation) call to vic411_put_data.TJB
...
**********************************************************************/
  ...
  **************************************************/
  if (rec < 0) {
    ErrorFlag2 = vic411_put_data(prcp, atmos, soil_con, veg_con,
  ...
  **************************************************/
  ...
  if(vic411_options.DIST_PRCP) {
    ...
    *******************************************/
    ...
    NEW_MU = 1.0 - exp(-vic411_options.PREC_EXPT*atmos->prec[vic411_NR]);
    for ( veg = 0; veg <= veg_con[0].vegetat_type_num; veg++ ) {
      ANY_SNOW[veg] = FALSE;
      for ( i = 0; i < vic411_options.SNOW_BAND; i++ )
        /* Check for snow on ground or falling */
	if ( prcp->snow[veg][i].swq > 0 
	     || prcp->snow[veg][i].snow_canopy > 0. ) 
	  ANY_SNOW[veg] = TRUE;
      if ( ANY_SNOW[veg] || atmos->snowflag[vic411_NR] ) {
        /* If snow present, mu must be set to 1. */
	NEW_MU = 1.;
	if ( rec == 0 ) {
          /* Set model variables if first time step */
	  prcp->mu[veg] = NEW_MU;
	  if ( atmos->prec[vic411_NR] > 0 ) 
	...
	else if(atmos->prec[vic411_NR] == 0 && DRY_TIME[veg] >= 24.) {
          /* Check if storm has ended */
	  NEW_MU           = prcp->mu[veg];
	  STILL_STORM[veg] = FALSE;
          DRY_TIME[veg]    = 0;
	}
        else if ( atmos->prec[vic411_NR] == 0 ) {
	  /* May be pause in storm, keep track of pause length */
	  NEW_MU         = prcp->mu[veg];
	  DRY_TIME[veg] += vic411_global_param->dt;
	}
      }

      if ( !STILL_STORM[veg] && (atmos->prec[vic411_NR] > STORM_THRES 
				 || ANY_SNOW[veg] ) ) {
	/** Average soil moisture before a new storm **/
	ErrorFlag = vic411_initialize_new_storm(prcp->cell,prcp->veg_var,
	...
	if (veg < veg_con[0].vegetat_type_num) 
	  Wdmax = vic411_veg_lib[veg_con[veg].veg_class].Wdmax[month];
	else 
	  Wdmax = 0;
	vic411_redistribute_during_storm(prcp->cell, prcp->veg_var, veg, 
    ...
    ErrorFlag = vic411_full_energy(NEWCELL, cellnum, rec, atmos, prcp, dmy, vic411_global_param, 
  ...
  ************************************/
  ...
  if ( filep->statefile != NULL
       &&  ( dmy[rec].year == vic411_global_param->stateyear
	     && dmy[rec].month == vic411_global_param->statemonth 
	     && dmy[rec].day == vic411_global_param->stateday
	     && ( rec+1 == vic411_global_param->nrecs
		  || dmy[rec+1].day != vic411_global_param->stateday ) ) )
