...
static char vcid[] = "$Id: vic411_func_canopy_energy_bal.c,v 5.3.2.3 2009/09/14 23:58:39 vicadmin Exp $";

double vic411_func_canopy_energy_bal(double Tfoliage, va_list ap)
/*********************************************************************
  vic411_func_canopy_energy_bal    Keith Cherkauer         January 27, 2001
 ...
 ********************************************************************/
{
  ...
  *NetLongOver  = LongOverIn - (*LongOverOut);
  ...
  if ( IntSnow > 0 ) {
    ...
    if (vic411_options.AERO_RESIST_CANSNOW == AR_COMBO || vic411_options.AERO_RESIST_CANSNOW == AR_406 || vic411_options.AERO_RESIST_CANSNOW == AR_406_LS || vic411_options.AERO_RESIST_CANSNOW == AR_406_FULL)
      Ra_used[1] *= 10.;

    /** Calculate the vapor mass flux between intercepted snow in 
	the canopy and the surrounding air mass **/
  
    EsSnow = vic411_svp(Tfoliage); 
    
    /* Apply stability correction to aerodynamic resistance */
    if (vic411_options.AERO_RESIST_CANSNOW == AR_COMBO || vic411_options.AERO_RESIST_CANSNOW == AR_410) {
      if (Wind[1] > 0.0) {
        Ra_used[1] /= vic411_StabilityCorrection(ref_height[1], displacement[1], Tfoliage, 
    ...
    *Evap = 0;
    ...
    if (vic411_options.AERO_RESIST_CANSNOW == AR_406_FULL || vic411_options.AERO_RESIST_CANSNOW == AR_410 || vic411_options.AERO_RESIST_CANSNOW == AR_COMBO) {
      Ra_used[0] = Ra[0];
      Ra_used[1] = Ra[1];
    }
    else {
    ...
    prec[DRY] = 0;
    *Evap = vic411_canopy_evap(layer_wet, layer_dry, veg_var_wet, veg_var_dry, FALSE, 
