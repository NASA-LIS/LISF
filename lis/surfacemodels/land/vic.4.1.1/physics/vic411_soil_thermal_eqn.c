...
static char vcid[] = "$Id: vic411_soil_thermal_eqn.c,v 4.1.4.5 2007/10/08 20:25:18 vicadmin Exp $";

double vic411_soil_thermal_eqn(double T, va_list ap) {
 ...
 /******************************************************************
  ...
  Apr 24, 2007: Added patch for the "cold nose" problem using the EXPLICIT  
                option. (see comments on this in vic411_fda_heat_eqn in frozen_soil.c)	JCA
...
#else
#if EXCESS_ICE
    ice = moist - vic411_maximum_unfrozen_water(T,porosity,effective_porosity,max_moist,bubble,expt);
#else
    ice = moist - vic411_maximum_unfrozen_water(T,max_moist,bubble,expt);
#endif
#endif
    if(ice<0.) ice=0.;
