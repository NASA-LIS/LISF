...
static char vcid[] = "$Id: vic411_prepare_full_energy.c,v 5.2.2.3 2008/05/05 20:31:15 vicadmin Exp $";

void vic411_prepare_full_energy(int               iveg,
			 int               Nveg,
			 int               Nnodes,
			 ...
			 double           *ice0) {
/*******************************************************************
  vic411_prepare_full_energy.c      Keith Cherkauer       January 20, 2000
...
*******************************************************************/
  ...
  layer = (vic411_layer_data_struct *)calloc(vic411_options.Nlayer,
  ...
  for(band=0;band<vic411_options.SNOW_BAND;band++) {
    ...
    if (soil_con->AreaFract[band] > 0.0) {
      ...
      for(i=0;i<vic411_options.Nlayer;i++) 
        layer[i] = vic411_find_average_layer(&(prcp->cell[WET][iveg][band].layer[i]),
      ...
      if(vic411_options.FROZEN_SOIL && soil_con->FS_ACTIVE){
        if((prcp->energy[iveg][band].T[0] 
	    + prcp->energy[iveg][band].T[1])/2.<0.) {
	  ice0[band] = moist0[band] 
	    - vic411_maximum_unfrozen_water((prcp->energy[iveg][band].T[0]
      ...
      vic411_compute_soil_layer_thermal_properties(layer,soil_con->depth,
