/*
 * SUMMARY:      vic411_SnowPackEnergyBalance.c - Calculate snow pack energy balance
 * USAGE:        Part of DHSVM
 *
 * AUTHOR:       Bart Nijssen
 ...
 * DESCRIP-END.
 * FUNCTIONS:    vic411_SnowPackEnergyBalance()
 * COMMENTS:     
 */
...
static char vcid[] = "$Id: vic411_SnowPackEnergyBalance.c,v 5.7.2.2 2009/09/20 02:32:07 vicadmin Exp $";

/*****************************************************************************
  Function name: vic411_SnowPackEnergyBalance()
  ...
  05-Aug-04 Removed lag_one, sigma_slope, fetch, iveg, Nveg, month, overstory,
	    ...
	    needed to pass to vic411_latent_heat_from_snow(), which no longer needs
	    them.							TJB
...
*****************************************************************************/
double vic411_SnowPackEnergyBalance(double TSurf, va_list ap)
{

  extern vic411_option_struct vic411_options;

  const char *Routine = "vic411_SnowPackEnergyBalance";
  ...
  double VaporMassFlux;           /* Mass flux of water vapor to or from the
				     intercepted snow (kg m-2 s-1) */
  double BlowingMassFlux;         /* Mass flux of water vapor from blowing snow. (kg m-2 s-1) */
  double SurfaceMassFlux;         /* Mass flux of water vapor from pack snow. (kg m-2 s-1) */
  ...
  if (Wind > 0.0) 
    Ra_used[0] = Ra / vic411_StabilityCorrection(Z, 0.f, TMean, Tair, Wind, Z0[2]); 
  else
  ...
  *SensibleHeat = AirDens * Cp * (Tair - TMean) / Ra_used[0];
  ...
  if ( SnowCoverFract > 0 ) {
    *(AdvectedSensibleHeat) = vic411_advected_sensible_heat(SnowCoverFract, 
						     AirDens, Tair, TGrnd, 
						     Ra_used[0]);
  }
  ...
  vic411_latent_heat_from_snow(AirDens, EactAir, Lv, Press, Ra_used[0], TMean, Vpd,
