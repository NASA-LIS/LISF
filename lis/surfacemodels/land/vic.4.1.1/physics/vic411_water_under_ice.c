/*
 * SUMMARY:      vic411_water_under_ice.c 
 * USAGE:        
 *
...
//static char vcid[] = "$Id: vic411_water_under_ice.c,v 5.5.2.3 2008/06/16 22:03:39 vicadmin Exp $";

/*****************************************************************************
  Function name: vic411_water_under_ice()
...
*****************************************************************************/
int vic411_water_under_ice(int     freezeflag, 
		    double  sw_ice,
		    double  wind,
		    double *Ti,
	      ...
	      
{
  ...
  // compute the vic411_eddy diffusivity 
  vic411_eddy(freezeflag, wind, Ti, water_density, de, lat, numnod, dz, surfdz);
  ...
  *qw = qw_init;
  ...
  vic411_energycalc(Ti, &jouleold, numnod,dz, surfdz, surface, water_cp, water_density);
  ...
  while((fabs(qw_mean - *qw) > epsilon) && iterations < MAX_ITER) {
    ...
    vic411_temp_area (sw_underice_visible, sw_underice_nir, -1.*(*qw) , Ti, Tnew,
    ...
    *deltaH = (joulenew - jouleold)/(surface[0]*dt*SECPHOUR);
    ...
    vic411_tracer_mixer (Tnew, &mixdepth, freezeflag,
...
//    fprintf(stderr, "Lake temps under ice failed to converge; temporary work around used.\n");
    *qw = 0.0;
    for(k=0; k<numnod; k++)
      Ti[k] = Tcutoff;
    vic411_energycalc(Ti, &joulenew, numnod,dz, surfdz, surface, water_cp, water_density);
    *deltaH = (joulenew - jouleold)/(surface[0]*dt*SECPHOUR);
    return(0);
    // return (ERROR);
  }
