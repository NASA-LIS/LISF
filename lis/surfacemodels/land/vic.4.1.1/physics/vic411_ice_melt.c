...
static char vcid[] = "$Id: vic411_ice_melt.c,v 5.9.2.7 2009/10/08 21:30:59 vicadmin Exp $";
...
/*****************************************************************************
  ...
  Required     :
    ...
    double Le           - Latent heat of vaporization (J kg-13)
    double atmos->net_short      - Net exchange of shortwave radiation (W m-2)
    double atmos->longwave       - Incoming long wave radiation (W m-2)
  ...
  04-Jun-04 Added descriptive error message to beginning of screen dump in
	    vic411_ErrorPrintIcePackEnergyBalance.					TJB
  16-Jul-04 Changed VaporMassFlux to vapor_flux, to make it consistent with
	    vic411_IceEnergyBalance(), in which VaporMassFlux is in (kg m-2 s-1) and
  ...
  25-Aug-04 Added calculations for surface_flux and blowing_flux.  Note that
	    ...
	    future with a call to vic411_CalcBlowingSnow().				TJB
  27-Aug-04 Replaced *DeltaColdContent with DeltaColdContent in parameter
	    list for vic411_ErrorIcePackEnergyBalance() and
	    vic411_ErrorPrintIcePackEnergyBalance() so that types match in the caller
	    and callee.								TJB
  ...
  2007-Apr-03 Return ERROR value on error from vic411_CalcBlowingSnow and vic411_root_brent.	GCT/KAC. 
  2007-Aug-31 Checked vic411_root_brent return value against -998 rather than -9998.	JCA
  2007-Sep-25 Return ERROR instead of exiting, if vic411_ice_melt could not converge to
	      a solution in vic411_root_brent.						JCA
  2007-Nov-06 New parameters for vic411_CalcBlowingSnow().  Replaced lake.fraci,
	      ...
	      double            fracprv)
{
  ...
  double Qnet;                   /* Net energy exchange at the surface (W m-2) */
  double PackRefreezeEnergy;     /* refreeze/melt energy in pack layer (W m-2) */
  double RefreezeEnergy;         /* refreeze energy (W m-2) */
  ...
  vic411_icerad (net_short, lake->hice, SnowIce*RHO_W/RHOSNOW, &avgcond, &SWconducted, &deltaCC);
  ...
  if(vic411_options.BLOWING && snow->swq > 0.) {
    Ls = (677. - 0.07 * snow->surf_temp) * JOULESPCAL * GRAMSPKG;
    snow->blowing_flux = vic411_CalcBlowingSnow((double) delta_t, air_temp,
    ...
    if ( (int)snow->blowing_flux == ERROR ) {
      fprintf( stderr, "ERROR: vic411_ice_melt.c has an error from the call to vic411_CalcBlowingSnow\n");
  ...
  Qnet = vic411_CalcIcePackEnergyBalance((double)0.0, (double)delta_t, aero_resist,
  ...
  /* Else, vic411_IceEnergyBalance(T=0.0) <= 0.0 */
  else  {
    /* Calculate surface layer temperature using "Brent method" */
    if (SurfaceSwq > MIN_SWQ_EB_THRES) {
      snow->surf_temp = vic411_root_brent((double)(snow->surf_temp-SNOW_DT), 
				   (double)(snow->surf_temp+SNOW_DT), ErrorString,
				   vic411_IceEnergyBalance, (double)delta_t, 
      ...
      if (snow->surf_temp <= -998) {
        if (vic411_options.TFALLBACK) {
          ...
          vic411_ErrorIcePackEnergyBalance(snow->surf_temp, (double)delta_t, aero_resist,
				    aero_resist_used, z2, displacement, Z0, wind, net_short,
				    longwave, density, Le, air_temp,
				    pressure * 1000., vpd * 1000., vp * 1000.,
				    RainFall, SurfaceSwq, 
...
//        fprintf(stderr,"Snow/Ice layer is too thin to solve separately \n");
    ...
    if (snow->surf_temp > -998 && snow->surf_temp < 999) {
      Qnet = vic411_CalcIcePackEnergyBalance(snow->surf_temp, (double)delta_t, aero_resist,
				      aero_resist_used, z2, displacement, Z0, wind, net_short,
				      longwave, density, Le, air_temp,
...
/*****************************************************************************
  Function name: vic411_CalcIcePackEnergyBalance()

  Purpose      : Dummy function to make a direct call to
                 vic411_IceEnergyBalance() possible.
...
double vic411_CalcIcePackEnergyBalance(double Tsurf, ...)
{
  ...
  va_start(ap, Tsurf);
  Qnet = vic411_IceEnergyBalance(Tsurf, ap);
  va_end(ap);
...
double vic411_ErrorIcePackEnergyBalance(double Tsurf, ...)
{
  ...
  va_start(ap, Tsurf);
  Qnet = vic411_ErrorPrintIcePackEnergyBalance(Tsurf, ap);
  va_end(ap);
  
  return Qnet;
}

double vic411_ErrorPrintIcePackEnergyBalance(double TSurf, va_list ap)
{
  ...
  double Wind;                   /* Wind speed (m s-1) */
  double ShortRad;               /* Net incident shortwave radiation (W m-2) */
  double LongRadIn;              /* Incoming longwave radiation (W m-2) */
  ...
  double *surface_flux;          /* Mass flux of water vapor to or from
                                    snow pack (m/timestep) */
  double *AdvectedEnergy;        /* Energy advected by precipitation (W m-2) */
  double DeltaColdContent;       /* Change in cold content (W m-2) */
  double Tfreeze;
  double AvgCond;
  double SWconducted;
  double SWabsorbed;
  ...
  double *GroundFlux;
  double *LatentHeat;		/* Latent heat exchange at surface (W m-2) */
  double *LatentHeatSub;	/* Latent heat exchange at surface (W m-2) due to sublimation */
  double *SensibleHeat;		/* Sensible heat exchange at surface (W m-2) */
  double *LWnet;
  ...
  fprintf(stderr, "ERROR: vic411_ice_melt failed to converge to a solution in vic411_root_brent.  Variable values will be dumped to the screen, check for invalid values.\n");
