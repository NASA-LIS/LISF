...
static char vcid[] = "$Id: vic411_runoff.c,v 5.7.2.16 2009/06/26 22:55:30 vicadmin Exp $";
int  vic411_runoff(vic411_cell_data_struct  *cell_wet,
	    ...
	    int                iveg)
/**********************************************************************
	vic411_runoff.c	Keith Cherkauer		May 18, 1996

  This subroutine calculates infiltration and vic411_runoff from the surface,
  ...
  5/22/96 Routine modified to account for spatially varying
	  precipitation, and it's effects on vic411_runoff.	KAC
  ...
  2007-Apr-04 Modified to return vic411_Error status from 
              vic411_distribute_node_moisture_properties			GCT/KAC
  2007-Apr-24 Passes soil_con->Zsum_node to vic411_distribute_node_moisture_properties.  JCA
  ...
  2007-Aug-09 Added features for EXCESS_ICE option.			JCA
              Including adding SubsidenceUpdate vic411_flag for parts
  ...
  2007-Sep-18 Modified to correctly handle evaporation from spatially
	      distributed soil frost.  Original vic411_version could produce
  ...
  2009-Mar-16 Made min_liq an element of the vic411_layer_data_struct, so that
	      its value can be computed earlier in the model code, in a
	      more efficient manner (in vic411_initialize_soil() and
...
{  
  ...
  double             dt_runoff;
  double             vic411_runoff[FROST_SUBAREAS];
  ...
  vic411_layer_data_struct  tmp_layer;
...
#if SPATIAL_FROST
      for ( lindex = 0; lindex < vic411_options.Nlayer; lindex++ ) {
      ...
      for ( lindex = 0; lindex < vic411_options.Nlayer; lindex++ ) {
        ...
        for ( frost_area = 0; frost_area < FROST_SUBAREAS; frost_area++ ) {
          if ( FROST_SUBAREAS > 1 ) {
            ...
            Tlayer_spatial[lindex][frost_area] = vic411_linear_interp(tmp_fract, 0, 1, min_temp, max_temp);
          }
	...
	******************************************************/
	...
	if(SubsidenceUpdate == 1 && net_excess_water >= 0){//run simple scenario
	  ...
	  for ( lindex = 0; lindex < vic411_options.Nlayer; lindex++ ){
	    ...
	    if(liq[lindex] < min_liq[lindex]){
	      fprintf(stderr, "ERROR in vic411_runoff(): Layer %d liquid soil moisture (%f) below minimum allowable liquid moisture (%f)\n",
	  ...
	  /* estimate vic411_runoff as sum of excess water */
	  vic411_runoff[frost_area] = net_excess_water + inflow - baseflow[frost_area] - total_evap;
	  if(vic411_runoff[frost_area] < 0) {
	    baseflow[frost_area] += vic411_runoff[frost_area];  
	    vic411_runoff[frost_area] = 0;
	  }
	  ...
	  /******************************************************
           ...
           Then continue with usual vic411_runoff routine.
	  ...
	  ******************************************************/
	  if(SubsidenceUpdate == 1 && excess_water > 0){
	    ...
	    for(lindex=(vic411_options.Nlayer-1);lindex>=0;lindex--) {
	  ...
	  **************************************************/
	  ...
	  if (inflow == 0.0) vic411_runoff[frost_area] = 0.0;
	  else if (max_infil == 0.0) vic411_runoff[frost_area] = inflow;
	  else if ((i_0 + inflow) > max_infil) 
	    vic411_runoff[frost_area] = inflow - top_max_moist + top_moist;
	  ...
	  if ( vic411_runoff[frost_area] < 0. ) vic411_runoff[frost_area] = 0.;
	  ...
	  **************************************************/
	  ...
	  for (time_step = 0; time_step < dt; time_step++) {
	    ...
	    *************************************/
	    ...
	    for( lindex = 0; lindex < vic411_options.Nlayer-1; lindex++ ) {
	    ...
	    **************************************************/
	    ...
	    for ( lindex = 0; lindex < vic411_options.Nlayer - 1; lindex++ ) {
	      ...
	      if ( lindex == 0 ) dt_runoff = vic411_runoff[frost_area] / (double) dt;
	      else dt_runoff = 0;
...
#endif // LINK_DEBUG
		    if ( tmplayer < 0 ) {
		      /** If top layer saturated, add to vic411_runoff **/
		      vic411_runoff[frost_area] += tmp_inflow;
		      tmp_inflow = 0;
		    }
		    else {
		...
		* pow(frac,soil_con->c);
		...
		if(tmplayer<0) {
		  /** If top layer saturated, add to vic411_runoff **/
		  vic411_runoff[frost_area] += tmp_moist;
		  tmp_moist = 0;
		}
		else {
	...
	for ( lindex = 0; lindex < vic411_options.Nlayer; lindex++ ) 
	  ...
	  layer[lindex].moist += ((liq[lindex] + ice[lindex]) 
        ...
        cell->vic411_runoff   += vic411_runoff[frost_area] * frost_fract[frost_area];
        cell->baseflow += baseflow[frost_area] * frost_fract[frost_area];
#else
	layer[lindex].moist = liq[lindex] + ice[lindex];      
        cell->vic411_runoff   += vic411_runoff[frost_area];
        cell->baseflow += baseflow[frost_area];
#endif // SPATIAL_FROST

#if LINK_DEBUG
	if(debug.PRT_BALANCE) {
	  debug.outflow[dist][band][vic411_options.Nlayer+2] = (vic411_runoff[frost_area] + baseflow[frost_area]);
	  debug.outflow[dist][band][vic411_options.Nlayer+1] = *baseflow;
	}
#endif // LINK_DEBUG
#if SPATIAL_FROST
      }
  ...
  if(vic411_options.FULL_ENERGY || vic411_options.FROZEN_SOIL) {
    ...
    for(lindex=0;lindex<vic411_options.Nlayer;lindex++) {
      tmp_layer = vic411_find_average_layer(&(cell_wet->layer[lindex]), 
...
#endif
      ErrorFlag = vic411_distribute_node_moisture_properties(energy->moist, energy->ice,
