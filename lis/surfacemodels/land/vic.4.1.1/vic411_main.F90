...
!EOP
   ...
   vt_scheme = vic411_struc(n)%veg_tiling_scheme ! added by Shugong Wang 04/30/2012
   if ( alarmCheck ) then 
      ts = vic411_struc(n)%tscount
      nest = n
      do t = 1, LIS_rc%npatch(n,LIS_rc%lsm_index)
         ...
         call vic411_run(t, ts, vt_scheme,  & ! vt_scheme is added by Shugong on 04/30/12
      ...
      enddo
      vic411_struc(n)%tscount = vic411_struc(n)%tscount + 1
   endif
end subroutine vic411_main
