...
!BOP
! !ROUTINE: noahmp401_getsnodeppred
! \label{noahmp401_getsnodeppred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp401_getsnodeppred(n, k, obs_pred)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if (SNODEP_obs_obj(n)%mesh .eq. 8) then
        snwd(t) = noahmp401_struc(n)%noahmp401(t)%snowh*39.37 !convert from meter to inch
