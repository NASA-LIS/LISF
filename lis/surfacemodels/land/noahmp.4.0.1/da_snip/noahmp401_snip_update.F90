...
!EOP
  ...
  zsnso(-nsnow+1:nsoil) = &
       noahmp401_struc(n)%noahmp401(t)%zss(1:nsnow+nsoil)
  ...
  IF (ISNOW == 0 .AND. SNOWH >= 0.025.and. &
     ...
     SNOWH    = 0.
     STC(0)   = &
          MIN(273.16, NOAHMP401_struc(n)%noahmp401(t)%sfctmp)   ! temporary setup
     SNICE(0) = SNEQV
     SNLIQ(0) = 0.
  END IF
  ...
  do snl_idx=isnow+1,0
     if (snice(snl_idx)+snliq(snl_idx)>0.0) then
        ficeold(snl_idx) = &
             snice(snl_idx) / (snice(snl_idx)+snliq(snl_idx))
     else
        ficeold(snl_idx)  = 0.0
     endif
  enddo
  ...
  do j = 1,nsoil
     ...
     if(stc(j) < tfrz) then
        ...
        SUPERCOOL(J) = &
             parameters%SMCMAX(J) * &
             (SMP/parameters%PSISAT(J))**(-1./parameters%BEXP(J))
...
end subroutine noahmp401_snip_update
