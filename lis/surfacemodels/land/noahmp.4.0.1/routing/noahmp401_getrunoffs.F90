...
!BOP
! !ROUTINE: noahmp401_getrunoffs
!  \label{noahmp401_getrunoffs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp401_getrunoffs(n)
! !USES:
  use ESMF
...
!if option is not defined, then assume that no evap calculations will be done
  ...
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     runoff1(t) = NOAHMP401_struc(n)%noahmp401(t)%runsf/LIS_CONST_RHOFW
     runoff2(t) = NOAHMP401_struc(n)%noahmp401(t)%runsb/LIS_CONST_RHOFW
  enddo
...
!gather the model tiles before assigning to the global data structure.
  ...
  if(evapflag.ne.0)then
    ...
    if(LIS_masterproc) then
      ...
      call LIS_verify(status, "noahmp401_getrunoffs: ESMF_StateGet failed for Total Evapotranspiration")

      call ESMF_FieldGet(evapotranspiration_Field,localDE=0,farrayPtr=evapotranspiration,rc=status)
      call LIS_verify(status, "noahmp401_getrunoffs: ESMF_FieldGet failed for Total Evapotranspiration")
...
end subroutine noahmp401_getrunoffs
