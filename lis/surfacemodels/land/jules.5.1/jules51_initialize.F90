...
!BOP
!
! !ROUTINE: jules51_initialize
! \label{jules51_initialize}
!
! !REVISION HISTORY:
!
! !INTERFACE:
subroutine jules51_initialize(mtype)
   ! !USES:
...
! !DESCRIPTION:
!
!  This routine initializes the jules51 state variables with some
...
!EOP
   ...
   do n=1, LIS_rc%nnest
      write(LIS_logunit,*) "MSG: jules51_initialize -- initialzie for jules51"
      k = 1
      do
         ...
         do t=start_k, end_k
           ...
           bexp_soilt(1,1,:)   = jules51_struc(n)%jules51(t)%b
           sathh_soilt(1,1,:)  = jules51_struc(n)%jules51(t)%sathh
           satcon_soilt(1,1,:) = jules51_struc(n)%jules51(t)%satcon
           smvcst_soilt(1,1,:) = jules51_struc(n)%jules51(t)%sm_sat
           smvccl_soilt(1,1,:) = jules51_struc(n)%jules51(t)%sm_crit
           smvcwt_soilt(1,1,:) = jules51_struc(n)%jules51(t)%sm_wilt
           hcap_soilt(1,1,:)   = jules51_struc(n)%jules51(t)%hcap
           hcon_soilt(1,1,:)   = jules51_struc(n)%jules51(t)%hcon
           albsoil_soilt(1,1)  = jules51_struc(n)%jules51(t)%albsoil 
         ...
         call init_ic(jules51_struc(n)%namelist_dir) 
         call init_params(jules51_struc(n)%namelist_dir)
         call init_parms()
