...
! Specific humidity (Kg/Kg)
  qw_1_ij(1,1) = jules51_struc(n)%jules51(t)%qair/jules51_struc(n)%forc_count
! Air temperature (k)
  tl_1_ij(1,1) = jules51_struc(n)%jules51(t)%tair/jules51_struc(n)%forc_count
! W'ly wind component (m s-1)
  u_1_ij(1,1)  = jules51_struc(n)%jules51(t)%wind_e/jules51_struc(n)%forc_count
! S'ly wind component (m s-1)
  v_1_ij(1,1)  = jules51_struc(n)%jules51(t)%wind_n/jules51_struc(n)%forc_count
! Surface pressure (Pascals)
  pstar_ij(1,1)   = jules51_struc(n)%jules51(t)%psurf/jules51_struc(n)%forc_count
...
! All rain goes to Large-scale rainfall (kg m-2/s)
  if(jules51_struc(n)%jules51(t)%rainf .ne. LIS_rc%udef) then
    ls_rain_ij(1,1) = jules51_struc(n)%jules51(t)%rainf/ jules51_struc(n)%forc_count
  else
    ls_rain_ij(1,1) = 0.0
  endif

  if (LIS_FORC_Snowf%selectOpt .eq. 1) then
    ls_snow_ij(1,1) = jules51_struc(n)%jules51(t)%snowf / jules51_struc(n)%forc_count
  else 
    if (jules51_struc(n)%jules51(t)%tair .lt. LIS_CONST_TKFRZ) then
...
! Surface downward SW radiation (W m-2)
  sw_down_ij(1,1) = jules51_struc(n)%jules51(t)%swdown/jules51_struc(n)%forc_count
! Surface downward LW radiation (W m-2)
  lw_down_ij(1,1) = jules51_struc(n)%jules51(t)%lwdown/jules51_struc(n)%forc_count
