...
use time_varying_input_mod,    only: update_model_variables
use output_mod,                only: output_initial_data, &
                                     sample_data
...
!CALL update_model_variables()
CALL update_derived_variables()

!------------------------------------------------------------------
! Check if this is a timestep that we need to output initial data
! for (i.e. start of spinup cycle or start of main run), and output
...
CALL surf_couple_implicit(                                                    &
  ...
  wt_ext_surft, l_flux_bc, cdr10m, cdr10m, cdr10m_n, cdr10m_n,                &
  !Diagnostics, INTENT(INOUT)
  sf_diag,                                                                    &
  !Fluxes INTENT(INOUT)
