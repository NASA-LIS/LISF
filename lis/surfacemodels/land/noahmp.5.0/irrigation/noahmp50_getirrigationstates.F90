...
!----------------------------------------------------------------------
  ...
  !   sfctemp = Noahmp50_struc(n)%noahmp50(t)%sfctmp
     ...
     if((Noahmp50_struc(n)%dveg_opt == 2 .OR. &
         ...
         Noahmp50_struc(n)%dveg_opt == 6) .AND. LIS_rc%irrigation_dveg == 1) then
        shdfac = shdfac_avg
...
!                if(ippix.gt.0.0) then  ! irrigated tile
                 ...
                 !shdmin = minval(Noahmp50_struc(n)%noahmp50(t)%shdfac_monthly)
                 !shdmax = maxval(Noahmp50_struc(n)%noahmp50(t)%shdfac_monthly)
                ...
                !!!!! SPRINKLER IRRIGATION
                  ...
                  elseif(LIS_rc%irrigation_type.eq."Drip") then
                     ...
                     twater = 0.0
                         ...
                         Noahmp50_struc(n)%noahmp50(t)%smc(1) + (twater-twater2)/(sldpth(1)*1000.0) !! check this with Sujay
                       ...
                       !!!!! FLOOD IRRIGATION
                         ...
                         if(lroot.gt.0) then 
                            do k=1,lroot
                               asmc = asmc + Noahmp50_struc(n)%noahmp50(t)%smc(k)*&
                                    rdpth(k)*1000.0
                               tsmcwlt = tsmcwlt + smcwlt * rdpth(k)*1000.0
                               tsmcref = tsmcref + smcref * rdpth(k)*1000.0
                            enddo
...
!                            ma = (asmc-tsmcwlt) /(tsmcref - tsmcwlt)   ! Original
                            ...
                            if( ma .le. LIS_rc%irrigation_thresh ) then
                              do l = 1, LIS_rc%irrigation_mxsoildpth
                                 if( l == 1 ) then
                                   twater = (SMCMAX - Noahmp50_struc(n)%noahmp50(t)%smc(l))*sldpth(l)*1000.0
                                 else
                                 ! BZ modification 4/2/2015 to saturate entire column and apply ippix 
                                   twater = twater + (smcmax - Noahmp50_struc(n)%noahmp50(t)%smc(l))*sldpth(l)*1000.0
                                 endif
                              end do
                            ...
                            ! BZ modification 4/2/2015 to account for ippix and all soil layers:
                               do l = 1, LIS_rc%irrigation_mxsoildpth
                                  Noahmp50_struc(n)%noahmp50(t)%smc(l) = IrrigScale(t)*smcmax + &
                                                 (1-IrrigScale(t))*Noahmp50_struc(n)%noahmp50(t)%smc(l)
                               end do
                            endif  
                         endif
           ...
           if (LIS_rc%irrigation_GWabstraction.eq.1) then
              ...
              if (LIS_rc%irrigation_SourcePartition.eq.1) then
                  ...
                  Noahmp50_struc(n)%noahmp50(t)%wa = AWS - irrigRate(t)*Dtime*irriggwratio(t)/100
