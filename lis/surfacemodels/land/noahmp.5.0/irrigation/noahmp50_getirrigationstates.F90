...
!BOP
!
! !ROUTINE: noahmp50_getirrigationstates
! \label{noahmp50_getirrigationstates}
!
! !INTERFACE:
...
!----------------------------------------------------------------------
  ...
  do i=1,LIS_rc%npatch(n,LIS_rc%lsm_index)/LIS_rc%nensem(n)
     ...
     do m=1,LIS_rc%nensem(n)
        ...
        sfctemp_avg = sfctemp_avg + Noahmp50_struc(n)%noahmp50(t)%sfctmp
        shdfac_avg  = shdfac_avg + Noahmp50_struc(n)%noahmp50(t)%fveg
        do k=1,nsoil
           smc_avg(k) = smc_avg(k) + Noahmp50_struc(n)%noahmp50(t)%smc(k)
        end do
     end do
     ...
     do m=1,LIS_rc%nensem(n)
        t = (i-1)*LIS_rc%nensem(n)+m
        timestep = Noahmp50_struc(n)%dt
        soiltyp  = Noahmp50_struc(n)%noahmp50(t)%soiltype

        ! Adjust bounds by timestep to account for the fact that LIS_rc%hr,
        ! etc will represents the END of the integration timestep window
        ...
        ! this code block will need to be changed to account for variable
        ...
        if((Noahmp50_struc(n)%dveg_opt == 2 .OR. &
             Noahmp50_struc(n)%dveg_opt == 5 .OR. &
             Noahmp50_struc(n)%dveg_opt == 6) .AND. &
             LIS_rc%irrigation_dveg == 1) then
           shdfac = shdfac_avg
        else
           shdfac =  Noahmp50_struc(n)%noahmp50(t)%shdfac_monthly(LIS_rc%mo)
        end if
        ...
        ! This temperature check avoids irrigating at temperatures near or
        ! below 0C
        if((ltime.ge.shift_otimes).and.(ltime.le.shift_otimee).and. &
           ...
           if(LIS_rc%lcscheme.eq."UMD") then
              veg_index1 = 6
              veg_index2 = 11
           elseif(LIS_rc%lcscheme.eq."MODIS".or. &
                LIS_rc%lcscheme.eq."IGBPNCEP") then
              veg_index1 = 6
              veg_index2 = 14
           ...
           if(vegt.ge.veg_index1.and.vegt.le.veg_index2&
              ...
              if(irrigFrac(t).gt.0) then
                 ...
                 ! Determine the amount of irrigation to apply if irrigated
                 ! tile
                 if( IrrigScale(t).gt.0.0 ) then ! irrigated tile
                    shdmin =placeshdmin(LIS_surface(n, &
                         LIS_rc%lsm_index)%tile(t)%col,     &
                         LIS_surface(n,LIS_rc%lsm_index)%tile(t)%row)
                    shdmax =placeshdmax(LIS_surface(n, &
                         LIS_rc%lsm_index)%tile(t)%col,     &
                         LIS_surface(n,LIS_rc%lsm_index)%tile(t)%row)

                    ! let gsthresh be a function of the range, which means the
                    ! larger the range is, the higher GVF threshold will be
                    ! for this grid.
                    ! Gsthresh is a GVF threshold used to identify a growing
                    ! season for each pixel and allow irrigation during that
                    ! time
                    gsthresh = shdmin + &
                    ...
                    ! Changes needed to this code block to account for
                    ! variable soil layers in Noah-MP
                    ...
                    if(shdfac .ge. gsthresh) then
                       ...
                       if(crootd.gt.0.and.crootd.lt.zdpth(1)) then
                          lroot = 1
                          rdpth(1) = crootd
                       elseif(crootd .ge. zdpth(1).and. &
                            crootd .lt. zdpth(2) ) then
                          lroot = 2
                          rdpth(1) = sldpth(1)
                          rdpth(2) = crootd - zdpth(1)
                       elseif ( crootd.ge.zdpth(2).and. &
                            crootd .lt. zdpth(3) ) then
                          lroot = 3
                          rdpth(1) = sldpth(1)
                          rdpth(2) = sldpth(2)
                          rdpth(3) = crootd - zdpth(2)
                       elseif ( crootd.ge.zdpth(3).and. &
                            crootd .lt. zdpth(4) ) then
                          lroot = 4
                          rdpth(1) = sldpth(1)
                          rdpth(2) = sldpth(2)
                          rdpth(3) = sldpth(3)
...
!!!!! SPRINKLER IRRIGATION
                       if(LIS_rc%irrigation_type.eq."Sprinkler") then
                       !-------------------------------------------------------
                       ! Set the irrigation rate at start time; keep the
                       ! value till next day
                       ! If local time at the tile fall in the irrigation check
                       ! hour then check the root zone average soil moisture
                       !-------------------------------------------------------
                          if(ltime.eq.shift_otimes) then ! Check moisture
                                                         ! availability at
                                                         ! otimes only
                             !-------------------------------------------------
                             ! Compute the root zone accumlative soil
                             ! moisture [mm], field capacity [mm], and
                             ! wilting point [mm]
                             !-------------------------------------------------
                             if(lroot.gt.0) then
                                do k=1,lroot
                                ...
                                enddo
                                !----------------------------------------------
                                ! Get the root zone moisture availability to
                                ! the plant
                                !----------------------------------------------
                                ...
                                if(ma.le.LIS_rc%irrigation_thresh) then
                                   ...
                                   !-------------------------------------------
                                   ! Scale the irrigation intensity to the
                                   ! crop % when intensity < crop%.
                                   ! Expand irrigation for non-crop,
                                   ! non-forest when intensity > crop %
                                   ! in preference order of grassland first
                                   ! then rest.
                                   ! *scale is pre-computed for each tile in
                                   ! getirrpmapetc module in a way that is
                                   ! transparent to every tile irrigated or
                                   ! non-irrigated
                                   !-------------------------------------------
                                   twater1 = twater
...
!!!!! DRIP IRRIGATION (NOT CURRENTLY IMPLEMENTED)
                       elseif(LIS_rc%irrigation_type.eq."Drip") then
                          ...
                          ! Add enough water to get from actual Transp to
                          ! unstressed Transp
                          twater = 0.0
                          !----------------------------------------------------
                          ! Apply efficiency correction
                          ...
                          ! Compute irrigation rate
                          !----------------------------------------------------
                          irrigRate(t) = twater  ! for drip calculation,
                                                 ! twater is a rate [kg/m2/s]
                          Noahmp50_struc(n)%noahmp50(t)%smc(1) = &
                               Noahmp50_struc(n)%noahmp50(t)%smc(1) + &
                               (twater-twater2)/(sldpth(1)*1000.0)

!!!!! FLOOD IRRIGATION
                       elseif(LIS_rc%irrigation_type.eq."Flood") then
                          !----------------------------------------------------
                          ! Compute the root zone accumlative soil moisture
                          ! [mm], field capacity [mm], and wilting point [mm]
                          !----------------------------------------------------
                          if(lroot.gt.0) then
                             do k=1,lroot
                                asmc = asmc + &
                                     Noahmp50_struc(n)%noahmp50(t)%smc(k)*&
                                     rdpth(k)*1000.0
                                tsmcwlt = tsmcwlt + smcwlt * rdpth(k)*1000.0
                                tsmcref = tsmcref + smcref * rdpth(k)*1000.0
                             enddo
                             !-------------------------------------------------
                             ! Get the root zone moisture availability to the
                             ! plant
                             !-------------------------------------------------
                             ma = (asmc-tsmcwlt) / &
                                  (tsmcref - tsmcwlt)/IrrigScale(t)

                             if( ma .le. LIS_rc%irrigation_thresh ) then
                                do l = 1, LIS_rc%irrigation_mxsoildpth
                                   if( l == 1 ) then
                                      twater = (SMCMAX - &
                                      Noahmp50_struc(n)%noahmp50(t)%smc(l)) * &
                                      sldpth(l)*1000.0
                                   else
                                      twater = twater + &
                                           (smcmax - &
                                ...
                                !----------------------------------------------
                                ! Scale the irrigation intensity to the crop %
                                ! when intensity < crop%.
                                ! Expand irrigation for non-crop, non-forest
                                ! when intensity > crop %
                                ! in preference order of grassland first then
                                ! rest.
                                ! *scale is pre-computed for each tile in
                                ! getirrpmapetc module in a way that is
                                ! transparent to every tile irrigated or
           ...
           if (LIS_rc%irrigation_GWabstraction.eq.1) then
              AWS = Noahmp50_struc(n)%noahmp50(t)%wa
              Dtime = Noahmp50_struc(n)%ts
...
end subroutine noahmp50_getirrigationstates
