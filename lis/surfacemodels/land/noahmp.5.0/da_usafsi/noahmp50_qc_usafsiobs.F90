...
!BOP
! !ROUTINE: noahmp50_qc_usafsiobs
! \label{noahmp50_qc_usafsiobs}
!
! !REVISION HISTORY:
!  May 2023: Cenlin He; update to work with refactored NoahMP (v5.0 and newer)
!
! !INTERFACE:
subroutine noahmp50_qc_usafsiobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in noahmp50_qc_usafsiobs")
  call ESMF_FieldGet(obs_snow_field,localDE=0,farrayPtr=snowobs,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet failed in noahmp50_qc_usafsiobs")
...
end subroutine noahmp50_qc_usafsiobs
