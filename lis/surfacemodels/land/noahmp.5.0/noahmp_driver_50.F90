...
! !INTERFACE
subroutine noahmp_driver_50(n, LISparam)
...
!!!!! print all the options not supported in offline mode
  if (NoahmpIO%IOPT_SFC > 2) then
     ...
     abort_message(1) = &
          "(opt_sfc == 3) and (opt_sfc == 4) are not for offline use"
...
! Bondville location in Illinois, which uses Central time.
  ...
  call LIS_tick(loctime,locdoy,locgmt,locyr,locmo,locda,lochr,locmn,locss, &
       change)
  ...
  call calc_declin(nowdate(1:4)//"-"//nowdate(5:6)//"-"//nowdate(7:8)//"_"// &
       ...
       NoahmpIO%xlat(1,1), NoahmpIO%xlon(1,1), NoahmpIO%coszen(1,1), &
       NoahmpIO%yearlen, NoahmpIO%julian)

  read(nowdate(1:4), '(I4)') NoahmpIO%YR

  if ((NoahmpIO%IOPT_DVEG .eq. 1).or.(NoahmpIO%IOPT_DVEG .eq. 6).or. &
       (NoahmpIO%IOPT_DVEG .eq. 7)) then
     ! with dveg_opt==1/6/7, shdfac is fed directly to fveg
     NoahmpIO%vegfra(1,1) = month_d_new(NoahmpIO%shdfac_monthly(1,:,1), &
          nowdate)
  else
     ! with dveg_opt==2/3/8, fveg is computed from lai and sai, and shdfac is
     ! unused
     ! with dveg_opt==4/5/9, fveg is set to the maximum shdfac, and shdfac is
     ! unused
  ...
  NoahmpIO%QV_CURR(1,1,1)  = NoahmpIO%QV_CURR(1,1,1)/&
       (1.0 - NoahmpIO%QV_CURR(1,1,1)) ! Convert specific humidity to
  ...
  ! z0 and znt should be passed to WRF, if coupled.

  ! Code from module_NoahMP_hrldas_driver.F.  Initial guess only.
  if ((trim(LIS_rc%startcode).eq."coldstart").and. &
       (NoahmpIO%itimestep.eq.1)) then
     NoahmpIO%eahxy(1,1) = NoahmpIO%P8W(1,1,1) * &
          (NoahmpIO%QV_CURR(1,1,1)/(0.622+NoahmpIO%QV_CURR(1,1,1)))
...
end subroutine noahmp_driver_50

real function month_d_new(a12, nowdate) result (nowval)
  !
  ! Given a set of 12 values, taken to be valid on the fifteenth of each
  ! month (Jan through Dec) and a date in the form <YYYYMMDD[HHmmss]> ....
  ! Return a value valid for the day given in <nowdate>, as an interpolation
  ! from the 12 monthly values.
  !
  use kwm_date_utilities_50
  implicit none
  real, dimension(12), intent(in) :: a12 ! 12 monthly values,
                                         ! taken to be valid on the 15th of
                                         ! the month
  character(len=12), intent(in) :: nowdate ! Date, in the form
                                           ! <YYYYMMDD[HHmmss]>
  integer :: nowy, nowm, nowd
  integer :: prevm, postm
  real    :: factor
  integer, dimension(12) :: ndays = (/ 31, 28, 31, 30, 31, 30, 31, 31, 30, &
       31, 30, 31 /)
  !
  ! Handle leap year by setting the number of days in February for the year
  ! in question.
  !
...
!-----CALCULATE LONGITUDE OF THE SUN FROM VERNAL EQUINOX:
  ...
  TLOCTIM = REAL(IHOUR) + REAL(IMINUTE)/60.0 + REAL(ISECOND)/3600.0 + &
       LONGITUDE/15.0 ! Local time in hours
  tloctim = AMOD(tloctim+24.0, 24.0)
  HRANG=15.*(TLOCTIM-12.)*DEGRAD
  COSZ=SIN(LATITUDE*DEGRAD)*SIN(DECLIN)+COS(LATITUDE*DEGRAD)*COS(DECLIN)* &
       COS(HRANG)
  COSZ=MIN(COSZ,1.0);   !Address floating point roundoff errors
  COSZ=MAX(COSZ,-1.0);
