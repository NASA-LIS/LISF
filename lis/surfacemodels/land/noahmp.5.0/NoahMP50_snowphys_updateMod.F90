...
! ----------------------------------------------------------------------
    ...
    INTEGER,                         INTENT(IN) :: NSNOW  !maximum no. of
                                                          ! snow layers [ =3]
    INTEGER, DIMENSION(-NSNOW+1:0) , INTENT(IN) :: IMELT  !melting state
                                                          ! index [0-no melt;
                                                          ! 1-melt]
    REAL,                            INTENT(IN)  :: DT     !time step (sec)
    REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(IN)  :: STC    !snow layer
                                                           ! temperature [k]
    REAL, DIMENSION(-NSNOW+1:    0), INTENT(IN)  :: SNICE  !snow layer ice [mm]
    REAL, DIMENSION(-NSNOW+1:    0), INTENT(IN)  :: SNLIQ  !snow layer liquid
                                                           ! water [mm]
    REAL, DIMENSION(       1:NSOIL), INTENT(IN)  :: ZSOIL  !depth of
                                                   ! layer-bottom from soil srf
    REAL, DIMENSION(-NSNOW+1:    0), INTENT(IN)  :: FICEOLD ! ice fraction at
                                                            !  last timestep

    ! input and output
    INTEGER,                         INTENT(INOUT) :: ISNOW  ! actual no. of
                                                             ! snow layers
    REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(INOUT) :: DZSNSO ! snow layer
                                                             ! thickness [m]
    REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(INOUT) :: ZSNSO  ! depth of
                                                       ! snow/soil layer-bottom
    ...
    REAL, PARAMETER     :: DM = 100.0    ! upper Limit on destructive
...
!   REAL, PARAMETER     :: ETA0 = 0.8e+6 !viscosity coefficient [kg-s/m2]
    ...
    REAL, PARAMETER     :: ETA0 = 1.33e+6 ! Optimized based on SNOTEL obs
                                         ! (He et al. 2021 JGR)
    REAL :: BURDEN !pressure of overlying snow [kg/m2]
    REAL :: DDZ1   !rate of settling of snow pack due to destructive
                   ! metamorphism.
    ...
    REAL :: TD     !STC - TFRZ [K]
    REAL :: PDZDTC !nodal rate of change in fractional-thickness due to
...
! ----------------------------------------------------------------------
    ...
    DO J = ISNOW+1, 0
       ...
       IF (VOID > 0.001 .AND. SNICE(J) > 0.1) THEN
          ...
          DDZ2 = -(BURDEN+0.5*WX)* &
               EXP(-0.08*TD-C2*BI)/ETA0 ! 0.5*WX -> self-burden
          ...
          DZSNSO(J) = MIN(MAX(DZSNSO(J),(SNICE(J)+SNLIQ(J))/500.0), &
               (SNICE(J)+SNLIQ(J))/50.0)
  ...
  !  snow layer combination subroutine copied from NoahMP v4.5 (same physics as
  !  v5.0)
...
! ----------------------------------------------------------------------
    ...
    REAL, DIMENSION(1:NSOIL), INTENT(INOUT) :: SH2O !soil liquid moisture
                                                    ! (m3/m3)
    REAL, DIMENSION(1:NSOIL), INTENT(INOUT) :: SICE !soil ice moisture (m3/m3)
    REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(INOUT) :: STC   !snow layer
                                                            ! temperature [k]
    REAL, DIMENSION(-NSNOW+1:0), INTENT(INOUT) :: SNICE !snow layer ice [mm]
    REAL, DIMENSION(-NSNOW+1:0), INTENT(INOUT) :: SNLIQ !snow layer liquid
                                                        ! water [mm]
    REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(INOUT) :: DZSNSO !snow layer depth
                                                             ! [m]
    REAL,                            INTENT(INOUT) :: sneqv !snow water
                                                            ! equivalent [m]
...
!-----------------------------------------------------------------------
    ...
    DO J = ISNOW_OLD+1,0
       IF (SNICE(J) <= 0.1) THEN
          ...
          ELSE
             IF (ISNOW_OLD < -1) THEN    ! Change to ISNOW
                SNLIQ(J-1) = SNLIQ(J-1) + SNLIQ(J)
                SNICE(J-1) = SNICE(J-1) + SNICE(J)
                DZSNSO(J-1) = DZSNSO(J-1) + DZSNSO(J)
             ELSE
                IF(SNICE(J) >= 0.0) THEN
                   PONDING1 = SNLIQ(J) ! ISNOW WILL GET SET TO ZERO BELOW;
                                       ! PONDING1 WILL GET
                   SNEQV = SNICE(J)    ! ADDED TO PONDING FROM PHASECHANGE
                                       ! PONDING SHOULD BE
                   SNOWH = DZSNSO(J)   ! ZERO HERE BECAUSE IT WAS CALCULATED
                                       ! FOR THIN SNOW
                ELSE   ! SNICE OVER-SUBLIMATED EARLIER
                   PONDING1 = SNLIQ(J) + SNICE(J)
                   IF(PONDING1 < 0.0) THEN  ! IF SNICE AND SNLIQ SUBLIMATES
                                            ! REMOVE FROM SOIL
                      SICE(1) = SICE(1)+PONDING1/(DZSNSO(1)*1000.0) ! negative
                               ! SICE due to oversublimation is adjusted below
                      PONDING1 = 0.0
                   END IF
                   SNEQV = 0.0
                   SNOWH = 0.0
    ...
    IF(ISNOW ==0) RETURN   ! Get out if no longer multi-layer
    ...
    IF (SNOWH < 0.025 .AND. ISNOW < 0 ) THEN ! Change limit
       ISNOW  = 0
       SNEQV = ZWICE
...
! input
    ...
    INTEGER, INTENT(IN)                            :: NSNOW !maximum no. of
                                                            ! snow layers [ =3]
    INTEGER, INTENT(IN)                            :: NSOIL !no. of soil
                                                            ! layers [ =4]
    ...
    REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(INOUT) :: STC   !snow layer
                                                            ! temperature [K]
    REAL, DIMENSION(-NSNOW+1:    0), INTENT(INOUT) :: SNICE !snow layer ice
                                                            ! [mm]
    REAL, DIMENSION(-NSNOW+1:    0), INTENT(INOUT) :: SNLIQ !snow layer liquid
                                                            ! water [mm]
    REAL, DIMENSION(-NSNOW+1:NSOIL), INTENT(INOUT) :: DZSNSO!snow layer depth
...
! input
    ...
    real, parameter :: CWAT = 4.188E06 !specific heat capacity of water
                                       ! (J/m3/L)
    real, parameter :: TFRZ   = 273.16    !freezing/melting point (K)
