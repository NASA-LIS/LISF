...
!EOP
  ...
  write(LIS_logunit,*) &
       "[INFO] Start reading LIS configuration file for Noah-MP.5.0 ", &
       "(v5.0 or newer)"
  ...
  do n=1,LIS_rc%nnest
     ios = nf90_open(path=trim(LIS_rc%paramfile(n)), mode=NF90_NOWRITE, &
          ncid=nids(n))
     call LIS_verify(ios,'Error in nf90_open in '//trim(LIS_rc%paramfile(n)) &
          //' in NoahMP50_readcrd')
  enddo
  ...
  do n=1, LIS_rc%nnest
     do i = 1, Noahmp50_struc(n)%nsoil
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%sldpth(i), &
             rc=rc)
        call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     ios = nf90_get_att(nids(n), NF90_GLOBAL, 'LANDCOVER_SCHEME',  &
          landuse_scheme_name)
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, &
          Noahmp50_struc(n)%noahmp_tbl_name, rc=rc)
     call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%dveg_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  !  6->VIC; 7->XAJ; 8->DynVIC)
  call ESMF_ConfigFindLabel(LIS_config, &
       "Noah-MP.5.0 surface runoff option:", rc = rc)
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%runsfc_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  ! subsurface runoff and groundwater (1->SIMGM; 2->SIMTOP; 3->Schaake96;
  !  4->BATS; 5->MMF; 6->VIC; 7->XAJ; 8->DynVIC)
  call ESMF_ConfigFindLabel(LIS_config, &
       "Noah-MP.5.0 subsurface runoff and groundwater option:", rc = rc)
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%runsub_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  ! infiltration options for dynamic VIC (1->Philip; 2-> Green-Ampt;
  !  3 ->Smith-Parlange)
  call ESMF_ConfigFindLabel(LIS_config, &
       "Noah-MP.5.0 dynamic VIC infiltration option:", rc = rc)
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%infdv_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%tbot_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%soil_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%pedo_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%crop_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%urban_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  ! snow thermal conductivity option (1->Yen1965; 2->Anderson1976;
  !  3->Constant; 4->Verseghy1991; 5->Yen1981)
  call ESMF_ConfigFindLabel(LIS_config, &
       "Noah-MP.5.0 snow thermal conductivity option:", rc = rc)
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%tksno_opt, &
          rc=rc)
     call LIS_verify(rc, &
          "Noah-MP.5.0 snow thermal conductivity option: not defined")
     write(LIS_logunit,33) "snow thermal conductivity:", &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%irrm_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  do n=1, LIS_rc%nnest
     call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%tdrn_opt, &
          rc=rc)
     call LIS_verify(rc, &
  ...
  ! Setting some values to PLANTING, HARVEST, SEASON_GDD, SOILCOMP,
  !  SOILCL1-->SOILCL4 in NoahMP50_main.F90
  do n=1, LIS_rc%nnest
     !'NoahMP50_TBOT'
     Noahmp50_struc(n)%LDT_ncvar_tbot           = 'TBOT'
     !'NoahMP50_SHDFAC_MONTHLY'
     Noahmp50_struc(n)%LDT_ncvar_shdfac_monthly = 'GREENNESS'
     !'NoahMP50_PLANTING'
     Noahmp50_struc(n)%LDT_ncvar_planting       = 'PLANTING'
     !'NoahMP50_HARVEST'
     Noahmp50_struc(n)%LDT_ncvar_harvest        = 'HARVEST'
     !'NoahMP50_SEASON_GDD'
     Noahmp50_struc(n)%LDT_ncvar_season_gdd     = 'SEASON_GDD'
     !'NoahMP50_SOILCOMP'
     Noahmp50_struc(n)%LDT_ncvar_soilcomp       = 'SOILCOMP'
     !'NoahMP50_SOILCL1'
     Noahmp50_struc(n)%LDT_ncvar_soilcL1        = 'SOILCL1'
     !'NoahMP50_SOILCL2'
     Noahmp50_struc(n)%LDT_ncvar_soilcL2        = 'SOILCL2'
     !'NoahMP50_SOILCL3'
     Noahmp50_struc(n)%LDT_ncvar_soilcL3        = 'SOILCL3'
     !'NoahMP50_SOILCL4'
     Noahmp50_struc(n)%LDT_ncvar_soilcL4        = 'SOILCL4'
  ...
  if (trim(LIS_rc%startcode) == "restart") then
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%rfile, &
             rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%rformat, &
             rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, &
             Noahmp50_struc(n)%init_tskin, rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_sneqv,&
             rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_snowh,&
             rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, &
             Noahmp50_struc(n)%init_canwat, rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        do i=1, Noahmp50_struc(n)%nsoil
           call ESMF_ConfigGetAttribute(LIS_config, &
                Noahmp50_struc(n)%init_tslb(i), rc=rc)
        end do
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        do i=1, Noahmp50_struc(n)%nsoil
           call ESMF_ConfigGetAttribute(LIS_config, &
                Noahmp50_struc(n)%init_smc(i), rc=rc)
        end do
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_zwt, &
             rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_wa, &
             rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_wt, &
             rc=rc)
        call LIS_verify(rc, &
     ...
     do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_lai, &
             rc=rc)
        call LIS_verify(rc, &
