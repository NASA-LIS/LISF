...
!EOP
    ...
    write(LIS_logunit,*) &
         "[INFO] Start reading LIS configuration file for NoahMP.5.0 (v5.0 or newer)"
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 model timestep:", rc = rc)
    do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Time, rc = rc)
        call LIS_verify(rc, "NoahMP.5.0 model timestep: not defined")
        call LIS_parseTimeString(time, Noahmp50_struc(n)%ts)
    enddo

    ! Noah-MP soil process timestep
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 soil timestep:", rc = rc)
    do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Time, rc = rc)
        call LIS_verify(rc, "NoahMP.5.0 soil timestep: not defined")
        call LIS_parseTimeString(time, Noahmp50_struc(n)%ts_soil)
    enddo
    
    ! restart timestep
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 restart output interval:", rc = rc)
    do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Time, rc = rc)
        call LIS_verify(rc, &
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 number of soil layers:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%nsoil, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 number of soil layers: not defined")
    enddo
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 reference height of temperature and humidity:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%dz8w, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 reference height of temperature and "//&
             "humidity: not defined")
    enddo
 
    ! thickness of soil layers
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 thickness of soil layers:", rc = rc)
    do n=1, LIS_rc%nnest
        do i = 1, Noahmp50_struc(n)%nsoil
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%sldpth(i), rc=rc)
            call LIS_verify(rc, &
                 'NoahMP.5.0 thickness of soil layers: not defined')
        enddo
    enddo
    ...
    do n=1, LIS_rc%nnest
        ...
        elseif (trim(landuse_scheme_name) .eq. "UMD") then
         ...
         write(LIS_logunit,*) "[ERR] are supported by NoahMP.5.0 LSM"
         write(LIS_logunit,*) "[ERR] program stopping ..."
         call LIS_endrun()
        endif
    enddo
 
    ! NoahMP.5.0 parameter table (merged SOILPARM.TBL,GENPARM.TBL,MPTABLE.TBL)
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 parameter table:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%noahmp_tbl_name, rc=rc)
        call LIS_verify(rc, &
        "NoahMP.5.0 parameter table: not defined")
    enddo

    write(LIS_logunit,*) &
          "[INFO] Setting NoahMP.5.0 physics options:"
 
    ! dynamic vegetation
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 dynamic vegetation option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%dveg_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 dynamic vegetation option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 canopy stomatal resistance option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%crs_opt, rc=rc)
        call LIS_verify(rc, &
          "NoahMP.5.0 canopy stomatal resistance option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 soil moisture factor for stomatal resistance:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%btr_opt, rc=rc)
        call LIS_verify(rc, &
           "NoahMP.5.0 soil moisture factor for stomatal resistance:"//&
           " not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 surface runoff option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%runsfc_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 surface runoff option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 subsurface runoff and groundwater option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%runsub_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 subsurface runoff and groundwater option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 dynamic VIC infiltration option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%infdv_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 dynamic VIC infiltration option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 surface layer drag coefficient option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%sfc_opt, rc=rc)
        call LIS_verify(rc, &
            "NoahMP.5.0 surface layer drag coefficient option:"//&
           " not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 supercooled liquid water option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%frz_opt, rc=rc)
        call LIS_verify(rc, &
           "NoahMP.5.0 supercooled liquid water option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 frozen soil permeability option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%inf_opt, rc=rc)
        call LIS_verify(rc, &
            "NoahMP.5.0 frozen soil permeability option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 canopy radiative transfer option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%rad_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 canopy radiative transfer option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 snow surface albedo option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%alb_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 snow surface albedo option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 rain-snow partition option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%snf_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 rain-snow partition option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 lower boundary of soil temperature option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%tbot_opt, rc=rc)
        call LIS_verify(rc, &
           "NoahMP.5.0 lower boundary of soil temperature option:"//&
           " not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 snow&soil temperature time scheme option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%stc_opt, rc=rc)
        call LIS_verify(rc, &
           "NoahMP.5.0 snow&soil temperature time scheme option:"//&
           " not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 glacier ice option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%gla_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 glacier ice option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 surface resistance option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%rsf_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 surface resistance option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 soil configuration option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%soil_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 soil configuration option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 soil pedotransfer function option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%pedo_opt, rc=rc)
        call LIS_verify(rc, &
          "NoahMP.5.0 soil pedotransfer function option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 crop model option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%crop_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 crop model option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 urban physics option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%urban_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 urban physics option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 snow thermal conductivity option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%tksno_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 snow thermal conductivity option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 irrigation trigger option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%irr_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 irrigation trigger option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 irrigation method option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%irrm_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 irrigation method option: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, &
         "NoahMP.5.0 tile drainage option:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%tdrn_opt, rc=rc)
        call LIS_verify(rc, &
             "NoahMP.5.0 tile drainage option: not defined")
...
!------------------------------------------------------------------------------------------
    ...
    if (trim(LIS_rc%startcode) == "restart") then 
        Call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 restart file:", rc=rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%rfile, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 restart file: not defined")
        enddo
        
        Call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 restart file format:", rc=rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%rformat, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 restart file format: not defined")
        enddo
    ...
    ! coldstart run, read initial state variables
        ...
        ! skin temperature
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial surface skin temperature:", rc = rc)
        do n=1, LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_tskin, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial surface skin temperature:"//&
                 " not defined")
        enddo

        ! snow water equivalent
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial snow water equivalent:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_sneqv, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial snow water equivalent:"//&
                 " not defined")
        enddo

        ! physical snow depth
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial snow depth:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_snowh, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial snow depth:"//&
                 " not defined")
        enddo

        ! total canopy water + ice
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial total canopy surface water:", rc = rc)
        do n=1,LIS_rc%nnest
        ...
        ! soil temperature
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial soil temperatures:", rc = rc)
        do n=1,LIS_rc%nnest
            do i=1, Noahmp50_struc(n)%nsoil
               call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_tslb(i), rc=rc)
            end do
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial soil temperatures:"//&
                 " not defined")
        enddo

        ! volumetric soil moisture
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial total soil moistures:", rc = rc)
        do n=1,LIS_rc%nnest
            do i=1, Noahmp50_struc(n)%nsoil
                call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_smc(i), rc=rc)
            end do
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial total soil moistures:"//&
                 " not defined")
        enddo

        ! water table depth
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial water table depth:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_zwt, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial water table depth:"//&
                 " not defined")
        enddo

        ! water in the "aquifer"
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial water in the aquifer:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_wa, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial water in the aquifer:"//&
                 " not defined")
        enddo

        ! water in aquifer and saturated soil
        call ESMF_ConfigFindLabel(LIS_config, &
            "NoahMP.5.0 initial water in aquifer and saturated soil:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_wt, rc=rc)
            call LIS_verify(rc, &
            "NoahMP.5.0 initial water in aquifer and saturated soil:"//&
                 " not defined")
        enddo

        ! leaf area index
        call ESMF_ConfigFindLabel(LIS_config, &
             "NoahMP.5.0 initial leaf area index:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, Noahmp50_struc(n)%init_lai, rc=rc)
            call LIS_verify(rc, &
                 "NoahMP.5.0 initial leaf area index:"//&
                 " not defined")
        enddo
 ...
 33   format(a47,i4)
    ...
    write(LIS_logunit, *) &
        "[INFO] Finish reading LIS configuration file for NoahMP.5.0"
