...
!EOP
  ...
  ! set restart alarm
  write(fnest,'(i3.3)') n
  alarmCheck = LIS_isAlarmRinging(LIS_rc, "NoahMP50 restart alarm "// &
       trim(fnest))
...
!    smcwtd       - NoahMP soil moisture content in the layer to the water
!                     table when deep [-]
...
! The routines invoked are:
! \begin{description}
!   \item[LIS\_writeGlobalHeader\_restart]
!      (\ref{LIS_writeGlobalHeader_restart})\\
...
!EOP
  ...
  call LIS_writeHeader_restart(ftn, n, dimID, smc_ID, "SMC", &
       ...
       "m3/m3", vlevels=NoahMP50_struc(n)%nsoil , valid_min=-99999.0, &
       valid_max=99999.0, &
       var_flag = "dim2")
  ...
  call LIS_writeHeader_restart(ftn, n, dimID, tslb_ID, "TSLB", &
       "soil temperature", &
       "K", vlevels=NoahMP50_struc(n)%nsoil , valid_min=-99999.0, &
       valid_max=99999.0, &
       var_flag = "dim2")
  ...
  call LIS_writeHeader_restart(ftn, n, dimID, tsno_ID, "TSNO", &
       ...
       "K", vlevels=NoahMP50_struc(n)%nsnow , valid_min=-99999.0, &
       valid_max=99999.0, &
       var_flag = "dim3")
  ...
  call LIS_writeHeader_restart(ftn, n, dimID, zss_ID, "ZSS", &
       ...
       "m", vlevels=NoahMP50_struc(n)%nsnow+NoahMP50_struc(n)%nsoil , &
  ...
  call LIS_writeHeader_restart(ftn, n, dimID, snowice_ID, "SNOWICE", &
       "snow layer ice", &
       "mm", vlevels=NoahMP50_struc(n)%nsnow , valid_min=-99999.0, &
       valid_max=99999.0, &
       var_flag = "dim3")
  ...
  call LIS_writeHeader_restart(ftn, n, dimID, accetrani_ID, "ACC_ETRANI", &
       ...
       "m/s", vlevels=NoahMP50_struc(n)%nsoil , valid_min=-99999.0, &
       valid_max=99999.0, &
       var_flag = "dim2")
  ...
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%sfcrunoff, &
       varid=sfcrunoff_ID, dim=1, wformat=wformat)

  ! accumulated sub-surface runoff
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%udrrunoff, &
  ...
  ! total canopy water + ice
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%canwat, &
       varid=canwat_ID, dim=1, wformat=wformat)

  ! accumulated snow melt leaving pack
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%acsnom, &
       varid=acsnom_ID, dim=1, wformat=wformat)

  ! accumulated snow on grid
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%acsnow, &
       varid=acsnow_ID, dim=1, wformat=wformat)

  ! actual no. of snow layers
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%isnow, &
       varid=isnow_ID, dim=1, wformat=wformat)

  ! vegetation leaf temperature
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%tv, &
  ...
  if (NoahMP50_struc(n)%tdrn_opt >0) then
     ! accumulated tile drainage discharge
     call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
          NoahMP50_struc(n)%noahmp50%qtdrain, &
          varid=qtdrain_ID, dim=1, wformat=wformat)
  endif

  ! mass of grain XING
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%grain, &
       varid=grain_ID, dim=1, wformat=wformat)

  ! growing degree days XING (based on 10C)
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%gdd, &
       varid=gdd_ID, dim=1, wformat=wformat)

  ! growing degree days XING
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%pgs, &
       varid=pgs_ID, dim=1, wformat=wformat)

  ! for additional variables
  ! accumulated ground heat flux
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accssoil, &
       varid=accssoil_ID, dim=1, wformat=wformat)

  ! accumulated soil surface water flux
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accqinsur, &
       varid=accqinsur_ID, dim=1, wformat=wformat)

  ! accumulated soil surface evaporation
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accqseva, &
  ...
  ! accumulated water storage change
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accdwater, &
       varid=accdwater_ID, dim=1, wformat=wformat)

  ! accumulated precipitation
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accprcp, &
       varid=accprcp_ID, dim=1, wformat=wformat)

  ! accumulated canopy evaporation
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accecan, &
       varid=accecan_ID, dim=1, wformat=wformat)

  ! accumulated transpiration
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accetran, &
       varid=accetran_ID, dim=1, wformat=wformat)

  ! accumulated net soil evaporation
  call LIS_writevar_restart(ftn, n, LIS_rc%lsm_index, &
       NoahMP50_struc(n)%noahmp50%accedir, &
