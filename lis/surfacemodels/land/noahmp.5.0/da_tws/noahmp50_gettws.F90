...
!BOP
! !ROUTINE: noahmp50_gettws
! \label{noahmp50_gettws}
!
! !REVISION HISTORY:
...
! !INTERFACE:
subroutine noahmp50_gettws(n, LSM_State)
...
!EOP
  ...
  call LIS_verify(status,'ESMF_StateGet failed for sm1 in noahmp50_gettws')
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,'ESMF_StateGet failed for sm2 in noahmp50_gettws')
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,'ESMF_StateGet failed for sm3 in noahmp50_gettws')
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status,'ESMF_StateGet failed for sm4 in noahmp50_gettws')
  ...
  call LIS_verify(status,'ESMF_FieldGet failed for sm2 in noahmp50_gettws')
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,'ESMF_FieldGet failed for sm3 in noahmp50_gettws')
  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status,'ESMF_FieldGet failed for sm4 in noahmp50_gettws')
