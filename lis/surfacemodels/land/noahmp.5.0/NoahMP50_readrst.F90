...
!EOP
  ...
  do n=1, LIS_rc%nnest
     ...
     elseif(LIS_rc%startcode .eq. "restart") then
        ...
        write(LIS_logunit,*) &
             "[INFO] Noah-MP.5.0 restart file used: ", &
             trim(NoahMP50_struc(n)%rfile)
        ...
        do l=1, NoahMP50_struc(n)%nsnow + NoahMP50_struc(n)%nsoil
           call LIS_readvar_restart(ftn, n, LIS_rc%lsm_index, tmptilen, &
                varname="ZSS", &
                dim=l, vlevels = NoahMP50_struc(n)%nsnow + &
                NoahMP50_struc(n)%nsoil, wformat=wformat)
           do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
        ...
        if (NoahMP50_struc(n)%runsub_opt == 5) then
           ...
           ! read: soil moisture content in the layer to the water table
           !  when deep
           call LIS_readvar_restart(ftn, n, LIS_rc%lsm_index, &
