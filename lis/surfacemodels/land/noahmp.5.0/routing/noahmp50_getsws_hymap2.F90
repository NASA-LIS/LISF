...
!BOP
! !ROUTINE: noahmp50_getsws_hymap2
...
!EOP
  ...
  if(enable2waycpl==1) then 
     ...
     where(rivstotmp/=LIS_rc%udef) &
          NoahMP50_struc(n)%noahmp50(:)%rivsto=rivstotmp/NoahMP50_struc(n)%ts
     ...
     where(fldstotmp/=LIS_rc%udef)&
          NoahMP50_struc(n)%noahmp50(:)%fldsto=fldstotmp/NoahMP50_struc(n)%ts
     ...
     NoahMP50_struc(n)%noahmp50(:)%fldfrc=fldfrctmp
  endif  
end subroutine noahmp50_getsws_hymap2
