...
!EOP
    ...
    do n=1, LIS_rc%nnest
       ...
       do t=1, LIS_rc%npatch(n, LIS_rc%lsm_index)
          ...
          allocate(Noahmp50_struc(n)%noahmp50(t)%zss(Noahmp50_struc(n)%nsnow &
               + Noahmp50_struc(n)%nsoil))
          allocate(Noahmp50_struc(n)%noahmp50(t)%snowice( &
               Noahmp50_struc(n)%nsnow))
          allocate(Noahmp50_struc(n)%noahmp50(t)%snowliq( &
               Noahmp50_struc(n)%nsnow))
          allocate(Noahmp50_struc(n)%noahmp50(t)%smoiseq( &
               Noahmp50_struc(n)%nsoil))
          allocate(Noahmp50_struc(n)%noahmp50(t)%accetrani( &
               Noahmp50_struc(n)%nsoil))
       enddo
      ...
      ! Add soil timestep that is allowed to be different from main timestep
       call LIS_registerAlarm("NoahMP50 model alarm "//trim(fnest),&
            Noahmp50_struc(n)%ts, &
       ...
       ! Add alarm to reset tair_agl_min for RHMin.  This should
       ...
       ! Output soil layer thicknesses in centimeters for
