...
!BOP
! !ROUTINE: NoahMP50_qcsoilm
! \label{NoahMP50_qcsoilm}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine NoahMP50_qcsoilm(n, LSM_State)
...
!  real, pointer          :: soilm4(:)
  ...
  call LIS_verify(status,&
       "ESMF_AttributeGet: Max Value failed in NoahMP50_qcsoilm")

  call ESMF_AttributeGet(sm1Field,"Min Value",smmin1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Min Value failed in NoahMP50_qcsoilm")
...
end subroutine NoahMP50_qcsoilm
