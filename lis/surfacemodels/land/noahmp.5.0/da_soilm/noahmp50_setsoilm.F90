...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     ! Check  MIN_THRESHOLD < volumetric liquid soil moisture < threshold
     if(NoahMP50_struc(n)%noahmp50(t)%sh2o(1)+delta1.gt.MIN_THRESHOLD .and.&
        ...
        ! Save the flag for each tile (col*row*ens)   (64*44)*20
...
!------------------------------------------------------------------------------
! Create new falg: if update falg for 50% of the ensemble members is true
	...
	!icount = 1
	  ...
	  if(update_flag_tile(t)) then
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           ! Check update status for each tile
           if(update_flag_tile(t)) then
              ...
              if(NoahMP50_struc(n)%noahmp50(t)%sh2o(2)+delta2.gt.MIN_THRESHOLD .and.&
                 ...
                 if(soilm2(t).lt.0) then
                    ...
                    write(abort_message(1),*) 'setsoilm2 ',t,soilm2(t)
                    call LIS_abort(abort_message)
                 endif
              endif
              ...
              if(NoahMP50_struc(n)%noahmp50(t)%sh2o(3)+delta3.gt.MIN_THRESHOLD .and.&
                 ...
                 if(soilm3(t).lt.0) then
                    ...
                    write(abort_message(1),*) 'setsoilm3 ',t,soilm3(t)
                    call LIS_abort(abort_message)
                 endif
              endif
              ...
              if(NoahMP50_struc(n)%noahmp50(t)%sh2o(4)+delta4.gt.MIN_THRESHOLD .and.&
                 ...
                 if(soilm4(t).lt.0) then
                    ...
                    write(abort_message(1),*) 'setsoilm4 ',t,soilm4(t)
                    call LIS_abort(abort_message)
                 endif
              endif

!------------------------------------------------------------------------------
              ! randomly resample the smc from [MIN_THRESHOLD,  Max value
              ! from DA @ that tiem step]
!------------------------------------------------------------------------------
           else
