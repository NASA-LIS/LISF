...
!rescaling to relative wetness space. 
     if(LIS_rc%dascaloption(k).eq."Linear scaling") then 
        ...
        if(noahmp50_dasm_struc(n)%ntimes.gt.1) then 
           kk = LIS_rc%mo
        else
           kk = 1
        endif
        ...
        do i=1,LIS_rc%obs_ngrid(k)
           do m=1,LIS_rc%nensem(n)
              obs_tmp = (obs_pred(i,m) - noahmp50_dasm_struc(n)%model_xrange(i,kk,1))/&
                   (noahmp50_dasm_struc(n)%model_xrange(i,kk,noahmp50_dasm_struc(n)%nbins) - & 
                   noahmp50_dasm_struc(n)%model_xrange(i,kk,1))
     ...
     do i=1,LIS_rc%npatch(n,LIS_rc%lsm_index),LIS_rc%nensem(n)
        do m=1,LIS_rc%nensem(n)
           ...
           inputs_tp(1) = NoahMP50_struc(n)%noahmp50(t)%smc(1)
           inputs_tp(2) = NoahMP50_struc(n)%noahmp50(t)%prcp*(1-NoahMP50_struc(n)%noahmp50(t)%fpice) !Noah33 rainf
           inputs_tp(3) = NoahMP50_struc(n)%noahmp50(t)%prcp*NoahMP50_struc(n)%noahmp50(t)%fpice     !Noah33 snowf    
           !MN: NOTE:  noahmp50 --> prcp (total precip), fpice (snow fraction in precipitation [-])
           inputs_tp(4) = NoahMP50_struc(n)%noahmp50(t)%fveg !Noah33 shdfac
           inputs_tp(5) = NoahMP50_struc(n)%noahmp50(t)%sstc(NoahMP50_struc(n)%nsnow+1)  !Noah33 stc(1)
           inputs_tp(6) = NoahMP50_struc(n)%noahmp50(t)%sneqv*LIS_CONST_RHOFW
