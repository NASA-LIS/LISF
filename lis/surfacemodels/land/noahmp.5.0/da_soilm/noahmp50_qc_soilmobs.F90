...
!  with snow (4) ground is covered with vegatation (more than 50%).
!
!  The arguments are:
!  \begin{description}
...
!EOP
  ...
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       ...
       rainf_obs)  ! prcp is total precip
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
  ...
  do t = 1,LIS_rc%obs_ngrid(k)
     ...
     if(smobs(t).ne.LIS_rc%udef) then
        ...
        elseif(sca_obs(t).gt.0.0001) then  ! Var name sca
           smobs(t) = LIS_rc%udef
           ! Check for green vegetation fraction
        elseif(shdfac_obs(t).gt.0.9) then  ! var name Noah36 shdfac 12-month
                                           ! green veg. frac.
                                           ! The threshold has been tuned for
                                           ! spatial coverage
                                           ! Higher than Noah.3.9 because max
                                           ! greenness is used for shdfac in
                                           ! Noah-MP.4.0.1 while Noah3.9 uses
                                           ! monthly climatological greenness.
           smobs(t) = LIS_rc%udef
