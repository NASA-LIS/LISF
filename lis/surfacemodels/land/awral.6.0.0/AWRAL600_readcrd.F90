...
!EOP
    ...
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 model timestep:", rc = rc)
    do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Time, rc = rc)
        call LIS_verify(rc, "[ERR] AWRAL600 model timestep: not defined")
        call LIS_parseTimeString(time, AWRAL600_struc(n)%ts)
    enddo
    
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 restart output interval:", rc = rc)
    do n=1,LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, Time, rc = rc)
        call LIS_verify(rc,"[ERR] AWRAL600 restart output interval: not defined")
    ...
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 nhru:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%nhru, rc=rc)
        call LIS_verify(rc, "[ERR] AWRAL600 nhru: not defined")
    enddo

    ! number of hypsometric curve percentile bins
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 nhypsbins:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%nhypsbins, rc=rc)
        call LIS_verify(rc, "[ERR] AWRAL600 nhypsbins: not defined")
    enddo

    ! scaling factor for slope
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 slope_coeff:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%slope_coeff, rc=rc)
        call LIS_verify(rc, "[ERR] AWRAL600 slope_coeff: not defined")
    enddo
 
    ! air pressure
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 pair:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%pair, rc=rc)
        call LIS_verify(rc, "[ERR] AWRAL600 pair: not defined")
    enddo
 
    ! scaling factor for ratio of saturated hydraulic conductivity
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 kr_coeff:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%kr_coeff, rc=rc)
        call LIS_verify(rc, "[ERR] AWRAL600 kr_coeff: not defined")
    enddo

    ! hypsometric percentile bins
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 hypsperc:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhypsbins
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 hypsperc: not defined')
        enddo
    enddo

    ! dry soil albedo for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 alb_dry:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 alb_dry: not defined')
        enddo
    enddo
 
    ! wet soil albedo for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 alb_wet:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 alb_wet: not defined')
        enddo
    enddo
 
    ! coefficient relating vegetation photosynthetic capacity to maximum stomatal conductance for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 cgsmax:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 cgsmax: not defined')
        enddo
    enddo
 
    ! specific ratio of the mean evaporation rate and the mean rainfall intensity during storms for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 er_frac_ref:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 er_frac_ref: not defined')
        enddo
    enddo
 
    ! soil evaporation scaling factor corresponding to unlimited soil water supply for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 fsoilemax:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 fsoilemax: not defined')
        enddo
    enddo
 
    ! reference leaf area index (at which fv = 0.63) for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 lairef:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 lairef: not defined')
        enddo
    enddo
 
    ! rooting depth for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 rd:", rc = rc)
    do n=1, LIS_rc%nnest
        allocate(AWRAL600_struc(n)%rd(AWRAL600_struc(n)%nhru))
        do i = 1, AWRAL600_struc(n)%nhru
            call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%rd(i), rc=rc)
            call LIS_verify(rc, '[ERR] AWRAL600 rd: not defined')
        enddo
    enddo
 
    ! specific canopy rainfall storage per unit leaf area for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 s_sls:", rc = rc)
    do n=1, LIS_rc%nnest
    ...
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 tgrow: not defined')
        enddo
    enddo
 
    ! characteristic time scale for vegetation senescence towards equilibrium for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 tsenc:", rc = rc)
    do n=1, LIS_rc%nnest
    ...
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 w0lime:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 w0lime: not defined')
        enddo
    enddo
 
    ! Reference value of w0 that determines the rate of albedo decrease with wetness for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 w0ref_alb:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 w0ref_alb: not defined')
        enddo
    enddo
 
    ! water-limiting relative water content of the deep soil store for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 wdlimu:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 wdlimu: not defined')
        enddo
    enddo
 
    ! water-limiting relative water content of the shallow soil store for each hru
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 wslimu:", rc = rc)
    do n=1, LIS_rc%nnest
        ...
        do i = 1, AWRAL600_struc(n)%nhru
            ...
            call LIS_verify(rc, '[ERR] AWRAL600 wslimu: not defined')
        enddo
    enddo
 
    ! number of daily timesteps
    call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 timesteps:", rc = rc)
    do n=1, LIS_rc%nnest
        call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%timesteps, rc=rc)
        call LIS_verify(rc, "[ERR] AWRAL600 timesteps: not defined")
    enddo
    ...
    if (trim(LIS_rc%startcode) == "restart") then 
        Call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 restart file:", rc=rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%rfile, rc=rc)
            call LIS_verify(rc, "[ERR] AWRAL600 restart file: not defined")
        enddo
        
        Call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 restart file format:", rc=rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%rformat, rc=rc)
            call LIS_verify(rc, "[ERR] AWRAL600 restart file format: not defined")
        enddo
    ! cold start run, read initial state variables
    else 
        ! volume of water in the surface water store
        call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 initial sr:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%init_sr, rc=rc)
            call LIS_verify(rc, "[ERR] AWRAL600 initial sr: not defined")
        enddo

        ! groundwater storage in the unconfined aquifer
        call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 initial sg:", rc = rc)
        do n=1,LIS_rc%nnest
            call ESMF_ConfigGetAttribute(LIS_config, AWRAL600_struc(n)%init_sg, rc=rc)
            call LIS_verify(rc, "[ERR] AWRAL600 initial sg: not defined")
        enddo

        ! water storage in the surface soil layer for each hru
        call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 initial s0:", rc = rc)
        do n=1,LIS_rc%nnest
            ...
            call LIS_verify(rc, "[ERR] AWRAL600 initial s0: not defined")
        enddo

        ! water content of the shallow soil store for each hru
        call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 initial ss:", rc = rc)
        do n=1,LIS_rc%nnest
            ...
            call LIS_verify(rc, "[ERR] AWRAL600 initial ss: not defined")
        enddo

        ! water content of the deep soil store for each hru
        call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 initial sd:", rc = rc)
        do n=1,LIS_rc%nnest
            ...
            call LIS_verify(rc, "[ERR] AWRAL600 initial sd: not defined")
        enddo

        ! leaf biomass
        call ESMF_ConfigFindLabel(LIS_config, "[INFO] AWRAL600 initial mleaf:", rc = rc)
        do n=1,LIS_rc%nnest
            ...
            call LIS_verify(rc, "[ERR] AWRAL600 initial mleaf: not defined")
        enddo
