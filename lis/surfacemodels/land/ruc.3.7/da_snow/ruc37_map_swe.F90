...
!
! !INTERFACE:
subroutine RUC37_map_swe(n,OBS_State,LSM_State)
! !USES:
  use ESMF
...
! Based on SWE, we manually update snowdepth    
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if(sweobs(LIS_surface(n,1)%tile(t)%index).gt.0) then 
        ...
        newsn = swe(t)
        snowhc = RUC37_struc(n)%ruc37(t)%snowh*100.0
        newsnc = newsn*100.0
        if(RUC37_struc(n)%ruc37(t)%tair.lt.LIS_CONST_TKFRZ) then 
           tempc = RUC37_struc(n)%ruc37(t)%tair-LIS_CONST_TKFRZ
        else
           tempc = 0.0
        endif
