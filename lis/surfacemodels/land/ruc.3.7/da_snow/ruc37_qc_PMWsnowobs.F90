...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in RUC37_qc_PMWsnowobs")
  call ESMF_FieldGet(obs_snow_field,localDE=0,farrayPtr=snowobs,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet failed in RUC37_qc_PMWsnowobs")
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(snowobs(gid).ne.LIS_rc%udef) then 
        !GVF mask
!        if(PMW_snow_struc(n)%gvf_mask.eq.1 .and. RUC37_struc(n)%ruc37(t)%shdfac.gt.0.7) then 
!           snowobs(gid) = LIS_rc%udef
        !forest mask        
        if(PMW_snow_struc(n)%lctype_mask.eq.1 .and. RUC37_struc(n)%ruc37(t)%vegetype.le.4) then
!        elseif(PMW_snow_struc(n)%lctype_mask.eq.1 .and. RUC37_struc(n)%ruc37(t)%vegetype.le.4) then
           snowobs(gid) = LIS_rc%udef
        !LSM temperature mask
        elseif(PMW_snow_struc(n)%temp_mask.eq.1 .and. RUC37_struc(n)%ruc37(t)%tskin.ge.278.15) then 
           snowobs(gid) = LIS_rc%udef
        !LSM temperature mask
        elseif(PMW_snow_struc(n)%temp_mask.eq.1 .and. RUC37_struc(n)%ruc37(t)%stc(1).ge.278.15) then 
           snowobs(gid) = LIS_rc%udef
        endif
     endif
  enddo

end subroutine RUC37_qc_PMWsnowobs
