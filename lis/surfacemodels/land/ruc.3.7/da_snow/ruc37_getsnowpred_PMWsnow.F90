...
!BOP
! !ROUTINE: RUC37_getsnowpred_PMWsnow
! \label{RUC37_getsnowpred_PMWsnow}
!
! !REVISION HISTORY:
...
! !INTERFACE:
subroutine RUC37_getsnowpred_PMWsnow(n, obs_pred)
  ...
  do i=1,LIS_rc%npatch(n,LIS_rc%lsm_index),LIS_rc%nensem(n)
     do m=1,LIS_rc%nensem(n)
        ...
        if (PMW_snow_struc(n)%data_var .eq. 'snow depth') then
            obs_pred(gid,m)= RUC37_struc(n)%ruc37(t)%snowh*c1
        elseif (PMW_snow_struc(n)%data_var .eq. 'SWE') then
            obs_pred(gid,m)= RUC37_struc(n)%ruc37(t)%sneqv*c1
        else
...
end subroutine RUC37_getsnowpred_PMWsnow
