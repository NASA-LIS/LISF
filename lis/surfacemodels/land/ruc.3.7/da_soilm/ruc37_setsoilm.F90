...
!BOP
! !ROUTINE: RUC37_setsoilm
!  \label{RUC37_setsoilm}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine RUC37_setsoilm(n, LSM_State)
! !USES:
  use ESMF
...
! !DESCRIPTION:
!  
!  This routine assigns the soil moisture prognostic variables to ruc37's
!  model space. 
! 
!EOP
  ...
  do t=1,LIS_rc%ntiles(n)
     RUC37_struc(n)%ruc37(t)%smc(1) = soilm1(t)
     RUC37_struc(n)%ruc37(t)%smc(2) = soilm2(t)
     RUC37_struc(n)%ruc37(t)%smc(3) = soilm3(t)
     RUC37_struc(n)%ruc37(t)%smc(4) = soilm4(t)
     RUC37_struc(n)%ruc37(t)%smc(5) = soilm5(t)
     RUC37_struc(n)%ruc37(t)%smc(6) = soilm6(t)
     RUC37_struc(n)%ruc37(t)%smc(7) = soilm7(t)
     RUC37_struc(n)%ruc37(t)%smc(8) = soilm8(t)
     RUC37_struc(n)%ruc37(t)%smc(9) = soilm9(t)
  enddo
end subroutine RUC37_setsoilm
