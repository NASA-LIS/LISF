...
!BOP
! !ROUTINE: RUC37_getLbandTbPred
! \label{RUC37_getLbandTbPred}
!
! !REVISION HISTORY:
! 13 Sept 2012: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine RUC37_getLbandTbPred(n, obs_pred)
! !USES:
  use ESMF
...
!call the forward model
  ...
  call LIS_verify(status, &
       "Error in StateGet in RUC37_getLbandTbPred for Lband_Hpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbH, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in RUC37_getLbandTbPred for Lband_Hpol')

  call ESMF_StateGet(LIS_forwardState(n), "Lband_Vpol", varField, rc=status)
  call LIS_verify(status, &
       "Error in StateGet in RUC37_getLbandTbPred for Lband_Vpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbV, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in RUC37_getLbandTbPred for Lband_Vpol')
...
end subroutine RUC37_getLbandTbPred
