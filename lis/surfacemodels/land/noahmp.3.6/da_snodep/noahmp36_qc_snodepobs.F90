...
!BOP
! !ROUTINE: noahmp36_qc_snodepobs
! \label{noahmp36_qc_snodepobs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp36_qc_snodepobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in noahmp36_qc_snodepobs")
  call ESMF_FieldGet(obs_snow_field,localDE=0,farrayPtr=snowobs,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet failed in noahmp36_qc_snodepobs")
  
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     stc1(t) = noahmp36_struc(n)%noahmp36(t)%sstc(1) ! get snow/veg temp.
     vegt(t) = LIS_surface(n,1)%tile(t)%vegt
  enddo

  call LIS_convertPatchSpaceToObsSpace(n,k,&       
       LIS_rc%lsm_index, noahmp36_struc(n)%noahmp36(:)%tv,tv_obs) !tv: vegetation temperature. unit: K 
  call LIS_convertPatchSpaceToObsSpace(n,k,LIS_rc%lsm_index, &    !fveg: green vegetation fraction. unit: - 
       noahmp36_struc(n)%noahmp36(:)%fveg,fveg_obs)
...
end subroutine noahmp36_qc_snodepobs
