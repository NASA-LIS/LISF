...
!BOP
! !ROUTINE: noahmp36_getsmpred
! \label{noahmp36_getsmpred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp36_getsmpred(n, k,obs_pred)
! !USES:
  use ESMF
...
!rescaling to relative wetness space. 
     if(LIS_rc%dascaloption(k).eq."Linear scaling") then 
        ...
        if(noahmp36_dasm_struc(n)%ntimes.gt.1) then 
           kk = LIS_rc%mo
        else
           kk = 1
        endif
        ...
        do i=1,LIS_rc%obs_ngrid(k)
           do m=1,LIS_rc%nensem(n)
              obs_tmp = (obs_pred(i,m) - noahmp36_dasm_struc(n)%model_xrange(i,kk,1))/&
                   (noahmp36_dasm_struc(n)%model_xrange(i,kk,noahmp36_dasm_struc(n)%nbins) - & 
                   noahmp36_dasm_struc(n)%model_xrange(i,kk,1))
     ...
     do i=1,LIS_rc%npatch(n,LIS_rc%lsm_index),LIS_rc%nensem(n)
        do m=1,LIS_rc%nensem(n)
           ...
           inputs_tp(1) = NOAHMP36_struc(n)%noahmp36(t)%smc(1)
           inputs_tp(2) = NOAHMP36_struc(n)%noahmp36(t)%prcp*(1-NOAHMP36_struc(n)%noahmp36(t)%fpice) !Noah33 rainf
           inputs_tp(3) = NOAHMP36_struc(n)%noahmp36(t)%prcp*NOAHMP36_struc(n)%noahmp36(t)%fpice     !Noah33 snowf    
           !MN: NOTE:  Noahmp36 --> prcp (total precip), fpice (snow fraction in precipitation [-])
           inputs_tp(4) = NOAHMP36_struc(n)%noahmp36(t)%fveg !Noah33 shdfac
           inputs_tp(5) = NOAHMP36_struc(n)%noahmp36(t)%sstc(NOAHMP36_struc(n)%nsnow+1)  !Noah33 stc(1)
           inputs_tp(6) = NOAHMP36_struc(n)%noahmp36(t)%sneqv*LIS_CONST_RHOFW
...
#endif
end subroutine noahmp36_getsmpred
