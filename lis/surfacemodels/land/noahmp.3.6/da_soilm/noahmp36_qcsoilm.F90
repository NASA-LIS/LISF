...
!BOP
! !ROUTINE: noahmp36_qcsoilm
! \label{noahmp36_qcsoilm}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp36_qcsoilm(n, LSM_State)
...
!  real, pointer          :: soilm4(:)
  ...
  call LIS_verify(status,&
       "ESMF_StateGet for Soil Moisture Layer 1 failed in noahmp36_qcsoilm")
 
  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet for Soil Moisture Layer 1 failed in noahmp36_qcsoilm")

  call ESMF_AttributeGet(sm1Field,"Max Value",smmax1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Max Value failed in noahmp36_qcsoilm")

  call ESMF_AttributeGet(sm1Field,"Min Value",smmin1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Min Value failed in noahmp36_qcsoilm")
...
end subroutine noahmp36_qcsoilm
