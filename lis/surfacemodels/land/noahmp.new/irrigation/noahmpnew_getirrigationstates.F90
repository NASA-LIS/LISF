...
!BOP
! 
! !ROUTINE: noahmpnew_getirrigationstates
...
!----------------------------------------------------------------------
  ...
  !   sfctemp = NoahmpNew_struc(n)%noahmpnew(t)%sfctmp
     ...
     if((NoahmpNew_struc(n)%dveg_opt == 2 .OR. &
        ...
        shdfac =  NoahmpNew_struc(n)%noahmpnew(t)%shdfac_monthly(LIS_rc%mo)
     end if
                ...
                !!!!! SPRINKLER IRRIGATION
                  ...
                  elseif(LIS_rc%irrigation_type.eq."Drip") then
                     ...
                     twater = 0.0
                         ...
                         NoahmpNew_struc(n)%noahmpnew(t)%smc(1) = &
                         NoahmpNew_struc(n)%noahmpnew(t)%smc(1) + (twater-twater2)/(sldpth(1)*1000.0) !! check this with Sujay
                       ...
                       !!!!! FLOOD IRRIGATION
                         ...
                         if(lroot.gt.0) then 
                            do k=1,lroot
                               asmc = asmc + NoahmpNew_struc(n)%noahmpnew(t)%smc(k)*&
                                    rdpth(k)*1000.0
                               tsmcwlt = tsmcwlt + smcwlt * rdpth(k)*1000.0
                               tsmcref = tsmcref + smcref * rdpth(k)*1000.0
                            enddo
...
!                            ma = (asmc-tsmcwlt) /(tsmcref - tsmcwlt)   ! Original
                            ...
                            if( ma .le. LIS_rc%irrigation_thresh ) then
                              do l = 1, LIS_rc%irrigation_mxsoildpth
                                 if( l == 1 ) then
                                   twater = (SMCMAX - NoahmpNew_struc(n)%noahmpnew(t)%smc(l))*sldpth(l)*1000.0
                                 else
                            ...
                            ! BZ modification 4/2/2015 to account for ippix and all soil layers:
                               do l = 1, LIS_rc%irrigation_mxsoildpth
                                  NoahmpNew_struc(n)%noahmpnew(t)%smc(l) = IrrigScale(t)*smcmax + &
                                                 (1-IrrigScale(t))*NoahmpNew_struc(n)%noahmpnew(t)%smc(l)
                               end do
                            endif  
                         endif
           ...
           if (LIS_rc%irrigation_GWabstraction.eq.1) then
              AWS = NoahmpNew_struc(n)%noahmpnew(t)%wa
              Dtime = NoahmpNew_struc(n)%ts
              if (LIS_rc%irrigation_SourcePartition.eq.1) then
                  if(irriggwratio(t).gt.0) then
                  NoahmpNew_struc(n)%noahmpnew(t)%wa = AWS - irrigRate(t)*Dtime*irriggwratio(t)/100

                  end if
              else
                 NoahmpNew_struc(n)%noahmpnew(t)%wa = AWS - irrigRate(t)*Dtime
              end if
           end if
       end if
  ...
  end subroutine noahmpnew_getirrigationstates
