...
!---------------------------------------------------------------------
    ...
    do n=1, LIS_rc%nnest
        isnow = -NoahmpNew_struc(n)%nsnow

        ! Added by Shugong
        allocate(zsnso(-NoahmpNew_struc(n)%nsnow+1:NoahmpNew_struc(n)%nsoil))
        ! Modified by Zhuo Wang
        allocate(tsnow(-NoahmpNew_struc(n)%nsnow+1:0))
        allocate(snice(-NoahmpNew_struc(n)%nsnow+1:0))
        allocate(snliq(-NoahmpNew_struc(n)%nsnow+1:0))
        allocate(zsoil(NoahmpNew_struc(n)%nsoil))
        ! Added by David
        allocate(tsnoxy(1,-NoahmpNew_struc(n)%nsnow+1:0,1))
        allocate(zsnsoxy(1,-NoahmpNew_struc(n)%nsnow+1:NoahmpNew_struc(n)%nsoil,1))
        allocate(snicexy(1,-NoahmpNew_struc(n)%nsnow+1:0,1))
        allocate(snliqxy(1,-NoahmpNew_struc(n)%nsnow+1:0,1))
        allocate(smoiseqxy(1,NoahmpNew_struc(n)%nsoil,1))
        ...
        do l=2, NoahmpNew_struc(n)%nsoil
          zsoil(l) = zsoil(l-1) - NoahmpNew_struc(n)%sldpth(l) 
        enddo
        ! end add 
...
!----------------------------------------------------------------------
        if (trim(LIS_rc%startcode) .eq. "coldstart") then
           ...
           do t=1, LIS_rc%npatch(n,mtype)
            ...
            ! added by shugong 
            zsnso = 0.0
            tmp_swe(1,1) = NoahmpNew_struc(n)%init_sneqv
            ...
            call snow_init_401(1, 1, 1, 1, 1, 1, 1, 1,           & !input 
             ...
             snicexy(1,-NoahmpNew_struc(n)%nsnow+1:0,1) = snice(-NoahmpNew_struc(n)%nsnow+1:0)
             snliqxy(1,-NoahmpNew_struc(n)%nsnow+1:0,1) = snliq(-NoahmpNew_struc(n)%nsnow+1:0)
             zsnsoxy(1,-NoahmpNew_struc(n)%nsnow+1:NoahmpNew_struc(n)%nsoil,1) = zsnso(-NoahmpNew_struc(n)%nsnow+1:NoahmpNew_struc(n)%nsoil) 
             tsnoxy(1,-NoahmpNew_struc(n)%nsnow+1:0,1) = tsnow(-NoahmpNew_struc(n)%nsnow+1:0)
             ...
             ISLTYP(1,1) = NoahmpNew_struc(n)%noahmpnew(t)%soiltype

             do l=1, NoahmpNew_struc(n)%nsoil
               tslb_3d(1,l,1) = NoahmpNew_struc(n)%init_tslb(l)  ! Noah-MP.4.0.1 initial soil temperatures: 
               smois_3d(1,l,1) = NoahmpNew_struc(n)%init_smc(l)  ! Noah-MP.4.0.1 initial total soil moistures:
               sh2o_3d(1,l,1) = 0.0 ! Noah-MP.4.0.1 initial liquid soil moistures set in NOAHMP_INIT
               DZS(l) = NoahmpNew_struc(n)%sldpth(l)    ! Noah-MP.4.0.1 thickness of soil layers:
             enddo

             FNDSOILW = .true.
             FNDSNOWH = .true.

             TSK(1,1) = NoahmpNew_struc(n)%init_tskin       ! Noah-MP.4.0.1 surface skin temperature:
             ...
             tmnxy(1,1) = NoahmpNew_struc(n)%noahmpnew(t)%tbot ! Noah-MP.4.0.1 soil temperature lower boundary:
! Fix later (should it be zero?)
!             XICE(1,1) = NoahmpNew_struc(n)%seaice   ! Noah-MP.4.0.1 sea ice fraction:
             XICE(1,1) = 0.0
             eahxy(1,1) = 0.0
             tahxy(1,1) = 0.0
             cmxy(1,1) = 0.0   
             ...
             wslakexy(1,1) = 0.0 
             zwtxy(1,1) = NoahmpNew_struc(n)%init_zwt         ! Noah-MP.4.0.1 initial water table depth:
             waxy(1,1) = NoahmpNew_struc(n)%init_wa           ! Noah-MP.4.0.1 initial water in the aquifer:
             wtxy(1,1) = NoahmpNew_struc(n)%init_wt           ! Noah-MP.4.0.1 initial water in aquifer and saturated soil:
             ...
             laixy(1,1) = NoahmpNew_struc(n)%init_lai         ! Noah-MP.4.0.1 initial leaf area index:
             grainxy(1,1) = 0.0
             gddxy(1,1) = 0.0
             cropcatxy(1,1) = 0.0
             ...
             runoff_option = NoahmpNew_struc(n)%run_opt       ! Noah-MP.4.0.1 runoff and groundwater option:
             crop_option = NoahmpNew_struc(n)%crop_opt        ! Noah-MP.4.0.1 crop model option:
             sf_urban_physics = NoahmpNew_struc(n)%urban_opt  ! Noah-MP.4.0.1 urban physics option:
...
! Fix terrain height later for crop model (Get from lis_input file)
            ...
            CALL NOAHMP_INIT(    LLANDUSE, tmp_swe, tmp_snodep, canwatxy,   ISLTYP,   IVGTYP, XLAT, & 
                ...
                NoahmpNew_struc(n)%noahmpnew(t)%sfcrunoff = sfcrunoffxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%udrrunoff = udrrunoffxy(1,1)

                do l=1, NoahmpNew_struc(n)%nsoil
                    NoahmpNew_struc(n)%noahmpnew(t)%smc(l) = smois_3d(1,l,1) 
                enddo
                do l=1, NoahmpNew_struc(n)%nsoil
                    NoahmpNew_struc(n)%noahmpnew(t)%sh2o(l) = sh2o_3d(1,l,1)
                enddo
                do l=1, NoahmpNew_struc(n)%nsoil
                    NoahmpNew_struc(n)%noahmpnew(t)%tslb(l) = tslb_3d(1,l,1)
                enddo
                NoahmpNew_struc(n)%noahmpnew(t)%sneqv = tmp_swe(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%snowh = tmp_snodep(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%canwat = canwatxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%acsnom = acsnomxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%acsnow = acsnowxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%isnow = isnowxy(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%tv = tvxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%tg = tgxy(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%canice = canicexy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%canliq = canliqxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%fwet = fwetxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%sneqvo = sneqvoxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%albold = alboldxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%qsnow = qsnowxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%wslake = wslakexy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%zwt = zwtxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%wa = waxy(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%wt = wtxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%lfmass = lfmassxy(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%rtmass = rtmassxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%stmass = stmassxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%wood = woodxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%stblcp = stblcpxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%fastcp = fastcpxy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%lai = laixy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%sai = saixy(1,1)
                NoahmpNew_struc(n)%noahmpnew(t)%tauss = taussxy(1,1)

                do l=1, NoahmpNew_struc(n)%nsoil
                    NoahmpNew_struc(n)%noahmpnew(t)%smoiseq(l) = smoiseqxy(1,l,1) 
                enddo
                NoahmpNew_struc(n)%noahmpnew(t)%smcwtd = smcwtdxy(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%deeprech = deeprechxy(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%rech = rechxy(1,1) 
                NoahmpNew_struc(n)%noahmpnew(t)%grain = grainxy(1,1) 
               ...
               NoahmpNew_struc(n)%noahmpnew(t)%isnow = isnowxy(1,1) 
               NoahmpNew_struc(n)%noahmpnew(t)%tsno(1:NoahmpNew_struc(n)%nsnow) = tsnoxy(1,-NoahmpNew_struc(n)%nsnow+1:0,1)
!-----------------------------------------------------------------------
            enddo   ! t=1,1
