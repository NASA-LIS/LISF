...
!BOP
! !ROUTINE: noahmpnew_getsws_hymap2
!  \label{noahmpnew_getsws_hymap2}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_getsws_hymap2(n)
! !USES:
  use ESMF
...
!EOP
  ...
  if(enable2waycpl==1) then 
     ...
     where(rivstotmp/=LIS_rc%udef) &
          NoahMPnew_struc(n)%noahmpnew(:)%rivsto=rivstotmp/NoahMPnew_struc(n)%ts
     ...
     where(fldstotmp/=LIS_rc%udef)&
          NoahMPnew_struc(n)%noahmpnew(:)%fldsto=fldstotmp/NoahMPnew_struc(n)%ts
     ...
     NoahMPnew_struc(n)%noahmpnew(:)%fldfrc=fldfrctmp
  endif  
end subroutine noahmpnew_getsws_hymap2
