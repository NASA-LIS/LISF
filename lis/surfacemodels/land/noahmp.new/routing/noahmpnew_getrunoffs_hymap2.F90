...
!BOP
! !ROUTINE: noahmpnew_getrunoffs_hymap2
!  \label{noahmpnew_getrunoffs_hymap2}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_getrunoffs_hymap2(n)
! !USES:
  use ESMF
...
!if option is not defined, then assume that no evap calculations will be done
  ...
  if(evapflag.ne.0)then
    ...
    call LIS_verify(status, "noahmpnew_getrunoffs_hymap2: ESMF_StateGet failed for Total Evapotranspiration")
        
    call ESMF_FieldGet(evapotranspiration_Field,localDE=0,&
         farrayPtr=evapotranspiration,rc=status)
    call LIS_verify(status, "noahmpnew_getrunoffs_hymap2: ESMF_FieldGet failed for Total Evapotranspiration")
    
    do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)  
       evapotranspiration1(t)  =  &
            NoahMPnew_struc(n)%noahmpnew(t)%ecan + &
            NoahMPnew_struc(n)%noahmpnew(t)%etran + &
            NoahMPnew_struc(n)%noahmpnew(t)%edir 
    enddo
...
end subroutine noahmpnew_getrunoffs_hymap2
