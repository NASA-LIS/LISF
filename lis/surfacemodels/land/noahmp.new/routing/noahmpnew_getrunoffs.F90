...
!BOP
! !ROUTINE: noahmpnew_getrunoffs
!  \label{noahmpnew_getrunoffs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_getrunoffs(n)
! !USES:
  use ESMF
...
!if option is not defined, then assume that no evap calculations will be done
  ...
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     runoff1(t) = NoahMPnew_struc(n)%noahmpnew(t)%runsf/LIS_CONST_RHOFW
     runoff2(t) = NoahMPnew_struc(n)%noahmpnew(t)%runsb/LIS_CONST_RHOFW
  enddo
...
!gather the model tiles before assigning to the global data structure.
  ...
  if(evapflag.ne.0)then
    ...
    if(LIS_masterproc) then
      ...
      call LIS_verify(status, "noahmpnew_getrunoffs: ESMF_StateGet failed for Total Evapotranspiration")

      call ESMF_FieldGet(evapotranspiration_Field,localDE=0,farrayPtr=evapotranspiration,rc=status)
      call LIS_verify(status, "noahmpnew_getrunoffs: ESMF_FieldGet failed for Total Evapotranspiration")

    endif

    do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
       evapotranspiration1(t)  =  NoahMPnew_struc(n)%noahmpnew(t)%ecan + NoahMPnew_struc(n)%noahmpnew(t)%etran + NoahMPnew_struc(n)%noahmpnew(t)%edir
    enddo
...
end subroutine noahmpnew_getrunoffs
