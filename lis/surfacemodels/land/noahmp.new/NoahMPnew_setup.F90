...
!BOP
!
! !ROUTINE: NoahMPnew_setup
! \label{NoahMPnew_setup}
!
! !REVISION HISTORY:
!  This subroutine is generated with the Model Implementation Toolkit developed
!  by Shugong Wang for the NASA Land Information System Version 7. The initial 
!  specification of the subroutine is defined by Sujay Kumar. 
!  10/25/18: Shugong Wang, Zhuo Wang; initial implementation for LIS 7 and NoahMPnew
!  05/01/23: Cenlin He, update to work with refactored Noah-MP (v5.0 and later)
! !INTERFACE:

subroutine NoahMPnew_setup()
...
!  required for NoahMPnew.  These include: 
...
!
! !INTERFACE:
subroutine NOAHMPnew_read_MULTILEVEL_param(n, ncvar_name, level, placeholder)
! !USES:
    use netcdf
...
!EOP      
    ...
    if(file_exists) then
        ...
        call LIS_verify(ios, 'Error in nf90_open in NOAHMPnew_read_MULTILEVEL_param')

        ! inquire the ID of east-west dimension
        ios = nf90_inq_dimid(nid, 'east_west', nc_ID)
        call LIS_verify(ios, 'Error in nf90_inq_dimid in NOAHMPnew_read_MULTILEVEL_param')
        ...
        call LIS_verify(ios, 'Error in nf90_inquire_dimension in NOAHMPnew_read_MULTILEVEL_param')
        ...
        call LIS_verify(ios, 'Error in nf90_get_var in NOAHMPnew_read_MULTILEVEL_param')

        ! close netcdf file 
        ios = nf90_close(nid)
        call LIS_verify(ios, 'Error in nf90_close in NOAHMPnew_read_MULTILEVEL_param')
