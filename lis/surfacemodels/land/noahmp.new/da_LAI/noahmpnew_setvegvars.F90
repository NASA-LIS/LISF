...
!BOP
! !ROUTINE: noahmpnew_setvegvars
!  \label{noahmpnew_setvegvars}
!
! !REVISION HISTORY:
...
! !INTERFACE:
subroutine noahmpnew_setvegvars(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(NoahMPnew_struc(n)%noahmpnew(t)%param%sla.ne.0) then 
        NoahMPnew_struc(n)%noahmpnew(t)%lai = lai(t)
        lfmass = lai(t)*1000.0/(NoahMPnew_struc(n)%noahmpnew(t)%param%sla)
        NoahMPnew_struc(n)%noahmpnew(t)%lfmass = lfmass
     endif
  enddo

end subroutine noahmpnew_setvegvars
