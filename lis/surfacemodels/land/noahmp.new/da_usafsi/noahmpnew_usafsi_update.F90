...
!
! !INTERFACE
subroutine noahmpnew_usafsi_update(n, t, dsneqv, dsnowh, NoahmpIO, noahmp)
...
! local
  ...
  do isoil = 1, size(soiltype)
    BEXP(isoil)   = NoahmpIO%BEXP_TABLE   (SOILTYPE(isoil))
    PSISAT(isoil) = NoahmpIO%PSISAT_TABLE (SOILTYPE(isoil))
    SMCMAX(isoil) = NoahmpIO%SMCMAX_TABLE (SOILTYPE(isoil))
  end do
...
! TODO: may need a variable mapping from local to noahmp or direct associate noahmp type to local variables
! TODO: or directly copy paste the original compact, combine, divide subroutine to this da folder.
  if(isnow < 0) call SnowpackCompaction(noahmp)
!       call  compact (parameters, nsnow, nsoil, noahmpnew_struc(n)%ts,     & !in
...
!                   isnow, stc, snice, snliq, dzsnso) !inout
  ...
  NoahmpNew_struc(n)%noahmpnew(t)%zss(1:nsnow+&
