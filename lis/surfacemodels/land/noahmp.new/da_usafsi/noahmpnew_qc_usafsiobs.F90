...
!BOP
! !ROUTINE: noahmpnew_qc_usafsiobs
! \label{noahmpnew_qc_usafsiobs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_qc_usafsiobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in noahmpnew_qc_usafsiobs")
  call ESMF_FieldGet(obs_snow_field,localDE=0,farrayPtr=snowobs,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet failed in noahmpnew_qc_usafsiobs")
...
end subroutine noahmpnew_qc_usafsiobs
