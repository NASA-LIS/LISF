...
!BOP
! !ROUTINE: noahmpnew_map_snodep
! \label{noahmpnew_map_snodep}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_map_snodep(n,k,OBS_State,LSM_Incr_State)
! !USES:
  use ESMF
...
!EOP
  ...
  real, allocatable            :: noahmpnew_swe(:)
  real, allocatable            :: noahmpnew_snod(:)
  real, allocatable            :: snod(:)

  allocate(noahmpnew_swe(LIS_rc%npatch(n,LIS_rc%lsm_index)))
  allocate(noahmpnew_snod(LIS_rc%npatch(n,LIS_rc%lsm_index)))
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     noahmpnew_swe(t)  = NoahMPnew_struc(n)%noahmpnew(t)%sneqv
     noahmpnew_snod(t) = NoahMPnew_struc(n)%noahmpnew(t)%snowh
  enddo
...
! working with an model grid finer than the observation grid
     ...
     if(snodepobs(st_id).ge.0) then 
        if(noahmpnew_snod(t).gt.1e-6) then 
           tmpsneqv = noahmpnew_swe(t)/noahmpnew_snod(t)
        else
           tmpsneqv = 0.0
        endif
...
! Based on SNODEP, we manually update SWE
        ...
        sweincr(t) = tmpsneqv*snod(t) - noahmpnew_swe(t)
        snodincr(t) = snod(t) - noahmpnew_snod(t)
     else
        sweincr(t) = 0 
        snodincr(t) = 0 
     endif
...
!  stop
  ...
  deallocate(noahmpnew_swe)
  deallocate(noahmpnew_snod)
  deallocate(snod)

end subroutine noahmpnew_map_snodep
