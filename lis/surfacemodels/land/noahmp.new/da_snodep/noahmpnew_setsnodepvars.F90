...
!BOP
! !ROUTINE: noahmpnew_setsnodepvars
! \label{noahmpnew_setsnodepvars}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_setsnodepvars(n, LSM_State)
! !USES:
  use ESMF
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     ! update
     call noahmpnew_snodep_update(n, t, dsneqv, dsnowh)
  ...
  if (LIS_rc%snowsrc(n) .gt. 0) then
     ...
     do t = 1, LIS_rc%npatch(n, LIS_rc%lsm_index)
        ...
        LIS_snow_struc(n)%snowdepth(gid) = &
             LIS_snow_struc(n)%snowdepth(gid) + &
             NoahMPnew_struc(n)%noahmpnew(t)%snowh
...
end subroutine noahmpnew_setsnodepvars
