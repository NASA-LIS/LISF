...
!BOP
! !ROUTINE: noahmpnew_qc_snodepobs
! \label{noahmpnew_qc_snodepobs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_qc_snodepobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in noahmpnew_qc_snodepobs")
  call ESMF_FieldGet(obs_snow_field,localDE=0,farrayPtr=snowobs,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet failed in noahmpnew_qc_snodepobs")
  
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     !stc1(t) = NoahMPnew_struc(n)%noahmpnew(t)%sstc(1) ! get snow/veg temp.
     stc1(t) = NoahMPnew_struc(n)%noahmpnew(t)%tslb(1) ! get snow/veg temp.
     vegt(t) = LIS_surface(n,1)%tile(t)%vegt
  enddo

  call LIS_convertPatchSpaceToObsSpace(n,k,&       
       LIS_rc%lsm_index, NoahMPnew_struc(n)%noahmpnew(:)%tv,tv_obs) !tv: vegetation temperature. unit: K 
  call LIS_convertPatchSpaceToObsSpace(n,k,LIS_rc%lsm_index, &    !fveg: green vegetation fraction. unit: - 
       NoahMPnew_struc(n)%noahmpnew(:)%fveg,fveg_obs)
...
end subroutine noahmpnew_qc_snodepobs
