...
!BOP
! !ROUTINE: noahmpnew_getsnodeppred
! \label{noahmpnew_getsnodeppred}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_getsnodeppred(n, k, obs_pred)
...
!EOP
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if (SNODEP_obs_obj(n)%mesh .eq. 8) then
        snwd(t) = NoahMPnew_struc(n)%noahmpnew(t)%snowh*39.37 !convert from meter to inch
...
end subroutine noahmpnew_getsnodeppred
