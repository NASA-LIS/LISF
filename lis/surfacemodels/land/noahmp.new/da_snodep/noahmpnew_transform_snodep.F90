...
!BOP
! !ROUTINE: noahmpnew_transform_snodep
! \label{noahmpnew_transform_snodep}
!
! !REVISION HISTORY:
...
!EOP
  ...
  call LIS_verify(status, 'attributeget error in noahmpnew_transform_snodep')
  call ESMF_StateGet(OBS_State,"Observation01",obs_snodep_field,&
       rc=status)
  call LIS_verify(status,'stateget error in noahmpnew_transform_snodep')
  call ESMF_FieldGet(obs_snodep_field,localDE=0,farrayPtr=snodepobs,rc=status)
  call LIS_verify(status,'fieldget error in noahmpnew_transform_snodep')
...
end subroutine noahmpnew_transform_snodep
