...
!BOP
! !ROUTINE: noahmpnew_settws
!  \label{noahmpnew_settws}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmpnew_settws(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
!  real, parameter        :: MIN_THRESHOLD = 0.02 
  ...
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 1 failed in noahmpnew_settws")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 2 failed in noahmpnew_settws")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 3 failed in noahmpnew_settws")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 4 failed in noahmpnew_settws")
  call ESMF_StateGet(LSM_State,"Groundwater Storage",gwField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Groundwater Storage failed in noahmpnew_settws")
  call ESMF_StateGet(LSM_State,"SWE",sweField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: SWE failed in noahmpnew_settws")
  call ESMF_StateGet(LSM_State,"Snowdepth",snodField,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Snowdepth failed in noahmpnew_settws")


  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in noahmpnew_settws")
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in noahmpnew_settws")
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in noahmpnew_settws")
  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 4 failed in noahmpnew_settws")
  call ESMF_FieldGet(gwField,localDE=0,farrayPtr=gws,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Groundwater Storage failed in noahmpnew_settws")
  call ESMF_FieldGet(sweField,localDE=0,farrayPtr=swe,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: SWE failed in noahmpnew_settws")
  call ESMF_FieldGet(snodField,localDE=0,farrayPtr=snod,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Snowdepth failed in noahmpnew_settws")
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     if(NoahMPnew_struc(n)%noahmpnew(t)%smc(1).gt.MAX_THRESHOLD.or.&
        NoahMPnew_struc(n)%noahmpnew(t)%smc(1).gt.0.50) then 
        largeSM(i) = .true.
     endif
  enddo
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     i = LIS_domain(n)%gindex(c,r)
     if(largeSM(i)) then 
        soilm1(t) = NoahMPnew_struc(n)%noahmpnew(t)%smc(1)
        ...
        gws(t)    = NoahMPnew_struc(n)%noahmpnew(t)%wa
        snod(t)   = NoahMPnew_struc(n)%noahmpnew(t)%snowh 
        swe(t)    = NoahMPnew_struc(n)%noahmpnew(t)%sneqv
     endif
  enddo
  ...
  do i=1,LIS_rc%ngrid(n)
     ...
     if(.not.ensCheck(i).and.diffCheck(i).and.(.not.largeSM(i))) then
        call noahmpnew_tws_reorderEnsForOutliers(i,&
             LIS_rc%nensem(n),&
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     delta = soilm1(t) - NoahMPnew_struc(n)%noahmpnew(t)%smc(1)    
     NoahMPnew_struc(n)%noahmpnew(t)%smc(1) = soilm1(t)
     NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1) = &
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1) + delta

     delta = soilm2(t) - NoahMPnew_struc(n)%noahmpnew(t)%smc(2)    
     NoahMPnew_struc(n)%noahmpnew(t)%smc(2) = soilm2(t)
     NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2) = &
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2) + delta

     delta = soilm3(t) - NoahMPnew_struc(n)%noahmpnew(t)%smc(3)    
     NoahMPnew_struc(n)%noahmpnew(t)%smc(3) = soilm3(t)
     NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3) = &
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3) + delta

     delta = soilm4(t) - NoahMPnew_struc(n)%noahmpnew(t)%smc(4)    
     NoahMPnew_struc(n)%noahmpnew(t)%smc(4) = soilm4(t)
     NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4) = &
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4) + delta
     
     NoahMPnew_struc(n)%noahmpnew(t)%wa=gws(t)
  enddo
  
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     swe_old = NoahMPnew_struc(n)%noahmpnew(t)%sneqv
     snowh_old = NoahMPnew_struc(n)%noahmpnew(t)%snowh

     dsneqv = swe(t) - NoahMPnew_struc(n)%noahmpnew(t)%sneqv !in mm
     dsnowh = snod(t) - NoahMPnew_struc(n)%noahmpnew(t)%snowh  !in m

     !alternate option
     call noahmpnew_snow_update(n, t, dsneqv, dsnowh)

     if(NoahMPnew_struc(n)%noahmpnew(t)%sneqv.eq.0.or.&
        NoahMPnew_struc(n)%noahmpnew(t)%snowh.eq.0) then
        NoahMPnew_struc(n)%noahmpnew(t)%sneqv = 0
        NoahMPnew_struc(n)%noahmpnew(t)%snowh = 0
     endif
  enddo

end subroutine noahmpnew_settws


subroutine noahmpnew_tws_reorderEnsForOutliers(i,nensem, statevec, &
...
end subroutine noahmpnew_tws_reorderEnsForOutliers
