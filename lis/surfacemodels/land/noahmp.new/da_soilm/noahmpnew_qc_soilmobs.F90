...
!BOP
! !ROUTINE: NoahMPnew_qc_soilmobs
! \label{NoahMPnew_qc_soilmobs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine NoahMPnew_qc_soilmobs(n,k,OBS_State)
! !USES:
  use ESMF
  use LIS_coreMod
...
! mn
  ...
  call LIS_verify(status,&
       "ESMF_StateGet failed in NoahMPnew_qc_soilmobs")
  call ESMF_FieldGet(obs_sm_field,localDE=0,farrayPtr=smobs,rc=status)
  call LIS_verify(status,& 
       "ESMF_FieldGet failed in NoahMPnew_qc_soilmobs")
  
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)
     smc1(t) = NoahMPnew_struc(n)%noahmpnew(t)%smc(1)
     smc2(t) = NoahMPnew_struc(n)%noahmpnew(t)%smc(2)
     smc3(t) = NoahMPnew_struc(n)%noahmpnew(t)%smc(3)
     smc4(t) = NoahMPnew_struc(n)%noahmpnew(t)%smc(4)

     sh2o1(t) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1)
     sh2o2(t) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2)
     sh2o3(t) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3)
     sh2o4(t) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4)
!--------------------------------------------------------------------------------------------------------- 
...
!---------------------------------------------------------------------------------------------------------
 !    stc1(t) = NoahMPnew_struc(n)%noahmpnew(t)%sstc(NoahMPnew_struc(n)%nsnow+1)
 !    stc2(t) = NoahMPnew_struc(n)%noahmpnew(t)%sstc(NoahMPnew_struc(n)%nsnow+2)
 !    stc3(t) = NoahMPnew_struc(n)%noahmpnew(t)%sstc(NoahMPnew_struc(n)%nsnow+3)
 !    stc4(t) = NoahMPnew_struc(n)%noahmpnew(t)%sstc(NoahMPnew_struc(n)%nsnow+4)

     stc1(t) = NoahMPnew_struc(n)%noahmpnew(t)%tslb(1)
     stc2(t) = NoahMPnew_struc(n)%noahmpnew(t)%tslb(2)
     stc3(t) = NoahMPnew_struc(n)%noahmpnew(t)%tslb(3)
     stc4(t) = NoahMPnew_struc(n)%noahmpnew(t)%tslb(4)

     vegt(t) = NoahMPnew_struc(n)%noahmpnew(t)%vegetype

     !SOILTYP = NoahMPnew_struc(n)%noahmpnew(t)%soiltype        
     !SMCMAX(t)  =  NoahMPnew_struc(n)%noahmpnew(t)%param%SMCMAX(1)  !  SMCMAX(t)  = MAXSMC (SOILTYP) 
     !SMCWLT(t)  =  NoahMPnew_struc(n)%noahmpnew(t)%param%SMCWLT(1)   ! SMCWLT(t) = WLTSMC (SOILTYP)
     SOILTYP   = NoahMPnew_struc(n)%noahmpnew(t)%soiltype
     SMCMAX(t) = NoahMPnew_struc(n)%noahmpnew(t)%param%SMCMAX(1) !SMCMAX_TABLE(SOILTYP)
     SMCWLT(t) = NoahMPnew_struc(n)%noahmpnew(t)%param%SMCWLT(1) !SMCWLT_TABLE(SOILTYP)
  enddo

  call LIS_convertPatchSpaceToObsSpace(n,k,&       
       LIS_rc%lsm_index, &
       NoahMPnew_struc(n)%noahmpnew(:)%prcp,&
       rainf_obs)  ! MN prcp is total precip 
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       NoahMPnew_struc(n)%noahmpnew(:)%sneqv,&
       sneqv_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       NoahMPnew_struc(n)%noahmpnew(:)%snowc,&   ! MP36 fsno
       sca_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       NoahMPnew_struc(n)%noahmpnew(:)%fveg,&
       shdfac_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       NoahMPnew_struc(n)%noahmpnew(:)%tg,&
       t1_obs)
  call LIS_convertPatchSpaceToObsSpace(n,k,&
       LIS_rc%lsm_index, &
       smcmax, &
...
end subroutine NoahMPnew_qc_soilmobs
