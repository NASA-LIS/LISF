...
!BOP
! !ROUTINE: NoahMPnew_setsoilm
!  \label{NoahMPnew_setsoilm}
!
! !REVISION HISTORY:
! 27Feb2005: Sujay Kumar; Initial Specification
! 25Jun2006: Sujay Kumar: Updated for the ESMF design
! 15 Dec 2018: Mahdi Navari: Modified for NoahMPnew 
...
! !INTERFACE:
subroutine NoahMPnew_setsoilm(n, LSM_State)
! !USES:
  use ESMF
  use LIS_coreMod
  use LIS_logMod
...
! mn
  ...
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 1 failed in NoahMPnew_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 2",sm2Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 2 failed in NoahMPnew_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 3",sm3Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 3 failed in NoahMPnew_setsoilm")
  call ESMF_StateGet(LSM_State,"Soil Moisture Layer 4",sm4Field,rc=status)
  call LIS_verify(status,&
       "ESMF_StateSet: Soil Moisture Layer 4 failed in NoahMPnew_setsoilm")

  call ESMF_FieldGet(sm1Field,localDE=0,farrayPtr=soilm1,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 1 failed in NoahMPnew_setsoilm")
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 2 failed in NoahMPnew_setsoilm")
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 3 failed in NoahMPnew_setsoilm")
  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet: Soil Moisture Layer 4 failed in NoahMPnew_setsoilm")
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     MAX_THRESHOLD = NoahMPnew_struc(n)%noahmpnew(t)%param%SMCMAX(1)   ! MAXSMC (SOILTYP)
     sm_threshold  = NoahMPnew_struc(n)%noahmpnew(t)%param%SMCMAX(1) - 0.02  ! MAXSMC (SOILTYP) - 0.02
     ...
     !MN: delta = X(+) - X(-)
     !NOTE: "noahmpnew_updatesoilm.F90" updates the soilm_(t)     
     delta1 = soilm1(t)-NoahMPnew_struc(n)%noahmpnew(t)%smc(1)
     delta2 = soilm2(t)-NoahMPnew_struc(n)%noahmpnew(t)%smc(2)
     delta3 = soilm3(t)-NoahMPnew_struc(n)%noahmpnew(t)%smc(3)
     delta4 = soilm4(t)-NoahMPnew_struc(n)%noahmpnew(t)%smc(4)

     ! MN: check    MIN_THRESHOLD < volumetric liquid soil moisture < threshold 
     if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1)+delta1.gt.MIN_THRESHOLD .and.&
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1)+delta1.lt.&
          sm_threshold) then 
     ...
     if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2)+delta2.gt.MIN_THRESHOLD .and.&
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2)+delta2.lt.sm_threshold) then 
        update_flag(gid) = update_flag(gid).and.(.true.)
        update_flag_tile(t) = update_flag_tile(t).and.(.true.)
     else
        update_flag(gid) = update_flag(gid).and.(.false.)
     ...
     if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3)+delta3.gt.MIN_THRESHOLD .and.&
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3)+delta3.lt.sm_threshold) then 
        update_flag(gid) = update_flag(gid).and.(.true.)
        update_flag_tile(t) = update_flag_tile(t).and.(.true.)
     else
        update_flag(gid) = update_flag(gid).and.(.false.)
     ...
     if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4)+delta4.gt.MIN_THRESHOLD .and.&
          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4)+delta4.lt.sm_threshold) then 
        update_flag(gid) = update_flag(gid).and.(.true.)
        update_flag_tile(t) = update_flag_tile(t).and.(.true.)
     else
        update_flag(gid) = update_flag(gid).and.(.false.)
	  ...
	  if(update_flag_tile(t)) then
        ...
        do m=1,LIS_rc%nensem(n)
           ...
           if(update_flag_tile(t)) then
              ...
              NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1)+&
                   delta1
              NoahMPnew_struc(n)%noahmpnew(t)%smc(1) = soilm1(t)
              ...
              if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2)+delta2.gt.MIN_THRESHOLD .and.&
                   NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2)+delta2.lt.sm_threshold) then 
                 NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2)+&
                      soilm2(t)-NoahMPnew_struc(n)%noahmpnew(t)%smc(2)
                 NoahMPnew_struc(n)%noahmpnew(t)%smc(2) = soilm2(t)
              ...
              if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3)+delta3.gt.MIN_THRESHOLD .and.&
                   NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3)+delta3.lt.sm_threshold) then 
                 NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3)+&
                      soilm3(t)-NoahMPnew_struc(n)%noahmpnew(t)%smc(3)
                 NoahMPnew_struc(n)%noahmpnew(t)%smc(3) = soilm3(t)
              ...
              if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4)+delta4.gt.MIN_THRESHOLD .and.&
                   NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4)+delta4.lt.sm_threshold) then 
                 NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4) = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4)+&
                      soilm4(t)-NoahMPnew_struc(n)%noahmpnew(t)%smc(4)
                 NoahMPnew_struc(n)%noahmpnew(t)%smc(4) = soilm4(t)
...
!-----------------------------------------------------------------------------------------
              ...
              NoahMPnew_struc(n)%noahmpnew(t)%sh2o(1) = smc_tmp 
              NoahMPnew_struc(n)%noahmpnew(t)%smc(1) = smc_tmp
                          
              smc_tmp = (MaxEnsSM2 - MinEnsSM2)/2 + MinEnsSM2            
              NoahMPnew_struc(n)%noahmpnew(t)%sh2o(2) = smc_tmp
              NoahMPnew_struc(n)%noahmpnew(t)%smc(2) = smc_tmp
               
              smc_tmp = (MaxEnsSM3 - MinEnsSM3)/2 + MinEnsSM3
              NoahMPnew_struc(n)%noahmpnew(t)%sh2o(3) = smc_tmp
              NoahMPnew_struc(n)%noahmpnew(t)%smc(3) = smc_tmp
              
              smc_tmp = (MaxEnsSM4 - MinEnsSM4)/2 + MinEnsSM4
              NoahMPnew_struc(n)%noahmpnew(t)%sh2o(4) = smc_tmp
              NoahMPnew_struc(n)%noahmpnew(t)%smc(4) = smc_tmp
	      ...
	      t_unpert = i+LIS_rc%nensem(n)-1
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(m.ne.LIS_rc%nensem(n)) then 
                       delta(j) = delta(j) + &
                            ...
                            NoahMPnew_struc(n)%noahmpnew(t_unpert)%sh2o(j))
                    endif
              ...
              do j=1,4
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    !t = (i-1)*LIS_rc%nensem(n)+m
                    SOILTYP       = NoahMPnew_struc(n)%noahmpnew(t)%soiltype  
                    MAX_THRESHOLD = NoahMPnew_struc(n)%noahmpnew(t)%param%SMCMAX(1)        !SMCMAX_TABLE(SOILTYP) 
                    sm_threshold  = NoahMPnew_struc(n)%noahmpnew(t)%param%SMCMAX(1) - 0.02 !SMCMAX_TABLE(SOILTYP) - 0.02
                    
                    tmpval = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) - delta(j)
                    if(tmpval.le.MIN_THRESHOLD) then 
                       NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) = &
                            max(NoahMPnew_struc(n)%noahmpnew(t_unpert)%sh2o(j),&
                            MIN_THRESHOLD)
                       NoahMPnew_struc(n)%noahmpnew(t)%smc(j) = &
                            max(NoahMPnew_struc(n)%noahmpnew(t_unpert)%smc(j),&
                            MIN_THRESHOLD)
                       ens_flag(m) = .false. 
                    elseif(tmpval.ge.sm_threshold) then
                       NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) = &
                            min(NoahMPnew_struc(n)%noahmpnew(t_unpert)%sh2o(j),&
                            sm_threshold)
                       NoahMPnew_struc(n)%noahmpnew(t)%smc(j) = &
                            min(NoahMPnew_struc(n)%noahmpnew(t_unpert)%smc(j),&
                            sm_threshold)
                       ens_flag(m) = .false. 
                    endif
                 enddo
              ...
              do j=1,4
                 ...
                 do m=1,LIS_rc%nensem(n)-1
                    ...
                    if(ens_flag(m)) then 
                       tmpval = NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) - &
                            delta(j)
                       SOILTYP = NoahMPnew_struc(n)%noahmpnew(t)%soiltype  
                       MAX_THRESHOLD = NoahMPnew_struc(n)%noahmpnew(t)%param%SMCMAX(1) !SMCMAX_TABLE(SOILTYP)  
                       ...
                       if(.not.(tmpval.le.0.0 .or.&
                          ...
                          NoahMPnew_struc(n)%noahmpnew(t)%smc(j) = &
                               NoahMPnew_struc(n)%noahmpnew(t)%smc(j) - delta(j)
                          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) = &
                               NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) - delta(j)
              ...
              if(nIter.gt.10.and.bounds_violation) then 
                 ...
                 do j=1,4
                    do m=1,LIS_rc%nensem(n)
                       ...
                       if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j).gt.MAX_THRESHOLD.or.&
                            NoahMPnew_struc(n)%noahmpnew(t)%smc(j).gt.MAX_THRESHOLD) then                        
                          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) = MAX_THRESHOLD
                          NoahMPnew_struc(n)%noahmpnew(t)%smc(j) = MAX_THRESHOLD
                       endif
                       
                       if(NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j).lt.MIN_THRESHOLD.or.&
                            NoahMPnew_struc(n)%noahmpnew(t)%smc(j).lt.MIN_THRESHOLD) then                        
                          NoahMPnew_struc(n)%noahmpnew(t)%sh2o(j) = MIN_THRESHOLD
                          NoahMPnew_struc(n)%noahmpnew(t)%smc(j) = MIN_THRESHOLD
                       endif
!                    print*, i, m
!                    print*, 'smc',t, NoahMPnew_struc(n)%noahmpnew(t)%smc(:)
!                    print*, 'sh2o ',t,NoahMPnew_struc(n)%noahmpnew(t)%sh2o(:)
!                    print*, 'max ',t,MAX_THRESHOLD !NoahMPnew_struc(n)%noahmpnew(t)%smcmax
                    enddo
!                  call LIS_endrun()
                 enddo
              endif          
...
end subroutine NoahMPnew_setsoilm
