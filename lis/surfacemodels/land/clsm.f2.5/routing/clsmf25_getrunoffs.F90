...
!BOP
! !ROUTINE: clsmf25_getrunoffs
!  \label{clsmf25_getrunoffs}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine clsmf25_getrunoffs(n)
! !USES:
  use ESMF
  use LIS_coreMod
...
!if option is not defined, then assume that no evap calculations will be done
  ...
  do t=1, LIS_rc%npatch(n,LIS_rc%lsm_index)  
     runoff1(t) = clsmf25_struc(n)%cat_diagn(t)%runsrf/LIS_CONST_RHOFW
     runoff2(t) = clsmf25_struc(n)%cat_diagn(t)%bflow/LIS_CONST_RHOFW
  enddo
...
end subroutine clsmf25_getrunoffs
