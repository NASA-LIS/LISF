...
MODULE NESDIS_LandEM_Module
  ...
  USE CRTM_Surface_Define   ! YDT added 
...
!------------------------------------------------------------------------------------------------------------
  ...
  SUBROUTINE NESDIS_LandEM2(Angle,                 &   ! Input
                           Frequency,             &   ! Input
                           Surface,               &   ! Input
                           Emissivity_H,          &   ! Output
                           Emissivity_V)              ! Output
    ...
    ! Local variables
    REAL(fp) :: Soil_Moisture_Content
    REAL(fp) :: Canopy_water_Content
    REAL(fp) :: Vegetation_Fraction
    REAL(fp) :: Soil_Temperature
    REAL(fp) :: t_skin
    REAL(fp) :: Snow_Depth
    REAL(fp) :: sand, clay 
    ! Parameters to be disturbed
    REAL(fp) :: water_content_per_lai
    REAL(fp) :: ssalb_factor 
    ...
    Soil_Moisture_Content = Surface%Soil_Moisture_Content
    Canopy_Water_Content  = Surface%Canopy_Water_content 
    Vegetation_Fraction   = Surface%Vegetation_Fraction
    lai                   = Surface%lai 
    Soil_Temperature      = Surface%Soil_Temperature
    t_skin                = Surface%Land_Temperature
    Snow_Depth            = Surface%Snow_Depth
    sand                  = Surface%sand 
    clay                  = Surface%clay 
    leaf_thick            = Surface%leaf_thick 
    sigma                 = Surface%sigma 
    water_content_per_lai = Surface%water_content_per_lai
    ssalb_factor          = Surface%ssalb_factor 
...
!YDT      lai = THREE*veg_frac + POINT5
!YDT      mge = POINT5*veg_frac
!      mge = lai / 6.0_fp 
      mge = lai * water_content_per_lai + Canopy_water_content + 0.1
!YDT      leaf_thick = 0.07_fp
      mu  = COS(theta)
!YDT      sigma = POINT5
      ...
      ! now perturb ssalb
      ssalb_v = ssalb_factor * ssalb_v
      ssalb_h = ssalb_factor * ssalb_h

      CALL Two_Stream_Solution(mu,gv,gh,ssalb_h,ssalb_v,tau_h,tau_v, &
                               r21_h,r21_v,r23_h,r23_v,t21_v,t21_h,Emissivity_V,Emissivity_H)
    END IF

  END SUBROUTINE NESDIS_LandEM2
...
! YDT
!  write(*, *) "mg=",  mg, "vf=", vf, "vb=", vb, "R(esv)=", real(esv), "I(esv)=", AIMAG(esv)      
