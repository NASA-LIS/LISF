...
!BOP
! !ROUTINE: GLS_param_reset
!  \label{GLS_param_reset}
!
! !REVISION HISTORY:
! 06 Feb 2008: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine GLS_param_reset()
! !USES:
  use ESMF
  use LIS_coreMod, only : LIS_rc
  use LIS_timeMgrMod, only : LIS_calendar, LIS_clock
  use LIS_logMod,  only : LIS_verify
  use GLSMod,      only : GLS_data, GLS_ctl
...
!need to reinitialize all alarms: 
  ...
  call ESMF_TimeIntervalSet(alarmInterval, h=GLS_ctl%acc_interval1, rc=rc)
  call LIS_verify(rc, 'Time interval set failed in GLS_init')
  ...
  call ESMF_TimeIntervalSet(alarmInterval, h=GLS_ctl%acc_interval2, rc=rc)
  call LIS_verify(rc, 'Time interval set failed in GLS_init')
  ...
  call ESMF_TimeIntervalSet(alarmInterval, h=GLS_ctl%acc_interval3, rc=rc)
  call LIS_verify(rc, 'Time interval set failed in GLS_init')
...
end subroutine GLS_param_reset
