...
! !USES:
  ...
  use LIS_coreMod, only : LIS_rc, LIS_domain, LIS_config, &
       ...
       LIS_ews_halo_ind, LIS_nss_halo_ind, LIS_ewe_halo_ind, &
       LIS_nse_halo_ind
...
!  real, allocatable :: param_dd(:,:)
  ...
  call ESMF_ConfigFindLabel(LIS_config,"Run domain true lat1:",rc=rc)
  do n=1,LIS_rc%nnest    
     call ESMF_ConfigGetAttribute(LIS_config,run_dd(n,3),rc=rc)
     call LIS_verify(rc,'Run domain true lat1: not defined')
  enddo
  call ESMF_ConfigFindLabel(LIS_config,"Run domain true lat2:",rc=rc)
  do n=1,LIS_rc%nnest    
     call ESMF_ConfigGetAttribute(LIS_config,run_dd(n,4),rc=rc)
     call LIS_verify(rc,'Run domain true lat2: not defined')
  enddo
  ...
  do n=1,LIS_rc%nnest    
     ...
     call LIS_verify(rc,'Run domain standard lon: not defined')
  enddo
...
!        endif
  ...
  write(LIS_logunit,*)'DOMAIN details: Running in lambert projection'
...
!call map_set on the overall running domain
     ...
     LIS_rc%gridDesc(n,7) = truelat2
     LIS_rc%gridDesc(n,8) = xmesh
     LIS_rc%gridDesc(n,9) = xmesh
     LIS_rc%gridDesc(n,10) = truelat1
...
!     enddo
     ...
     call map_set(PROJ_LC,LIS_rc%gridDesc(n,4),LIS_rc%gridDesc(n,5),&
          ...
          truelat2,LIS_rc%lnc(n),LIS_rc%lnr(n),LIS_domain(n)%lisproj)

!global domain
     call map_set(PROJ_LC, stlat, stlon, &
          LIS_rc%gridDesc(n,8)*1000.0,LIS_rc%gridDesc(n,11),&
          LIS_rc%gridDesc(n,10),&
          truelat2,LIS_rc%gnc(n),LIS_rc%gnr(n),LIS_domain(n)%lisglbproj)

!parameter data 
     call map_set(PROJ_LC, LIS_rc%gridDesc(n,34),LIS_rc%gridDesc(n,35),&
          LIS_rc%gridDesc(n,8)*1000.0,LIS_rc%gridDesc(n,11),&
          LIS_rc%gridDesc(n,10),&
          truelat2,LIS_rc%pnc(n),LIS_rc%pnr(n),LIS_domain(n)%lisparamproj)

! setting the extents to the 'global' domain so that the 
! obs space domain mapping doesn't encounter boundary issues

     LIS_rc%minLat(n) = LIS_rc%gridDesc(n,34) 
     LIS_rc%minLon(n) = LIS_rc%gridDesc(n,35) 
     LIS_rc%maxLat(n) = LIS_rc%gridDesc(n,37) 
     LIS_rc%maxLon(n) = LIS_rc%gridDesc(n,38) 
  enddo
  deallocate(run_dd)
  deallocate(gnc)
  deallocate(gnr)
