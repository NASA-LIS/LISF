...
#define METHOD "LIS_ESMF_FerretScriptWrite"
!BOP
! !IROUTINE: LIS_ESMF_FerretScriptWrite_preset - Write Ferret Script to file
! !INTERFACE:
  ! call using generic interface: LIS_ESMF_FerretScriptWrite
  subroutine LIS_ESMF_FerretScriptWrite_preset(varName, dataFile, &
...
!EOP
  !-----------------------------------------------------------------------------
    ...
    select case (trim(mapPreset))
      case ('global','GLOBAL','Global')
        call LIS_ESMF_FerretScriptWrite(varName, dataFile, gridFile, slices, &
          map=LIS_ESMF_MAPPRESET_GLOBAL, scale=scale, jnlFile=jnlFile, &
          uniformRect=uniformRect, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, PASSTHRU)) return  ! bail out
      case ('conus','CONUS','Conus')
        call LIS_ESMF_FerretScriptWrite(varName, dataFile, gridFile, slices, &
          map=LIS_ESMF_MAPPRESET_CONUS, scale=scale, jnlFile=jnlFile, &
          uniformRect=uniformRect, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, PASSTHRU)) return  ! bail out
      case ('irene','IRENE','Irene')
        call LIS_ESMF_FerretScriptWrite(varName, dataFile, gridFile, slices, &
          map=LIS_ESMF_MAPPRESET_IRENE, scale=scale, jnlFile=jnlFile, &
          uniformRect=uniformRect, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, PASSTHRU)) return  ! bail out
      case ('frontrange','FRONTRANGE','FrontRange')
        call LIS_ESMF_FerretScriptWrite(varName, dataFile, gridFile, slices, &
          map=LIS_ESMF_MAPPRESET_FRONTRANGE, scale=scale, jnlFile=jnlFile, &
          uniformRect=uniformRect, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, PASSTHRU)) return  ! bail out
      case default
        call ESMF_LogSetError(rcToCheck=ESMF_RC_ARG_VALUE,   &
...
#define METHOD "LIS_ESMF_FerretScriptWrite"
!BOP
! !IROUTINE: LIS_ESMF_FerretScriptWrite_default - Write Ferret Script to file
! !INTERFACE:
  ! call using generic interface: LIS_ESMF_FerretScriptWrite
  subroutine LIS_ESMF_FerretScriptWrite_default(varName, dataFile, gridFile, &
