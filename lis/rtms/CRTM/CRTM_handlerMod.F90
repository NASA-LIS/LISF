...
!    CRTM from within LIS. The subroutines provide mapping of the surface
!    and atmospheric profiles to CRTM, which then runs the forward 
!    model to generate the radiative quantities. 
!
! !REVISION HISTORY:
! 26 Mar 2009; Sujay Kumar, Initial Code
! 03 Nov 2009: Ken Harrison; Implemented geometry, land table/unit conversions. 
!
! !USES:        
...
!-----------------------------------------------------------------------------
  public :: CRTM_Kmatrix_initialize
  public :: CRTM_Kmatrix_f2t
  public :: CRTM_Kmatrix_geometry
  public :: CRTM_Kmatrix_run
  public :: CRTM_Kmatrix_output
...
!BOP
! 
! !ROUTINE: CRTM_Kmatrix_initialize
! \label{CRTM_Kmatrix_initialize}
! 
! !INTERFACE:
  subroutine CRTM_Kmatrix_initialize()
! !USES:
...
!  \begin{description}
!   \item[readCRTMcrd](\ref{readCRTMcrd}) \newline
...
!EOP
    ...
    do n=1,LIS_rc%nnest
       ...
       allocate(crtm_struc(n)%GeometryInfo(crtm_struc(n)%nprofiles))
...
!---------------------------------------------------------------------
       ...
       status = CRTM_Init(crtm_struc(n)%ChannelInfo, &
            Sensor_Id = (/crtm_struc(n)%Sensor_Id/), &
...
!---------------------------------------------------------------------
       ...
       allocate(crtm_struc(n)%Atm_K(n_Channels, n_Profiles))
       allocate(crtm_struc(n)%Sfc_K(n_Channels, n_Profiles))
       allocate(crtm_struc(n)%RTSolution_K(n_Channels, n_Profiles))

!---------------------------------------------------------------------
! Allocate FORWARD structure
!---------------------------------------------------------------------
       status = CRTM_Allocate_Atmosphere(crtm_struc(n)%nlayers, &
            crtm_struc(n)%nabsorbers, crtm_struc(n)%nclouds, &
            crtm_struc(n)%naerosols, crtm_struc(n)%Atm)
       call LIS_verify(status, 'Error allocating CRTM Atmosphere structure')
!---------------------------------------------------------------------
! Allocate output K-MATRIX structure
!---------------------------------------------------------------------
       do m=1,n_Profiles
          status = CRTM_Allocate_Atmosphere(crtm_struc(n)%nlayers, &
               crtm_struc(n)%nabsorbers, crtm_struc(n)%nclouds, &
               crtm_struc(n)%naerosols, crtm_struc(n)%Atm_K(:,m))
          call LIS_verify(status, 'Error allocating CRTM Atmosphere_K structures')
       enddo
...
!---------------------------------------------------------------------
  ...
  end subroutine CRTM_Kmatrix_initialize 
...
!    Level Pressure       
       ...
       call LIS_verify(status,'ESMF_StateGet with LPressure in CRTM_Kmatrix_f2t')
       
       call ESMF_FieldGet(lpressField, localDE=0,farray=lpress,rc=status)
       call LIS_verify(status, 'ESMF_FieldGet with LPressure in CRTM_Kmatrix_f2t')
...
! Pressure
       ...
       call LIS_verify(status,'ESMF_StateGet with Pressure in CRTM_Kmatrix_f2t')
       
       call ESMF_FieldGet(pressField, localDE=0,farray=press,rc=status)
       call LIS_verify(status, 'ESMF_FieldGet with Pressure in CRTM_Kmatrix_f2t')
       kk = crtm_struc(n)%nlayers+2-k
       do t=1,LIS_rc%ntiles(n)
...
! Temperature
       call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_Tair%varname(k)), &
            tmpField, rc=status)
       call LIS_verify(status,'ESMF_StateGet with Temperature in CRTM_Kmatrix_f2t')
       
       call ESMF_FieldGet(tmpField, localDE=0,farray=tmp,rc=status)
       call LIS_verify(status, 'ESMF_FieldGet with Temperature in CRTM_Kmatrix_f2t')
       kk = crtm_struc(n)%nlayers+2-k
       do t=1,LIS_rc%ntiles(n)
...
! Absorber 1: H2O
       call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_Qair%varname(k)), &
            q2Field, rc=status)
       call LIS_verify(status,'ESMF_StateGet with Q2 in CRTM_Kmatrix_f2t')
       
       call ESMF_FieldGet(q2Field, localDE=0,farray=q2,rc=status)
       call LIS_verify(status, 'ESMF_FieldGet with Q2 in CRTM_Kmatrix_f2t')
...
!          crtm_struc(n)%Atm(t)%Absorber(kk,1) = max(q2(t),qsmall)*1000.0_fp
          crtm_struc(n)%Atm(t)%Absorber(kk,1) = SA_to_MR(max(q2(t)*1000.0_fp,qsmall))
       enddo 
...
! Absorber 2: O3
       if(LIS_FORC_O3%selectOpt.eq.1) then 
          call ESMF_StateGet(LIS_FORC_State(n), trim(LIS_FORC_O3%varname(k)), &
               o3Field, rc=status)
          call LIS_verify(status,'ESMF_StateGet with O3 in CRTM_Kmatrix_f2t')
          
          call ESMF_FieldGet(o3Field, localDE=0,farray=o3,rc=status)
          call LIS_verify(status, 'ESMF_FieldGet with O3 in CRTM_Kmatrix_f2t')
       endif
       kk = crtm_struc(n)%nlayers+2-k
       do t=1,LIS_rc%ntiles(n)
          if(LIS_FORC_O3%selectOpt.eq.1) then 
...
!hack
  ...
  end subroutine CRTM_Kmatrix_f2t


  subroutine CRTM_Kmatrix_geometry(n)
...
! this needs to be replaced by a real calculation. 
    crtm_struc(n)%GeometryInfo%Sensor_Zenith_Angle = crtm_struc(n)%zenith_angle
...
! Yudong: converted radians to degrees
    ...
    crtm_struc(n)%GeometryInfo%Sensor_Scan_Angle &
         = DASIN(r*DSIN(crtm_struc(n)%GeometryInfo%Sensor_Zenith_Angle))*180.0/3.14159265


  end subroutine CRTM_Kmatrix_geometry


  subroutine CRTM_Kmatrix_run(n)
! !USES: 
    use LIS_coreMod, only : LIS_rc
    use LIS_RTMMod, only : LIS_sfcState
    use LIS_logMod, only : LIS_verify
    ...
    integer             :: status
    integer             :: t
    real,    allocatable    :: wind_speed(:), land_coverage(:), land_temperature(:), snow_temperature(:),&
...
!   zero the K-matrix output structures
    ...
    call CRTM_Zero_Atmosphere(crtm_struc(n)%Atm_K )
    call CRTM_Zero_Surface(crtm_struc(n)%Sfc_K )

! Initialize the K-matrix INPUT so that all the 
! results are dTb/dx
    crtm_struc(n)%RTSolution_K%Brightness_Temperature = ONE
    crtm_struc(n)%RTSolution_K%Radiance = ONE

    status = CRTM_K_Matrix(crtm_struc(n)%Atm, &
         crtm_struc(n)%Sfc,                   &
         crtm_struc(n)%RTSolution_K,          &
         crtm_struc(n)%GeometryInfo,          & 
         crtm_struc(n)%ChannelInfo,           & 
         crtm_struc(n)%Atm_K,                 & 
         crtm_struc(n)%Sfc_K,                 & 
         crtm_struc(n)%RTSolution) 
    call LIS_verify(status, 'Error in CRTM K_Matrix Model')


  end subroutine CRTM_Kmatrix_run

!BOP
! !ROUTINE: CRTM_Kmatrix_output
! \label{CRTM_Kmatrix_output}
!
! !INTERFACE: 
  subroutine CRTM_Kmatrix_output(n)
! !USES: 
...
!EOP
    ...
    real              :: datafield(LIS_rc%ngrid(n))
     ...
     if(LIS_masterproc) then 
        ...
        call LIS_create_output_directory('CRTM',style=2)
...
! data needs to be averaged upto the grid scale first. 
  ...
  end subroutine CRTM_Kmatrix_output
...
! !USES: 
    ...
    call ESMF_FieldGet(varField, localDE=0,farray=var, rc=status)
