...
!  MERRA-Land data, transforms into 9 LDT forcing 
...
!EOP
  ...
  real      :: tair(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: qair(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: uwind(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: vwind(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: ps(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: prectot(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: precsno(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: preccon(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: prectot_flx(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: preccon_flx(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: swgdn(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: lwgab(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: swland(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: pardr(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: pardf(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
  real      :: hlml(merraland_struc(n)%nc, merraland_struc(n)%nr,24)
!  real      :: emis(merraland_struc(n)%nc, merraland_struc(n)%nr,24)

#if (defined USE_NETCDF3 || defined USE_NETCDF4) 
  ferror = 0 
  nr_index = merraland_struc(n)%nr
  nc_index = merraland_struc(n)%nc
  mo = LDT_rc%lnc(n)*LDT_rc%lnr(n)

  inquire(file=slvname,exist=file_exists) 
  if(file_exists) then 
     write(LDT_logunit,*) 'Reading .. ', trim(slvname)
     ...
     if(merraland_struc(n)%uselml.eq.1) then 
        ...
        if(.not.file_exists1) then 
           write(LDT_logunit,*) trim(flxname)//' does not exist'
           call LDT_endrun()
        endif
        write(LDT_logunit,*) 'Reading .. ', trim(flxname)
     ...
     write(LDT_logunit,*) trim(slvname)//' does not exist'
     call LDT_endrun()
  endif
  ...
  if(file_exists) then 
     ...
     write(LDT_logunit,*) 'Reading .. ', trim(mldname)
     call LDT_verify(nf90_open(path=trim(mldname), mode=NF90_NOWRITE, &
          ncid=ftn_mld),'nf90_open failed for mldfile in read_merraland')

     call LDT_verify(nf90_inq_varid(ftn_mld,'prectot',prectotId), &
          'nf90_inq_varid failed for prectot (mld) in read_merraland')

     call LDT_verify(nf90_get_var(ftn_mld,prectotId, prectot), &
     ...
     if (LDT_FORC_CRainf%selectOpt.eq.1) then
        call LDT_verify(nf90_inq_varid(ftn_flx,'prectot',prectot2Id), &
             'nf90_inq_varid failed for prectot (flx) in read_merraland')

        call LDT_verify(nf90_inq_varid(ftn_flx,'preccon',preccon2Id), &
             'nf90_inq_varid failed for preccon (flx) in read_merraland')

        call LDT_verify(nf90_get_var(ftn_flx,prectot2Id, prectot_flx), &
             'nf90_get_var failed for prectot (flx) in read_merraland')

        call LDT_verify(nf90_get_var(ftn_flx,preccon2Id, preccon_flx), &
             'nf90_get_var failed for preccon (flx) in read_merraland')
     endif

     if (LDT_FORC_Snowf%selectOpt.eq.1) then
        call LDT_verify(nf90_inq_varid(ftn_mld,'precsno',precsnoId), &
             'nf90_inq_varid failed for precsno (mld) in read_merraland')  

        call LDT_verify(nf90_get_var(ftn_mld,precsnoId, precsno), &
     ...
     write(LDT_logunit,*) trim(mldname)//' does not exist'
     call LDT_endrun()
  endif
  
  inquire(file=radname,exist=file_exists) 
  if(file_exists) then 
     write(LDT_logunit,*) 'Reading .. ', trim(radname)
...
!          'nf90_get_var failed for emis in read_merraland')
     ...
     write(LDT_logunit,*) trim(radname)//' does not exist'
     call LDT_endrun()
  endif
  ...
  if(read_lnd) then 
     ...
     if(file_exists) then 
        write(LDT_logunit,*) 'Reading .. ', trim(lndname)
        call LDT_verify(nf90_open(path=trim(lndname), mode=NF90_NOWRITE, &
             ncid=ftn_lnd), 'nf90_open failed in read_merraland')
        if(LDT_FORC_SWnet%selectOpt.eq.1) then 
           call LDT_verify(nf90_inq_varid(ftn_lnd,'swland',swlandId), &
                'nf90_inq_varid failed for swland in read_merraland')

           call LDT_verify(nf90_get_var(ftn_lnd,swlandId,swland), &
        ...
        if(LDT_FORC_Pardr%selectOpt.eq.1) then 
           call LDT_verify(nf90_inq_varid(ftn_lnd,'pardr',pardrId), &
                'nf90_inq_varid failed for pardr in read_merraland')

           call LDT_verify(nf90_get_var(ftn_lnd,pardrId,pardr), &
  ...
  call LDT_verify(nf90_close(ftn_mld),&
...
! !ARGUMENTS:
  integer, intent(in)    :: n
  integer, intent(in)    :: findex
  real,    intent(in)    :: input_var(merraland_struc(n)%nc, &
       merraland_struc(n)%nr, 24)
...
!EOP
  ...
  real      :: f (merraland_struc(n)%nc*merraland_struc(n)%nr)
  logical*1 :: lb(merraland_struc(n)%nc*merraland_struc(n)%nr)
  logical*1 :: lo(LDT_rc%lnc(n)*LDT_rc%lnr(n))

  do t=1,24
     lb = .true.
     do r=1,merraland_struc(n)%nr
        do c=1,merraland_struc(n)%nc
           k= c+(r-1)*merraland_struc(n)%nc
           f(k) = input_var(c,r,t)
           if ( f(k) == 1.e+15 ) then 
              f(k)  = LDT_rc%udef
              lb(k) = .false. 
     ...
     if(pcp_flag.and.&
          LDT_rc%met_gridtransform(findex) .eq."budget-bilinear" ) then 
        call conserv_interp(LDT_rc%gridDesc(n,:),lb,f,lo,&
             merraforc(var_index,t,:), &
             merraland_struc(n)%mi,LDT_rc%lnc(n)*LDT_rc%lnr(n),& 
     ...
     elseif( LDT_rc%met_gridtransform(findex).eq."bilinear".or.&
             LDT_rc%met_gridtransform(findex).eq."budget-bilinear") then 
     ...
     elseif( LDT_rc%met_gridtransform(findex).eq."neighbor" ) then 
        call neighbor_interp(LDT_rc%gridDesc(n,:),lb,f,lo,&
             merraforc(var_index,t,:),merraland_struc(n)%mi,&
             LDT_rc%lnc(n)*LDT_rc%lnr(n),&
             LDT_domain(n)%lat, LDT_domain(n)%lon,&
             merraland_struc(n)%n113,LDT_rc%udef,iret)

     else
        write(LDT_logunit,*) 'Spatial interpolation option '//&
