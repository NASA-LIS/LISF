...
!  27 May 2010: David Mocko, changed to hourly forcing for LDT6.0
!  25 Oct 2010: David Mocko, updated for LDT6.1 public release
...
! startime of 2005-11-01T00:00:00 with time-step of 1hr
  ...
  if( LDT_rc%nts(n).gt.3600 ) then   ! > 1-hr timestep
     write(LDT_logunit,*) 'ERR: When running LDT with MERRA-Land, the clock '
  ...
  if ( LDT_get_nstep(LDT_rc,n) == 1 .or. LDT_rc%rstflag(n) == 1 ) then
     ...
     call LDT_tick(ringtime,doy1,gmt1,yr1,mo1,da1,hr1,mn1,ss1,ts1)
  endif
  ...
  ! Assign MERRA-Land forcing fields to two LDT time-interp placeholders (metdata1,2):
     do r=1,LDT_rc%lnr(n)
        do c=1,LDT_rc%lnc(n)
