...
!BOP
! !ROUTINE: nldas1_ec_removal
! \label{nldas1_ec_removal}
!
! !REVISION HISTORY:
!  20 Oct 2006: Kristi Arsenault; Adapted elevation correction code
!               to remove such a correction from NLDAS1 fields
!
! !INTERFACE:

subroutine nldas1_ec_removal( nest, point, force_tmp, force_hum, &
                             force_lwd, force_prs )
...
!  North American Land Data Assimilation (NLDAS1) project, Journal of 
...
! ----------------------------------------------------------------
     ...
     elevdiff = nldas1_struc(nest)%orig_ediff(point)
...
!-- Reassign uncorrected fields to LDT forcing fields::
    force_tmp = orig_tmp
    force_hum = orig_hum
    force_lwd = orig_lwd
    force_prs = orig_prs
...
end subroutine nldas1_ec_removal
