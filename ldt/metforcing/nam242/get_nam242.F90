...
!      Interpolates NAM data to LDT grid
...
!-----------------------------------------------------------------
              call LDT_tick(dumbtime1, doy1, gmt1, yr1, mo1, da1, &
                            hr1, mn1, ss1, ts1)
           end if
        endif
        if ( try > ndays ) then  
...
!-----------------------------------------------------------------
           write(LDT_logunit,*) 'ERROR: NAM data gap exceeds 10 days on file 1'
           call LDT_endrun
        endif
     end do
  end if
...
! ferror still=0, so roll back one day & start again
!-----------------------------------------------------------------
           call LDT_tick(dumbtime2, doy2, gmt2, yr1, mo1, da1, &
                         hr1, mn1, ss1, ts2 )
        end if
        if ( try > ndays ) then  
!-----------------------------------------------------------------
! data gap exceeds 10 days so stop
!-----------------------------------------------------------------
           write(LDT_logunit,*) 'ERROR: NAM data gap exceeds 10 days on file 2'
           call LDT_endrun
        end if
     end do
  end if
