...
!    Reads and Interpolates GEOS5 forecast data to the LDT grid
!  \end{description}
!
!EOP
  ...
  if(LDT_rc%ts.gt.3600) then 
     ...
     write(LDT_logunit,*) '[WARN] LDT does not support this mode currently'
     write(LDT_logunit,*) '[WARN] Program stopping ...'
     call LDT_endrun()
  endif
  ...
  if(geos5fcst_struc(n)%findtime1.eq.1) then
     ...
     do 
        ...
        if(try.gt.11)then
           write(*,*)'[ERR] GEOS5fcst data gap exceeds 10 days on file 1'
           stop
        endif
     enddo
...
!=== the following looks back 10 days, at the same hour to fill data gaps.
     ...
     do 
        ...
        if(try.gt.11)then
           write(*,*)'[ERR] GEOS5fcst data gap exceeds 10 days on file 2'
           stop
        endif
     enddo
  endif   
