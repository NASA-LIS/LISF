...
!  24 Nov 2003: Sujay Kumar; Included the scheme in LDT
! !INTERFACE:
...
!EOP
  ...
  if ( LDT_get_nstep(LDT_rc,n) == 1 .or. &
       LDT_rc%rstflag(n) == 1 ) then !beginning of the run
  ...
  if (ecmwfreanal_struc(n)%findtime1==1) then  !need to get new time1 from the past
     ...
     do
        ...
        if ( try > ndays ) then 
           write(LDT_logunit,*) 'ERROR: ECMWF Reanalysis data gap exceeds 10 days on file 1'
           STOP
        end if
     end do
  endif
  ...
  if(ecmwfreanal_struc(n)%findtime2.eq.1) then ! need new time2 data
     ...
     do
        ...
        if ( try > ndays ) then 
           write(LDT_logunit,*) 'ERROR: ECMWF Reanalysis data gap exceeds 10 days on file 2'
           STOP
        end if
     end do
  endif
