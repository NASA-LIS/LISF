...
!
! !REVISION HISTORY:
! 25Aug2006: Yudong Tian; Modified for LDT 4.2 release 
...
!      Interpolates TRMM 3B42V6 data to LDT grid
!  \end{description}
!EOP
...
!  alarmCheck = LDT_isAlarmRinging(LDT_rc,"TRMM 3B42V6 alarm") ! SY
!  if(alarmCheck) then ! SY
 ...
 !------------------------------------------------------------------------
   ...
   ss1 = LDT_rc%ss
   ts1 = 0
   call LDT_tick( TRMM3B42V6_struc(n)%LDT_timeAtTStepEnd, doy1, gmt1, &
        yr1, mo1, da1, hr1, mn1, ss1, real(ts1))
   ts1 = (-1)*LDT_rc%nts(n) ! SY: for start of model time step
   call LDT_tick( TRMM3B42V6_struc(n)%LDT_timeAtTStepStart, doy1, gmt1, &
        yr1, mo1, da1, hr1, mn1, ss1, real(ts1))
 ...
 !------------------------------------------------------------------------
   ...
   ss2 = 0
   ts2 = 0
   call LDT_tick( TRMM3B42V6_struc(n)%TRMM3B42V6time_TStepStart, doy2, gmt2, &
   ...
   ss3 = 0
   ts3 = 0
   call LDT_tick( TRMM3B42V6_struc(n)%TRMM3B42V6time_TStepEnd, doy3, gmt3, &
