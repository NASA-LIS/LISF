...
! !USES:
    ...
    use LDT_logMod,         only    : LDT_verify, LDT_logunit ! SY
...
!EOP
  ...
  if ((TRMM3B42V6_struc(n)%TRMM3B42V6yr_TStepEnd .EQ. &
    ...
    wt1 = (abs(BreakpointTime-TRMM3B42V6_struc(n)%LDT_timeAtTStepStart)) / &
          (TRMM3B42V6_struc(n)%LDT_timeAtTStepEnd- &
           TRMM3B42V6_struc(n)%LDT_timeAtTStepStart) ! SY
    wt2 = 1.0 - wt1
  endif
  ...
  call ESMF_StateGet(LDT_FORC_Base_State(n,findex),trim(LDT_FORC_Rainf%varname(1)),pcpField,&
       rc=status)
  call LDT_verify(status, 'Error: enable Rainf in forcing variables ldtt')
...
!-- Applying a convective rainfall amount to observed precip field: 
   elseif( LDT_FORC_CRainf%selectOpt == 1 ) then
     ...
     call ESMF_StateGet(LDT_FORC_Base_State(n,findex),trim(LDT_FORC_CRainf%varname(1)),cpcpField,&
          rc=status)
     call LDT_verify(status, 'Error: enable CRainf in forcing variables ldtt')
