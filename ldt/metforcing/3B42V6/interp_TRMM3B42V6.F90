...
subroutine interp_TRMM3B42V6(n, nTRMM3B42V6, f, lb, ldt_gds, nc, nr, varfield, &
                             findex) ! SY
! !USES:
...
!EOP
  ...
  select case( LDT_rc%met_gridtransform(findex) ) ! SY

   case( "budget-bilinear" )
     call conserv_interp(ldt_gds,lb,f,lo,ldt1d,TRMM3B42V6_struc(n)%mi,mo,&
  ...
  ! SY: begin for nearest neighbor
   call neighbor_interp(ldt_gds,lb,f,lo,ldt1d,TRMM3B42V6_struc(n)%mi,mo,&
        LDT_domain(n)%lat, LDT_domain(n)%lon, &
        TRMM3B42V6_struc(n)%n112, LDT_rc%udef,iret)
