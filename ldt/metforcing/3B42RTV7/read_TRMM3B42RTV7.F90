...
!
! !INTERFACE:
subroutine read_TRMM3B42RTV7 (n, filename_TRMM3B42RT, findex, &
...
!  TRMM 3B42RT V7 data and interpolates to the LDT domain.
!
!  The arguments are:
!  \begin{description}
!  \item[n]
...
! Must reverse grid in latitude dimension to be consistent with LDT grid
!------------------------------------------------------------------------
...
! Check for and read in a gzipped file:
  ...
  if (file_exists) then 
     write(LDT_logunit, *) trim(filename)//".bin.gz"
   ...
   ! Check for and read in native binary file:
     ...
     inquire(file=dirfile, EXIST=file_exists) 
     if (file_exists) then 
        write(LDT_logunit, *) trim(filename)//".bin"
      ...
      ! Check for and read in processed 1gd4r binary file:
        ...
        inquire(file=dirfile, EXIST=file_exists) 
        if (file_exists) then 
          write(LDT_logunit, *) trim(filename)//".1gd4r"
...
!------------------------------------------------------------------------
   ...
   call interp_TRMM3B42RTV7(n, xd, yd, precip, LDT_rc%gridDesc(n,:), &
        LDT_rc%lnc(n),LDT_rc%lnr(n),precip_regrid, findex)
   ...
   Do j=1, yd
      Do i = 1, xd 
         if( rr(i, j).GE.0 ) then
           ...
           output(i, yd-j+1) = LDT_rc%udef   ! -9999.0
         end if
      End Do
   End Do
...
!print *, "after y-rev: ",dirfile(41:50),", x=1133, y=84: ", &
!          rr(1133, 84)*0.01, precip(1133, 397)
