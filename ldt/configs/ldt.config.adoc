...
|"`LDTSI analysis`" |LDT Snow and Ice analysis
|===
...
[[ssec-ldtsi]]
=== LDT Snow and Ice Analysis

`LDTSI valid date (YYYYMMDDHH):` valid date and hour (UTC) of analysis

`LDTSI fractional snow data directory:` directory with CDFS-II fractional snow data

`LDTSI modified data directory:` directory with legacy SNODEP "`modified`" data

`LDTSI surface obs data directory:` directory with surface snow reports

`LDTSI SSMIS data directory:` directory with SSMIS snow depth retrievals

`LDTSI surface temperature data directory:` directory with legacy 0.25 deg LIS ungribbed surface temperatures

`LDTSI static data directory:` directory with legacy SNODEP static data

`LDTSI unmodified data directory:` directory with legacy SNODEP "`unmodified`" data

`LDTSI VIIRS data directory:` directory with VIIRS TIFF files

`LDTSI SSMIS raw data directory:` directory with SSMIS BUFR files

`LDTSI SSMIS snow depth retrieval algorithm:` version of SSMIS retrieval algorithm. Acceptable values are:
...
`LDTSI SSMIS forest fraction file:` path to netCDF forest fraction file (for Foster et al (1997) algorithm)
...
`LDTSI decimal fraction adjustment of snow depth towards climo):` controls drift to climo in data void region

`LDTSI default snow depth (m) when actual depth unknown:` bogus value for when snow detected but depth unknown

`LDTSI minimum snow depth (m) for which to print a diagnostic:` threshold for printing surface ob report

`LDTSI maximum number of surface observations allowed:` self-explanatory

`LDTSI SSMIS shallow snow depth threshold (m):` minimum SSMIS snow depth considered reasonable

`LDTSI latitudes (deg * 100) for summer climo check:` three latitude bands used for sanity checking snow reports

`LDTSI elevations (m) for summer climo check:` four elevations used for sanity checking snow reports

`LDTSI temperature (deg K * 10) above which no snow is allowed:` used to adjust snow analysis

`LDTSI minimum ice concentration (%) needed to set ice flag:` used when constructing binary sea ice field

`LDTSI LDTSI high latitude thresholds (deg) for sea ice::` 24 latitudes, each row defining a hemisphere (NH, then SH), each column defining a month (J F M A M J J A S O N D). Used to define "high latitudes" in SSMIS-based sea ice analysis.

`LDTSI low latitude thresholds (deg) for sea ice:` 24 latitudes, each row defining a hemisphere (NH, then SH), each column defining a month (J F M A M J J A S O N D). Used to define "low latitudes" in SSMIS-based sea ice analysis.

`LDTSI max age of VIIRS pixels to use:` used to ignore old VIIRS data

`LDTSI min VIIRS fraction to mark point as bare ground:` fraction of no-snow VIIRS pixels in LDT grid box required to mark as "`no-snow`"

`LDTSI min VIIRS/CDFS-II fraction to mark point as snow:` fraction of snowy VIIRS pixels or CDFS-II in LDT grid box required to mark as "`snow`"

`LDTSI use CDFS-II fractional snow data:` self-explanatory

`LDTSI use VIIRS snow mask:` self-explanatory
...
`LDTSI observation error variance (m^2):` error variance assigned to surface snow reports

`LDTSI background error variance (m^2):` error variance assigned to first-guess field

`LDTSI background error horizontal correlation length (m):` correlation length for spreading snow depth corrections horizontally

`LDTSI background error vertical correlation length (m):` correlation length for spreading snow depth corrections vertically

`LDTSI elevQC difference threshold (m):` threshold for sanity checking snow report elevation against LDT

`LDTSI skewed backQC snow depth threshold (m):` threshold for rejecting abnormally small snow depth reports
...
`LDTSI bogus climatology snow depth value (m):` bogus value for climatology if not available

`LDTSI GOFS SST data directory:` directory with GOFS sea surface temperature netCDF files

`LDTSI GOFS CICE data directory:` directory with GOFS sea ice netCDF files

`LDTSI LIS GRIB2 data directory:` directory with LIS GRIB2 2-m temperature analyses

`LDTSI LIS GRIB2 security class:` used for constructing LIS GRIB2 filename

`LDTSI LIS GRIB2 data category:` used for constructing LIS GRIB2 filename

`LDTSI LIS GRIB2 data resolution:` used for constructing LIS GRIB2 filename

`LDTSI LIS GRIB2 area of data:` used for constructing LIS GRIB2 filename
