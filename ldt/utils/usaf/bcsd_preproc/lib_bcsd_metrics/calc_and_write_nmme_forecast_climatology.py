...
# Output file will contain total LEAD + 1 column where LEAD can be
# provided as input (e.g. 6 months, 9 months). First column is sorted
# quantiles ranging from about 0 to 1 and the rest of the columns are
# just sorted climatology of forecasts for all lead times for a given
...
def create_sorted_ts(data_ts):
    ...
    ## np.arange creates an array where values range from 1 to the
    ## length of time series and then simply dividing every value by
    ##  length of the time series (n) + 1
...
## Both FCST_SYR and CLIM_SYR, and FCST_EYR and CLIM_EYR will be
## the same since we are using the entire hindcast for
...
MODEL_NAME = str(sys.argv[15])
OUTDIR = str(sys.argv[16])
ENSS = int(sys.argv[17])
ENSF = int(sys.argv[18])
...
for LEAD_NUM in range(0, LEAD_FINAL): ## Loop from lead =0 to Final Lead
    ...
    for ens in range(ENS_NUM):
        ...
        for INIT_FCST_YEAR in range(CLIM_SYR, CLIM_EYR+1):
            ...
            INFILE = INFILE_TEMPLATE.format(INDIR, INIT_FCST_YEAR, ens1,
...
CLIM_ARRAY = np.empty((LEAD_FINAL+1, ((CLIM_EYR-CLIM_SYR)+1)*ENS_NUM,
                       len(LATS), len(LONS)))
...
for lat_num, LATS in  enumerate(LATS):
    for lon_num in LONS in enumerate(LONS):
        ...
        if ((LAT1 <= LATS[lat_num]) and (LATS[lat_num] <= LAT2) and \
            ...
            ## tweleve columns have sorted climatology values for all year
