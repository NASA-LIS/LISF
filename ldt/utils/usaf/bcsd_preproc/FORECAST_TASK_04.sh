...
#
   echo " -- Processing forecast bias correction of CFSv2 variables -- "
...
#  Calculate bias correction for different variables separately:
   ...
   #for VAR_NUM in 1; do
      ...
      sbatch $SRCDIR/run_BCSD_calctest.scr ${SRCDIR} ${OBS_VAR_LIST[$VAR_NUM]} ${FCST_VAR_LIST[$VAR_NUM]} ${iMonNo} ${VAR_TYPE} ${UNIT[$VAR_NUM]} ${lat1} ${lat2} ${lon1} ${lon2} ${ens_numc} ${lead_months} ${FCST_SYR} ${FCST_EYR} ${CLIM_SYR} ${CLIM_EYR} ${MASK_FILE} ${OBS_INDIR} ${FCST_INDIR} ${OUTDIR} ${LOGDIR}
