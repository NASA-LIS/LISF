...
#
    FORCEDIR=${PROJDIR}'/data/forecasts/NMME/'
    ...
    # fi
     ...
     for model in ${nmmemod}; do
	   ...
	   for ((YEAR=$FCST_SYR; YEAR<=$FCST_EYR; YEAR++)); do
        ...
        sbatch $SRCDIR/run_NMME_Temporal_disagg.scr ${SRCDIR} ${OBS_VAR} ${FCST_VAR} $iMonNo $VAR_TYPE ${UNITS} $lat1 $lat2 $lon1 $lon2 $model $ens_num $lead_months $YEAR $YEAR $MASK_FILE_PRECIP $MASK_FILE_NONPRECIP $MONTHLY_BC_FCST_DIR $MONTHLY_RAW_FCST_DIR $SUBDAILY_RAW_FCST_DIR $OUTDIR $LOGDIR $enss $ensf
