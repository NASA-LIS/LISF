...
!BOP
! 
! !ROUTINE: compute_earth_coord_latlon_ll
!  \label{compute_earth_coord_latlon_ll}
!
! !REVISION HISTORY: 
!   20 Sept 08 Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine compute_earth_coord_latlon_ll(gridDesco,gridDesci,npts,fill,xpts,ypts,& 
                                         rlon,rlat,nret)
...
! !DESCRIPTION:
!  This subroutine computes the lower left earth coordinates of 
...
!  endif
  ...
  if( gridDesco(1).eq.0 ) then  ! Output Lat-lon
...
!--  Translate grid coordinates to earth coordinates
     do n=1,npts
        if( xpts(n).ge.xmin .and. xpts(n).le.xmax .and. & 
           ...
           rlat(n) = max(lat_ll, rlat(n) - dlat/2)
!           rlon(n) = max(lon_ll, rlon(n) - dlon/2)
           tmp_lon = max(lon_ll, tmp_lon - dlon/2)
...
end subroutine compute_earth_coord_latlon_ll
