...
!        CALL h5dclose_f(dataset_id3,hdferr)
    ...
    INTEGER(HSIZE_T),DIMENSION(2):: dims, maxdims
       ...
       CALL h5open_f(hdferr) !Initialize hdf5
       CALL h5fopen_f (trim(filename),H5F_ACC_RDONLY_F,file_id,hdferr) !Open file
       write(ldt_logunit,*)'EMK: hdferr, filename = ', hdferr, trim(filename)
       !        PRINT*, 'sd_id, hdferr', file_id, hdferr
       CALL h5dopen_f (file_id, trim(dataset1),dataset_id1, hdferr) !Open dataset
       write(ldt_logunit,*)'EMK: hdferr, dataset1 = ', hdferr, trim(dataset1)

       call h5dget_space_f(dataset_id1,dspace_id,hdferr)
       write(ldt_logunit,*)'EMK: hdferr, dspace = ', hdferr
       call h5sget_simple_extent_dims_f(dspace_id, dims, maxdims, hdferr)
       write(ldt_logunit,*)'EMK: hdferr, dims = ', hdferr, maxdims, dims
       CALL h5dopen_f (file_id, trim(dataset2),dataset_id2, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset2 = ', hdferr, trim(dataset2)

       CALL h5dopen_f (file_id, trim(dataset3),dataset_id3, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset3 = ', hdferr, trim(dataset3)

       CALL h5dopen_f (file_id, trim(dataset4),dataset_id4, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset4 = ', hdferr, trim(dataset4)

       CALL h5dopen_f (file_id, trim(dataset5),dataset_id5, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset5 = ', hdferr, trim(dataset5)

       CALL h5dopen_f (file_id, trim(dataset6),dataset_id6, hdferr) !Open dataset       
       write(ldt_logunit,*)'EMK: hdferr, dataset6 = ', hdferr, trim(dataset6)

       CALL h5dopen_f (file_id, trim(dataset7),dataset_id7, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset7 = ', hdferr, trim(dataset7)

       CALL h5dopen_f (file_id, trim(dataset8),dataset_id8, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset8 = ', hdferr, trim(dataset8)

       CALL h5dopen_f (file_id, trim(dataset9),dataset_id9, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset9 = ', hdferr, trim(dataset9)

       CALL h5dopen_f (file_id, trim(dataset10),dataset_id10, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset10 = ', hdferr, trim(dataset10)

       CALL h5dopen_f (file_id, trim(dataset11),dataset_id11, hdferr) !Open dataset        
       write(ldt_logunit,*)'EMK: hdferr, dataset11 = ', hdferr, trim(dataset11)

       CALL h5dopen_f (file_id, trim(dataset12),dataset_id12, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset12 = ', hdferr, trim(dataset12)

       CALL h5dopen_f (file_id, trim(dataset13),dataset_id13, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset13 = ', hdferr, trim(dataset13)

       CALL h5dopen_f (file_id, trim(dataset14),dataset_id14, hdferr) !Open dataset 
       write(ldt_logunit,*)'EMK: hdferr, dataset14 = ', hdferr, trim(dataset14)

       CALL h5dopen_f (file_id, trim(dataset15),dataset_id15, hdferr) !Open dataset       
       write(ldt_logunit,*)'EMK: hdferr, dataset15 = ', hdferr, trim(dataset15)
...
!        print *, col_temp_ind(192571), col_temp_ind(205664)
    ...
    ! Forked version of GetSMAP_L1B to SMAP files in operation, with
    ! fault tolerance and some special logic for older NRT files missing
    ! some variables.  Eric Kemp/SSAI.
    SUBROUTINE GetSMAP_L1B_NRT(filename, tb_time_seconds, &
         tb_v_surface_corrected, tb_h_surface_corrected, &
         tb_v, tb_h, &
         surface_water_fraction_mb_v, surface_water_fraction_mb_h, &
         nedt_v, nedt_h, tb_lat, tb_lon, &
         tb_qual_flag_v, tb_qual_flag_h, sc_nadir_angle, &
      ...
      real*4, allocatable, intent(out) :: tb_h_surface_corrected(:,:)
      real*4, allocatable, intent(out) :: tb_v(:,:), tb_h(:,:)
      real*4, allocatable, intent(out) :: surface_water_fraction_mb_v(:,:)
      real*4, allocatable, intent(out) :: surface_water_fraction_mb_h(:,:)
      real*4, allocatable, intent(out) :: nedt_v(:,:), nedt_h(:,:)
      ...
      ! Open tb_time_seconds and get dimensions
      ...
      allocate(tb_h_surface_corrected(n,m)); tb_h_surface_corrected = 0
      allocate(tb_v(n,m)) ; tb_v = 0
      allocate(tb_h(n,m)) ; tb_h = 0
      allocate(surface_water_fraction_mb_v(n,m))
      surface_water_fraction_mb_v = 0
      allocate(surface_water_fraction_mb_h(n,m))
      surface_water_fraction_mb_h = 0
      allocate(nedt_v(n,m)) ; nedt_v = 0
      allocate(nedt_h(n,m)) ; nedt_h = 0
      ...
      call get_dataset_id(file_id, dataset, dataset_id, ierr)
      if (ierr == 1) return
      call read_dataset_real(file_id, dataset, dataset_id, tb_v, &
           dims, ierr)
      if (ierr == 1) return

      ! Get tb_h
      dataset = "/Brightness_Temperature/tb_h/"
      call get_dataset_id(file_id, dataset, dataset_id, ierr)
      if (ierr == 1) return
      call read_dataset_real(file_id, dataset, dataset_id, tb_h, &
           dims, ierr)
      if (ierr == 1) return

      ! Get nedt_v
      dataset = "/Brightness_Temperature/nedt_v/"
      call get_dataset_id(file_id, dataset, dataset_id, ierr)
      if (ierr == 1) return
      call read_dataset_real(file_id, dataset, dataset_id, nedt_v, &
           dims, ierr)
      if (ierr == 1) return

      ! Get nedt_h
      dataset = "/Brightness_Temperature/nedt_h/"
      call get_dataset_id(file_id, dataset, dataset_id, ierr)
      if (ierr == 1) return
      call read_dataset_real(file_id, dataset, dataset_id, nedt_h, &
           dims, ierr)
      if (ierr == 1) return
      ...
      ! extra logic (via the handle_missing_nrt_field flag) to only
      ! deallocate these four arrays if a field is found to be missing at this
      ! point.  The "uncorrected" fields will be sent back to the caller
      ! for potential use, along with the error flag indicating an error
      ! was encountered by the HDF5 library.
      ...
      ! Get tb_h_surface_corrected
      dataset = "/Brightness_Temperature/tb_h_surface_corrected/"
      call get_dataset_id(file_id, dataset, dataset_id, ierr, &
           handle_missing_nrt_field=.true.)
      if (ierr == 1) return
      call read_dataset_real(file_id, dataset, dataset_id, &
      ...
      ! Get surface_water_fraction_mb_v
      dataset = "/Brightness_Temperature/surface_water_fraction_mb_v/"
      call get_dataset_id(file_id, dataset, dataset_id, ierr, &
           handle_missing_nrt_field=.true.)
      if (ierr == 1) return
      call read_dataset_real(file_id, dataset, dataset_id, &
      ...
      ! Get surface_water_fraction_mb_h
      dataset = "/Brightness_Temperature/surface_water_fraction_mb_h/"
      call get_dataset_id(file_id, dataset, dataset_id, ierr, &
           handle_missing_nrt_field=.true.)
      if (ierr == 1) return
      call read_dataset_real(file_id, dataset, dataset_id, &
    ...
    contains
      ...
      subroutine get_dataset_id(file_id, dataset, dataset_id, ierr, &
          ...
          if (hdferr == -1) then
             write(LDT_logunit,*)'[ERR] Cannot use dataset ', trim(dataset)
             call h5fclose_f(file_id, hdferr)
             call h5close_f(hdferr)
             if (present(handle_missing_nrt_field)) then
                call freeall(ierr, handle_missing_nrt_field)
             else
                call freeall(ierr)
             endif
          end if
        ...
        subroutine freeall(ierr, handle_missing_nrt_field)
          implicit none
          integer, intent(out) :: ierr
          logical, optional, intent(in) :: handle_missing_nrt_field
          if (allocated(tb_v_surface_corrected)) &
               deallocate(tb_v_surface_corrected)
          if (allocated(tb_h_surface_corrected)) &
               deallocate(tb_h_surface_corrected)
          if (allocated(surface_water_fraction_mb_v)) &
               deallocate(surface_water_fraction_mb_v)
          if (allocated(surface_water_fraction_mb_h)) &
               deallocate(surface_water_fraction_mb_h)
          ! If special flag is present and set to .true., we will use the
          ! "uncorrected" data in the retrieval, and other fields.
          if (present(handle_missing_nrt_field)) then
             if (handle_missing_nrt_field) then
                ierr = 1
                return
             end if
          end if
          if (allocated(tb_time_seconds)) deallocate(tb_time_seconds)
          if (allocated(tb_v)) deallocate(tb_v)
          if (allocated(tb_h)) deallocate(tb_h)
          if (allocated(nedt_v)) deallocate(nedt_v)
          if (allocated(nedt_h)) deallocate(nedt_h)
        ...
        call LDE_endrun()
#endif
      end subroutine GetSMAP_L1B_NRT
