...
!
! !REVISION HISTORY:
!  22 Aug 2025: Eric Kemp, Initial Specification
!
! !INTERFACE:
subroutine readSNIP(n, yyyymmdd, hh, SnowDepth, rc)
...
!EOP
  ...
  external :: create_SNIP_filename
  external :: read_SNIP_data

  rc = 1 ! Default to error, will update below if SNIP file read in.
...
end subroutine readSNIP

!BOP
!
! !ROUTINE: read_SNIP_data
! \label{read_SNIP_data}
!
! !INTERFACE:
subroutine read_SNIP_data(n, fname, SnowDepth, rc)
!
! !USES:
  use LDT_logMod
  use LDT_coreMod
...
end subroutine read_SNIP_data

!BOP
! !ROUTINE: create_SNIP_filename
! \label{create_SNIP_filename}
!
! !INTERFACE:
subroutine create_SNIP_filename(LISdir, yyyymmdd, hh, filename)
! !USES:
...
!EOP
  ...
  filename = trim(LISdir) // '/SNIP_' // trim(yyyymmdd) &
       // trim(hh) // '.nc'

end subroutine create_SNIP_filename
