...
module SNODEP_netcdfMod
   ...
   ! Public routines
   public :: SNODEP_write_netcdf
   public :: SNODEP_read_netcdf
   public :: SNODEP_read_netcdf_12z

contains

#if(defined USE_NETCDF3 || defined USE_NETCDF4)
   ! Subroutine for writing SNODEP analysis to netCDF
   subroutine SNODEP_write_netcdf(date10)
      ...
      if (LDT_masterproc) then
         ...
         case default
            write(LDT_logunit,*) &
                 '[ERR] Only latlon map projection supported for SNODEP'
            call LDT_endrun()
         end select
         ...
         cyyyy = snodep_settings%date10(1:4)
         cmm = snodep_settings%date10(5:6)
         cdd = snodep_settings%date10(7:8)
         chh = snodep_settings%date10(9:10)
         write(time_units,'(A)') &
         ...
         do j = 1,nr
            do i = 1,nc
               if (SNODEP_arrays%icecon(i,j) < 0) then
                  icecon_tmp(i,j) = -1
               else
                  icecon_tmp(i,j) = 0.01*(SNODEP_arrays%icecon(i,j))
               endif
            end do
         end do
      ...
      write(LDT_logunit,*) &
           '[INFO] Finished writing LDT SNODEP parameters to netcdf file'

   end subroutine SNODEP_write_netcdf
   ...
   subroutine SNODEP_write_netcdf(date10)
      ...
      write(LDT_logunit,*)'Cannot write out SNODEP data'
...
#if(defined USE_NETCDF3 || defined USE_NETCDF4)
   ! Subroutine for reading SNODEP analysis to netCDF
   subroutine SNODEP_read_netcdf(date10,ierr)
      ...
      if (ntime .ne. 1 .or. &
           ...
           nlon .ne. nc) then
         write(LDT_logunit, *) &
              '[ERR] Dimension mismatch between LDT and SNODEP input'
         write(LDT_logunit,*) &
      ...
      ! Fetch the SNODEP analysis variable IDs
      ...
      do r = 1, nr
         do c = 1, nc
            if (tmp(c,r,1) < 0) then
               SNODEP_arrays%oldcon(c,r) = -1
            else
               SNODEP_arrays%oldcon(c,r) = nint(100*tmp(c,r,1))
            end if
         end do
      end do
      ...
      !SNODEP_arrays%oldmask(:,:) = tmp(:,:,1)
      do r = 1, nr
         do c = 1, nc
            SNODEP_arrays%oldmask(c,r) = nint(tmp(c,r,1))
         end do ! c
      end do ! r
      ...
      !SNODEP_arrays%iceage(:,:) = tmp(:,:,1)
      do r = 1, nr
         do c = 1, nc
            SNODEP_arrays%iceage(c,r) = nint(tmp(c,r,1))
         end do ! c
      end do ! r
      ...
      ! Normal exit
      ierr = 0
   end subroutine SNODEP_read_netcdf
   ...
   subroutine SNODEP_read_netcdf(date10, ierr)
      ...
      ierr = 1
      write(LDT_logunit,*)'[ERR] LDT not compiled with NETCDF support!'
      write(LDT_logunit,*)'Cannot read out SNODEP data'
      write(LDT_logunit,*)'Recompile with NETCDF support and try again!'
      call LDT_endrun()
   end subroutine SNODEP_read_netcdf
...
#if(defined USE_NETCDF3 || defined USE_NETCDF4)
   ! Subroutine for reading 12Z SNODEP analysis to netCDF
   ! Only snoage and iceage are read.
   subroutine SNODEP_read_netcdf_12z(date10,ierr)
      ...
      call LDT_verify(nf90_get_var(ncid=ncid, &
           ...
           values=tmp), &
           '[ERR] Error in nf90_get_var for snoage')
      !SNODEP_arrays%snoage12z(:,:) = tmp(:,:,1)
      do r = 1, nr
         do c = 1, nc
            SNODEP_arrays%snoage12z(c,r) = nint(tmp(c,r,1))            
         end do ! c
      end do ! r
      ...
      call LDT_verify(nf90_get_var(ncid=ncid, &
           ...
           values=tmp), &
           '[ERR] Error in nf90_get_var for iceage')
      !SNODEP_arrays%iceage12z(:,:) = tmp(:,:,1)
      do r = 1, nr
         do c = 1, nc
            SNODEP_arrays%iceage12z(c,r) = nint(tmp(c,r,1))
         end do ! c
      end do ! r
      ...
      ! Normal exit
      ierr = 0
   end subroutine SNODEP_read_netcdf_12z
