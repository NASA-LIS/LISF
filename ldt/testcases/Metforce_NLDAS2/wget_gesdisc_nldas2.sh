...
#  03.04.2017 -- Kristi Arsenault: v1.2: Updated testcase for LDT
#
...
# Date inputs:
  ...
  startdy=01
  finalyr=2010
  finalmo=jul    #jan,feb,mar,apr,may,jun,jul,aug,sep,oct,nov,dec
  finaldy=01
...
### WGET FILES:: ###
  ...
  ## All GES DISC NLDAS-2 files, by year/doy:
     ...
     ## NLDAS-2 A files:
        ...
        wget -nv -r -l1 -nH --accept "grb" ${file} --cut-dirs=4  -a ${rundir}/download_nldas-2a.log

     ## NLDAS-2 B files:
        file=${serverhost}/${sourcedirb}/$year/$doydir/
        wget -nv -r -l1 -nH --accept "grb" ${file} --cut-dirs=4  -a ${rundir}/download_nldas-2b.log
