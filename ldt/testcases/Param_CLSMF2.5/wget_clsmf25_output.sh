...
#
# Settings for wgetting CLSM F2.5 output data files from the 
...
# Enter your destination directory (Default below matches LDT inputs files):
  ...
  sourcedir=/lisdata_pub/data/TESTCASES/LDT/Param_CLSMF2.5

  echo "== Downloading the LDT CLSMF2.5 Output Files == "
...
## Create directory structure:
 ...
 # Download the original output files to compare with:
   cd ${rundir}
   wget --no-check-certificate -r --accept "*.gz" -l1 -nv ${file} -nH --cut-dirs=5 -a ${targetdir}/download_clsmf25_output.log
