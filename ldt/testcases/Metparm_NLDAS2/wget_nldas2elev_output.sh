...
#
# Settings for wgetting NLDAS forcing data files from the 
...
# Enter your destination directory (Default below matches LDT inputs files):
  ...
  sourcedir=/lisdata_pub/data/TESTCASES/LDT/Metparm_NLDAS2

  echo "== Downloading the LDT NLDAS2 Elevation Output Test Files == "
...
## Create directory structure:
 ...
 # Download the original output files to compare with:
   cd ${rundir}
   wget --no-check-certificate -r --accept "*.gz" -l1 -nv ${file} -nH --cut-dirs=5 -a ${targetdir}/download_nldas2elev_output.log
