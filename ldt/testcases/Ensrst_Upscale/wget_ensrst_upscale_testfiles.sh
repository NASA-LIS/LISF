...
# Enter your destination directory (Default below matches LDT inputs files):
  ...
  sourcedir=/lisdata_pub/data/TESTCASES/LDT/Ensrst_Upscale

  echo "== Downloading the LDT Ensemble Restart - Upscaling Test Files == "
...
## Create directory structure:
 ...
 # Download the input ensemble restart file:
   wget --no-check-certificate -r --accept "*.nc" -l1 -nv ${file} -nH --cut-dirs=5 -a download_ensrstup.log

 # Download the original output files to compare with:
   cd ${rundir}
   wget --no-check-certificate -r --accept "*.gz" -l1 -nv ${file} -nH --cut-dirs=5 -a ${targetdir}/download_ensrstup.log
