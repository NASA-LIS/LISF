...
#
# Settings for wgetting GeoWRSI output data files from the 
...
# Enter your destination directory (Default below matches LDT inputs files):
  ...
  sourcedir=/lisdata_pub/data/TESTCASES/LDT/Param_GeoWRSI

  echo "== Downloading the LDT GeoWRSI Output Files == "
...
## Create directory structure:
 ...
 # Download the original output files to compare with:
   cd ${rundir}
   wget --no-check-certificate -r --accept "*.gz" -l1 -nv ${file} -nH --cut-dirs=5 -a ${targetdir}/download_geowrsi_output.log
