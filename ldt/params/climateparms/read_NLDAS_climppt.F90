...
!BOP
!
! !ROUTINE: read_NLDAS_climppt
! \label{read_NLDAS_climppt}
!
! !REVISION HISTORY:
!  2 Oct 2017: Bailing Li; Initial Specification
!
! !INTERFACE:
 subroutine read_NLDAS_climppt(n, out_ncols, out_nrows, &
...
! !DESCRIPTION:
!  This subroutine retrieves the NLDAS precipitation data climatology.
!  
!  The arguments are:
!  \begin{description}
!  \item[n]
!   index of the nest
!  \item[ppt_data]
!   output field with NLDAS precipitation climatogy data
!  \end{description}
!
!EOP      
  integer   :: ftn
...
!- Grid transform arrays:
  ...
  real, parameter   :: scale_factor = 0.001

! __________________________________________________________________________________________

   ppt_data = LDT_rc%udef

   inquire(file=trim(LDT_climate_struc(n)%climpptfile2), exist=file_exists)
   if(.not. file_exists) then 
      write(LDT_logunit,*) "[ERR] NLDAS Clim PPT map ",&
            trim(LDT_climate_struc(n)%climpptfile2)," not found."
   ...
   select case ( LDT_climate_struc(n)%clim_gridtransform )
    ...
    case default
      write(LDT_logunit,*) "[ERR] The spatial transform option selected for NLDAS Climatological" 
   ...
   open(ftn, file=trim(LDT_climate_struc(n)%climpptfile2),form="formatted",status="old" )
   ...
   read(ftn,'(a9,1x,f4.1)')  temp, cellsize
...
! -------------------------------------------------------------------
!     downscale FINE-SCALE LIS OUTPUT GRID
! -------------------------------------------------------------------
...
!- Set parameter grid array inputs:
   ...
   param_gridDesc(4)  =  25.0625     ! Edges of gridcell (ArcGIS)
   param_gridDesc(5)  = -124.9375    ! Edges of gridcell
   param_gridDesc(6)  =  128 
   param_gridDesc(7)  =  52.9375 
   param_gridDesc(8)  = -67.0625 
...
!- Assign 2-D array to 1-D for aggregation routines:
   ...
   do r = 1, nrows
      do c = 1, ncols;  i = i + 1
         ...
         if( read_input(c,r) .ne. nodata_value ) li1(i) = .true.
      enddo
   enddo
...
!- Select grid spatial transform option:
   select case ( LDT_climate_struc(n)%clim_gridtransform2 )
...
end subroutine read_NLDAS_climppt
