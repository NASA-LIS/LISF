...
module LDT_NAFPA_ERA5_climpptMod

! DESCRIPTION:
! This module implements routines to read, average, and interpolate monthly
! precipitation values from ERA5 interpolated to the NAFPA grid, already
! processed by LVT.
!
! REVISION HISTORY:
! 20 Sep 2022: Eric Kemp/SSAI: First version.

  ! Defaults
  implicit none
  private

  ! Class type for reading NAFPA ERA5 fields from LVT files,
  ! interpolating to LIS grid, summing, and averaging each month
  type, public :: LDT_NAFPA_ERA5_climppt_t
     private
   ...
   contains
     ! Object methods
     procedure :: new => LDT_NAFPA_ERA5_climppt_new
     procedure :: delete => LDT_NAFPA_ERA5_climppt_delete
     procedure :: process => LDT_NAFPA_ERA5_climppt_process
  end type LDT_NAFPA_ERA5_climppt_t

  ! Public readers that use the object under the hood
  public :: LDT_read_NAFPA_ERA5_climppt

contains

  ! Public reader
  subroutine LDT_read_NAFPA_ERA5_climppt(nest, ncols_out, nrows_out, &
    ...
    ! Locals
    type(LDT_nafpa_era5_climppt_t) :: nafpa_era5
    integer :: imonth
    character(3) :: months(12)
    months = (/'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', &
         'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'/)

    call nafpa_era5%new(nest, ncols_out, nrows_out, gridDesc_out)
    imonth = LDT_climate_struc(nest)%climpptimonth
    write(LDT_logunit,*)'[INFO] Processing ERA5 data for ', months(imonth)
    call nafpa_era5%process(nest, imonth, ncols_out, nrows_out, pcp_out)
    call nafpa_era5%delete()

  end subroutine LDT_read_NAFPA_ERA5_climppt

  ! Constructor method
  subroutine LDT_NAFPA_ERA5_climppt_new(this, nest, ncols_out, nrows_out, &
       gridDesc_out)
    ...
    ! Arguments
    class(LDT_NAFPA_ERA5_climppt_t), intent(inout) :: this
    integer, intent(in) :: nest
    integer, intent(in) :: ncols_out
    integer, intent(in) :: nrows_out
    real, intent(in) :: gridDesc_out(20)
    ...
    call ESMF_ConfigFindLabel(LDT_Config, &
         label='NAFPA ERA5 start years by month:', rc=rc)
    call LDT_verify(rc, 'NAFPA ERA5 start years by month: not specified')
    do c = 1, 12
       call ESMF_ConfigGetAttribute(LDT_config, this%startyears(c), rc=rc)
       call LDT_verify(rc, 'NAFPA ERA5 start years by month: not specified')
    end do
    call ESMF_ConfigFindLabel(LDT_Config, &
         label='NAFPA ERA5 end years by month:', rc=rc)
    call LDT_verify(rc, 'NAFPA ERA5 end years by month: not specified')
    do c = 1, 12
       call ESMF_ConfigGetAttribute(LDT_config, this%endyears(c), rc=rc)
       call LDT_verify(rc, 'NAFPA ERA5 end years by month: not specified')
    end do

  end subroutine LDT_NAFPA_ERA5_climppt_new

  ! Destructor method
  subroutine LDT_NAFPA_ERA5_climppt_delete(this)
  ...
  end subroutine LDT_NAFPA_ERA5_climppt_delete

#if (defined USE_NETCDF3 || defined USE_NETCDF4)

  ! Handle processing of one month of NAFPA ERA5 precipitation data
  subroutine LDT_NAFPA_ERA5_climppt_process(this, nest, imonth, &
    ...
    do iyear = this%startyears(imonth), this%endyears(imonth)
       ...
       case ("average")
          if (ipass .eq. 1) then
             ...
             write(ldt_logunit,*) &
                  '[INFO] Averaging NAFPA ERA5 data to LDT grid'
  ...
  end subroutine LDT_NAFPA_ERA5_climppt_process
  ...
  subroutine LDT_NAFPA_ERA5_climppt_process(this, nest, imonth, &
    ...
    write(LDT_logunit,*)'[ERR] Cannot process NAFPA ERA5 files!'
    write(LDT_logunit,*) &
  ...
  subroutine create_filename(this, imonth, iyear, filename)
    ...
    ! Arguments
    class(LDT_NAFPA_ERA5_climppt_t), intent(in) :: this
    integer, intent(in) :: imonth
...
end module LDT_NAFPA_ERA5_climpptMod
