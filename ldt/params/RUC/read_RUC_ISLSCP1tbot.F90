...
!BOP
!
! !ROUTINE: read_RUC_ISLSCP1tbot
! \label{read_RUC_ISLSCP1tbot}
!
! !REVISION HISTORY:
!  25 Jul 2005: Sujay Kumar; Initial Specification
!  05 Jun 2015: KR Arsenault; Modified to read in raw data file
!
! !INTERFACE:
subroutine read_RUC_ISLSCP1tbot(n, array)
...
!- Set parameter grid array inputs:
  ...
  if(.not.file_exists) then 
     write(LDT_logunit,*) "[ERR] ISLSCP1 TBOT map ",trim(RUC_struc(n)%tbotFile)," not found"
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif
  select case ( RUC_struc(n)%tbot_gridtransform )
    case( "none", "neighbor", "average", "bilinear", "budget-bilinear" )  ! Discrete data type
     ...
     if( RUC_struc(n)%tbot_gridtransform == "average" ) then
...
!- Open 60-minute deep soil temperature data file: 
  open (ftn, file=trim(RUC_struc(n)%tbotFile), form="unformatted", status='old', &
...
!- Transform parameter from original grid to LIS output grid:
   call LDT_transform_paramgrid(n, RUC_struc(n)%tbot_gridtransform, &
...
end subroutine read_RUC_ISLSCP1tbot
