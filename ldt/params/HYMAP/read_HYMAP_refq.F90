...
!BOP
!
! !ROUTINE: read_HYMAP_refq
! \label{read_HYMAP_refq}
!
! !REVISION HISTORY:
!  25 Jul 2005: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine read_HYMAP_refq(n, array)
! !USES:
  use ESMF
...
!EOP      
  ...
  if(.not.file_exists) then 
     write(LDT_logunit,*) '[ERR] HYMAP reference discharge map, ',&
           trim(HYMAP_struc(n)%refqfile),', not found.'
     write(LDT_logunit,*) 'Program stopping ...'
     call LDT_endrun
  endif

  open(ftn, file=trim(HYMAP_struc(n)%refqfile), access='direct',&
...
end subroutine read_HYMAP_refq
