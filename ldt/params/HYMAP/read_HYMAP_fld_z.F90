...
!BOP
!
! !ROUTINE: read_HYMAP_fld_z
! \label{read_HYMAP_fld_z}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine read_HYMAP_fld_z(n, array)
! !USES:
  use ESMF
...
!EOP      
  ...
  if(.not.file_exists) then 
     write(LDT_logunit,*) '[ERR] HYMAP floodplain roughness map, ',&
           trim(HYMAP_struc(n)%fldzfile),', not found.'
     write(LDT_logunit,*) 'Program stopping ...'
     call LDT_endrun
  endif

  open(ftn, file=trim(HYMAP_struc(n)%fldzfile), access='direct',&
...
end subroutine read_HYMAP_fld_z
