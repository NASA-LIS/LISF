...
!BOP
!
! !ROUTINE: read_HYMAP_river_height
! \label{read_HYMAP_river_height}
!
! !REVISION HISTORY:
!  25 Jul 2005: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine read_HYMAP_river_height(n, array)
...
!EOP      
  ...
  if(.not.file_exists) then 
     write(LDT_logunit,*) '[ERR] HYMAP river height map, ',&
           trim(HYMAP_struc(n)%riverheightfile),', not found.'
     write(LDT_logunit,*) 'Program stopping ...'
     call LDT_endrun
  endif

  open(ftn, file=trim(HYMAP_struc(n)%riverheightfile), access='direct',&
...
end subroutine read_HYMAP_river_height
