...
!EOP
    ...
    call ESMF_ConfigFindLabel(LDT_config, &
         "HYMAP floodplain height levels:",rc=rc)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%hymap_fld_height%num_bins,rc=rc)
    ...
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%riverwidthfile,rc=rc)
    ...
    call ESMF_ConfigFindLabel(LDT_config, &
         "HYMAP runoff time delay multiplier map:",rc=runoffdelaymcode)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%runoffdelaymfile,rc=rc)
       if (runoffdelaymcode.eq.0) then
          call LDT_verify(rc, &
               'HYMAP runoff time delay multiplier map: not specified')
       endif
    ...
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%baseflowdelayfile,rc=rc)
       if (baseflowdelaycode.eq.0) then
          call LDT_verify(rc, &
               'HYMAP baseflow time delay map: not specified')
       endif
    ...
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%basinmaskfile,rc=rc)
       call LDT_verify(rc,'HYMAP basin mask map: not specified')
    enddo

    call ESMF_ConfigFindLabel(LDT_config,"HYMAP river flow type map:", &
         rc=rivflocode)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%flowtypefile,rc=rc)
    ...
    call ESMF_ConfigFindLabel(LDT_config, &
         "HYMAP baseflow dwi ratio map:",rc=bfdwicode)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%baseflowdwiratiofile,rc=rc)
       if (bfdwicode.eq.0) then
          call LDT_verify(rc, &
               'HYMAP baseflow dwi ratio map: not specified')
       endif
       HYMAP_struc(n)%bfdwicode = bfdwicode
    enddo

    call ESMF_ConfigFindLabel(LDT_config,"HYMAP runoff dwi ratio map:", &
         rc=rundwicode)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%runoffdwiratiofile,rc=rc)
    ...
    call ESMF_ConfigFindLabel(LDT_config, &
         "HYMAP urban drainage outlet map:",rc=urbdroutcode)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%urbandrainoutletfile,rc=rc)
       if (urbdroutcode.eq.0) then
          call LDT_verify(rc, &
               'HYMAP urban drainage outlet map: not specified')
       endif
       HYMAP_struc(n)%urbdroutcode = urbdroutcode
    enddo

    call ESMF_ConfigFindLabel(LDT_config,"HYMAP levee height map:", &
         rc=leveehgtcode)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%leveeheightfile,rc=rc)
    ...
    call ESMF_ConfigFindLabel(LDT_config,"Land grid area map:", &
         rc=landareacode)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%landgridareafile,rc=rc)
    ...
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%nodelonfile,rc=rc)
    ...
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%nodelatfile,rc=rc)
    ...
    call LDT_verify(rc, &
         'HYMAP params map projection: option not specified in the config file')

    HYMAP_struc(:)%hymap_proj = hymap_proj

    call ESMF_ConfigFindLabel(LDT_config, &
         "HYMAP params spatial transform:",rc=rc)
    do n=1,LDT_rc%nnest
       call ESMF_ConfigGetAttribute(LDT_config, &
            HYMAP_struc(n)%hymap_gridtransform, &
            rc=rc)
       call LDT_verify(rc, &
            'HYMAP params spatial transform: option not specified in the config file')
    enddo
    ...
    do n=1,LDT_rc%nnest
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%riverzfile)
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%fldzfile)
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%fldheightfile)
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%flowdirxfile)
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%flowdiryfile)

       call LDT_gridOptChecks( n, "HYMAP grid elevation", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) &
            'reading '//trim(HYMAP_struc(n)%gridelevfile)
       call read_HYMAP_grid_elev(n,&
            HYMAP_struc(n)%hymap_grid_elev%value(:,:,1))
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%gridelevfile)
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%griddistfile)
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%gridareafile)

       call LDT_gridOptChecks( n, "HYMAP drainage area", &
            HYMAP_struc(n)%hymap_gridtransform, hymap_proj, &
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) &
            'reading '//trim(HYMAP_struc(n)%drainareafile)
       call read_HYMAP_drain_area(n,&
            HYMAP_struc(n)%hymap_drain_area%value(:,:,1))
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%drainareafile)
       ...
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%basinfile)
       ...
       if (HYMAP_struc(n)%runoffdelaycode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%runoffdelayfile)
       endif
       ...
       if (HYMAP_struc(n)%runoffdelaymcode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%runoffdelaymfile)
       endif
       ...
       if (HYMAP_struc(n)%baseflowdelaycode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%baseflowdelayfile)
       endif
       ...
       call LDT_gridOptChecks( n, "HYMAP basin mask", &
            ...
            hymapparms_gridDesc(n,9) )
       write(LDT_logunit,*) &
            'reading '//trim(HYMAP_struc(n)%basinmaskfile)
       call read_HYMAP_basin_mask(&
            n,HYMAP_struc(n)%hymap_mask%value(:,:,1))
       write(LDT_logunit,*) &
            'Done reading '//trim(HYMAP_struc(n)%basinmaskfile)
       ...
       if (HYMAP_struc(n)%rivflocode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%flowtypefile)
       endif
       ...
       if (HYMAP_struc(n)%bfdwicode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%baseflowdwiratiofile)
       endif
       ...
       if (HYMAP_struc(n)%rundwicode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%runoffdwiratiofile)
       endif
       ...
       if (HYMAP_struc(n)%urbdroutcode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%urbandrainoutletfile)
       endif
       ...
       if (HYMAP_struc(n)%leveehgtcode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%leveeheightfile)
       endif
       ...
       if (HYMAP_struc(n)%landareacode.eq.0) then
          call LDT_gridOptChecks( n, "Land grid area", &
               ...
               hymapparms_gridDesc(n,9) )
          write(LDT_logunit,*) &
               'reading '//trim(HYMAP_struc(n)%landgridareafile)
          call read_land_grid_area(n, &
               HYMAP_struc(n)%land_grid_area%value(:,:,1))
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%landgridareafile)
       endif
       ...
       if (HYMAP_struc(n)%nodeloncode.eq.0) then
          ...
          write(LDT_logunit,*) &
               'Done reading '//trim(HYMAP_struc(n)%nodelonfile)
       endif
