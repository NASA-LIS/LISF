...
!BOP
!
! !ROUTINE: read_VIC412_lc
!  \label{read_VIC412_lc}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine read_VIC412_lc(n, num_types, fgrd, maskarray)
...
!
! !DESCRIPTION:
!  This subroutine reads the VIC412 landcover data and returns the 
...
!
!  This VIC412-based land cover includes the following land cover classes:
!
!    layer0  -- water
   ...
   select case ( LDT_rc%lc_type(n) )
    ...
    case default ! non-supported options
      write(LDT_logunit,*) "Land classification: ",trim(LDT_rc%lc_type(n)),&
                           " does not exist for VIC412 source."
      write(LDT_logunit,*) " -- Please select:  VIC412 "
      write(LDT_logunit,*) "program stopping ..."
      call LDT_endrun
   end select

   if( LDT_rc%mask_type(n) == "readin" ) then
     write(unit=LDT_logunit,fmt=*) " [INFO]  Currently for the VIC 4.1.2 UMD landcover "
...
!- Double-check tiled landcover if it is a tiled file:
   if( file_dim == 2 .and. LDT_rc%lc_gridtransform(n)=="tile" .and. &
      ...
      write(LDT_logunit,*) "[ERR] in read_VIC412_lc :: The 'tile' spatial transform option " 
...
!            if( LDT_rc%mask_source(n) == "VIC412" ) then
!               call read_VIC412_maskfile( n, fgrd, maskarray )
!            endif
!         endif

    ! ---
      else
         write(unit=LDT_logunit,fmt=*) "[ERR] Resolutions of landcover file grid and LIS runtime grid "
       ...
       write(LDT_logunit,*) '[ERR] No other land cover aggregation types are currently supported ... '
       write(LDT_logunit,*) ' Program stopping ...'
       call LDT_endrun
