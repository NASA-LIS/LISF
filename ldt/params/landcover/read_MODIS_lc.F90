...
!  23  Mar 2014: KR Arsenault; Rewrote MODIS LC (LIS-based) reader
!
! !INTERFACE:
subroutine read_MODIS_lc(n, num_types, fgrd, maskarray)
...
!- Assign additional land cover types, including generic water points: 
   select case ( LDT_rc%lc_type(n) )
    ...
    case ( "PFT" )
      LDT_rc%urbanclass   = 9
      LDT_rc%snowclass    = 10
      LDT_rc%glacierclass = 10
      LDT_rc%bareclass    = 11
      LDT_rc%waterclass   = 12
      LDT_rc%wetlandclass = 0
    case default ! non-supported options
      write(LDT_logunit,*) "The land classification: ",trim(LDT_rc%lc_type(n)),&
                           " does not exist for MODIS source."
      write(LDT_logunit,*) " -- Please select: UMD, IGBP, IGBPNCEP, PFT "
...
!- Exceptions for Latlon/Lambert/Mercator projections:
  ...
  !- Double-check tiled landcover if it is a tiled file:
     if( file_dim == 3 .and. LDT_rc%lc_gridtransform(n)=="tile" ) then
        write(LDT_logunit,*) " (in read_MODIS_lc) :: The 'tile' spatial transform(n) option "
        write(LDT_logunit,*) "      is NO longer supported for 3D vegetation tiled files."
        write(LDT_logunit,*) "      The older LIS-aggregated parameter canned resolution sets"
        write(LDT_logunit,*) "       will not be supported in future LDT releases."
        write(LDT_logunit,*) " Program stopping ..."
        call LDT_endrun
     endif
  !- Tiling is not supported when input and output resolutions are the same:
     if( file_dim == 2 .and. LDT_rc%lc_gridtransform(n)=="tile" ) then
       if( subparam_gridDesc(9) == (LDT_rc%gridDesc(n,9)/LDT_rc%lis_map_resfactor(n)) ) then
        write(LDT_logunit,*) "[ERR] in read_MODIS_lc :: The 'tile' spatial transform option "
...
! -------------------------------------------------------------------
 ...
 !- Other transformations applied:
    ...
    !- (b) Estimate TILED land cover files (vegcnt):
       ...
       endif  ! End spatial transform check
       deallocate( gi, li )
