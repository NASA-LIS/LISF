...
!
! !INTERFACE:
subroutine read_AVHRR_lc(n, num_types, fgrd, maskarray)
   ...
   select case ( LDT_rc%lc_type(n) )
    ...
    case default ! non-supported options
      write(LDT_logunit,*) "The land classification: ",trim(LDT_rc%lc_type(n)),&
                           " does not exist for AVHRR source."
 ...
 !     (Old LIS-6 handling)
      if( file_dim == 3 ) then   ! 3-D file-size
         ...
         do t = 1, LDT_rc%nt
            ...
            if( LDT_rc%lc_type(n) == "UMD" .and. t == LDT_rc%nt ) exit
            do r = 1, subpnr
               do c = 1, subpnc
            ...
            enddo
         enddo
      !- Assigning counts for UMD water class tiles -- current solution (KRA):
         ...
         if( LDT_rc%lc_type(n) == "UMD" ) then
            ...
            do r = 1, subpnr
               do c = 1, subpnc
                  ...
                  read_vegcnt(c,r,LDT_rc%waterclass) = maxtile - isum
               enddo
            enddo
         endif
...
end subroutine read_AVHRR_lc


subroutine veg3d_neighbor( n, param_gridDesc, subpnc, subpnr, read_vegcnt, vegcnt )
...
! !USES:
  ...
  implicit none
   integer, intent(in) :: n
   real, intent(in)    :: param_gridDesc(20)    ! Input parameter grid desc array
   integer, intent(in) :: subpnc
   integer, intent(in) :: subpnr
   real,    intent(in) :: read_vegcnt(subpnc,subpnr,LDT_rc%nt)   ! Read input parameter
   real,    intent(out):: vegcnt(LDT_rc%lnc(n),LDT_rc%lnr(n),LDT_rc%nt)   ! Read input parameter
 
   integer               :: i, t, c, r, k 
   real                  :: isum, maxtile
   integer               :: mi, mo                     
   real,    allocatable  :: read_sumvegcnt(:,:)   ! Read input parameter
   integer, allocatable  :: n11(:)     ! Maps each input grid point to output grid points.
   real,    allocatable  :: gi(:)      ! Input parameter 1d grid
   real,    allocatable  :: gi2(:,:)   ! Input parameter 1d grid
   logical*1,allocatable :: li(:)      ! Input logical mask (to match gi)
   integer               :: ibi, ibo
   real      :: go1(LDT_rc%lnc(n)*LDT_rc%lnr(n))            ! Output lis 1d grid
   logical*1 :: lo1(LDT_rc%lnc(n)*LDT_rc%lnr(n))            ! Output logical mask (to match go1)
   real      :: go2(LDT_rc%lnc(n)*LDT_rc%lnr(n), LDT_rc%nt) ! Output lis tiled grid
   logical*1 :: lo2(LDT_rc%lnc(n)*LDT_rc%lnr(n), LDT_rc%nt) ! Output logical mask (to match go2)
! ______________________________________________________________________________________________

    write(LDT_logunit,*) "#Regridding: Applying nearest neighbor to input parameter (Veg 3D)"

    mo = LDT_rc%lnr(n)*LDT_rc%lnc(n)
    allocate( n11(mo) )
    call neighbor_interp_input( n, param_gridDesc, n11 )
                                 

    mi = subpnr*subpnc
    allocate( gi(mi), li(mi), gi2(mi,LDT_rc%nt) )
    allocate( read_sumvegcnt(subpnc,subpnr) )
    ...
    do r = 1, subpnr
       do c = 1, subpnc;  i = i + 1
          read_sumvegcnt(c,r) = sum(read_vegcnt(c,r,1:LDT_rc%nt))
          gi(i) = read_sumvegcnt(c,r)
          if( gi(i) .ne. LDT_rc%udef ) li(i) = .true.
          do t = 1, LDT_rc%nt
             gi2(i,t) = read_vegcnt(c,r,t)
          enddo
       enddo
    enddo

  ! Perform nearest neighbor approach:
    ibi = 1
    do k=1, mo
       go1(k)=0.
       go2(k,:)=0.
       if(n11(k).gt.0) then
         if(ibi.eq.0.or.li(n11(k))) then
           lo1(k) = .true.
           go1(k)   = gi(n11(k))
           go2(k,:) = gi2(n11(k),:)
         endif
 ...
 !- Convert 1D vegcnt to 2D grid arrays:
    ...
    do r = 1, LDT_rc%lnr(n)
       ...
       enddo
     enddo
    deallocate( read_sumvegcnt )
    deallocate( li, gi, gi2, n11 )

end subroutine veg3d_neighbor
