...
!BOP
!
! !ROUTINE: read_MODISNative_PFT
!  \label{read_MODISNative_PFT}
!
! !REVISION HISTORY:
...
!  23  Mar 2017: Shugong Wang; Modified for creating PFT for JULES based on MODIS modified IGBP  
!
! !INTERFACE:
subroutine read_MODISNative_PFT(n, num_types, fgrd, maskarray )
...
! IGBP-NCEP landcover version:
   ...
   real      :: go2(LDT_rc%lnc(n)*LDT_rc%lnr(n), 20) ! Output lis 1d grid
   logical*1 :: lo2(LDT_rc%lnc(n)*LDT_rc%lnr(n), 20) ! Output logical mask (to match go)
   ...
   real      :: vegcnt(LDT_rc%lnc(n),LDT_rc%lnr(n),20)
...
end subroutine read_MODISNative_PFT
