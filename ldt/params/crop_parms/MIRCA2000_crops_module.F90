...
CONTAINS
  ...
  subroutine readMIRCA2000Cropfiles( n, tempfile, cropcaflag, croptypefrac, &
    ...
    real,intent(out) :: &
         plantday(LDT_rc%lnc(n),LDT_rc%lnr(n),LDT_LSMCrop_struc(n)%multicroppingmax) ! lon x lat x seasons
    real,intent(out) :: &
         harvestday(LDT_rc%lnc(n),LDT_rc%lnr(n),LDT_LSMCrop_struc(n)%multicroppingmax) ! lon x lat x seasons
    ...
    case( "none", "average", "neighbor", "bilinear", "budget-bilinear" )
       ...
       do r = 1, LDT_rc%lnr(n)
          ...
          area = (sin(d2r*(latc+0.5*LDT_rc%gridDesc(n,9))) - &
               sin(d2r*(latc-0.5*LDT_rc%gridDesc(n,9))))*(LDT_rc%gridDesc(n,10)*d2r)
