...
!BOP
!
! !ROUTINE: read_CLSMF25_gfracmax
! \label{read_CLSMF25_gfracmax}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
 subroutine read_CLSMF25_gfracmax(n, array)
...
!
!EOP      
  real, parameter :: max_val = -10000.0
  integer :: ftn
  integer :: c,r,k
  integer :: month
...
! __________________________________________________________
  ...
  !- Determaxe global/complete parameter domain number of points:
...
! Read greenness from global file:
  ...
  do month = 1, 12
         ...
         do c = 1, glpnc
            if( LDT_rc%global_mask(c,r) > 0. ) then
               ...
               if( tmpreal(k) .ge. 0 ) then
                  if(tmpreal(k).gt.tmparray(c,r)) then 
   ...
   do r=1,glpnr
      do c=1,glpnc
         if(tmparray(c,r).eq.max_val) then 
...
end subroutine read_CLSMF25_gfracmax
