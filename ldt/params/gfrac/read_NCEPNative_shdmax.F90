...
!BOP
!
! !ROUTINE: read_NCEPNative_shdmax
! \label{read_NCEPNative_shdmax}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine read_NCEPNative_shdmax(n, array)
...
!- Set parameter grid array inputs:
   ...
   select case ( LDT_gfrac_struc(n)%gfrac_gridtransform)
    case( "none", "neighbor", "average", "bilinear", "budget-bilinear" )
      write(LDT_logunit,*) "[INFO] Reading NCEP Native greenness max file: ",&
              trim(LDT_gfrac_struc(n)%shdmaxFile)
    case default
      write(LDT_logunit,*) "[ERR] The spatial transform option selected for NCEP 'Native'"
      write(LDT_logunit,*) "   maximum gfrac files (@0.144 deg resolution) is not recognized"
   ...
   open(ftn, file=trim(LDT_gfrac_struc(n)%shdmaxFile),form="formatted",status='old' )
...
!- Convert 1D to 2D grid output arrays:
   ...
   write(LDT_logunit,*) "[INFO] Done reading NCEP Native greenness max file."

end subroutine read_NCEPNative_shdmax
