...
!BOP
!
! !ROUTINE: read_GTOPO30_slope
! \label{read_GTOPO30_slope}
!
! !REVISION HISTORY:
!  03 Sept 2004: Sujay Kumar; Initial Specification
!  01 Aug  2012: KR Arsenault; Expanded for slope tiling
!
! !INTERFACE:
subroutine read_GTOPO30_slope( n, num_bins, fgrd, slopeave )
...
! !DESCRIPTION:
!  This subroutine retrieves static slope data from the GTOPO30 source
...
!EOP
   ...
   real,    allocatable :: read_slope(:,:)   ! Read input parameter
...
!________________________________________________________________________
  ...
  inquire(file=trim(LDT_rc%slfile(n)), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) "Slope map ",trim(LDT_rc%slfile(n))," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif
  write(LDT_logunit,*) "Reading GTOPO30 (LIS-based) slope file: ",&
                       trim(LDT_rc%slfile(n))
...
!- Transform parameter grid to LIS run domain:
 ...
 !- Transforming 3-D slope field: 
   ...
   !- Initialize parameter read-in array:
      allocate( read_slope(subpnc,subpnr) )
      read_slope = LDT_rc%udef
      line = 0
      do r = 1, subpnr
         do c = 1, subpnc
            line = (lat_line(c,r)-1)*glpnc + lon_line(c,r)
            read(ftn,rec=line) read_slope(c,r)
         enddo
      enddo
      mi = subpnc*subpnr
   ...
   write(LDT_logunit, *) "[INFO] Done reading GTOPO30 slope file."

end subroutine read_GTOPO30_slope
