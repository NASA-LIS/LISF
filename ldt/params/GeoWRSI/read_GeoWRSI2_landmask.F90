...
!BOP
!
! !ROUTINE: read_GeoWRSI2_landmask
!  \label{read_GeoWRSI2_landmask}

! !REVISION HISTORY:
!  25 Oct 2013: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_GeoWRSI2_landmask(n,array)
...
! _________________________________________________________________________
   ...
   inquire( file=trim(GeoWRSI_struc(n)%wrsimask_file)//".bil", exist=file_exists )
   if(.not. file_exists) then
      write(LDT_logunit,*) "The WRSI landmask file, ",trim(GeoWRSI_struc(n)%wrsimask_file),&
                           " does not exist."
...
!- Assign filename pointer:
   ...
   bil_filename%str = GeoWRSI_struc(n)%wrsimask_file
   fileread_ok = populate_array_from_bilfile_2d(    &
                          mapFilename=bil_filename, &
                          real4ptr2d=real4ptr2d )

   if( fileread_ok ) then
      write(LDT_logunit,*) "WRSI-Parameter: Regional landmask file read-in successfully."
   else
      write(LDT_logunit,*) "[ERR]  Regional landmask file NOT read-in."
...
end subroutine read_GeoWRSI2_landmask
