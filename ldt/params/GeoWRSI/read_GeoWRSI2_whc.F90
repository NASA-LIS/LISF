...
!BOP
!
! !ROUTINE: read_GeoWRSI2_whc
!  \label{read_GeoWRSI2_whc}

! !REVISION HISTORY:
!  25 Oct 2013: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_GeoWRSI2_whc(n,array)
...
!
! !DESCRIPTION:
!  This subroutine retrieves the WRSI model water holding capacity
!  (WHC) parameter from a *BIL formatted set of files (includes
!   *hdr file). 
!
!  The arguments are:
!  \begin{description}
...
! _________________________________________________________________________
   ...
   inquire( file=trim(GeoWRSI_struc(n)%whc_file)//".bil", exist=file_exists )
   if(.not. file_exists) then
      write(LDT_logunit,*) "The WRSI WHC file, ",trim(GeoWRSI_struc(n)%whc_file),&
                           " does not exist."
...
!- Assign filename pointer:
   ...
   bil_filename%str = GeoWRSI_struc(n)%whc_file
   fileread_ok = populate_array_from_bilfile_2d(    &
                          mapFilename=bil_filename, &
                          real4ptr2d=real4ptr2d )

   if( fileread_ok ) then
      write(LDT_logunit,*) "WRSI-Parameter: Water holding capacity (WHC) read-in successfully."
   else
      write(LDT_logunit,*) "[ERR] Water holding capacity (WHC) NOT read-in."
...
end subroutine read_GeoWRSI2_whc
