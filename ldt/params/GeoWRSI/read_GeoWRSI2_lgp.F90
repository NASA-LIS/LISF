...
!BOP
!
! !ROUTINE: read_GeoWRSI2_lgp
!  \label{read_GeoWRSI2_lgp}

! !REVISION HISTORY:
!  25 Oct 2013: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_GeoWRSI2_lgp(n,array)
...
!
! !DESCRIPTION:
!  This subroutine retrieves the WRSI model length of growing
!   period (LGP) parameter from a *BIL formatted set of files (includes
!   *hdr file). 
!
!  The arguments are:
!  \begin{description}
...
! _________________________________________________________________________
   ...
   inquire( file=trim(GeoWRSI_struc(n)%lgp_file)//".bil", exist=file_exists )
   if(.not. file_exists) then
      write(LDT_logunit,*) "The WRSI LGP file, ",trim(GeoWRSI_struc(n)%lgp_file),&
                           " does not exist."
...
!   if( real4ptr2dIsNull .eq. .true. ) then
!      write(LDT_logunit,*) " [ERR] When running GeoWRSI v2.0 model, &
!           the LIS run domain dimensions must match that of the WRSI model &
!           parameter dimensions."
!      write(LDT_logunit,*) " [ERR] Please check the WRSI parameter file &
!           read in or LIS run domain. "
!      write(LDT_logunit,*) "Stopping ..."; call LDT_endrun
!   endif

!- Assign filename pointer:
   allocate(bil_filename%str)
   bil_filename%str = GeoWRSI_struc(n)%lgp_file
   fileread_ok = populate_array_from_bilfile_2d(    &
                          mapFilename=bil_filename, &   
                          real4ptr2d=real4ptr2d )

   if( fileread_ok ) then
      write(LDT_logunit,*) "WRSI-Parameter: Length of growing period (LGP) read-in successfully."
   else
      write(LDT_logunit,*) "[ERR]  Length of growing period (LGP) NOT read-in."
      write(LDT_logunit,*) "Stopping ..."; call LDT_endrun
...
end subroutine read_GeoWRSI2_lgp
