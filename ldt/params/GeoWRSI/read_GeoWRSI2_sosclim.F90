...
!BOP
!
! !ROUTINE: read_GeoWRSI2_sosclim
!  \label{read_GeoWRSI2_sosclim}

! !REVISION HISTORY:
!  25 Oct 2013: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_GeoWRSI2_sosclim(n,array)
...
!
! !DESCRIPTION:
!  This subroutine retrieves the WRSI model SOS climatology
!   (SOSclim) parameter from a *BIL formatted set of files (includes
!   *hdr file). 
!
!  The arguments are:
!  \begin{description}
...
! _________________________________________________________________________
   ...
   inquire( file=trim(GeoWRSI_struc(n)%sosclim_file)//".bil", exist=file_exists )
   if(.not. file_exists) then
      write(LDT_logunit,*) "The WRSI SOSclim file, ",trim(GeoWRSI_struc(n)%sosclim_file),&
                           " does not exist."
...
!- Assign filename pointer:
   ...
   bil_filename%str = GeoWRSI_struc(n)%sosclim_file
   fileread_ok = populate_array_from_bilfile_2d(    &
                          mapFilename=bil_filename, &
                          real4ptr2d=real4ptr2d )

   if( fileread_ok ) then
      write(LDT_logunit,*) "WRSI-Parameter: SOS-climatology read-in successfully."
   else
      write(LDT_logunit,*) "[ERR]  SOS-climatology NOT read-in."
...
end subroutine read_GeoWRSI2_sosclim
