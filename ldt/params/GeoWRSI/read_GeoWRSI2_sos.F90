...
!BOP
!
! !ROUTINE: read_GeoWRSI2_sos
!  \label{read_GeoWRSI2_sos}

! !REVISION HISTORY:
!  25 Oct 2013: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_GeoWRSI2_sos(n,array)
...
!
! !DESCRIPTION:
!  This subroutine retrieves the WRSI model start-of-season (SOS)
...
! _________________________________________________________________________
   ...
   inquire( file=trim(GeoWRSI_struc(n)%sos_file)//".bil", exist=file_exists )
   if(.not. file_exists) then
      write(LDT_logunit,*) "The WRSI SOS file, ",trim(GeoWRSI_struc(n)%sos_file),&
                           " does not exist."
...
!- Assign filename pointer:
   ...
   bil_filename%str = GeoWRSI_struc(n)%sos_file
   fileread_ok = populate_array_from_bilfile_2d(    &
                          mapFilename=bil_filename, &
                          real4ptr2d=real4ptr2d )

   if( fileread_ok ) then
      write(LDT_logunit,*) "[INFO] WRSI-Parameter: SOS file read-in successfully."
   else
      write(LDT_logunit,*) "[ERR]  SOS file NOT read-in."
...
end subroutine read_GeoWRSI2_sos
