...
!BOP
!
! !ROUTINE: read_GeoWRSI2_wrsiclim
!  \label{read_GeoWRSI2_wrsiclim}

! !REVISION HISTORY:
!  25 Oct 2013: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_GeoWRSI2_wrsiclim(n,array)
...
!
! !DESCRIPTION:
!  This subroutine retrieves the WRSI model WRSI climatology 
!   (wrsiclim) parameter data from a *BIL formatted set of files (includes
!   *hdr file). 
!
!  The arguments are:
!  \begin{description}
...
!    index of the nest
!   \item[array]
!    output field for WRSIclim data
!  \end{description}
!
!EOP      
  integer  :: ftn, rc
...
! _________________________________________________________________________
   ...
   inquire( file=trim(GeoWRSI_struc(n)%wrsiclim_file)//".bil", exist=file_exists )
   if(.not. file_exists) then
      write(LDT_logunit,*) "The WRSI Climatology file, ",trim(GeoWRSI_struc(n)%wrsiclim_file),&
                           " does not exist."
...
!- Assign filename pointer:
   ...
   bil_filename%str = GeoWRSI_struc(n)%wrsiclim_file
   fileread_ok = populate_array_from_bilfile_2d(    &
                          mapFilename=bil_filename, &
                          real4ptr2d=real4ptr2d )

   if( fileread_ok ) then
      write(LDT_logunit,*) "WRSI-Parameter: WRSI climatology read-in successfully."
   else
      write(LDT_logunit,*) "[ERR]  WRSI climatology NOT read-in."
...
end subroutine read_GeoWRSI2_wrsiclim
