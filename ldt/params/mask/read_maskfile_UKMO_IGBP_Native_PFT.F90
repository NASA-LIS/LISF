...
!
! SUBROUTINE: read_maskfile_UKMO_IGBP_Native_PFT
!
! REVISION HISTORY:
! 12 Sep 2017  Initial version....Eric Kemp, SSAI/NASA GSFC
!
! DESCRIPTION:
! Source code for reading and interpolating land mask data in netCDF format
! produced from UKMO_IGBP_Native_PFT land use data.  The logic in these
...
subroutine read_maskfile_UKMO_IGBP_Native_PFT(n, vegtype, fgrd, localmask)
   ...
   ! Local variables
   integer :: nlon,nlat
   real, allocatable :: landmask_native(:,:)
   
   localmask(:,:) = LDT_rc%udef
   
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
   
   call get_landmask(n,nlon,nlat,landmask_native)
   call interp_landmask(n, nlon, nlat, landmask_native, localmask)
   deallocate(landmask_native)
   call check_landmask(n,vegtype,fgrd,localmask)
   write(LDT_logunit,*)'[INFO] Done processing UKMO_IGBP_Native_PFT land mask!'
   
#else
   
   write(LDT_logunit,*) &
        '[ERR] netCDF4 required to process UKMO_IGBP_Native_PFT land mask!'
   write(LDT_logunit,*) &
...
contains
   ...
   subroutine get_landmask(n, nlon, nlat, landmask_native)
      ...
      real, allocatable, intent(out) :: landmask_native(:,:)
      ...
      ! Get land mask
      allocate(landmask_native(nlon,nlat))
      status = nf90_get_var(ncid,varid,landmask_native)
      if (status .ne. NF90_NOERR) then
         write(LDT_logunit,*)&
              '[ERR] NETCDF90 unable to find lat dimension length in ', &
              trim(LDT_rc%mfile(n))
   ...
   subroutine interp_landmask(n, nlon, nlat, landmask_native, localmask)
      ...
      real,    intent(in)  :: landmask_native(nlon,nlat)
      real,    intent(out) :: localmask(LDT_rc%lnc(n),LDT_rc%lnr(n))
      ...
      case( "none", "mode", "neighbor", "tile")
         write(LDT_logunit,*) "[INFO] Reading UKMO_IGBP_Native_PFT land mask "
      case default
         write(LDT_logunit,*) &
              "[ERR] The spatial transform option selected for "
         write(LDT_logunit,*) &
              "   UKMO_IGBP_Native_PFT is not recognized nor recommended."
         write(LDT_logunit,*) &
              "   Please select: "
         write(LDT_logunit,*) &
   ...
   subroutine check_landmask(n,vegtype,fgrd,localmask)
      ...
      if( cnt_1mask_0lc > 0 .or. cnt_0mask_1lc > 0) then
         ...
         write(LDT_logunit,*)" ... mismatch count: ", &
              cnt_1mask_0lc + cnt_0mask_1lc
         write(LDT_logunit,*) &
...
end subroutine read_maskfile_UKMO_IGBP_Native_PFT
