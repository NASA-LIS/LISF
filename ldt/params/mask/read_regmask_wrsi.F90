...
!BOP
!
! !ROUTINE: read_regmask_wrsi
!  \label{read_regmask_wrsi}
!
! !REVISION HISTORY:
!  25 Jan 2014: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_regmask_wrsi(n,array)
...
!
! !DESCRIPTION:
!  This subroutine retrieves the WRSI model regional landmask 
!   parameter from a *BIL formatted set of files (includes
!   *hdr file). 
!
!  The arguments are:
!  \begin{description}
...
!    index of the nest
!   \item[array]
!    output field for landmask data
!  \end{description}
!
!EOP      
...
! _________________________________________________________________________
   ...
   inquire( file=trim(LDT_rc%regfile(n))//".bil", exist=file_exists )

   if(.not. file_exists) then
      write(LDT_logunit,*) "The WRSI landmask file, ",trim(LDT_rc%regfile(n)),&
                           " does not exist."
...
!   print *, gCoords%minLon, gCoords%maxLon, gCoords%minLat,gCoords%maxLat 
...
!- Assign filename pointer:
   ...
   bil_filename%str = LDT_rc%regfile(n)
   fileread_ok = populate_array_from_bilfile_2d(    &
                          mapFilename=bil_filename, &
                          real4ptr2d=real4ptr2d )

   if( fileread_ok ) then
      write(LDT_logunit,*) "[INFO] Regional (WRSI) landmask file read-in successfully."
   else
      write(LDT_logunit,*) "[ERR]  Regional (WRSI) landmask file NOT read-in."
...
end subroutine read_regmask_wrsi
