...
!BOP
!
! !ROUTINE: read_NCEP_tbot
! \label{read_NCEP_tbot}
!
! !REVISION HISTORY:
!  25 Jul 2005: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine read_NCEP_GFS_tbot(n, array)
...
! _______________________________________________
  ...
  inquire(file=trim(Noah_struc(n)%tbotFile), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) "[ERR] GFS TBOT map ",trim(Noah_struc(n)%tbotFile)," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif

  write(LDT_logunit,*)"[INFO] Reading NCEP-GFS Bottom Temperature file: ",&
        trim(Noah_struc(n)%tbotfile)

  ftn = LDT_getNextUnitNumber()
  open(ftn, file=trim(Noah_struc(n)%tbotFile), access='sequential',status='old', &
       form="unformatted", recl=4)
  
  subparam_gridDesc = 0.
  call LDT_RunDomainPts(n, Noah_struc(n)%tbot_proj, Noah_struc(n)%tbot_gridDesc(:), &
  ...
  select case (Noah_struc(n)%tbot_gridtransform)
    case("none")
      do r = 1,subpnr
        do c = 1,subpnc
    ...
    case default
       write(LDT_logunit,*) "[ERR] The spatial transform, ",&
                            trim(Noah_struc(n)%tbot_gridtransform),&
...
end subroutine read_NCEP_GFS_tbot
