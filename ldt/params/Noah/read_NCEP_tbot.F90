...
!
! !INTERFACE:
subroutine read_NCEP_tbot(n, array)
...
!EOP      
  ...
  if(.not.file_exists) then 
     write(LDT_logunit,*) "[ERR] NCEP (LIS) TBOT map ",trim(Noah_struc(n)%tbotFile)," not found."
  ...
  call readLISdata(n, ftn, Noah_struc(n)%tbot_proj,  &
                   ...
                   Noah_struc(n)%tbot_gridDesc(:), 1, array)
