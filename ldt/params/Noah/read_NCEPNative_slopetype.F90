...
!BOP
!
! !ROUTINE: read_NCEPNative_slopetype
! \label{read_NCEPNative_slopetype}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine read_NCEPNative_slopetype(n, array)
...
!- Set parameter grid array inputs:
   ...
   select case ( Noah_struc(n)%slopetype_gridtransform )
    case( "none", "neighbor", "mode" )  ! Discrete data type
      ...
      if( Noah_struc(n)%slopetype_gridtransform == "mode" ) then
   ...
   open(ftn, file=Noah_struc(n)%slopetypefile, form='formatted', status='old')
...
!- Transform parameter from original grid to LIS output grid:
   call LDT_transform_paramgrid(n, Noah_struc(n)%slopetype_gridtransform, &
...
end subroutine read_NCEPNative_slopetype
