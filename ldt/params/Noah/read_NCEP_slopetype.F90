...
!BOP
!
! !ROUTINE: read_NCEP_slopetype
! \label{read_NCEP_slopetype}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine read_NCEP_slopetype(n, array)
...
! ____________________________
  ...
  if(.not.file_exists) then 
     write(LDT_logunit,*) "SLOPETYPE map ",trim(Noah_struc(n)%slopetypefile)," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif
  select case ( Noah_struc(n)%slopetype_gridtransform )
    case( "none", "neighbor", "mode" )  ! Discrete data type
      write(LDT_logunit,*) "[INFO] Reading NCEP-LIS slopetype file: ",&
  ...
  open(ftn, file=Noah_struc(n)%slopetypefile, access='direct',status='old', &
       form="unformatted", recl=4)

  call readLISdata(n, ftn, Noah_struc(n)%slopetype_proj, &
                   Noah_struc(n)%slopetype_gridtransform,&
                   Noah_struc(n)%slopetype_gridDesc(:), 1, array)   ! 1 indicates 2D layer
  ...
  write(LDT_logunit, *) "[INFO] Done reading NCEP-LIS slope type file"

end subroutine read_NCEP_slopetype
