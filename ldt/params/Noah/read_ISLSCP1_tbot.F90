...
!BOP
!
! !ROUTINE: read_ISLSCP1_tbot
! \label{read_ISLSCP1_tbot}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine read_ISLSCP1_tbot(n, array)
...
!- Set parameter grid array inputs:
  ...
  if(.not.file_exists) then 
     write(LDT_logunit,*) "ISLSCP1 TBOT map ",trim(Noah_struc(n)%tbotFile)," not found"
  ...
  select case ( Noah_struc(n)%tbot_gridtransform )
    case( "none", "neighbor", "average", "bilinear", "budget-bilinear" )  ! Discrete data type
     ...
     if( Noah_struc(n)%tbot_gridtransform == "average" ) then
...
!- Transform parameter from original grid to LIS output grid:
   call LDT_transform_paramgrid(n, Noah_struc(n)%tbot_gridtransform, &
...
end subroutine read_ISLSCP1_tbot
