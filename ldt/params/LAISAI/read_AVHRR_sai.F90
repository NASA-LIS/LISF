...
!BOP
!
! !ROUTINE: read_AVHRR_sai
! \label{read_AVHRR_sai}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine read_AVHRR_sai(n, array)
! !USES:
  use ESMF
...
! _________________________
  ...
  inquire(file=trim(LDT_laisai_struc(n)%saifile), exist=file_exists)
  if(.not.file_exists) then 
     write(LDT_logunit,*) "SAI map ",trim(LDT_laisai_struc(n)%saifile)," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
  endif

  ftn = LDT_getNextUnitNumber()
  open(ftn, file=trim(LDT_laisai_struc(n)%saifile), &
