...
contains
  ...
  subroutine SNIP_proc_ssmis(date10, ssmis_in, ssmis, option)
    ...
    do
       ...
       call read_bufr_attributes(filename, satid0, date0, lat0, lon0, &
            surflag0, tb19h0, tb19v0, tb37h0, tb37v0, tb22v0, &
            tb91v0, tb91h0)
    ...
    write (LDT_logunit,*) &
         '[INFO] calculate sea ice concentration from ssmis'
    ...
    do i=1,size(lat)
       ...
       if (ct(i) >=0 .or. snowdepth(i) >=0) then
          if (lat(i) > 0) then
             ...
             rlat = real(lat(i))
             rlon = real(lon(i))
             call latlon_to_ij(snodep_0p25deg_proj,rlat,rlon, &
                  icoord,jcoord)
             write(10, '(A10, I3, I6, I7, 2I5, 2I6)') date10_arr(i), &
                  satid(i), nint(real(lat(i))*100), &
                  nint(real(lon(i))*100), &
             ...
             call latlon_to_ij(snodep_0p25deg_proj,rlat,rlon,icoord, &
                  jcoord)
  ...
  subroutine calculate_sea_ice_concentration(n, surflag, lat, lon, &
       tb19h, tb19v, tb37h, tb37v, ct)
  ...
  subroutine checkgrid(lat_grid, lon_grid, lat, lon, plat, plon)
    ...
    do x=1, nc
       if ((.not. rlon < lon_grid(x)-0.125) .and. &
            (.not. rlon > lon_grid(x)+0.125)) then
    ...
    do y=1, nr
       if ((.not. rlat < lat_grid(y)-0.125) .and. &
            (.not. rlat > lat_grid(y)+0.125)) then
          plat=y
          exit
       end if
    end do ! do y
    ...
    do i=1, 3
       ...
       if (st_hr < 0 .and. hr >= 0) then
          ...
          do j=st_hr, 23
             ...
             file_path = trim(ssmis_in)//'tdrcr_f'//satid(i)//'_d' &
                  //date10_prev(1:8)//'_s'//st_hr_str// &
