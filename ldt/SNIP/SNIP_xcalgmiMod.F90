...
!  09 Jul 2025: Eric Kemp; Ported to SNIP.
!
! DESCRIPTION:
! Source code for the retrieval of snow depth and sea ice concentration
! from XCAL GMI brightness temperature observations
!--------------------------------------------------------------------------
...
contains
  ...
  subroutine SNIP_proc_xcalgmi(date10, gmi_in, gmi, option)
    ...
    real,              dimension(:), allocatable :: lat0, lon0, tb10h0, &
         tb10v0, tb19h0, tb19v0, &
         tb23v0, tb37h0, tb37v0, tb89h0, tb89v0
    real,              dimension(:), allocatable :: tb10h, tb10v, tb19h, &
         tb19v, &
    ...
    do
       ...
       call read_xcalgmi_attributes(filename, date0, lat0, lon0, &
            tb10h0, tb10v0, &
            tb19h0, tb19v0, tb23v0, tb37h0, tb37v0, tb89h0, tb89v0, &
            qcflag0)
    ...
    write (LDT_logunit,*) &
         '[INFO] calculate sea ice concentration from gmi'
    call calculate_sea_ice_concentration(nArr, lat, lon, tb19h, &
         tb19v, tb37h, tb37v, ct, qcflag)

    ! write file (kept the name of "ssmis" to minimize code changes in
    ! other parts)
    ...
    satid = 13
    !satid does not work for GMI, but need a space for satid in txt file
    !for further procecure in SNIP, which was origianlly developed for
    !SSMIS
    do i=1,size(lat)
  ...
  subroutine read_xcalgmi_attributes(filename, date10_arr, lat, lon, &
       tb10h, tb10v, &
    ...
    real,         allocatable, intent(inout) :: tb10h(:), tb10v(:), &
         tb19h(:), tb19v(:), &
    ...
    integer(hsize_t), dimension(1)           :: dims_yr, dims_mon, &
         dims_day, dims_hr
    integer(hsize_t), dimension(2)           :: maxdims_yr, &
         maxdims_mon, maxdims_day, maxdims_hr
    integer(hid_t)                           :: file_id, gr_id, grst_id
    integer(hid_t)                           :: dspace_lat_id, &
         dspace_lon_id
    integer(hid_t)                           :: dspace_tb_id
    integer(hid_t)                           :: dspace_yr_id, &
         dspace_mon_id
    integer(hid_t)                           :: dspace_day_id, &
         dspace_hr_id
    integer(hid_t)                           :: tb_field_id, lat_id, &
         lon_id
    integer(hid_t)                           :: yr_id, mon_id, day_id, &
         hr_id
    ...
    call LDT_verify(status, &
         'Error opening S1/ScanTime group in XCAL GMI file')
    ...
    call LDT_verify(status, &
         'Error opening Latitude field in XCAL GMI file')

    call h5dopen_f(gr_id,"Longitude",lon_id, status) !dimension: npix1 x nscan
    call LDT_verify(status, &
         'Error opening Longitude field in XCAL GMI file')
    ...
    call LDT_verify(status, &
         'Error opening DayOfMonth field in XCAL GMI file')
    ...
    call h5sget_simple_extent_dims_f(dspace_qc_id, dims_qc, maxdims_qc, &
         status)
    if(status.eq.-1) then
       call LDT_verify(status, &
            'Error in h5sget_simple_extent_dims_f: Quality')
    endif
    ...
    do i=1,n
       ...
       if (flag .eqv. .true.) then
          if (option==1) then
             ...
             ! Threshold are checked to ensure cold snow conditions are
             ! potentially present in the 36 GHz Tb

             if (tb37h(i) < 245 .and. tb37v(i) < 255) then
                ! snow is possible
                ! shallow or medium depth of snow is retrieved

                ! A minimum polarization difference of 3 is applied to
                ...
                if (tb10v(i)-tb37v(i)>0 .or. tb10h(i)-tb37h(i)>0) then
                   ...
                   !
                   ! snow presence is possible, but it is likely to be
                   ! shallow snow
                   if (tb89v(i)<255 .and. tb89h(i)<265 .and. tb23v(i)-tb89v(i)>0 .and. Tphy<267) then
                      ! "tb23h-tb89h>0" has not been used for GMI due to
                      ! the absence of tb23h snow depth is assumed to be
                      ! 5.0 cm
                      snowdepth(i) = 50 ! mm
                   else
                      snowdepth(i) = -0.1
  ...
  subroutine calculate_sea_ice_concentration(n, lat, lon, tb19h, &
    ...
    do i=1,n
       ...
       if (tb19v(i)<-100 .or. tb19h(i)<-100 .or. tb37v(i)<-100 .or. &
            tb37h(i)<-100) then  !check NaN value (-9999)
          flag=.false.
       end if
  ...
  subroutine write_netcdf(nc_filename, n, lat, lon, snowdepth, ct)
    ...
    real, dimension(1:nc,1:nr)                     :: gmidep, ndep, &
         gmicon, ncon
    integer                                        :: plat, plon
    integer                                        :: iret, ncid, &
    ...
    iret=nf90_create(path=trim(nc_filename),cmode=nf90_netcdf4, &
         ncid=ncid)
  ...
  subroutine checkgrid(lat_grid, lon_grid, lat, lon, plat, plon)
    ...
    do y=1, nr
       if ((lat >= lat_grid(y)-0.125 .and. &
            lat <= lat_grid(y)+0.125)) then
          plat=y
          exit
       end if
    end do ! do y
  ...
  subroutine search_files(date10, gmi_in)
    ...
    if (st_hr < 0 .and. hr >= 0) then
       ...
       do j=st_hr, 23
          ...
          file_path = trim(gmi_in)//'1C-R.GPM.GMI.XCAL2016-C.'// &
               date10_prev(1:8)//'-S'//st_hr_str// &
