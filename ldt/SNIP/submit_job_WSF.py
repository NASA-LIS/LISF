#!/usr/bin/env python3
"""
submit_job_WSF.py  –  WSF Pipeline Manager

Mode 1 (Submitter): Submit SNIP + LDT SLURM jobs for Discover or HPC11.
    Usage: python submit_job_WSF.py <YYYYMMDDHHMM> [--system discover|hpc11] [--dry-run]

Mode 2 (Worker): Run OPL Resampling (called automatically inside the SLURM job).
    Usage: python submit_job_WSF.py --resample <START_DT> <END_DT>
"""
...
TEMPLATE_PATH = Path("./job_template_WSF.sh")
...
# =============================================================================
# MODE 2: RESAMPLING LOGIC (WORKER)
# =============================================================================
...
def main() -> None:
    ...
    parser.add_argument("--system", choices=list(SYSTEMS), default=None, help="HPC system profile")
    parser.add_argument("--dry-run", action="store_true", help="Write the script but do not call sbatch")
    
    # Mode 2: Worker Arguments
    parser.add_argument("--resample", nargs=2, metavar=("START_DT", "END_DT"), help="Worker mode: Run OPL resampling")
    parser.add_argument("--config", default="./pipeline_config.json", help="Path to config (for resampling)")
    ...
    if not args.datetime:
        parser.error("You must provide a target datetime for job submission, or use --resample.")
