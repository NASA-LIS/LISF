...
!  09 Jul 2025: Eric Kemp; Updated for SNIP.
!
! DESCRIPTION:
...
contains
  ...
  subroutine read_amsr2_attributes(filename, date10_arr, lat, lon, &
       tb10h, tb10v, &
    ...
    !Arguments
    character(LDT_CONST_PATH_LEN), intent(in) :: filename
    ...
    real*8                                   :: scan_days
    integer(2),   allocatable                :: tb10h_raw(:,:), &
         tb10v_raw(:,:), &
         tb19h_raw(:,:), tb19v_raw(:,:), &
         tb23h_raw(:,:), tb23v_raw(:,:), &
         tb37h_raw(:,:), tb37v_raw(:,:), &
    ...
    integer(hid_t)                           :: tb10h_id, tb10v_id, &
         tb19h_id, tb19v_id, &
  ...
  subroutine calculate_snowdepth(n, tb10h, tb10v, tb19h, tb19v, &
       tb23h, tb23v, tb37h, tb37v, tb89h, tb89v, snowdepth, option, &
       lat, lon)
    ...
    ! Local variables
    integer                             :: i
    character(LDT_CONST_PATH_LEN)       :: ff_filename
    character(LDT_CONST_PATH_LEN)       :: fd_filename
          ...
          if (option==1) then
             ...
             if (tb37h(i) < 245 .and. tb37v(i) < 255) then
                ...
                if (tb10v(i)-tb37v(i)>0 .or. tb10h(i)-tb37h(i)>0) then
                   ...
                   if (tb89v(i)<255 .and. tb89h(i)<265 .and. &
                        tb23v(i)-tb89v(i)>0 .and. &
  ...
  subroutine read_forestfraction(ff_filename, ff)
    ...
    ! Arguments
    character (LDT_CONST_PATH_LEN), intent(in) :: ff_filename
  ...
  subroutine read_forestdensity(fd_filename, fd)
    ...
    ! Arguments
    character(LDT_CONST_PATH_LEN), intent(in) :: fd_filename
  ...
  subroutine search_files(date10, amsr2_in)
    ...
    character(LDT_CONST_PATH_LEN), intent(in) :: amsr2_in

    ! Local variables
    integer            :: n, j, k
    character(LDT_CONST_PATH_LEN) :: file_path, cmd
