...
def main() -> None:
    ...
    parser.add_argument("--resample", nargs='+', required=True, metavar=("START_DT", "END_DT"), 
                        help="Run OPL resampling (WSF only). Provide either two args (START_DT END_DT) or a single END_DT.")
    parser.add_argument("--config", default="./SNIP_ops/config/SNIP_config.json", help="Path to config")
    ...
    run_resampling(cfg, program, template, out_base, start_dt, end_dt, args.batch_size)
