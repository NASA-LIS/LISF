...
def main():
    ...
    parser.add_argument("--target-datetime",
                        ...
                        help="Target datetime in YYYYMMDDHHMM. "
                             "If provided, a temporary config will be "
                             "created from the given config file "
                             "with this datetime.")
    ...
    try:
        ...
        if args.target_datetime:
            ...
            tmp = tempfile.NamedTemporaryFile(
                mode='w', delete=False,
                dir=base.parent,
                prefix=f"tmp_snip_config_{args.target_datetime}_",
                suffix='.json',
                encoding='utf-8')
        ...
        if tmp_cfg_path:
            try:
                ...
                logging.warning("Could not remove temporary config %s",
                                tmp_cfg_path)
