...
!BOP
! 
! !ROUTINE: readLISlsmSMobs
! \label{readLISlsmSMobs}
!
! !INTERFACE: 
subroutine readLISlsmSMobs(n)
! !USES:
...
!EOP
  ...
  integer          :: igrib,nvars
  real             :: smvalue1d(lsmsmobs%nc*lsmsmobs%nr)
  real             :: smvalue2d(lsmsmobs%nc, lsmsmobs%nr)
  integer          :: c,r
  character*20     :: vname
  integer          :: varid
  ...
  if(file_exists) then 
     ...
     if(lsmsmobs%format.eq."binary") then 
...
#endif
     elseif(lsmsmobs%format.eq."grib1") then 
        if(lsmsmobs%wstyle.ne."WMO convention") then 
        ...
        do 
           ...
           call LDT_verify(iret, &
                'grib_get: indicatorOfParameter failed in readLISlsmSMObs')

           call grib_get(igrib, 'topLevel',topLev, iret)
           call LDT_verify(iret, &
                'grib_get: topLevel failed in readLISlsmSMObs')

           call grib_get(igrib, 'bottomLevel',botLev, iret)
           call LDT_verify(iret, &
                'grib_get: bottomLevel failed in readLISlsmSMObs')

           call grib_get(igrib, 'timeRangeIndicator',trange, iret)
           call LDT_verify(iret, &
                'grib_get: timeRangeIndicator failed in readLISlsmSMObs')

!right now specifically geared for AFWA outputs.            
           if(param_num.eq.201.and.topLev.eq.0.and.botlev.eq.10.and.&
                trange.eq.1) then 

              call grib_get(igrib,'values',smvalue1d,iret)
              call LDT_verify(iret,&
                   'grib_get: values failed in readLISlsmSMobs')
                           
              call transformDataToLDTgrid(n,smvalue1d,sm_data)
     ...
     elseif(lsmsmobs%format.eq."netcdf") then 
#if(defined USE_NETCDF3 || defined USE_NETCDF4) 
...
!  the time averaged data fields will be read in. 
        ...
        if(lsmsmobs%datares.eq.LDT_rc%gridDesc(n,10)) then 
           call LDT_readLISSingleNetcdfVar(n,ftn, vname,&
                1,lsmsmobs%nc, lsmsmobs%nr, smvalue2d)
        else
           call LDT_readLISSingleNetcdfVar(n,ftn, vname,&
                1,lsmsmobs%nc, lsmsmobs%nr, smvalue2d)
        endif
        ...
        do r=1,lsmsmobs%nr
           do c=1, lsmsmobs%nc
              smvalue1d(c+(r-1)*lsmsmobs%nc) = smvalue2d(c,r)
           enddo
        enddo
...
#endif
end subroutine readLISlsmSMobs


!BOP
!
! !ROUTINE: create_lsm_output_filename
! \label{create_lsm_output_filename}
!
! !INTERFACE:
subroutine create_lsm_output_filename(n, form, fname, odir, wstyle, wopt, &
                                      run_dd, map_proj, security_class,   &
...
!EOP
   ...
   if(wstyle.eq."4 level hierarchy") then 
      ...
      case default
         call ldt_log_msg('ERR: create_lsm_output_filename -- '// &
...
!BOP
! 
! !ROUTINE: transformDataToLDTgrid
! \label{trasnformDataToLDTgrid}
!
! !INTERFACE: 
 subroutine transformDataToLDTgrid(n, sm_inp, sm_out)
! !USES:    
   use LDT_coreMod
   use LISlsmSM_obsMod
...
! !ARGUMENTS: 
   integer         :: n 
   real            :: sm_inp(lsmsmobs%nc*lsmsmobs%nr)
...
!EOP
   integer         :: ios
   integer         :: c,r
   logical*1       :: sm_data_b(lsmsmobs%nc*lsmsmobs%nr)
   real            :: smobs_ip(LDT_rc%lnc(n)*LDT_rc%lnr(n))
   logical*1       :: smobs_b_ip(lsmsmobs%nc*lsmsmobs%nr)

   do r=1,lsmsmobs%nr
      do c=1, lsmsmobs%nc
         if(sm_inp(c+(r-1)*lsmsmobs%nc).ne.LDT_rc%udef) then 
            sm_data_b(c+(r-1)*lsmsmobs%nc) = .true. 
         else
            sm_data_b(c+(r-1)*lsmsmobs%nc) = .false.
         endif
         if(sm_inp(c+(r-1)*lsmsmobs%nc).gt.1) then 
            sm_inp(c+(r-1)*lsmsmobs%nc) = LDT_rc%udef
            sm_data_b(c+(r-1)*lsmsmobs%nc) = .false.
         endif
      enddo
   enddo
...
!-------------------------------------------------------------------------- 
      call bilinear_interp(LDT_rc%gridDesc(n,:),&
           ...
           lsmsmobs%w11, lsmsmobs%w12, &
           lsmsmobs%w21, lsmsmobs%w22, &
           lsmsmobs%n11, lsmsmobs%n12, &
           lsmsmobs%n21, lsmsmobs%n22, &
           LDT_rc%udef, ios)
      ...
      call neighbor_interp(LDT_rc%gridDesc(n,:),&
           ...
           LDT_rc%lnc(n)*LDT_rc%lnr(n), &
           LDT_domain(n)%lat, LDT_domain(n)%lon,&
           lsmsmobs%n11, LDT_rc%udef, ios)
   else
      call upscaleByAveraging(&
           lsmsmobs%nc*lsmsmobs%nr,&
           LDT_rc%lnc(n)*LDT_rc%lnr(n),LDT_rc%udef, &
           lsmsmobs%n11,sm_data_b, sm_inp, smobs_b_ip,smobs_ip)
 ...
 end subroutine transformDataToLDTgrid
